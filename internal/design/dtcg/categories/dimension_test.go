...
// TestValidateDimension_Positive: validation of valid dimension formats (positive cases).
// DTCG 2025.10 §8.2: accepts both the structured form {value, unit} and the legacy string form "16px".
func TestValidateDimension_Positive(t *testing.T) {
	...
	tests := []struct {
		...
		// Structured form (DTCG 2025.10 recommended).
		...
		// Legacy string form (backward compatibility).
...
// TestValidateDimension_Negative: validation of invalid dimension formats (negative cases).
func TestValidateDimension_Negative(t *testing.T) {
	...
	tests := []struct {
		...
		// Errors in the legacy string form.
