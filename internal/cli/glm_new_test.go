...
// --- Tests for createProjectEnvGLM (additional edge cases) ---

func TestCreateProjectEnvGLM_NoAPIKey(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	t.Setenv("USERPROFILE", tmpHome)
	t.Setenv("GLM_API_KEY", "")
	...
	if err == nil {
		t.Fatal("createProjectEnvGLM should error when no API key is available")
	}
	if !strings.Contains(err.Error(), "GLM API key not found") {
		t.Errorf("error should mention API key not found, got: %v", err)
	}
...
func TestCreateProjectEnvGLM_ShellExportFormat(t *testing.T) {
	tmpHome := t.TempDir()
	t.Setenv("HOME", tmpHome)
	t.Setenv("USERPROFILE", tmpHome)
	...
	if err := os.WriteFile(
		...
		[]byte("GLM_API_KEY=\"export-test-key\"\n"),
		0o600,
	); err != nil {
		t.Fatal(err)
	}
	...
	if err := createProjectEnvGLM(glmConfig, projectRoot); err != nil {
	...
	if !strings.Contains(contentStr, "Agent Teams") {
		t.Error("content should mention Agent Teams in header")
	}
}
