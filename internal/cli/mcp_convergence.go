...
func defaultBackendCaller(ctx context.Context, backend, target, focus, projectRoot string) ReviewOutput {
	...
	case BackendGLM:
		// GLM is an HTTP call to z.ai with no working-directory notion, so the
		// root has nothing to bind to there. It is dropped deliberately rather
		// than threaded and ignored.
