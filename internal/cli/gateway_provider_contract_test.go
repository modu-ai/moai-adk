...
func TestDefaultLaunchKeepsClaudeAndGLMOnLegacyAuthentication(t *testing.T) {
	oldRoot := findProjectRootFn
	t.Cleanup(func() { findProjectRootFn = oldRoot })
	sentinel := errors.New("legacy root resolution observed")
	findProjectRootFn = func() (string, error) { return "", sentinel }
	for _, mode := range []string{"claude", "glm"} {
		selected := false
		err := unifiedLaunchDefaultWithFactory("default", mode, nil, func(string) (*gatewayLaunchBinding, error) {
			selected = true
			return nil, errors.New("gateway must not start")
		})
		if selected || !errors.Is(err, sentinel) {
			t.Fatalf("mode=%s gateway=%v err=%v", mode, selected, err)
		}
	}
}
...
func TestGatewayExactModelContextAndGPTToolExposure(t *testing.T) {
	...
	for _, mode := range []string{"claude", "gpt", "glm"} {
		...
		if mode == "glm" && values["CLAUDE_CODE_DISABLE_1M_CONTEXT"] != "1" {
			t.Errorf("%s permits implicit unregistered [1m] ID", mode)
		}
		if mode != "glm" && values["CLAUDE_CODE_DISABLE_1M_CONTEXT"] != "" {
