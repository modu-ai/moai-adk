    ...
    if command -v go >/dev/null 2>&1; then
        ...
        for _mr in $MODROOTS; do
            ...
            if ! ( cd "$_mr" && go vet $BT_TAGS $PKGS >/dev/null 2>&1 ); then
                printf '\n[pre-commit] FAILED: go vet reported issues in the packages holding your staged files (module root: %s; vetted in the working tree, not the index).\n' "$_mr" >&2
