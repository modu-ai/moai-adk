...
if [ -n "$STAGED_GO" ]; then
    # gofmt format check on the STAGED bytes. Skipped when gofmt is not on
    # PATH (non-Go environment).
    #
    # Why the index and not the file on disk: 'gofmt -l "$f"' reads the working
    # tree, so a file staged unformatted and then fixed in the tree was let
    ...
    if command -v gofmt >/dev/null 2>&1; then
        ...
        FMT_ERR="$(printf '%s\n' "$FMT_SCAN" | sed -n 's/^err //p')"
        NEED_FMT="$(printf '%s\n' "$FMT_SCAN" | sed -n 's/^fmt //p')"

        # A non-zero gofmt exit is a tool error, never a pass. Deciding on an
        # empty stdout alone would let a gofmt that could not run at all --
        # /dev/stdin missing, input unreadable, staged bytes that do not parse
        # -- report a clean gate, and a gate that passes silently is worse
        # than no gate: nobody trusts an absent one, everybody trusts a green.
        if [ -n "$FMT_ERR" ]; then
            printf '\n[pre-commit] FAILED: gofmt could not check the staged bytes of:\n%s\n' "$FMT_ERR" >&2
            printf '[pre-commit] Cause: the staged content does not parse, or gofmt could not read it.\n' >&2
            printf '[pre-commit] Override: SKIP_MOAI_PRECOMMIT=1 git commit\n' >&2
            exit 1
        fi
        if [ -n "$NEED_FMT" ]; then
            printf '\n[pre-commit] FAILED: the following staged files need formatting (checked against the staged bytes, not the working tree):\n%s\n' "$NEED_FMT" >&2
...
'
        for _mr in $MODROOTS; do
            ...
            [ -n "$PKGS" ] || continue
            # vet's diagnostics reach the terminal (stdout folded onto stderr,
            # which is where vet already writes): a gate that blocks without
            # saying why leaves the user one move -- turn the gate off. go vet
            # prints nothing when it finds nothing, so a passing commit stays
            # silent.
            # shellcheck disable=SC2086
            if ! ( cd "$_mr" && go vet $BT_TAGS $PKGS >&2 ); then
                printf '\n[pre-commit] FAILED: go vet reported the issues above in the packages holding your staged files (module root: %s; vetted in the working tree, not the index).\n' "$_mr" >&2
