...
var profileSetupTexts = map[string]profileSetupText{
	"en": {
		...
		EffortLevelDesc:       "Reasoning depth for the Claude session launched with this profile. xhigh/max need a model that supports them (Opus 5.5, Sonnet 5, Opus 4.7+). Per-agent effort comes from the agent model policy instead.",
		EffortLevelLow:        "low - fastest, least thorough",
		EffortLevelMedium:     "medium - balanced (Recommended)",
		...
		PermAuto:              "Auto mode (auto) - Classifier-gated approvals. REQUIRES Max/Team/Enterprise/API plan + Sonnet 5+. Session errors at runtime if unsupported.",
	...
	"ko": {
		...
		ModelOpus1M:           "opus[1m] (Opus 5.5 + 1M 컨텍스트, 권장)",
		...
		EffortLevelMedium:     "medium - 균형 (권장)",
		...
		PermAuto:              "자동 모드 (auto) - 분류기 기반 자동 승인. Max/Team/Enterprise/API 플랜 + Sonnet 5+ 필수. 미지원 환경에서는 런타임 오류 발생.",
	...
	"ja": {
		...
		ModelOpus1M:           "opus[1m] (Opus 5.5 + 1Mコンテキスト、推奨)",
		...
		EffortLevelMedium:     "medium - バランス (推奨)",
		...
		PermAuto:              "オートモード (auto) - 分類器による自動承認。Max/Team/Enterprise/APIプラン + Sonnet 5+ 必須。未対応環境では実行時エラーが発生します。",
	...
	"zh": {
		...
		ModelOpus1M:           "opus[1m] (Opus 5.5 + 1M 上下文，推荐)",
		...
		EffortLevelMedium:     "medium - 均衡 (推荐)",
		...
		PermAuto:              "自动模式 (auto) - 分类器把关自动批准。需要 Max/Team/Enterprise/API 计划 + Sonnet 5+。不支持时会产生运行时错误。",
