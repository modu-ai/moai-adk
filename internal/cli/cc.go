...
var ccCmd = &cobra.Command{
	Use:   "cc [-p profile] [-k [SPEC-ID] | -k --name <role> | -f | -f agent | -f agent-<n>] [-- claude-args...]",
...
Factory Mode (dedicated -f entry):
  -f, --factory                Enter as the LEAD of a factory run. The
                                ...
                                agents join one at a time via the agent
                                role token or the incremental agent form
                                below. The lead routes operator-picked cards
                                to free agents over cross-session messages —
                                each card goes WHOLE to one agent, which
...
Examples:
  ...
  moai cc -f                           # Factory lead: one agent (agent-1)
  moai cc -f agent                     # Join the running factory as the next free agent
  moai cc -f agent-2                   # Add agent 2 to the running factory
  moai glm -f agent-3                  # Same agent on the GLM backend`,
	GroupID:            "launch",
...
func runClaudeEntry(cmd *cobra.Command, args []string, commandName, mode, backend string, launch func(string, string, []string) error) error {
	...
	// dedicated -f surface (bare -f / -f agent / -f agent-<n>). Parsed after
