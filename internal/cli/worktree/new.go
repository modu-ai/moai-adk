...
origin/main is safer because it always reflects the latest merged state.`,
	...
	// The former --team flag is retired. Launching a session inside a worktree
	// is now the launchers' job: `moai cc -w <name>` enters one in place, and
	// `moai cc -w <name> --spawn` opens it in a new tmux window (see
