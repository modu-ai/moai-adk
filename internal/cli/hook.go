...
fail-open).`,
	...
	// REQ-MCP-008). Stop-hook gate that runs the codex JSON-RPC reviewer on
	// uncommitted changes and blocks on a FAIL verdict. Opt-in default-off via
	// workflow.codex.review_gate.enabled (the manifest timeout override is M4).
	hookCmd.AddCommand(&cobra.Command{
		Use:          "codex-review-gate",
		Short:        "Stop-hook codex review gate (opt-in; blocks on codex FAIL verdict)",
		Long:         `Run the codex JSON-RPC reviewer against uncommitted changes and emit the standard ALLOW/BLOCK Stop-hook output. Opt-in via workflow.codex.review_gate.enabled (default off). Fail-open: any error logs to stderr and exits 0. SPEC-MOAI-MCP-SERVER-001 REQ-MCP-008 / AC-MCP-009 / AC-MCP-010.`,
		SilenceUsage: true,
