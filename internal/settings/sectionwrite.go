...
// effort, cache.yaml spec_ttl/min_cacheable_tokens 등)가 파괴된다 (AP-1 — 첫
// 쓰기에서 파일 손상; REQ-WC13-005/006). 영속화는 반드시
...
// statusline, 제외군(REQ-WC11-018; cache는 REQ-WC13-001로 제외군 이탈) 및
// 미지명 섹션(db, research 포함 — 콘솔 표면에서 제거됨)은 전부 오류로
// 거부하며 파일을 건드리지 않는다.
