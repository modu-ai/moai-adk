...
// 이 파일은 Save() 경로가 없는 6개 섹션(workflow, harness, ralph,
// feedback, observability, security)의 seam 쓰기 라우팅을 담는다
// (SPEC-WEB-CONSOLE-011 M2a, REQ-WC11-017/018 — design.md §A.3). db 섹션은
// 콘솔 표면에서 제거되어 (settings SSOT) 더 이상 seam-writable이 아니며,
// research 섹션은 SPEC-WEB-CONSOLE-012 REQ-WC12-010에서 폐선되었다.
//
// @MX:WARN: [AUTO] WriteSectionViaSeam은 프로필 스토어가 아닌 *프로젝트 설정*
// (.moai/config/sections/<section>.yaml)을 디스크에 쓰는 세 번째 영속화 경계다.
// typed Save() 경로가 존재하지 않는 6개 섹션의 유일한 쓰기 경로이며, 웹/TUI
// 양쪽이 공유한다.
// @MX:REASON: [AUTO] 이 6개 섹션에 typed struct 재직렬화(ConfigManager.Save 계열)를
...
// edits를 yamlpatch seam으로 기록한다. RouteSeam으로 라우팅되는 6개 섹션만
// 허용한다 — typed 섹션(user/language/quality/git-convention/git-strategy/llm),
// statusline, 제외군(REQ-WC11-018) 및 미지명 섹션(db, research 포함 — 콘솔
// 표면에서 제거됨)은 전부 오류로 거부하며 파일을 건드리지 않는다.
