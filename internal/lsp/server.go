...
// @MX:ANCHOR: [AUTO] ServerManager is the core interface managing concurrent lifecycle of multi-language servers. All methods are designed to be thread-safe.
// @MX:REASON: fan_in=10+, entry point for LSP server management, called from multiple locations
...
// @MX:WARN: [AUTO] Uses goroutine pool to start servers in parallel. Semaphore is limited by maxParallel.
// @MX:REASON: [AUTO] Goroutines created proportional to langs length, potential resource overhead
// @MX:NOTE: [AUTO] Individual server failure is non-fatal (graceful degradation).
// StartAll starts servers for all given languages concurrently,
...
// @MX:WARN: [AUTO] Uses goroutines to collect diagnostics from all active servers in parallel.
// @MX:REASON: [AUTO] Goroutines created proportional to server count, potential resource overhead
// CollectAllDiagnostics collects diagnostics from all active servers concurrently.
...
// @MX:WARN: [AUTO] Goroutine blocks on process termination. Does not respond to context cancellation.
// @MX:REASON: [AUTO] Goroutine may block permanently at process.Wait()
