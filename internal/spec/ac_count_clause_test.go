...
func acEmitBaseline(t *testing.T, root, counter string, matches []string, w io.Writer) {
	...
	fmt.Fprintf(w, "# AC-count corpus baseline — depth-1 glob .moai/specs/*/acceptance.md (_archive excluded)\n")
	fmt.Fprintf(w, "# regenerated by %s on %s from source tree %s; every line is a measurement.\n",
		acRegenerateCommand, time.Now().Format("2006-01-02"), acTreeSHA(t, root))
	fmt.Fprintf(w, "# lifecycle cascade (same-commit rule): .moai/docs/ac-count-baseline-refresh.md\n")
	...
	for _, rel := range rels {
		...
		if code != 0 {
			...
			ids := strings.Fields(strings.TrimPrefix(strings.SplitN(stdout, "\n", 2)[0], "AMBIGUOUS"))
			sort.Strings(ids)
			fmt.Fprintf(w, "%s  HALT %s  owner=%s reason=ambiguous-partial-marking\n",
		...
		excluded, _ := strconv.Atoi(m[2])
		fmt.Fprintf(w, "%s  COUNT %d  live=%d excluded=%d ambiguous=0\n", rel, live, live, excluded)
	}
}
