...
// discriminator B — verbatim, byte-frozen. The in-section scan shape mirrors the
// t528/t747 plan probe (break on any "##"-prefixed line) so the numbers stay
// comparable with 1240 in-section; extractACLines' anchor-level break is a known
// second-order difference, recorded in the SPEC and not resolved here.
package spec
...
// using the probe-region break rule (any "##"-prefixed line) for comparability
// with the plan-phase frozen numbers. Each entry is "lineNo:trimmed text".
func t747InSectionDecls(lines []string, start int) []string {
...
func TestT747AnchorScope(t *testing.T) {
	...
	err := filepath.Walk(root, func(p string, info os.FileInfo, err error) error {
		...
		fr.baseIn = t747InSectionDecls(lines, fr.baseStart)
		fr.liveIn = t747InSectionDecls(lines, fr.liveStart)
	...
	beforeInTotal, afterInTotal := 0, 0
	declAnywhereTotal := 0
	var fileList []string
	...
	// Per-file comparison instrument for AC-747-003: the in-section declaration
	// set (line:text entries) must be identical across the two columns.
	controlTotal := 0
	var controlDeltas []string
	for _, fr := range results {
		...
		if strings.Join(fr.baseIn, "\n") != strings.Join(fr.liveIn, "\n") {
			controlDeltas = append(controlDeltas,
				fmt.Sprintf("%s\tbaseline=%d entries\tlive=%d entries", fr.path, len(fr.baseIn), len(fr.liveIn)))
		}
	}

	// Prose bound (AC-747-004): every newly in-section declaration must sit in a
	// defect-set file. Newly-included entries in control files would already be
	// control deltas; this list attributes each new inclusion to its file class.
	var newlyIncluded []string
	for _, fr := range results {
		baseSet := map[string]bool{}
		for _, e := range fr.baseIn {
			baseSet[e] = true
		}
	...
	// repaired = the live anchor's region holds >=1 declaration line; otherwise
	// the classification code names what the file's declarations actually are.
	...
	for _, fr := range results {
		...
		dispositions = append(dispositions, fmt.Sprintf("%s\t%s\tbase=%s\tlive=%s\tbaseIn=%d\tliveIn=%d\t%s",
	...
	t.Logf("declarations IN-SECTION: before=%d after=%d", beforeInTotal, afterInTotal)
	t.Logf("CONTROL set (decl-bearing, non-defect) = %d, deltas = %d (list: %s/control-delta.txt)", controlTotal, len(controlDeltas), out)
