...
- **`git log --follow` for transition detection**: When verifying `status:` transitions against git history (e.g., `OwnershipTransitionRule` checking that `manager-develop` authored the `in-progress` transition), use `git log --follow --format='%an|%ae|%s' <file>` and parse author + commit-subject pattern. Git environment checks (`git rev-parse --git-dir`, `git rev-parse --verify main`) are cached per-`Lint()` run via `gitquery_cache.go` (`startGitQueryCache`/`stopGitQueryCache` in `Lint()` entry/exit, keyed by query-kind per REQ-PERF-001-A §H R1). Per-run invalidation (REQ-PERF-001-B) discards the cache at `Lint()` exit. Direct callers outside `Lint()` (e.g., `DetectDrift`) bypass the cache and check directly (preserving original behavior). UnreachableGit branch (sandbox without git, e.g., `t.TempDir()` test fixture) MUST gracefully no-op without raising error severity.
