...
func TestDeriveTitleFromPrompt(t *testing.T) {
	...
	tests := []struct {
		...
		// Claude Code wraps a slash-command turn in <local-command-caveat> +
		// <command-name>/x</command-name> tags. That wrapper is a command
		// artifact, NOT a user prompt, so it must never become a session title.
		// (Regression: /clear-started sessions were titled after the caveat
		// text — "Caveat: The messages below are genera…".)
		{
			"local-command-caveat wrapper only -> empty",
			"<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>\n<command-name>/clear</command-name>\n<command-message>clear</command-message>\n<command-args></command-args>\n<local-command-stdout></local-command-stdout>",
			"",
		},
		{
			"caveat wrapper + real prompt survives -> real prompt",
...
func TestBuildSessionTitle_NoSPEC_TranscriptPolicy(t *testing.T) {
	...
	const (
		...
		// Claude Code records a slash-command turn as a user message whose
		// content is wrapped in <local-command-caveat> + <command-name> tags.
		// That record is a command artifact, not a prompt — the title policy
		// must skip it and use the next real user prompt (or yield empty).
		userClearCmd = `{"type":"user","message":{"role":"user","content":"<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>\n<command-name>/clear</command-name>\n<command-message>clear</command-message>"}}`
	)
	...
	tests := []struct {
		...
		// Regression: a session started with a slash command (e.g. /clear) has
		// the command artifact as its first user record. That artifact must NOT
		// become the title; the policy skips it and uses the next real prompt.
		{
			name:  "slash-command artifact first, real prompt after -> real prompt title",
			lines: []string{userClearCmd, assistantRec, userDashboard},
			want:  "대시보드 성능 개선",
		},
		{
			name:  "slash-command artifact only, no real prompt -> empty",
			lines: []string{userClearCmd},
			want:  "",
		},
	}
