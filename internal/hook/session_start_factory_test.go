...
// must name that form alongside the numbered `-f agent-<n>` form, and the lane
...
// `-f agent-<i>` form, the entry-point guidance naming `moai glm -f <N>` and
// the `-f agent-<n>` form, the per-lane fan-out line, the leader socket
...
func TestFactoryLeadNoticeCarriesLaneLinesSocketAndEntryGuide(t *testing.T) {
	...
	// `moai cc -f agent-<n>` mention is not a launch line, so the count
	// matches digits only.
...
// factory lead notice (`moai cc -f agent-<i>`, i a number) — the entry
// guide's `agent-<n>` placeholder is excluded by the digit class.
var factoryLaunchLineRe = regexp.MustCompile(`moai cc -f agent-[0-9]+`)
...
// while the call passed the label first, rendering %!d(string=agent-4) —
// a Contains("agent-4") assertion passed right through that garbage, so the
// whole sentence is asserted here.
func TestFactoryWorkerNoticeNamesLabel(t *testing.T) {
	clearKanbanEnv(t)

	t.Setenv(config.EnvMoaiFactoryWorker, "agent-4")
	t.Setenv(config.EnvMoaiFactoryWorkers, "3")
	...
	if !strings.HasPrefix(got, "Factory Mode: joined a 3-lane run as agent-4.") {
	...
	// The incremental `-f agent-<n>` form carries no count (workers=0); the
	...
	if got := factoryBootstrapNotice("", langEnglish); !strings.HasPrefix(got,
		"Factory Mode: joined the factory run as agent-4.") {
	...
	t.Setenv(config.EnvMoaiFactoryWorker, "not-a-agent-label")
	if got := factoryBootstrapNotice("", langEnglish); got != "" {
...
func TestFactoryWorkerNoticeLocaleWordOrders(t *testing.T) {
	...
	for _, lang := range []string{"en", "ko", "ja", "zh"} {
		got := factoryWorkerNotice("agent-2", 5, lang)
		if !strings.Contains(got, "agent-2") || !strings.Contains(got, "5") ||
			strings.Contains(got, "%!") {
			t.Errorf("locale %q worker join rendered wrong: %q", lang, got)
		}
		gotNoCount := factoryWorkerNotice("agent-2", 0, lang)
		if !strings.Contains(gotNoCount, "agent-2") || strings.Contains(gotNoCount, "%!") {
...
func TestFactoryLeadNoticeCarriesDispatchDiscipline(t *testing.T) {
	...
	root := t.TempDir()
	// agent-2 claimed by THIS test process — a pid that is genuinely alive,
	...
	if err := kanban.SaveFactoryRegistry(kanban.FactoryRegistryPath(root), map[string]kanban.FactoryWorkerEntry{
		"agent-2": kanban.NewFactoryWorkerEntry(),
	}); err != nil {
	...
	for _, want := range []string{
		...
		"Free lane slots right now: agent-1, agent-3.",
	} {
		if !strings.Contains(notice, want) {
