...
var kanbanLocales = map[string]kanbanMessages{
	langEnglish: {
		...
		nameChoices:    "Name options: the default Agent worker / `judge` (Claude verdicts — the GLM foreman's only Claude path) / `agent-N` (numbered factory workers).",
	...
	"ko": {
		...
		nameChoices:    "이름 선택지: 기본 Agent 작업자 / `judge` (Claude 판정 — GLM 공장장이 Claude 를 쓰는 유일한 경로) / `agent-N` (번호가 붙은 팩토리 작업자).",
	...
	"ja": {
		...
		nameChoices:    "名前の選択肢: デフォルトの Agent 作業者 / `judge`（Claude による判定 — GLM 親方が Claude を使う唯一の経路） / `agent-N`（番号付きファクトリー作業者）。",
	...
	"zh": {
		...
		nameChoices:    "命名选项：默认 Agent 作业者 / `judge`（Claude 判定 —— GLM 工头使用 Claude 的唯一途径） / `agent-N`（编号的工厂作业者）。",
