...
var factoryLocales = map[string]factoryMessages{
	langEnglish: {
		...
		leadManual: "This session dispatches cards to %d lanes over cross-session messages.\n" +
			...
			"Lanes are named lane-1..lane-%d, and a lane that joins with `-f agent` is named agent-<n>; a number whose label is held by a live session is bumped to the next free number.",
	...
	"ko": {
		...
		leadManual: "이 세션이 세션 간 메시지로 카드를 레인 %d개에 배분합니다.\n" +
			...
			"레인 이름은 lane-1..lane-%d 이고, `-f agent` 로 합류한 레인은 agent-<n> 이라는 이름을 받습니다. 생존 세션이 이미 쓰고 있는 번호는 다음 빈 번호로 늘어납니다.",
	...
	"ja": {
		...
		leadManual: "このセッションが、セッション間メッセージでカードをレーン %d 本に割り振ります。\n" +
			...
			"レーン名は lane-1..lane-%d で、`-f agent` で参加したレーンは agent-<n> という名前になります。生存セッションが保持する番号は次の空き番号へ繰り上がります。",
	...
	"zh": {
		...
		leadManual: "本会话通过跨会话消息把卡片分发给 %d 条泳道。\n" +
			...
			"泳道命名为 lane-1..lane-%d，通过 `-f agent` 加入的泳道命名为 agent-<n>；已被存活会话占用的编号会顺延到下一个空位。",
