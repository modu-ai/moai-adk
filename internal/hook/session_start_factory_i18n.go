...
// directive citation — verbatim in every locale, following the glmSubstitute
...
var factoryLocales = map[string]factoryMessages{
	langEnglish: {
		...
		glmSubstitute: "Substitute 'moai glm -k %d --name ...' for 'moai cc -k %d --name ...' on any worker to run it on the GLM backend.",
	...
	"ko": {
		...
		glmSubstitute: "워커를 GLM 백엔드로 돌리려면 'moai cc -k %d --name ...' 대신 'moai glm -k %d --name ...' 을 사용하세요.",
	...
	"ja": {
		...
		glmSubstitute: "ワーカーを GLM バックエンドで動かす場合は、'moai cc -k %d --name ...' の代わりに 'moai glm -k %d --name ...' を使用してください。",
	...
	"zh": {
		...
		glmSubstitute: "如需让某个工作会话运行在 GLM 后端，请将 'moai cc -k %d --name ...' 换成 'moai glm -k %d --name ...'。",
