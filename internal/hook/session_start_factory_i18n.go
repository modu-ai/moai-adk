...
var factoryLocales = map[string]factoryMessages{
	langEnglish: {
		...
		leadManual: "This session dispatches cards to %d lanes over cross-session messages.\n" +
			...
			"Lanes are named worker-1..worker-%d, and a lane that joins with `-f worker` takes the next free worker-<n>; a number whose label is held by a live session is bumped to the next free number.",
		entryGuide: "Entry points: `moai cc -f` starts a Claude factory lead and `moai glm -f` a GLM lead — " +
			...
			"to add a lane using the same provider as this lead, run `moai %[2]s -f worker`, which takes the next free worker-<n>, " +
			"or pin a number with `moai %[2]s -f worker-<n>`.",
	...
	"ko": {
		...
		leadManual: "이 세션이 세션 간 메시지로 카드를 레인 %d개에 배분합니다.\n" +
			...
			"레인 이름은 worker-1..worker-%d 이고, `-f worker` 로 합류한 레인은 비어 있는 다음 worker-<n> 이름을 받습니다. 생존 세션이 이미 쓰고 있는 번호는 다음 빈 번호로 늘어납니다.",
		entryGuide: "진입점: `moai cc -f`는 Claude, `moai glm -f`는 GLM을 사용하는 팩토리 리더를 시작합니다. " +
			...
			"현재 리더와 같은 공급자로 레인을 추가할 때는 `moai %[2]s -f worker`를 쓰면 비어 있는 다음 worker-<n> 번호를 알아서 잡습니다. " +
			"번호를 직접 정하려면 `moai %[2]s -f worker-<n>`을 쓰세요.",
	...
	"ja": {
		...
		leadManual: "このセッションが、セッション間メッセージでカードをレーン %d 本に割り振ります。\n" +
			...
			"レーン名は worker-1..worker-%d で、`-f worker` で参加したレーンには空いている次の worker-<n> が割り当てられます。生存セッションが保持する番号は次の空き番号へ繰り上がります。",
		entryGuide: "起動コマンド: `moai cc -f` は Claude、`moai glm -f` は GLM のファクトリーリーダーを起動します。" +
			...
			"現在のリーダーと同じプロバイダーでレーンを追加するには `moai %[2]s -f worker` を使ってください。空いている次の worker-<n> が自動で割り当てられます。" +
			"番号を指定したい場合は `moai %[2]s -f worker-<n>` を使います。",
	...
	"zh": {
		...
		leadManual: "本会话通过跨会话消息把卡片分发给 %d 条泳道。\n" +
			...
			"泳道命名为 worker-1..worker-%d，通过 `-f worker` 加入的泳道会占用下一个空闲的 worker-<n>；已被存活会话占用的编号会顺延到下一个空位。",
		entryGuide: "启动命令：`moai cc -f` 使用 Claude，`moai glm -f` 使用 GLM，分别启动工厂主导会话。" +
			"启动器决定后端，`-f` 决定工厂。`-f` 不带数量时以一条泳道的默认配置启动；" +
			"如需使用与当前主导会话相同的提供商添加泳道，请运行 `moai %[2]s -f worker`，它会自动占用下一个空闲的 worker-<n>；" +
			"如需指定编号，请运行 `moai %[2]s -f worker-<n>`。",
