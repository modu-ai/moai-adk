...
var factoryLocales = map[string]factoryMessages{
	langEnglish: {
		...
		entryGuide: "Entry points: `moai cc -f %[1]d` starts a Claude factory lead, `moai glm -f %[1]d` a GLM lead, and `moai gpt -f %[1]d` a GPT lead — " +
	...
	"ko": {
		...
		entryGuide: "진입점: `moai cc -f %[1]d`는 Claude, `moai glm -f %[1]d`는 GLM, `moai gpt -f %[1]d`는 GPT를 사용하는 팩토리 리더를 시작합니다. " +
	...
	"ja": {
		...
		entryGuide: "起動コマンド: `moai cc -f %[1]d` は Claude、`moai glm -f %[1]d` は GLM、`moai gpt -f %[1]d` は GPT のファクトリーリーダーを起動します。" +
	...
	"zh": {
		...
		entryGuide: "启动命令：`moai cc -f %[1]d` 使用 Claude，`moai glm -f %[1]d` 使用 GLM，`moai gpt -f %[1]d` 使用 GPT，分别启动工厂主导会话。" +
