...
- **YAML frontmatter format**: `tools:`/`allowed-tools:` are CSV strings (space-separated), `skills:` is a YAML array — this is the only documented exception (CLAUDE.local.md §12). `metadata.*` fields are quoted strings. The loader uses `gopkg.in/yaml.v3` strict mode — unknown keys fail loud.
