...
// PermissionsRegionHasDirective reports whether the "permissions": { ... }
// region of a settings-file body contains a Go-template directive (`{{` / `}}`).
//
// A permissions block that carries a template directive is intentionally
// render-time-conditional — for example the git_mode gating in
// settings.json.tmpl, which excludes AI git-write specifiers (git commit /
// push / tag) unless the project's git_mode permits them (F1/F2 security
// hardening). Such a block CANNOT be regenerated by this codegen: the codegen
// emits a flat, directive-free JSON block (AssertPermissionBlockNoTemplate),
// and reading the existing block back requires valid JSON (extractPermissions
// → json.Unmarshal), which a `{{if}}` directive would break. The
// tool-policy build command uses this predicate to SKIP such a target rather
// than corrupt the conditional or error on the non-JSON block.
//
// The permissions region is located via the same string-literal-aware brace
// matcher used by the codegen; balanced `{{ }}` directives inside the block do
// not fool the region boundaries.
func PermissionsRegionHasDirective(body []byte) (bool, error) {
	region, err := locatePermissionsRegion(body)
	if err != nil {
		return false, err
	}
	seg := body[region.start:region.end]
	return bytes.Contains(seg, []byte("{{")) || bytes.Contains(seg, []byte("}}")), nil
}
