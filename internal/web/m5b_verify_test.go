...
func TestM5bD1_AtomicSave_AllPanelsInDOM(t *testing.T) {
	...
	// 1. All 7 tabpanels present in DOM.
	panelCount := strings.Count(body, `class="tabpanel`)
	if panelCount < 7 {
		t.Errorf("expected >= 7 tabpanel divs in DOM (7-tab console — workflow restored Issue 3, atomic Save contract), found %d", panelCount)
	}

	// 2. Tab nav present with data-tab attributes (7 tabs including workflow).
	tabCount := strings.Count(body, `data-tab=`)
	if tabCount < 7 {
		t.Errorf("expected >= 7 data-tab buttons (7-tab console — workflow restored Issue 3), found %d", tabCount)
	}
