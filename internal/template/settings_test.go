...
func TestSettingsTemplateDefaultModelLever(t *testing.T) {
	...
	// was removed so new Claude models (e.g. fable) are not locked out of the
	// /model picker and GLM swaps are admitted without an allowlist.
	if _, ok := settings["enforceAvailableModels"]; ok {
		t.Errorf(`enforceAvailableModels present, want absent (enforcement removed so new models are not locked out)`)
	}

	// availableModels must be ABSENT — a hand-maintained closed allowlist is the
	// mechanism that hid unlisted models; removing it lets every model surface.
	if _, ok := settings["availableModels"]; ok {
		t.Errorf(`availableModels present, want absent (closed allowlist removed so new models auto-appear)`)
	}
}
