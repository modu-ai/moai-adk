...
- Mode 4 fan-out bounds: hard bound is the runtime subagent cap (`CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`, default 20 per turn); MoAI's 3-5 is a cache/coordination ADVISORY for fan-out, distinct from the 3-5 TEAM-SIZE advisory that binds Mode 3 teammates only. See orchestration-mode-selection.md §C.2 (SSOT).
