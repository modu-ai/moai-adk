...
The one tracked citation target is the **verdict file** — in this repository, `.moai/reports/<card-id>/verdict.md`, the same file `kanban-dispatch.md` already fixes as a card's verdict. The directory around it is **not** tracked: the ignore rules re-include that single filename and leave the rest of the card directory ignored, so no other artifact there may be described as tracked, and writing one does not make it reachable off this machine. The obligation is therefore **carry the deciding evidence into the verdict**: before a claim is made, the command that decided it and the lines of output that decided it are written into the verdict file, and the claim cites that file. Scratch is where a command's raw output lands; the verdict file is what a document is allowed to point at.
