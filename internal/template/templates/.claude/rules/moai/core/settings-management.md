...
> Sequential Thinking MCP was retired in an earlier deep-reasoning consolidation. Use the `ultrathink` keyword (Adaptive Thinking on Opus 4.7+) for deep reasoning.
...
Use the `ultrathink` keyword in user prompts to activate Adaptive Thinking (Opus 4.7+). This is the canonical deep-reasoning path; Sequential Thinking MCP was retired in an earlier consolidation.
