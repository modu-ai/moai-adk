...
| **`.claude/agents/local/`** | **maintainer-only hand-authored agents** (e.g., release-update-specialist, github-specialist; entry via `97-*`/`98-*` thin command wrappers per dev-only-commands-isolation.md) | user project (maintainer authors) | **NEVER delete/modify + preserve backup** |
...
Fork subagents are **enabled by default** as of Claude Code v2.1.232 (the earlier opt-in `CLAUDE_CODE_FORK_SUBAGENT` era ended there): a subagent spawned with `subagent_type: "fork"` inherits the full conversation and prompt cache, which makes it useful for branching exploratory work off the current state at reduced context cost. The in-session fork is invoked with `/subtask`; `/fork` copies the conversation into a new background session (its own row in `claude agents`) while you keep working — the in-session subagent it used to launch was renamed `/subtask` (changelog places the rename at v2.1.213 while the official doc says "before v2.1.212"; 2.1.212 was never published — the upstream inconsistency is recorded here, not resolved). A forked subagent cannot itself fork, but it can spawn other subagent types. Nesting depth is configurable via `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` and is enabled by default (depth 3) on v2.1.219+ (`=1` disables); on v2.1.232+ non-teammate agent spawns in interactive sessions also run in the background by default. MoAI's flat hierarchy holds by configuration — the retained agents omit the `Agent` tool from their `tools` list, so they do not spawn nested or forked subagents regardless of the runtime default.
...
START
  ...
  │   │         (subject to the 12-agent retention ceiling per the
...
- **Adding a new MoAI-custom agent without a SPEC** — the 12-agent retention ceiling is an architectural invariant; new agent additions must justify the "keep spawning the same worker" criterion via a SPEC that documents recurrence evidence
