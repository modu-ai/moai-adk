...
- MoAI-ADK system agents: `.claude/agents/{core,expert,meta}/<agent-name>.md` (template-distributed)
- **User-generated harness agents**: `.claude/agents/harness/<agent-name>.md` — created by `moai-meta-harness` skill during `/moai project` Phase 5+ interview. **NOT template-distributed**, **NOT touched by `moai update`**.

[ZONE:Frozen] [HARD] Agent Directory Convention (Namespace Separation):

| Path | 범위 | Source of Truth | `moai update` 동작 |
|------|------|-----------------|---------------------|
| `.claude/agents/core/` | manager-* (범용 manager agents) | template | 삭제 후 신규 설치 (overwrite) |
| `.claude/agents/expert/` | expert-* (범용 domain experts) | template | 삭제 후 신규 설치 (overwrite) |
| `.claude/agents/meta/` | plan-auditor, evaluator-active, claude-code-guide 등 (범용 meta agents) | template | 삭제 후 신규 설치 (overwrite) |
| **`.claude/agents/harness/`** | **사용자 생성 domain specialist agents** | user project (meta-harness가 generate) | **절대 삭제/modify 금지 + 백업 보존** |

Rules:
- [HARD] `.claude/agents/{core,expert,meta}/` 만 moai-adk template (`internal/template/templates/.claude/agents/`) 에 포함된다.
- [HARD] `internal/template/templates/.claude/agents/harness/` 디렉토리는 **존재 자체가 금지**. 누출 발견 시 cleanup chore + 본 섹션 cross-reference.
- [HARD] `moai-meta-harness` skill은 `.claude/agents/harness/*.md` 에만 generate. 다른 directory에 emit 금지.
- [HARD] `moai update`는 `.claude/agents/harness/` 디렉토리와 그 안의 모든 파일을 sync 대상에서 제외 (user-owned 보호).

Cross-references:
- `CLAUDE.local.md` §24.2 + §24.4 (Namespace + moai update contract)
