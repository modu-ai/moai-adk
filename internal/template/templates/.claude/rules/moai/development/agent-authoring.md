...
| **`.claude/agents/harness/`** | **사용자 생성 domain specialist agents** (auto-generated by `moai-meta-harness`) | user project (meta-harness가 generate) | **절대 삭제/modify 금지 + 백업 보존** |
| **`.claude/agents/local/`** | **메인테이너 전용 hand-authored agents** (e.g., release-update-specialist, github-specialist; entry via `97-*`/`98-*` thin command wrappers per dev-only-commands-isolation.md) | user project (maintainer가 author) | **절대 삭제/modify 금지 + 백업 보존** |
...
- [HARD] `internal/template/templates/.claude/agents/local/` 디렉토리는 **존재 자체가 금지**. 누출 발견 시 cleanup chore + 본 섹션 cross-reference. (메인테이너 전용 dev-only 자산이므로 사용자 프로젝트에 절대 배포 금지.)
- [HARD] `moai-meta-harness` skill은 `.claude/agents/harness/*.md` 에만 generate. 다른 directory에 emit 금지.
- [HARD] `moai update`는 `.claude/agents/harness/` 디렉토리와 `.claude/agents/local/` 디렉토리, 그리고 그 안의 모든 파일을 sync 대상에서 제외 (user-owned + maintainer-owned 보호).
