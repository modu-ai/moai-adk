...
|------|------------|-----------------|------------------------|
| `high` | Quality first — Opus throughout; `max` on the plan rows and where the marginal point is decision-critical | Opus on every row except the two single-shot procedural rows (manager-git, Explore); Fable 0 | `max` on manager-spec + plan-auditor + manager-develop + super-advisor; `high` on sync-auditor/docs/design/harness; `medium` on e2e; procedural `low` |
| `medium` (default) | Phase-weighted — plan is where a mistake is cheapest to catch, so it carries the ceiling; the rest of the pipeline sits one level down | Opus on every reasoning / coding / authoring agent; Fable 0 | `max` on manager-spec + plan-auditor; `high` on manager-develop/sync-auditor/manager-docs/super-advisor; `medium` on design/harness; e2e `low`; procedural `low` |
...
The per-agent effort defaults across the 3-tier profile system (high/medium/low) are the `llm.profiles` matrix (33 cells; Go SSOT `template.DefaultProfileMatrix`). The `model_routing_profiles` cells in `workflow.yaml` carry the `{model, effort}` pair for each Tier-Phase × perfTier combination. Six matrix cells use `max` effort — `manager-spec`, `plan-auditor`, `manager-develop` and `super-advisor` in the `high` column, plus `manager-spec` and `plan-auditor` in the `medium` column — and no cell uses `xhigh`, which on Opus scores the same as `high` at materially higher cost, leaving `max` as the only level above `high`.

The `medium` column is phase-weighted rather than cost-minimal: plan (`manager-spec`, `plan-auditor`) takes `max`, while run (`manager-develop`), review (`sync-auditor`) and sync (`manager-docs`) take `high`. This deliberately spends past the `medium` knee of the Opus cost/score curve, so the column is no longer the benchmark-derived cost floor and must not be re-derived back onto that anchor. Under a GLM backend the change is only partly observable: z.ai's 3-state reasoning control collapses `{medium, high}` onto one state and `{xhigh, max}` onto another, and `manager-develop` is pinned to the ceiling by the coding-max override regardless — so plan and sync move, run and review do not.
