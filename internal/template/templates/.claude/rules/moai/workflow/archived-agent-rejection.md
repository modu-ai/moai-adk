...
| 2 | `manager-quality` | Phantom archive (0 invocations) | Stop hook enforcement (lint + test + coverage delta) per REQ-ATR-009; OR `/moai gate` skill for ad-hoc invocation | Hook auto-fires on sync-phase commit; OR user runs `Skill("moai", arguments: "gate")` |
...
### §C.1 Why per-spawn prompt injection (Finding A5)
...
- **Re-introducing archived agents in template** — `internal/template/templates/.claude/agents/{core,meta,expert,agency}/<archived>.md` MUST NOT contain any of the 12 archived files; re-introduction triggers AC-ATR-005 failure
