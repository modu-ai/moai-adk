...
- **Take the window before entering, and the record is the first check.** `moai integration acquire --name <lane>` records the hold; `moai integration status` says who has it; `moai integration release` gives it back when the completion report is sent. A recorded hold is what the PreToolUse guard reads to refuse a second lane's `git merge`, so it is checked BEFORE `MERGE_HEAD` and before the pre-commit `HEAD` re-read — those stay, as the later checks. The deny layer is opt-in (`workflow.integration_lock.enabled`, default off); the record works either way. Taking a live holder's window needs `--force`, which records what it displaced — deliberate, never quiet.
