...
`review` is not one thing. The lead picks the lenses from what the card actually changed, and states the choice in the `sync` dispatch so the gate runs that review rather than re-deriving it. Lens table (which lenses per touched surface): `kanban-dispatch-detail.md` § Review lens selection.
