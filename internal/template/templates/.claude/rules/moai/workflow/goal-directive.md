...
When arming an infinite goal via `moai goal arm "<condition>" --max-turns 0 --max-duration <seconds>`, the default `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=8` silently terminates the loop long before the wall-clock / stagnation bounds fire. Raise `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` (e.g. to 200) when arming a `--max-turns 0` goal so the loop actually persists to a REAL bound. The `moai cc` / `moai cg` launchers do this automatically: when an armed `--max-turns 0` goal exists for the resolving session at launch time, the launcher injects `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=200` into the session env (best-effort, fail-open). For a session already running without the inject, set the env explicitly before arming: `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=200` (or higher) so the runtime cap does not pre-empt the goal loop.
