...
Guidance for `/moai goal` — MoAI's session-scoped completion condition that keeps the session working across turns until the condition holds. `/moai goal` is the single goal-arming surface for every orchestrator emission path.

> **Loading scope**: intentionally always-loaded (no `paths:` restriction) — the proactive triggers fire during active work (run-phase / migration / TDD), so this rule must be reachable in those contexts just like `askuser-protocol.md` / `session-handoff.md`. Read when the orchestrator arms a goal, when deciding between `/moai goal`, `/moai loop`, and a bespoke Stop hook, or when it recognizes a Proactive Recommendation Trigger (§ below).

## What It Is

`/moai goal "<condition>"` registers a completion condition and arms it for the active session. The condition text is parsed into a `conditions[]` array mixing two kinds: **mechanical** conditions (a shell command whose exit code decides, e.g. `go test ./... exits 0`) and **model** conditions (a claim the transcript must demonstrate, e.g. `all AC rows show PASS in the transcript`). The `moai hook stop-goal` Stop-hook evaluator loads the session's goal state at each turn-end and emits a block decision until the goal converges or a bound fires — so the session keeps working without a prompt at each step.

State lives at `.moai/state/goal/<session-id>.json`, one file per session. A turn ceiling (default 30) bounds the loop; at the ceiling the evaluator emits a 5-section verdict (Claim / Evidence / Baseline-attribution / Gaps / Residual-risk) and stops blocking. The ceiling is not the only bound: because the evaluator is a Stop hook, the runtime's consecutive-block cap (default 8, `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`) overrides the block first on an unattended run, ending the turn *without* the ceiling verdict — so the effective bound is `min(ceiling, cap)` and a missing verdict must not be read as convergence. Reconcile the two before arming a long unattended goal: `.claude/skills/moai/workflows/goal.md` § The runtime block cap bounds the loop before the turn ceiling does. A stagnation guard halts the loop after N consecutive no-progress iterations with an E1/E3 escalation note.

### Raising the block cap for an infinite goal (`--max-turns 0`) — SPEC-INFINITE-GOAL-001

When arming an infinite goal via `moai goal arm "<condition>" --max-turns 0 --max-duration <seconds>`, the default `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=8` silently terminates the loop long before the wall-clock / stagnation bounds fire. Raise `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP` (e.g. to 200) when arming a `--max-turns 0` goal so the loop actually persists to a REAL bound. The launchers do this automatically under **either** of two trigger conditions, not one: (1) an armed `--max-turns 0` goal exists for the resolving session at launch time, **or** (2) the session is entered in Kanban Mode (the `--kanban` / `-k` entry switch on the launchers that support it). In either case the launcher injects `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=200` into the session env (best-effort, fail-open). The second trigger exists because a Kanban Mode chain arms its goal mid-session, after the launch-time goal read has already happened — so trigger (1) alone would leave such a session capped at the default 8, and the chain would end with no error and no obvious cause. The raised cap is session-wide in both cases, not scoped to the goal or the chain that motivated it. For a session already running without the inject, set the env explicitly before arming: `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=200` (or higher) so the runtime cap does not pre-empt the goal loop.

`/moai goal` is **arm-only** — see § Goal-Presentation Timing for the property and its consequences. The delivered verbs are `/moai goal "<condition>"` (register + arm), `/moai goal status [--all]`, and `/moai goal clear`; a `resume` verb is documented as deferred and NOT delivered. Full verb surface, the progression-mode axis, and the safety invariants live in `.claude/skills/moai/workflows/goal.md`.

Availability: `/moai goal` needs hooks enabled, because its evaluator IS a Stop hook — it is unavailable when `disableAllHooks` or `allowManagedHooksOnly` is set. It carries no runtime-version floor of its own; a version floor stated for the native command must not be transposed onto it (see the native-command prohibition section below).
...
The preceding sections describe `/moai goal` semantics and condition authoring. This section adds the **proactive layer**: when the orchestrator, mid-workflow, recognizes a situation where a condition-declared loop is the right continuation primitive, it arms one rather than driving the work turn by turn. This closes the gap between the contexts where goal-arming is already wired (the session-handoff resume flow, the run-phase autonomy wiring) and the active-workflow context, where previously nothing prompted the orchestrator to consider it.

### Goal-Presentation Timing

**`/moai goal` is arm-only.** Arming records the condition in goal state and causes the `stop-goal` evaluator to block turn-end until the condition holds; it starts no work of its own. The consequence is concrete: a goal armed while nothing is running spins idle turns until the ceiling, because each turn-end finds the condition unmet and no work advancing it. Arming is therefore always paired with a work-starting action, and it never substitutes for one. This is why a paste-ready resume keeps a work-starting command (`/moai run SPEC-X`) as Block 5's single primary action rather than a bare goal-arming directive — see `.claude/rules/moai/workflow/session-handoff.md` § Canonical Format (Block 5) for the Block 5 rule that owns the arm-only consequence on the handoff surface.

**The goal is presented at the Implementation Kickoff Approval gate.** When the orchestrator runs Implementation Kickoff Approval (the `AskUserQuestion` round at the plan→run boundary), the goal is offered there as the **autonomous vs semi-autonomous progression-mode axis** — a DISTINCT axis from the approve/decline decision. The orchestrator arms the goal only after the gate passes. The axis and its two modes are specified in `.claude/skills/moai/workflows/goal.md` § Progression Mode; this section fixes the timing (which gate, and in what order relative to arming), not the mechanism.
...
**Rejected alternative — `/moai goal --run SPEC-X "<condition>"`.** A composite argument that would arm the goal and start the run in one invocation. Rejected on two independent grounds: (1) it **inverts the gate order** — arming would occur before Implementation Kickoff Approval, and arming begins a turn immediately, so run-phase work would start before the human gate, contradicting the invariant above; (2) it **would require amending a constraint** — `session-handoff.md` § Diet Constraints caps Block 5 at a single primary action, and a composite arm-and-run directive either violates that cap or forces it to be rewritten. Recorded here so the reasoning survives and the option is not silently re-proposed.

### Hard preconditions for every recommendation

- **Implementation Kickoff Approval comes first**: any run-phase goal-arming is downstream of the Implementation Kickoff Approval human gate (`AskUserQuestion`, plan→run) and never substitutes for or bypasses it. `run.md` § Run-phase Autonomy #1 owns the preferences-drained rationale (all user preferences are collected at that gate before any autonomy begins).
- **Arming is programmatic, and that is the point**: `/moai goal` is orchestrator-invocable, so the orchestrator arms the condition itself once the gate has passed — no user typing is required, and no reminder-to-type mechanism is needed. Where the nearest native equivalent is HUMAN-ONLY, this MoAI subcommand is the only pipeline path (see the native-command prohibition section below).
- **Safety boundary unchanged** (see § MoAI Integration Notes for the full statement).
- **`run.md` "set" shorthand**: `run.md` § Run-phase Autonomy states that the orchestrator MAY set the `ac_converge` goal. Read literally — the orchestrator arms it via `/moai goal` after the gate passes. T1/T3 below defer to `run.md`'s condition under this reading.
...
- `.claude/skills/moai/workflows/goal.md` — the `/moai goal` verb surface, the progression-mode axis, the semi-autonomous checkpoint flow, and the safety invariants
- `.claude/rules/moai/workflow/native-invocation-model.md` § Classification Matrix / § Axis B — the HUMAN-ONLY classification and the automation justification this rule's prohibition rests on
