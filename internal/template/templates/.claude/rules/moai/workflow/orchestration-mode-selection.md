...
**A teammate costs I/O for as long as it is alive, not only while it works.** An in-process teammate polls its mailbox on a fixed interval whether or not anything is addressed to it, so team size sets a standing floor of filesystem reads that no amount of idleness lowers. Measured across a two-dozen-session corpus of Claude Code debug logs: the poll interval is **0.50s per teammate** — median and p10 identical, so it is a fixed schedule rather than backoff — and a session running 8 teammates logged **941 polls/min**. Mailbox polling accounted for **34.2%** of every line those logs contain, and the overwhelming majority of the reads found no mailbox file at all.

Read that as a cost curve, not a prohibition: each teammate adds roughly 2 filesystem reads per second for its whole lifetime, so a team held open across a long session pays far more than its spawn count suggests. It is a second reason the 3-5 band is the right neighborhood, arrived at independently of the team-composition advice above — and a reason to end a team when its work is done rather than leave it idling. The mechanism is Claude Code runtime behavior, not a MoAI surface; the lever available here is how many teammates exist and for how long.
