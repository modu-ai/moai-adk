...
## Attributable diff-check pattern (the attributable diff-check pattern)
...
### Fallback-to-re-execution contract (the attributable diff-check pattern — the safety boundary)

The fallback is the safety boundary A9 exists to preserve. Omitting it turns the diff-check into a verification bypass that violates `verification-claim-integrity.md` §1.1 (the named anti-pattern the verification-bypass anti-pattern in `the attributable diff-check pattern SPEC/plan.md` §G). The fallback contract binds three properties:
...
The diff-check is strictly additive: it collapses wall-time on the happy path (all-three match → no re-execution) while preserving the verification invariant on every path. A regression that silently drops the fallback re-introduces the verification-bypass anti-pattern (the named anti-pattern in `the attributable diff-check pattern SPEC/plan.md` §G).

### When the diff-check does NOT apply

The diff-check predicates on the shared diagnostic snapshot (`moai verify check --key-current`), so it applies ONLY where the snapshot interface is reachable. When the snapshot CLI is absent, returns stale, or the §E evidence was recorded against a different tree, the orchestrator proceeds directly to re-execution — the diff-check is opt-in per-dimension, never mandatory. This is the the attributable diff-check pattern fallback operating at the composition layer, identical to the per-mismatch fallback but at coarser granularity.
...
- `.claude/rules/moai/core/agent-common-protocol.md` § Parallel Execution → Attributable diff-check doctrinal switch (the attributable diff-check pattern — the composition-time switch that selects consume-vs-re-execute).
- `.claude/rules/moai/development/manager-develop-prompt-template.md` § Section E → Attribution discipline (the attributable diff-check pattern — the §E attribution triple the diff-check consults).
...
Version: 1.1.0 (attributable diff-check pattern + fallback contract added)
