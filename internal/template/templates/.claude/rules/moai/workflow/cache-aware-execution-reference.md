...
Both TTL rows are authentication-mode defaults, not fixed properties: each cache hit refreshes the window (sliding TTL), and at the API level the 5-minute TTL is the default while the 1-hour TTL is an explicit `cache_control` override (`ttl: "1h"`). Before Claude Code 2.1.243 a session selected neither — the runtime placed cache breakpoints itself (parent rule, Non-goals). Since 2.1.243 (official docs floor: 2.1.242) the `promptCacheTtl` and `subagentPromptCacheTtl` settings let API-key and cloud-provider users opt the main conversation into the 1-hour TTL while subagents stay at 5 minutes, with per-bucket env-var twins `CLAUDE_CODE_PROMPT_CACHE_TTL` / `CLAUDE_CODE_SUBAGENT_PROMPT_CACHE_TTL` (Claude Code 2.1.243 changelog; official prompt-caching docs); the runtime still places the breakpoints. Whether a third-party gateway (`ANTHROPIC_BASE_URL`, e.g. z.ai) honors the setting is unmeasured — keep the 5-minute default as the operating assumption there until measured.
