...
- [HARD] Write-heavy sub-agents (expert-backend, expert-frontend, expert-refactoring, manager-cycle, researcher) MUST use `isolation: "worktree"` in agent definition frontmatter (SPEC-V3R2-ORC-004)
- [HARD] Read-only agents (permissionMode: plan) MUST NOT set `isolation: "worktree"` — plan mode already prevents writes
