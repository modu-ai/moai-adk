...
|----------------|----------|-----------------------------|
| `/code-review` | HUMAN-ONLY (manual-invocation-only since v2.1.215; environment-conditional since v2.1.246 — see anchor) | `[Skill]` marker — commands reference: `/code-review [low\|medium\|high\|xhigh\|max\|ultra] [--fix] [--comment] [target]` **Skill**. Claude no longer runs it on its own in first-party API environments — since v2.1.246 it may also start it itself on Bedrock, Vertex AI, and Foundry, through the Claude apps gateway, and when telemetry or non-essential traffic is disabled; invoke with `/code-review` to guarantee it (it runs as a background subagent as of v2.1.218) |
...
| `/deep-research` | HUMAN-ONLY (manual-invocation-only since v2.1.218) | `[Workflow]` marker — commands reference: `/deep-research <question>` **Workflow**. Starts only when invoked manually |
...
| `/review` | HUMAN-ONLY (alias of `/code-review` since v2.1.223) | Changelog 2.1.223: "Changed `/review` to be an alias of `/code-review`, which reviews the current diff or a PR (`/code-review <level> <pr#>`)". Its classification therefore follows the `/code-review` row above. The older `skills.md` `Skill`-tool-bridge listing predates the alias; where that listing still names `/review`, treat it as upstream doc lag, not as evidence of auto-invocability |
...
**Second correction (v2.1.223) — `/review` only.** The PROGRAMMATIC finding above was itself superseded for `/review` at CC v2.1.223, which made `/review` an alias of `/code-review`. Because `/code-review` is HUMAN-ONLY (manual-invocation-only since v2.1.215), `/review` inherits that classification and the matrix row was updated accordingly. `/security-review` is unaffected and remains the surviving PROGRAMMATIC built-in-exposed-via-`Skill`-tool case. The correction chain is preserved rather than rewritten, so the evidence trail (provisional expectation → run-phase observation → v2.1.223 alias supersession) stays legible.
...
- **`review` (including `review --security`).** The nearest native counterparts are `/security-review` and `/code-review` (which `/review` aliases since v2.1.223). Only `/security-review` is PROGRAMMATIC; `/code-review` — and therefore `/review` — is HUMAN-ONLY, manual-invocation-only since v2.1.215 (environment-conditional since v2.1.246 — see the matrix row), and `/deep-research` likewise since v2.1.218. So `/moai review`'s justification rests on a **mixed** footing: **Axis A** for the security pass (prefer invoking the native `/security-review` via `Skill()` where it is actually auto-invocable) and **Axis B** for the code-review pass (no tool call can trigger the HUMAN-ONLY `/code-review`, so automating it inside the pipeline is the only pipeline path). Both sit under a broader-orchestration argument: `/moai review` composes a security pass with `@MX` tag-compliance checking and code-review synthesis into one pipeline step, a composition the native single-purpose commands do not provide.
...
- The `Skill` tool itself can be denied in `/permissions`, which also removes the `Skill`-tool bridge that makes `/security-review` orchestrator-invocable. The official `skills.md`: "Disable all skills by denying the `Skill` tool in `/permissions`."
...
This doctrine (1) names the invocation-model axis, (2) classifies the overlapping native commands, and (3) applies the axis to `/moai feedback` as the worked example that proves it. Refactoring existing MoAI subcommands onto native bundled skills via Axis A (for example, pointing `/moai clean` at native `/simplify`, or `/moai review` at native `/code-review` — note `/code-review` and `/deep-research` are manual-invocation-only since v2.1.215/v2.1.218, `/code-review` conditionally since v2.1.246 per the matrix row) is a separate follow-up effort — the axis observation is recorded above, but no existing subcommand is retired or re-pointed by this doctrine. The classification is codification only; no runtime mechanism enforces the PROGRAMMATIC / HUMAN-ONLY tags.

## Cross-references

- Official Claude Code `commands.md` (commands reference) — the `[Skill]` / `[Workflow]` marker source for the classification matrix.
- Official Claude Code `skills.md` — the "bundled skills are prompt-based" defining quote, the `Skill`-tool bridge (for `/init` and `/security-review`; its `/review` entry predates the v2.1.223 alias), and the `disable-model-invocation` / `disableBundledSkills` caveats.
