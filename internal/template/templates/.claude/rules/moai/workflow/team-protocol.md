...
- **designer role_profile — Absorbed by manager-design**: the `designer` role_profile remains in `workflow.yaml` for team-mode backward compatibility, but its primary consumer is now the `manager-design` agent (Claude Design collaboration, D1-D5 pipeline). UI-surfaced SPECs route `plan → design → run` via manager-design rather than spawning a `designer` teammate.
- No teammate can override the isolation flag downward (implementer in plan mode is rejected).
- [ZONE:Evolvable] [HARD] team-mode teammates default to `model: sonnet`. **(Sonnet 5 / Opus 4.8 era: the historical 200K-window rapid-refill circuit breaker is resolved.)** Sonnet 5 ships a single 1M context variant (no 200K fallback target exists), and Opus 4.8 serves 1M by default, so the Anthropic #36670 `[1m]` suffix-strip mechanism has zero observable impact on the current default lineup — a stripped-suffix teammate still resolves to 1M and the cascade cannot start. The `model: inherit` convention is retained as defense-in-depth and remains valid for **legacy 200K-variant models** (Haiku 4.5 is still 200K). Additionally, team mode is now **disabled by default** (`workflow.team.enabled: false`) per the Phase 0.95 re-design — see `.claude/rules/moai/workflow/orchestration-mode-selection.md`; subagent fanout (Mode 4) replaces it for multi-domain research/review, sequential sub-agent (Mode 5) for coding. See `.claude/rules/moai/development/model-policy.md` § Baseline-Refill Breaker.
