...
- `kanban` contract (Kanban Mode, the `--kanban` / `-k` entry switch on the session launchers): **extends** `full-pipeline` and defines no second chaining mechanism. It inherits the run→sync auto-chain verbatim, including the clause above that `gate-sync-1` and `gate-sync-2` still fire unchanged inside the chained sync phase, and adds exactly two deltas — a plan-phase chain head, and a verify exit gate at run-phase exit. Everything else about how phases chain is the inherited contract, unmodified. See `workflows/run/mode-orchestration.md` § Verify Exit Gate and `workflows/kanban.md`.
