...
- Forward-link: Builder Workflow `.claude/workflows/harness-build.js` (not yet implemented — see Phase 4 below)
...
## Phase 4: Builder Delegation (forward-link)

On `Build` approval, delegate to the Builder Workflow. The delegation target is `.claude/workflows/harness-build.js` — a Claude Code dynamic-workflow script that runs the 4-phase Builder (ANALYZE / PLAN / GENERATE / ACTIVATE).

> **Forward-link — not yet implemented.** The Builder Workflow `harness-build.js` does NOT exist yet at the time this entry workflow is authored. It is a follow-up milestone. When this entry workflow is invoked before the Builder lands, the orchestrator MUST:
>
> 1. Surface a clear "Builder not yet implemented" notice to the user (in their `conversation_language`).
> 2. Record the confirmed profile + derived name + approval timestamp to `.moai/harness/build-requests/<ISO-DATE>-<name>.json` so the build can be resumed once the Builder lands.
> 3. Stop. Do NOT attempt to hand-build the harness inline — that would bypass the manifest-driven Runner contract that the Builder establishes.

The confirmed profile + derived name passed to the Builder (when it lands) is the single source of truth for the manifest's `source_request` field — the Builder MUST carry the original natural-language request verbatim into `manifest.json.source_request` per the manifest schema.

## Phase 5: Post-Build Summary

After the Builder Workflow returns (once implemented), render a one-paragraph summary in the user's `conversation_language` covering:

1. What was built (harness name + domain + entry command `/harness:<name>`).
2. Where the artifacts live (`.claude/commands/harness/<name>.md`, `.claude/workflows/harness-<name>-run.js`, `.claude/agents/harness/<name>-*-specialist.md`, `.claude/skills/harness-<name>-*/`, manifest).
3. Suggested next step (e.g., "Run `/harness:<name>` to execute the harness on a sample task").

Until the Builder lands, Phase 5 renders the "request recorded" summary instead (per Phase 4 forward-link notice).
...
| Builder not yet implemented | `harness-build.js` absent | Phase 4 forward-link path — record request, surface notice, stop |
