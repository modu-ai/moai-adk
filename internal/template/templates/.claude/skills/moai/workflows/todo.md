...
| `moai todo list` | Render the queue, lock-free. The default view is the live load: `queued` and `picked` cards, with the dropped set collapsed into one count line naming `--dropped`. `moai todo list --dropped` renders the discarded set with its markers — the surface `undrop` reads. `--json` emits the structured records, dropped cards included, so a machine consumer filters by the `state` field rather than by absence. |
