...
**Shared-snapshot wiring (SPEC-AUDIT-SNAPSHOT-001 A4, REQ-AUDIT-SNAPSHOT-004).** The snapshot is keyed by HEAD SHA (`internal/verify.Key` — HEAD + porcelain-v2 + diff hash); a new commit invalidates the prior snapshot (AC-AUDIT-SNAPSHOT-004b). Three sync-phase consumers — the `sync-auditor` Evidence cells, the `.claude/hooks/moai/sync-phase-quality-gate.sh` Stop hook, and the `.claude/workflows/sync-audit-4dim.js` 4-dimension judges — all consume this single snapshot keyed by HEAD SHA rather than each independently re-executing `go test` / `golangci-lint` / `go vet` / `go test -cover`. Concurrent recording requests for the SAME HEAD SHA are serialized via the per-key claim/lock in `internal/verify.RecordCheck` (in-process mutex + cross-process `O_EXCL` claim-stamp with staleness reclaim), so exactly one consumer's recording per dimension lands and the rest read — last-writer-wins never silently drops a dimension (REQ-004 ¶4, the D3 fix).
