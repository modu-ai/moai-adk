...
**[HARD] P1/P2 violations BLOCK sync, and halt BEFORE Phase 10 coverage (A7 — ).** If any P1 (missing @MX:ANCHOR on fan_in >= 3 function) or P2 (missing @MX:WARN on goroutine pattern) violations are found, sync is halted and the user must resolve them before proceeding. The P1/P2 gate fires BEFORE Phase 10 (Coverage Analysis) executes — eliminating the "30-min coverage then 1 missing tag aborts all" worst case, because the coverage command never runs when P1/P2 violations are present. The prior scheduling paid the full coverage cost before discovering the MX violation; A7 inverts the ordering so the cheap MX scan gates the expensive coverage run.
