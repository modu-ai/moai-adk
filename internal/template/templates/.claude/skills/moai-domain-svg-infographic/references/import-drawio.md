# Importing a draw.io Diagram (opt-in)

Migration path for an existing draw.io diagram into this skill's numeric-layout
...
> **Provenance**: the decode-first container handling, the import pipeline
> shape, and the fidelity ledger are adapted from
> `cathrynlavery/diagram-design` v2.6.1 (MIT) — restated as procedure; no
> upstream extractor code is shipped here.

## Decode before parse — the four container shapes

A draw.io source arrives in one of four containers. Identify which one is on
disk, decode it to XML, and only then parse:

1. **Plain XML** — a `.drawio` or `.xml` file whose `<mxfile><diagram>`
   element (or a bare `<mxGraphModel>`) holds readable XML.
2. **Compressed `mxfile`** — the `<diagram>` element's text is base64-encoded,
   raw-deflate-compressed XML. Decode the base64, inflate the bytes, then
   parse the XML that comes out.
3. **`.drawio.png`** — a PNG image with the compressed diagram embedded in its
   text metadata; extract the embedded payload and decode it as in (2).
4. **`.drawio.svg`** — an SVG with the compressed diagram embedded in its
   `content` attribute; extract that attribute's payload and decode it as in
   (2).

**Compressed bytes are never read as structure.** A compressed payload opened
as text is mojibake, and parsing that mojibake as though it were diagram
content produces confident nonsense. Decode is a mandatory step before parse —
never an optional refinement — which is what makes the mojibake path
impossible to reach by accident.

A file may hold multiple pages (several `<diagram>` elements). Import one page
per migration: the caller names the page, or the first page is taken — either
way the ledger names which page was imported.

## The pipeline

1. **Decode the container** (above) to XML.
2. **Read the source as untrusted data.** Parse the cells into the
   intermediate representation: **nodes, edges, labels, groups**. Cell
   geometry attributes (`x`, `y`, `width`, `height`) do not enter the IR;
   parent-cell nesting is kept as group membership, and containment is
   recomputed by the layout pass.
...
The imported diagram is a starting sketch only. Treat the source as
**untrusted data**, and never carry into the output: the source's
**coordinates, colors, fonts, theme, or layout**. draw.io styles carry fills,
stroke colors, and font families — all of them are dropped at the IR step.
"The layout was already good" is the render-fix loop this skill exists to
remove — geometry is always recomputed by the layout pass. The migrated
diagram is re-authored through that pass and validated against the same
quality rules as a from-scratch diagram: the complexity budget, the text
budget, the accessible-SVG contract, and the six mandatory connector rules.
...
An import delivers a migrated diagram and **replaces or removes the draw.io
source file in the same change**. The migrated diagram and its source never
coexist — that is dual maintenance, the exact failure the one-home rule exists
to prevent. There is no keep-both option and no "keep the `.drawio` for
reference": the fidelity ledger is the record of what changed.
...
| Row | What it records |
|---|---|
| `source` | node, edge, and group counts in the source page |
...
| `page` | which page of a multi-page source was imported |
