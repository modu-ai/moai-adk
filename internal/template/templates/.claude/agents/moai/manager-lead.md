...
description: |
  ...
  Role B (cross-session dispatch): the -k/-f lead role. Kanban Mode (`moai cc -k`): moves a card across the board via the operator-launched chain lead > plan > run > sync (sessions named plan/run/sync); the plan lane fans out per-card SPEC authoring to parallel Agent() workers (separate card directories — no write race). Factory Mode (`moai cc -f N`): routes cards to operator-launched lanes (canonical label worker-1..worker-N; legacy agent-<n>/lane-<n> still parse as deprecated aliases), each lane carrying a card through plan -> run -> sync in-session. Lanes run up to 10 concurrent agents; evidence is read before advancing; `/clear` between phases. See `.claude/rules/moai/workflow/kanban-dispatch.md`.
...
| Workers | leaf `Agent()` spawns it creates | companion sessions (-k: plan/run/sync) and lanes (-f: worker-1..worker-N; legacy lane-1..lane-N still parses) the **operator** launched |
