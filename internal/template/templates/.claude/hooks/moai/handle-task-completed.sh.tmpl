...
# Project-local hook: .claude/hooks/moai/task-completed.sh
...
if [ -f "$HOME/go/bin/moai" ]; then
    exec "$HOME/go/bin/moai" hook task-completed 2>>"$MOAI_HOOK_STDERR_LOG"
fi

# Try ~/.local/bin/moai (Linux install location)
if [ -f "$HOME/.local/bin/moai" ]; then
    exec "$HOME/.local/bin/moai" hook task-completed 2>>"$MOAI_HOOK_STDERR_LOG"
fi
