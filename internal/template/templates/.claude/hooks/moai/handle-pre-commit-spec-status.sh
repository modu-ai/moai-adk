#!/bin/bash
# Lifecycle Sync Gate — pre-commit hook for spec.md/progress.md cross-file
# status invariant enforcement.
#
# Behavior:
#   - Verifies that spec.md frontmatter `status:` field and progress.md §E.3
#     `status:` field agree whenever either file is staged with a status change.
#   - When a commit subject matches the canonical 4-phase close pattern
#     (`chore(SPEC-XXX): 4-phase close — atomic`), also enforces that
#     spec.md `status: completed` is present in the staged diff.
#   - On mismatch: exit 2 + structured JSON output. NEVER invokes
#     AskUserQuestion — the orchestrator translates the exit-code + JSON
#     into a user-facing prompt per the hook invocation surface policy.
#
# Cross-reference: `.claude/rules/moai/core/agent-common-protocol.md`
...
# -----------------------------------------------------------------------------
# Stage 4 — Canonical 4-phase close subject enforcement
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Stage 5 — Cross-file status invariant
# -----------------------------------------------------------------------------
