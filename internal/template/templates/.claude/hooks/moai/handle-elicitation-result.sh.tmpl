...
# RETIRE-OBS-ONLY (SPEC-V3R2-RT-006 + SPEC-V3R2-MIG-002): not registered in settings.json.
# Enable via system.yaml hook.observability_events list (opt-in only).
# This script forwards stdin JSON to the moai hook elicitation-result command.
# Project-local hook: .claude/hooks/moai/elicitation-result.sh
...
if command -v moai &> /dev/null; then
    exec moai hook elicitation-result 2>>"$MOAI_HOOK_STDERR_LOG"
fi

# Try default ~/go/bin/moai
if [ -f "$HOME/go/bin/moai" ]; then
    exec "$HOME/go/bin/moai" hook elicitation-result 2>>"$MOAI_HOOK_STDERR_LOG"
fi

# Try ~/.local/bin/moai (Linux install location)
if [ -f "$HOME/.local/bin/moai" ]; then
    exec "$HOME/.local/bin/moai" hook elicitation-result 2>>"$MOAI_HOOK_STDERR_LOG"
fi
