...
Default (natural language): Routes to /moai design with interactive path selection (Claude Design vs code-based)
...
- First, preload the user-question tool schema via `ToolSearch` per `.claude/rules/moai/core/askuser-protocol.md`
- Conduct Socratic interview (max 4 questions per round, see SSOT for full protocol)
...
Constraints:
- Maximum 4 questions per interaction call (Claude Code limit)
...
2. Ask user confirmation before searching (standard interaction channel)
