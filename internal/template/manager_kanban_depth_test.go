...
//
//  1. `.claude/agents/moai/manager-kanban.md` is the SOLE retained agent whose
...
//     omission (every retained MoAI agent other than manager-kanban).
//  3. Any future leaf-worker agent file that declares itself a manager-kanban
//     spawn target (frontmatter `leaf_of: manager-kanban` OR body marker
//     `<!-- manager-kanban leaf-worker -->`) MUST omit `Agent` from `tools:`.
...
// agent omitting `Agent` from `tools:`. manager-kanban is the sole carve-out
// (depth-1 carrier). The CI test catches (a) a future agent file that adds
// `Agent` to `tools:` without being manager-kanban, (b) a future leaf-worker
// file that nests further by carrying `Agent`, and (c) a regression that
// removes `Agent` from manager-kanban's own `tools:` list.
//
// Cross-references:
//   - .claude/agents/moai/manager-kanban.md § Depth-2 Seal
...
// TestManagerLeadIsSoleAgentCarrier asserts that manager-kanban.md is the only
...
func TestManagerLeadIsSoleAgentCarrier(t *testing.T) {
	...
	// Verify exactly one carrier and that it is manager-kanban.md.
	if len(carriers) != 1 {
		t.Errorf(
			"DEPTH_SEAL_VIOLATION: expected exactly 1 agent under .claude/agents/moai/ "+
				"carrying Agent in tools: (manager-kanban.md); found %d carriers: %v. "+
				"manager-kanban is the sole depth-1 carrier; every other retained agent "+
				"preserves the flat hierarchy by tool omission.",
			len(carriers), carriers,
		)
		return
	}
	if carriers[0] != "manager-kanban.md" {
		t.Errorf(
			"DEPTH_SEAL_VIOLATION: the sole Agent-carrier under .claude/agents/moai/ "+
				"must be manager-kanban.md; found %s. Either rename the carrier or remove "+
...
// TestManagerLeadCarriesAgent asserts that manager-kanban.md itself carries the
// Agent token in its tools: list — the depth-1 carrier invariant. A regression
// that removes Agent from manager-kanban's tools would silently close the fan-out
// seam and defeat the agent's purpose.
func TestManagerLeadCarriesAgent(t *testing.T) {
	t.Parallel()

	projectRoot := findProjectRootDepth(t)
	leadPath := filepath.Join(projectRoot, ".claude", "agents", "moai", "manager-kanban.md")

	data, err := os.ReadFile(leadPath)
	if err != nil {
		t.Skipf("manager-kanban.md not found at %s; skipping (test ships alongside the agent file)", leadPath)
	}
	...
	if toolsLine == "" {
		t.Fatalf(
			"DEPTH_SEAL_VIOLATION: manager-kanban.md has no tools: field in frontmatter; "+
				"the depth-1 carrier MUST declare tools: with Agent present.",
		)
	}
	if !hasAgentToken(toolsLine) {
		t.Errorf(
			"DEPTH_SEAL_VIOLATION: manager-kanban.md tools: field is missing the Agent "+
				"token (found: %q). manager-kanban is the sole Agent-carrier; removing "+
...
// declare themselves manager-kanban leaf-worker spawn targets, and asserts they
...
func TestNoNestedLeafWorkerCarrier(t *testing.T) {
	...
	err := filepath.WalkDir(agentsDir, func(path string, d os.DirEntry, err error) error {
		...
		// manager-kanban.md itself is the depth-1 carrier; skip.
		if strings.HasSuffix(path, filepath.Join("moai", "manager-kanban.md")) {
			return nil
		}
		data, err := os.ReadFile(path)
		if err != nil {
	...
	if len(violations) > 0 {
		t.Errorf(
			"DEPTH_SEAL_VIOLATION: the following files declare themselves manager-kanban "+
...
// isLeafWorkerOf reports whether the file content declares itself a manager-kanban
// leaf-worker spawn target. The declaration is opt-in via either:
//   - frontmatter field `leaf_of: manager-kanban`
//   - body marker `<!-- manager-kanban leaf-worker -->`
func isLeafWorkerOf(data []byte) bool {
	content := string(data)
	if strings.Contains(content, "leaf_of: manager-kanban") {
		return true
	}
	if strings.Contains(content, "<!-- manager-kanban leaf-worker -->") {
		return true
	}
	return false
}
