...
// verifyStampResolves answers only whether the stamped commit exists as an
// object in this checkout. It is deliberately narrower than comparability:
// resolution says nothing about whether HEAD can reach the commit.
func verifyStampResolves(projectRoot, sha string) error {
...
// verifyStampAncestorOfHEAD answers the second comparability condition: the
// resolved commit must be an ancestor-or-self of HEAD. Ancestor-OR-SELF is
// deliberate — a tree stamped at its own HEAD is comparable, and a strict test
// would turn every just-stamped tree into a system error.
func verifyStampAncestorOfHEAD(projectRoot, sha string) error {
	// `merge-base --is-ancestor` exits non-zero both for a genuine
	// non-ancestor and for an unreadable history. Both are treated as
	// unreachable: fail closed, since neither establishes a comparison window.
...
func checkCodemaps(projectRoot string, th Thresholds) (LayerReport, error) {
	...
	// Step 1 — the stamped commit must RESOLVE. This stays ahead of the
	// body probe below because the pre-existing contract already ordered it
	// that way: an unresolvable stamp is a failed measurement whatever the
	// body looks like, and this SPEC does not renegotiate that (spec.md §D
	// row 2).
	if err := verifyStampResolves(projectRoot, pv.CommitSHA); err != nil {
		rep.Verdict = VerdictAbsent
	...
	// Step 1.5 — body presence. C1 (no codemaps prose at all) is a
	// determinate observation, not a failed measurement, and it OUTRANKS the
	// ancestry check below: a tree with nothing to be stale about is absent
	// with a nil error (exit 1), the disposition the sibling citations layer
	// already uses. Deciding it here rather than inside resolveContentAnchor
	// is what keeps an unreachable stamp from relabelling a body-absent tree
	// as a failed measurement (spec.md §D row 1 vs row 3).
	...
	// Step 2 — HEAD ancestry, BEFORE any anchor or diff work. Resolution and
	// ancestry are SEPARATE conditions: `git cat-file -e` answers only the
	// first, and a squash or a rebase leaves the original commit resolvable
	// while dropping it out of HEAD's history. Measuring a diff across that
	// gap produces a number for a window that was never established, which is
	// why this yields no freshness value (REQ-GSA-001/002/004). The reason
	// differs from step 1 because the recovery differs: an unresolved object
	// needs history, an unreachable one needs a regenerated body stamped at a
	// reachable commit.
	if err := verifyStampAncestorOfHEAD(projectRoot, pv.CommitSHA); err != nil {
		rep.Verdict = VerdictAbsent
