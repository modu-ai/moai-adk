...
func TestRequestToolPairAdversaries(t *testing.T) {
	for _, which := range []string{"duplicate call", "duplicate result", "wrong call role", "wrong result role", "input array", "nonboolean error", "result image", "extra call", "extra result", "empty id", "empty name", "duplicate definition"} {
