...
function getMoAIVersion(projectRoot) {
  try {
    ...
    if (homeDir) {
      const marketplacePluginPath = path__namespace.join(
        homeDir,
        ".claude",
        "plugins",
        "marketplaces",
      ...
      if (fs__namespace.existsSync(marketplacePluginPath)) {
        const pluginData = fs__namespace.readFileSync(marketplacePluginPath, "utf-8");
        const plugin = JSON.parse(pluginData);
        if (plugin.version) {
          return plugin.version;
        }
      }
      const cachePluginPath = path__namespace.join(
        homeDir,
        ".claude",
        "plugins",
        "cache",
      ...
      if (fs__namespace.existsSync(cachePluginPath)) {
        const pluginData = fs__namespace.readFileSync(cachePluginPath, "utf-8");
        const plugin = JSON.parse(pluginData);
        if (plugin.version) {
          return plugin.version;
        }
...
// src/hooks/session-notice/message-builder.ts
...
// src/hooks/session-notice/index.ts
