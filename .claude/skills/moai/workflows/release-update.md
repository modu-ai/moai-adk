...
description: >
  ...
  NOT distributed to user projects. Tracks CC release notes since last analyzed
  version, classifies changes by impact tier (Tier 1/2/3), cross-references
  official docs, generates update plan or umbrella SPEC, synchronizes docs-site
  4-locale + README via manager-docs, and commits via manager-git.
  Activated by /release-update command.
...
Docs-site rules: CLAUDE.local.md §17 is the authoritative reference. Do NOT duplicate its content here — defer to it for URL standards, Mermaid direction, locale sync obligations, and build checklist.
...
|-------|-------------|------|-------|
| 0 | Orchestrator (direct) | — | State file read |
| 1 | Orchestrator (direct + AskUserQuestion) | — | Release notes collection |
...
## Verification Gate (CLAUDE.md §16 Self-Check)
...
| Using hardcoded WebFetch URLs without fallback | Docs URLs may shift after training cutoff | Try alternate URL variants; prefer Option A (user paste) |

---

## References

- CLAUDE.local.md §17 — docs-site 4-locale sync rules (authoritative)
