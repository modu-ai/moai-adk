  ...
  essentials-analysis:
    ...
    steps:
      ...
      - name: Install Unified Essentials
        run: pip install moai-essentials-unified
...
pipeline {
    ...
    stages {
        stage('Setup') {
            steps {
                sh 'pip install moai-essentials-unified'
            }
        }
        ...
        stage('Analysis') {
            parallel {
                stage('Debug & Refactor') {
                    steps {
                        sh 'moai-essentials-unified --workflow debug_first'
                    }
                }
                
                stage('Performance') {
                    steps {
                        sh 'moai-essentials-unified --workflow performance_focus'
                    }
                }
                
                stage('Quality') {
                    steps {
                        sh 'moai-essentials-unified --workflow quality_gate'
                    }
                }
            }
        }
...
moai-essentials-unified --workflow complete --config enterprise.yaml

# Run specific component
moai-essentials-unified --component debugger --error "TypeError: ..."

# Performance profiling
moai-essentials-unified --component profiler --target ./src/ --output perf.json

# Code review
moai-essentials-unified --component reviewer --target ./src/ --trust5

# Technical debt analysis
moai-essentials-unified --component refactorer --analyze-debt ./src/

# Test generation
moai-essentials-unified --component tester --generate-tests ./src/ --coverage 0.95
```
...
```bash
# Custom workflow
moai-essentials-unified --workflow custom --definition workflow.yaml

# Parallel execution
moai-essentials-unified --workflow complete --parallel --max-concurrent 8

# CI/CD integration
moai-essentials-unified --workflow complete --cicd github-actions

# Report generation
moai-essentials-unified --report --format html --output report.html

# Cache management
moai-essentials-unified --cache clear --type context7
moai-essentials-unified --cache status
```
