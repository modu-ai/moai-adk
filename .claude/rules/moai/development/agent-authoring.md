...
|------|------|-----------------|---------------------|
| `.claude/agents/moai/` | manager-*, plan-auditor, evaluator-active, builder-harness (범용 retained agents, FLAT layout per v.2.x baseline) | template | 삭제 후 신규 설치 (overwrite) |
...
Rules:
- [HARD] `.claude/agents/moai/` 만 moai-adk template (`internal/template/templates/.claude/agents/`) 에 포함된다 (FLAT layout per v.2.x baseline).
