...
| **G — fact-force state-file dir** | 1 PreToolUse gate (`gateguard-fact-force.sh`) | `grep -l '\.moai/state/fact-force' gateguard-fact-force.sh` → 1; the gate is the ONLY consumer of the `${CLAUDE_PROJECT_DIR:-$PWD}/.moai/state/fact-force/` directory | **acceptable-by-design** | A single-consumer dependency is by definition NOT a shared mode (one script does not make a shared failure). The gate fails *open* (`mkdir -p ... || exit 0`) on an unwritable state dir, so the failure mode degrades to "no advisory notice" rather than "session halt". |
