...
**`fanout`'s own 3-5 is an advisory band, not a hard cap — and not the team-size quote.** The fan-out band is grounded in cache/prefix economics (concurrent spawns of the same agent definition cannot read a cache entry still being written: spawn ONE first and let the remaining N−1 read its cache — `.claude/rules/moai/workflow/cache-aware-execution.md` directive 2, stagger-spawn) and in coordination overhead (every fan-out result lands back in the orchestrator's context to reconcile). Its numeric coincidence with limit 3 is just that — a coincidence; the derivations are independent, and the former text that quoted "Start with 3-5 teammates" as a `fanout` cap conflated the two. The hard bound for fan-out is limit 1 (runtime cap, default 20): exceeding the 3-5 band violates no cap — it degrades cache economics and reconciliation quality. Write fan-out is bounded separately and more strictly (one writer per working tree — parallel write-capable agents only in independent worktrees, `agent-common-protocol.md` § Background Agent Execution), regardless of count.

**A teammate costs I/O for as long as it is alive, not only while it works.** An in-process teammate polls its mailbox on a fixed interval whether or not anything is addressed to it, so team size sets a standing floor of filesystem reads that no amount of idleness lowers. Measured over 24 sessions of this project's own debug logs (2026-08-16 → 08-24): the poll interval is **0.50s per teammate** — median and p10 identical, so it is a fixed schedule, not backoff — and a session running 8 teammates logged **941 polls/min**. Mailbox polling accounted for **34.2%** of every line those logs contain (79,988 poll entries plus 92,449 mailbox reads out of 504,582 lines), and the overwhelming majority of the reads found nothing (133,665 `file does not exist`).

Read that as a cost curve, not a prohibition: each teammate adds ~2 filesystem reads per second for its whole lifetime, so a team held open across a long session pays far more than its spawn count suggests. It is a second reason the 3-5 band is the right neighborhood, arrived at independently of Anthropic's team-composition advice — and a reason to end a team when its work is done rather than leave it idling. The mechanism is Claude Code runtime behavior, not a MoAI surface; the lever available here is how many teammates exist and for how long.
