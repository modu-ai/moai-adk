...
- [ZONE:Evolvable] [HARD] team-mode teammates default to `model: sonnet` (200K window). Under MoAI's heavy baseline a 200K window can saturate at spawn and trip the runtime rapid-refill circuit breaker → zero output. Switching to `model: inherit` does NOT reliably fix this (Team `[1m]` non-inheritance, Anthropic issue #36670 OPEN → teammate falls back to 200K). For large SPECs prefer a single `manager-develop` (`model: inherit`, 1M) + Round split; reserve team mode for small SPECs. See `.claude/rules/moai/development/model-policy.md` § Baseline-Refill Breaker.
  - **CC 2.1.198 caveat (Sonnet 5 native 1M)**: On the Anthropic API `model: sonnet` now resolves to **Sonnet 5** with a native 1M window (no `[1m]` suffix, no usage credits — CC 2.1.197), so the 200K-saturation premise above is now scoped to teammates that are actually budgeted at 200K: behind an LLM gateway (`ANTHROPIC_BASE_URL` non-Anthropic, e.g. GLM/z.ai), with `CLAUDE_CODE_DISABLE_1M_CONTEXT=1`, or on older models. Whether a Sonnet 5 (1M-native) teammate on the Anthropic API still trips the rapid-refill breaker needs **live team-mode measurement** before the operational guidance above is weakened — the guidance stands (do NOT delete) pending that measurement. Issue #36670 state was NOT re-checked this run (gap).
