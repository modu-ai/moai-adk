...
- **Route B — PR route (Tier L OR explicit `--pr`):** Tier L / `--pr`: `manager-git` creates a feature branch and opens a PR per phase (`gh pr create`); phase transitions are triggered by PR merges into `main`. This is the route the Late-Branch closure pattern (below) applies to. Tier S/M Route B (without `--pr`, per `repo-local-pr-policy.md` ALL-tier Route B override): the orchestrator handles push+PR directly via Bash (`git switch -c` / `git push -u` / `gh pr create`) with `MOAI_BRANCH_GUARD_EXEMPT=1` + the Pre-Spawn Sync Check — no `manager-git` spawn (SPEC-ORCH-GIT-RELAX-001).
