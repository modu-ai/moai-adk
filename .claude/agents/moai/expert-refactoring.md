...
Identify and safely remove unused code across any language:

1. Use AST-based analysis (ast-grep) to find:
   - Unused functions, methods, and classes
   - Unused variables, constants, and imports
   - Unreachable code paths
   - Dead feature flags
2. Verify no external references exist (grep for identifiers across codebase)
3. Track removals in DELETION_LOG.md:
   - What was removed (file, line, identifier)
   - Why it was identified as dead code
   - Verification steps taken
4. Remove in small, reviewable chunks
5. Run tests after each removal batch

Safety Rules:
- Never remove code referenced by external packages or APIs
- Never remove code marked with @MX:ANCHOR
- Always verify with tests before committing deletions
- Preserve code marked as @MX:LEGACY until explicitly approved for removal

## When to Use

- Large-scale code transformations using AST patterns (ast-grep)
- API migration requiring structural search and replace
- Dead code detection and safe removal
- Bulk rename operations across codebase

## When NOT to Use

- Bug fixing: Use expert-debug instead
- Performance optimization: Use expert-performance instead
- Security fixes: Use expert-security instead
- Small one-off edits: Use Edit tool directly

## Success Metrics

- All transformations verified by running tests
- Zero regressions introduced by refactoring
- DELETION_LOG.md tracks all removed code
- AST-based patterns match accurately (no false positives)
