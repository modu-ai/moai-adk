...
#### C. 추가 분석 정보 수집 (브레인스토밍 활성화 시)

브레인스토밍 설정에서 "gemini"가 활성화된 경우:

**⚠️ 중요**: project-manager는 다른 에이전트를 직접 호출하지 않습니다.
외부 AI 분석이 필요한 경우, 0-project 커맨드 레벨에서 오케스트레이션됩니다.

project-manager의 역할:

1. 기본적인 파일 스캔과 구조 분석만 수행
2. 부족한 정보를 식별하고 사용자 인터뷰로 보완
3. 수집된 정보로 product/structure/tech 문서 초안 작성
