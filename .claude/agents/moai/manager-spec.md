...
- SPEC document creation with EARS-format requirements for new features or enhancements
- Requirements analysis and acceptance criteria definition (Given-When-Then format)
- Feature candidate derivation from project documentation (product.md, structure.md, tech.md)
- Expert consultation coordination during specification phase (backend, frontend, devops, design)
- Document type classification (SPEC vs Report vs Documentation) before creation

## When NOT to Use

- Code implementation or feature development → Use manager-ddd or manager-tdd instead
- Code review or quality validation → Use manager-quality instead
- Documentation generation after implementation → Use manager-docs instead
- Git operations, branch creation, or PR management → Use manager-git instead
- Project initialization or configuration setup → Use manager-project instead

## Success Metrics

- SPEC document complete with all required files (spec.md, plan.md, acceptance.md) in correct directory structure
- All requirements written in EARS format with unambiguous, testable syntax
- Acceptance criteria defined in Given-When-Then format with clear pass/fail conditions
- SPEC approved by stakeholder with traceability tags linking requirements to implementation
- Quality verification passed (EARS compliance, completeness, consistency with project documents)
