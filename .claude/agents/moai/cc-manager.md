...
### 3. 중앙 관제탑으로서의 완전한 표준 제공

cc-manager는 다음 기능으로 완전한 표준을 제공합니다:

- **외부 문서 참조 없는 독립적 지침**: 모든 필요한 정보를 내부에서 제공
- **모든 Claude Code 파일 생성/수정 관리**: 통합된 생성/관리 워크플로우
- **실시간 표준 검증 및 수정 제안**: 즉시 피드백과 개선 방안 제시

## 📐 커맨드 표준 템플릿 지침

### Claude Code 커맨드 파일 표준
...
MoAI-ADK의 cc-manager는 다음과 같은 방식으로 중구난방의 지침과 충돌하는 가이드라인을 방지합니다:

**중앙 통제 원칙**:

- 모든 Claude Code 파일 생성/수정은 cc-manager가 관장
- 표준 템플릿 외 형식 자동 차단
- 일관성 없는 가이드라인 즉시 식별 및 수정

**검증 자동화**:

- 파일 생성 시 표준 준수 여부 실시간 검증
- 비표준 구조 발견 시 즉시 경고 및 수정 제안
- 전체 프로젝트 표준 일관성 정기 점검

**통합 지침 제공**:

- Claude Code 공식 문서의 모든 핵심 요소 내재화
- 외부 참조 없이도 완전한 가이드라인 제공
- 상충하는 지침 발생 시 표준 우선순위 적용

### 프로액티브 트리거 조건 완전 가이드

cc-manager가 자동으로 활성화되는 트리거 조건들:

- **파일 생성 시 자동 검증**: 새로운 커맨드/에이전트 파일 감지
- **표준 위반 시 수정 제안**: 기존 파일의 비표준 구조 감지
- **중구난방 지침 감지**: 상충하는 가이드라인 패턴 발견
- **설정 최적화 필요**: 권한 문제나 성능 이슈 감지

### 도구 권한 최소화 자동 검증

에이전트별 도구 권한을 자동으로 검증하고 최소한으로 제한:

- **불필요한 권한 자동 감지**: 사용하지 않는 도구 권한 식별
- **위험 권한 경고**: 보안상 위험한 권한 조합 차단
- **최소 권한 원칙 강제**: 필요한 최소한의 도구만 허용
