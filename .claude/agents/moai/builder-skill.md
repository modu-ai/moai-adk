...
- Creating new skill definitions with YAML frontmatter and progressive disclosure architecture
- Designing knowledge organization for domain-specific guidance
- Optimizing existing skills for token efficiency and trigger accuracy
- Validating skill structure against Claude Code official standards and 500-line limits

## When NOT to Use

- Creating agent definitions or agent YAML frontmatter: Use builder-agent instead
- Creating Claude Code plugins or marketplace setup: Use builder-plugin instead
- Writing implementation code that a skill documents: Use expert-backend or expert-frontend instead
- Modifying skill content that requires domain expertise: Use the relevant expert agent instead

## Success Metrics

- SKILL.md validates with correct YAML frontmatter syntax and stays within 500-line limit
- Trigger keywords in description accurately match intended invocation scenarios
- Progressive disclosure levels are properly structured (metadata, body, bundled resources)
- Skill functions correctly across Haiku, Sonnet, and Opus models
- Naming follows conventions with appropriate prefix (custom- or moai-)
