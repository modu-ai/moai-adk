...
- Push and PR creation → `manager-git` owns both at every Tier; this agent never pushes and never opens a PR itself. On the repository's git-flow route a card carries no PR at all — the lane merges into local `develop` inside the integration window and the lead batches the push — so "no PR" here is the route working as configured, not a step skipped.
