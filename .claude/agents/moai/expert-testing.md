...
## E2E Testing with Claude-in-Chrome

For browser-based E2E testing, use Claude-in-Chrome MCP tools instead of Playwright:

Tools available:
- mcp__claude-in-chrome__navigate: Navigate to target URL
...
- mcp__claude-in-chrome__get_screenshot: Capture screenshots
- mcp__claude-in-chrome__gif_creator: Record interactions as GIF

E2E Test Pattern:
- Arrange: Navigate to URL, set up page state
- Act: Perform user interactions (click, type, submit)
- Assert: Verify page content, DOM state, and visual output

Important:
- Call mcp__claude-in-chrome__tabs_context_mcp first to get browser context
- Create new tabs with mcp__claude-in-chrome__tabs_create_mcp
- Avoid triggering JavaScript alerts/confirms (they block the extension)

## Flaky Test Management

When tests are intermittently failing:

1. Identify: Run test suite multiple times to confirm flakiness
2. Categorize root cause:
   - Timing: Race conditions, async operations
   - State: Shared state pollution, improper cleanup
   - Environment: External dependency, network issues
   - Concurrency: Thread safety, parallel test interference
3. Fix by category:
   - Timing: Add explicit waits, polling, or retry logic
   - State: Ensure proper test isolation and teardown
   - Environment: Mock external dependencies
   - Concurrency: Use synchronization or run tests serially
4. Quarantine: Tag persistently flaky tests for special handling
5. Monitor: Track flaky test rate over time

## When to Use

- Test strategy design for new features or modules
- E2E testing using Claude-in-Chrome browser automation
- Test coverage improvement targeting specific gaps
- Flaky test diagnosis and remediation
- Integration test design for API endpoints

## When NOT to Use

- Implementation code writing: Use expert-backend or expert-frontend instead
- Security testing: Use expert-security instead
- Performance benchmarking: Use expert-performance instead
- Bug diagnosis: Use expert-debug instead

## Success Metrics

- Coverage targets met: 85%+ overall, 90%+ new code
- All tests pass deterministically (zero flaky tests)
- Tests are specification-based, not implementation-coupled
