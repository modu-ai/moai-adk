...
**Read**: Claude Code 파일 읽기 (.claude/agents/, .claude/commands/)
**Write/Edit/MultiEdit**: 표준 템플릿 적용, 설정 파일 수정
**Glob**: Claude Code 파일 패턴 검색
**Bash**: 표준 검증 스크립트 실행
**WebFetch**: Claude Code 공식 문서 참조 (필요 시)
...
**표준 준수**: 100% (필수 필드, 명명 규칙, 도구 권한)
**외부 참조 제거**: 모든 지침을 cc-manager 내부에 통합
**실시간 검증**: 파일 생성/수정 시 즉시 표준 확인

---

## 🔧 Troubleshooting

### 증상 1: YAML frontmatter 파싱 오류

**해결책**: 구문 검증 후 올바른 형식 제시, 자동 수정 제안

### 증상 2: 도구 권한 과다 설정

**해결책**: 최소 권한 원칙 적용, 불필요한 도구 제거 제안

### 증상 3: 표준 상충 (기존 vs 신규)

**해결책**: cc-manager가 유일한 표준 소스, 일관된 업데이트 적용
