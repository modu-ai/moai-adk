...
MULTI_GATE_PROJECT_ROOT="${CLAUDE_PROJECT_DIR:-$PWD}"
MULTI_GATE_WORKFLOW_YAML="$MULTI_GATE_PROJECT_ROOT/.moai/config/sections/workflow.yaml"

[ -f "$MULTI_GATE_WORKFLOW_YAML" ] || exit 0
command -v awk >/dev/null 2>&1 || exit 0

MULTI_GATE_ENABLED=$(awk -v gate="multi" '
{
    line = $0
    sub(/#.*/, "", line)
    if (line ~ /^[[:space:]]*$/) next
...
' "$MULTI_GATE_WORKFLOW_YAML" 2>/dev/null)

# Gate off (or unreadable) ⇒ ALLOW with zero moai cold-start.
[ "$MULTI_GATE_ENABLED" = "true" ] || exit 0
