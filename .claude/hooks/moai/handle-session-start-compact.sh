...
# This script forwards stdin JSON to the moai hook session-start-compact command.
# Infinite-goal guard: re-injects goal + SPEC context after auto-compact.
# Production firing rides on the deps.go SessionStart(compact) handler registration
# (fired by handle-session-start.sh -> moai hook session-start); this wrapper is
# the isolated-invocation / manual-debug surface (moai hook session-start-compact).
# Project-local hook: .claude/hooks/moai/handle-session-start-compact.sh
...
if command -v moai &> /dev/null; then
    exec moai hook session-start-compact 2>>"$MOAI_HOOK_STDERR_LOG"
fi

# Try default ~/go/bin/moai
if [ -f "$HOME/go/bin/moai" ]; then
    exec "$HOME/go/bin/moai" hook session-start-compact 2>>"$MOAI_HOOK_STDERR_LOG"
fi

# Try ~/.local/bin/moai (Linux install location)
if [ -f "$HOME/.local/bin/moai" ]; then
    exec "$HOME/.local/bin/moai" hook session-start-compact 2>>"$MOAI_HOOK_STDERR_LOG"
fi
