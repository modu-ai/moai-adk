...
class HooksTestSuite:
    ...
    def test_timeout_manager_timeout(self) -> None:
        ...
        try:
            ...
            try:
                ...
                duration = time.time() - start
                self.log_test_result("Timeout Manager - Timeout", True, "Graceful degradation working", duration)

            except HookTimeoutError:
                duration = time.time() - start
                self.log_test_result("Timeout Manager - Timeout", True, "Timeout exception raised correctly", duration)
    ...
    def test_git_operations_manager(self) -> None:
        ...
        try:
            ...
            duration = time.time() - start
            self.log_test_result("Git Operations Manager", True, f"Cached {len(project_info)} fields", duration)
