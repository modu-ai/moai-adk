...
class HooksTestSuite:
    ...
    def log_test_result(
        self, test_name: str, passed: bool, message: str = "", duration: float = 0
    ):
        """Log a test result"""
        ...
        print(
            f"{status} {test_name}{f' - {message}' if message else ''} ({duration:.3f}s)"
        )
    ...
    def test_timeout_manager_basic(self) -> None:
        ...
        if not TIMEOUT_MANAGER_AVAILABLE:
            self.log_test_result(
                "Timeout Manager - Basic", False, "Module not available"
            )
            return
        ...
        try:
            ...
            result = manager.execute_with_timeout(
                "test_hook", quick_task, config=config
            )
    ...
    def test_timeout_manager_timeout(self) -> None:
        ...
        if not TIMEOUT_MANAGER_AVAILABLE:
            self.log_test_result(
                "Timeout Manager - Timeout", False, "Module not available"
            )
            return
        ...
        try:
            ...
            try:
                result = manager.execute_with_timeout(
                    "test_timeout", slow_task, config=config
                )
    ...
    def test_git_operations_manager(self) -> None:
        ...
        if not GIT_MANAGER_AVAILABLE:
            self.log_test_result(
                "Git Operations Manager", False, "Module not available"
            )
            return
    ...
    def test_config_validator(self) -> None:
        ...
        if not CONFIG_VALIDATOR_AVAILABLE:
            self.log_test_result(
                "Configuration Validator", False, "Module not available"
            )
            return
    ...
    def test_hook_integration(self) -> None:
        ...
        try:
            ...
            duration = time.time() - start
            self.log_test_result(
                "Hook Integration - Session Start", success, message, duration
            )
            ...
            duration = time.time() - start
            self.log_test_result(
                "Hook Integration - Session Start", False, str(e), duration
            )
    ...
    def test_error_handling(self) -> None:
        ...
        try:
            ...
            if TIMEOUT_MANAGER_AVAILABLE:
                ...
                try:
                    result = manager.execute_with_timeout(
                        "test_error", failing_task, config=config
                    )
                    errors_tested += 1
                    # Should return graceful degradation result
                    if isinstance(result, dict) and "graceful_degradation" in str(
                        result
                    ):
                        errors_passed += 1
                except Exception:
            ...
            message = (
                f"Error handling: {errors_passed}/{errors_tested} scenarios passed"
            )
    ...
    def generate_summary(self) -> None:
        ...
        if GIT_MANAGER_AVAILABLE:
            try:
                ...
                print(
                    f"  Cache Size: {stats['cache']['size']}/{stats['cache']['size_limit']}"
                )
            except Exception:
                pass
