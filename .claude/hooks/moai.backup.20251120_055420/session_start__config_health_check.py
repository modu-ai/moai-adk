...
# Add lib to path for ConfigManager import
...
def should_suggest_update() -> bool:
    ...
    # Check version match (but don't suggest update for pre-release versions)
    is_matched, installed_version, latest_version, _ = check_moai_version_match()
    if not is_matched and installed_version and latest_version:
        # Only suggest update if installed version is actually older (not pre-release)
        try:
            from packaging import version
            installed_v = version.parse(installed_version)
            latest_v = version.parse(latest_version)
            if installed_v > latest_v:
                # Installed version is newer (pre-release), don't suggest update
                return False
            else:
                return True
        except Exception:
            # If version parsing fails, suggest update to be safe
            return True
    elif not is_matched:
        return True
