...
def generate_config_report() -> str:
    ...
    if not check_config_exists():
        report_lines.append("❌ Project configuration missing - run /alfred:0-project")
        return "\n".join(report_lines)
    ...
    if installed_version and config_version:
        if is_matched:
            report_lines.append(f"✅ Version match: {installed_version}")
        else:
            report_lines.append(
                f"⚠️  Version mismatch: config {config_version} vs installed {installed_version} "
                f" - consider running /alfred:0-project"
            )
