...
def _run_daily_analysis(project_root: Path, moai_dir: Path, analysis_date: datetime) -> AnalysisResult:
    ...
    Returns:
        AnalysisResult: Analysis operation result
    """
    # Import session analyzer from scripts
    scripts_dir = moai_dir / "scripts"
    session_analyzer_path = scripts_dir / "session_analyzer.py"

    if not session_analyzer_path.exists():
        return AnalysisResult(
            message="Session analyzer script not found - skipping analysis"
        )

    try:
        # Import and run session analyzer
        import sys
...
def _generate_daily_report(analysis_results: Dict[str, Any], analysis_date: datetime) -> str:
    ...
    """
    report_lines = [
        f"# Daily Session Analysis Report",
        f"",
    ...
    report_lines.extend([
        ...
        f"## Daily Insights",
        f"",
        f"1. Monitor tool usage patterns for daily optimization opportunities",
        f"2. Address any recurring error patterns immediately",
        f"3. Review frequently used tools for performance optimization",
        f"4. Track daily productivity and workflow efficiency",
        f"",
        f"---",
        f"*Generated by MoAI-ADK Daily Analysis*"
    ])
