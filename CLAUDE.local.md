...
## 21. Library Update Checklist

### [HARD] 릴리스 시 안정 버전 라이브러리 업데이트 체크리스트

모든 릴리스 전에 다음 체크리스트를 준수하세요.
...
# 안정 버전(stable)만 필터링 (pre-release/rc 제외)
go list -m -u all | grep -v "\-alpha\|\-beta\|\-rc"
```

#### 21.2 버전 유형 분류

**안정 버전만 업데이트** (SemVer 기준):
- **Patch 업데이트** (v1.2.3 → v1.2.4): 항상 안전, 무조건 업데이트
- **Minor 업데이트** (v1.2.3 → v1.3.0): 안정적, 테스트 후 업데이트
- **Major 업데이트** (v1.2.3 → v2.0.0): Breaking changes 확인 필요, 주의 필요

**금지 버전** (업데이트 보류):
- Pre-release: `-alpha`, `-beta`, `-rc` 접미사
...
§18.8 Release 프로세스에 이 체크리스트를 추가:

**Pre-Release (tag 생성 전)**:
1. [ ] `go list -m -u all` 실행으로 업데이트 가능 확인
2. [ ] 안정 버전(stable)만 필터링
3. [ ] Major 업데이트 있을 경우 Breaking Changes 확인
4. [ ] `go get -u ./...` 로 업데이트 실행
5. [ ] `go test ./... -race` 로 호환성 확인
6. [ ] 업데이트 커밋 생성 (위 21.4 형식)
7. [ ] 그 다음 릴리스 단계 진행

**주요 원칙**:
- **안정 버전만** 업데이트 (pre-release 제외)
- **테스트 필수** (`go test ./... -race` PASS 기준)
- **Major 업데이트**는 Breaking Changes 확인 후 진행
- **목표**: 모든 릴리스에 최신 안정 버전 반영

#### 21.6 Dependabot와의 관계

Dependabot는 주간 자동 PR을 생성하지만, **수동 릴리스에서는 이 체크리스트를 우선**합니다:

- Dependabot: 주간 자동 업데이트 PR (patch 위주)
- 릴리스 체크리스트: 전체 업데이트 + minor/patch 안정 버전

Dependabot PR은 CI pass 시 자동 머지 (`.github/workflows/auto-merge.yml`).
