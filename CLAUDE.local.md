...
| 측정 | 값 | 비고 |
|---|---|---|
| 3-class 정확도 (한국어 원문 124장) | **31.5%** | t943 M1 |
| 같은 축, 관측 설계 수리 후 | **47.6%** | t943 M2 |
| `premise_dead` 정밀도 | **26% → 29%** | M1 → M2 |
| 「항상 not-dead」 상수 응답 기준선 | **75%** | 모델보다 높다 |
| 신뢰도 게이트 0.30~0.80 | 전 구간 30-33% 평평 | 게이트를 올려도 회복되지 않음 |

종전에 기대던 성질(「오답은 전부 낮은 신뢰도에 몰린다」)도 깨졌다 — 오답 85 건 중 67 건이 0.30 을
넘었다. 즉 **높은 신뢰도가 정답률을 보증하지 않는다.**
