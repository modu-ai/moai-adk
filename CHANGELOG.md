...
- **[SPEC-HUMANIZE-002](.moai/specs/SPEC-HUMANIZE-002/spec.md)** — `moai-domain-humanize` 스킬 **v1.1.0 → v1.2.0**: claude.mo.ai.kr slop-review 스킬 지식을 2개 신규 모듈로 이식 (Tier M, v0.1.0, 문서 저작 SPEC — Go 코드 변경 0). **(1) `copy-review.md`** — CRS-1..7 리뷰 파이프라인(Stage 1-6: language detection → context → tell detection → fix-proposal(Original/Reason/Alternatives≥3/Preferred) → severity report), review-only 기본 모드(auto-apply 금지, 사용자 검토 후 적용) 및 4개 언어 slop 사전(CR-KO×8, CR-EN×9, CR-JA×4, CR-ZH×5 — 각 항목 부모 카탈로그 cross-ref, ungrounded 항목 0건); **(2) `design-copy.md`** — 랜딩/숏폼 카피 장르 가이드(헤드라인/CTA/카드/슬라이드/커버), 언어별 길이 휴리스틱(KO 자수/EN 단어수/JA 문자수+낭독성/ZH 자수+밀도), S1-S5 시나리오 검증(팩트 앵커 보존, 언어별 report severity 태깅); **(3) SKILL.md Genre-Module Routing** — 신규 2모듈을 기존 4개 언어 모듈과 연계하는 라우팅 테이블 추가; **(4) catalog.yaml** humanize entry `1.1.0 → 1.2.0` + hash regen (`gen-catalog-hashes --all`, 2-line diff only). **24/24 AC PASS** (acceptance.md SSOT — 13 mechanical / 2 hybrid / 9 manual; 언어 모듈 8개 byte-freeze 검증(`git diff` 0 files), template↔local parity IDENTICAL, neutrality 7-class grep PASS + `TestTemplateNoInternalContentLeak` PASS, MIT 잔재 0). plan-audit PASS. Run-phase (manager-develop, 격리 worktree branch M1-M3 `08cf1ca76`/`a5c713a97`/`199c5c1a8` → fast-forward merge `6b96377e9`로 main 편입; Template-First 양쪽 트리 `diff -rq` IDENTICAL, cross-platform build green darwin+windows). Pre-existing debt: `TestOutputStylesTemplateLiveParity`(moai-easy.md parity, 별도 chore, humanize scope 무관). README 무변경 — 내부 스킬 콘텐츠 확장, 사용자 가시적 CLI surface 변경 0. 🗿 MoAI
