...
### Added
- **SPEC-V3R6-V2-V3-CLEAN-REINSTALL-001 — v2-to-v3 Clean Reinstall paradigm shift + v.2.x FLAT layout restoration** (Tier M, v3.0.0-rc2 target). Replaces the file-level enumeration `moai update` flow with a version-aware **backup → REMOVE → reinstall → MERGE-back → integrity verify** seven-step canonical order for projects detected as v2.x via the new `detectV2Fingerprint` heuristic (3 signals: `.moai/config/sections/system.yaml` `moai.version` field reads as `v2.*` / empty / file-missing → Signal 1; `.agency/` legacy directory presence → Signal 2; any of 43 enumerated `DeprecatedPaths` hits → Signal 3). Reverts the rc1-stage SPEC-V3R6-AGENT-FOLDER-SPLIT-001 `{core,expert,meta}/` subdirectory split (commit `1bd083725`, 2026-05-22) to the canonical v.2.x FLAT `.claude/agents/moai/<agent>.md` layout — verified by `git ls-tree -r 1bd083725^`. Supersedes SPEC-V3R6-AGENT-FOLDER-SPLIT-001. 22 AC-VVCR (17 AC-VVCR + 5 AC-VVCR-LR), 100% bidirectional REQ↔AC traceability (29 REQ-VVCR + 6 REQ-VVCR-LR = 35 REQs), 7 HARD constraints, 7 OOS exclusions. Plan-auditor iter-3 PASS skip-eligible 0.91 (Tier M threshold 0.80 +0.11; trajectory iter-1 0.83 → iter-2 0.89 → iter-3 0.91 monotone). Version bump: `pkg/version/version.go` `Version = "v3.0.0-rc1"` → `Version = "v3.0.0-rc2"` + `.moai/config/sections/system.yaml` `moai.version` / `template_version` synchronized.

### Fixed
- **SPEC-V3R6-TEMPLATE-INTERNAL-ISOLATION-001: Template 내 moai-adk 내부 개발 토큰 영구 격리** (Tier M, Sprint 10 cohort 8/8, 완료): moai-adk dev-internal-content 토큰 (SPEC ID / REQ token / Audit 인용 / 개발 날짜 / commit sha) 35개 파일에서 영구 제거. **5개 deliverable**: (a) templates 35→0 leak files cleanup (5-class pattern: SPEC-ID literal + REQ token + Audit citation + archive date + commit sha full/short), (b) CLAUDE.local.md §25 NEW HARD rule + 5-item self-check checklist (≥80 lines body), (c) Go lint test `TestTemplateNoInternalContentLeak` 신규 (red-green proof + allowlist 메커니즘 + D-007 short-sha trailing-space pattern), (d) CI workflow extension (docstring + linting cross-reference), (e) maintainer-only audit 통과 ({{.GoBinPath}} 하드코딩 스캔, output-styles scope-exclusion, allowlist 4-class false-positive 차단). **AC 결과**: 12 AC 매트릭스 8 PASS + 2 PASS-WITH-VARIANCE (AC-TII-001 scope-excluded 4 files 의도된 allowlist + AC-TII-007 5 occurrences in output-styles scope-excluded) + 1 PASS-WITH-DEBT (output-styles scope-exclusion 정책) + 1 N/A (AC-TII-010 조건부). **Run-phase 구조**: 13 REQ-TII (100% GEARS Ubiquitous 4 + When 2 + While 2 + Where 2 + Unwanted 3) + 12 AC-TII bidirectional traceability + 5 substitution pattern 일관성 (predecessor pass 1/2 호환) + CLAUDE.local.md §25 pedagogical allowlist 메커니즘 정착 (future SPECs pattern 외 자산 식별용). **L52 case 29 2회차 mitigation**: M1 content + attribution hijack (race-absorbed `d9838995d`, 비파괴 attribution chore `c5ed59907` corrected), M4.2-redo 후속 attribution fix 재현 (scope-disjoint fast-forward 7회 누적). **검증**: `grep -rln 'SPEC-V3R6-\|REQ-ATR-\|Audit 3\|Finding A[1-6]\|archive-2026-05-25'` 0 (35→0, allowlist 4-class 외), self-check 5-item ✓, coverage internal/template 84.6% ≥85% requirement near, lint `TestTemplateNoInternalContentLeak` PASS + synthetic-leak FAIL (red-green proof), CI gate extension 적용, output-styles/moai/moai.md 1 occurrence scope-excluded (외부 합성 rule flow로 판단), CLAUDE.local.md `.gitignore` false-positive (allowlist row AC-TII-001 table에 명시). **선례 패턴**: TEMPLATE-INTERNAL-ISOLATION governance discipline은 사용자 명시 정책 (feedback_template_internal_content_isolation.md 2026-05-25 기록) → 본 SPEC으로 HARD rule 정착 → CLAUDE.local.md §25 신규 추가 → 향후 SPECs 자동 audit (lint gate로 강제 불가능하나 정책 명시로 팀 동기화, 예: §21 dev-only commands, §24 harness namespace). **이하 레벨**: Tier M 정책-규칙화 → 단순 governance signal(향후 compliance sweep 후보).
