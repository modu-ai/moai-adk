...
- **[SPEC-HOOK-DEADCODE-001](.moai/specs/SPEC-HOOK-DEADCODE-001/spec.md)** — `internal/hook` package dead-code cleanup, 3 independently-corroborated scopes (Tier M, `internal/hook` + `internal/cli` — 12/12 AC PASS). Each scope required TWO independent absence signals (`go list -deps` / `deadcode` / registry-registration grep) before deletion, per `verification-claim-integrity.md` — a `deadcode`-only signal on ~120 registry-dispatched handlers elsewhere in the package is a false-positive class explicitly left untouched (Out of Scope). **M1** (`43c996bb7`) deletes `internal/hook/agents/` + `internal/hook/lifecycle/` (18 files, 13 production + 5 test, 2874 deletions) — a never-wired agent-specific handler architecture absent from the `moai` binary's dependency graph (`go list -deps`) and from `registry.go`'s registration; corrects the dangling `internal/hook/agents/factory.go` doc reference in `agent-hooks.md` (live + template mirror) to describe the actual generic `EventType`-based dispatch. **M2** (`f7bfa86a9`) deletes `internal/hook/dual_parse.go` (172 LOC — 5 `deadcode`-confirmed unreachable functions + 2 orphaned package-level error vars) and its dedicated test file, plus 3 dependent test functions in `response_test.go` that exercised the same dead functions; removes the now-dangling `ParseHookOutput` doc-comment reference in `response.go`. `HookResponse` (the type these functions operated on) and its 9 live consumer references in `internal/permission/resolver.go` are explicitly PRESERVED and unmodified. **M3** (this session, orchestrator-direct) removes the dead `input.Data = actionJSON` injection in `internal/cli/hook.go`'s `runAgentHook` — `HookInput.Data` has zero readers repo-wide, confirmed by a repo-wide grep excluding the distinct, live `HookOutput.Data` field (written via `NewAllowOutputWithData`, never conflated); the field *declaration* itself is deliberately retained as a documented follow-up. Also corrects a pre-existing `agent-hooks.md` staleness (adds the missing `sync-auditor` → `evaluator-completion` row) in both the live copy and the template mirror. **Zero behavior change**: the `moai hook agent <action>` subcommand, `.claude/hooks/moai/handle-agent-hook.sh(.tmpl)`, and all 4 agent-frontmatter `hooks:` registrations (`manager-develop`, `manager-docs`, `manager-spec`, `sync-auditor`) remain fully functional and untouched — an earlier evidence framing had suggested the wrapper was orphaned, but independent re-verification found it live and load-bearing for 4 production agents (see spec.md §A.4). `go build ./...` and `go test ./...` green after each of the 3 milestones independently; net LOC delta strongly negative (~3500 lines removed across the SPEC). README unchanged — pure internal dead-code removal with no user-facing CLI surface change. 🗿 MoAI

### Added

- **[SPEC-MODEL-TIER-PLANTYPE-001](.moai/specs/SPEC-MODEL-TIER-PLANTYPE-001/spec.md)** — plan_type-aware model tier profiles (API vs subscription) + GLM backend effort overlay (Tier L, v0.3.1, `internal/config` + `internal/template` + `internal/cli` + `internal/web` + `internal/tmux` — 29 REQ / 31 AC groups / 43 sub-criteria). Splits the per-agent model policy by billing context: DeepSWE-leaderboard evidence shows the optimal {model, effort} assignment differs fundamentally between API-metered billing (dollars are the constraint — Opus/Fable win $/solved-task) and subscription billing (weekly Opus-weighted quota — the opusplan pattern wins), so one matrix can no longer serve both. **M1 (`539aa3e3f`)** adds the `llm.plan_type` config field (closed set {`api`, `subscription`}, `LLMConfig.PlanType` + `EffectivePlanType` resolving absent/empty → `subscription` for backward compat), joins `"fable"` to the `validRoutingModels` closed set (accepted by BOTH `ValidateModelRouting` flat-map and `ValidateModelRoutingProfiles` paths) and corrects the stale `inherit, haiku` error-message literal (No-Haiku), with template `llm.yaml`/`workflow.yaml` twins carrying the neutral `plan_type` key + `fable` closed-set comment. **M2 (`374de51fd`)** replaces the dual `agentModelMap` (`[3]string`) + `agentEffortMap` two-pass representation with a single (plan_type × tier × agent) profile structure yielding one atomic {model, effort} pair per agent — encoding the Plan A rev2 (§B.6) and Plan B (§B.7) matrices verbatim (60 cells, all 10 retained agents explicit; auditors + manager-design + super-advisor move off implicit-inherit; Explore stays `inherit`) — and a single `ApplyTierProfile` pass (D1 replace-both precedence: the tier profile is SSOT for shipped-agent model/effort, since 9/9 shipped agent files carry `effort:` frontmatter the historical preserve-existing rule would have made the whole effort matrix inert) wired at both production call sites (`initializer.go` + `update.go`); the old `ApplyModelPolicy`/`ApplyEffortPolicy` and the dangling `EFFORT-MAP-RETIREMENT` deferral pointer are removed (net −252 lines). **M3 (`98db6a9d4`)** adds `moai init --plan-type` (closed-set validation) + a wizard plan-type question (recommended default `subscription`) + `moai update --plan-type` override (reads persisted `llm.plan_type` by default; an explicit flag takes precedence for the run and persists); the legacy `template.ModelPolicy` {high,medium,low} mapping is tier-only (high→max, medium→medium, low→low — never selects a plan). **M4 (`2dd252ae7`)** makes the `moai web` `/model-policy` view render a persisting plan_type selector + a per-tier {model, effort} preview for BOTH plans (structure-derived from the Go profile, no second matrix literal), backed by exactly ONE scoped write path `POST /model-policy/plan-type` (a deliberate, user-approved single-field exception to the SPEC-WEB-CONSOLE-013 REQ-WC13-021 read-only doctrine — closed-set validation, out-of-set 4xx leaves `llm.yaml` byte-identical, page route keeps 405 on non-GET, no other field writable) with 4-locale i18n (en/ko/ja/zh) and regenerated `modelpolicy_templ.go`. **M5 (`6e90acbd4`)** adds a GLM backend effort overlay (NOT a third plan_type — plan_type stays {api, subscription}) that remaps ONLY the per-agent effort produced by the active plan_type profile when the session backend is GLM (predicate `LLMConfig.TeamMode ∈ {cg, glm}` — the actual persisted `persistTeamMode` signals — OR the defensive dormant `Mode == glm`, cross-referencing `internal/tmux/cg_detect.go` `IsCGMode` rather than re-implementing it), collapsing Claude's 5-level effort to z.ai's binary-thinking + 2-level `reasoning_effort` state via named constants (low→thinking-disabled; medium/high→reasoning-high; xhigh/max→reasoning-max) with a coding-max override set {`manager-develop`, `builder-harness`} forcing `reasoning-max`; the model dimension is untouched (already carried under GLM by `llm.glm.models`). The overlay is WIRED into the GLM launch path via a shared `glmReasoningEnvVars()` helper called from BOTH `setGLMEnv` (process env) and `injectGLMEnvForTeam` (`settings.local.json`), delivered as **Branch B** (explicit `ANTHROPIC_REASONING_EFFORT` env write) with inject↔clear parity in `buildTmuxClearVars()` (REQ-CGH-009). **KNOWN RESIDUAL (verification-claim-integrity, AC-MTP-032b PASS-WITH-RESIDUAL)**: the GLM overlay code + wiring + tests are complete, but whether z.ai actually consumes `ANTHROPIC_REASONING_EFFORT` through the Anthropic-compat shim (Branch A passthrough vs Branch B request-body requirement) is EMPIRICALLY UNVERIFIED — a live GLM session with outbound-request inspection is required to settle it; the overlay is NOT claimed effective on the wire. Delivery granularity is session-global (not per-agent) per research.md §D — the per-agent collapse logic is defined and unit-tested regardless. **31/31 AC groups PASS** (30 clean + AC-MTP-032b PASS-WITH-RESIDUAL; AC-MTP-001..024, 026..032 + a/b sub-criteria = 43 verification line items; AC-MTP-025 retired with the D5 36-cell-derivation descope to follow-up SPEC-MODEL-TIER-ROUTING-PROFILES-001). Touched-package suites green (`internal/{config,template,web,cli,core/project}`), `go build ./...` exit 0, `moai spec lint` 0 SPEC-attributable errors (the pre-close `StatusGitConsistency` draft-vs-implied warning is resolved by this close), coverage — changed FILES ≥85% (plan_type.go 100%, glm_effort_overlay 7 fns 100%, internal/template pkg 85.1%); internal/config package aggregate 80.9% is a pre-existing baseline, not SPEC-attributable. Run-phase Route A main-direct (Hybrid Trunk, Tier L, 1-person OSS), Mode 5 sub-agent sequential (M1/M2 foundations gate M3-M5; Template-First `make build` cadence is inherently sequential). Design SSOT `.moai/reports/model-tier-redesign-20260712.md` (rev2, user-approved) is a local-only dev artifact (CLAUDE.local.md §2), excluded from the commit. README/docs-site 4-locale plan_type documentation deferred as a follow-up per spec.md §C — no user-facing README CLI surface regression. Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh; `sync_commit_sha` backfilled in a follow-up). 🗿 MoAI

- **[SPEC-LOOP-SWEEP-001](.moai/specs/SPEC-LOOP-SWEEP-001/spec.md)** — `/moai loop` redefined as a project-wide improvement-sweep goal preset built ON the goal engine (Tier M, docs-heavy — no new Go engine; Epic AGENTIC-CORE, SPEC 3 of 3, depends on `SPEC-GOAL-ENGINE-001`). `/moai loop` no longer reimplements "iterate until done" as bespoke prose machinery — its scan stage still supplies a finite issue queue (LSP + lint + test failures + review lenses), but the iterate-until-clean decision now delegates to the goal engine (`/moai goal`, `stop-goal`) whose completion condition is "issue queue drained AND diagnostics clean", bounded by an iteration ceiling. `internal/loop` and `internal/ralph` engine code is UNCHANGED (empty-diff guard verified) — this is a documentation/workflow-prose redefinition, not an engine rewrite. Touched: `.claude/skills/moai/workflows/{loop,fix,review}.md`, `.claude/rules/moai/workflow/{goal-directive,spec-workflow}.md`, `CLAUDE.md`, `internal/cli/loop.go`, `.claude/skills/moai-workflow-loop/SKILL.md` + 7 template mirrors under `internal/template/templates/` + regenerated `internal/template/catalog.yaml`. **20/20 AC PASS** (AC-LSW-001..014 with 001b/002b/004b/008b/009b/013b split-outs, acceptance.md SSOT) — orchestrator-independently re-verified on the main integration tree post-landing (composition-reachability grep, empty-diff guard, template-neutrality grep, live==mirror parity). `go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0, touched-package tests green, `golangci-lint` 0 issues. Run-phase landed via an isolated L1 worktree; the worktree base had gone stale relative to `main`, so the orchestrator integrated via a net patch to `main` + `make build` catalog regen rather than a range cherry-pick. Mode 5 sub-agent sequential (Tier M docs-heavy rewrite, not multi-domain research nor high-volume mechanical transform). Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh; `sync_commit_sha` backfilled in a follow-up). 🗿 MoAI

- **[SPEC-ANALYZE-FIRST-ROUTING-001](.moai/specs/SPEC-ANALYZE-FIRST-ROUTING-001/spec.md)** — Analyze-First routing reform — language-independent intent analysis as default `/moai` routing (Tier M, docs/config only, no Go code; Epic AGENTIC-CORE, SPEC 1 of 3, entry SPEC with `GOAL-ENGINE-001` + `LOOP-SWEEP-001` depending on this). **Five deliverables**: (D1) CLAUDE.md §2 rewrite — Analyze-First pipeline as the default main-session behavior (5 ordered stages: ① intent analysis → ② context-sufficiency → ③ execution-plan → ④ approval → ⑤ execute/verify), ~15 lines compact prose, removed stale "Detect technology keywords" phrasing. (D2) `/moai` SKILL.md Intent Router P3 language-independence clause — cue words are English exemplars, not literal-match requirements; intent classified for any `conversation_language`. P1 fast-path unchanged. P3 `lint` collision resolved — assigned to `fix` bucket (gate retains "quality gate / pre-commit / check"). (D3) Agent description diet — 9 `.claude/agents/moai/*.md` bodies: replaced "MUST INVOKE when ANY…" + 4-language keyword blocks with concise semantic scope prose + "Match user intent language-independently — do not require literal keyword matches." Total trigger-block character count reduced from 139,860 → 134,862 (delta −4,998). `manager-design.md` gained equivalent trigger/scope prose. (D4) Hygiene — fixed `moai-foundation-core` `related-skills` drift (adopted template value), updated `skill-authoring.md` `triggers:` section to reflect semantic-matching reality, removed dead `team/*.md` references. (D5) Template-First mirror — 9 template agents carry diet line; 0 stale phrases in template CLAUDE; 2 language-independent lines in template CLAUDE; 0 internal SPEC IDs leaked (`grep -rn SPEC-ANALYZE-FIRST|SPEC-GOAL-ENGINE|SPEC-LOOP-SWEEP|AGENTIC-CORE|REQ-AFR` template tree` = 0); `make build` exit 0. **17/17 AC PASS** (AC-AFR-001..017, acceptance.md SSOT). Cross-platform build green (`go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0). CLAUDE.md char count 26,432 (< 40,000 limit). `go test ./...` exit 0, `golangci-lint run` 0 issues. Run-phase changes: 14 local + 13 template mirror + 1 catalog.yaml (28 total files). Mode 5 sub-agent sequential (single-domain instruction-doc editing, bespoke prose per file). README.md/README.ko.md unchanged — no user-facing CLI surface change; this SPEC touches only internal routing workflow prose. Docs-site 4-locale sync deferred as follow-up per spec.md §Out of Scope. Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh). 🗿 MoAI

- **[SPEC-HARNESS-EVOLVE-001](.moai/specs/SPEC-HARNESS-EVOLVE-001/spec.md)** — Routing Observation Ledger, Loop 0 (Generator) of the self-evolving harness (Tier M, v0.1.2, `internal/harness/routing` + `internal/cli` + `.claude/skills/moai` — 21 REQ / 27 AC, Epic HARNESS-EVOLVE 1/5, entry SPEC with no `depends_on`). New greenfield package `internal/harness/routing/` (types, digest, outcome, pending store, writer/reader) appends a schema-v1 `routing-ledger.jsonl` recording orchestrator routing decisions (matched subcommand, convergence class, delegation chain, non-verbatim SHA-256 request digest) — kept explicitly SEPARATE from the pre-existing `usage-log.jsonl`/`harness.Event` observation sink (AC-HEV-012/013, zero cross-references). New `moai harness ledger {record,evidence,list}` CLI verbs (`internal/cli/harness_ledger.go`) — `record`/`evidence` carry NO `--outcome` flag by design (AC-HEV-011): outcome is derived only by `DeriveOutcome` (fixed precedence abort > fail > success > pending, no manual override) running inside the existing HOI-gated Stop-hook finalizer (`internal/cli/hook.go`, 1 registration site), keeping `outcome` un-fakeable by prose. Routing capture inherits both pre-existing harness-observe-family gates unchanged (gate 0 `isHookOptInEnabled`, fail-closed default OFF; gate 1 `isHarnessLearningEnabled`, fail-open default true) — no new gate, no default flip; Stop-path `success`/`fail` finalization stays dormant under default shipped config, with reroute/lazy-abort-sweep covering the record-time path regardless. Recording obligations wired into `.claude/skills/moai/{SKILL.md,workflows/{plan,run,sync}.md}` plus their 5 template mirrors (byte-parity + neutrality-clean, `grep -c SPEC-HARNESS-EVOLVE-001 internal/template/templates/` = 0). **27/27 AC PASS** (AC-HEV-001..027, acceptance.md SSOT; 34 verification line items via 022a-d/023a-e sub-lettering). `internal/harness/routing` coverage 91.6% (≥90 target), `-race` clean on concurrent-append, `golangci-lint` 0 new issues, subagent-boundary grep 0 (`AskUserQuestion`/`mcp__askuser` absent from `routing/` + `hook.go` non-test code), cross-platform build green (darwin/windows/linux `go build`/`GOOS=windows go build` exit 0), full suite `go test ./...` exit 0 with 0 existing test modified, `.gitignore` `.moai/state/` rule pre-existing (no touch needed for the new ledger path), `moai harness ledger --help` exit 0 listing all 3 verbs. Local dev-tree M4 additionally flips `.moai/config/sections/system.yaml` `hook.opt_in.enabled: false → true` to dogfood-arm the Stop-path locally (template default stays `false`, unaffected). Run-phase M1-M4 landed on an isolated worktree branch (clean FF-descendant of local main), orchestrator-integrated to `main` (`728e088db`), Mode 5 sub-agent sequential (milestone dependency chain: M1 core types → M2 CLI+hook → M3 skill wiring → M4 verify forbids fan-out). Residual risk: Stop-path `success`/`fail` finalization is exercised by `TestHarnessObserveStop_RoutingLedgerGated` (unit) but not yet by a live Claude Code Stop event this run — the dogfood-enable arms it for future live sessions. README unchanged — the new `moai harness ledger` verbs are internal self-evolving-harness observation infrastructure (dormant by default, no user-facing workflow change); reviewed and explicitly out of scope per spec.md. Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh). 🗿 MoAI

- **[SPEC-HARNESS-EVOLVE-002](.moai/specs/SPEC-HARNESS-EVOLVE-002/spec.md)** — Curator Editable Surfaces, Loop 2 (write layer) of the self-evolving harness (Tier L, `internal/harness/curator` + `internal/harness/layer3.go` + `internal/merge/strategies.go` + `internal/template/templates/CLAUDE.md` — 36 REQ / 53 AC, Epic HARNESS-EVOLVE 2/5, depends_on SPEC-HARNESS-EVOLVE-001). New greenfield package `internal/harness/curator/` (writer, marker, crud, budget, tier, approval, recall, antifabrication, append, rollback) provides the typed managed-block writer API `WriteManagedBlock` + single-line bullet CRUD (Add/Update/Delete by `ledger_key`) for the `MOAI:LEARNED-WORKFLOW` digest block in CLAUDE.md and the append-only `LOCAL` learned section in CLAUDE.local.md. Enforcement contracts: ≤3K-char digest budget (`ErrDigestBudgetExceeded`), ≤20-bullet cap (`ErrBulletCapExceeded`), anti-fabrication rejection of internal SPEC-ID/REQ-token/digest/SHA patterns in bullet text (locale-neutral — CJK prose preserved), idempotent re-application (byte-diff 0 on identical re-apply), byte-preservation of pre/post-block content, append-only dedup (`ErrDuplicateAppend` on same ledger_key). Tier-differentiated writes: Tier 3 → CLAUDE.local.md append surface, Tier 4 → CLAUDE.md managed block, `ErrTierNotQualified` on under-tier attempt; L5 orchestrator approval-token gate (rejection records lineage + no file write). Snapshot/rollback extended to per-surface restore units (`LineageEntry` carries `LearnedSurface`/`BulletsChanged`/`SnapshotDir`; manifest carries ≥2 distinct `learned_surface` values on dual-surface snapshots); `mergeSectionBased` consults a new `managedSectionHeadings` allow-list to preserve populated learned blocks across template sync (no silent clobber — conflict surfaced, not auto-resolved). Template-First: empty `MOAI:LEARNED-WORKFLOW` marker ships in `internal/template/templates/CLAUDE.md` (AC-HEV2-052), embedded via `make build`; template-neutrality clean (AC-HEV2-038). 2-layer recall contract (digest-layer summary-only + ledger-layer search) with cross-layer linkage (null ledger_key → provisional marker). **53/53 AC PASS** (AC-HEV2-001..053, acceptance.md SSOT, incl. 6 cross-file reachability ACs 047..052). `internal/harness/curator` coverage 91.0% (≥90 target), `golangci-lint` 0 issues, cross-platform build green, subagent-boundary grep 0 (`AskUserQuestion`/`mcp__askuser` absent from `curator/` non-test code, AC-HEV2-044), no new hook surface (settings.json hook-registration unchanged + no new wrapper, AC-HEV2-045/046). Residual risk: `TierGatedWrite`/`WriteManagedBlockGated` are the completed write-layer API surface; their production wiring (tier→surface activation from harness config; L5 orchestrator approval round) is EVOLVE-003+ scope, explicitly out of scope per spec.md §E. Run-phase M1-M7 landed on isolated worktree `worktree-evolve-002` (clean FF-descendant of main; M1-M7 + SHA backfills direct-pushed to main per Hybrid Trunk), Mode 5 sub-agent sequential (milestone dependency chain: M1 writer → M2 digest/budget → M3 local-append → M4 merge-preserve → M5 snapshot/lineage → M6 template/recall → M7 integration). README unchanged — curator is internal self-evolving-harness infrastructure with no user-facing CLI surface; reviewed and explicitly out of scope. Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh). 🗿 MoAI

- **[SPEC-HARNESS-EVOLVE-003](.moai/specs/SPEC-HARNESS-EVOLVE-003/spec.md)** — Curator Production Wiring, Loop 2 activation of the self-evolving harness (Tier L, `internal/harness/curator_dispatch.go` + `internal/harness/curator/{dispatch,canary_shadow}.go` + `internal/harness/negative_evidence.go` + `internal/harness/safety/{frozen_rules,contradiction,frozen_guard,pipeline}.go` + `internal/config/auto_detection.go` + `internal/harness/types.go` — 35 REQ / 35 AC, Epic HARNESS-EVOLVE 3/5, depends_on SPEC-HARNESS-EVOLVE-002). Activates the PRODUCTION write path that EVOLVE-002 deliberately left INERT — the completed write-layer API surface (`TierGatedWrite`/`WriteManagedBlockGated`) now has a live Curator caller driving it (AC-HEV3-008/009, 0 → 1 production caller each, both in `internal/harness/curator_dispatch.go:176` and `:187`; criterion is ≥1 per acceptance.md, independently re-verified by orchestrator before sync). Seven pillars: A1 Frozen-rule registry expansion (`frozenPrefixes` 3 → 7 entries, guard-source self-protection, AC-HEV3-023/024/025); A6 Tier↔Surface mapping with value-range validation (`EvolvableSurfaces[auto_detection]` registered Tier 4 IsConfigEdit, `ValidateAutoDetectionConditions` rejects `file_count<=0` and `<=99999`, cross-surface pairing rejected, AC-HEV3-001..004); A7 negative-evidence registry (`NegativeEvidence` struct + Append/Read round-trip, `cooldown_until` never null/9999 = no permanent suppression, AC-HEV3-018/022); L3 contradiction activation (replaces the no-op `return ContradictionReport{}` with live `DetectFrozenRuleContradictions`, RejectedBy==3 behavior, reason cites "frozen-moai-rules", AC-HEV3-015..017/033); L2 shadow-apply canary for the Curator path (`ShadowApplyCanary` evaluates in-memory without touching file, budget-exceeding block rejected, veto→A7 outcome="rolled-back", AC-HEV3-012..014/034); production wiring breakthrough (approval path sole-writes via `WriteManagedBlock`, rejection path records via `RejectionRecorder` on DISJOINT branches — never both on the same branch, AC-HEV3-005..011); GLM observe-only gate (`IsGLMSession`/`model_class` in dispatch layer, AC-HEV3-026/027); anti-fabrication pattern-key validation rejects SPEC/REQ/AC tokens (AC-HEV3-028); no dead-code/feature-flag guards around the wiring (AC-HEV3-011); no new hook surface (AC-HEV3-032). **35/35 AC PASS** (AC-HEV3-001..035, acceptance.md SSOT; independently re-verified by orchestrator before sync: `go build ./...` exit 0, `GOOS=windows GOARCH=amd64 go build ./...` exit 0, `go test ./internal/harness/{curator,safety,harness}/` all ok, `golangci-lint run --timeout=3m` `0 issues.`, subagent-boundary grep 0 `AskUserQuestion` in new production code AC-HEV3-031). Coverage: `internal/harness/curator` 91.4%, `internal/harness/safety` 86.8%, `internal/harness` 87.3% (all ≥85% package target, AC-HEV3-030a). Run-phase M0-M6 recovered from backup tag `recovery-backup-20260713` (2a2368454) after a main history rewrite, integrated onto restored main (8f4d09c7e base) as single commit `d61399742` — `go build`/`go test` green post-integration. Mode 5 sub-agent sequential (milestone dependency chain: M0 frozen expansion → M1 negative-evidence → M2 tier-surface mapping → M3 L3 contradiction → M4 L2 canary → M5 production wiring → M6 integration). Ownership note: the `draft → in-progress` frontmatter transition that manager-develop should have performed on the M1 run commit was skipped during the worktree-recovery integration; this sync commit transitions `draft → completed` in one step, with the ownership gap recorded honestly in progress.md §E.4. README/CLAUDE.md surface unchanged — curator production wiring is internal self-evolving-harness infrastructure with no user-facing CLI surface (same as EVOLVE-001/002); reviewed and explicitly out of scope per spec.md §E. Sync-phase 3-phase close (this commit — `draft → completed` merged into the single sync commit + `updated:` refresh; `in-progress` intermediate step skipped per the ownership-gap note above). 🗿 MoAI

- **[SPEC-AGENT-PROGRESS-PUSH-001](.moai/specs/SPEC-AGENT-PROGRESS-PUSH-001/spec.md)** — Dual-channel agent interim progress reporting + orchestrator narration + background-default realignment (Tier L, `.claude/agents/moai` (9 agents) + new SSOT `.claude/rules/moai/core/progress-reporting-protocol.md` + `agent-common-protocol.md`/`moai-constitution.md` + `worktree-integration.md`/`zone-registry.md` + `internal/template/templates` mirror + `rule_template_mirror_test.go` allowlist — 37 REQ / 50 AC, 29 files: 9 agents × 2 trees). Closes the interim-progress gap — a long-running delegated agent gave the user NO signal during the run, only a terminal "Waiting for 1 background agent" line. Root cause was an allowlist omission, NOT a platform limit: MoAI agents declare an explicit `tools:` list that simply omitted the progress-carrying tools. **Dual-channel contract**: primary = `TaskCreate`/`TaskUpdate`/`TaskList` (officially documented, durable — the load-bearing channel); secondary = `SendMessage({to:"main"})` (undocumented runtime behavior — exists only in the runtime tool schema, NOT in the docs — best-effort immediacy layer that degrades from immediate to pull-visible if a runtime update breaks it, never an outage). The agent registers milestones with `Task*` and pushes an `[n/N]` status line via `SendMessage` at each boundary (≤6 pushes/run, milestone-only — never per-tool-call); the orchestrator narrates + relays in `conversation_language` and does NOT idle immediately after a background spawn (queued pushes drain at the next tool-call boundary). **Background-default realignment (new scope)**: as of Claude Code v2.1.198 subagents run in the background by default, but 4 MoAI doctrine surfaces still asserted the pre-flip "background-write-ban" world — realigned `CLAUDE.md §14`, `agent-common-protocol.md § Background Agent Execution`, `worktree-integration.md`, and amended 2 constitution clauses (`CONST-V3R2-020`/`044`) from a backgrounding-based safeguard to a better-targeted concurrency-based one (never run two write-capable agents concurrently; orchestrator work concurrent with a write-capable agent is read-only — the actual file-write-race hazard the old ban never addressed). **Measured on v2.1.207** (AC-APP-037b, not inferred): `SendMessage({to:"main"})` delivery observed from BOTH background and foreground subagents — the runtime schema's "background subagents only" annotation is NOT enforced at the tool-call layer; foreground push succeeds too. **50/50 AC PASS** (AC-APP-001..037 with a/b sub-lettering = 50 verification line items; acceptance.md SSOT, with §A shared `sec`/`seccount` helpers that eliminate the vacuous-compound-grep and `-A N` window-truncation failure modes this repo has hit). Independently re-verified by orchestrator on the main integration tree before sync: SendMessage in `tools:` 9/9 × 2 trees, `Task*` added to 4 agents (`manager-design`/`plan-auditor`/`super-advisor`/`sync-auditor`) × 2 trees, AC-APP-026/026b background-write-restriction literal 0/0 (live+tmpl), AC-APP-009a `AskUserQuestion(` in agent bodies 0 (user-question boundary preserved), AC-APP-029b `moai constitution guard CONST-V3R2-020,044` exit 0 + `moai constitution validate` 77 errors (== pre-existing baseline, no regression), `make build` exit 0 (catalog.yaml regen no-diff), `go test ./internal/template/...` exit 0, `moai spec audit` MUST-FIX 0. Run-phase landed via an isolated worktree (`32cfaa7a0`), integrated to `main` by orchestrator cherry-pick (`2c086db6d` → rebased `3d4addc2f`) — stale-worktree cherry-pick pattern per `feedback_stale_worktree_orchestrator_cherry_pick` (target files verified byte-identical to the shared-checkout baseline before integration). Mode 5 sub-agent sequential (single-domain agent/rule-doc editing with strict Template-First mirror-parity milestone ordering — no fan-out benefit). README.md/README.ko.md + docs-site 4-locale sync explicitly OUT OF SCOPE — the dual-channel progress-reporting user-facing narrative is deferred to the pending v3.0 README redesign (`project_readme_v3_rc11_redesign_draft` memory); CHANGELOG carries user visibility in the meantime. Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh; `sync_commit_sha` backfilled in a follow-up). 🗿 MoAI

### Fixed

- **[SPEC-SESSIONSTART-PERF-001](.moai/specs/SPEC-SESSIONSTART-PERF-001/spec.md)** — Session-start latency durability fix (Tier L, `internal/spec` + `internal/cli` + `internal/config` + `internal/hook` + `scripts/i18n-validator` — 3 milestones, 24/24 AC PASS). `moai hook session-start` blocked Claude Code session initialization by ~13-15s and the cost grew linearly with the SPEC corpus size (477 SPEC directories), because `internal/spec/drift.go` `DetectDrift` spawned one `git log --grep` subprocess per active SPEC (plus a second `git log` for the combined-scope fallback) — up to 274 `git log` invocations / 548 total git subprocesses on the real corpus. **M1 (`f376ee7d2`)** rewrites `DetectDrift` to a bounded, constant number of `git log` invocations: a single full-message (`%B`) `git log` pass builds an in-memory commit index (new `internal/spec/drift_index.go`), terminal/grandfather SPECs are pre-filtered before any git work, and a HEAD-SHA-keyed result cache (new `internal/spec/drift_cache.go`, persisted to `.moai/state/drift-cache.json`) short-circuits repeat calls entirely; a new `--no-cache` flag on `internal/cli/spec_drift.go` forces a fresh recompute. The in-memory index (`inMemImpliedStatus`) replicates the pre-refactor two-stage matching semantics exactly (full-message candidate window, then subject word-boundary re-filter) — verified behavior-preserving against the real 477-SPEC corpus: drift `count` 78 → 78, 474 records, canonical record-set SHA256 identical pre/post-refactor. Wall-clock: **~13-15s → ~0.2s cold / ~0.01s warm** (~65-1500x). **M2 (`a814656c5`)** adds `moai spec archive [--dry-run] [--yes] [--grace-days N] [--json]` (new `internal/spec/archive.go` + `internal/cli/spec_archive.go`), which relocates terminal-status SPECs (`completed`/`superseded`/`archived`/`rejected`) whose last git activity predates a configurable grace window (default **90 days**, Template-First `internal/config` `DefaultArchiveGraceDays` + new `.moai/config/sections/archive.yaml` in both the template and local trees) from `.moai/specs/` into `.moai/archive/specs/<year>/` via `git mv` — archived SPECs stay git-tracked and grep-discoverable; nothing is ever deleted. Era/grandfather classification is reported on every candidate but is never a gate (neither forces nor forbids archival), directly codifying the near-incident in `verification-claim-integrity.md` §5 where a frontmatter text-pattern inference nearly batch-touched 29 protected SPECs. The archive capability is statically guarded as unreachable from the session-start critical path (`internal/hook/session_start_archive_guard_test.go`; 0 archive-symbol call sites across 58 non-test hook sources). A real-corpus dry-run at the default 90-day window found 0 eligible SPECs — correct, not a defect: a prior bulk sweep re-touched the whole 477-SPEC corpus ~60 days ago, resetting every SPEC's last-activity clock, so the capability becomes effective as that sweep recedes past the grace window (or immediately at a shorter `--grace-days`). **M3 (`ab4ea7d74`)** installs a CI regression guard (`internal/spec/drift_perf_test.go`) asserting `git log` invocation count stays exactly 1 at a synthetic N=500-SPEC fixture (`config.DefaultDriftPerfFixtureSpecs`) — proven to catch an injected O(n) regression (501 invocations) before being reverted — and time-boxes the session-start drift check via a new `DriftCountCtx` (goroutine + `context.WithTimeout(DefaultSessionStartDriftTimeout)`, 2s default) so a slow drift computation can never block session-start unboundedly; on deadline exceed, `internal/hook/session_start.go` emits the existing advisory ("Run `moai spec drift` for details.") instead of blocking. All three new thresholds (2s perf budget, 2s session-start time-box, N=500 fixture size) are named consts in `internal/config/defaults.go` — no inline literals. M3 also fixed a pre-existing flaky `scripts/i18n-validator` wall-clock test at root cause (it was walking 64 gitignored ephemeral `.claude/worktrees/agent-*` checkouts, inflating the scan corpus ~60x; excluded from `corpusExclusions` and switched to root-relative path matching). **24/24 AC PASS** (AC-SSP-001..024, acceptance.md SSOT). A new `## Advisory-Check Discipline` [HARD] rule ("session-start / Stop advisory checks must be cached, asynchronous, or on-demand — never unbounded-blocking") was codified Template-First in `.claude/rules/moai/development/coding-standards.md` (both the template and local mirror, byte-identical, content-neutral). Full suite `go test ./...` green (96 ok / 0 FAIL, 3× deterministic `-count=1`, `-race` clean on all M3-touched packages); `golangci-lint` 0 issues; `go vet` exit 0; cross-platform build green (darwin/windows/linux). Coverage: `internal/spec` 88.9%, `internal/hook` 83.8% (both ≥85% or touched-function 100%). Known pre-existing, out-of-scope flake (unchanged by this SPEC, 0 lines touched): `internal/hook/dbsync` `TestCheckDebounceConcurrency` is a load-sensitive debounce-race test, flagged for a possible follow-up hardening SPEC. Mode 5 sub-agent sequential (M1→M2→M3, each milestone builds on the prior's algorithmic foundation). 🗿 MoAI

### Added

- **[SPEC-HARNESS-VERIFY-PROMOTE-001](.moai/specs/SPEC-HARNESS-VERIFY-PROMOTE-001/spec.md)** — Harness-generation offer promotion + mandatory verify skill + specialist-agent template rules (Tier S, doc-only, no Go code; SPEC 3 of the 3-SPEC "Project-Harness Pipeline" Epic, depends on SPEC-PROJECT-HARNESS-BRIDGE-001). **M1** promotes the harness-generation offer from a buried Phase 4.2 next-step menu option (unchanged, still hosted in `.claude/skills/moai/workflows/project/doc-generation.md` as a fallback entry point) to a **post-project-type-confirmation proposal** in `.claude/skills/moai/workflows/project/meta-harness.md` (the Phase 5.1 handoff module) — surfaced right after the adaptive interview confirms the project type ("이 프로젝트에 <type> 개발 하네스를 생성할까요?") — and adds the same proposal as `harness-build-entry.md`'s entry-interview **final-round offer**, both entry points reachable. **M2** makes `harness-builder.md` GENERATE mandate a new 6th artifact — a `harness-<name>-verify` companion skill (namespace `harness-*`) that discovers and codifies the project's build/launch/test recipe from a clean environment, mirroring the official `/run-skill-generator` runnable-verification pattern; it is ALWAYS emitted, falling back to a documented "no recipe found" stub when no recipe is discoverable. M2 also injects two short mandatory rule blocks — verbatim per spec.md §D.2 — into every generated specialist-agent body: a **Tool Priority** decision tree (category-fit MCP → search → file tools → inline response) and a **Skill-First Execution** rule (read the companion SKILL.md before any file/code work), plus a **3-7-specialists-maximum** PLAN-phase guardrail requiring each specialist to be justified as a recurring same-instruction worker (else emit a companion skill instead). The optional artifact 7 (MCP fragment, owned by SPEC-HARNESS-MCP-PROVISION-001) is unaffected — the Epic's canonical artifact order is now 6 = mandatory verify skill (this SPEC) / 7 = optional MCP fragment. **12/12 AC PASS** (AC-HVP-001..012, acceptance.md SSOT; doc-only SPEC — verification is grep/diff/`make build`, no test-coverage ACs), including AC-HVP-012's anti-bloat bound (both injected blocks ≤8 lines). Template-First byte-parity verified across all 3 touched file-pairs (`diff -q` PARITY OK ×3), `make build` exit 0, `go test ./internal/template/...` green (`ok internal/template 1.318s`), `grep -rn SPEC-HARNESS-VERIFY-PROMOTE-001 internal/template/templates/` = 0 (neutrality clean). `harness-*` namespace-only invariant + FROZEN guard (rejecting `.claude/agents/moai/` / `.claude/skills/moai-*/` / `.claude/rules/moai/`) and the NO-SPEC scope guard (0 `.moai/specs/` write paths) both explicitly re-verified intact. Full suite `go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0, `go test ./...` exit 0 (96 packages ok / 0 FAIL). Run-phase M1 (`2a042bef1`) + M2 (`75fa9217a`) + run-evidence M3 (`40a45701a`), L1 worktree integrated to `main`, Mode 5 sub-agent sequential (Tier S doc-only single-domain work with a Template-First mirror + `make build` cadence between milestones is inherently sequential — no fan-out benefit). README / docs-site 4-locale sync deferred as a follow-up per spec.md §E — no user-facing CLI surface change; this SPEC touches only internal `/moai project` and harness-generation workflow skill prose. Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh). This closes the 3-SPEC "Project-Harness Pipeline" Epic (SPEC-PROJECT-HARNESS-BRIDGE-001 → SPEC-HARNESS-MCP-PROVISION-001 → SPEC-HARNESS-VERIFY-PROMOTE-001). 🗿 MoAI
- **[SPEC-HARNESS-MCP-PROVISION-001](.moai/specs/SPEC-HARNESS-MCP-PROVISION-001/spec.md)** — Per-project-type MCP server provisioning in `/moai project` + harness generation (Tier M, doc/config-only, no Go feature code; SPEC 2 of the 3-SPEC "Project-Harness Pipeline" Epic, depends on SPEC-PROJECT-HARNESS-BRIDGE-001). New `.moai/config/sections/mcp-matrix.yaml` (both local + `internal/template/templates/` trees, byte-identical) — a per-project-type MCP recommendation matrix keyed by `web-frontend` / `mobile` / `backend-db` rows plus a `universal_starter` fallback (GitHub + Context7 + Playwright), 16-language neutral (no privileged language), referenced by filename rather than duplicated into skill prose. New `## Phase 3.6 MCP Server Provisioning` in `.claude/skills/moai/workflows/project/doc-generation.md`, inserted between Phase 3.5 (LSP) and Phase 3.7 (dev-mode): detects the project stack from `harness-spec.yaml` (`external_systems`/`ui_surface`), selects a 3-5 server cap preferring vendor-maintained servers from the matrix, routes approval through the orchestrator's `AskUserQuestion` (subagents return a blocker report — no subagent user-prompting), requires an EXPLICIT per-server approval for credentialed servers, and writes selected servers ADDITIVELY (merge, never clobber) into the repo-root `.mcp.json` at project scope with secrets expressed as `${VAR}` env-var references (never an inlined literal token) — `.moai/specs/**` is never a write target (NO-SPEC scope guard preserved). `.claude/skills/moai/workflows/project.md` (the `/moai project` router): registered Phase 3.6 in both the Phase Routing Table and the Invocation Flow diagram (mechanically ordered 3.5 < 3.6 < 3.7), and backfilled a pre-existing router drift — the missing Phase 3.2 (`harness-spec.yaml` emission) row/diagram line shipped by SPEC-PROJECT-HARNESS-BRIDGE-001 but never registered in the router. `.claude/skills/moai/workflows/harness-builder.md`: added optional `### Artifact 7` under the GENERATE Output Contract — a `.mcp.json` fragment (`artifact_type=mcp-server`, reusing existing `builder-harness` capability) emitted ONLY when the harness PLAN declares an MCP need via `harness-spec.yaml` `external_systems`; when no MCP need is declared, GENERATE output stays byte-identical to the without-artifact-7 baseline. The optional manifest `mcp` block is TOLERATE-ONLY (zero Go change — no `DisallowUnknownFields` in `internal/harness/v4manifest/` or at either decode site); reconciled all six stale "exactly 5 artifact types" prose/heading sites (frontmatter description, GENERATE heading, Primitive line, fan-out line, `## GENERATE Output Contract` H2 heading, body sentence) to the canonical order (5 base artifacts + mandatory verify skill artifact 6 + optional MCP fragment artifact 7). `internal/config/audit_loader_completeness_test.go`: registered `mcp-matrix` in the `acknowledgedUnloadedSections` allowlist — a discovered cascade of the new config file, acknowledging it as an intentional standalone data resource with no typed Go loader (per the plan-phase resolved clarification). **16/16 AC PASS** (AC-HMP-001..016, acceptance.md SSOT; doc/config-only SPEC — verification is grep/diff/`make build`, no test-coverage ACs). Template-First byte-parity verified across 4 file-pairs (`diff -q` PARITY OK ×4), `make build` exit 0, `go test ./internal/template/...` green, `grep -rn SPEC-HARNESS-MCP-PROVISION-001 internal/template/templates/` = 0 (neutrality clean), full suite `go test ./...` exit 0 (96 ok packages, 0 FAIL). Cross-platform build green (`go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0). Mode 5 sub-agent sequential (single-domain doc/config change with strict Template-First milestone ordering M1→M4; each milestone consumes the prior milestone's output, so parallel fan-out offered no benefit). 🗿 MoAI
- **[SPEC-PROJECT-HARNESS-BRIDGE-001](.moai/specs/SPEC-PROJECT-HARNESS-BRIDGE-001/spec.md)** — `/moai project` interview clarity-scoring + machine-readable `harness-spec.yaml` bridge to harness generation (Tier M, v0.2.0 in-place amendment, `.claude/skills/moai/workflows/project/*` + `.moai/config/sections/interview.yaml` — doc-only, no Go code, foundation SPEC of the 3-SPEC "Project-Harness Pipeline" Epic). Replaces the previous static fixed 3-round × 3-question `/moai project` interview with a **two-stage adaptive interview** reusing the `plan/clarity-interview.md` 0-10 scale mechanism. **Stage A (clarity-scored adaptive discovery)** — Phase 0.3 (new-project, `mode-detection.md`) and Phase 1.5 (existing-project, `codebase-analysis.md`) run Rounds 1-3 with `clarity_threshold: 4` (from `interview.yaml`) as the entry floor, sufficiency exit at score ≥ 8, abandon at score ≤ 3, capped by `project.max_rounds: 3`; the early exit is gated on all four REQUIRED base fields (`domain`, `goal`, `constraints`, `scope`) being answered and terminates Stage A ONLY. **Stage B (mandatory extended-axes round)** — a Round 4 that ALWAYS runs after Stage A ends by any path (early exit, abandon, or cap) and is EXEMPT from the round cap, the early-exit skip, and clarity scoring, eliciting 4 extended axes — `verification` (test/e2e command), `ui_surface` (has-ui | headless), `external_systems` (DB/API/service list), `team_sharing` (solo | team-shared) — config-declared in `.moai/config/sections/interview.yaml` `project.additional_axes`, with the existing-project variant pre-filling axes it can infer from Phase 1 codebase analysis rather than re-asking. This closes an in-place-amendment defect (F-1..F-5) found by an adversarial sync-audit against the prior v0.1.1 shipped behavior: Round 4 was UNREACHABLE (the `max_rounds: 3` cap AND the early-exit both blocked it), so the four extended axes were never collected. **Machine-readable `harness-spec.yaml` bridge** — Phase 3.2 in `doc-generation.md` reads `.moai/project/interview.md` (written by the Stage A/B interviews) and emits `.moai/project/harness-spec.yaml`, an 8-field schema partitioned REQUIRED base (`domain`, `goal`, `constraints`, `scope`) vs EXTENDED (`verification`, `external_systems`, `ui_surface`, `team_sharing`) with OVERWRITE re-run semantics matching `interview.md`; `meta-harness.md` Phase 5.1 and `harness-build-entry.md` Phase 1 compose their harness-generation request from `harness-spec.yaml` + `product.md`/`structure.md`/`tech.md`, PRE-SATISFYING already-answered fields so Discovery re-asks only fields that are absent or ambiguous — closing the information-leak where the interview's structured answers were previously discarded and re-elicited. The NO-SPEC scope guard in `mode-detection.md` (prohibiting any write under `.moai/specs/`) is explicitly preserved and re-verified — `harness-spec.yaml` is written only to `.moai/project/`. **22/22 AC PASS** (AC-PHB-001..014 v0.1.1 regression guard + AC-PHB-015..020 REACHABILITY exit criteria for the v0.2.0 amendment + AC-PHB-021/022 for F-3r/F-5, acceptance.md SSOT; all doc-only grep/diff/`make build`/`moai init` sandbox verification per plan-phase design — test-coverage-style ACs are N/A, no Go code touched). Template-First byte-parity verified across 7 file-pairs (6 skill/doc files + interview.yaml, local↔template `diff -q` PARITY OK ×7), `make build` exit 0, `grep -rn SPEC-PROJECT-HARNESS-BRIDGE-001 internal/template/templates/` = 0 (neutrality clean). Run-phase landed as `9f1f0f53b` (v0.1.x M1-M4, Claude Code L1 worktree integrated to `main` by the orchestrator) + `3af01567f` (v0.2.0 M5 two-stage-interview amendment), Mode 5 sub-agent sequential (interdependent Template-First mirror-parity milestones). Cross-platform build green (`go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0), `go test ./internal/template/...` green; whole-repo `go test ./...` exit 0 fully green on the post-amendment tree (zero FAIL lines; the v0.1.1 `§E.3` entry's flake attribution to `TestDoctorPermission_NoMatchTrace` was itself a mis-attribution, corrected during the amendment). README.md / docs-site 4-locale sync explicitly OUT OF SCOPE per spec.md §Out of Scope — deferred follow-up (no user-facing CLI surface change; this SPEC touches only internal `/moai project` workflow skill prose + config). This `[Unreleased]` entry was REWRITTEN in place (not appended) at the v0.2.0 close: both v0.1.1 and v0.2.0 land in the same unreleased cycle, so the entry describes final shipped behavior rather than a broken-then-fixed narrative — the prior entry's "14/14 AC PASS" and "skip remaining rounds [into a delivered Round 4]" framing described the pre-amendment, unreachable-feature state and has been corrected. Sync-phase 3-phase close of the v0.2.0 amendment (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh). 🗿 MoAI

### Removed

- **[SPEC-AGENT-TEAM-RETIRE-001](.moai/specs/SPEC-AGENT-TEAM-RETIRE-001/spec.md)** — Agent Teams static-layer retirement + two replacement dynamic workflows (Tier L, v0.1.2, `internal/cli` + `internal/config` + `internal/web` + `internal/settings` + `.claude/rules` + `.claude/skills` + `internal/template` — 22 REQ / 29 AC, 100% AC→REQ coverage). **Boundary principle**: removes ONLY MoAI's STATIC Agent Teams layer built on top of Claude Code's native teammate runtime; the native runtime itself (`Agent(name=...)`, `RoleProfile`+`Sandbox`, GitStrategy `Team ModeProfile`, `internal/session` `MergeTeamCheckpoints`, worktree `team_launch`/`swarm_registry`/`handoff_guidance`, `teammateMode`, `moai cg`/`moai glm`/`moai cc` GLM-teammate panes) is fully PRESERVED. **M0** (`279d2f688`) — de-risk migration of shared dependencies BEFORE any deletion: NEW `internal/lockfile` (cross-platform advisory file-lock pair, 100% coverage) + NEW `internal/cli/taskledger` (`ClaimTask`/`AppendTask`, 92.7% coverage), re-pointing `internal/cli/settings.go` + `glm_tools.go` lock callers; whole-repo green gate BEFORE deletion (REQ-ATR-003). **M1** (`aff4a2537`) — deleted `internal/cli/team_spawn*` (5 files, 0 non-test callers at removal time) + `internal/config` `TeamConfig`/`RoleProfileEntry`/`TeamAutoSelectionConfig`/`WorkflowConfig.Team` (GitStrategy `Team ModeProfile` + `RoleProfile.Sandbox` PRESERVED); repurposed `moai workflow lint` (agentlint) from `role_profiles` validation to `model_routing_profiles` closed-set validation (sentinel `ORC_WORKTREE_REQUIRED` → `MODEL_ROUTING_INVALID`). **M2** (`db4c08aa7`) — removed the web-console Agent Teams surfaces: 66 `f.workflow.team.*` i18n keys × 4 locales (264 lines), `schema_sections.go` `workflow.team` FieldDef rows + `agentSettingsFields()`/`RoleProfileNames()`, `SectionAgentSettings` (GitStrategy `f.git_strategy.team.*` (16) + `f.git_strategy.mode.opt.team` (4) i18n keys PRESERVED, unchanged). **M3** (`0303e8c75`) — deleted `.claude/rules/moai/workflow/{team-protocol.md,team-pattern-cookbook.md}` + all 6 `.claude/skills/moai/team/*.md` skills (both live + template trees); migrated the CG Mode (Claude + GLM teammates) guidance from `team/glm.md` into `glm-web-tooling.md` BEFORE deleting `team/glm.md` (migrate-then-delete ordering, REQ-ATR-022); tombstoned Mode 3 in `orchestration-mode-selection.md`; removed `--team` routing + `## Team Mode` sections from 12 `.claude/skills/moai/workflows/*.md` files (GitStrategy team-profile prose in `sync/delivery.md:285-338` PRESERVED as the sole named exception); removed the `team:` block from the template `workflow.yaml`, `make build` regenerated the embedded FS. **M4** (`604dbf04a`) — final whole-repo verification sweep (`go build`/`GOOS=windows go build`/`go vet`/`go test ./...` all exit 0, `golangci-lint` 0 issues) + Scope B: two NEW local-only (NOT template-mirrored — `.claude/workflows/` is user-owned per `dynamic-workflows.md`) replacement dynamic workflows following the `codemaps-extract.js` house style: `.claude/workflows/sync-audit-4dim.js` (Context → 4 parallel read-only Judge agents → in-script harmonic-mean Verdict, `INCOMPLETE`-on-null-judge guard, Tier M/L gate) and `.claude/workflows/plan-research-fanout.js` (3-4 read-only lens Explorer agents → 1 Synthesizer marking cross-lens contradictions, `>=2` null-lens abort, lens hard-cap `slice(0, 4)`, no in-workflow writes). **29/29 AC PASS** (AC-ATR-017 PASS-WITH-DEBT — deployed template clean per AC-ATR-018 fresh-init verification; local dev-tree `workflow.yaml` `role_profiles`/`team` block intentionally left untouched per CLAUDE.local.md §22 dev-local exemption, a documented M3 deviation, not a defect). Cross-platform build green (darwin+windows), `golangci-lint` 0 issues (baseline-identical), `internal/lockfile` 100% / `internal/cli/taskledger` 92.7% coverage. Run-phase M0-M4 landed as 5 separate per-milestone commits (M2/M3/M4 via isolated L1 worktrees, orchestrator-integrated to `main`), Mode 5 sub-agent. Known pre-existing environmental flake documented separately, unrelated to this SPEC's scope: `scripts/i18n-validator` `TestBudget_FullRepoScanWithin35Sec` is timing-sensitive against ~59 leftover `.claude/worktrees/agent-*` directories from prior sessions inflating the full-repo scan past its 35s budget under load (net Go-file deletion + 2 new local JS files in this SPEC reduce scan load; isolated reruns pass, e.g. 34.8s observed during this sync). Follow-up flagged (out of this SPEC's REQ scope, not fixed here): `.claude/rules/moai/development/orchestrator-templates.md` still describes team-orchestrator patterns — a residual doc-drift surface for a future SPEC. README.md/README.ko.md's "Dual Execution Modes" / `--team`/`--solo` flag documentation was NOT updated in this sync (large user-facing section describing the now-retired static routing plus the still-valid CG Mode content interleaved; flagged as a dedicated follow-up rather than a partial edit risking further staleness). 🗿 MoAI

### Added

- **[SPEC-CLI-TUX-V3-001](.moai/specs/SPEC-CLI-TUX-V3-001/spec.md)** — Charm v2 CLI/TUI migration + unified output gateway (Tier L, v0.1.0, `internal/tui` + `internal/cli/printer` + `internal/cli` + `internal/merge` — CI non-TTY/NO_COLOR ANSI-free contract, no user-visible flag changes). **M1a** — `internal/tui` (15 files) ported from `github.com/charmbracelet/lipgloss` v1 to `charm.land/lipgloss/v2` v2.0.5 while `statusline`/`wizard`(huh v1)/`uikit` consumer packages remain source-unmodified on lipgloss v1 + bubbletea v1 (dual-major coexistence proven by full-suite green, REQ-CTX-001..007). **M1b** — new `internal/cli/printer` package (`printer.go`, 561 lines) as the single output gateway: `Info`/`Warn`/`Error`/`Success`/`Data`/`Step`/`Spinner`/`Progress` interface with a fixed channel-discipline contract (`Data` → stdout only; every status/warn/error/step/spinner/progress event → stderr only) across 3 render modes (`ModeTTY`/`ModePlain`/`ModeJSON`) selected once at construction (explicit `WithMode` override > isatty detection on stderr; `NO_COLOR` forces a resolved `ModeTTY` down to `ModePlain` so no ANSI ever reaches either channel). All colours are sourced from `internal/tui` string tokens — zero hex-literal colours in printer/reporter code (REQ-CTX-008/011..014). `internal/core/project.ConsoleReporter`'s 3 hex `AdaptiveColor` literals + direct `fmt.Printf`-to-stdout calls were replaced by a new `internal/cli.printerReporter` adapter (`reporter.go`, `core <- cli` dependency direction preserved) implementing `project.ProgressReporter` (REQ-CTX-015); `init.go`'s 8 stdout-leaking `Warning:` lines were migrated to `Printer.Warn` (stderr) (REQ-CTX-016). Coverage `internal/cli/printer` 97.3% (AC-CTX-024 ≥90% threshold). **M1c** — `charm.land/fang/v2` v2.0.1 integrated at CLI root (`internal/cli/fang.go`, 114 lines): `runFang` wraps `fang.Execute` around the existing root `cobra.Command`, preserving the `ExitCoder` chain (`moai worktree verify` 0/1/2/3 custom exit codes) verbatim via `TestFangExitCoderCharacterization` (REQ-CTX-018/020); `fangOptions()` pins fang's `--version` surface to the existing `pkg/version` source (no duplicate version implementation), disables the `man` subcommand, and wires `moaiColorScheme`/`moaiErrorHandler` so fang's help/error palette is derived entirely from `internal/tui` light/dark tokens (zero hex outside `internal/tui`, REQ-CTX-022) while `ExitCoder`-carrying errors are passed through silently instead of triggering fang's styled error box (REQ-CTX-021 NO_COLOR/non-TTY ANSI-free holds through the fang path). The version/help/completion trivial fast-path (`REQ-CTX-019`) stays outside the fang wrapper, so lazy `InitDependencies` is not triggered by `--version`/`--help`. `internal/merge/confirm.go`'s bubbletea-v1 confirmation UI is deliberately deferred past this SPEC's scope and marked with a 3-line `@MX:DEBT`/`@MX:CEILING`/`@MX:UPGRADE` annotation (valid while `huh` v1 keeps lipgloss/bubbletea v1 in the module graph; upgrade trigger = SPEC-CLI-TUX-V3-003 promoting the UI to a `charm.land/bubbles/v2` list component). **26/26 AC PASS** (AC-CTX-001..026, 24 clean PASS + 2 PASS-WITH-DEBT per acceptance.md §D machine-verifiable matrix). Orchestrator-independent verification: `go build ./...` + `GOOS=windows`/`GOOS=linux` cross-builds exit 0, `go test ./...` green (2 pre-existing unrelated flakes documented separately: `TestHookWrapper_TempFileCleanup` in SPEC-AGENT-ARCH-V2-001/progress.md, `TestBudget_FullRepoScanWithin35Sec` in SPEC-SIMPLICITY-LADDER-001/progress.md — both untouched by this SPEC and pass in isolation), `golangci-lint run` 0 issues, `go vet` exit 0, `internal/cli/printer` coverage 97.3%, hex-literal grep 0 (AC-CTX-008), fmt.Printf/Println ratchet 46→40 (AC-CTX-017), `NO_COLOR=1` ANSI-escape grep 0 on `--help`/`init --help` (AC-CTX-021), protected paths (`internal/statusline`/`internal/cli/wizard`/`internal/cli/uikit` golden files) diff 0 (AC-CTX-009/010). Run-phase M1a/M1b/M1c (`b9df38f5b`, `5824c812c`, `3be403dc4`), Mode 5 sub-agent sequential. Sync-phase 3-phase close (this commit). README unchanged — no user-facing CLI flag or command surface change; the migration is internal-dependency and internal-package scoped. 🗿 MoAI

### Changed

- **[SPEC-DISCOVERY-UNKNOWNS-001](.moai/specs/SPEC-DISCOVERY-UNKNOWNS-001/spec.md)** — Unknowns framework Tier-1 for Context-First Discovery — Blind Spot Pass + decision-reversibility plan ordering + 4-quadrant ambiguity lens (Tier M, doc/rule/agent-body only, zero Go code, 4 doctrine surfaces + 4 template mirrors). Adapts the Tier-1 subset of the "Finding Your Unknowns" framework to moai-adk's orchestrator model — Interviews and References are already covered by AskUserQuestion Socratic rounds and `Agent(Explore)`/Context7 and are explicitly NOT re-added; only the three genuinely-absent items ship. **T1 — Blind Spot Pass**: a new, OPTIONAL `## Blind Spot Pass` subsection in `.claude/rules/moai/core/askuser-protocol.md` defines a pre-plan-phase technique — where the user is working in an unfamiliar domain and unknown-unknowns are suspected, the orchestrator MAY spawn `Agent(Explore)` in read-only mode to scan the relevant domain, then surface the user's likely unknown-unknowns through a single `AskUserQuestion` round before the SPEC is authored; the pass stays optional (never a mandatory plan-phase gate) and preserves the asymmetric orchestrator-subagent boundary (Explore never prompts the user directly). Wired via one-line references from `.claude/rules/moai/workflow/spec-workflow.md` § Plan Phase and CLAUDE.md §7 Rule 5. **T2 — Plan ordering by decision-reversibility**: `.claude/agents/moai/manager-spec.md`'s plan.md authoring guidance now instructs that plan.md milestones/sections lead with the decisions most likely to change (data-model changes, new type interfaces, user-facing/UX flows), deferring mechanical/refactoring steps to the bottom; CLAUDE.md §7 Rule 1 (Approach-First Development) carries the same change-likelihood-first instruction as a new sub-bullet. Ordering rule only — no new tooling, agents, or schema. **T3 — Unknowns 4-quadrant lens**: `askuser-protocol.md` § Ambiguity Triggers and Exceptions gains the Known-Knowns / Known-Unknowns / Unknown-Knowns / Unknown-Unknowns taxonomy as an explicit classification lens, routing suspected Unknown-Unknowns to a Blind Spot Pass; CLAUDE.md §7 Rule 5 carries a single one-line framing + SSOT pointer (diet-respecting, no section-block expansion). **17/17 AC PASS** (AC-DU-001..017, acceptance.md SSOT), including the two enforced CI parity guards `TestRuleTemplateMirrorDrift` (byte-identical `spec-workflow.md` mirror) and `TestSanitizedPairParity` (doctrine-propagation parity for the `askuser-protocol.md` mirror). `make build` exit 0 (embedded template FS + catalog.yaml regenerated), §25 template neutrality clean (0 internal SPEC-ID / date / audit-citation leakage), zero `.go` files touched. Run-phase landed via a stale L1 worktree fast-forwarded to `main` before editing, then orchestrator-integrated. README/README.ko.md unchanged — internal Discovery-doctrine enhancement with no user-facing CLI surface change. Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh; `sync_commit_sha` backfilled in a follow-up commit). 🗿 MoAI

- **[SPEC-WEB-CONSOLE-013](.moai/specs/SPEC-WEB-CONSOLE-013/spec.md)** — moai web console 신규 config 도메인 노출 (Track 2, Tier M, v0.1.1, `internal/settings` + `internal/web` — 사용자 가시적 CLI surface 변경 0). **M1 라우팅 기반** — `sectionroute.go`에 `"handoff"`/`"cache"`를 RouteSeam으로 등재, cache를 machine/state `ExcludedSections()`에서 제거(REQ-WC11-018 부분 SUPERSEDE, 12→11), `SeamSections()` 6→8, `sectionRootKeys` +2(cache→cacheStrategy root-key 매핑, REQ-WC13-001..006). **M2 handoff+cache FieldDef** — `handoff.{mode,guide}` + `cacheStrategy.{enabled,session_ttl}` PersistSeam FieldDef 추가, closed-set validation이 export된 `config.ValidSessionTTLs()` SSOT를 소비(mode=bogus 거부/mode=auto 수락), i18n 4-locale(en/ko/ja/zh) 완비(REQ-WC13-010..016). **M3 Model Policy 읽기전용 뷰** — `llm.performance_tier` 표시 + perfTier(max/medium/low) × Tier(S/M/L) × Phase(plan/run/sync/mx) 3×12 셀 라우팅 표, FieldDef 없는 read-only view(hx-post/form/input 0건, no persist binding), legacy flat `workflow.model_routing`/`workflow_agents`는 의도적으로 미노출(REQ-WC13-020..026). **M4 검증 스윕** — cross-platform build(darwin+windows) + touched-package suite + lint 0 issues 확인. **AC-WC13-001..032(25 AC ID, sub-letter 포함 29 PASS row) 전부 PASS**. statusline/template 트리 무접촉 확인(AC-030/031, `git diff --stat` empty). Run-phase M1-M4(`83b72e036`/`31e44513a`/`c6a19dbc1`/`819532241`), isolated worktree branch → main으로 rebase+landed, Mode 5 sub-agent. cross-platform build green, `golangci-lint` 0 issues(baseline 0 issues 유지), touched-package 테스트 전부 green(`internal/settings`/`internal/web`/`internal/cli`/`internal/config`). Known debt(스코프 무관, 사전 결함): `go test ./...` 전체 스위트에서 load-sensitive 2건 — `internal/hook` `TestHookWrapper_TempFileCleanup`(격리 재실행 시 통과) + `scripts/i18n-validator` `TestBudget_FullRepoScanWithin35Sec`(병렬 부하 시 53.66s vs 35s 예산 초과, 본 SPEC 미접촉 패키지). Sync-phase 3-phase close(단일 sync commit + `sync_commit_sha` follow-up backfill commit). README 무변경 확인(moai web console 설정 섹션에 대한 사용자 가시적 문서 언급 없음 — 검토 완료, statusline segment는 명시적으로 스코프 외). 🗿 MoAI

- **[SPEC-WEB-CONSOLE-014](.moai/specs/SPEC-WEB-CONSOLE-014/spec.md)** — moai web console 기존-섹션 key 증설 + dormant-config 노출 가드 (Track 3, Tier M, v0.2.0, `internal/settings` + `internal/web` — 사용자 가시적 CLI surface 변경 0). **M1 dormant-config 노출 금지 가드** — `learning.auto_apply`/`observability.output_path`/`sunset.*`/`harness.model_upgrade_review.*`/`security.permission.{pre_allowlist,session_rules}` 등 미배선 dormant 키가 seam 편집 대상으로 노출되지 않도록 `TestDormantConfigNeverEditable` + `TestApplySchemaEditsRejectsUnknownAndReadOnly` 회귀 가드 신설, legacy flat `workflow.model_routing`은 편집 불가 유지(REQ-WC14-001/012/050/051). **M2 learning augmentation + honesty demotion** — `learning.tier_thresholds`/`learning.rate_limit`을 편집 불가 raw read-only 블록으로 노출(config 미배선 컴파일 상수임을 F2-style 정직 라벨로 명시), `observability.output_path`는 dead config(reader 0건, 실제 기록 경로는 `hookMetricsRelPath` 상수)로 판명되어 read-only 강등 + 쓰기 거부, `slow_hook_threshold_ms`는 editable로 핀 고정(REQ-WC14-002/003/040a/040b). **M3 git_strategy merge_method 편집 select** — `git_strategy.{team,solo,gitflow}.merge_method` 3-mode select를 export된 `ValidMergeMethods` SSOT enum(squash/merge/rebase) 소비로 노출, absent-key(빈 값)는 유효 멤버가 아닌 것으로 별도 표시 처리, YAML 라운드트립 시 기존 주석 보존(REQ-WC14-010/011). **M4 raw read-only view** — `security.sandbox.{network_allowlist,env_scrub_extra}`(config-unwired scaffold, F6 informational 라벨) + `mx.*`를 RAW READ-ONLY 뷰 블록으로 렌더(편집 바인딩 없음, `TestRawBlockValues`/`TestMXRawViewRendered`/`TestExcludedSectionsAllRejected` PASS, REQ-WC14-020/021/030/031). **M5 i18n 4-locale parity + 양표면 브리지 스윕** — 신규 14개 키 × 4-locale(en/ko/ja/zh) 존재를 `TestSPEC014I18nKeysFourLocale`로 기계 검증, `internal/cli` bridge 4종 테스트(`TestI18nKeySetParity`/`TestI18nSegmentParity`/`TestBridgeFieldDefResolver`/`TestTUIRendersSchemaFieldSet`) 무수정 green(REQ-WC14-060/061). **AC-WC14-000..063(23 AC) 전부 PASS**([B] 22 + [N] 1). `internal/statusline` 무접촉 확인(AC-WC14-062, `git diff --name-only` 0줄). Run-phase M1-M6(`a807ca062`/`9aa99c0f3`/`109499908`/`ee3e1afe2`/`02294ef85`/`36894621e`), isolated worktree branch → main으로 rebase+landed, Mode 5 sub-agent. cross-platform build green(darwin+windows), `golangci-lint` 0 issues(baseline 유지), coverage `internal/settings` 89.4%(≥85%) / `internal/web` 70.8%(pre-existing render-path baseline, 회귀 아님). Known debt(스코프 무관, 사전 결함): `go test ./...` 전체 스위트에서 load-sensitive 2건 — `scripts/i18n-validator` `TestBudget_FullRepoScanWithin35Sec`(격리 시 30.5s vs 35s 예산, 병렬 부하 시 초과) + `internal/hook` `TestHookWrapper_TempFileCleanup`(격리 재실행 시 통과), 양쪽 모두 본 SPEC 미접촉 패키지. Sync-phase 3-phase close(단일 sync commit + `sync_commit_sha` follow-up backfill commit). README 무변경 확인(moai web console 설정 섹션에 대한 사용자 가시적 문서 언급 없음 — 검토 완료). 🗿 MoAI

### Added

- **[SPEC-GOAL-ENGINE-001](.moai/specs/SPEC-GOAL-ENGINE-001/spec.md)** — `/moai goal` engine — MoAI-owned PROGRAMMATIC reimplementation of native `/goal` command (Tier M, v0.2.1, `internal/goal` + `internal/cli` + `.claude/skills/moai/workflows/goal.md` — 29 REQ / 34 AC, Epic AGENTIC-CORE, SPEC 2 of 3, depends on SPEC-ANALYZE-FIRST-ROUTING-001). New greenfield package `internal/goal/` (schema, state, prune, evaluate + tests) implements per-session goal state (`.moai/state/goal/<session-id>.json`) and hybrid 2-tier Stop-hook evaluator (`moai hook stop-goal`): Tier 1 = mechanical conditions (exit code, grep count, file existence, bounded turn count), Tier 2 = orchestrator self-eval via goal.md checkpoint. NEW `internal/cli/hook_stop_goal.go` CLI verb (`moai hook stop-goal`), NEW `.claude/hooks/moai/handle-stop-goal.sh` wrapper + template mirror, NEW `.claude/skills/moai/workflows/goal.md` workflow (4 verbs: register/arm, status, clear, resume; plus progression-mode axis: autonomous/semi-autonomous kickoff progression modes; checkpoint orchestrator-bridge for semi-autonomous), EXTEND `.claude/skills/moai/SKILL.md` (P1 `**goal**` registration + Quick Reference `### goal` section), EXTEND `internal/template/templates/.claude/settings.json.tmpl` Stop-hook COMPOSE (handle-stop-goal.sh entry, timeout 120; handle-stop.sh preserved), EXTEND doctrine: `goal-directive.md` (`/moai goal` row + Axis B), `native-invocation-model.md` (Axis B illustration), `session-handoff.md` (Block 5 `/moai goal` variant), `moai.md` (phase-granular vs task-granular boundary), `run.md` + `orchestration-mode-selection.md` (progression-mode axis), `CLAUDE.md` §2 stage ④ (progression-mode axis) + §2 stage ⑤ (goal evaluator) — all mirrored to template. EXTEND `internal/cli/hook_test.go`, `hook_pre_push_test.go` (subcommand count 36→37), `hook_e2e_test.go` (utilitySubcmds +stop-goal). **34/34 AC PASS** (AC-GLE-001..034, acceptance.md SSOT). Coverage `internal/goal` 86.5% (≥85 target). MX tag boundary grep 0 (C-HRA-008 preserved: `AskUserQuestion`/`mcp__askuser` absent from `internal/goal/` + `hook_stop_goal.go` + `handle-stop-goal.sh` non-test code). Kickoff-mandatory invariant preserved in both modes: Implementation Kickoff Approval remains mandatory regardless of progression-mode selection (AC-GLE-015/034). Run-phase landed as single commit `23a9a7249` (draft→in-progress frontmatter transition + M1–M7 implementation + progress.md §E.2/§E.3 evidence), orchestrator-integrated to `main`, Mode 5 sub-agent sequential (single-domain goal-engine implementation, TDD cycle_type=tdd). Cross-platform build green (`go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0), full suite `go test ./...` green, `golangci-lint run` 0 issues. Docs-site 4-locale sync deferred as follow-up per spec.md §D.5. Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh). 🗿 MoAI

- **[SPEC-GOAL-ENGINE-001](.moai/specs/SPEC-GOAL-ENGINE-001/spec.md)** — `/moai goal` arm CLI + orphan-prune wiring (in-place amendment v0.3.0, reachability fix; Tier L, `internal/cli` + `internal/hook`). The v0.2.1 close had shipped the goal engine + `stop-goal` evaluator, but two deliverables were INERT: (1) there was no `moai goal` CLI verb to arm a goal — `internal/goal`/`stop-goal` could only load and evaluate an already-armed goal, so nothing ever wrote `.moai/state/goal/<session-id>.json` despite `goal.md` promising it; (2) `internal/goal.PruneOrphans` had zero call sites. This amendment adds NEW `internal/cli/goal.go` — `moai goal arm "<condition>" [--session <id>]`, `moai goal status [--all]`, `moai goal clear` — wired through the SAME per-session state file the `stop-goal` hook evaluator reads (E2E-verified: arm writes `<session-id>.json`, hook loads and evaluates the identical file), with session-id resolution consistency (no silent `pid-*` fallback when a real session id is resolvable). EXTENDS `internal/hook/session_start.go` to call `goal.PruneOrphans` on session start (fail-open: a prune error never blocks session start), closing the previously-unreachable orphan-cleanup path. `resume` remains explicitly deferred/out of scope (documented in `goal.md`, not implemented). **39/39 AC PASS** (AC-GLE-035..039 new reachability pins + AC-GLE-001..034 preserved with no regression, acceptance.md SSOT). Coverage `internal/goal` 86.5% (unchanged, ≥85 target); new `internal/cli/goal.go` functions 78.6-100% per-function. Subagent-boundary grep 0 (`AskUserQuestion`/`mcp__askuser` absent from `internal/cli/goal.go` + `internal/hook/session_start.go` non-test code, C-HRA-008 preserved). Run-phase landed as single amendment commit `3f68742d4` (cherry-picked onto `main` from an isolated worktree via the stale-worktree cherry-pick pattern; `frontmatter status` was already `in-progress` from the amendment re-entry, so no status transition performed by this run). Cross-platform build green (`go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0), full suite `go test ./...` 99 packages ok / 0 FAIL, `golangci-lint run` 0 issues. Mode 5 sub-agent sequential (coding-heavy, single-domain, bounded — 6 files). Sync-phase 3-phase close of the amendment (this commit — `in-progress → completed` merged into the single sync commit + `updated:` refresh). 🗿 MoAI
...
- **[SPEC-HOOK-OFFICIAL-COMPLIANCE-001](.moai/specs/SPEC-HOOK-OFFICIAL-COMPLIANCE-001/spec.md)** — Template Hooks Official-Doc Compliance (Tier L, v0.2.0, 템플릿 훅 시스템 공식 문서 준수 — 32 gap, 8 recommendations). Claude Code hooks 공식 문서(`code.claude.com/docs/en/hooks`) 대비 32개 gap(2 HIGH / 8 MEDIUM / 15 LOW / 7 INFO)와 8개 우선 recommandation을 8개 milestone(M1-M8)로 해결. **M1(HIGH)** — 차단 gate JSON 계약 수정(`team-ac-verify.sh` TaskCompleted 이벤트 `continue:false+stopReason`로 정정, `sync-phase-quality-gate.sh` Stop hook `hookSpecificOutput` nesting 구조; AC-001/002) + PreToolUse exit-code passthrough(`handle-pre-tool.sh.tmpl` `exit $?`로 moai 바이너리 exit code 전달, stderr 보존; AC-003) + SessionStart probe `hookEventName` 추가(AC-005) + agent-common-protocol.md template mirror(Template-First byte-identical, AC-006) + coverage test 추가(AC-007). **M2(MEDIUM)** — hooksystem.md 8개 교화점 정정(PreCompact Can Block No→Yes, SubagentStop stdout+decision:block+reason 추가, Notification/StopFailure matcher value 8→10/10 확장, FileChanged matcher literal filenames 규칙, UserPromptSubmit/PermissionRequest stdout JSON contract, Elicitation/ElicitationResult handler-type 4-value 제한, exit-2 "universal"→per-event enum으로 재작성, PostToolUse async constraint 기술). **M3(MEDIUM)** — async observation taps(UserPromptSubmit/Stop/SubagentStop + InstructionsLoaded) settings.json.tmpl에 추가(AC-017/018). **M4(MEDIUM)** — timeout headroom(TeammateIdle/TaskCompleted 60s, PreCompact 30s; >5s baseline 충족, AC-019/020). **M5(MEDIUM)** — FileChanged/ConfigChange matcher Go inspection(downstream consumer / never-blocker 확인, 설정 파일 변경 불필, AC-021/022). **M6(MEDIUM)** — fail-open semantics correction(WorktreeCreate/PermissionRequest header comment + SessionStart PermissionRequest 우선순위 차원 명시, AC-023/024). **M7(LOW)** — input hardening(MOAI_HOOK_STDERR_LOG allowlist 대 62개 wrapper; passthrough token `${CLAUDE_PROJECT_DIR}` 보정 + `/dev/null` opt-out 명시, AC-025) + gateguard-fact-force.sh state-file escaping(JSON→key=value form, AC-026). **M8(LOW)** — coverage holes + defects(csharp glob `find` form 교정, agent exec form 명시적 quoted tokens 7건, compact comment stale 참조 수정, AC-028/029/030). **36/36 AC PASS**. Run-phase M1(820cf2d12, JSON contract+exit-code), M3/M4/M8a(7e641d959, async+timeout+matcher), M2/M5(c6ee34184, doctrine+Go inspection), test-fix(1e5be72f5), M6(dde73b0f7), M8bcd(5a845a5e2), M7(3a5ff6a78 + 3d74559aa allowlist refinement). 전체 커밋 범위: template hook scripts 35개 파일(handle-*.sh.tmpl 32개 + gateguard/sync/team-ac-verify/status-transition 3개), settings.json.tmpl wiring, doctrine 2개(hooks-system.md + hook-independence.md). Template-First(live↔template byte-identical) + neutrality CI PASS. 테스트 substantive green(`go test ./...` — 1 pre-existing flaky `TestHookWrapper_TempFileCleanup`/internal/hook는 sync 무관, 격리 rerun PASS 2/2), cross-platform build green(linux+windows), lint 0 issues. Sync-phase 3-phase close(단일 sync commit으로 `in-progress → implemented → completed` + `sync_commit_sha` backfill). 🗿 MoAI

### Changed

- **[SPEC-WEB-CONSOLE-012](.moai/specs/SPEC-WEB-CONSOLE-012/spec.md)** — moai web console stale/dead config surface cleanup (Track 1, Tier S, v0.2.1, `internal/settings` + `internal/web` — 사용자 가시적 CLI surface 변경 0, 설정 데이터 무손실). **M1 research seam write-path 폐선** — `sectionroute.go`의 `"research": RouteSeam` 및 `SeamSections()` research 항목, `sectionwrite.go`의 `sectionRootKeys["research"]`, `schema.go`의 `SectionResearch` const를 제거 + research seam-write 거부 회귀 테스트 신설(`WriteSectionViaSeam(root, "research", …)` → error + research.yaml 무변경 단언, REQ-WC12-010/011). **M2 llm.glm ghost tier 필드 제거 + fable 노출** — `schema_sections.go` `llmFields()` tier 루프에서 `{opus,sonnet,haiku}` 제거하고 `fable` 추가, `sectionapply.go` `applyLLMKey`의 `glm.models.{opus,sonnet,haiku}` case 삭제 + `glm.models.fable` case 추가, `i18n.js` 전 locale에서 `f.llm.glm.models.{opus,sonnet,haiku}.*` 키 제거 + `f.llm.glm.models.fable.{title,desc}` 추가(high와 동수 locale parity), `schemaform.go`의 `opus/sonnet/haiku` 잔재 제거(REQ-WC12-001..005). **M3 잔류 확인 게이트(코드 무접촉)** — `auto_detection`(GitConvention) 5개 필드 + harness `min_coverage_per_commit`/`validation.enforce_on_push` FieldDef가 실사용 중임을 재확인(REQ-WC12-006/020/021/023, field-dot + bare-symbol pair 재검증 — 코드 변경 0). **M4 dead code 제거** — `errDictKey` sentinel(Where-gate PASSED 분기 확인) + `WorkflowAgentPurposes()` 함수(전체 트리 참조 0건 확인 후 삭제, REQ-WC12-030..032). **M5 scope-contract doc comment 재작성** — `sectionroute.go`/`sectionwrite.go` 파일 헤더 주석의 "7개 seam 섹션" 등 stale 카운트를 6개로 정정 + research/db가 "폐선 섹션"으로만 등장함을 명시(REQ-WC12-040). **AC 20개 중 19 PASS + 1 PASS-WITH-DEBT**(AC-WC12-020 — `go test ./...` 전체 스위트 결과 중 `internal/hook` `TestHookWrapper_TempFileCleanup` 사전 존재 flake 1건, 격리 재실행 3/3 PASS로 확인된 스코프 외 결함; 본 SPEC이 유발한 것 아님). 양표면 회귀 테스트(`TestI18nKeySetParity` web+cli 양쪽) + `TestWriteSectionViaSeamRejectsResearchPreservesFile` 신설 PASS. Run-phase M1-M5(M3는 코드 무접촉 검증 전용 milestone) `28e199b89`/`ebece333a`/(M3 no-diff)/`67929244e`/`3143d2293`, Mode 5 sub-agent. cross-platform build green(darwin+windows), `golangci-lint` 0 issues(baseline 유지), coverage `internal/settings` 88.6% / `internal/settings/agentfm` 85.3% / `internal/settings/yamlpatch` 86.3% / `internal/web` 69.8%(baseline-identical, net-negative dead-code 제거로 회귀 아님). README 무변경 확인(moai web console 설정 섹션에 대한 사용자 가시적 문서 언급 없음 — 검토 완료, 영향 없음). Sync-phase 3-phase close(단일 sync commit + `sync_commit_sha` follow-up backfill chore commit). 🗿 MoAI
...
- **[SPEC-TEMPLATE-I18N-CANONICAL-001](.moai/specs/SPEC-TEMPLATE-I18N-CANONICAL-001/spec.md)** — Multilingual canonical-form sweep for always-loaded template rule files (Tier L, v0.2.0): askuser-protocol.md·session-handoff.md·context-window-management.md을 English-canonical framing으로 전환하면서 4-locale localization mechanism 보존. CI lint C9 class(natural-language-canonical-form) 추가 + doctrine §25.6 amendment. AC 23/0/0 (모두 PASS). V8 diagnostics cleanup(template_neutrality_audit_test.go: Go 1.26+ `max()` built-in + loop variable per-iteration 적용). Run-phase M1-M4 (`9ef2117e4`, `273963326`, `b9a24fee0`, `5aa05403e`), Mode 5 sub-agent. Sync-phase 3-phase close (본 commit). 🗿 MoAI

### Fixed

- **[SPEC-CLIFIX-CONTRACT-001](.moai/specs/SPEC-CLIFIX-CONTRACT-001/spec.md)** — CLI Contract Remediation (P1, v0.1.0, `internal/cli` — 11 os.Exit → ExitCoder adoption, exit-code contracts, github --dry-run / spec_status --yes / pre-push stderr, flock test rename + contract tests). **BREAKING CHANGE**: `moai spec audit` MUST-FIX drift exit code 변경 1→2 (REQ-CONT-001-005, per-plan.md §G CI-pipeline risk clause — CI pipelines gating on `spec audit` exit code == 1 as the sole failure signal would break; help text already documents exit 2, implementation aligns to it). **M1 ExitCoder adoption** — 8개 파일(astgrep.go:119; hook.go:231+:362; spec_drift.go:74; harness/execute.go:333; hook_pre_push.go:196; spec_lint.go:70+:96; migrate_agency.go:650+:652; agentlint/workflow_lint.go:158)의 11 os.Exit 사이트를 ExitCoder return으로 치환(inline comment 주장 "ONLY os.Exit site"은 허용 대상 아님, 제거 대상; Windows process-replacement 예외: launch_exec_windows.go:37,41 + update.go:487 유지). **M2 exit-code contracts** — astgrep HasErrors(json/sarif) exit 1 구현; constitution check 실패 시 exit 2; spec lint invalid args exit 3; spec audit MUST-FIX drift exit 2 (위 BREAKING CHANGE). **M3 flags + streams** — github --dry-run flag 연결(실제 registry 쓰기 금지 + 플랜 출력); spec_status --confirm dead flag 제거 + --yes(canonical non-interactive flag)로 대체 + non-TTY context에서 abort(git mutation 차단, fmt.Scanln hang 제거); pre-push violation details stderr로 라우팅(Claude Code는 exit 2 시 stderr만 표시). **M4 test enablement + closure** — team_spawn_lock_test_unix.go → team_spawn_lock_unix_test.go rename(테스트 실행 가능 + testing 패키지 생산 binary에서 제거); contract test suite 추가(REQ-CONT-001-008, 도움텍 명시된 exit code vs 실제 생성 exit code 단언). **8/8 AC PASS** (acceptance.md §D AC Matrix). Run-phase M1-M4(`6027424d4`, `6fa48a7e9`, `017f176f3`, `64ebeacca`), Mode 5 sub-agent(TDD, sequential per-milestone). cross-platform build green, `golangci-lint run` 0 issues. 🗿 MoAI

- **[SPEC-CLIFIX-CONCURRENCY-001](.moai/specs/SPEC-CLIFIX-CONCURRENCY-001/spec.md)** — CLI Concurrency Remediation (P2, v1.0.0, `internal/cli` + `internal/cli/preference` — 6 REQ / 6 AC, 다중 세션 경쟁 hardening). **M1** — 5개 `settings.local.json` writer(glm.go:548/638/971 + launcher.go:288/685)를 기존 `mutateSettingsLocal` locked+atomic seam(settings.go:79)으로 재라우팅 + `removeGLMEnv` delete-set에 `CLAUDE_CODE_AUTO_COMPACT_WINDOW` 추가(launcher.go:292). **M2** — `~/.claude.json` RMW guard: `mutateClaudeJSONAtomic`(glm_tools.go:466) — advisory flock + compare-and-retry 병행 RMW(`claudeJSONGuardMaxRetries=3`)로 concurrent write 손실 방지. **M3** — atomic-writer 6-site inventory(3 named helper + 2 inline + 1 option-b exception) → `writeFileAtomic`(settings.go:121)로 통합; `atomicWriteJSON`(update_noise.go:74) + `writeClaudeJSONAtomic`(glm_tools.go) 삭제; `atomicWrite`(preference/filestore.go:473)은 package-internal exception(별도 leaf package 추정은 scope expansion). **M4** — Preference store TOCTOU hardening: DecayScan crash-reconciliation(`reconcileRecallArchival`), `MarkScanned` atomic-write(`atomicWrite` 위임), TogglePersonalization O_EXCL locked seam(stdlib `os.OpenFile` with `O_CREATE|O_EXCL` — no build-tag, no import cycle). **6/6 AC PASS** — concurrent writer tests + RMW race tests + atomic-writer characterization tests + preference crash/TOCTOU tests. Run-phase M1-M4(`37b535820`, `2e4513f90`, `3260d0839`, `83e139bb0`), Mode 5 sub-agent(TDD, 순차). Cross-platform build green(darwin+windows), `golangci-lint run` 0 issues, `go test -race ./internal/cli/... ./internal/cli/preference/...` green. Subagent boundary clean(grep 0 matches). 🗿 MoAI

### Fixed

- **[SPEC-CLIFIX-CRITICAL-001](.moai/specs/SPEC-CLIFIX-CRITICAL-001/spec.md)** — CLI Critical Remediation P0 8결함 수정 (Tier M, v0.1.0, `internal/cli` bug fixes — 사용자 설정 파일 데이터 손실·손상 방지, CLI surface 변경 0). CLI 감사 2026-07-10 §1 Critical 8건을 Reproduction-First(TDD RED→GREEN)로 전부 수정: (a) **REQ-CRIT-001-001** `.claude/settings.local.json` read-modify-write가 closed `SettingsLocal` struct를 marshal해 struct가 모델링하지 않는 top-level key(hooks/outputStyle/permissions/model/defaultMode)를 wipe하던 결함 → 6개 write-back path 전부(`mutateSettingsLocal` settings.go:28 seam + `ensureSettingsLocalJSON` + `injectGLMEnvForTeam` + `injectGLMEnv` + `removeGLMEnv` launcher.go + `syncPermissionModeToSettingsLocal`)를 `map[string]any` round-trip으로 전환(closed struct는 read-only convenience로 잔류, plan-audit D1 SHOULD-FIX가 5 paths로 undercount → run에서 6th path까지 확장); (b) **REQ-CRIT-001-002** team-mode tasklist ledger `ClaimTask`가 `O_RDWR`로 offset 0에 써 ledger header를 덮어쓰던 결함 → `O_APPEND|O_WRONLY`(team_spawn.go:324); (c) **REQ-CRIT-001-003** `saveWorkflowMuteConfig`가 minimal-struct YAML marshal로 `workflow.yaml` sibling key(agentic_loop/team)를 wipe하던 결함 → yaml.v3 Node API(`setYAMLNodeSequence`, harness.go:363 pattern 재사용); (d) **REQ-CRIT-001-004** `RemoveHarness`/`EditHarness`의 bare `harness-<name>` prefix match가 `release` vs `release-update` 형제 harness를 오삭제하던 결함 → `harnessArtifactBelongsTo` longest-match disambiguation(4 sites); (e) **REQ-CRIT-001-005** `acquireUpdateLock`가 production caller 0건으로 lockless `moai update` → `runUpdate` 파괴적 단계 전 lock 획득 wiring(update.go:208) + 완료 시 release + 경합 시 fail-fast diagnostic; (f) **REQ-CRIT-001-006** migrate_agency rollback이 pre-existing dir까지 무조건 삭제하던 결함 → `transactionLog.restoredFiles` snapshot + copyDir/copyFile pre-existence check로 migration-created path만 삭제; (g) **REQ-CRIT-001-007** archive walk의 `os.Stat`이 symlink 따라 out-of-tree content 복사 + `ModeSymlink` guard dead code → `os.Lstat`(migrate_agency.go:488); (h) **REQ-CRIT-001-008** Stop-hook auto-classify가 매 세션 tier-promotion 중복 append로 `tier-promotions.jsonl` 무한 증식 → per-pattern high-water mark(`readPromotionHighWater`)로 tier 변경 시에만 record. **11/11 AC PASS**(acceptance.md §D 매트릭스 — 9 numbered AC를 11 verification rows로 검증, 001b/002b grep variants 포함). orchestrator 독립 재검증: `go test ./internal/cli/... -count=1` exit 0(9 패키지), `golangci-lint run` 0 issues(baseline 0 유지), `go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0, `go test -race -run 'ClaimTask|UpdateLock'` exit 0, `@MX:TODO` 0건(전부 [AUTO] 사전 존재 주석). plan-audit PASS 0.88(8 앵커 live-tree CONFIRMED, D1 SHOULD-FIX + D2 MINOR — non-BLOCKING). Run-phase M1 repro batch(RED) → M2 config/ledger → M3 destructive-path → M4 growth/closure (`5703512c8`, `daba6b7ab`, `0bace43be`, `a1c38328e`, SHA backfill `bd1c3c6ed`), Mode 5 sub-agent(coding-heavy + 8 상이 fix mechanism → sequential). 병렬 세션 race absorbed(HOOK-OFFICIAL + TEMPLATE-I18N commits clean ff). Sync-phase **orchestrator-direct**(manager-docs 위임 context-window limit abort → CLAUDE.local.md §13 GLM orchestrator-direct sync/Mx fallback; 단일 sync commit으로 3-phase close, `sync_commit_sha` follow-up backfill). README 무변경 — 내부 CLI 결함 수정, 사용자 가시적 surface 변경 0(데이터 무결성 복원은 동작 변경). 🗿 MoAI

- **[SPEC-OBSERVE-HYGIENE-001](.moai/specs/SPEC-OBSERVE-HYGIENE-001/spec.md)** — 관측 싱크(observation sink) 위생 정리: 6개 write-only/dormant 싱크 그룹에 consume-or-document-or-retire 적용 (Workflow-Reflex Epic 6/6, Tier S, v0.1.0). 3-lens 워크플로 감사가 확증한 H4/H5/H6/L7 4개 verified gap을 기계적 AC로 봉쇄 — (1) **REQ-OBH-001 status-transition-audit.log consumer**: `moai spec audit`가 `.moai/logs/status-transition-audit.log`를 cross-check 입력으로 소비 — 전이 사이트를 Status Transition Ownership Matrix 소유자와 대조, mismatch 시 INFO finding 방출(absent/corrupt log은 graceful no-op, M1 `2a6238c63`, `internal/spec/audit.go` 확장 + fixture 테스트, coverage 88.3%); (2) **REQ-OBH-002 telemetry pruning**: SessionEnd 경로가 zero-byte `trace-*.jsonl` 정리 + 30일 age retention 적용(`DefaultTraceRetentionDays` in `internal/config/defaults.go`), `task-metrics.jsonl`은 root-cause note + document write-only + age-out 처리(writer path `internal/hook/post_tool.go:~166`, 2026-05-30 이후 쓰기 중단, 원인 추정 upstream Agent/Task event shape change — inconclusive → default document+age-out, M2 `507f72f2c`); (3) **REQ-OBH-003 fact-force-skip.log documented write-only**: `gateguard-fact-force.sh` header에 one-line write-only 명시(local audit trail, no mechanical consumer); (4) **REQ-OBH-004 D3=Promote sync-gate blocking**: `sync-phase-quality-gate.sh`의 `go vet`/`go build` deterministic check를 advisory(opt-in `MOAI_SYNC_GATE_BLOCKING=1`)에서 **default-blocking**으로 승격 — env var가 opt-OUT(`=0/off/false/advisory/no`)으로 전환(legacy `=1`은 redundant로 수용), tests/coverage는 advisory 유지, stdout-JSON exit-0 signaling + `--skip-hook` escape + runtime-recovery §4 carve-out 보존; (5) **REQ-OBH-005 loop.md best-effort marker**: snapshot/resume 섹션에 best-effort marker 추가(`.moai/cache/loop-snapshots/` dir의 mechanical writer 보장 없음 명시 — documentation-only, mechanical persistence는 SPEC-LOOP-VERDICT-CONTRACT-001 follow-up 소관); (6) **REQ-OBH-006 dormancy annotations**: `sunset.yaml`(DORMANT — typed struct loaded, no runtime hot path, Go-side notice 존재 REQ-MIG003-006/-018) + `harness.yaml` `model_upgrade_review`(dormant-in-practice — consumer `emitModelUpgradeReminder` 존재하나 trigger env `CLAUDE_MODEL_PREVIOUS`/`CLAUDE_MODEL` setter 부재)에 one-line dormancy comment 추가(comment-only, zero semantic change — config tests bit-identical green). M3 `85a2579ed` + catalog hash regen `c4671b4ae`. template-first mirrors(live↔template byte-identical: 2 hook scripts + 2 config + loop.md), `make build` exit 0. **7/7 AC PASS**(AC-OBH-001..007; AC-OBH-007 PASS-WITH-DEBT — 2 pre-existing FAIL은 OBSERVE 유발 0건: (a) `internal/cli` TestDoctor/Status golden mismatch = TEST-003 AC-006 external debt[stale golden, ARCH-001 M0 소관], (b) `internal/template` TestOutputStylesTemplateLiveParity moai-easy.md = 별도 chore). cross-platform build green(linux+windows exit 0), lint 0 issues, subagent-boundary grep 0, no `.moai/logs/` committed. D3 decision = Promote(Implementation Kickoff Approval에서 사용자 확정). Run-phase (manager-develop TDD Mode 5 sub-agent, M1→M2→M3 순차 — manager-develop M3 중 API 429 abort → orchestrator-direct M3 완료, Hybrid Trunk main-direct Route A). Sync-phase **now** (manager-docs, 단일 sync commit으로 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close / Status Transition Ownership Matrix; spec/plan/acceptance frontmatter status `in-progress → completed` atomic + `updated: 2026-07-09` + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry + `sync_commit_sha` follow-up chore commit backfill). README 무변경 — 내부 관측 위생 + gate default 변경, 사용자 가시적 CLI surface 변경 0(단 D3 gate promotion은 sync commit 시 `go vet`/`go build` 실패가 차단되는 동작 변경 — `MOAI_SYNC_GATE_BLOCKING=0`으로 opt-out 가능). 🗿 MoAI

- **[SPEC-MOAI-SKILL-DOCTRINE-FIX-001](.moai/specs/SPEC-MOAI-SKILL-DOCTRINE-FIX-001/spec.md)** — `/moai` 스킬 폴더(`.claude/skills/moai/`) 전면 독트린 드리프트 정리: 9-way 병렬 read-only 감사가 42개 파일(~10,043줄)에서 확증한 70건(CRITICAL 7 / MAJOR 32 / MINOR 31)을 9개 파일 분리(file-disjoint) write-group으로 병렬 fanout 수정 (Tier L, v0.1.0, 문서 정정 + CI 테스트 1건). 6개 교차 테마: (1) **Tier-based PR 라우팅 복원** — `run/task-decomposition.md`·`sync/delivery.md`가 모든 SPEC에 manager-git을 무조건 spawn하던 것을 spec-workflow.md Frozen HARD Route A/B(Tier S/M = manager-develop main-direct, Tier L/`--pr` = manager-git)로 분기; (2) **문서↔코드/설정 드리프트** — `spec-assembly.md`의 harness.yaml plan_audit 정반대 기술 정정(minimal=lightweight 1-iteration, not skip), `team/run.md` 역할-모델 표 정정(architect opus·reviewer sonnet, workflow.yaml 대조), `team/glm.md` legacy `CLAUDE_CODE_TEAMMATE_DISPLAY`→`teammateMode` 필드, `meta-harness.md` FROZEN 경로를 `frozen_guard.go` 4-prefix 실측과 일치, `harness.md`의 거짓 "CLI verb path retired" 전제를 실제 등록 상태로 정정 + `internal/cli/root.go` stale 주석 동반 수정, `doc-generation.md` `db.auto_sync.enabled` 중첩 스키마; (3) **죽은/깨진 참조** — 은퇴한 coverage/e2e 플래그 섹션·죽은 `moai-design-craft` 스킬·순환 Detection Keywords 참조·깨진 상대 링크 제거; (4) **세대 교체 잔재** — EARS→GEARS 표기, deprecated `moai-meta-harness` 리다이렉트, 은퇴 용어(Sprint/Wave) 치환; (5) **템플릿 중립성 누출** — 내부 SPEC-ID(`SPEC-DB-SYNC-RELOC-001` 등)·REQ/AC/`C-PH`/`REQ-HRN-FND` 토큰 제거 + `internal_content_leak_test.go`에 누락 shape 4종 탐지 클래스 확장(`skillMoaiScoped` 스코프 게이트로 sibling-skill false-positive 구조적 배제); (6) **에이전트 가시성/계약** — plan-auditor Quick Reference 등재, `@MX:DEBT` 태그 추가, sync-auditor 4-dimension 루브릭(Functionality40/Security25/Craft20/Consistency15) 정합, feedback.md `(권장)` 라벨, Phase 0.95 §F 로깅 계약. plan-auditor iter-1 FAIL 0.71 → iter-2 PASS 0.90 (Tier L threshold 0.85; iter-1이 원 감사의 CRITICAL 처방 2건 자체가 사실 오류임을 포착 — frozen_guard.go 4-prefix 오독 + harness Go-binary 실재 → 재작성 후 재감사 통과, verification-claim-integrity §1.1 surface 3 준수). Run-phase (manager-develop ddd, Mode 4 파일 분리 병렬 fanout 2 wave(WG-A~E, WG-F~H) + WG-I CI regex 순차; Template-First 양쪽 트리 byte-identical + make build catalog 재생성). **스코프 내 완전 green**: `go test ./internal/template/...` exit 0(누출+manifest-hash 통과), `go build ./...`+windows cross-build+`go vet` exit 0, 내부 토큰 누출 스캔 0. Pre-existing 트렁크 실패(`internal/cli` `TestDoctor`/`TestStatus` 골든 스냅샷 + `TestHookWrapper_TempFileCleanup`)는 base HEAD에서도 재현되는 스코프 밖 사전 결함(stash 후 재실측으로 귀속 확정, PRESERVE). 🗿 MoAI

- **[SPEC-AUDIT-GATE-INTEGRITY-001](.moai/specs/SPEC-AUDIT-GATE-INTEGRITY-001/spec.md)** — 감사 게이트 무결성 P0 4결함 수정 (Tier M, v0.1.2, doc-only — Go 코드 동작 무변경, 2026-07-09 3-agent 병렬 감사 P0 후속): (R1) **plan-auditor D7/D8 BLOCKING을 Must-Pass에 배선** — M5 Must-Pass Firewall에 **(MP-5)** "no unresolved D7 BLOCKING" / **(MP-6)** "no unresolved D8 BLOCKING" 추가(must-pass-equivalent: aggregate 점수 무관 `Verdict: FAIL` 강제 + `## Defects Found` severity=critical fold), Output Format `## Must-Pass Results` 표에 MP-5/MP-6 행 추가로 BLOCKING finding이 점수에 흡수-소멸하는 구조 차단; (R2) **sync-auditor 채점모델 정합 + 차원별 기계 검증** — flat 가중치 모델(Functionality 40/Security 25/Craft 20/Consistency 15)을 default로, `harness.yaml` `evaluator_mode: hierarchical` 시 HRN-003 계층 모델(동일 4차원의 sub-criteria refinement, canonical anchors 0.25/0.50/0.75/1.00) 적용을 규범화 + `### Hierarchical-Mode Output Example` worked example + 차원별 언어중립 기계 검증 명령 표(project-language auto-detection, 4개 언어 동등 예시, Evidence verbatim 인용 의무 — verification-claim-integrity §1.1 surface 2) + frontmatter skills에 `moai-ref-owasp-checklist`/`moai-ref-testing-pyramid` preload; (R3) **plan-audit 보고서 파일명 split-brain 해소** — `{SPEC-ID}-review-{N}.md` **plan-phase review stream**(iteration 기반)과 `<SPEC-ID>-<YYYY-MM-DD>.md` **run-gate stream**(날짜 기반, `internal/runtime/audit_report.go`)을 의도된 별개 스트림으로 spec-workflow.md § Report Persistence + plan-auditor.md § Output Format 양쪽에 상호 참조 명문화, skip-eligibility 입력을 review stream **final-iteration verdict** + **plan-artifact hash**(`audit_cache.go` `ComputeHash` 4-파일 whitespace-normalized SHA-256)로 규범화(run-gate date-file은 verdict record surface일 뿐 hash 대상 아님); (R4) **manager-spec SPEC ID 자가 점검 mental → executed Bash** — `[[ "$ID" =~ ^SPEC(-[A-Z][A-Z0-9]*)+-[0-9]{3}$ ]]` 실행형 one-liner(verbatim 출력 인용 의무, mental-only PASS 주장 금지) 전환 + ~30줄 역사 서사를 memory/lessons로 압축 + stale `lint.go:573` 라인 인용을 `specIDPattern` content-token 앵커로 교정. 편집 대상: live 4(`.claude/agents/moai/{plan-auditor,sync-auditor,manager-spec}.md` + `.claude/rules/moai/workflow/spec-workflow.md`) + template mirror 4(Template Content Neutrality strip — 내부 토큰 누출 grep 0) + `internal/template/catalog.yaml` hash regen, `make build` exit 0. AC 25/25 PASS (acceptance.md SSOT 매트릭스). plan-audit iter-1 FAIL 0.78 → iter-2 PASS-WITH-DEBT 0.87 → iter-3 PASS 0.90. 🗿 MoAI

- **[SPEC-TEMPLATE-RULES-CLEANUP-001](.moai/specs/SPEC-TEMPLATE-RULES-CLEANUP-001/spec.md)** — 템플릿 배포 `.claude/rules/` 62파일 3-렌즈 감사(2026-07-09, session 98a5197a) 정리 + CI 가드 4종 확장 (Tier L, v0.1.1, doctrine/template/Go-test hygiene — 사용자 가시적 CLI 변경 0). 현행 가드 전부 PASS인 사각지대 5개 finding 그룹을 봉쇄 (28 REQ / 33 AC, 전부 PASS). **Finding A — 깨진 cross-reference 5건** (`REQ-TRC-010..012`): `.moai/docs/dev-only-commands-isolation.md`·`git-local-workflow-doctrine.md`·`git-workflow-doctrine.md`·`.moai/design/v3-redesign/synthesis/pattern-library.md` 등 배포되지 않는 문서 인용 5건을 normative 내용을 prose/shipped-equivalent로 보존하며 정리 (배포 후 404 HIGH 결함). **Finding B — 중립성 4클래스** (`REQ-TRC-020..027`): (1) REQ/AC 토큰 7건(askuser-protocol `AC-ADM-005..017`, sprint-round-naming `AC-LR-009`, settings-management `REQ-MIG003-006`) 중립 프로즈로 치환(pedagogical `REQ-XXX-*`/`AC-XXX-*` placeholder는 allowlist 보존); (2) `lessons #N`·incident `W#` 관용형 5라인(`W3 meta-analysis`/`W0 fix`/`W1/W2`/`W3 케이스`/`W3에서`) + `lessons #N` 4라인 전수 실측 소거(D4); (3) 내부 작업 날짜 9건(`2026-05-17` policy 등) — 날짜만 제거 정책 내용 유지, `NOTICE.md`는 Out-of-Scope 명시 제외(D1); (4) **`zone-registry.md` 템플릿 배포 제거** — 내부 거버넌스 레지스트리(1,019줄, `CONST-V3R*` 121건)를 template tree에서 제거, local dev 사본(`.claude/rules/moai/core/zone-registry.md`)은 보존(사용자 결정 2); `zone-registry` 참조 7줄/2파일(construction 5곳 + runtime-recovery-doctrine 2곳)을 비인용 프로즈로 재서술; `CONST-V3R*` 잔여 토큰 + `MIG-003` standalone heading 제거. **Finding C — 선별 백포트** (`REQ-TRC-030..032`): `§I Token Accounting` section-map row(template 부재 → 백포트, 내용 중립) + `runtime-recovery-doctrine.md` 제품명 보정(`MoAI`→`moai-adk` 14 라인) 백포트; sanitization-stripped 요소(`.moai/research/` 참조·`Version:`/`Origin:` footer·`CONST-V3R6-001` 토큰)는 template 부재가 sanitized-pair 계약에 의한 의도적 결과임을 명시(미복사). **Finding D — retired 어휘** (`REQ-TRC-040..041`): `design/constitution.md` Sprint Contract block(lines 323-350) retired-historical annotation 부여(기존 RETIRED 배너와 정합, 활성 `[HARD]` 조항 제거) + `.moai/sprints` path string 소거; `orchestration-mode-selection.md` standalone retired 용어 `cohort` 정식 분류 어휘로 치환. **Finding E — design 역드리프트 local 완결** (`REQ-TRC-050..051`, 사용자 결정 1): `bda9a3f33`이 template/Go 양측만 갱신하고 local 누락한 drift 해소 — local `design/constitution.md`을 post-cleanup template과 byte-identical 동기화(21,766B stale → 갱신본) + local `design.yaml`의 stale `design_docs:` block 제거(top-level key-set template과 정합, 활성 config 유지). **Finding F — CI 가드 4종 FULL 확장** (`REQ-TRC-060..066`, 사용자 결정 3): `internal/template/` 신규 regression 가드 4종 — (a) **REQ/AC-token rules-스코프**(`S3-req-ac-token-any-prefix` rules 확장), (b) **provenance lessons/W#**(RE2 호환 패턴), (c) **internal-date-provenance**(별도 test 함수 — default-tier `leakClasses`에서 분리, `TestLeakClassNoDateShaInDefaultTier` tier-ownership 보존), (d) **governance CONST/SPEC-V3R**(skill-body → rules 스코프 확장). 각 가드 file:line:matched-token + greppable sentinel 출력 + recurrence backstop self-test(synthetic re-leak probe). TDD RED→GREEN 증명(M1 시점 4종 FAIL = 기지 위반 ≥1건 실명 검출 → M7 시점 4종 PASS). 7 마일스톤: M1 `b32441484`(CI guards RED baseline), M2 `29f434a98`(Finding A 깨진 참조), M3+M5 `729bc31cd`(중립성 + retired 어휘), M4+M6 `93abb5082`(백포트 + local design 완결), M7 `1e24f9d9a`(GREEN 가드 + §E.2 evidence). Template-First 양쪽 트리 동기화(`make build` catalog hash regen, byte-parity `rule_template_mirror_test.go` + `sanitized_pair_parity_test.go` 8-entry registry 보존). 33/33 AC PASS(Group P 3 + A 4 + B 10 + C 4 + D 3 + E 3 + F 6). plan-audit iter-1 D1-D8 defects → iter-2 PASS 0.91(Tier L 임계 0.85). 한국어 콘텐츠 정규화 클래스는 사용자 결정 4로 후속 SPEC 명시 이연. Run-phase (manager-develop TDD, Hybrid Trunk main-direct Route A, M1 TDD RED 선행 → M2-M6 순차 cleanup → M7 GREEN). Sync-phase **now** (manager-docs, 단일 sync commit으로 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close / Status Transition Ownership Matrix; spec.md frontmatter status `in-progress → completed` atomic + `updated: 2026-07-09` + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry). README 무변경 — 내부 template hygiene + regression 가드, 사용자 가시적 CLI 변경 0. 🗿 MoAI

### Added

- **[SPEC-HUMANIZE-002](.moai/specs/SPEC-HUMANIZE-002/spec.md)** — `moai-domain-humanize` 스킬 **v1.1.0 → v1.2.0**: claude.mo.ai.kr slop-review 스킬 지식을 2개 신규 모듈로 이식 (Tier M, v0.1.0, 문서 저작 SPEC — Go 코드 변경 0). **(1) `copy-review.md`** — CRS-1..7 리뷰 파이프라인(Stage 1-6: collection → language detection → pattern matching → context inference → fix-proposal(Original/Reason/Alternatives≥3/Preferred) → severity report), review-only 기본 모드(auto-apply 금지, 사용자 검토 후 적용) 및 4개 언어 slop 사전(CR-KO×8, CR-EN×9, CR-JA×4, CR-ZH×5 — 각 항목 부모 카탈로그 cross-ref, ungrounded 항목 0건); **(2) `design-copy.md`** — 랜딩/숏폼 카피 장르 가이드(헤드라인/CTA/카드/슬라이드/커버), 언어별 길이 휴리스틱(KO 자수/EN 단어수/JA 문자수+낭독성/ZH 자수+밀도), S1-S5 시나리오 검증(팩트 앵커 보존, 언어별 report severity 태깅); **(3) SKILL.md Genre-Module Routing** — 신규 2모듈을 기존 4개 언어 모듈과 연계하는 라우팅 테이블 추가; **(4) catalog.yaml** humanize entry `1.1.0 → 1.2.0` + hash regen (`gen-catalog-hashes --all`, 2-line diff only). **24/24 AC PASS** (acceptance.md SSOT — 16 mechanical / 3 hybrid / 5 manual; 언어 모듈 8개 byte-freeze 검증(`git diff` 0 files), template↔local parity IDENTICAL, neutrality 7-class grep PASS + `TestTemplateNoInternalContentLeak` PASS, MIT 잔재 0). plan-audit PASS. Run-phase (manager-develop, 격리 worktree branch M1-M3 `08cf1ca76`/`a5c713a97`/`199c5c1a8` → fast-forward merge `6b96377e9`로 main 편입; Template-First 양쪽 트리 `diff -rq` IDENTICAL, cross-platform build green darwin+windows). Pre-existing debt: `TestOutputStylesTemplateLiveParity`(moai-easy.md parity, 별도 chore, humanize scope 무관). README 무변경 — 내부 스킬 콘텐츠 확장, 사용자 가시적 CLI surface 변경 0. 🗿 MoAI

- **[SPEC-HUMANIZE-001](.moai/specs/SPEC-HUMANIZE-001/spec.md)** — `moai-domain-humanize` 스킬 **v1.0.0 → v1.1.0**: 4개 언어 모듈 전체에 마케팅 카피(copy-genre) AI-tell 탐지 레이어 추가 (Tier L, v0.3.0, 문서 저작 SPEC — Go 코드 변경 0). 기존 prose-only 카탈로그가 커버하지 못하는 구조적·수사적 카피 tell 표면(랜딩 헤드라인, CTA, 태그라인, 브랜드/파운더 스토리텔링, 슬라이드 헤드라인 — *delve*/*tapestry* 없이도 기계 저작임이 드러나는 장르)을 언어별 native 카탈로그로 봉쇄. **(1) korean.md 전면 재저작(original work)** — v0.3.0 사용자 결정: v1.0.0의 "im-not-ai (Humanize KR) faithful port" 자기 서술이 부정확했음을 grep으로 검증(reference source는 메인테이너 자체 taxonomy, im-not-ai lineage 0건) → MIT 의존 해소, prose A–J + copy layer 전체를 원저작으로 재작성, NOTICE.md 생성 대신 5개 dangling `See NOTICE.md` pointer 제거 + courtesy credit 1줄, `license: Apache-2.0` 무변경 (AC-HUM-016/017: NOTICE 참조 0 + MIT 토큰 0 기계 검증); **(2) 3개 언어 native copy layer 신규** — english.md ENC-1..9, japanese.md JA-10..14, chinese.md CN-L..Q (20 IDs, 전부 research.md verified-source 추적 — §5 quarantined hypothesis 승격 0건, prose 레이어는 additive-only로 byte-보존); **(3) SKILL.md copy-mode fact-anchor guard** — 숫자/고유명사/구체 스펙 앵커의 rewrite 보존 강제 + prose/copy 이원 grading table (`### Prose-Mode Grade Table` + `### Copy-Mode Grade Table`); **(4) catalog.yaml** humanize entry `1.1.0` + hash regen (`gen-catalog-hashes --all` → HASH FRESH). **19/19 AC checks PASS** (17 AC IDs incl. 006a/b/c; method mix 13 mechanical / 2 hybrid / 4 manual, acceptance.md SSOT) — 8-sample detect→rewrite→grade matrix 8/8 grade A + meaning drift 0, false-positive guard 3/3 NOT flagged (EN 인간 슬라이드 fragment / JA 전략적 体言止め 1회 / ZH crafted 排比). plan-audit iter-1 FAIL 0.78 → iter-2 FAIL 0.84 → iter-3 PASS-WITH-DEBT 0.87 + debt 2건 closed (Tier L 임계 0.85; AC-006b `verbless` false-FAIL 등 self-collision류 정비). Run-phase (manager-develop, 격리 worktree M1-M6 + M3 follow-up + M7 evidence commits `d5df06b5f..85dc1c870` → merge `9e9c3d5bf`로 main 편입; Template-First 양쪽 트리 `diff -rq` IDENTICAL, `make build` exit 0, cross-platform build green; pre-existing 5 template test FAILs는 base HEAD `92d955daa` 재현으로 무관 입증). Sync-phase **now** (manager-docs, 단일 sync commit으로 3-phase close per Status Transition Ownership Matrix; spec.md frontmatter status `in-progress → completed` atomic + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry + `sync_commit_sha` follow-up chore commit backfill — 확립된 convention). README/docs-site 무변경 — skill-internal 변경, CHANGELOG 외 사용자 가시적 문서 표면 0. 🗿 MoAI

- **[SPEC-WORKFLOW-LIFECYCLE-001](.moai/specs/SPEC-WORKFLOW-LIFECYCLE-001/spec.md)** — SPEC 워크플로 수명주기 P1 개선 (Tier L, v0.1.0, doc-only — Go 코드 동작 무변경, SPEC-AUDIT-GATE-INTEGRITY-001 P1 백로그 3건 통합): (R1) **delta-spec 수명주기 — completed 사후 개정 전이** — `completed → in-progress` in-place amendment 전이를 Status Transition Ownership Matrix에 신규 행으로 추가(manager-spec 소관, canonical commit subject `feat(SPEC-{ID}): in-place amendment <rationale>`) + Optional frontmatter 필드 `amendment_of:` 도입(self-referential=in-place / parent-ID=successor) + HISTORY `## Amendments` sub-section(prior version + prior_completed_sha + rationale + scope, additive 보존), 새 9번째 `amended` enum 값 추가 없이 기존 `in-progress` 재전이로 해결(audit.go completed-no-drift predicate는 frontmatter가 in-progress이므로 비발화); (R2) **depends_on run-phase 집행 — Phase 0.5 pre-flight 차단** — `Depends_on Pre-flight Check`를 Phase 0.5 첫 sub-step으로 확장(0.6 신설 아님 — 단계 인플레이션 회피)하여 plan-auditor 호출 전 dependency `status: completed` 충족 여부를 엄격 판정(나머지 7값 전부 미충족, 점수 우회 불가), 미충족 시 AskUserQuestion 3-option blocker(wait / override `--ignore-deps` + `.moai/logs/depends-on-override.log` rationale 기록 의무 / abort) 방출; (R3) **Tier L 산물 집합 + plan-auditor 입력 계약 + hash 정합** — spec-frontmatter-schema.md `tier:` Optional 필드에 5-file 집합(spec/plan/acceptance/design/research) 명시(S/M/L 3-tier 차등) + plan-auditor.md § Input Contract를 Tier-differentiated로 재작성(literal token "Tier L: design.md + research.md are required inputs" + "Tier-differentiated input contract") + spec-workflow.md § Report Persistence에 plan-artifact hash subject list를 Go 구현 verbatim 4-file `{spec.md, plan.md, acceptance.md, tasks.md}`(`internal/runtime/audit_cache.go` `planArtifactNames` 정합)로 명문화 + design/research를 `manual-skip judgment inputs`으로 서술 + amendment시 `cache-invalidating event`로 작동 서술(tasks.md는 V3R4 잔재 보존). 편집 대상: live 4(`.claude/agents/moai/{plan-auditor,manager-spec}.md` + `.claude/rules/moai/development/spec-frontmatter-schema.md` + `.claude/rules/moai/workflow/spec-workflow.md`) + template mirror 4(Template Content Neutrality strip — AC-WFL-028 grep 0), `make build` exit 0. AC 30/30 PASS (acceptance.md SSOT 매트릭스). plan-audit iter-1 PASS-WITH-DEBT 0.96 skip-eligible(Tier L 임계 0.85, `.moai/reports/plan-audit/SPEC-WORKFLOW-LIFECYCLE-001-review-1.md`). Run-phase (manager-develop, M1 `b00cc6380` / M2 `bcb35f7d1` / M3 `e558f1e0f` / M4 `b3afb98bd` / M5 `477e700de` + merge `3c410a2db`, Hybrid Trunk main-direct Route A — local main 6 ahead of origin, NOT pushed). Sync-phase **now** (manager-docs, 단일 sync commit으로 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close / Status Transition Ownership Matrix; spec.md frontmatter status `in-progress → completed` atomic + `updated: 2026-07-09` + progress.md §E.3 `run_commit_sha` backfill `477e700de` + §E.4 audit-ready signal + CHANGELOG 본 entry; `sync_commit_sha` follow-up chore commit backfill 예정). README 무변경 — 내부 워크플로 doctrine + 산물 계약, 사용자 가시적 CLI 변경 0. 🗿 MoAI
- **[SPEC-CADENCE-BRIDGE-001](.moai/specs/SPEC-CADENCE-BRIDGE-001/spec.md)** — cadence(정기 발견) 작업을 위한 승인된 레시피 카탈로그: native `/loop` 스케줄러 + Cron을 **엄격히 read-only/advisory** `/moai` 진입점과 조합 (Workflow-Reflex Epic 4/6, Tier S, v0.1.0, doctrine-text-only, 신규 Go 코드 0). 스케줄러는 존재하지만 read-only 진입점과의 조합이 없어 drift·over-engineering creep·ceiling-exit 잔여물이 아무것에도 발견되지 않던 갭을 메움 — **레시피 3종**: (1) **Drift Watcher** `/loop 30m /moai gate`(<30s validation pass, fix 미적용); (2) **Lean Review** 야간 `/moai review --lean`(over-engineering-only scan, PASS/FAIL verdict 없음, 파일 수정 0); (3) **Backlog Re-Discovery** 주기적 `.moai/state/loop-verdict-*.json` read(prose-reader, 도구 호출 0). **카탈로그-레벨 HARD 불변식**: 스케줄된 실행은 commit/push/run-phase 진입 금지, Level-1 미커밋 working-tree edit만 유일 예외; Implementation Kickoff Approval은 인간 전용·cadence-unsatisfiable. **Discovery-to-Queue 계약**: cadence 실행이 작업을 발견하면 TaskList 또는 `.moai/reports/cadence/<date>.md`에 queue, 자동 remediation 금지. eligibility table로 `/moai run`·`sync`·`loop`·`fix`(Level-1 제외) 등 write-capable 서브커맨드를 명시적 비승인 분류. **6/6 AC PASS** (Tier S 임계 0.75; plan-auditor PASS-WITH-DEBT 0.86, D1 headline/body 정정 `f2f0975c2`). Run-phase (manager-develop, 커밋 `eb601d0b7`(M1+M2 sanctioned recipe catalog), Hybrid Trunk main-direct Route A; live↔template byte-identical `cadence-bridge.md` 신규 + `goal-directive.md` 교차참조). Sync-phase **now** (orchestrator-direct — GLM backend + 공유-checkout 레이스로 temp-worktree 격리 착지 패턴 per `feedback_glm_orchestrator_direct_sync_mx` + `feedback_shared_checkout_concurrent_commit_race`; 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001: spec/plan/acceptance frontmatter status `in-progress → completed` atomic on single sync commit + `updated: 2026-07-09` + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry + `sync_commit_sha` follow-up chore commit backfill). README 무변경 — doctrine-text-only, 사용자 가시적 CLI 변경 0. 🗿 MoAI
- **[SPEC-FIX-LOOPMAP-001](.moai/specs/SPEC-FIX-LOOPMAP-001/spec.md)** — fix Phase 4 기계적 검증 + 회귀 가드 + 루프 분류 매핑 (Tier S 신규, v0.1.0, doctrine/skill-doc, 신규 Go 코드 0). LOOP-VERDICT-CONTRACT-001이 이연(L5)한 fix Phase 4 비독립 검증 부채를 해소 — (1) **증거 기반 Phase 4**: prose-only "Confirm/Detect"를 exit code + 카운트 기반 claim/evidence rows로 전환(fix.md Step 4); (2) **baseline 회귀 가드**: fix 적용 전후 baseline 대비 회귀 탐지; (3) **one-shot-residue 영속화**: 미해결 exit 시 잔여 이슈를 `/moai loop` 승격 권고와 함께 영속화(자동 진입 금지); (4) **Loop Taxonomy Position**: fix.md + loop.md 양쪽에 턴/목표/시간/선제 4-루프 분류에서의 위치 문서화. **10/10 AC PASS** (Tier S 임계 0.75; plan-auditor PASS-WITH-DEBT 0.88). Run-phase (manager-develop, 커밋 `47ffb9f51`(plan) + `b116a5223`(M1-M3), Hybrid Trunk main-direct Route A; live↔template byte-identical `fix.md`+`loop.md` 2 pairs). Sync-phase **now** (orchestrator-direct — GLM backend + 공유-checkout 레이스로 temp-worktree 격리 착지 패턴 per `feedback_glm_orchestrator_direct_sync_mx` + `feedback_shared_checkout_concurrent_commit_race`; 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001: spec.md frontmatter status `in-progress → completed` atomic on single sync commit + `updated: 2026-07-09` + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry + `sync_commit_sha` follow-up chore commit backfill. plan.md/progress.md는 YAML frontmatter 없음 — `# ` 헤더 구조, acceptance.md 부재[Tier S 2-artifact], era.go는 spec.md frontmatter + §E.* headings 파싱). README 무변경 — doctrine/skill-doc, 사용자 가시적 CLI 변경 0. 🗿 MoAI
- **[SPEC-LOOP-VERDICT-CONTRACT-001](.moai/specs/SPEC-LOOP-VERDICT-CONTRACT-001/spec.md)** — Utility loop 기계적 종료 술어 + ceiling-exit verdict contract (Workflow-Reflex Epic 3/6, Tier M, v0.1.0, doctrine/skill-doc 전용, 신규 Go 코드 0). 외부 리서치 28출처(Anthropic 공식·arXiv 4편·Kong/Datadog 사례)가 확증한 3가지 검증 가능한 결함을 기계적 AC로 봉쇄 — (1) **자가 선언 종료 문장**: loop builder가 수행+선언 겸직이던 exit 문장을 기계적 술어(`loop.md` Step 1)로 전환 — `/moai gate` exit code + convergence 카운트 기반; (2) **비독립 최종 패스(L5)**: fix Phase 4 검증이 builder 자신에게 있던 것을 Step 1.5 독립 평가자(`/moai gate` 재실행, divergence→계속/승격 분기)로 분리; (3) **ceiling 4-표면 파편화**: Step 9 ceiling을 5-섹션 verdict contract로 통일 + `.moai/state/loop-verdict-<id>.json` 영속화(exit_kind / remaining_issues / ceiling signal). 추가: 우선순위 3-표면 통일(`--max` > ralph.yaml 10 > loop_prevention) + stale loader 주석 수정. **13/13 AC PASS** (Tier M 임계 0.80; plan-auditor PASS-WITH-DEBT 0.81). Run-phase (manager-develop ddd, 커밋 `e0298c230`(M1+M2) + `083cf52cf`(M3), Hybrid Trunk main-direct Route A; live↔template byte-identical 4 file pairs). Sync-phase **now** (orchestrator-direct — GLM backend + 활발한 공유-checkout 레이스[RATCHET sync 착지 `d99892883` 직후]로 temp-worktree 격리 착지 패턴 재사용 per `feedback_glm_orchestrator_direct_sync_mx` + `feedback_shared_checkout_concurrent_commit_race`; 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001: spec/plan/acceptance frontmatter status `in-progress → completed` atomic on single sync commit + `updated: 2026-07-09` + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry + `sync_commit_sha` follow-up chore commit backfill). README 무변경 — doctrine/skill-doc 전용, 사용자 가시적 CLI 변경 0. 🗿 MoAI
- **[SPEC-HARNESS-RATCHET-REWIRE-001](.moai/specs/SPEC-HARNESS-RATCHET-REWIRE-001/spec.md)** — 하네스 학습 루프(learning loop)에 failure 신호 배선: FAILURE→RATCHET 반루프 실체화 (Workflow-Reflex Epic 1/6, Tier M, v0.1.0). 3-lens 워크플로우 감사가 확증한 3개 verified gap(H1/H2/L2)를 기계적 AC로 봉쇄 — (1) **H1 failure 신호 블랙홀**: PostToolUseFailure 핸들러 + evidence-writer test-fail 검출이 관찰 로그(`.moai/harness/usage-log.jsonl`)에 lifecycle 이벤트만 기록하고 failure 이벤트는 분류기/제안 파이프라인에 전혀 먹이지 않던 결함 → 신규 이벤트 키 `tool_failure:<tool>:<signature>` / `test_fail:<package>:` 기록 추가 (signature = 저카디널리티 `ErrorCategory` 토큰, D1 resolved); (2) **H2 degenerate 키 프로모션**: tier 분류기가 `subagent_stop:unknown:` (empty context_hash, 41 obs) / `session_stop::` / `user_prompt::` 등 학습 불가능한 lifecycle 노이즈를 confidence 1로 프로모션하던 결함 → `IsEligibleForPromotion(p)` predicate로 분류 시점 필터링(D4), 회귀 테스트로 `subagent_stop:unknown:` 프로모션 0건 입증; (3) **L2 파이프라인 후반부 굶주림**: `.moai/harness/proposals/` 디렉토리가 한 번도 생성된 적 없고 제안 생성이 수동 `moai harness propose` 트리거 전용이던 결함 → Stop-path classify 완료 후 프로모션 ≥1건이면 `generateProposals(root)` 자동 체인 (D2: callable seam, subprocess 없이 5s hook budget 내 fail-open). **4개 마일스톤**: M1 `e70c77576` failure-event recording (types.go EventTypeToolFailure/EventTypeTestFail + PatternBearing, post_tool_failure.go, evidence_writer.go, failure_observer.go 신규 + failure_event_test.go); M2 `838e38102` classifier eligibility + propose auto-run (IsEligibleForPromotion + classifyHarnessPatterns 루프 내 필터 + Stop-path generateProposals 체인, AC-HRR-005/006/007 — auto_apply: false 무결, apply 호출 0건); M3 `69041f969` lessons inbox + doctor + doctrine (`.moai/lessons-inbox.jsonl` append-only JSONL stub — ts/event_key/summary/source, 0o600 perms, D3 + `moai harness doctor` checkPipelineDormancy dormancy 경고 + moai-constitution.md § Lessons Protocol lessons-inbox drain 상호참조 live↔template mirror); M4 `87471fb18` coverage hardening (internal/hook + internal/cli/harness touched files ≥85%); §E.3 `3d81db272` run-phase audit-ready signal. **인간 apply 게이트 보존 (REQ-HRR-005, HARD)**: `learning.auto_apply: false` (harness.yaml:116) 무결, Stop 체인은 제안 생성에서 종료, apply는 오케스트레이터 AskUserQuestion 게이트 + 명시적 `moai harness apply` 뒤에만 동작. **Fail-open 훅 규율 (REQ-HRR-004, HARD)**: propose 체인은 기존 classify 체인의 fail-open 동작을 미러 (오류 stderr 로그, exit 0, session end 차단 없음, 5s hook budget). **13/13 AC PASS** (AC-HRR-001..013, 전부 MUST-PASS; safety-adjacent 파이프라인이므로 SHOULD/NICE 티어 미정의). Sync-phase scoped 검증 (RATCHET 패키지 한정 — working tree에 병렬 세션 수정 파일 존재하여 full `go test ./...` 회피): `go test ./internal/harness/... ./internal/hook/... ./internal/cli/harness/...` exit 0 (22 패키지 all ok, 0 FAIL), `golangci-lint run` 동일 scope exit 0 (0 issues, NEW vs baseline = 0). Subagent boundary (AC-HRR-012, C-HRA-008): `AskUserQuestion(` call-site 패턴 = 0 (oversight.go:22 docstring 1건 제외). Plan-auditor PASS 0.90 (Tier M 임계 0.80, skip-eligible 경계). Run-phase (manager-develop TDD, Mode 5 sub-agent, Hybrid Trunk main-direct Route A; M1→M2→M3 순차 의존 + M4 coverage + §E.3 signal, 5 commits origin/main). Sync-phase **now** (manager-docs, 단일 sync commit으로 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close / Status Transition Ownership Matrix; 3-artifact frontmatter status `in-progress → completed` atomic on single sync commit + `updated: 2026-07-09` + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry. `sync_commit_sha`는 follow-up chore commit에서 backfill — 확립된 convention per SPEC-BRAND-DIR-REMOVE-001 `2c637ece2` / SPEC-TOKEN-BUDGET-STOP-001 `ab4f38def` / SPEC-INTERNAL-TEST-002 `6116a0972`). README 무변경 — 내부 학습 루프 배선 + doctor dormancy additive check, README Harness 섹션은 v4 Builder + lifecycle를 문서화하며 learning-loop 내부/doctor 서브체크는 미문서화 → 사용자 가시적 README surface 변경 0. 🗿 MoAI
- **[SPEC-HANDOFF-ONEPASTE-001](.moai/specs/SPEC-HANDOFF-ONEPASTE-001/spec.md)** — 세션 핸드오프 1-paste화: 방출↔소비 파이프라인 배선 (Tier M, v0.1.2, doctrine+config 전용, 신규 Go 코드 0). SPEC-HANDOFF-AUTORESUME-001이 구축한 소비 인프라(`moai handoff save/clear` CLI + SessionStart claim-then-inject 주입)를 방출 측과 연결: (1) session-handoff.md(SSOT) **§ Emission-Time Save Obligation** 신설 — paste-ready resume 방출 시 오케스트레이터가 `moai handoff save --stdin --spec --phase [--goal|--ultrathink|--ultracode|--lang|--session]` 병행 실행 [HARD], fail-open(CLI 부재/오류 시 수동 경로 무손상, 재시도 루프 금지); (2) **§ Auto-Injected Resume Flow** 신설 — mode=auto ONE-message 흐름(/clear → claim-then-inject 자동 주입 → run-phase+기계검증 조건이면 `/goal` 1줄 goal-first, 그 외 승인 메시지 + `ultrathink` 키워드 포함 권고), /clear-only 주입 경계(startup/resume/compact = notice-only, guide 기본 false → 터미널 재시작 시 무주입, L3 Block 0 재개는 auto-inject 밖), 주입된 Block 4 전제의 재개-턴 시작 시 검증 의무(goal-first에서 가장 첨예), 2단계 `/goal` follow-up은 manual 폴백으로 보존; (3) § Auto-Memory Integration 항목 6 신설 — SPEC close 시 소비 완료된 verbatim resume 블록을 1줄 요약으로 축약(verbatim 보존은 `.moai/state/handoff/consumed/` 감사 트레일이 대체; 생성 시점 verbatim 영속 의무 불변, 시간적 분리, forward-looking only); (4) moai.md §8 압축 방출 조항 + goal-directive.md § MoAI Integration Notes 상호참조 + § Paste-Time Activation Matrix auto-inject 행. doctrine 3표면 live↔template mirror **byte-identical 동일 커밋**(session-handoff.md는 `rule_template_mirror_test.go` byte-parity 등록 — plan-audit iter-1이 live-only 편집 계획의 CI 실패를 BLOCKING D1으로 사전 포착); 로컬 `.moai/config/sections/handoff.yaml` `mode: manual → auto` 전환, **template 기본값 manual 유지**(사용자 opt-in 보존). Implementation Kickoff Approval 인간 게이트 불변(자동 주입·goal은 run 진입을 사전 승인하지 않음). 공식 근거: 큐잉된 슬래시 커맨드 리터럴-텍스트 전달로 2-paste 우회 불가 확증, SessionStart additionalContext 10K자 한도·턴 시작 불가, `/goal` `--resume` 복원·`/clear` 제거. **AC 17/17 PASS**(MUST-PASS 9 포함 AC-OP-017 `diff -q` byte-parity ×3 + mirror drift/leak 테스트 ok), plan-auditor iter-1 FAIL 0.78 → iter-2 PASS-WITH-DEBT 0.89(Tier M 0.80; R1-R3 잔여 부채 pre-run touch-up으로 해소), `moai spec lint` 0 findings, cross-platform build exit 0, `moai constitution validate` 77 findings는 base baseline과 byte-identical(신규 0). Run-phase (manager-develop ddd, M1 doctrine 이중-트리 + M2 config flip; 런타임 L1 worktree 격리 → orchestrator가 origin/main 기점 temp worktree cherry-pick 착지 `5faab23a2`/`78a8aefb3`/`303c41e94` + 독립검증 5/5, Route A main-direct push). Sync-phase **now** (orchestrator-direct — 활발한 병렬 세션 레이스로 동일 temp-worktree 착지 패턴 재사용, per `feedback_glm_orchestrator_direct_sync_mx` 선례; 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001: 4-artifact frontmatter status `→ completed` atomic + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry + `sync_commit_sha` follow-up chore commit backfill). README 무변경 — doctrine+로컬 config, 사용자 가시적 CLI 변경 0. 🗿 MoAI
- **[SPEC-TOKEN-BUDGET-STOP-001](.moai/specs/SPEC-TOKEN-BUDGET-STOP-001/spec.md)** — Token Budget graceful-abort + `/tmp` 증거 영속화 (Token-Economy Epic D 4/4 완결, Tier M, v0.1.0). `Tracker.IsAtHardLimit`(사용량 ≥ `HardClearThreshold` 90%) 신호를 graceful-abort 경로에 연결: WARN 로그 + session-handoff.md § Canonical Format 6-block 구조(`✂────` cut-line markers + `ultrathink.` opener + `applied lessons:` + `전제 검증:` + `실행:` + `머지 후:`)를 준수하는 paste-ready 핸드오프 자동 생성 + turn 종료 권고 (`shouldAbort` signal). **hard-fail 아님** — BC-V3R3-006 warning-first 보존, `RecordCall` signature 불변(error return 추가 없음), `/clear` 자동호출 금지 HARD constraint 보존(budget.go:18). file-redirect contract에 `.moai/state/verify/<session>/` 영속 의무 추가 — `/tmp` 클리어(macOS reboot / Linux tmpfs) 후에도 증거 도달 가능, `verification-claim-integrity.md` §1.1 surface 1(orchestrator self-report)+2(manager §E) 보존, **"persist evidence ≠ drop evidence"** 명시. doctrine LIVE↔template mirror 동기화(`agent-common-protocol.md` + `moai.md` §8 evidence path L413/L615, byte-identical edits); Go code는 template 미러링 안 함(per §15/§25). 9/9 MUST-PASS AC PASS, coverage `internal/runtime` **90.5%**(≥85%), golangci-lint 0, `go build`+`GOOS=windows` exit 0, `go vet` clean, mirror parity test PASS. Run-phase (manager-develop TDD, M1 `032eb9733` + M2 `841843a9b`, Hybrid Trunk main-direct Route A). Sync-phase **now** (orchestrator-direct, 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001: spec/plan/acceptance frontmatter status `in-progress → completed` atomic + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry + `sync_commit_sha` follow-up chore commit에서 backfill). README 무변경 — 내부 runtime + doctrine, 사용자 가시적 CLI 변경 0. 🗿 MoAI

- **[SPEC-INTERNAL-PERF-001](.moai/specs/SPEC-INTERNAL-PERF-001/spec.md)** — `internal/` 성능 개선: audit-origin 6건 verified defect를 자원 소비 상한(subprocess 수·할당량·렌더 횟수)으로 봉쇄 (Tier M, v0.1.1). spec-lint / mx fan-in / CLI / regex / merge / template 6개 논리 그룹. **M1 spec-lint git-query cache** (commit `d69d4a868`, REQ-PERF-001 P0): 신규 `internal/spec/gitquery_cache.go` (117 LOC) — per-SPEC `git rev-parse` 스폰을 per-run cached lookup으로 전환 (`cachedGitDirAvailable`/`cachedMainBranch`); 캐싱 메커니즘 구조적으로 sound (per-SPEC 4-spawn baseline → per-run 상수-spawn으로 N 증가에 무관하게 상한). **M2 regex package-level promotion** (commit `9d0f0acfa`, REQ-PERF-004): `era.go`·`status.go` 함수 본문 내 `regexp.MustCompile`을 package-level var로 이관 (본문 내 재컴파일 0, `progressFieldYAMLPattern` `@MX:ANCHOR`). **M3 mx fan-in single-traversal index** (commit `ad0ddaed0`, REQ-PERF-002): `internal/hook/mx/validator.go` `fanInIndex` 신규 — exported 함수당 full-project `filepath.WalkDir` 반복(θ(F×project_size))을 단일 순회 인덱스로 축소; 순회 중 `ctx.Done()` 인지로 timeout 중단 가능. **M4 CLI lazy initialization** (commit `96e0e1026`, REQ-PERF-003): `internal/cli/root.go` — trivial subcommand(`--version` 등) 경로에서 gopls bridge/security scanner/astgrep analyzer 초기화 회피 (초기화 카운터 테스트로 검증). **M5 merge diff size guard** (commit `48a5a9ff7`, REQ-PERF-005): `internal/merge/differ.go` `diffLinesThreshold` 가드 — 5,000줄 초과 입력 시 full O(m×n) DP 할당(~200MB) 대신 bounded alternate 경로(`@MX:NOTE` REQ-PERF-005-A). **M6 template single render** (commit `de4d1762f`, REQ-PERF-006): `internal/template/deployer.go` `renderCache` — `ValidateAll`/`Deploy` 시퀀스에서 `.tmpl` 2N 렌더를 N으로 (`@MX:NOTE` REQ-PERF-006-A, 선-검증 계약 보존). **lint cleanup** (commit `38f614960`, `differ_perf_test.go`). 17/17 AC sub-criteria across 6 REQ (AC-PERF-001a..006b) PASS — P0 방향성 검증(AC-PERF-001a code-inspection + sound per-run caching mechanism; 정량 upper-bound counting test double은 §K Run-phase Debt로 투명 기록, 사용자 승인). coverage `internal/spec` 88.3% / `internal/merge` 87.4% / `internal/template` 86.0% / `internal/hook/mx` 88.5% (전부 baseline 이상, non-decreasing). `go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0, golangci-lint 0 issues, `-race` clean. plan-auditor PASS (Tier M threshold 0.80). Run-phase (manager-develop TDD, Hybrid Trunk main-direct, 7 per-milestone commits M2→M5→M6→M1→M3→M4→lint 순차 + M7 evidence). Sync-phase **now** (manager-docs, 단일 sync commit으로 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close / Status Transition Ownership Matrix; 4-artifact frontmatter status `in-progress`/`draft → completed` atomic + `updated: 2026-07-08` + progress.md §E.4 audit-ready signal + V8 evidence-SHA correction(§E.2/§E.3 stale SHAs re-derived post-rebase) + V9 §K Run-phase Debt(AC-PERF-001a-mechanical-gap) + MX tag validation PASS + CHANGELOG 본 entry. `sync_commit_sha`는 follow-up chore commit에서 backfill — 확립된 convention per SPEC-BRAND-DIR-REMOVE-001 `2c637ece2` / SPEC-INTERNAL-SECURITY-001 `f3193bac8` / SPEC-HANDOFF-GOALFIX-001 `bf983aaf5`). README 무변경 — 내부 성능 최적화, 사용자 가시적 CLI 변경 0. Pre-existing baseline failures (out of SPEC scope per acceptance.md §D.4): `internal/cli` `TestDoctor`/`TestStatus` statusline env flaky + `internal/template` `TestOutputStylesTemplateLiveParity` (parallel-session residue, PRESERVE). 🗿 MoAI
- **[SPEC-TOKEN-VERIFY-DIET-001](.moai/specs/SPEC-TOKEN-VERIFY-DIET-001/spec.md)** — Verification output diet: file-redirect contract for the trust-but-verify batch and the `manager-develop`/`manager-docs` `§E` self-verification, cutting the context double-burn of large verification output while preserving `verification-claim-integrity` §1.1 surface 1 (orchestrator self-report) + surface 2 (manager `§E`) verbatim-evidence reachability (Tier M, Token-Economy Epic 3/4 (C), v0.1.0, doctrine-text-only). Added a `### File-redirect contract` subsection to `agent-common-protocol.md` § Parallel Execution stating the contract: when a verification command's verbatim output exceeds a bounded-tail ceiling (`≤50 lines OR ≤2KB`, whichever is smaller), redirect the verbatim output to a file on disk (`/tmp/moai-verify/<session>/...`) and surface only **exit code + bounded-tail summary** in conversation context; the cited file path appears in the Verification Matrix / Completion Report banner row. The contract explicitly rejects the "drop the evidence" interpretation — verbatim evidence lives on disk with a citable path; context carries exit code + bounded tail (inline quotation PERMITTED below the ceiling; the redirect obligation triggers only on exceedance). Cross-referenced the contract across **3 synchronized surfaces** (agent-common-protocol.md SSOT + verification-batch-pattern.md + moai.md §8 Verification Matrix `└─ evidence:` + Completion Report `📎 Evidence:` continuation fields), all live↔template byte-identical. **8/8 AC PASS** (6 MUST-PASS + 2 SHOULD-PASS: AC-VD-001..008 — contract existence, bounded-tail ceiling, vci §1.1 surface 1+2 named + "NOT drop the evidence", moai.md §8 banner file-path fields + 5 HARD rules intact, 7-keyword non-regression, parallel-execution HARD clause intact, 3-surface consistency, E1-E7 boundary unchanged). Plan-auditor PASS-WITH-DEBT 0.91 (Tier M threshold 0.80; iter-1; D1 REQ-VD prefix + D2 KI-1 relabel resolved in commit `3d35cc18d`; D3 BUDGET-STOP stub resolved pre-run via spec.md parenthetical per Implementation Kickoff Approval). Run-phase (manager-develop ddd, M1 `f1bfd557f` + M2 `12a13688f` per-milestone Route A main-direct; doctrine-only, no Go code; go build/golangci-lint/make build exit 0; l44 pre-push fetch 3 3 race → non-overlapping rebase → 0 0 post-push synced). Sync-phase **now** (orchestrator-direct — manager-docs CHANGELOG thrashing fallback per `feedback_glm_orchestrator_direct_sync_mx`; shared-checkout pathspec commit per `feedback_shared_checkout_concurrent_commit_race`; 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001: spec/plan/acceptance/progress.md frontmatter status `in-progress → completed` atomic on single sync commit + `updated: 2026-07-08` + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry + `sync_commit_sha` backfill via follow-up chore commit). README unchanged — doctrine-text-only, zero CLI changes. 🗿 MoAI
- **[SPEC-HANDOFF-GOALFIX-001](.moai/specs/SPEC-HANDOFF-GOALFIX-001/spec.md)** — Retire the inert in-block `# /goal` resume line; two-step post-paste `/goal` follow-up handoff (Tier M, doctrine-text-only, v0.1.1). The Block 1 `# /goal <completion-condition>` line from SPEC-V3R6-HANDOFF-GOAL-BINDING-001 was structurally inert — official slash-command parsing recognizes `/` only at input start, so a mid-paste `#`-prefixed slash line never fires. Defect verified against 3 ground-truth sources: (1) official interactive-mode docs (input-start-only), (2) official `/goal` semantics (user-only TUI command, model cannot invoke), (3) runtime 6.3MB session debug log (zero goal-evaluator activity when user pasted `# /goal ...`). Fix replaces the non-firing line with a **two-step handoff**: main resume block omits any `/goal` reference; when emission condition holds (unchanged: run-phase next SPEC + machine-verifiable end-state), a separate **post-paste follow-up block** is emitted OUTSIDE and AFTER the main cut-line-bounded block, containing exactly one line `/goal <completion-condition>` (no `#` prefix). The resumed session's orchestrator gains a **reminder obligation** to surface via natural-language guidance (NOT AskUserQuestion) that the user must send the `/goal` line as its own standalone message (slash commands parse only at input start) at the recommended moment (after Implementation Kickoff Approval), because the model cannot set a goal on the user's behalf. Added a **Paste-Time Activation Matrix** (4 classes: paste-time keywords YES; natural-language phrases YES; orchestrator-interpreted metadata YES via orchestrator; user-only TUI commands NO — requires standalone user message). Introduced **goal-first bootstrap variant** (documented alternative to the two-step default): users wanting one-paste autonomous continuation may send a standalone `/goal <resume pointer + compact condition>` at session start; caveats stated (effort keywords inside slash-command arguments NOT documented to fire; precondition verification shifts to model discretion; condition must stay compact; Implementation Kickoff Approval unaffected). **6 synchronized surfaces** (3 live + 3 template mirrors, all byte-identical): session-handoff.md (SSOT skeleton + Block 1 line-order invariant rewrite + Post-Paste /goal Follow-up Block section + Paste-Time Activation Matrix + reminder obligation + goal-first bootstrap subsection + Localization row + Example + Output Surface + Anti-Patterns rewrite + Pre-emit self-check reword); moai.md §8 (render surface, `/goal` annotation removed, follow-up block render note + Pre-emit checklist 12-item reword); goal-directive.md (resume-pairing bullet rewrite + reminder obligation bullet added); 3 template mirrors byte-identical. **13 AC groups PASS** (AC-GF-001a/b/c/d/e through AC-GF-013a/b/c): `# /goal` literal → 0 (baseline 6/6/1/1) / `re-set` → 0 (baseline 4/1/3/1/3/3) / "Block 1 `/goal` line" phrase → 0 (baseline 1) / Post-Paste Follow-up Block section = 1 new / Paste-Time Activation Matrix = 1 new + both official URLs / reminder obligation ≥1 on 2 surfaces / goal-first bootstrap tokens ≥1 each / live↔template byte-identical diffs exit 0 / `make build` exit 0 / template SPEC-ID neutrality 0 matches. Plan-auditor PASS-WITH-DEBT 0.87 (Tier M threshold 0.80; iter-1 remediated; PASS non-vacuous). Run-phase (manager-develop ddd, M1 commit a2af8a23a → isolated worktree rebase f2f6b33bf; 6 doctrine surfaces byte-identical; push state 0 0). Non-blocking F1 debt (sync-auditor 0.94): goal-first bootstrap "Implementation Kickoff Approval unaffected" coexists with "setting a goal starts a turn immediately" — coherent only under implicit assumption goal-first presumes run-phase resume already past Kickoff-Approval (future one-sentence clarification deferred, documented). Sync-phase **now** (manager-docs, single 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001: spec/plan/acceptance/progress.md frontmatter status `in-progress → completed` atomic on sync commit + `updated: 2026-07-08` + progress.md §E.4 audit-ready signal + `sync_commit_sha` backfill via follow-up chore commit). README unchanged — doctrine-text-only, zero CLI changes. 🗿 MoAI

- **[SPEC-HANDOFF-FANOUT-001](.moai/specs/SPEC-HANDOFF-FANOUT-001/spec.md)** — Mode 4 (parallel-subagents) paste-time fan-out steering phrase in resume-message Block 1 opener (Tier S, v0.1.1, doctrine-text-only). The directive-coupling table (session-handoff.md SSOT) had Mode 3→`--team` append, Mode 6→bare `ultracode` append, Mode 5→omit v1 byte-identical, but Mode 4 lacked explicit paste-time trigger — orchestrator relied solely on `mode:` metadata parsing (contra moai-constitution.md Principle 4: fan-out "must be instructed explicitly"). Closed gap with natural-language phrase `fan out subagents (<read-only investigation scope>)` appended to Block 1 opener line (user-authored multi-agent opt-in, same semantic class as `ultrathink`/`ultracode` keywords). **Implementation**: Block 1 line-order invariant preserved (`opener → mode: → # /goal`); IGGDA Path B binding (fan-out phrase does NOT override Implementation Kickoff Approval gate per SEED-not-permission doctrine); 3-5 ceiling and Mode 4 vs Mode 3 UI tip disambiguation added to session-handoff.md §B/§C prose. **4 synchronized surfaces** (2 SSOT/render live + 2 template mirrors): session-handoff.md (canonical SSOT with directive-coupling table row + Field-by-Field prose + anti-pattern entry AC-HFO-007) / session-handoff.md template mirror / moai.md §8 render surface (compact mapping table + skeleton / pre-emit checklist 11→12 items AC-HFO-006b) / moai.md template mirror (byte-identical both pairs per AC-HFO-009). **16/16 AC PASS** (AC-HFO-001a/b/c through AC-HFO-011): `fan out subagents` phrase grep 6 matches across 4 surfaces / canonical form parity `<read-only investigation scope>` / Implementation Kickoff Approval 4-count delta (baseline 3→+1 non-vacuous per AC-HFO-004a/SEED-not-permission binding) / 3-5 ceiling language (AC-HFO-004c delta baseline 1→+1) / Mode 4 UI tip (AC-HFO-005 sends-a-team guidance row) / locale-verbatim declaration (AC-HFO-011 delta 1→+2 across both fan-out coupling coverage) / pre-emit 10-items (AC-HFO-006a delta) / template-neutrality 0 SPEC-ID leaks / make build embed regeneration. Plan-auditor PASS-WITH-DEBT 0.88 (Tier S threshold 0.75; D1-D4 remediated v0.1.1 per progress.md §E.2 remediation table). Run-phase (manager-develop ddd, single M1 commit `354855da3` → rebase post-parallel-session race to `d2d1d88ba`, Hybrid Trunk main direct). Sync-phase **now** (manager-docs, 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged close: spec.md frontmatter status `in-progress → completed` atomic on single sync commit + updated:2026-07-08 + progress.md §E.4 audit-ready signal + CHANGELOG 本 entry + sync_commit_sha backfill via follow-up chore commit per established convention). Template-First byte-identical (both doctrine-text mirrors). README unchanged — doctrine text-only, zero user-facing CLI changes. 🗿 MoAI

### Removed

- **[SPEC-BRAND-DIR-REMOVE-001](.moai/specs/SPEC-BRAND-DIR-REMOVE-001/spec.md)** — Clean removal of dead `.moai/project/brand/` directory and live wiring (Tier M, v0.1.0). The brand scaffold (3 markdown files `brand-voice.md`, `target-audience.md`, `visual-identity.md`) was originally designed for the `/moai design` system, which is now retired. Removal eliminates a dead code path with zero consumers. **M1**: removed `.moai/project/brand/` prefix from `frozen_guard.go` frozen-guard list and `pre_tool.go` frozenZonePrefixes routing row; retained `SentinelHarnessFrozenConfig` constant (8-sentinel taxonomy, REQ-BDR-011). **M2**: removed `migrate_agency.go` Phase 2 (context → brand migration) + both stale `/moai design` references (summary + logf lines); `.agency/context/` files preserved in `.agency.archived/` backup (no data loss). **M3**: deleted 6 brand markdown files (template + local) + regenerated embedded FS (`make build`). **M4**: removed stale brand guard-mechanism examples from skill docs. **Preserved by design**: `dirs.go` DeprecatedPaths entry (moai update cleanup driver for existing users), SentinelHarnessFrozenConfig constant, adjacent design-system config (deferred to separate SPEC). All 11 AC PASS (AC-BDR-001a/b/c through AC-BDR-009): in-scope grep 0 matches (frozen_guard/pre_tool/migrate_agency), full-tree grep exactly 11 declared survivors, `moai init` scaffold-clean, cross-platform build exit 0. 🗿 MoAI

### Added

- **[SPEC-V3R6-AGENTIC-LOOP-CONFIG-001](.moai/specs/SPEC-V3R6-AGENTIC-LOOP-CONFIG-001/spec.md)** — `workflow.agentic_loop.max_iterations` Go-side typed loader registration (prose-read → 기계적 상한) (Tier S, v0.1.0). Parent `SPEC-MOAI-AGENTIC-LOOP-001` §E "Out of Scope — Go code changes"의 deferred follow-up — REQ-MAL-023 pipeline iteration ceiling key를 orchestrator prose-read에서 mechanical Go field로 전환. §A.4 critical-distinctness invariant: `agentic_loop.max_iterations`(pipeline-level, default 10) vs 기존 `loop_prevention.max_iterations`(per-operation, default 100)는 별개 YAML block + 별개 semantics + **별개 Go field** — merge/alias/fallback-chain 금지, dedicated anti-aliasing test로 기계적 강제. **4 production file**: `internal/config/types.go` (`AgenticLoopConfig` struct line 406 + `WorkflowConfig.AgenticLoop` field line 326), `internal/config/defaults.go` (`DefaultAgenticLoopMaxIterations = 10` single const line 41 + factory wiring), `internal/settings/schema_sections.go` (key registration line 193, distinct path token `"agentic_loop", "max_iterations"` vs loop_prevention line 195), loader.go NO-OP (whole-section unmarshal auto-wires struct tag). **1 test file**: `internal/config/agentic_loop_distinctness_test.go` (211 LOC, 6 test functions — table-driven AC-002/003a/003b + dedicated `TestAgenticLoopDistinctness_AntiAliasing`/`RuntimeMutation` for AC-DISTINCT-002 + `TestAgenticLoopDefault_NeverZero_NeverLoopPrevention` for AC-DISTINCT-001). 13/13 AC PASS (AC-001..011 + AC-DISTINCT-001/002; SHOULD AC-006/010 포함; OPTIONAL AC-011 template-parity CI는 spec.md §E 명시적 제외). **DoD blocker 통과**: AC-DISTINCT-001/002 (SPEC-LEVEL INVARIANT) 전부 PASS — two distinct Go fields, separate defaults (10 vs 100), no pointer aliasing (runtime mutation test `AgenticLoop.MaxIterations=999`가 `LoopPrevention.MaxIterations`에 전파되지 않음 verified). coverage `internal/config` 80.0% (-count=1 sync-phase 재실측, run-phase 80.3%와 근사 — non-decreasing AC-010 PASS), `go build ./internal/config/... ./internal/settings/...` exit 0, golangci-lint 0 NEW issues, go vet clean, cross-platform build N/A (config-struct addition, syscall/build-tag 변경 0). plan-auditor PASS. Run-phase (manager-develop TDD, 단일 M1+M2+M3 run-phase commit `8a6f807e0` → shared-checkout 병렬 세션 race 정리 후 rebase로 `8812e2bdb`, Tier S tight TDD cycle). Sync-phase **now** (orchestrator-direct — shared-checkout 병렬 세션 c891b435 dead 확정으로 pathspec commit; 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close: spec.md frontmatter status `in-progress → completed` atomic on single sync commit + `updated: 2026-07-08` + progress.md §E.4 audit-ready signal + CHANGELOG 본 entry. `sync_commit_sha`는 다음 chore commit에서 backfill 예정 — 확립된 convention per SPEC-BRAND-DIR-REMOVE-001 `2c637ece2` / SPEC-TOKEN-ROUTING-001 `e4465ec5d`). README 무변경 — 내부 config loader 추가, 사용자 가시적 CLI 변경 0. Template 무변경 (REQ-ALC-011 — template `workflow.yaml`이 이미 `agentic_loop:` block shipping, Go-only SPEC). 🗿 MoAI

- **[SPEC-TOKEN-ROUTING-001](.moai/specs/SPEC-TOKEN-ROUTING-001/spec.md)** — Tier×Phase 선언적 모델/effort 라우팅 매트릭스 + typed loader (`internal/config/model_routing.go` 신규 214 LOC) (Tier M, Token-Economy Epic 2/4 (B), v0.1.0). Token-Economy Epic A(`SPEC-TOKEN-ACCOUNTING-001`)가 측정 baseline을 영속화한 후속으로, "어떤 (Tier, Phase) 조합이 어떤 `{model, effort}`를 써야 하는가"를 declarative YAML + closed-set validated typed accessor로 인코딩 — orchestrator가 `Agent()` spawn 시 per-spawn override로 소비. **4 마일스톤**: M1 `.moai/config/sections/workflow.yaml` 최상위 `model_routing` 블록 (3 Tier × 4 Phase = 12 엔트리) + template mirror (`internal/template/templates/.moai/config/sections/workflow.yaml` 동일 블록, SPEC-ID 주석 0 — neutrality); M2 `internal/config/types.go` `ModelRoutingConfig` 필드 + `ModelRoutingEntry` struct (`{tier, phase, model, effort, fallback_applied}`) + `internal/config/model_routing.go` `RouteModelFor(tier, phase) ModelRoutingEntry` + `ValidateModelRouting` + `LoadModelRouting` (closed-set 검증 + fallback entry + `@MX:ANCHOR` godoc on `RouteModelFor`); M3 DD2 call-site 해결 — Go code에 `Agent(model:, effort:)` spawn-assembly call-site이 존재하지 않음(orchestrator는 LLM main thread, spawn은 prompt/skill layer)을 발견, 최소 patch는 typed accessor 자체 + `@MX:ANCHOR` godoc (plan.md §B DD2 (b)+(a) hybrid 정확 매칭); M4 `internal/config/model_routing_test.go` 11 tests (happy path / fallback / nil map / invalid input / closed-set{model,effort} / shared closed-set with workflow_agents / GLM allowed / unavailable model + round-trip). 12/12 AC PASS (AC-TR-001..012; SHOULD AC-TR-012 포함). coverage `internal/config` 80.0% (패키지 전체 — `model_routing.go` 파일 자체는 >85%; `types.go`/`workflow.go` 광범위 미접촉 표면으로 패키지 평균은 85% 목표 미달이나 본 SPEC 추가 코드는 한정됨). build host + `GOOS=windows GOARCH=amd64` exit 0, golangci-lint 0 issues (NEW = 0), go vet clean. plan-auditor PASS (Tier M 임계 0.80 충족). **§I Token Accounting dogfood** (Epic A 메커니즘 자체 적용): 5 subagents 관측 — plan-auditor Phase 0.5 + manager-spec (plan + D1/D2/D3 amendment 2회) + manager-develop (run) + manager-docs (sync) — 의 JSONL transcript `message.usage` 4필드 jq 합산. tokens_spent 26,851,010 / cache_hit_ratio 0.9204 / subagent-set / confidence `low` (orchestrator parent session + post-return 검증 토큰 제외 — snapshot은 strict under-count; verification-claim-integrity §2 준수, fabricated 숫자 0). Run-phase (manager-develop TDD, L1 격리 worktree → run commit `8e7352348` main 직진 Hybrid Trunk). Sync-phase **now** (manager-docs, 단일 sync commit으로 3-phase close — SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close per Status Transition Ownership Matrix; spec.md frontmatter status `in-progress → completed` atomic + `updated: 2026-07-08` + progress.md §E.4 audit-ready signal + §I 실측값 + CHANGELOG 본 entry. `sync_commit_sha`는 다음 chore commit에서 backfill 예정 — 확립된 convention per SPEC-BRAND-DIR-REMOVE-001 `2c637ece2`). README 무변경 — 내부 config/loader 추가, 사용자 가시적 CLI 변경 0. **Pre-existing 3 test failures** (`statusline TestBuild_WritesContextUsageWithSessionID` + `template TestOutputStylesTemplateLiveParity` + `TestTemplateNeutralityAuditC8Preserve`)는 본 SPEC 변경 이전 main HEAD에서 독립 재실행으로 PRE-EXISTING 입증 — 병렬 세션 작업 영역(statusline renderer / output-styles template parity) 소관이며 본 SPEC scope 밖. 🗿 MoAI

- **[SPEC-TOKEN-ACCOUNTING-001](.moai/specs/SPEC-TOKEN-ACCOUNTING-001/spec.md)** — per-SPEC 토큰 계정 runtime 측정 baseline (`internal/tokenusage` 신규 패키지) (Tier M, Token-Economy Epic 1/4, v0.1.1). 토큰-대-품질 KPI 계산을 위해 "한 SPEC이 누적 소비한 토큰"을 영속·감사 가능한 측정값으로 만드는 Gap A 인프라. **4 마일스톤**: M1 `SumSession`/`CacheHitRatio`/`Usage` — transcript `message.usage` 4필드 합산 (read-only, `~/.claude/projects/**`); M2 `Attribute(progressPath, transcriptRootDir, specsDir, activeSessionUUID)` — session-set 합산(REQ-TA-005) + lineage 부재 current-session 폴백(REQ-TA-006) + 공유-UUID 강등 high/low 신뢰도 한정자(REQ-TA-007); M3 `BuildSectionI`/`WriteSectionI` (멱등 replace-or-append, scope-safe — era.go §E.* namespace 무충돌, AC-TA-009 회귀) + Section Map SSOT `## §I Token Accounting` 행; M4 `AuditResult.TokensSpent *int` (JSON `tokens_spent,omitempty`) + `parseTokensSpentFromSectionI` round-trip 파서 (단일/`--filter-spec` 감사 시 노출, 다중 SPEC 감사는 nil — 정밀도-정직, REQ-TA-012 fabricate 금지). 12/12 AC PASS (AC-TA-001..012; SHOULD AC-TA-011 포함). coverage internal/tokenusage 90.1% / internal/spec 88.2%, build host+windows exit 0, golangci-lint 0, `-race` clean. plan-auditor PASS-WITH-DEBT 0.84 (Tier M 임계 0.80; D1+D2 해소). **§I 자기측정 dogfood**: 본 SPEC이 도입한 메커니즘을 자신에게 적용 — throwaway 헬퍼로 `Attribute()`+`WriteSectionI()` 실행하여 §I에 실측값 기록 (tokens_spent 30,304,046 / cache_hit_ratio 0.9229 / session-set / high / 2 sessions — 첫 real per-SPEC 토큰 측정 데이터포인트; residual: plan-phase 세션 UUID 미회수 + CLI write 경로 미연결). Run-phase (manager-develop TDD, L1 격리 worktree → shared-checkout ff/cherry-pick 회수 + 독립 재검증). Sync-phase (orchestrator-direct — GLM backend + shared-checkout 병렬 세션 race → path-limited commit; 3-phase close: spec.md status in-progress→completed + §E.3/§E.4 signals + §I 실측값 + sync_commit_sha backfill 예정). 🗿 MoAI

- **[SPEC-CLI-UIKIT-KERNEL-001](.moai/specs/SPEC-CLI-UIKIT-KERNEL-001/spec.md)** — internal/cli flat 패키지에서 공유 TUI 커널(render/banner/schema_bridge)을 신규 leaf 패키지 `internal/cli/uikit`으로 추출 (Tier L, v1.0.0). SPEC-CLI-SUBPKG-SPLIT-001 (M1-only close, agentlint cluster)의 첫 후속 SPEC — SPLIT-001 §F가 전망한 cluster ladder(migrate/doctor/update)의 axis-(i) blocker였던 "`package cli`의 커널 헬퍼들을 하위 서브패키지가 import할 수 없다"는 순환 의존 문제를 해소. **b-ii split 해법 (RegisterSchemaBridge 콜백 패턴)**: ANALYZE 단계에서 `schemaFieldBridge`/`schemaSegmentBridge` 두 맵이 `profileSetupText` 타입에 커플링되어 단순 이동이 불가함을 발견 → uikit은 `TuiLabel`/`SchemaKeyToTUIField`/`FieldDefTUILabel` 헬퍼만 보유하고 `RegisterSchemaBridge` 콜백 등록으로 dispatch, profileSetupText-인식 맵들은 package cli에 잔류(retyped to `uikit.TuiLabel`) — 표준 Go DI 패턴(worktree.WorktreeProvider 선례), uikit의 profileSetupText 코드 참조 0(5개 comment-only mentions은 b-ii split 문서화 전용). **PRESERVE-IMPROVE (ddd)**: `moai --help` byte-identical(AC-CUK-003), `go test ./...` 동일 4 pre-existing 실패만(본 SPEC 무관), `go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` 모두 exit 0, lint NEW 0건, uikit 패키지 커버리지 98.9%. **uikit 패키지 신규**: 6 source(types.go/styles.go/status.go/render.go/banner.go/schema_bridge.go — CheckStatus/CheckOK/CheckWarn/CheckFail + 6 style vars + SymSuccess/Error/Warning + StatusIcon + Render*Card/RenderStatusLine/PrintBanner/PrintWelcomeMessage/ResolveTheme/GoVersion/ClaudeVersion/GitVersionOverride/GhVersionOverride/GoosArch + RegisterSchemaBridge 콜백) + 4 test(render_test.go/banner_test.go/welcome_test.go + NEW characterization_test.go — resolver dispatch/StatusIcon branches/version helpers/Sym* helpers DDD PRESERVE-phase). **cli 패키지 변경**: render.go/banner.go/render_test.go/banner_test.go 삭제(uikit으로 이동) + theme.go 신규(local resolveTheme wrapper, 13 callers 무변경 보존) + schema_bridge.go 재작성(맵 `uikit.TuiLabel`로 retype, init()에서 resolver 등록) + 14 production 파일(doctor/doctor_cache/doctor_harness/update/migrate_agency/hook/inventory/research/init/init_layout/root/glm/misc_coverage)의 moved-helper call sites → `uikit.*` 재작성 + 13 test 파일 CheckStatus refs → `uikit.CheckStatus` 재작성. **total 39 파일**(4 deleted + 2 new + 33 modified). **7 기존 서브패키지 무변경**(worktree/harness/preference/wizard/specid/pr/agentlint). 12/13 AC PASS(AC-CUK-001..012); 1 AC deferred(AC-CUK-013 checkpoint — migrate/doctor/update cluster는 별도 후속 SPEC이며 본 uikit leaf에 gate됨). Plan-auditor iter-2 PASS 0.91(Tier L 임계 0.85; skip-eligible ≥0.90, 24h hash-unchanged, Phase 0.5 NOT 재실행). Run-phase(manager-develop ddd, 단일 atomic M1 commit `5326b7632` — previous session에서 origin/main에 push 완료, 본 sync-phase 이전). Sync-phase **now**(manager-docs, 단일 sync commit으로 3-phase close — SPEC-V3R6-LIFECYCLE-REDESIGN-001 REQ-LR-008/009에 따른 merged 3-phase close; spec.md frontmatter status `in-progress → completed` + `updated: 2026-07-07 → 2026-07-08`, progress.md §E.4 audit-ready signal block, CHANGELOG `[Unreleased] > Added` entry. README 무변경 — Tier L 내부 refactor, 사용자 가시적 변경 0). 🗿 MoAI

- **[SPEC-MOAI-AGENTIC-LOOP-001](.moai/specs/SPEC-MOAI-AGENTIC-LOOP-001/spec.md)** — `/moai` agentic completion loop: orchestrator router 요구사항 분석 → 6-모드 카탈로그 배선 → Implementation Kickoff Approval 게이트 보존 → 자율 run→sync→verify 파이프라인 (Tier L, v0.1.1). 사용자 지시 ("request arrives → plan workflow → drive agentic loop until completed, sequential/parallel/team/dynamic-workflow orchestrate")를 3핵심 기둥으로 실행: (1) **Router-level requirement analysis** (SKILL.md Execution Directive에 진입 전 요구사항 요약 + 완료조건 + 파이프라인 계약 + orchestration-shape 신호 생성), (2) **6-mode Phase 0.95 catalog wiring** (orchestration-mode-selection.md §A 단일 SSOT; `--mode` 축 × scale table 명시적 crosswalk; 임계값 1회 정의 + 상호참조), (3) **Autonomous completion loop** (plan→run 후 Implementation Kickoff Approval 정확히 1회 통과 후 run→sync→verify 자율 반복, max_iterations=10, no-progress 감지, 의미론적 실패 즉시 escalation, context-threshold handoff 통합 — plan 재진입은 사용자 승인 REQ-MAL-022/024 경로만). M1-M5 milestone commits (5022bdf63 ~ 8578c2a47): SSOT 임계값 문장 + 6-모드 crosswalk + 라우터 요구사항 분석 Step 2.8 + 파이프라인 게이트 명시 + agentic_loop 반복 정의 + run→sync auto-chain 정책 + 인접 스킬 표면 (loop 관계 문단 + sync 체인 진입). M6 검증: 38/38 AC PASS (AC-MAL-001..026 레터 접미 포함, AC-MAL-R01..R04); REQ-MAL-020 Implementation Kickoff Approval 정확히 1회 + REQ-MAL-022 run→sync→verify only + REQ-MAL-028 subagent boundary 강제 + REQ-MAL-040/041 template 중립성 모두 PASS. Pre-flight M1 characterization: orchestration-mode-selection.md pre-existing MIRRORED-DIVERGED 상태(§I.3 maintainer-path detail 의도적 변형, template 중립성 goal 위반 X) + `.moai/` statusline 런타임 누수 junk 삭제. 모든 skill 편집 20파일 byte-identical mirror(git-tracked markdown parity 42/42 + untracked 제외). Zero Go-code 변경(diff --stat -- '*.go' empty per AC-MAL-025). Plan-auditor iter-2 PASS-WITH-DEBT 0.89(Tier L 임계 0.85; iter-1 FAIL 0.83 → D1-D10 해소, D11/D1r debt 해소 완료). Run-phase(manager-develop ddd, 5 milestone + M6 backfill commit a8f6bc629 + 29060364d backfill). Sync-phase(orchestrator-direct, 3-phase close: spec/plan/acceptance/progress.md 모두 status in-progress→completed + era V3R6 + updated:2026-07-08 + progress §E.4 sync_commit_sha). 🗿 MoAI

- **[SPEC-HOOK-CONFIG-SAFETY-001](.moai/specs/SPEC-HOOK-CONFIG-SAFETY-001/spec.md)** — `moai update` Template sync complete 분기에 `/hooks` 리뷰 안내 추가 (Tier S, v0.1.0). Claude Code가 세션 시작 시점의 hooks 스냅샷을 사용하고 외부 수정 시 자동 경고하지만 (CC 공식 "Configuration Safety" 정책), `moai update`가 템플릿 재렌더링으로 `.claude/settings.json` hooks 섹션을 갱신해도 완료 메시지에는 `/hooks` 메뉴 리뷰 유도가 전혀 없어 — 사용자가 CC의 generic 경고로부터 `moai update`가 원인이라는 사실을 역추적해야 하는 UX/discoverability gap 해소. **Option B**(Template-sync-complete 분기에서 무조건 emit) 채택 — Option A(JSON-diff 조건부 emit)는 Tier S 단순성에 과잉, Option C(init 포함)는 빈도 낮아 제외(per spec.md §E.2 simplicity-ladder). 구현: `internal/cli/update.go` `emitHooksReviewGuidance(w io.Writer)` 헬퍼(신규, line 365) + 단일 call site(`Label: "Template sync complete"` pill 직후, content-token anchored per `feedback_line_number_drift_asymmetry`). 3 no-op 분기(`Already up to date` / `Template version up-to-date · Skipping sync` / `Binary updated (template sync skipped)`)는 동작 분기가 다르므로 무조건 발화 안 함(AC-CS-002). 7/7 AC PASS(AC-CS-001..007). 신규 테스트 `internal/cli/update_hooks_guidance_test.go` 5개 (characterization 4 + static-guard 1): TemplateSyncComplete-EmitsHooksGuidance / AlreadyUpToDate/SkippingSync/BinaryOnly-NoHooksGuidance (3 no-op) / NoAskUserQuestion (C-HRA-008 / REQ-PGN-012 subagent boundary static guard, `TestNew_NoAskUserQuestion` 패턴 mirror). emit 헬퍼 `emitHooksReviewGuidance` 100% 커버리지 측정(focused run). AC-CS-006 PASS-WITH-DEBT: `go test ./...` 3 pre-existing 실패 (`TestRunHookEvent_ReadInputError` nil-pointer panic / `TestCloseSubjectDoctrineAmendment` lifecycle-sync-gate.md drift / `TestBuild_WritesContextUsageWithSessionID` 1M-vs-256K assertion) 전부 stash-and-retest로 본 SPEC 무관 입증. Verify-before-author 파이프라인 준수 (`feedback_hypothesis_as_defect`): Greps 1-3 (spec.md §A.2) 독립 재실행 0-hit로 "guidance absent" 결함 가설 확인 — P2/P3/P4(stale-memory non-defects)와 반대로 P5는 real gap. P5 of moai-adk hooks-hardening Epic (P1 = SPEC-HOOK-SESSIONSTART-PROBE-001 completed; P2/P3/P4 absorbed) — Epic close. Run-phase (manager-develop ddd, L1 격리 worktree → run commit `e87bc205c` + run_commit_sha backfill `f7107f76b`). Sync-phase (orchestrator-direct 격리 worktree — 다중 세션 race로 local main checkout 불안정; 3-phase close: 4-artifact frontmatter status in-progress→completed atomic on single sync commit per Status Transition Ownership Matrix + progress §E.4 sync_commit_sha 별도 chore commit에서 backfill 예정). 🗿 MoAI

- **[SPEC-CLI-LINT-COMPLETED-001](.moai/specs/SPEC-CLI-LINT-COMPLETED-001/spec.md)** — `internal/spec/lint.go` `terminalStatusEnum`에 `completed` 추가 (Tier S chore, v1.0.0). V3R6 3-phase close 최종 상태인 `completed`가 누락되어 `StatusGitConsistencyRule`이 completed SPEC에 drift false-positive를 뱉던 버그 수정 — 단일 라인 추가(`"completed": true,`)로 L984 early-return gate가 `completed`를 단락시켜 drift 분기(L996) 도달 전 복귀. `status_terminal_exemption_test.go`에 Pattern H(`completed`)/Pattern I(`rejected`) characterization test 2개 추가(기존 Pattern D/E/F/G와 동일한 `setupGitFixture` + `os.Chdir` 패턴 — D2 vacuous-RED 회피 목적의 git fixture 필수 구성). 3/3 AC PASS(AC-LC-001/002/003). TDD RED→GREEN 증명: `completed` 추가 전 Pattern H 테스트가 `StatusGitConsistency` finding 1개로 FAIL → 추가 후 0 finding으로 PASS(RED 비공헌 — `setupGitFixture` 전제 만족으로 `getGitImpliedStatus` graceful-skip 미발행 확인). D2-extra 발견: fixture ID는 `SPEC-TERMH-001`/`SPEC-TERMI-001` 형식 사용(`ExtractSPECIDs` regex가 pure-digit 종단 세그먼트 요구 — `SPEC-TERMINAL-H001` 불가; 기존 Pattern D/E/F/G도 동일 관행). Run-phase commit `1169d2afb`(main 직진, Route A Hybrid Trunk). Sync-phase(orchestrator-direct — Tier S consolidated lifecycle, draft → completed 단일 sync commit). Full-package `go test ./...` 1 FAIL(`TestCloseSubjectDoctrineAmendment/lifecycle-sync-gate.md` AC-DLC-011)은 본 SPEC 무관 — 병렬 세션 commit `ccc036872`가 `.claude/rules/moai/workflow/lifecycle-sync-gate.md`를 수정한 것에 기인(hotfix 대상 아님, parallel-session 소관). 🗿 MoAI
...
- **[SPEC-HOOK-SESSIONSTART-PROBE-001](.moai/specs/SPEC-HOOK-SESSIONSTART-PROBE-001/spec.md)** — SessionStart probe 추가: moai-binary 3-tier 동시 미해결 시 비차단 경고로 silent degradation을 surfaced signal로 전환 (Tier S, 3-phase close, v0.1.0). hook-independence.md §6 Recommendation 구현 — 31개 wrapper 스크립트의 IDENTICAL 3-tier 체인(PATH → `$HOME/go/bin` → `$HOME/.local/bin`)이 모두 실패하면 31개 전부가 동시에 silent `exit 0`로 退化하는 correlated-degradation를 SessionStart wrapper fallback 분기에서 source-gated dual-channel 경고로 surface. **Branch A**(wrapper-only, self-contained bash) 선택: Branch B(Go subcommand)는 structural paradox(fallback 도달 시 moai 부재이므로 subcommand 호출도 실패), Branch C(separate probe script)는 needless moving parts(Tier S 과잉)로 각각 기각. 7/7 AC PASS(AC-HOOK-001..007). REQ-HOOK-001 success-path byte-identical(tier chain 무접촉, fallback 분기만 수정) · REQ-HOOK-002 surfaced signal(stdout JSON `hookSpecificOutput.additionalContext` + stderr `$HOME/.moai/logs/hook-stderr.log` dual-channel) · REQ-HOOK-003 non-blocking `exit 0` 보존 · REQ-HOOK-004 source-gated dedup(`source=startup`일 때만 emit, resume/clear/compact suppress — stateless, marker file 없음) · REQ-HOOK-005 30개 비-SessionStart wrapper byte-identical · REQ-HOOK-006 Template-First mirror parity(local `.sh` + template `.sh.tmpl` 동일 commit, byte-identical) · REQ-HOOK-007 warning actionability(3 tier 열거 + 31 wrappers consequence + non-blocking framing + reinstall/restore PATH/make build remediation). 파일: `.claude/hooks/moai/handle-session-start.sh`(fallback 분기 +19 lines) + `internal/template/templates/.claude/hooks/moai/handle-session-start.sh.tmpl`(동일 내용 mirror) + `internal/hook/sessionstart_probe_test.go`(NEW 311 LOC, AC-002/003/004/006/007 + §D.7 edge cases empty/malformed/non-JSON stdin). plan-auditor iter-2 PASS 0.92(Tier S 임계 0.75; iter-1 PASS-WITH-DEBT 0.84 → 6 defects 해소: D1 surface over-claim, D2 resume-coverage gap 문서화, D3 tier:S frontmatter, D4 grep -F fixed-string, D5 REQ-HOOK-002 GEARS compound, D6 channel-role separation). Run-phase(manager-develop TDD, single M1 commit `f34f19550`, 4 files). Sync-phase(orchestrator-direct — manager-docs, 3-phase close: status draft→completed + progress §E.4 sync_commit_sha). 🗿 MoAI

### Changed

- **[SPEC-HOOK-FACTFORCE-ADVISORY-001](.moai/specs/SPEC-HOOK-FACTFORCE-ADVISORY-001/spec.md)** — `gateguard-fact-force.sh` PreToolUse hook을 exit-2 blocking → exit-0 advisory로 재작성 (Tier S, v0.1.0). 선행 SPEC-HOOK-PREEDIT-INVESTIGATE-001 supersede — 첫 Edit/Write/MultiEdit per (session, file path)가 더 이상 작업을 block하지 않고 one-time advisory `systemMessage` JSON을 stdout에 emit 후 `exit 0`(허용)으로 전환. state-file logic(session_id + 절대경로 SHA-1 keying, 0o600 single JSON line), `MOAI_FACT_FORCE=off` opt-out, Read-as-investigation(Read 후 첫 Edit는 advisory skip), self-loop prevention(non-Edit/Write/MultiEdit/Read payload → exit 0), fail-open 전부 unchanged 보존. settings.json 등록은 선행 SPEC이 이미 배선 → 무접촉. **jq-free NFR**(§C.5): `awk` escape only로 single-line JSON systemMessage 생성 — `jq -e '.systemMessage | type'` == `"string"` 검증. **hook-independence.md Mode G row**: rationale `"no gate"` → `"no advisory notice"` (template + local byte-identical). **4 file**: template hook `internal/template/templates/.claude/hooks/moai/gateguard-fact-force.sh` + local mirror `.claude/hooks/moai/gateguard-fact-force.sh` (byte-identical) + `hook-independence.md` Mode G row (template `internal/template/templates/.claude/rules/moai/development/hook-independence.md` + local mirror). `make build`로 embedded FS 재컴파일 (catalog hash shift only). 15/15 AC PASS (AC-FA-001..015; 13 MUST + 2 SHOULD; SHOULD AC-FA-006 <100ms soft-target 미달, 5s hard timeout met으로 PASS-WITH-DEBT — process-spawn overhead inherited from predecessor shell architecture). orchestrator 독립 검증 7/7 PASS — V1 exit-2 제거(`grep 'exit 2'` rc=1 양쪽) · V2 template==local byte-identical parity · V3 jq-free(comment only) · V4 systemMessage + 4 substrings(First-edit/IMPORTERS/DATA SCHEMAS/USER INSTRUCTION) · V5 hook-independence Mode G row · V6 run commit == origin/main · V7 spec lint clean. shell-only (Go 무변경, `git diff -- '*.go'` empty). plan-auditor PASS 0.99 (skip-eligible; MP-1..4 PASS). Run-phase (manager-develop TDD, L1 격리 worktree cherry-pick → main SHA `3455b0ee8` origin/main 확정). Sync-phase **now** (orchestrator-direct — manager-docs 위임 prompt-too-long 실패 + shared-checkout 다중 세션 race(INTERNAL-SECURITY-001 sync f3193bac8~c0ac267b5 + TOKEN-VERIFY-DIET-001 plan e62d52e2b 적재)로 pathspec commit fallback; 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close: spec.md frontmatter status `in-progress → completed` atomic on single sync commit + `updated: 2026-07-08` + progress.md §E.4 audit-ready signal + §E.3 `run_commit_sha` backfill `3455b0ee8` + CHANGELOG 본 entry. plan/acceptance/progress.md는 YAML frontmatter 미보유(`# Title` 직시작) → status field 없음, spec.md가 canonical frontmatter 소유자. `sync_commit_sha`는 다음 chore commit에서 backfill 예정 — 확립된 convention per SPEC-BRAND-DIR-REMOVE-001 `2c637ece2` / SPEC-V3R6-AGENTIC-LOOP-CONFIG-001). README 무변경 — hook 내부 동작 변경, 사용자 가시적 CLI 변경 0 (다만 edit-per-session UX 개선: hard block → soft advisory). 🗿 MoAI

- **[SPEC-CLAUDEMD-DIET-V2-001](.moai/specs/SPEC-CLAUDEMD-DIET-V2-001/spec.md)** — CLAUDE.md 2nd-round diet 405→319L (−86L): always-loaded 컨텍스트 예산 추가 절감 (Tier M, v0.1.1). Track 2 of official Claude Code docs audit 2026-07. 1st-round(SPEC-STEERING-ALIGN-CLAUDEMD-DIET-001, 650→409L) 후속으로 동일 파일 재압축 — §1/§2/§3/§4 canonical content 보존(HARD) 전제하에 6개 후보 섹션 재검토. **Option A 채택**(§16 Context Search Protocol in-place compress 50→13L, −37L): §16은 탐색 절차(search procedure)라 path-scoping 시 일반 코딩 세션에서 접근성 상실 → always-available 유지하면서 압축; 신규 룰 파일 생성(Option B) 불필요. **§5/§15 partial-KEEP**(AC-CMD2-009 preconditions로 distinctive-content 0-hit 확인: §15 CG Mode ASCII/`moai cg`는 SSOT에 부재해 genuinely distinctive → KEEP; §5 Agent Chain은 spec-workflow.md에 다른 표현으로 존재 → 일부 pointer-ization). **§8/§11/§14 pointer-ization**(askuser-protocol.md / agent-common-protocol.md § Error Recovery·Parallel Execution로 1-line pointer). **footer/§13 micro-trim**(2nd paragraph blockquote 병합 −2L, footer 3 meta lines → 1 pipe-separated line −3L)으로 M3 324L(4L over ceiling) → 319L 재압축 달성. **PRESERVE 불변**(`[HARD]`=14·`[ZONE:]`=14·`@<path>` embed=2·`manager-` rows=4·decision-tree=11·§1/§2/§3/§4=29/36/12/50 — 전종 baseline 보존, byte-parity DIFF_EXIT=0). Template-First 동기화(`CLAUDE.md` ↔ `internal/template/templates/CLAUDE.md` byte-identical 319L). 10/10 AC PASS(AC-CMD2-001..010; SHOULD AC-001 ≤210L은 3rd-round deferred per §D Out-of-Scope). plan-auditor iter-2 PASS-WITH-DEBT 0.904(Tier M 임계 0.80; skip-eligible ≥0.90, 24h hash-unchanged — Phase 0.5 NOT 재실행; D1 BLOCKING @import→`@<path>` regex fix + D2-D6 SHOULD/MINOR 해소). Run-phase(manager-develop ddd, 4 milestone commits `210ca1818`/`ab92ccaa0`/`614c71618`/`83b9c81e9` — L1 격리 worktree → shared-checkout으로 회수 후 origin/main 직진, Hybrid Trunk). Sync-phase **now**(manager-docs, 단일 sync commit으로 3-phase close — SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close; spec.md frontmatter status `in-progress → completed` + progress.md §E.4 audit-ready signal + sync_commit_sha 2-commit backfill convention. README 무변경 — CLAUDE.md line-count reference 부재). Non-blocking debts: AC-CMD2-007 `TestTemplateNeutralityAuditC8Preserve` pre-existing FAIL(plan-phase commit `365feae6b`가 scripts/ci-mirror 제거 → C8 3→2, 본 SPEC 무관 — directly-validating `TestTemplateNoInternalContentLeak`/`TestSplitHarnessNamespaceNoLeak` PASS); SHOULD ≤210L 미달성(319L, 3rd-round deferral). 🗿 MoAI

### Fixed

- **[SPEC-INTERNAL-SECURITY-001](.moai/specs/SPEC-INTERNAL-SECURITY-001/spec.md)** — `internal/` 3-subsystem 보안 하드닝: read-only 감사 기원 8건 verified finding을 기계적 AC로 봉쇄 (Tier M, v0.1.0). web / template-update / hook 3개 논리 그룹으로 묶어 P0 2건 + P1 5건 + P2 1건 봉쇄. **M1 web** (commit `5f33dfaa9`, REQ-SEC-001 P0 + REQ-SEC-002 P1): `handleIndex` GET 읽기 경로에 `profile.IsValidProfileName` 4xx 가드 추가 (`handleSave`와 대칭 복원) + `GetPreferencesPath`/`GetProfileDir` 중앙 traversal 가드 이식 (base dir 밖 경로 생성 원천 차단) + 상태 변경 route `Sec-Fetch-Site: same-origin` 강제 (cross-site / same-site / none / absent → 403, POST/PUT/PATCH) + `hostCheckMiddleware` 거짓 "CSRF 보호" 주석을 "2-layer 쓰기-안전 모델 (Host=DNS-rebinding, Sec-Fetch-Site=CSRF)" 정정. **M2 template-update** (commit `85e280599`, REQ-SEC-003..006): `collectUserOwnedFilesWith` walk 콜백에 `isSymlinkEntry(path)` Lstat 가드 추가 (심링크 역참조 target 내용의 `.moai/backups/` 유출 차단, `update.go:2179` 기존 패턴 재사용 — 새 패턴 발명 금지) + `internal/template/templates/.gitignore` `.moai/backups/` SLASH form 추가 (Template-First, `make build` 임베드) + `isUserOwnedNamespaceConservative` 신규 (reserved-prefix 모호 이름 `moai-my-notes` / `expert-mydomain.md` 보수적 백업 확대, clean-reinstall 빌드 보존) + `verifyNamespaceBackupCoverage` 신규 (파괴적 deploy 이전 real pre-modification abort gate — dead sentinel 배선, 사용자 결정 분기 a) + `update.go:1283` `@MX:REASON` fan_in=3 거짓 주장 정정 (AC-SEC-006c). **M3 hook** (commit `24482adf2`, REQ-SEC-007 P1 + REQ-SEC-008 P2): `checkFileAccess`에 `filepath.EvalSymlinks` resolve-recheck 전파 (CWE-61 심링크 경유 secret 덮어쓰기 차단, `file_changed.go:176-203` SPEC-SEC-HARDEN-004 패턴 전파 — new-file Write not-exist fallback 보존, AC-SEC-007c) + 민감 콘텐츠 스캔 `if toolName == "Write"` 게이트를 `Write || Edit` 확장 (Edit `new_string` 경유 secret 주입 deny 우회 봉쇄). 19/19 applicable AC PASS (20 SSOT rows AC-SEC-001a..008b; M1 6/6 AC-SEC-001..002 + M2 8/8 AC-SEC-003..006 + M3 5/5 AC-SEC-007..008; AC-SEC-006b N/A per WIRE-branch 분기 a 배선 decision → 19 applicable). coverage `internal/hook` 83.6%, cross-platform build darwin+windows exit 0, golangci-lint 0 issues. plan-auditor PASS 0.941 (iter-1, Tier M threshold 0.80). Run-phase (manager-develop, Hybrid Trunk main-direct, 3 milestone commits `5f33dfaa9` / `85e280599` / `24482adf2`). Sync-phase **now** (manager-docs, 단일 sync commit으로 3-phase close — SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close; 4-artifact frontmatter status `draft`/`in-progress → implemented → completed` atomic + `updated: 2026-07-08` + progress.md §E.4 audit-ready signal + §E.5 Mx-phase signal + CHANGELOG 본 entry. `sync_commit_sha` / `mx_commit_sha`는 `moai spec close`에서 backfill). Pre-existing baseline failures (out of SPEC scope): `internal/statusline` `TestBuild_WritesContextUsageWithSessionID` + `internal/template` `TestOutputStylesTemplateLiveParity` + `TestTemplateNeutralityAuditC8Preserve` — confirmed FAIL at origin/main baseline, independent of SECURITY SPEC (statusline/template do not reference `checkFileAccess` / `handlers.go`; `grep -rl` = no references). 🗯 MoAI

- **[SPEC-INTERNAL-TEST-001](.moai/specs/SPEC-INTERNAL-TEST-001/spec.md)** — internal/ 테스트·CI 베이스라인 복구: `go test ./internal/...` SIGSEGV 크래시 제거 + 커버리지 측정 불능 해소 + doc-content 게이트 2건 정합 (Tier S, v0.1.0). F1-F5 발견사항 전부 remediated (11/12 AC PASS, headline NOT-MET — 사용자 승인 PASS-WITH-DEBT). **F1 SIGSEGV 제거** (`internal/cli/coverage_test.go:58-85` `TestRunHookEvent_ReadInputError`): 구 fail-closed 단언(`err != nil`)을 현행 fail-open 계약(`err == nil` + default `HookOutput`)으로 갱신 → `internal/cli` 테스트 바이너리 nil-deref SIGSEGV 종식, 패키지 커버리지 측정 unblock (이전 측정불능 → 이제 71.6% 측정가능). **F2 agentlint doc 드리프트 정합** (`internal/cli/agentlint/agent_lint_test.go` `TestAuthoringDocHasEffortMatrix`): `expectedAgents` stale 17-agent 목록 → 현행 retained 8-agent 카탈로그로 교체 (archived 명칭 0건 — `manager-strategy`/`manager-quality`/`manager-project`/`expert-*` 6종/`researcher`/`manager-cycle`/`builder-platform` 전부 제거, AC-TEST-003b grep 0 매치); `.claude/rules/moai/development/agent-authoring.md` effort matrix 정합 (template-first mirror `internal/template/templates/.claude/rules/moai/development/agent-authoring.md` 동기화, AC-TEST-003c diff 0). **F3 AC-DLC-011 doc-content 게이트 충족** (`.claude/rules/moai/workflow/lifecycle-sync-gate.md`): close-subject full-ID mandate amendment 삽입 — same-line `combined/abbreviated + scope` 구문이 prohibition token(`MUST use individual full-ID`)과 ≤400자 co-location (oracle regex `drift_doctrine_test.go:47-48` 만족); dev-only 파일이라 template mirror 생성 안 함 (C-2 준수, AC-TEST-004b). **F4 migration 패키지 테스트 실체화** (`internal/migration/*_test.go`): 17개 `t.Skip("waiting for migration package implementation")` placeholder → 실 테스트 (crash-recovery `detectInFlightState`/`cleanupInFlightState` / idempotency / atomic version-file update 검증, 전부 `t.TempDir()` 격리), 패키지 커버리지 1.1% → 85.5%. **F5 constitution 테스트 추가** (`internal/constitution/{canary,contradiction}_test.go` 신규): F5 scope canary.go 87% / contradiction.go 95% 커버리지 달성, 패키지 41.3% → 67.5%. **production 코드 무접촉** (C-1/C-3 검증: `internal/cli/hook.go` + `internal/migration/*.go` + `internal/constitution/{canary,contradiction}.go` 전부 무변경 — tests-only; `git diff <run-base>..HEAD -- '*.go' | grep -v '_test.go'` empty). Cross-platform build darwin+windows exit 0, golangci-lint 0 issues, `-race` clean. **PASS-WITH-DEBT (사용자 승인 — 투명 문서화)**: AC-TEST-001 headline `go test ./internal/...` FAIL 잔존 — 0 panic이나 7 pre-existing FAIL (6× internal/cli `TestDoctor`/`TestStatus` TUI rendering `Current_Light`/`Dark`/`NoColor` — F1 SIGSEGV 수정 전까지 마스킹됨; + 1× internal/statusline `TestBuild_WritesContextUsageWithSessionID` env-flake `context_window_size=1000000, want 256000` — spec.md §Out of Scope 명시적 제외). AC-TEST-006b constitution 패키지 커버리지 67.5% < 85% floor (F5 scope 외 `pipeline.go` 8함수 0% integration-level 사전 부채). acceptance.md §D.1 "Binding gate (MUST-PASS): AC-TEST-001 headline" body는 무수정(manager-docs 금지 영역) — 본 재해석("F1-F5 per-finding gates PASS + residual debt 기록 + headline green 후속 SPEC으로 연기")은 사용자 승인 PASS-WITH-DEBT로 CHANGELOG 본 entry + progress.md §E.4 residual_debt block에 투명 문서화 (silent pretend-headline-is-green 아님). **후속 SPEC-INTERNAL-TEST-002가 잔여 3종 부채 소유 예정** (6 TUI 렌더링 실패 + statusline env-flake + constitution pipeline.go 커버리지 67.5%→85%) — 본 sync에서 후속 SPEC 디렉토리 미생성, 부채 소유자로만 명명. plan-auditor PASS-WITH-DEBT 0.91 (Tier S 임계 0.75; F1-F5 전부 CONFIRMED at HEAD, skip-eligible ≥0.90). Run-phase (manager-develop TDD, 7 milestone 커밋 M1-M5+M5-lint+evidence `72ca1ed77..aef10fcd5` main 직진 Hybrid Trunk + orchestrator §F/§G 로그 `b90178437`). Sync-phase (manager-docs, 단일 sync commit 3-phase close — SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close; spec/plan/acceptance/progress frontmatter status `in-progress → completed` atomic + `updated: 2026-07-08` + progress §E.4 sync_commit_sha + CHANGELOG `[Unreleased] > ### Fixed` 본 entry). 🗿 MoAI

- **[SPEC-INTERNAL-TEST-002](.moai/specs/SPEC-INTERNAL-TEST-002/spec.md)** — internal/ 사전 부채 3종 청소: SPEC-INTERNAL-TEST-001 PASS-WITH-DEBT residual 3건(DEBT-TEST-001/002/003) 정식 debt-owner SPEC으로 청산 (Tier M, v0.1.0). 저작 시점 HEAD 베이스라인에서 전용 도구(`go test -v`, `go test -coverprofile`, `git show HEAD:<path>`)로 3 부채 root cause 재검증 (verification-claim-integrity §1.1 surface 3 준수) — 2개 주요 정정발견: (a) DEBT-TEST-002는 commit `794bb4f84`(2026-07-09)로 이미 HEAD에서 해결됨(TEST-001 §E.4 기술 시점에는 deterministic FAIL이었으나 현재 PASS로 전환 확인), (b) DEBT-TEST-001은 stale-golden 확정(renderer 정상, 단일 문자 차이). **M1** (commit `ffea91710`, REQ-TEST-007 P0, ARCH-001 M0 prerequisite): `UPDATE_GOLDEN=1` 메커니즘으로 6개 golden testdata(`internal/cli/testdata/{doctor,status}-{light,dark,nocolor}.golden`) 재생성 — stale `v3.0.0-rc6` → 현재 `v3.0.0-rc7`(worktree HEAD `f3f348ff5` version.go 실측값; main 작업트리 uncommitted `rc8`는 release-process-owned PRESERVE — anti-hardcode directive per progress.md §E.2). byte-level diff = rc6→rc7 단일 문자만(layout/이모지/구조 변경 0건, C-1 renderer 무접촉 확인). 6 TestDoctor/TestStatus 테스트 동시 PASS. **M2** (REQ-TEST-008 P2, verify-only — 코드 변경 0건): `TestBuild_WritesContextUsageWithSessionID -count=10` 연속 PASS + GLM env 주입 상태 3 consecutive PASS — commit `794bb4f84` already-resolved evidence 재생성만(C-2 준수, `internal/statusline/` production 파일 무수정). **M3** (commit `65c23fbf2`, REQ-TEST-009 P1): `internal/constitution/{pipeline_test.go,human_oversight_test.go}` 신규 integration test 추가(451 LOC + 114 LOC) — pipeline.go 8함수(NewPipeline/Execute/createLogEntry/applyAmendment/acquireLock/releaseLock/updateSourceFile/updateRegistryClause) + human_oversight.go 2함수(Approve/printDiff) 전부 0% 탈출, package coverage 67.5% → **85.1%**(≥85% floor, CLAUDE.local.md §6). 전부 `t.TempDir()` isolation(C-5 준수, 실제 `.claude/rules/moai/core/` registry 무변경), production 코드 무접촉(C-3 — pipeline.go/human_oversight.go 무변경). 3/3 AC PASS(AC-TEST-007/008/009; acceptance.md §D AC Matrix SSOT). plan-auditor PASS-WITH-DEBT 0.96(iter-2 2026-07-09, skip-eligible; Tier M 임계 0.80; iter-1 FAIL 0.82 BLOCKING D1 → D1-D5 해소). Run-phase(manager-develop ddd, Mode 5 sub-agent, Hybrid Trunk main-direct Route A; M1 `ffea91710` + M3 `65c23fbf2` — M2 verify-only는 M3 commit에 bundle per progress.md §E.3 `m1_to_mN_commit_strategy`). Sync-phase **now**(manager-docs, 단일 sync commit으로 3-phase close per SPEC-V3R6-LIFECYCLE-REDESIGN-001 merged 3-phase close / Status Transition Ownership Matrix; 4-artifact frontmatter status `in-progress → completed` atomic + `updated: 2026-07-09` + progress.md §E.4 audit-ready signal + CHANGELOG `[Unreleased] > ### Fixed` 본 entry. `sync_commit_sha`는 follow-up chore commit에서 backfill — 확립된 convention per SPEC-BRAND-DIR-REMOVE-001 `2c637ece2` / SPEC-TOKEN-BUDGET-STOP-001 `ab4f38def`). **ARCH-001 선행 관계 (spec.md §A SSOT)**: 본 SPEC M1은 ARCH-001 re-entry의 necessary condition — ARCH-001 AC-ARCH-001(whole-repo `go test ./... exit 0`)이 잔여 2개 internal/web i18n FAIL(`TestDataI18nKeysSubsetOfDictionary`, `TestI18nKeySetParity`)로 본 SPEC만으로 미달성, 후속 web-i18n SPEC(TEST-003) 완료가 추가 선행 조건(Out of Scope 명시적 제외). README 무변경 — 내부 테스트 인프라 정비, 사용자 가시적 CLI 변경 0. 🗿 MoAI

## [v3.0.0-rc10] - 2026-07-09

### Fixed

- **세션 제목** — `UserPromptSubmit` 훅이 매 프롬프트마다 `"<프로젝트> / <브랜치>"` 제목을 내보내 동일한 `custom-title` 레코드를 누적시키고, Claude Code가 대화 내용에서 자동 생성하는 `ai-title`을 `/resume` 목록에서 가려버리던 문제를 수정. SPEC이 감지되면 종전대로 `SPEC-ID: 제목`을 쓰고, 첫 프롬프트에서는 제목을 내보내지 않아 네이티브 `ai-title` 생성에 자리를 내주며, 그마저 없을 때만 첫 사용자 프롬프트에서 제목을 한 번 도출한다. 제목 추출은 슬래시 커맨드 토큰과 `ultrathink`/`ultracode` 지시어를 제거하고 룬 경계에서 잘라 다국어 텍스트가 깨지지 않는다. `buildProjectBranchTitle`·`getGitBranch` 제거.

### Changed

- **버전 정합** — `pkg/version/version.go`와 `system.yaml`이 선언하던 `v3.0.0-rc9`는 대응하는 git 태그가 없어, `Makefile`이 `git describe --tags --abbrev=0`(= `v3.0.0-rc8`)로 `ldflags` 덮어쓰기를 하면서 빌드된 바이너리가 rc8을 보고하는 드리프트가 있었다. 태그가 SSOT라는 원칙(CLAUDE.local.md §5)에 맞춰 소스를 `v3.0.0-rc10`으로 올리고 동명의 태그를 생성해 정합을 회복한다. rc9는 태그 없이 건너뛴다.
...
- **[SPEC-HANDOFF-THRESHOLD-001](.moai/specs/SPEC-HANDOFF-THRESHOLD-001/spec.md)** — 핸드오프 임계 완성: 2단계 statusline suffix + `context-usage.json` 영속화 + Detection Heuristics state-file-first 독트린 (Tier M, Epic Handoff-v2 M4/4, v0.1.0). Epic Handoff-v2 최종 마일스톤 — M1 CTXGUIDE가 이연한 3항목(2-stage / config 소비 / 영속화)을 완결. **M1**: `internal/statusline/renderer.go` `shouldShowHandoffGuide`를 stage-enum(none/soft/hard)으로 전환 — soft `(⚠️/clear)`(≥500K→50% / else→90%) 유지 + hard `(🛑/clear!)` 신규(`min(HandoffHardCeilingCapPct=95, getAutoCompactThreshold()=85+HandoffHardCeilingMarginPct=10)=95` + `hard<soft` clamp; default 95는 auto-compact 85 선점으로 200K급서 드물게 발화 — 정직 문서화 REQ-005); 밴드 경계는 `internal/config/defaults.go` named const로 상수화(§14 하드코딩 방지, 신규 config 필드 0 — "M3 landing / M4 소비" 계약 준수). **M2**: `internal/statusline/context_usage.go` — render마다 `<projDir>/.moai/state/context-usage.json` atomic 기록(`model_cache.go` temp+rename 패턴 재사용), 9-field 스키마(schema_version/session_id/writer_pid/captured_at/context_window_size/tokens_used/raw_pct/stage/band), write-if-changed throttle(`int(raw_pct)`+stage+band, writer_pid 제외), `os.Getpid()` writer_pid discriminator, `builder.Build` 내 call-site(SessionID+Memory 스코프), fail-open. `isFreshForSession` 헬퍼(UUID→session_id 매칭 / empty→freshness+writer_pid). **M3**(독트린): `context-window-management.md` Detection Heuristics를 state-file-first로 재작성(LIVE + template mirror 양 사본 section-level edit, full-sync 금지 — M1 256K 행 회귀 방지), template mirror에 누락됐던 256K 행 추가(parity, plan-auditor D1 SHOULD-FIX), Template-First `make build`. **M1 무회귀 불변(AC-006)**: statusline suffix는 config-독립(HandoffConfig 시그니처 미참조, guide=false 기본에도 soft suffix 렌더) — Guide 양성 소비는 D4 독트린(오케스트레이터 advisory)로 표현, M3 `handoff_inject.go` 무접촉. 18/18 AC PASS(AC-THRESHOLD-001..018; D1 template-parity REQ/AC-017, D2 concurrent-empty-id writer_pid discriminator REQ/AC-018 포함). plan-auditor iter-1 PASS-WITH-DEBT 0.84(Tier M 임계 0.80, BLOCKING 0); D1(template/live 256K drift) + D2(B4 fallback-UUID 동시 empty-id 교차오염 hole) SHOULD-FIX 2건 해소. Run-phase 2026-07-06(manager-develop TDD, L1 격리 worktree 2회 — 1차 세션한도 abort→M1만 cherry-pick 회수 land `eb23fc075`, 2차 M2+M3 ff-merge 회수 → 병렬 web-console `a73ceedf6` 위 rebase land `22aef7987`). 독립 재검증(orchestrator, 7/7 PASS): go build+GOOS=windows exit 0 / go test 89 pkg ok(`internal/cli/TestRunHookEvent_ReadInputError` base-reproduced baseline panic 제외, M4 무접촉 diff empty) / make build exit 0 / 256K parity LIVE=1 template=1 / template §25 internal-token 0 / AC-006 statusline HandoffConfig 시그니처 grep 0 / 런타임 아티팩트 누출 0. Sync-phase(orchestrator-direct — manager-docs CHANGELOG autocompact thrashing 회피, 3-phase close: status in-progress→completed + era V3R6 + progress §E.4 sync_commit_sha). 🗿 MoAI

- **[SPEC-HANDOFF-AUTORESUME-001](.moai/specs/SPEC-HANDOFF-AUTORESUME-001/spec.md)** — 역방향 세션 핸드오프 auto-resume: `moai handoff save/clear` CLI + SessionStart 주입/소비 핸들러 (Tier L, Epic Handoff-v2 M3/4, v0.1.0). paste-ready resume를 `.moai/state/handoff/pending.json`에 저장하고, `/clear` 후 SessionStart가 조건부로 자동 주입·소비하는 역방향 핸드오프 완성. 기존 SessionEnd→memory 흐름(`persist.go`)과 `session-handoff/` 트리는 무접촉(경로 격리). M1: `HandoffConfig{mode,guide}` landing (ResearchConfig 패턴, `internal/config` types/defaults/loader/audit_registry + 중립 template `handoff.yaml` + audit parity 등록; config default `mode=manual`, `consume` 필드 YAGNI 제거). M2: `moai handoff save/clear` top-level cobra CLI(`internal/cli/handoff.go` self-register) + 별도 `handoff/pending.json` atomic writer(`internal/hook/handoff/pending.go`, 기존 `atomicWriteFile` 재사용) — `session-handoff/pending.md` 미생성/미변경. M3: SessionStart 주입/소비 핸들러(`internal/hook/handoff_inject.go` + render) — `source==clear ∧ mode==auto`에서만 claim-then-inject(rename-to-consumed audit trail 보존), degrade-to-guidance(주입 콘텐츠는 ultrathink/xhigh 활성 미주장, 복원 안내만 — verification-claim-integrity 준수), nonce fallback(NULL session_id → `crypto/rand` 8hex, 파일명 `^\d+-[0-9a-f]{8}\.json$`), errno-agnostic rename fail-open(Windows `MoveFileEx` 호환), i18n header(ko/en/ja/zh + fr→en fallback), auto-only stale TTL cleanup(manual mode는 stale이어도 pure no-op), 3-handler registry accumulate-all 공존(sessionStart attribution + handoff 안내 both-inject). 19/19 AC PASS(AC-AUTORESUME-001..019). plan-auditor iter-3 PASS-WITH-DEBT 0.90(Tier L 임계 0.85; iter-1 0.85→iter-2 0.89→iter-3 0.90 단조 증가, D1-D7+N1-N3 정정 검증; D1 MINOR[AC-008 GIVEN live-scope 한정어 누락, REQ-level 모순 없음]은 test-authoring guidance로 carry). Run-phase 2026-07-06(manager-develop TDD RED-GREEN-REFACTOR, L1 격리 worktree 6 atomic GREEN 커밋 → orchestrator origin/main FF land `2b0efe516`, 병렬 세션 로컬 main 86건 미커밋 무접촉). 독립 재검증(orchestrator, 8/8 PASS): go test config+hook ok / `-race` clean(AC-013) / AC-016 경계 grep 0 / go build host+windows exit 0 / golangci-lint 0 / go vet 0 / spec lint No findings / coverage handoff 86.6%. cli suite `TestRunHookEvent_ReadInputError` panic은 base `9fbc7a2be`에서도 재현되는 pre-existing baseline 결함(handoff 무관, `grep HandoffConfig`=0으로 확정). Sync-phase now(orchestrator-direct — manager-docs가 CHANGELOG 7720줄 read로 autocompact thrashing 실패하여 전환, 3-phase close: status in-progress→completed + era V3R6 + progress §E.4 sync_commit_sha). 🗿 MoAI

- **[SPEC-WEB-CONSOLE-011](.moai/specs/SPEC-WEB-CONSOLE-011/spec.md)** — 웹 콘솔 재설계: 10섹션 config 전면 편집 + Agent Settings 4표면 + 프로필 CRUD + SPEC 보드 + statusline cache_hit (Tier L, 6 마일스톤, 총 52 AC — [B] 전량 PASS). M1: scope contract 10섹션 전체로 supersede + yaml.Node 주석보존 seam 신설 (Save() 경로 없는 8섹션 load-bearing). M2: config 전면 확장 163개 필드 + seam persistence 인프라 + i18n ×4. M3: agent settings 4표면 전면 쓰기(llm tiers + workflow role_profiles + sub-agent frontmatter + workflow_agents) + frontmatter-only patch layer. M4: profile-name path-traversal 보안 수정(repro-test-first) + CRUD UI + i18n ×4. M5: SPEC READ-ONLY 보드 + status/tier 배지 + remediation copyable text + ListDocs export. M6: statusline cache_hit 노출 fan-out + segment-list SSOT + TUI-web 대칭성 완결. 병렬 renderer.go 미보호 및 PRESERVE @MX:NOTE 관리, internal/cli 선재 nil-pointer 무접촉으로 runphase 완전 격리. 🗿 MoAI
- **[SPEC-PRECOMMIT-001](.moai/specs/SPEC-PRECOMMIT-001/spec.md)** — 배포 계층 pre-commit 훅 설치기 추가 (fast-subset, pre-push installer 대칭, Tier S, v0.1.0). `moai init`/`moai update`가 사용자 프로젝트 `.git/hooks/pre-commit`에 경량 커밋-tier 검증 훅을 설치. staged Go 파일에만 `gofmt -l` + `go vet`(fast-subset)을 실행하고, 무거운 lint+test는 pre-push `make ci-local`이 담당하는 2단 구조(commit=fast / push=full-CI). 새 `PreCommitInstaller` + `installPreCommitHookOptional` + `.git_hooks/pre-commit`(1953B POSIX sh) 템플릿 + byte-identity 테스트(`TestPreCommitTemplateMatchesConstant`, pre-push 가드 대칭). `#!/bin/sh` POSIX + `# MoAI-ADK pre-commit hook` marker 기반 create/overwrite/preserve(`ErrUserHookExists` sentinel 재사용, `fileHasMoaiMarker`를 `fileHasMarker(path, marker)`로 일반화 — pre-push 동작 byte-for-byte 보존), `SKIP_MOAI_PRECOMMIT=1` bypass, toolchain-absent graceful skip(비-Go 프로젝트에서 `go`/`gofmt` 부재 시 exit 0 — 16개 언어 배포 안전), config-agnostic install(dead였던 `pre_commit` config 실체화; tri-state는 설치 게이트로 미독취). Template-First 배포(embed 재생성, catalog hash 무변경). plan-auditor iter-2 PASS 0.92(Tier S 임계 0.75; iter-1 PASS-WITH-DEBT 0.79 → D1-D7 해소 — D4는 "pre_push는 dead" 오주장 정정: `hook_pre_push.go` `resolvePrePushAction`이 runtime severity dial로 live, `pre_commit`만 genuinely dead, verification-claim-integrity §1.1 surface-3). 15/15 AC PASS(AC-PC-001..015: 생성/foreign 보존/marker overwrite/byte-identity/gofmt 차단/bypass/cross-platform/no-staged-Go no-op/go vet 차단/--no-hooks skip/2단 경계/install assertion/toolchain-absent/non-fatal 실패/config-agnostic). Run-phase(manager-develop TDD, L1 격리 worktree → orchestrator `git show`-회수 후 partial commit `git commit --only` — 병렬 세션 shared-index 90+파일 오염 회피, 우리 10파일만; local `2ebff1355` feat + `44d892d2d` §E.3 backfill). Sync-phase(orchestrator-direct — manager-docs validation-only 실패로 전환, 3-phase close: status in-progress→completed + progress §E.4). 🗿 MoAI

- **[SPEC-HARNESS-EVO-PIPE-REPAIR-001](.moai/specs/SPEC-HARNESS-EVO-PIPE-REPAIR-001/spec.md)** — 하네스 학습 파이프라인 수리 + v4 스모크 게이트: 제안 생성 구조적 0 해소 (파이프라인 Go 엔진), Stop-path classify 자동 실행 + PostToolUse observer 등록 (훅 배선), `moai harness doctor` 참조 무결성 4축 검증 (스모크 게이트 v4), exemplar Runner 부패 수리 (release-update) (Tier M, Epic Harness-Evolution 1/4). 1번 문제: `Tier.String()` 어휘(observation/heuristic/rule/auto_update)와 mapper의 actionable 어휘(recommendation/approval_required) 불일치로 제안 생성 0건 고착. 수리: mapper가 Tier.String() SSOT에서 파생하도록 정렬, 실데이터(tier-promotions.jsonl 16건) 모두 채택. 2번: pattern_key 스키마(`<event_type>:<subject>:<context_hash>`, EventType enum 파생) vs mapper 수기 prefix 목록(`{code_change,error_pattern,tool_failure,repeated_edit}`) 불일치로 regex full 거부. 수리: EventType enum 파생으로 통일, 빈 subject/context_hash 허용 정책 명시. 3번: classify 수동(`moai harness status` step 0만) 의존 제거. 수리: Stop hook 경로에 classify 자동 실행 체인 추가(fail-open 정책) + PostToolUse observer 등록(settings.json/template dual). 4번: v4 참조 무결성 기계 검증 부재. 수리: `moai harness doctor` 신규 명령(어플리케이션 verb 8개 완전화) — 4축 cross-ref(entry·manifest·Runner·specialist agent) 검증, manifest 없는 thin 하네스(github/release) INFO-only, exit 0 graceful. 5번: exemplar Runner(local dev-only `.claude/workflows/harness-release-update-run.js`)의 MANIFEST_PATH/agent 참조 오류. 수리: `.claude/commands/harness/release-update/manifest.json` + `harness-release-update-specialist` 정정, doctor 게이트로 회귀 방지. 20/20 AC PASS(구조적 0 제안 생성 3건 복구 실증, doctor 5-layer 정상/zero-harness graceful/exemplar 2-error 탐지/수리 후 clean, Phase 0 8-verb 완전화, rate_limit SSOT 정렬, template 8-surface parity 검증, 중립성 grep 0). 템플릿 sync(harness.md/harness-build-entry.md/harness-builder.md/moai SKILL/learner SKILL/commands/moai/harness.md + settings.json.tmpl/harness.yaml), 도그풋 Runner 로컬 전용(template 미유출), 도그풋 learner harness 미생성(frozen_guard default-deny). 3-phase close. 🗿 MoAI

- **[SPEC-ASTGREP-MULTILANG-001](.moai/specs/SPEC-ASTGREP-MULTILANG-001/spec.md)** — 템플릿에 큐레이션된 중립 다국어 ast-grep 프로덕션 베이스라인 배포 (Template-First, Tier M, v1.0.0). 기존 템플릿은 `go-hardcoding.yml` 단일 파일만 배포하고 도그풋 트리는 실험 스캐폴드(빈 `.gitkeep` stub, 한국어 메시지, 존재하지 않는 `utils` ruleDir + 내부 SPEC-ID를 declare하는 `sgconfig.yml`, `sg` 0.40.5에서 매칭 안 되거나 noisy한 룰)였음. 도그풋을 그대로 복사하면 이 결함이 16개 언어 배포에 전파되므로, `sg`-검증된 신뢰 가능 부분집합만 큐레이션: 중립 `sgconfig.yml`(`ruleDirs: [go, security]`, 토큰·한국어 0) + 이식된 `go/hardcoding.yml`(2 룰) + 영어 재작성 security 세트(`credentials.yml` hardcoded-credential CWE-798 × go/python/js/ts, `injection.yml` command-injection CWE-78 × 4언어, `crypto.yml` weak-hash MD5 × go). 모든 shipped 룰은 positive/negative fixture로 `sg` config-mode 검증(pos=1/neg=0); noisy·미매칭 도그풋 룰(XSS/JWT/path-traversal, `return $ERR` 계열)은 미배포 후 equal-priority future로 기록(§15 언어 중립성 — PRIMARY 언어 없음, uncovered=미지원 아님). 도그풋 트리 불변(로컬 divergence 허용). run-phase는 manager-develop이 L1 격리 worktree에서 수행 → 오케스트레이터가 main으로 reconcile + 독립 재검증(neutrality 0/0, sg scan 파싱 정상, astgrep+template 테스트 GREEN, embed/leak 회귀 0). 11/11 AC PASS. 3-phase close(Tier M). 🗿 MoAI

### Changed

- **[SPEC-HANDOFF-MSGMODE-001](.moai/specs/SPEC-HANDOFF-MSGMODE-001/spec.md)** — 핸드오프 메시지에 오케스트레이션-모드 시드 내장 (message-v2, Tier S, Epic Handoff-v2 M2/4, v0.1.0). paste-ready session-handoff 메시지의 Block 1(`ultrathink.` 라인)에 조건부 `mode:` 오케스트레이션-시드 추가하여 `/clear` 후 재개 세션이 **어떤 phase-0.95 모드로 재개할지** 신호를 받을 수 있도록 함. 4-enum mode catalog(solo-sequential↔Mode 5 sub-agent / parallel-subagents↔Mode 4 parallel / agent-team↔Mode 3 / dynamic-workflow↔Mode 6 workflow) — Phase 0.95 카탈로그와 1:1 대응(Mode 1 trivial·Mode 2 background는 handoff-seed 제외); solo-sequential(default) 생략으로 v1 byte-identical 보존. 본 SPEC은 doctrine-only(Go/config/state 변경 0)이며 4 표면 동기 갱신(`.claude/rules/moai/workflow/session-handoff.md` SSOT + `.claude/output-styles/moai/moai.md §8` render + 2 template mirror). B1(Block 1 placement 일반화) · B2(emit-discouraged 전향-가드) · B3(moai.md §8 자가검증 10→11 항목) + ultracode bare-opener vs `/effort` 세션-지속성 조화 + JSON-twin schema_version:2 forward-compat 주석 포함. 16/16 AC PASS (mode-seed core + exclusion-logic + 4-enum 매핑 + 임계값 재사용 + directive-binding + bare ultracode 정합 + permission-clarification + schema note + locale parity + ordering + emit-discouraged + template-completeness + mirror-parity + byte-identity + lint + concern-sentinel). 3-phase close. 🗿 MoAI

- **[SPEC-RULE-DIET-002](.moai/specs/SPEC-RULE-DIET-002/spec.md)** — always-loaded 컨텍스트 표면에서 6개 reference-doctrine 규칙을 `paths:` scope 처리해 per-turn 토큰 예산 절감 (Tier M, v1.0.0). Anthropic "Steering Claude Code" 원칙("특정 파일에만 적용되는 규칙은 `paths:`로 scope") 적용. `paths:` 없는 규칙은 매 턴 always-loaded, `paths:` 있는 규칙은 매칭 파일 작업 시에만 로드. 6개 규칙(runtime-recovery-doctrine, dynamic-workflows, native-invocation-model, sprint-round-naming, goal-directive, verification-batch-pattern)에 self-referential `paths: "**/<filename>.md"` frontmatter 추가(양 트리, frontmatter-only, 본문 무변경) → always-loaded LIVE 11→5, TEMPLATE 11→5 (≈ −18,367 tokens char/4). 5개 KEEP 규칙(session-handoff/agent-common-protocol/askuser-protocol/verification-claim-integrity/context-window-management)은 per-turn load-bearing이라 무변경. scoping 안전성 담보: recovery pointer는 agent-common-protocol.md(KEEP)에 존재 확인, retired-alias 앵커(Epic/SPEC/Milestone/Constitution canonical, Sprint/cohort/Round/Wave retired)를 moai.md § Banner Localization(KEEP, always-loaded)에 신규 추가. token_budget_guard 상대 재계산이라 소스 무변경 자동 적응, 인바운드 cross-ref는 self-referential paths로 전부 resolve 유지. 6 MUST-FIX AC 전부 PASS(11→5 delta, KEEP 무결성, byte-parity/sanitized-pair 검증, guard GREEN). 3-phase close(Tier M). 🗿 MoAI

### Removed

- **[SPEC-TOOLPOLICY-DEPLOY-REVIEW-001](.moai/specs/SPEC-TOOLPOLICY-DEPLOY-REVIEW-001/spec.md)** — 사용자 프로젝트에 배포되지만 소비되지 않는 orphan `tool-policy.yaml`(48KB/181 entries)을 템플릿 배포에서 제거 (Direction A, Tier S, v1.0.0). 조사로 검증: `settings.json`은 `settings.json.tmpl`에서 독립 렌더링되어 `tool-policy.yaml`과 무관하며, 유일 소비자는 `moai tool-policy` codegen CLI(`internal/cli/tool_policy.go`) 1개뿐 — 즉 배포본은 어떤 사용자 런타임 경로에서도 읽히지 않는 dead weight. `internal/template/templates/.moai/config/sections/tool-policy.yaml` 삭제 + `audit_loader_completeness_test.go` `acknowledgedDedicatedLoaders` 주석 dev-only 재배치 갱신. repo-root dev SSOT(codegen용)는 보존(`moai tool-policy list` 여전히 181 entries). settings.json permissions 블록 / `moai tool-policy` 명령 / codegen 패키지 불변. 기존 프로젝트는 `moai update`가 삭제하지 않으므로 무해(신규 init에만 영향). 5/5 AC PASS(신규 init tool-policy.yaml 부재 실측, settings.json permissions byte-identical, go test GREEN, dev SSOT 181, 소비자 1건 불변). 3-phase close(consolidated Tier S). 🗿 MoAI
...
- **[SPEC-HANDOFF-CTXGUIDE-001](.moai/specs/SPEC-HANDOFF-CTXGUIDE-001/spec.md)** — 256K 컨텍스트 윈도우 핸드오프 안내 임계 결함 수정 (Tier S, Epic Handoff-v2 M1/4, v0.1.0). `shouldShowHandoffGuide`가 `switch cwSize { case 1_000_000 / case 200_000 / default: return false }` exact-match였어서 256K(현재 non-1M Opus/Fable 클래스)가 사용량과 무관하게 핸드오프 안내가 영구 미표시되던 실사용 결함 해소. 크기 무관 밴드 로직으로 교체: `cwSize >= 500_000 → rawPct >= 50%`(1M 대형 윈도우), `0 < cwSize < 500_000 → rawPct >= 90%`(256K/200K headroom 밴드), `cwSize <= 0 → hidden`(안전 기본값). 500K 컷오프가 1M 대형(≥50%)과 standard/medium(≥90%) 클래스를 분리하며 256K는 200K headroom 밴드를 공유. 근본원인=기존 `TestShouldShowHandoffGuide_UnknownCwSizeFalse`가 cwSize=0만 검증해 256K exact-match 결함을 미포착. `.claude/rules/moai/workflow/context-window-management.md` § Context Window Targets 표에 256K 행(윈도우 256,000 / 임계 90% / 절대 여유 ≈230K) 추가(session-handoff.md Trigger #1이 이 표를 SSOT 참조하므로 수치 중복 편집 불필요). 9/9 AC PASS(AC-256K-001..009: 256K@90 show / 256K@89 hide / 1M·200K 보존 / 500K·499999 경계 / cwSize=0 hidden / 문서 grep / statusline 회귀 / 크로스플랫폼 build host+windows exit 0), plan-auditor iter-1 PASS-WITH-DEBT 0.82(Tier S 임계 0.75), `shouldShowHandoffGuide` func-level 커버리지 100%, statusline 패키지 회귀 0. Run-phase 2026-07-04(manager-develop TDD RED-GREEN-REFACTOR, L1 격리 worktree → M1 commit 092dcba9f origin/main FF push). Sync-phase 2026-07-04(orchestrator-direct 격리 worktree 3-phase consolidated close: spec/plan status in-progress→completed + era V3R6 + updated 2026-07-04 + progress §E.4 sync_commit_sha, 병렬 WEB-CONSOLE-011 세션 shared-index 오염[M1 역전 staged — stale-base 아티팩트로 확정, 3커밋 연속 미포함] 회피). 🗿 MoAI

- **[SPEC-DEPRECATEDPATHS-RECONCILE-001](.moai/specs/SPEC-DEPRECATEDPATHS-RECONCILE-001/spec.md)** — `DeprecatedPaths`가 여전히 배포·프로덕션 로드되는 `design.yaml`/`db.yaml`를 v3.0.0 제거 대상으로 잘못 등재한 모순 해소 (Tier S, v1.0.0). 두 파일은 v3 템플릿에 배포되고 프로덕션에서 로드됨(`internal/config/loader.go:92 loadDesignSection`; `internal/cli/hook.go:448 loadMigrationPatterns` → dbsync/SPEC-DB-SYNC-001)인데도 `DeprecatedPaths`(RemovalSchedule v3.0.0)에 남아 `moai update` churn(live config backup+삭제 후 재배포) + false-v2 감지를 유발. 원 제거 전제(SPEC-V3R6-V2-V3-CLEAN-REINSTALL-001 REQ-VVCR-011/012)는 authoring 시점엔 TRUE였으나 stale화됨(`db.yaml`는 SPEC-DB-SYNC-001이 live 재확립, `design.yaml`는 SPEC-DEAD-CONFIG-001 §D가 live 재분류) — ground-truth-shift reconciliation. `dirs.go` DeprecatedPaths 2개 엔트리 제거 + `dirs_test.go` 카운트 lockstep(총 43→41, Category B 31→29) + `@MX:ANCHOR` 이 SPEC을 41-entry authority로 redirect(완료 origin SPEC body 불변) + `v2_detection.go` doc comment. 템플릿·로더 무변경, 9/9 AC GREEN, cross-platform build 통과. Follow-up(plan-auditor debt): `.moai/project/brand` + `.claude/rules/moai/design/constitution.md`도 shipped+deprecated 형제라 false-v2 완전 해소는 별도 reconcile SPEC 소관(§E.2 기록). 🗿 MoAI
...
- **[SPEC-V3R6-DOCS-POSTREBUILD-CLEANUP-001](.moai/specs/SPEC-V3R6-DOCS-POSTREBUILD-CLEANUP-001/spec.md)** — docs-site 후속 cleanup: H1 restoration (D2) + maintainer-path leak (D6+new) + robots.txt domain (D7) + hugo.toml version bump + ko moai-feedback.md accuracy (Tier M, v0.1.2). SPEC-V3R6-DOCS-V3-REBUILD-001 sync-auditor Tier-L 검수에서 지적한 4 MUST-FIX 결함 중 D1/D3/D4는 이미 post-close commit 8f89a42b7로 수정됨. 본 SPEC은 user-deferred D2(16 pages × 4 locales H1 누락)/D6(worktree pages 경로 유출)/D7(robots.txt 도메인 오기)을 처리하고, 계획 기간 중 발견된 2개 신규 결함(hugo.toml stale v3.0.0-rc4 → v3.0.0-rc5, getting-started/inventory.md 경로 유출, ko moai-feedback.md 부정확성) 추가 해소. REQ-DPC-001..006 + NFR-DPC-001..004: H1 restoration 65→0 error (16 pages × 4 locale 동시 착수), maintainer-path `/Users/goos/MoAI/moai-project` → `/path/to/your-project` 28 매칭 (worktree 2files × 4locale + inventory.md × 4locale) + `/Users/goos/MoAI/moai-adk-go` 2매칭 (JSON example), robots.txt `cowork.mo.ai.kr` → `adk.mo.ai.kr` 도메인 정정, hugo.toml params.version `v3.0.0-rc4` → `v3.0.0-rc5` + params.releaseDate `2026-06-23` → `2026-07-01` (L54 SSOT 2-line paired edit), ko moai-feedback.md new "피드백 설정" subsection (4가지 enhancement documented: guaranteed diagnostics MoAI+OS / best-effort Go+error-context / dedupe-gate / gh-fallback draft-save, `.claude/skills/moai/workflows/feedback.md` + `internal/config/feedback_accessors.go` + `.moai/config/sections/feedback.yaml` 실측 근거), agent-chain diagram misattribution "sync-auditor" 제거(실제 workflow orchestrator-direct), "자동 수집되는 정보" table accuracy 정정(Claude Code 버전 / 현재 SPEC rows 제거, Go 버전 실제 수집 항목으로 복원). 78 파일(docs-site/ 영역만, Go/template source 무변경 per OUT-OF-SCOPE), 4-locale parity(ko/en/ja/zh 각 99-file 기준선 변경 0), mechanical symmetry 강제. 19/19 AC PASS (AC-DPC-001a..006f): H1 grep 0, 모든 locale × 페이지 frontmatter title 매칭, 파일 카운트 99 유지, maintainer-path leak 0 검증 × 3 surface, `/path/to/your-project` 1:1 replacement, robots.txt domain 정정 + cowork 0, hugo.toml version/date 동시 갱신 + stale rc 0, ko feedback 4개 enhancement 전부 소급 + misattribution 제거 + table 정정, ko-only scope(en/ja/zh feedback.repository 0 매치 = deferred 경계 보존). Ground truth: `scripts/docs-i18n-check.sh` fresh Check-3 64 H1-missing (구 plan baseline 정확), 28 `goos` pattern 매칭(worktree 8/locale + inventory 8/locale) 확인 재검증. Hugo build `hugo --minify` exit 0, v3.0.0-rc5 string output, 133 pages (ko/en/ja/zh 각 99→99 유지 안정화). Verification: orchestrator-independent per-AC check(19/19 문서화 command + expected output 일치), D1(H1 미포함 pre-existing Check-4 glossary defect) residual 1개 expected(spec.md Exclusions 정확히). Run-phase 2026-07-02 (모든 M1-M4 milestone committed + pushed origin/main 직진, Hybrid Trunk 1-person OSS 정책, squash 불필요 — 각 milestone discrete commit 목적성, 4 commits 1b97cb4d3/e06864285/9cf657de6/7c62c4882 + final HEAD 7c62c48829bf9367e6613e55c2b0540bbb76974a). Sync-phase **now** (orchestrator-direct, 3-phase close: spec.md status in-progress→completed + era V3R6 + updated 2026-07-02, progress.md §E.4 sync_commit_sha backfill, CHANGELOG entry, specific-path-discipline 2-commit close pattern). 🗿 MoAI

- **[SPEC-V3R6-DOCS-V3-REBUILD-001](.moai/specs/SPEC-V3R6-DOCS-V3-REBUILD-001/spec.md)** — adk.mo.ai.kr (Hugo/hugo-geekdoc, v3.0.0-rc4 정확도) 전체 rebuilding + 4-locale accuracy 동기화 (Tier L, v0.1.1). Plan-phase (IA redesign + research-backed 112-file CC-mirror + 16 NEW pages detection), Run-phase (M0 README 4-locale drift fix + hugo.toml rc4 lock, M1 57 ko pages validate-then-rewrite, M2 28 CC-mirror claude-code pages WebFetch rewrite, M3.1 38 en files 7-chunk propagation, M3.2 ja+zh propagation, M3.3 menu 4-locale, M4 build-fix) → Sync-phase (CHANGELOG ko prose + spec.md status completed + era: V3R6). 99×4 locale full parity 달성 (ko/en/ja/zh 각 99파일; hugo build clean, EN/JA/ZH 131 pages parity). Ground truth: 27 moai-* skills, 13 /moai commands, 8 retained agents, 45 run-phase commits all pushed origin/main. Verification: per-chunk orchestrator-independent validation (section parity / Hangul 0-leak / URL 0-forbidden / Mermaid LR 0 / FleetView 0 / git scope clean). Known residual (SHOULD-FIX, non-blocking): builder-agents.md L21 ASCII transliteration, 84 line-matched pages staleness spot-check. 45 commits, era V3R6, drift 0. 🗿 MoAI
...
- **[SPEC-TOKEN-EFFICIENCY-001](.moai/specs/SPEC-TOKEN-EFFICIENCY-001/spec.md)** — P0 토큰 효율성: always-loaded context 예산 가드 + prompt-cache 히트율 신호 (Tier M, v0.2.0). moai-adk이 CC 세션에 주입하는 always-loaded 계층(CLAUDE.md + .claude/rules/moai/** no-`paths:` 파일들 + output-style moai.md + MEMORY.md 헤드) 대한 토큰 예산을 CI/go-test 가드로 측정 + 초과 시 FAIL (P0-1); 동시에 prompt-cache 히트율을 statusline에 `💾 N%` 신호로 표면화해 모니터링 (P0-2, 기존 `context_window.current_usage` 필드 재사용, null/zero 우아한 degradation). 예산 추정=char/4 확률론적 모델(zero-dep tripwire), 상수=75,000 token (측정된 baseline ~64.6K + 15% 여유); 히트율 표시=`cache_read / (cache_read + cache_creation) * 100` (render-toggle via `cache_hit: false` statusline.yaml, default-on). P0-1과 P0-2는 독립 shippable (internal/config vs internal/statusline, separate packages). 7/7 ACs PASS (AC-TEF-001..007): budget guard measure + enumerate + document + scoped-rule exclude + graceful degrade; 2 새 파일 `internal/config/token_budget_guard*.go`, 3 edited `internal/statusline/**`, 무변경 설정 스키마. Cross-platform build ✓; `go test ./internal/{config,statusline}/` 모두 PASS; coverage ≥85%; golangci-lint 0. Plan-auditor v0.2.0 후 PASS 0.91 (Tier M 0.80, 원본 v0.1.0 3-item 중 "~7×" attribution reword 도메인-분리 SPEC-DIVECC-ATTRIBUTION-FIX-001로 carved-out). Run-phase 2026-07-02 (run_commit_sha 3e30fef48, main synced 0 0, all 7 ACs verified). Sync-phase **now** (orchestrator-direct, spec.md status in-progress→completed + era V3R6 + updated 2026-07-02, progress.md §E.4 sync_commit_sha backfill, specific-path-discipline). 🗿 MoAI

## [v3.0.0-rc5] — 2026-07-01

### Added

- **[SPEC-V3R6-HANDOFF-GOAL-BINDING-001](.moai/specs/SPEC-V3R6-HANDOFF-GOAL-BINDING-001/spec.md)** — Claude Code `/goal`을 세션 핸드오프 resume Block 1에 결합 (Tier M, v0.1.0). paste-ready resume Block 1에 조건부 `/goal <완료조건>` 재설정 라인 추가 — 기존 `/effort ultracode` 조건부 라인과 동일 메커니즘, 다음 SPEC이 run-phase이고 완료조건이 기계검증 가능할 때만 emit(그 외 생략, default-on-ambiguity omit). 공식문서 실측 근거: `/goal` 인자는 완료조건(평가용)이지 실행명령 아님, `/clear`로 소멸하고 `ultrathink.`로 미복원 → Block 1 명시적 재설정 필요. 6파일 편집(session-handoff.md SSOT + moai.md §8 render + goal-directive.md cross-ref, 각 LIVE+template mirror byte-identical) + `/goal` 라인은 run-phase 자동 진입을 승인하지 않음(Implementation Kickoff Approval 유지) invariant 명시 + session-handoff.md 템플릿 언어중립성 보존. 14/14 AC PASS, era V3R6, drift 0. Plan-auditor touch-up 후 clean PASS(Tier M 0.80). Run+Sync 2026-07-01 (orchestrator-direct 3-phase close: run 577f84c8a + sync eca4075ae + sync_commit_sha backfill 599fa9917). 🗿 MoAI

- **[SPEC-INVOCATION-MODEL-001](.moai/specs/SPEC-INVOCATION-MODEL-001/spec.md)** — Native Invocation-Model Alignment: doctrine rule + feedback workflow enhancement (Tier M, v0.1.0). Canonical axis for `/moai` subcommand justification: invocation model (orchestrator auto-invocable) vs invocation channel (human-only), NOT naive command duplication test. C1: **native-invocation-model.md doctrine** (`.claude/rules/moai/workflow/native-invocation-model.md` — two-category taxonomy PROGRAMMATIC/HUMAN-ONLY, classification heuristic via official-docs `[Skill]`/`[Workflow]` markers, AC-IM-022 WebFetch verification); C2: **feedback workflow enhancement** (`.claude/skills/moai/workflows/feedback.md` + `.moai/config/sections/feedback.yaml` — 4 robustness: gh transient-retry, duplicate-PR gate, org auto-detect, issue sanitization). 22 ACs PASS (AC-IM-001..022): doctrine taxonomy + content + skill-body prose + config file + WebFetch evidence AC-IM-022 + edge-case sanitization. Cross-platform build ✓; golangci-lint 0; spec-lint 0; coverage ≥85%; template-first byte-parity dual-tree. Plan-auditor iter-2 PASS 0.86 (Tier M 0.80 baseline). Run-phase 2026-07-01 (manager-develop M1-M4 TDD, worktree reconcile + FF to main, synced 0 0). Sync-phase **now** (orchestrator-direct, 3-phase close: spec.md in-progress→implemented→completed + era V3R6 + updated 2026-07-01, progress.md §E.4 sync_commit_sha backfill). 🗿 MoAI

- **[SPEC-OWASP-CHECKLIST-GAP-001](.moai/specs/SPEC-OWASP-CHECKLIST-GAP-001/spec.md)** — OWASP 스킬 5개 보안 원칙 추가 + 오래된 에이전트 참조 정정 (Tier S, v0.2.1). moai-ref-owasp-checklist skill에 5개 generalized security 원칙 추가(table format: Principle/Applies To/Defense columns) + stale agent reference 정정 (§F Target Agents 오래된 archived-agent 참조 제거, 현재 manager-spec/manager-develop/manager-docs만 수행하도록 정정). REQ-OCG-001..010: 원칙 5개 추가 + agent 참조 정정 (archived-agent 5종 제거 + current 3종 유지 + manager-* 명확화 + scope 명시). 2개 파일: template 원본 source (`internal/template/templates/.claude/skills/moai-ref-owasp-checklist/`) + local deployed mirror(byte-identical) + `make build` catalog.yaml hash 재생성. 8/8 ACs PASS (AC-OCG-001..008: 원칙 table 정합 + agent 참조 정정 + archived-agent 제거 확인 + deployed mirror parity + catalog hash + scope containment + vendor-neutrality + stale reference absence). Go test + cross-platform build ✓, golangci-lint 0, lint clean. plan-auditor **PASS-WITH-DEBT 0.92** (Tier S threshold ≥0.75; D9 AC-OCG-007 vacuous-pass git-state-timing 함정 + D10 HISTORY 오기 minor — 둘 다 should-fix 수준으로 M5 verification sweep에 적절). Run-phase 2026-07-01 (commit 292a70831, main synced 0 0, verified deployed). Sync-phase **now** (orchestrator-direct, 3-phase close: spec.md status in-progress→completed + era V3R6 + updated 2026-07-01, progress.md §E.4 sync_commit_sha backfill, CHANGELOG entry, specific-path-discipline). 🗿 MoAI

- **[SPEC-RETRY-IDEMPOTENCY-001](.moai/specs/SPEC-RETRY-IDEMPOTENCY-001/spec.md)** — Retry Safety Asymmetry: observe-before-retry gate for side-effecting tool calls (Tier S, v0.1.0). 오케스트레이터 error-recovery 정책이 idempotent/read-only 연산과 side-effecting 연산을 구분하지 않는 단순 균일 retry 3-ceiling을 적용했으나, 실제로는 side-effect 프로파일에 따라 retry 안전성이 비대칭이다는 원칙을 `agent-common-protocol.md` § Error Recovery Pattern에 증강. REQ-RI-001..RI-008: 재시도 안전 비대칭 원칙 명시 + idempotent 연산 최대 3회 재시도 허가 + side-effecting 연산은 "observe-before-retry" 게이트 (불명확한 실패 후 현재 상태 관측 필수, 중복 효과 위험 방지 — 중복 커밋/PR/배포 방지) + 기존 4-step Error Recovery Pattern 및 3회 천장 보존 + §Ledger Closure/§Pre-Spawn Sync Check와의 의존성 없음 명확화 + 배포 규칙 중립성 보장. 2개 파일 변경 (`.claude/rules/moai/core/agent-common-protocol.md` deployed + template mirror) + `make build` embedded 템플릿 재생성. AC-RI-001..RI-011 (11/11 PASS): 비대칭 원칙 + idempotent 허가 + observe-before-retry 게이트 + 중복-효과 위험 명시 + 3회 천장 보존 + §3 참조 + template mirror 바이트 동일성 + 배포 중립성 + embedded.go 키워드 포함 + 해석 엣지케이스 정합. 7/7 ACs 통과 검증(grep + diff + embedded keyword + CC build) — template mirror 바이트 동일성은 region-scoped(§ Error Recovery Pattern 삽입 영역만, 파일 전체 아님 — spec.md §B.3 내부 SPEC-ID 누출로 인해 전체 parity는 불가능하나 증강 블록은 양 복사에서 byte-identical). Plan-auditor PASS 0.83 (Tier S baseline 0.75). run-phase 2026-07-01 (commit bbd0d4bc6, main synced 0 0, deployed rule 행동 검증 완료). sync-phase **now** (orchestrator-direct, 3-phase close: spec.md status in-progress→completed + era V3R6 + updated 2026-07-01, progress.md §E.4 sync_commit_sha backfill, CHANGELOG entry, specific-path-discipline single close commit).

### Added

- **[SPEC-SIMPLICITY-LADDER-002](.moai/specs/SPEC-SIMPLICITY-LADDER-002/spec.md)** — In-codebase reuse rung 보강으로 Simplicity Decision Ladder 6→7-rung 완성 (Tier S, v0.2.0). 형제 SPEC-SIMPLICITY-LADDER-001이 DietrichGebert/ponytail (MIT) ladder를 포팅할 때 누락한 단 하나의 rung — "in-codebase reuse / DRY" rung — 을 복원. ponytail 원본 7-rung 대비 moai 6-rung에서 빠진 rung("이미 이 코드베이스에 helper/util/type/pattern이 있는가? 재사용")을 YAGNI(rung 1)와 standard library(구 rung 2) 사이 position 2에 삽입, 나머지 rung 2-6 → 3-7 renumber. REQ-1: constitution ladder insert + renumber (L249 안전 carve-out + L251 3x-LOC trigger byte-보존); REQ-2: template mirror + make build; REQ-3: framing prose를 "reuse-and-dependency-avoidance ordering axis"로 확장 (reuse는 dependency-avoidance와 구분되는 별도 first-party 축); REQ-5: karpathy-quickref.md 교차참조 line의 arrow enumeration을 "in-codebase reuse → stdlib → ..."로 선두 추가 + framing 정합 (plan-auditor가 발견한 4번째 drift surface — manager-spec "no third copy" 주장 반증). "rung redundant?" falsification: reuse는 YAGNI(build/no-build), stdlib(언어 런타임), installed-dependency(외부 패키지)와 모두 구분되는 first-party in-project 능력 소스 — count-matching 아닌 실제 gap 복원. 14/14 ACs PASS (AC-SL2-001..014, per-rung content anchor r3=stdlib/r4=native/r5=dependency/r6=one-line 포함). 4 doctrine 파일(constitution + karpathy ×2 mirror) + catalog.yaml; irony guard zero (신규 Go/config/lint/hook/@MX 0). Template-First byte-parity dual-tree (양 file pair diff -q clean); make build = gen-catalog-hashes --all + go build (directive //go:embed all:templates, NO embedded.go); go test ./internal/template/ ✓ (mirror parity + neutrality CI guard); template neutrality leak 0. plan-auditor PASS-WITH-DEBT 0.84 iter-1 (Tier S threshold 0.75; D1 karpathy 4th drift → REQ-5, D2 embed reframe, D3 per-rung anchor 해소 → v0.2.0). 외부 조사 기원: ponytail(66K stars)/Scott Logic 비평/gstack Ponystack 심층 조사 — moai-adk가 이미 ponytail 코어(LADDER-001 + AUDIT-001)를 흡수했고 단 하나 누락된 rung을 복원. Run+Sync 2026-06-30 (orchestrator-direct, Tier S 3-phase consolidated close: spec.md status completed + era V3R6, progress.md §E.4 sync_commit_sha, specific-path-discipline).

- **[SPEC-V3R6-ASKUSER-DECISION-MEMORY-001](.moai/specs/SPEC-V3R6-ASKUSER-DECISION-MEMORY-001/spec.md)** — AskUserQuestion decision memory system (Tier M, v0.1.0). 5-component preference capture + adaptive recommendation placement. C1: **3-tier memory layer** (`internal/cli/preference/` 9 files — Entry/Store/FileStore with core≤4KB cascade, atomic upsert, namespace separation). C2: **Policy-doc authoring** (askuser-protocol.md + moai.md Recommendation Placement Principles 5-rule live/template split: Fisher-information timing, question ordering, statistical-majority baseline, precondition disclosure, adaptive strength by proficiency; template neutrality enforced via CI guard). C3: **PostToolUse advisory capture hook** (`internal/hook/user_decision_capture.go` — 6-step subpipeline mirroring logEvidence pattern, KI-1 schema tolerance, domain classification; Recovery-Signal Carve-Out doctrine-honest per REQ-ADM-010; advisory/fail-open design, 5 error scenarios exit 0; C-HRA-008 grep-safe via split-literal constant). C4: **Decay policy** (`internal/cli/preference/decay.go` + `moai preference decay-scan` CLI — pure power-law weight `(age+1)^(-0.5)`, α=0.5 fixed; 28-day TTL with Touch reset-on-reuse; daily scan cadence 24h gate; stable/transient separation prevents Koren "signal loss"). C5: **Recovery controls** (`internal/cli/preference/` 5 files — toggle sentinel, sensitive-domain gate, proficiency estimator, freshness label, correction loop). 18 ACs PASS (AC-ADM-001~017 + NFR-002~006). Coverage 86.0% (≥85% threshold). golangci-lint 0 issues; cross-platform build PASS (darwin+windows). plan-auditor PASS 0.88 iter-2 (Tier M threshold 0.80). M1 commit 6a42cde91 (preference layer), M2 commit c51839d2f (policy rules), M3 commit 4ba8265fe (capture hook), M4 commit 58877ac35 (decay), M5 commit 00edd02e4 (recovery). Sync-phase: CHANGELOG entry, README/README.ko Decision Memory section, docs-site 4-locale moai-plan.md adaptive recommendation documentation, design.md §E.1 weight correction (age-7: 0.378→0.354, age-28: 0.189→0.186 per formula), spec.md status completed + era V3R6 + updated 2026-06-25, progress.md §E.4 sync_commit_sha backfill.

### Fixed

- **[SPEC-GLM-MODEL-ALLOWLIST-001](.moai/specs/SPEC-GLM-MODEL-ALLOWLIST-001/spec.md)** — GLM 모드 모델 allowlist 정합 (Tier M, v0.2.0). `enforceAvailableModels:true`의 Claude-backend 비용 레버와 GLM redirect 메커니즘의 충돌로 모든 `moai glm` 세션이 Sonnet으로 폴백되는 회귀 해소. Approach C (정적 템플릿 allowlist 확장): `.claude/settings.json.tmpl`의 `availableModels`를 `["sonnet","opus","haiku"]`에서 `["sonnet","opus","haiku","opus[1m]","sonnet[1m]"]`로 확장해 GLM의 canonical alias인 `opus[1m]`/`sonnet[1m]`을 CC 2.1.176 enforceAvailableModels 검증 통과 대상으로 등록. Settings 런타임 오버라이드(Approach A) 채택 회피 — settings.local.json의 의도적 "clean" 정책(#676 회귀 방지)과 `moai glm`→`claude` 종료 시 제거 경로 간 충돌. 구현: 5개 파일(settings.json.tmpl + settings_test.go + model-policy.md ×2 + rule_template_mirror_test.go), go test(설정값 테스트 + mirror parity) green, golangci-lint 0, cross-platform build(darwin+windows) ✓. 6/6 AC mechanical PASS (AC-GMA-002/003/004/005/007/008) + 2/2 AC DEFERRED-MANUAL(AC-GMA-001/006, 실제 GLM 세션 측정은 dev project에서 forbidden이나 post-deploy M1 절차 문서화). plan-auditor PASS 0.84 iter-2 (Tier M threshold 0.80). Run-phase 2026-06-23 (manager-develop TDD, run_commit_sha e84c05e6c, main synced). Sync-phase 2026-07-01 (orchestrator-direct, 3-phase close: spec.md status in-progress→completed + updated 2026-06-23→2026-07-01, progress.md §E.4 sync_commit_sha backfill, specific-path-discipline).

### Removed

- **[SPEC-SUBCOMMAND-RETIRE-001](.moai/specs/SPEC-SUBCOMMAND-RETIRE-001/spec.md)** — `/moai` 5개 유산 subcommand 제거 및 7개 dependent 스킬 폐지 (Tier L, v0.1.0 / en: Retire 5 legacy `/moai` subcommands and 7 dependent skills). 미사용/낮은 가치의 `/moai design`, `/moai brain`, `/moai e2e`, `/moai coverage`, `/moai security` 5개 subcommand와 의존 스킬 7개(`moai-domain-ideation`, `moai-domain-research`, `moai-domain-design-handoff`, `moai-domain-brand-design`, `moai-domain-copywriting`, `moai-workflow-design`, `moai-workflow-gan-loop`) 폐지. 17 → 12개 `/moai` 명령(clean, codemaps, feedback, fix, gate, harness, loop, mx, plan, project, review, run, sync). Replacement paths: `/moai security` → `/moai review --security`, `/moai coverage` → `go test -cover` / `/moai gate`, `/moai brain` → `/moai plan`, `/moai design` → `moai web` / brand configuration, `/moai e2e` → out-of-Go-CLI scope (Go testing ecosystem 권장). README.md + README.ko.md 제거(workflow chain examples, Design System section 1179 라인), CHANGELOG bilingual entry + docs-site 4-locale residual clean. 15 ACs (AC-SCR-001..012, acceptance.md SSOT 기준; 종전 "AC-SRT-001..010 (10)" 프리픽스·개수 오기 정정). AC-SCR-008 dangling-reference grep 0은 M3 router-only cross-ref 정리가 놓친 워크플로우 파일 내부 참조 4건(review.md security.md 포인터 + loop.md/task-decomposition.md coverage.md + quality-gates-quality.md security.md)을 본 후속 amendment commit에서 정정한 뒤 whole-tree 0으로 성립. docs-site en/ko/ja/zh menu residual 0, workflow examples 제거 확인. Template-First: `.claude/commands/moai/` 5개 command 파일 + 7개 skill 스켈레톤 폐지(template.go가 아예 reference 안 함, ci/deploy 영향 0); spec-lint clean. sync-auditor PASS-WITH-DEBT ≈ 0.81 (fresh-context whole-tree re-grep이 M3 정리가 놓친 잔존 dangling 워크플로우 파일 참조 4건 + README EN/KO Design System 비대칭을 포착 — 본 후속 amendment commit에서 해소; 종전 "PASS 0.94, no residual risk"는 실제 auditor 산출이 아님). Run-phase 2026-06-30 (run-phase complete, pushed origin/main tip 222e42503, shared checkout synced). Sync-phase **now** (orchestrator-direct, 3-phase close: spec.md status in-progress→completed + era V3R6 + updated 2026-07-01, progress.md §E.4 sync_commit_sha backfill, specific-path-discipline 1-commit close). 🗿 MoAI

### Changed

- **[SPEC-V3R6-SEC-SKILL-INTEGRATION-001](.moai/specs/SPEC-V3R6-SEC-SKILL-INTEGRATION-001/spec.md)** — 방어적 사이버보안 reference 스킬 3종 독립 재작성 (Tier L, v0.3.0). 기존 보안 표면(`moai-ref-owasp-checklist` dev-time web-app)이 다루지 않는 운영 사이버보안 방어 지식을 MoAI-ADK 스타일로 독립 재작성한 3개 `moai-ref-*` reference 스킬 추가: **`moai-ref-llm-security`**(AI/LLM 방어 — prompt-injection 방어, OWASP LLM Top 10, MCP/agentic tool-call 하드닝, MITRE ATLAS 방어 상관, NIST AI RMF; 289L), **`moai-ref-supply-chain`**(공급망 방어 — SBOM SPDX/CycloneDX, dependency-confusion, malicious-package triage, SLSA provenance 4-level, Sigstore/cosign, typosquatting; 345L), **`moai-ref-secops`**(DevSecOps+Container+API 운영 — CI/CD 하드닝, IaC 스캔, K8s RBAC, container-escape 방어, Falco runtime, OWASP API Top 10 운영 방어, WAF; HARD 400-line split → SKILL.md 159L overview + 3 modules devsecops/container/api-ops). Option B'(독립 재작성 — 외부 커뮤니티 upstream을 개념/절차 참조원으로만 사용, 원본 문장/스크립트 미복사, distributed 산출물에 출처/브랜드/URL 미노출; 재작성은 공개 1차 출처 OWASP/MITRE/NIST/SLSA/Sigstore 기반). dual-use 방어 한정 — offensive 절차/공격도구(Mimikatz/Kerberoast/C2 등)/credential-dumping 전면 제외, MITRE T-ID는 방어 상관용만(공격 레시피 없음, REQ-SI-007 misconfig→detect→prevent 프레이밍). 16-언어 중립 — 생태계 도구는 ≥3 생태계 컨텍스트 또는 "the standard \<category\> tool" carve-out. 각 스킬 `user-invocable: false` + progressive-disclosure + 3 evolvable block + `## NOT for`. 18 ACs: 15 PASS + AC-SI-007 PASS-with-debt(공개출처 authoring으로 verbatim-copy 구조적 0; D1 ≥10-passage upstream 비교 잔여 부채) + AC-SI-017 sync close. internal research.md가 upstream을 기록하나 `.moai/specs/`는 `//go:embed all:templates` subtree 밖이라 template binary 미임베드(AC-SI-018 E-CONTAIN 0 매치 검증). Template-First byte-identical dual-tree(3 SKILL + 3 module ×2 mirror); make build exit 0; go test ./internal/template/ ✓ (internal_content_leak + neutrality + catalog count 34/41 guards); golangci-lint 0; spec-lint 0; template neutrality §25 0 매치. plan-auditor PASS-WITH-DEBT 0.86 iter-2 (Tier L 임계 0.85, monotonic ↑ vs 0.82); plan-audit 중 산출물 D10-D13 자가 정정(AC-SI-018 grep satisfiable·embed.go·REQ-SI-008 매핑) → orchestrator가 verification-claim-integrity로 재검증. Run-phase 2026-06-24 (manager-develop M1-M3 TDD/AC-gated, L1 worktree → orchestrator cherry-pick/ff 통합 + 사전 검증; M4 cross-skill holistic 검수 orchestrator-direct). Sync-phase 2026-06-24 (orchestrator-direct, 3-phase close: spec.md status completed + era V3R6, progress.md §E.4 sync_commit_sha backfill, CHANGELOG entry, specific-path-discipline 2-commit close).

- **[SPEC-SIMPLICITY-AUDIT-001](.moai/specs/SPEC-SIMPLICITY-AUDIT-001/spec.md)** — over-engineering 전용 lean audit 모드 (`/moai review --lean`) (Tier S, v0.1.0). DietrichGebert/ponytail (MIT) review/audit 기법 흡수 — 종합 4-perspective `/moai review`에서 over-engineering만 short-circuit하는 `--lean` 플래그 모드. 5 태그(delete/stdlib/native/yagni/shrink) + `net: -N lines possible` / `Lean already. Ship.` 결산, diff-default + `--repo` repo-wide 2-scope. read-only advisory (제거 없음 — `/moai clean`과 구분), `@MX:DEBT` harvest READ-only (`moai mx query --kind DEBT`, 형제 LADDER-001 shipped Go 재사용), 언어중립. irony guard zero: 신규 Go/config/lint/hook 0, review.md(live)+template mirror 2파일만 (§J). 8/8 ACs PASS (AC-SA-001..008). Template-First byte-parity; spec-lint clean; template neutrality ✓; moai spec audit drift 0. plan-auditor PASS 0.89 iter-1 (Tier S threshold 0.75, D1-D4 minor run 반영). D(LLM-as-judge benchmark)는 §J 제외 — 별도 보류. Run-phase 2026-06-23 (manager-develop, TDD cycle, 98d289f1f + a7a910bbe main push, synced 0 0). Sync-phase 2026-06-23 (orchestrator-direct, 3-phase close: 격리 worktree from origin/main, sync_commit_sha 551076897).

- **[SPEC-DIVECC-INVENTORY-VIEW-001](.moai/specs/SPEC-DIVECC-INVENTORY-VIEW-001/spec.md)** — Unified Agent/Harness Inventory View (`moai inventory`) (Tier M, v0.1.2). Candidate N6 (final) of Epic Dive-into-CC dogfooding series (arXiv:2604.14228), realizing paper open-direction 1 "runtime-is-first-class". Adds a new top-level read-only `moai inventory [--json]` command composing the three previously-independent inventory surfaces — sessions (`session.QueryActiveWork`), worktrees (`WorktreeProvider.List`), harnesses (`harness.ListHarnesses`) — into one count-summary view. New file `internal/cli/inventory.go` (~330 LOC) + `inventory_test.go` (26 tests) + 1-line `root.go` registration; ZERO modification to the three backing packages (REQ-INV-011/012 invariant — `git diff` proves `internal/session`, `internal/cli/worktree`, `internal/cli/harness`, `internal/core/git` untouched). Per-surface graceful degradation with an intentional asymmetry: sessions/harnesses degrade-to-empty (0-count) when their backing dir is absent, the worktree surface degrades-to-error (in-band `.error` field) outside a git repo; the command exits 0 when ≥1 surface renders. `--json` emits a structured `{sessions,worktrees,harnesses}` `UnifiedInventoryReport`; `ensureWorktreeProvider` wires the existing `deps.EnsureGit` → `worktree.WorktreeProvider` so the worktree surface composes at runtime (a CALL into existing wiring, not a backing-package change). 10 ACs PASS (AC-INV-001..010 incl 005a/b/c degradation paths, 006b zero-backing-mod proof, 007 subagent-boundary grep); inventory.go coverage 91.2% (≥85%); cross-platform build (darwin+windows) ✓; golangci-lint 0; spec-lint clean; era V3R6 drift 0. plan-auditor PASS 0.86 (D1-D4 remediation, Tier M baseline 0.80). sync-auditor PASS 0.946 (Functionality 0.97 / Security 1.00 / Craft 0.92 / Consistency 0.90; REQ-INV-008 prose aligned to shipped in-band `.error` per v0.1.2). Run-phase 2026-06-23 (manager-develop, TDD cycle, be6bbc238 + d55be28c3 main push, synced 0 0). Sync-phase 2026-06-23 (orchestrator-direct, 3-phase close: spec.md status completed + era V3R6, progress.md §E.4 sync_commit_sha backfill, CHANGELOG entry, specific-path-discipline 2-commit close). **Epic Dive-into-CC 7/7 완결.**

- **[SPEC-STEERING-ALIGN-GUARDRAIL-HOOK-001](.moai/specs/SPEC-STEERING-ALIGN-GUARDRAIL-HOOK-001/spec.md)** — GLM web-tooling 라우팅 env-deterministic 가드레일 훅 + always-load 제거 (Tier M, v0.1.0). Epic Steering-Align P3 (RULE-SCOPING-001·CLAUDEMD-DIET-001 후속): RULE-SCOPING이 후속 SPEC으로 미룬 Class-B 규칙 중 `glm-web-tooling.md`를 deterministic 훅으로 전환 (Anthropic steering 블로그 hook-layer 정렬 — 확률적 prose 대신 결정적 hook). 신규 `internal/hook/session_start_glm_guardrail.go`: `hookProcessEnvHasGLM` (PROCESS-env `ANTHROPIC_BASE_URL` `z.ai` 부분문자열 — `cg_detect.go` `hasGLMEnv`와 동일 신호 재사용, `config.DefaultGLMBaseURL` 상수) + `glmGuardrailReminder` (간결 라우팅 요약 — web_search_prime/web_reader/zai-mcp-server + ToolSearch preload)을 SessionStart 핸들러에 연결해 GLM 백엔드 세션에서만 `HookSpecificOutput.AdditionalContext`로 on-demand 주입. advisory only (도구 차단 없음). cg-leader carve-out 자동 보존: PROCESS-env만 읽으므로 leader pane(env stripped)에서 false; `sessionEnvHasGLM`(tmux SESSION-env)·`IsCGMode` 미사용으로 2중 탐지기 함정(plan-audit D1) 회피. `glm-web-tooling.md`(live+template)에 `paths: "**/glm-web-tooling.md"` self-glob 추가 → always-load 세트에서 제거 (비-GLM 세션 0 로드, GLM 세션만 간결 요약 = 토큰 예산 절감, RULE-SCOPING §D (b2) on-demand 잇기). always-load LIVE 11→10 / TEMPLATE 10→9. 17/17 ACs PASS (AC-GH-001..014; carve-out 009b/009c; mirror byte-parity 006; load-on-touch 005b; 신규 func cov 100%/100%, 패키지 baseline 82.7% 유지). Template-First byte-identical 양 트리. Cross-platform ✓; go test ./internal/hook/ ✓; golangci-lint 0; spec-lint 0; template neutrality ✓; moai spec audit drift 0. Plan-auditor PASS-WITH-DEBT 0.83 → PASS 0.89 iter-2 (D1 MAJOR cg-leader 탐지기 pinning + D2 RULE-SCOPING REQ-SARS-010 정합 + D3-D7 해소). Run-phase 2026-06-23 (manager-develop, TDD cycle, 17b7c5ba9 + 4f67246b2 main push, synced 0 0). Sync-phase 2026-06-23 (orchestrator-direct, 3-phase close: spec.md status completed + era V3R6, progress.md §E.4 sync_commit_sha backfill, CHANGELOG entry, specific-path-discipline 2-commit close).

- **[SPEC-SIMPLICITY-LADDER-001](.moai/specs/SPEC-SIMPLICITY-LADDER-001/spec.md)** — 단순성 결정 사다리(simplicity decision ladder) + @MX:DEBT 지연-단순화 태그 (Tier M, v0.1.0). DietrichGebert/ponytail v4.7.0 (MIT) "lazy senior dev" 기법 + 기존 moai-constitution.md Enforce Simplicity 원칙 통합. REQ-1: 6계층 dependency-avoidance 순서 사다리 (YAGNI → stdlib → native platform → installed dep → one-liner → minimum code) + TRUST 5 Secured 안전 장벽; REQ-2: @MX:DEBT deliberate-simplification ceiling + upgrade-trigger 메커니즘 (JSON ledger 미사용, sub-line-kinds recognized set {CEILING,UPGRADE,REASON,SPEC,TEST,PRIORITY}). 17/17 ACs PASS (AC-SL-001..012, 회귀 방지 009b/009c, mirror parity 011a/011b/011c). 4 doctrine 파일(moai-constitution.md + karpathy-quickref.md + mx-tag-protocol.md + moai-constitution @MX list) + 4 template mirror + internal/mx/ tag/scanner + internal/cli/mx_query + 6 Go test files (신규 cov 87.5%); irony guard zero (config/lint/hook/JSON 제로 per §J). Cross-platform build (darwin+windows) ✓; go test ./... PASS; spec-lint clean; template neutrality ✓; moai spec audit drift 0. Plan-auditor PASS-WITH-DEBT 0.86→0.88 iter-2/iter-3 (D1-D5 + D-NEW-1/D-NEW-2 해소). Run-phase 2026-06-22 (manager-develop, TDD cycle, 5de6f19ee + ee53faf6f main push, synced 0 0). Sync-phase 2026-06-23 (orchestrator-direct, 3-phase close: spec.md status completed, progress.md §E.4 sync_commit_sha backfill, CHANGELOG entry, specific-path-discipline 2-commit close).

### Changed

- **[SPEC-CC2186-BG-PERMISSION-RATIONALE-001](.moai/specs/SPEC-CC2186-BG-PERMISSION-RATIONALE-001/spec.md)** — Background subagent 권한 근거(rationale)를 Claude Code 2.1.186에 정합 (Tier M, v0.2.0). `/harness:release-update` 하네스가 CC 2.1.183→2.1.186 델타에서 발견한 단 하나의 genuine doctrine drift — background-subagent write 제한의 *근거*(결론 아님)가 stale. CC 2.1.186이 background subagent를 auto-deny 대신 main-session에 권한 프롬프트를 surface하도록 변경 → "auto-deny ... because they cannot interact with the user" 인과 절이 사실과 어긋남. 6개 surface(3 loci × live+template mirror)에서 근거 정정: CLAUDE.md §14 "Background Agent Write Restriction", agent-common-protocol § Background Agent Execution, zone-registry CONST-V3R2-020. [HARD] write 제한(`run_in_background: false` for write tasks; MUST NOT Write/Edit)은 **그대로 유지** — 근거 정정일 뿐 규칙 완화 아님; CONST-V3R2-044(순수 결론)는 scope discipline으로 미변경. Run-phase가 공식 sub-agents 문서(`code.claude.com/docs/en/sub-agents`)를 재확인한 결과 plan-phase의 "allowlist 비상속 survives" 가정이 **반박됨**(`bypassPermissions`/`acceptEdits`가 subagent로 전파) → allowlist 주장 DROP, 유지된 결론은 flow-interruption/conservative-policy 근거로 재정립. Sync-phase가 이 검증된 upstream 사실에 맞춰 REQ-BGR-004 + AC-BGR-004를 amend(v0.2.0)하여 run-phase disclosed deviation 해소 → 10/10 ACs PASS, deviation 0. Go 파일 0 변경(doc-prose + `//go:embed all:templates` 직접 임베드); template mirror byte-parity(AC-BGR-005 per-locus diff); TestTemplateNeutralityAudit + TestTemplateNoInternalContentLeak green; `go build`/`go test ./...` green; era V3R6 drift 0. Plan-auditor PASS-WITH-DEBT 0.84. Run-phase 2026-06-23 (manager-develop, run_commit_sha 73bb6916c). Sync-phase 2026-06-23 (orchestrator-direct, 3-phase close: 격리 worktree from origin/main, REQ/AC amend + spec.md status completed + era V3R6, progress.md §E.4 sync_commit_sha backfill, specific-path-discipline 2-commit close).

- **[SPEC-SKILL-BODY-NEUTRALITY-001](.moai/specs/SPEC-SKILL-BODY-NEUTRALITY-001/spec.md)** — Purge moai-adk local-development traces from deployed skill bodies + extend the neutrality CI guard (Tier M, v0.1.1). Deployed skill bodies under `.claude/skills/` carried internal-development artifacts meaningless in end-user projects — internal SPEC/REQ tokens (the V3R2..V3R5 SPEC-ID families, REQ-BRAIN/LB/WF/ROUTE/...), internal Go-impl paths (`internal/{spec,cli,hook,ciwatch}/*.go`), CI-test references (`agentless_audit_test.go`), maintainer-doctrine notes, the docs-site "4-locale" maintainer annotation, and dev-only self-references (`97-release-update`/`99-release`). **Part A** purges all 4 leak classes via meaning-preserving generic-ization — sentinel keyword VALUES (e.g. `MODE_UNKNOWN`) retained, only the surrounding prose that named the internal test file rewritten; keep-list preserved (SPEC-AUTH-001 placeholders ×14, `golangci-lint` multi-language lists, `go install .../moai-adk/cmd/moai`, Vercel domain knowledge, the `adk.mo.ai.kr` URL ×3 with only its "4-locale" annotation dropped). **Part B** closes the structural guard gap: the existing `internal_content_leak_test.go` C1 SPEC-ID class matched only `SPEC-(V3R6|AGENCY|WORKTREE)-` and missed the V3R2..V3R5 families that dominate skill-body leaks (the exact reason the baseline guard passed despite 37 leaking files) — extended with 4 skill-body-scoped leak classes (broadened SPEC-ID, promoted single REQ-token class, package-restricted C7 Go-path, agentless-ref) + a pedagogical allowlist, RED(154 findings)→GREEN(0). The self-reference paradox was resolved by keeping the sentinel VALUE (asserted via `strings.Contains`, not by naming the test file). 20/20 ACs PASS (AC-SBN-001..020; AC-SBN-002/016 MUST-PASS). Template-First byte-identical dual-tree (50 files); make build exit 0; go test ./internal/template/ ✓; cross-platform build (host+windows) ✓; foundation-cc/core mirror diff confirmed pre-existing drift (out of scope). plan-auditor PASS-WITH-DEBT 0.84 (Tier M threshold 0.80; D1-D5 remediation pre-run). An 18-day/640-commit drift discarded the original stale run-phase; re-run fresh on current main with re-grepped leak inventory. Run-phase 2026-06-23 (manager-develop, TDD cycle, run_commit_sha bc350cf04, fresh worktree re-run + cherry-pick onto main). Sync-phase 2026-06-23 (orchestrator-direct, 3-phase close).

- **[SPEC-STEERING-ALIGN-OUTPUT-STYLE-SLIM-001](.moai/specs/SPEC-STEERING-ALIGN-OUTPUT-STYLE-SLIM-001/spec.md)** — always-loaded output-style `moai.md` body diet (§8 Session-Handoff pointer-ization + Localization-Contract 중복 prose 삭제, MODERATE bound, render-SSOT 보존) (Tier M, v0.1.0). Epic Steering-Align P5 (CLAUDEMD-DIET-001 자매 기법 계승): 항상 로드되는 output-style `moai.md` context-budget 절감 (782→756 lines, 55306→51662 bytes per tree, −26L/−3644B, M-DELETE + M-POINTER 귀속). §8 Response Templates(전체 66%)가 주 대상 — §8 Session Handoff(render-only 자기선언 블록)를 `session-handoff.md` SSOT pointer로 축약 + Localization Contract/Epic-taxonomy 중복 설명 prose 삭제. **render-SSOT 전량 보존** (P2 AC-CMD-009 over-cut 게이트 계승, here AC-OSS-006): 14 banner 스켈레톤 + 8 en/ko/ja/zh 번역테이블 + ko-canonical 매핑테이블 + §8 parity sentinel + §9/§10 directives + verbatim-preserve 심볼 + `ultrathink.` 토큰 0 삭제. Tier M 4-artifact (spec/plan/acceptance/progress). 9/9 MUST-blocking ACs PASS (AC-OSS-001..009): line [530,630] soft band → **behavioral-PASS 756** (§8 84L 중 ~60L이 render-SSOT라 정직 보존, over-cut 회피 — AP-OSS-003), byte-parity dual-tree, 14 banner 보존, col-header table==8, byte reduction 귀속, POINTER prose-duplication re-verification gate, neutrality CI-guard(TestTemplateNeutralityAudit) + output-styles parity(TestOutputStylesExactlyTwo/TemplateLiveParity), §9/§10+토큰 보존, 삭제 prose별 외부-SSOT home 증거. plan-vs-run drift 1건 정직 보고 (§8 Epic-taxonomy POINTER→KEEP, 분리 가능 중복 prose 부재). Template-first byte-identical dual-tree. Cross-platform N/A(markdown-only); go test ./internal/template/ ✓; spec-lint 0; moai spec audit drift 0. Plan-auditor FAIL 0.79 → PASS 0.87 iter-2 (D1 REQ-OSS-004 gap 문서화 + D2 AC-OSS-004 col-header 술어 + D3 locale 순서 + D4 behavioral-PASS escape 강화). Run-phase 2026-06-23 (manager-develop, TDD/AC-gated cycle, run_commit_sha b500f5209, L1 worktree→main FF reconcile + rebase over parallel SPEC-SIMPLICITY-AUDIT-001). Sync-phase 2026-06-23 (orchestrator-direct, 3-phase close: spec.md status completed + era V3R6, progress.md §E.4 sync_commit_sha backfill, CHANGELOG entry, specific-path-discipline 2-commit close).

- **[SPEC-STEERING-ALIGN-CLAUDEMD-DIET-001](.moai/specs/SPEC-STEERING-ALIGN-CLAUDEMD-DIET-001/spec.md)** — CLAUDE.md always-loaded body diet (per-line-test deletion + rule-SSOT pointer-ization + changelog-footer removal + @import token-neutrality honesty): Reduce CLAUDE.md context-budget bloat (650→409 lines, 35778→28066 bytes per tree, −37% reduction attributed to M-DELETE changelog footer + M-POINTER rule cross-refs; @import structure-only). Tier M: 4-artifact set (spec.md + plan.md + acceptance.md + progress.md). 10 ACs PASS (9 MUST-blocking + 1 SHOULD): per-tree line-drop [400,470], byte-parity dual-tree, [HARD]-count ≥14, @import-honesty, changelog-footer removal, byte-reduction attribution, neutrality CI-guard (TestTemplateNeutralityAudit), POINTER prose-duplication re-verification gate (AC-CMD-009), untagged-behavioral §4 anchors survive (8 retained-agents + archived-agent list + decision tree). plan-auditor PASS-WITH-DEBT 0.83 → PASS 0.86+ (iter-2: 5 demoted POINTER→KEEP; AC-CMD-008 SHOULD). Template-first + byte-identical dual-tree. Cross-platform ✓; go test ./... ✓; moai spec lint 0 errors. run-phase: orchestrator-direct TDD (2026-06-22, 8392c9d6c + c9ea65e72). sync-phase: orchestrator-direct 3-phase close.

### Added

- **[SPEC-WEB-CONSOLE-010](.moai/specs/SPEC-WEB-CONSOLE-010/spec.md)** — Web Console & moai profile setup 설정 표면을 공유 필드-스키마 SSOT(`internal/settings`)로 통일 (Tier L, v0.1.0). `moai web` 브라우저 콘솔과 `moai profile setup` TUI 는 각자 필드 정의, 옵션 목록, 빈값 라벨, 검증 규칙을 하드코딩하고 있어서 `internal/web/validate.go`에서 마법사의 unexported 옵션 목록을 재선언하는 상황 발생. 해결책: 신규 중립 패키지 `internal/settings` 에 정규 필드-스키마 정의 + 옵션 목록 1회 보유 후 양 표면이 파생. 6개 섹션 (Identity, Language, Launch, Statusline, Quality, Git Convention) / 34개 필드 통일. TUI는 스키마에서 하우 Select/MultiSelect 위젯 생성, web은 templ fieldset + validate.go 재선언 제거로 소싱. 웹 statusline 재추가 (theme + 15 segments, NO preset 은퇴 정책 준수). TUI의 7개 nested project-config 필드 (`quality.*`, `git_convention.*`) 추가 + 공유 nested write seam 활용. 라벨 정규화 + i18n key 통일 + `permission_mode acceptEdits` 정규화 의미론 보존. 21 ACs PASS (AC-WC10-001..021; coverage internal/settings 96.1%, internal/cli/web flat). 3개 파일 신규 (internal/settings/ Go package + test), 관련 CLI/web/profile 파일 수정. Cross-platform build (darwin+windows) ✓; lint 0 NEW; spec-lint clean; template neutrality (out-of-scope docs-site). Plan-auditor pending (Phase 0.5 audit gate). Run-phase 2026-06-23 (manager-develop, TDD cycle, run_commit_sha 16329f3b1). Sync-phase 2026-06-23 (3-phase close).

- **[SPEC-DIVECC-OBSERVABILITY-LOOP-001](.moai/specs/SPEC-DIVECC-OBSERVABILITY-LOOP-001/spec.md)** — Failure-Signature Clustering Engine, the observation arm of the harness-learning loop (Tier M, v0.1.0). Candidate N4 of Epic Dive-into-CC dogfooding series (arXiv:2604.14228). Fills the single MISSING step (`cluster`) of the harness-learning loop `trace → eval → cluster → policy → repair` — moai-adk previously did failure clustering only manually via the `feedback_*.md` lessons. Delivers a NEW read-only clustering engine `internal/harness/cluster/` that ingests `apply_outcome` events from `.moai/harness/usage-log.jsonl` and groups them by a **deterministic** signature key (sorted `outcome_regressed` dimension set + `outcome_verdict` + `outcome_decision`; NO machine learning, NO randomness; NOT `pattern_key`, which is absent from the event and one-way hashed into `outcome_proposal_id`) into `FailureCluster`s carrying member count, representative regressed dimensions, and first/last-seen timestamps. Adds a `moai harness clusters` CLI read surface (registered in the LIVE `newHarnessRouterCmd` tree alongside `status`; `--json` machine output; reuses the shared `resolveProjectRoot` helper, no new path-resolution path) and a deterministic report artifact under `.moai/harness/learning-history/`. Strictly read-only w.r.t. the proposal/apply path: ZERO diff to `applier.go` / `proposalgen/` / the safety L1-L5 pipeline / `regression_gate.go` / the `autoApply` default (false) / `evolution/learning.go`; the 56 manual `feedback_*.md` lessons are preserved unchanged (N4 augments observability, does not migrate the human-curated store). Fail-open ingestion (malformed line skipped; optional `manifest.jsonl` absent tolerated; empty/absent log → 0 clusters + success); `kept` outcomes excluded (only failure/`rolled-back` signatures cluster). 10 ACs PASS (AC-OBL-001..010): deterministic byte-identical repeated-run output, read-only boundary, subagent-boundary grep clean, cross-platform build (host + `GOOS=windows`) green, new-package coverage 90.8% (≥85%), golangci-lint 0 issues. plan-auditor PASS 0.91 (iter-2, Tier M baseline 0.80). Tier M 3-artifact set (spec.md + plan.md + acceptance.md) + progress.md. run-phase 2026-06-22 (orchestrator-direct fallback — Agent tool unavailable in session context; TDD discipline; run_commit_sha 3e9ecc3d2; independently re-verified by the orchestrator after a multi-session orphan/race reconciliation). sync-phase 2026-06-22 (orchestrator-direct, 3-phase close).
...
- **[SPEC-V3R6-CG-MODE-HARDENING-001](.moai/specs/SPEC-V3R6-CG-MODE-HARDENING-001/spec.md)** — moai cg (Claude+GLM) launch-mode hardening and detector SSOT (Tier M, v0.1.0). Hardens the cost-optimization hybrid session launcher (`moai cg` — Claude leader pane + GLM teammate panes) against three clusters of defects: (1) platform-specific launch path using unguarded `syscall.Exec` (Windows EWINDOWS hazard); (2) non-atomic multi-step settings mutation that can leave stale GLM credentials or drop `teammateMode` entirely; (3) CG-mode detector querying the PROCESS environment (cleaned on leader) instead of the deterministic `llm.yaml team_mode` SSOT, causing silent route-to-Claude defeats cost optimization. Fixes via: launch-path guard files per-platform (new `launch_exec_posix.go` + `launch_exec_windows.go`), atomic `settings.local.json` RMW with file-lock + single-write guarantee (new `settings.go` flock pattern), detector redesign via layered-OR with `sessionEnvReaderFn` seam, GLM `base_url` allowlist validation. 7 files production modified (launcher.go / glm.go / cg_detect.go / validation.go / CLAUDE.local.md), 4 new test files, 10 ACs PASS. Cross-platform build (darwin+windows) ✓; go test ./... all PASS; spec-lint clean; drift 0. Plan-auditor PASS 0.88 (Tier M baseline 0.80). Run-phase 2026-06-22 (manager-develop, TDD cycle, code absorbed into run_commit_sha 23901497a by parallel SPEC-V3R6-DEV-HARNESS-SPLIT-001 orphan-race; byte-identical verification). Sync-phase 2026-06-22 (manager-docs, 3-phase close).

## [v3.0.0-rc2] — 2026-06-19

### Added

- **[SPEC-V3R6-BASH-RISK-GOVERNANCE-001](.moai/specs/SPEC-V3R6-BASH-RISK-GOVERNANCE-001/spec.md)** — Bash Risk-Amplifier Doctrine: subcommand soft-cap + warn-only hook signal (Tier S, v0.1.0). Source: book1 ch04 + appendix A.3 litmus (github.com/wquguru/harness-books). Doctrine classifies Bash as risk-tier "write/irreversible" (distinct from Read/Glob/Grep "read"), introduces BASH_SUBCOMMAND_SOFT_CAP=5 soft-cap constant, enumerates destructive-primitive set (rm -rf, git push --force, git reset --hard, SQL DROP/TRUNCATE, chmod -R 777), and wires warn-only fail-open hook signal into handle-pre-tool.sh (lines 18-41). The signal detects compound-command subcommand count via heuristic shell-metacharacter counter (||, &&, |, ;), emits [moai:bash-risk] WARN to both stderr + $MOAI_HOOK_STDERR_LOG when count > 5, and exits 0 (fail-open, never blocks). 2 files modified: coding-standards.md (§Bash Risk-Amplifier Doctrine, lines 101-132) + handle-pre-tool.sh (warn-only signal, lines 18-41). 6/6 ACs PASS (AC-BRG-001 through AC-BRG-006: risk-tier classification + constant definition + destructive-primitive set + fail-open signal + CLAUDE.md cross-refs + gap closure grep). Cross-platform build ✓; spec-lint clean (no Go changes); era V3R6 drift 0. Plan-auditor PASS 0.85 (Tier S baseline 0.75). Run-phase 2026-06-18 (manager-develop, combined M1+M2 single commit b40721149). Sync-phase 2026-06-20 (orchestrator-direct, 3-phase close completed).

### Fixed

- **[SPEC-V3R6-CLI-CONFIG-INTEGRITY-001](.moai/specs/SPEC-V3R6-CLI-CONFIG-INTEGRITY-001/spec.md)** — CLI config-system integrity / mental-model alignment (Tier M, v0.1.0). 4 P0 defects: (F1) `moai update -c` help/doc clarity in `update.go`; (F2) model alias literal scatter → centralized `ModelAliasTable` SSOT (new `internal/template/model_policy.go`) per REQ-CCI-003, eliminating duplicates across `launcher.go`, `profile_setup.go`, `settings/schema.go`, `launcher_test.go`; (F3) `acceptEdits` normalization confirmation annotations in `profile_setup.go`, `launcher.go`; (P0-4) db.yaml `_TBD_` placeholders removal + `moai update` propagation verification per REQ-CCI-010. 10/10 ACs PASS (9 MUST + 1 SHOULD; AC-CCI-001..010). Run-phase evidence: M1 help/doc + M2 model alias SSOT + M3 acceptEdits confirmation + M4 db.yaml placeholder removal with propagation cross-check (`go test ./internal/template/... ./internal/cli/...` green, manual config-integrity spot-checks). Cross-platform build (darwin+windows) ✓; go vet clean; golangci-lint 0 issues; full internal/cli test suite PASS. Template-First doc-only sync (no code changes) — all implementation in run-phase. spec-lint clean; moai spec audit drift 0. plan-auditor PASS 0.83 (Tier M baseline 0.80). Run-phase 2026-06-24 (manager-develop, TDD cycle, run_commit_sha 3845373a3, implementation M1–M4 complete on main synced 0 0). Sync-phase 2026-06-24 (manager-docs, 3-phase close: spec.md `in-progress → completed` + plan.md `draft → completed` + acceptance.md `draft → completed` + era V3R6 added to all 3 artifacts + progress.md §E.4 sync_commit_sha backfill + CHANGELOG entry).

- **[SPEC-STATUSLINE-WINDOWS-FALLBACK-001](.moai/specs/SPEC-STATUSLINE-WINDOWS-FALLBACK-001/spec.md)** — Windows statusline phantom-path fallback repair (Tier M, v0.2.0, run-phase 2026-06-20, sync-phase 2026-06-21). Issue modu-ai/moai-adk#1068: The `moai init` / `moai update` generated `.moai/status_line.sh` wrapper whose Go-bin fallback baked a non-existent `C:\Users\{user}\go\bin\moai` path on Windows installer machines, causing the Claude Code statusline to render empty. Fix: (1) Add `TemplateContext.ResolvedMoaiPath` field populated via `os.Executable()` at init/update time; (2) Emit a guarded resolved-executable branch (`[ -f "..." ] && exec ...`) in the generated script, ordered after PATH lookup but before the `$HOME` fallbacks; (3) Omit the resolved-executable branch when the field is empty (graceful degradation). Reproduction test: `TestStatusLineEveryExecGuarded` asserts that every `exec moai statusline` line is guarded or PATH-resolved (preventing phantom executions of non-existent paths). 13 ACs (12 PASS + 1 PASS-WITH-DEBT for pre-existing unrelated test failure in internal/skills). Cross-platform build (darwin+windows) ✓; spec-lint clean; 7 files modified (internal/template: context.go, status_line.sh.tmpl, new status_line_fallback_test.go, hardcoded_path_audit_test.go; internal/core/project: initializer.go; internal/cli: update.go, update_clean_install.go). plan-auditor: PASS-WITH-DEBT 0.84 (iter-1, Tier M threshold 0.80); 4 doc defects (D1 env.go factual / D2 inert-test 묘사 + AC-SWF-013 render-and-assert 강화 / D3 REQ-SWF-006 label / D4 REQ 순서) fixed at 7b3b8cf7c. run-phase: manager-develop (TDD cycle, c4a42bc11). sync-phase: orchestrator-direct 3-phase close (manager-docs spawn context-limited).

### Changed

- **[SPEC-V3R6-WORKFLOW-AGENT-DOC-ALIGN-001](.moai/specs/SPEC-V3R6-WORKFLOW-AGENT-DOC-ALIGN-001/spec.md)** — Workflow + Agent + Doc Consistency Cleanup (archived-agent purge, broken cross-refs, stale ground-truth): Complete purge of 11 archived-agent spawn-target references (125 occurrences across 36 files, both live + template mirrors), valid cross-reference restoration (`team-reader`/`team-validator` invalid subagent_type → `general-purpose`; `§ Terminology Glossary` section ADD to worktree-integration.md), stale ground-truth content sync (GLM models `glm-5.1`→`glm-5.2[1m]`, retired terms `Round split`→`Milestone split` / `sprint contract`→`GAN-loop Sprint Contract Protocol`, SKILL.md version 2.6.0→2.7.0), and run.md LOC trim (246→181, below 200 ceiling) while preserving entry-router routing logic. 18 ACs PASS (AC-WADA-001 through AC-WADA-017 + 001a researcher KEEP baseline validation). Tier L: 5-artifact set (spec.md + plan.md + acceptance.md + design.md + research.md + progress.md), plan-phase 2026-06-20, run-phase 2026-06-20 (M1-M6: archive occurrence inventory, workflow skill files purge, team files + agent bodies + frontmatter purge, broken cross-refs + invalid dispatch, stale ground-truth updates, run.md LOC trim + verification + template parity), sync-phase 2026-06-20 (3-phase close completed). Template mirror parity (changed-line) PASS; template neutrality (TestTemplateNeutralityAudit) PASS; cross-platform build (darwin + windows) ✓; `go test ./...` exit 0; archived-proper residual both trees = 0; researcher role_profile KEEP count = 14 (no over-purge, AC-WADA-001a GREEN).

- **[SPEC-V3R6-HARNESS-V4-001](.moai/specs/SPEC-V3R6-HARNESS-V4-001/spec.md)** — Harness v4 Rebuild: orchestrator-direct Builder + manifest-driven Runner + conditional worktree isolation (Tier L, v0.3.0). Complete harness engineering system rebuild replacing the revfactory 7-Phase legacy with orchestrator-direct 4-phase Builder (ANALYZE → PLAN → GENERATE → ACTIVATE), manifest.json-driven Runner with dynamic-workflow specialist dispatch, `/harness:<name>` lifecycle command generation, conditional worktree isolation at sub-agent granularity, 4-specialist v4 migration (cli-template / quality / workflow / hook-ci), legacy 7-Phase redirect (moai-meta-harness DEPRECATED), and dogfooding validation (moai-adk-dev harness). 26 ACs PASS (24 MUST-FIX AC-HV4-001a/b ~ AC-HV4-013a/b + 2 SHOULD-FIX AC-HV4-004b/006b). Files modified: internal/harness/v4manifest/ (8 new Go files, 995 LOC, 100% coverage), internal/cli/harness/ (lifecycle + v4lifecycle), internal/cli/update.go (namespace protection), internal/template/templates/.claude/skills/moai/ (SKILL.md harness section + harness-build-entry.md + harness-builder.md 23KB), .claude/commands/moai/harness.md (argument-hint), .moai/docs/harness-namespace-doctrine.md (§24.4 contract matrix extension). Cross-platform build (darwin+windows) ✓; lint 0 NEW; spec-lint clean; template neutrality ✓; `moai spec audit` drift 0. plan-phase 2026-06-19, run-phase 2026-06-20 (M1-M6: namespace protection, NL-entry, Builder, manifest, commands/lifecycle, worktree-isolation, 4-specialist migration, legacy redirect, dogfooding), sync-phase 2026-06-20 (3-phase close completed). Dogfooding validation: moai-adk-dev harness built + runs real task (TestDogfood e2e PASS); full-live orchestrator build gap disclosed (author-measured, AC-HV4-012b).

- **[SPEC-V3R6-ORCH-IGGDA-001](.moai/specs/SPEC-V3R6-ORCH-IGGDA-001/spec.md)** — Intent-Gated Goal-Directed Autonomy (IGGDA): 4-phase plan→run→sync pipeline with safe-condition Implementation Kickoff Amendment, bounded recursive self-diagnosis loop, and moai-aware Stop hook goal driver (Tier L, v0.1.0). Amends the FROZEN Implementation Kickoff Approval gate with a 4-condition safe-condition predicate that auto-proceeds after Socratic intent collection in safe domains (Tier S/M, no dangerous keywords, no destructive scope) while preserving explicit-gate in dangerous domains. Path B amendment to SPEC-AUTONOMY-RUN-GOAL-001 run-phase autonomy, extending it across all 4 phases (Intent → Plan Audit → Run + Recursive Diagnosis → Sync + Independent Audit). 18 files modified (6 rules + 6 template mirrors + 2 skills + 2 Go source + 2 hooks/template sources); plan.md 6 Milestones (M1–M6: FROZEN-amend safe-condition predicate, bounded recursive loop, Stop hook driver, independent-audit preservation guard, --filter-spec flag for moai spec audit, lifecycle close). 42 ACs PASS (41 MUST + 1 SHOULD). Cross-platform build (darwin+windows) ✓; lint 0 NEW; spec-lint clean; template neutrality ✓; `moai spec audit` drift 0. plan-phase 2026-06-19, run-phase 2026-06-19 (M1-M6 merged to main, dce006106 final commit), sync-phase 2026-06-20 (3-phase close completed). Design.md §F FROZEN-invariant preservation analysis verified; safe-condition bounded + reversible.
...
- **[SPEC-V3R6-RULES-HOTFIX-001](.moai/specs/SPEC-V3R6-RULES-HOTFIX-001/spec.md)** — Low-risk `.claude/rules/` hotfix sweep: inverted tool names, inverted-logic clause, cpp flag typo, broken cross-references, path errors (Tier S, v0.2.0, run+sync 2026-06-19). Sprint 16 "rules-improvement" cohort. 6 verified defect groups (D1-D6) fixed across 14 files (7 deployed + 7 template mirror): zai vision-tool name inversion in `glm-web-tooling.md`, cpp `-fsanitize=address` typo, broken `moai-reference-anti-patterns` / `modules/trigger-handoff.md` cross-refs, worktree/git-workflow path errors. Token-only corrections applied identically to both copies (22 insertions / 22 deletions); mirror parity via `TestRuleTemplateMirrorDrift`; neutrality via `TestTemplateNeutralityAudit`. 9 ACs (AC-RHF-001..008 PASS + AC-RHF-009 PASS-WITH-DEBT for pre-existing out-of-scope output-styles drift). `go vet` + `go build` exit 0. sync-phase orchestrator-direct (GLM per `feedback_glm_orchestrator_direct_sync_mx`).

- **[SPEC-V3R6-RULES-CONST-RULEID-001](.moai/specs/SPEC-V3R6-RULES-CONST-RULEID-001/spec.md)** — Broaden constitution `ruleIDPattern` to accept the V3R6 namespace (Tier S, run+sync 2026-06-19). Sprint 16 "rules-improvement" cohort. Root-cause fix for `moai constitution validate` baseline-RED: 1-line regex constant in `internal/constitution/rule.go` `^CONST-V3R[25]-\d{3,}$` → `^CONST-V3R[256]-\d{3,}$` (admits `CONST-V3R6-NNN` alongside `V3R2`/`V3R5`); previously the deployed registry's `CONST-V3R6-001` entry failed ID validation and aborted `LoadRegistry` before the drift loop ran. 6 ACs PASS (AC-RCR-001..006); coverage 41.3% (1-line fix scope). `go test ./internal/constitution/...` green; GOOS-irrelevant (regex constant). sync-phase orchestrator-direct (GLM per `feedback_glm_orchestrator_direct_sync_mx`).

- **[SPEC-V3R6-RULES-SSOT-DEDUP-001](.moai/specs/SPEC-V3R6-RULES-SSOT-DEDUP-001/spec.md)** — Rules SSOT de-duplication + structural consolidation across `.claude/rules/moai/` (Tier L, v0.1.0, run-phase 2026-06-19, sync-phase 2026-06-19). Sprint 16 "rules-improvement" cohort SPEC 5/5. 9 de-dup target groups across deployed + template mirror trees: AskUserQuestion 4-way pointer reduction (M1), hooks timeout+alwaysLoad contradiction reconcile (M2, D7 both surfaces), context-window threshold (M3 verified no-op), verification-batch re-sync sentinel (M4, D4 no-op), ownership-matrix pointer (M5), team-files 4→2 + **DELETE `agent-teams-pattern.md`** + 4 referrer rewrites (M6, D8), skill 3-file boundary (M7), agent 3-file (M8 no-op), zone-registry `paths:` narrowing (M_Z, REQ-SSD-009 SHOULD partial — full clause reduction deferred to SPEC-V3R6-CONST-VALIDATE-LABEL-001 per B4 validator.go substring gate). 4 mirror classes fixed (design §4): byte-parity allowlist (hooks-system ONLY), byte-identical-by-discipline (13 files, explicit `diff -q`), §25-sanitized (3 files, leak-test), §25-divergent (zone-registry, scoped `diff`). 29 files, 172 insertions / 564 deletions (net de-dup). 15 ACs PASS (AC-SSD-009 PASS-WITH-BLOCKER-NOTE). Go-test trio green; constitution drift 77→77 (0 NEW); build darwin+windows exit 0. plan-auditor iter-2 PASS-WITH-DEBT 0.84 (D1-D8 independently re-confirmed + fixed). run-phase manager-develop worktree isolation → orchestrator cherry-pick (Sprint 16 multi-session race-mitigation pattern per `feedback_shared_main_orphan_race`). sync-phase orchestrator-direct (GLM per `feedback_glm_orchestrator_direct_sync_mx`).

- **[SPEC-V3R6-LIFECYCLE-REDESIGN-001](.moai/specs/SPEC-V3R6-LIFECYCLE-REDESIGN-001/spec.md)** — MoAI lifecycle redesign: restore the canonical **3-phase lifecycle (plan→run→sync)** and adopt **Epic-based naming** (Tier L, plan+run+sync 2026-06-19). Axis A (lifecycle restoration): `internal/spec/era.go` `ClassifyEra` H-4 reclassified to require `§E.2 + §E.4 + sync_commit_sha` (dropping `§E.5 + mx_commit_sha`); the three §E.5-keyed drift findings (`Y_N_N_Y`/`Y_Y_N_Y`/`Y_Y_Y_Y_StatusDrift`) retired and replaced by the single `SyncStatusDrift` dimension (`internal/spec/audit.go`); a one-time `internal/spec/migrate_3phase.go` backfill folded `§E.5` into `§E.4` for 65 modern-era SPECs; the `implemented → completed` transition merged into the **sync commit** (no separate Mx chore commit — REQ-LR-008); 6 drift-surface rule files + 8 agent/hook/output-style/skill surface files swept to "3-phase close". Axis B (Epic naming): `sprint-round-naming.md` SSOT rewritten to the **Epic / SPEC / Milestone / Constitution** taxonomy (Sprint/cohort/Round/Wave → Legacy Aliases) with T1-T4 surface migration (0 taxonomy residual). `closeInfixMatch` (`internal/spec/transitions.go`) retains BOTH `"3-phase close"` and legacy `"4-phase close"` (D4 reconciliation with SPEC-V3R6-DRIFT-LEGACY-CONVENTION-001). progress.md `§E.5` retired; §E structure is now 4 sections. 13 ACs (10 MUST-PASS + 3 SHOULD-PASS) verified; Mx-phase concept removed.

- **[SPEC-V3R6-OUTOFSCOPE-GUIDANCE-ALIGN-001](.moai/specs/SPEC-V3R6-OUTOFSCOPE-GUIDANCE-ALIGN-001/spec.md)** — Align SPEC-authoring guidance to the `OutOfScopeRule` spec-lint convention (Tier S, run+sync 2026-06-19). The `manager-spec` agent guidance (line 84), `plan-auditor` SC-6, and the `moai-workflow-spec` skill instructed a bare `## Exclusions` H2 heading, which the enforced `OutOfScopeRule` (`MissingExclusions`) rejects — it requires a `### Out of Scope — <topic>` H3 sub-heading with `-` bullets. This split SSOT caused recurring per-SPEC `MissingExclusions` errors patched at the symptom (e.g. ORCH-INTERRUPT-LEDGER CON-5, HARNESS-RUNTIME-RECOVERY D1). The fix aligns all three guidance surfaces + their template mirrors to the H3 convention (Go change 0 for the core fix; one CI re-drift guard test added under `internal/template/`). `internal/spec/lint.go` `OutOfScopeRule` + `internal/spec/CLAUDE.md` remain the unchanged SSOT.
- **[SPEC-V3R6-STATUSLINE-MILLION-BUDGET-001](.moai/specs/SPEC-V3R6-STATUSLINE-MILLION-BUDGET-001/spec.md)** — Isolate `TestCollectMemory_AutoCompactScaling` from ambient GLM model-env contamination (Tier S, v0.2.0, run+sync 2026-06-18). On GLM-ambient dev machines (`moai glm`/`moai cg`), `ANTHROPIC_DEFAULT_*_MODEL` env vars caused `resolveContextWindowOverride()` to return `1_000_000`, overriding the test's `ContextWindowSize: 200000` input and producing `TokensUsed = 850000, want 170000` failures. Fix = Direction B (test isolation): `isolateModelEnv(t)` helper neutralizes the 3 model env vars via `t.Setenv("","")` so the resolver returns 0 on priority-3 lookup and the 200K-derived expectations hold invariant to the machine's model-env state. Production `internal/statusline/memory.go` unchanged (Issue #653 GLM override contract preserved). Retires AC-HNS-011 PASS-WITH-DEBT of SPEC-V3R6-HARNESS-NAMESPACE-V2-001 (that entry's forward-gap was referenced as `STATUSLINE-1M-BUDGET-001`; this is the realizing SPEC under its actual ID `STATUSLINE-MILLION-BUDGET-001`). **Out-of-scope gap**: baseline `TestCollectMemory` (no suffix) shares the same contamination (3 subtests pre-existing FAIL under GLM-ambient, reproduced on clean main `ce3f23dc3`) — forward-gap to SPEC-V3R6-STATUSLINE-BASELINE-ISOLATION-001. 5/5 in-scope ACs PASS; coverage 86.9%; `go vet` + `golangci-lint` clean.

- **[SPEC-V3R6-HARNESS-NAMESPACE-V2-001](.moai/specs/SPEC-V3R6-HARNESS-NAMESPACE-V2-001/spec.md)** — Harness namespace doctrine-code drift catch-up: align Go enforcement + test fixtures + generator emission prefix + CI sentinel to the `harness-*` user-owned namespace the doctrine already declares (Tier M, v0.1.0, run-phase 2026-06-18, sync-phase 2026-06-18). The doctrine (`.moai/docs/harness-namespace-doctrine.md` §24.1) declared `harness-*` as user-owned on 2026-05-26, but the Go enforcement layer still recognized `my-harness-*`; this SPEC is the Phase 2 catch-up (no policy reversal — `moai-harness-*` stays template-managed). 5 surfaces migrated atomically in one commit group: (1) Go enforcement `isUserAreaPath`/`isUserOwnedNamespace` now recognize `harness-*` canonical + `my-harness-*` legacy dual-recognition (deprecation window, REQ-HNS-005); (2) test fixtures renamed `my-harness-*`→`harness-*` (28 files); (3) `moai-meta-harness` generator emission prefix → `harness-*`; (4) CI sentinel renamed `TestNamespaceLeakMyHarnessSkills`→`TestNamespaceLeakHarnessSkills` (`harness-*` leak detection); (5) backward-compat recognition for legacy `my-harness-*` user skill dirs. `harness-*` vs `moai-harness-*` substring separation via exact `strings.HasPrefix` (REQ-HNS-004). **12 ACs (10 MUST-PASS + 2 SHOULD-PASS)**: AC-HNS-001..010, 012 PASS; AC-HNS-011 (full test suite green) PASS-WITH-DEBT — `internal/statusline/memory_test.go` 200K→1M budget expectation failure is pre-existing (bef24877d baseline, out of SPEC scope, forward-gap to SPEC-V3R6-STATUSLINE-1M-BUDGET-001); AC-HNS-008/009 SHOULD-PASS. Verification: `go build ./...` exit 0; namespace-scoped packages (`internal/cli`, `internal/harness/*`, `internal/design/pipeline`, `internal/template`) all PASS; golangci-lint 0 issues; spec-lint clean. doctrine §24.5 drift note → RESOLVED; skill-authoring.md stale caveat synchronized (template + local mirror parity). plan-auditor PASS-WITH-DEBT 0.82.

### Removed

- **[SPEC-V3R6-STATUSLINE-PRESET-RETIRE-001](.moai/specs/SPEC-V3R6-STATUSLINE-PRESET-RETIRE-001/spec.md)** — Retire the statusline preset shorthand (`full`/`compact`/`minimal`/`custom`) and the residual mode-preferences axis; remove the entire web-console statusline panel (Tier M, v0.2.0, run-phase 2026-06-17, sync-phase 2026-06-17). Three removal axes: (1) **preset axis (D1)** — `PresetToSegments` + lowercase wrapper deleted, `StatuslineConfig.Preset` / `cli.statuslineFileConfig.Preset` removed, profile-sync preset-default/expansion branch deleted, wizard preset `Select` + helpers + i18n keys removed, web `statuslinePresetCanonical` + validation removed. (2) **mode axis (D3)** — `ProfilePreferences.StatuslineMode` removed, wizard mode `Select` + canonical/helpers + migration notice + summary row + i18n keys removed (`internal/statusline` Builder API PRESERVED). (3) **web-console panel (D2)** — `fieldsetStatusline` component + caller removed, handler statusline form bindings + view-model fields removed, 2 obsolete web tests deleted. Template `statusline.yaml` `preset:` key removed; docs-site 4-locale `advanced/statusline.md` `preset:`+`mode:` lines removed (D7, AC-SPR-018). Legacy `preset:` key silently ignored by loader (REQ-SPR-021, AC-SPR-020 characterization test). **28 ACs (25 MUST PASS + 3 SHOULD)**: AC-SPR-022 N/A (Display section restructured), AC-SPR-023 DEFERRED (`preset.go`→`segments.go` rename kept to minimize git churn, non-blocking), AC-SPR-024 subsumed by MUST AC-SPR-026. Verification: build green darwin+linux+windows; `go vet` clean; golangci-lint 0 issues; full test suite green except 6 pre-existing `internal/statusline` `memory_test.go` failures (PRESERVE scope, baseline-documented, not caused by this SPEC); race clean on affected packages; M6 grep audit battery 0 code-level matches; config struct↔YAML symmetry PASS; template-neutrality PASS. README en/ko preset examples updated (sync-phase). **Residual doc gap (out of AC-SPR-018 scope)**: `docs-site/content/{en,ko,ja,zh}/getting-started/faq.md` still describe the 4 presets (AC-SPR-018 verification targets `advanced/statusline.md` only) — follow-up cleanup recommended. plan-auditor PASS-WITH-DEBT 0.86 iter-2.

### Added

- **[SPEC-V3R6-CONTEXT-GOV-AXIS-001](.moai/specs/SPEC-V3R6-CONTEXT-GOV-AXIS-001/spec.md)** — Eager-vs-on-demand context-weight recording + Context-Governance Axis drift alarm (Tier M, v0.1.0, run-phase 2026-06-18, sync-phase 2026-06-19). Sprint 15 P2b harness-books application (book2 ch8.3 signal dilution + diag-05 three context-governance paths). The harness had token budgets for **skills** but no equivalent budget or audit for the **eagerly-loaded** context (`CLAUDE.md` + ~60 auto-loaded `.claude/rules/moai/*.md` + `moai.md` output-style + `MEMORY.md`); the existing `session-handoff.md` thresholds are REACTIVE (when to `/clear`), not PREVENTIVE. This SPEC adds the preventive complement. **M1** (schema extension + weight recording): `internal/harness/types.go` `LogSchemaVersion` bumped `"v2"` → `"v2.1"` (backward-compatible — legacy `v1`/`v2` lines parse unchanged with weight fields at sentinel); `Event` struct extended with 3 additive `omitempty` fields (`EagerContextWeight`, `OnDemandContextWeight`, `WeightUnit`); `internal/harness/observer.go` new `EstimateContextWeight` + `estimateEagerWeight` (fail-open, EC-1 skip-missing); `internal/cli/hook.go` all 4 `runHarnessObserve*` handlers wired. **M2** (drift alarm doctrine): `.moai/docs/harness-delivery-strategy.md` new §8 "Context-Governance Axis" — monotonic-growth → Tier-2 demote proposal rule, N=3 named constant (bounded [3,5]), book2 ch8.3 + diag-05 cited verbatim, additive Tier-2 signal (Tier system unchanged). **REQ-CGA-003 fail-open** (cross-reference P0 `SPEC-V3R6-HARNESS-RUNTIME-RECOVERY-001` death-spiral avoidance): observer hook never blocks a turn on weight-measurement failure (exit 0, warn-and-continue). **7/7 AC PASS** (AC-CGA-001..007); coverage `internal/harness` 87.3% (≥85%); cross-platform build `darwin`/`windows` exit 0; golangci-lint 0 NEW issues; subagent-boundary (C-HRA-008) 0 matches in touched Go source. Go change: 4 files (types/observer/hook/context_gov_test) + 1 doctrine markdown. plan-auditor PASS-WITH-DEBT. sync-phase orchestrator-direct (GLM manager-docs spawn context-limit fallback per `feedback_glm_orchestrator_direct_sync_mx`).

- **[SPEC-V3R6-TOOL-POLICY-SSOT-001](.moai/specs/SPEC-V3R6-TOOL-POLICY-SSOT-001/spec.md)** — Tool/Permission Policy declarative SSOT + codegen (Tier L, v0.1.0, run-phase 2026-06-18, sync-phase 2026-06-19). Sprint 15 P2a harness-books application (book2 ch4.3-4.4 "contractor with compliance office" + appendix B.3 "independently evaluable approval policy"). Tool/permission policy was scattered across 4 heterogeneous surfaces (`.claude/settings.json` permissions block, `status-transition-ownership.sh` comment block, `glm-web-tooling.md` HARD routing table, `agent-common-protocol.md` background-write prose) with no single parsable SSOT. This SPEC lifts them into one declarative artifact: **M1** `.moai/config/sections/tool-policy.yaml` — 6-field entry schema `{tool, args_pattern, risk_tier, decision, owner_agent, audit}` seeded from all 4 sources (allow=110/deny=60/ask=6, book2 ch8.4 risk-descending order); **M2** codegen mechanism — two-strategy (`moai tool-policy build`): parse-modify-serialize on `.claude/settings.json` (pure JSON), raw-text region replacement on `internal/template/templates/.claude/settings.json.tmpl` (mixed JSON + Go-template directives; post-condition sentinel AC-TPS-014b asserts zero `{{`/`}}` in the regenerated permissions block); **M3** thin `moai tool-policy list` query (NEW `internal/cli/tool_policy.go`) modeled on `moai constitution list` CLI SHAPE but NOT wrapping it (schemas disjoint — tool-policy 6-field vs constitution 7-field, REQ-TPS-006). A single rule change (flip allow→ask) now lands via YAML edit + regenerate with NO Go decision-path edit (REQ-TPS-009). YAML↔settings.json drift structurally prevented by construction (§24.5 analogy — both generated surfaces derive from one YAML; the markdown-doctrine-vs-Go-code drift class is explicitly out of scope §X.8 per verification-claim-integrity honest-scope doctrine). **14 ACs PASS** (AC-TPS-001..014; 010 MUST-FIX lint, 013 codegen idempotency, 014 template round-trip + sentinel); coverage `internal/config/toolpolicy` 91.1%; cross-platform build darwin/windows exit 0; golangci-lint 0 issues; subagent-boundary (C-HRA-008) 0 matches. Go change: new pkg `internal/config/toolpolicy/` + `internal/cli/tool_policy.go` + template mirror `internal/template/templates/.moai/config/sections/tool-policy.yaml`. plan-auditor PASS-WITH-DEBT. sync-phase orchestrator-direct (GLM manager-docs spawn context-limit fallback per `feedback_glm_orchestrator_direct_sync_mx`).

- **[SPEC-V3R6-SKILL-DECISION-HEURISTICS-001](.moai/specs/SPEC-V3R6-SKILL-DECISION-HEURISTICS-001/spec.md)** — Decision Heuristics device: 5 `if X → default Y` heuristics per foundation skill (Tier S, v0.2.0, run-phase 2026-06-19, sync-phase 2026-06-19). Sprint 15 P3 — a craft device that condenses the orchestrator's recurring decision patterns into a scannable `## Decision Heuristics` section appended to 4 foundation skills. **M1-M3**: additive `## Decision Heuristics` (5 heuristics each, within 3-5 bound, each with `(<- §section)` provenance pointer) to `moai-foundation-core`, `moai-workflow-spec`, `moai-foundation-cc`, `moai-meta-harness` — 54 insertions / 0 deletions (C-5 additive-only). Provenance honesty: `moai-workflow-spec` binds AP-SRN-004 (Wave→Round retired naming, recurred 2026-05-25 commit `64310df3f`, memory-verified); `core`/`cc` use honest pending-form (no fabricated dates); `meta-harness` deliverable (b) honestly N/A (0 evolvable markers — no content fabricated). **8 ACs PASS** (AC-SDH-001..008 incl. 005 PASS-WITH-DEBT for the 2 pending-form provenance bindings); M3 grep/lint verification gate (4/4 sections present, ≤13-line scroll-bound binary threshold AC-SDH-003, FL-2 marker balance preserved 3/3/3/0, EC-6 template source untouched). No Go code (additive-doc cycle). spec-lint 0 errors. plan-auditor PASS-WITH-DEBT. **FU-1 template mirror sync deferred** — separate step per C-4 scope protection (`internal/template/templates/.claude/skills/` mirror NOT touched in this SPEC; the LOCAL 4 skills carry the new section, the template mirror is an explicit forward follow-up). sync-phase orchestrator-direct (GLM manager-docs spawn context-limit fallback per `feedback_glm_orchestrator_direct_sync_mx`).

- **[SPEC-V3R6-ORCH-INTERRUPT-LEDGER-001](.moai/specs/SPEC-V3R6-ORCH-INTERRUPT-LEDGER-001/spec.md)** — Interrupt Ledger Closure: synthetic result for aborted Agent() delegations, team-ac-verify exit-2 ledger_note field, and Error Recovery banner annotation (Tier M, v0.2.0, sync-phase 2026-06-19). Adds `§Ledger Closure` subsection to `agent-common-protocol.md` defining the ledger-closing invariant (REQ-LED-001..006): (a) synthetic result on aborted Agent() delegation, (b) team-ac-verify.sh exit-2 `ledger_note` field, (c) TaskCompleted exit-2 task closure, (d) cross-references to book1 ch04/ch06/ch07. `moai.md` §8 Error Recovery banner gains Interrupt Closure annotation. 18 ACs (13 MUST + 5 SHOULD). Go change 0 (doctrine-only). plan-auditor PASS-WITH-DEBT 0.88 (Tier M threshold 0.80; D1..D7 MINOR residuals accepted). Direct successor to SPEC-V3R6-HARNESS-RUNTIME-RECOVERY-001 (sibling ledger-closure subsection in `agent-common-protocol.md` §Hook Invocation Surface — both sections now co-locate the ledger-closure invariant per the sibling SPEC consolidation decision). sibling-overflow follow-up: `feedback_shared_main_orphan_race` multi-session race mitigation + V0 abort gate doctrine (`feedback_v0b_bg_pty_host_false_positive` false-positive fix).

- **[SPEC-V3R6-DOCS-CODEMAPS-V3-001](.moai/specs/SPEC-V3R6-DOCS-CODEMAPS-V3-001/spec.md)** — v3.0 codemaps SSOT refresh + docs-truth checklist for the 4-SPEC docs rewrite cohort (Tier M, v0.1.0, run-phase 2026-06-17, sync-phase 2026-06-17). First SPEC of "Sprint 14 Docs-v3"; prerequisite for the README/DOCSITE/COVERAGE/I18N-PARITY cohort. A 7-agent dynamic-workflow analysis identified 34 docs gaps; this SPEC refreshes the architecture map + authors a machine-greppable facts checklist so later docs SPECs rewrite against a verified baseline (not frozen v2.x docs). **M1**: `.moai/project/codemaps/overview.md` + `modules.md` refreshed (v3.0.0-rc2 baseline — 10 capability layers, 11-layer module catalog covering 48 internal packages; deterministic `go list -deps -json` + `go doc` baseline + clearly-labeled Architectural Insight subsections per C4 determinism convention). **M2**: `.moai/project/codemaps/docs-truth.md` (NEW) — 5-section facts checklist (§1 8 retained agents / §2 8-value status enum / §3 12 frontmatter fields / §4 CLI surface — terminal verbs + 17 `/moai` skills / §5 GLM→Claude tier-models full 7-row table) with 6 `**Source:**` citations. **M3**: AC PASS/FAIL matrix. **M4**: run-phase audit-ready signal. **10 ACs (9 MUST [AC-CM-000..008] + 1 SHOULD [AC-CM-009])**: AC-CM-001/009 PASS-WITH-DEBT (substantive coverage present; 2 acceptance-regex defects Debt-1/Debt-2 blocked the mechanical gate). Go code change: 0 (REQ-CM-010 honored — additive Markdown only). Neutrality clean (AC-CM-007 PASS — only self-ref SPEC-ID). **3 pre-sync debt resolved in fix commit `fdf45b35a`** (manager-spec domain): Debt-1 acceptance.md:54 `"GLM\|Multi-LLM\|multi-llm"` `\|`→`|` (ERE alternation; `\|` was literal pipe); Debt-2 acceptance.md:304 `(^|[^a-z])pkg(/|[^a-z]|$)` → `\bpkg\b` (ugrep 7.5.0 word-boundary); Debt-3 spec.md §B.2 numbered list → dash bullets (`OutOfScopeRule` lint.go:716 counts only `-` lines; internal/spec/CLAUDE.md "H3 sub-headings" guidance is superseded by the dash-bullet execution truth). Verification: `go build ./...` exit 0; `go vet ./...` exit 0; `golangci-lint run` 0 issues; `moai spec lint spec.md` 0 ERROR after Debt-3 fix. plan-auditor PASS-WITH-DEBT (Tier M threshold 0.80). docs-site 4-locale + README skipped (internal architecture artifact — those surfaces belong to the README/DOCSITE cohort SPECs per acceptance §D.5). sync-phase manager-docs spawn context-limit risk (GLM) → orchestrator-direct fallback. **Known doc-vs-code drift (out of AC scope)**: internal/spec/CLAUDE.md `MissingExclusions` guidance says "H3 sub-headings" but lint.go:716 counts dash bullets — follow-up cleanup recommended.

- **[SPEC-V3R6-DOCS-V3-README-001](.moai/specs/SPEC-V3R6-DOCS-V3-README-001/spec.md)** — README v3 rewrite — reconcile README.md + README.ko.md against the docs-truth canonical facts baseline (Tier M, v0.2.1, plan-phase 2026-06-17, run-phase 2026-06-17, sync-phase 2026-06-17). Second SPEC of "Sprint 14 Docs-v3" cohort; consumes the `.moai/project/codemaps/docs-truth.md` baseline authored by predecessor CODEMAPS-V3-001. 7-axis drift reconciliation across the 2 repo-root README files (en/ko): (1) **agent catalog** — 8 retained agents (manager-spec/develop/docs/git + plan-auditor + sync-auditor + builder-harness + Explore); archived names removed from active sections INCLUDING the "Agent Model Assignment by Tier" tables (en/ko) which had listed 10-11 archived agents (`manager-strategy/quality/project`, 6-7 `expert-*`) as active model-policy rows; (2) **`/moai` command set** — stale "47 Skills"/"47개 스킬" headers + 13-row skill-category count tables replaced with a concise 17-command `/moai` listing (17 = `ls .claude/commands/moai/*.md | wc -l`); (3) **GLM tier-models** — `GLM-5.1` → `glm-5.2[1m]` (High/Opus, 1M ctx), `glm-4.7` (Medium/Sonnet), `glm-4.5-air` (Low/Haiku); (4) stale agent-count prose (ko "24개/26개 에이전트" 5 surfaces incl L308/L372 — D1 broadening); (5) stale category rows (en "Design System | 4 (+evaluator)" + ko "Agency | 6" — Dn-1); (6) statusline preset retire preserved; (7) statusline v3 multi-line redesign preserved. en/ko fact-for-fact synchronization (8 retained / glm-5.2[1m] / 17 commands). **8/8 AC PASS** (AC-1..AC-8, all mechanically grep/sed/diff verified) — orchestrator independently re-verified all 8 ACs post worktree→main fast-forward (AC-8(b) docs-site false-positive resolved: 11 docs-site files belong to the book.mo.ai.kr migration commit, not this SPEC; README-001 actual base → 0 docs-site changes). Go code change: 0 (doc-only). Neutrality: repo-root README is project-owned, not template-distributed (`find internal/template/templates -iname "README*"` → 0). plan-auditor iter-1 PASS-WITH-DEBT 0.83 → iter-2 0.86 (D1/D2 BLOCKING resolved: ko stale-count undercount + tier-table archived-names active rows; D3 SHOULD-FIX ko "47개 스킬" header) + v0.2.1 patch (Dn-1/Dn-2 MINOR: Design System/Agency row AC coverage + Edge-1b ko regex `(할당|배정)`). **Forward-looking gaps (out of SPEC §C.1 scope, symmetric en/ko, candidate for a follow-up SPEC)**: Mermaid architecture diagram stale count nodes (en L268-272, ko L318) + design-workflow prose describing archived agents as active `/moai design` participants (en L918/L939-969). Worktree note: manager-develop ran in an L1 worktree (Claude Code runtime autonomous), pushed M1-M6 (5 commits) to origin/main; main worktree fast-forwarded post-run (race-free, same author). sync-phase orchestrator-direct (GLM manager-docs spawn context-limit fallback per `feedback_glm_orchestrator_direct_sync_mx`).

- **[SPEC-V3R6-DOCS-DOCSITE-001](.moai/specs/SPEC-V3R6-DOCS-DOCSITE-001/spec.md)** — docs-site 4-locale content reconciliation against the docs-truth canonical facts baseline (Tier L, v0.2.0, plan-phase 2026-06-17, run-phase 2026-06-18, sync-phase 2026-06-18). Third SPEC of "Sprint 14 Docs-v3" cohort; extends README-001's 2-file (repo-root README) reconciliation to the full docs-site 4-locale tree (`docs-site/content/{en,ko,ja,zh}/`), consuming the `.moai/project/codemaps/docs-truth.md` baseline authored by CODEMAPS-V3-001. Factual reconciliation only — NO IA redesign, NO styling/build config change. 6 docs-truth axes × 4-locale parity: (1) **agent catalog** — 8 retained agents across all 4 locales; the worst drift was ja `core-concepts/what-is-moai-adk.md` (5 stale "28" surfaces at L7/L48/L226/L496/L670 + zero positive "8" forms — full pre-consolidation state); ko/zh had partial drift (ko L226 prose + L666 code-block, zh L674 code-block); en was already clean. (2) **mermaid structural rewrite** — ko/ja/zh mermaid `Managers` subgraph had phantom duplicate `M6/M7` manager-spec/docs slots inflating the count to 8 managers + misclassifying `M5 sync-auditor` as a Manager; rewritten to match en's clean structure (4 Managers M1-M4 + Evaluators subgraph [plan-auditor, sync-auditor] + Builder [builder-harness] + Explore built-in). (3) **GLM tier-models** — ko `multi-llm/_index.md` stale `GLM-5.1` → `glm-5.2[1m]` (High/Opus, 1M ctx); en/ja/zh `multi-llm/_index.md` + `model-policy.md` stubs (9 lines) expanded to ko-equivalent 72-line tier-models table with `glm-5.2[1m]` (achieves perfect 1:1:1:1 — 72/72/72/72 structural parity). (4) **language count** — "18 languages" → "16 languages" (en L216, ko L49+L216 + introduction L134, ja L216; zh already clean) per FROZEN rule CONST-V3R2-004. (5) **CLI 17 commands** + (6) **SPEC status enum** verified (AC-006 no stale count; AC-007 vacuous — no docs-site page enumerates the full 8-value lifecycle). **11/11 AC PASS** (4 Critical: AC-001/002/003/005/010; 3 High: AC-004/006/009; 3 Medium: AC-007/008/011) — orchestrator independently re-verified all ACs via acceptance.md digit-boundary-protected per-locale grep loops (`(^|[^0-9])` prefix prevents the "8 inside 28" substring false-positive per `feedback_digit_boundary_locale_grep_4parity`): stale-28 all=0, phantom-M6/M7 all=0, glm-5.1 all=0, glm-5.2 each≥1, positive-8 en=2/ko=5/ja=4/zh=5, 18-languages all=0, AC-010 regression guard 0 archived active-context added. i18n tool (`scripts/docs-i18n-check.sh`): in-scope multi-llm errors 0; remaining 56 errors are `claude-code/` reference docs (out-of-scope, not MoAI facts-bearing). Go code change: 0 (docs-site markdown only). Neutrality: docs-site content is project-owned, not template-distributed. plan-auditor iter-1 FAIL **0.62** → iter-2 PASS-WITH-DEBT **0.88** (monotonic +0.26, Tier L threshold 0.85, zero BLOCKING; iter-1 root cause = 6/10 AC regexes authored from imagination, never run against the live tree — Verify-Don't-Assume violation; iter-2 fix = digit-boundary prefix + per-locale loop + locale-native idioms + inline live-grep evidence). Race absorbed: run-phase executed in an L1 worktree; a parallel skill-audit session pushed 2 commits (`0684e7030`/`73dbdb672`) during the run, resolved via `git rebase origin/main` with no conflicts (disjoint file surfaces — DOCSITE-001 = docs-site/content, skill-audit = skills/config). sync-phase orchestrator-direct (GLM manager-docs spawn context-limit fallback per `feedback_glm_orchestrator_direct_sync_mx`). **Out-of-scope adjacent drift** (research.md recorded, NOT edited): "31 skills" count (uncoupled, not a FROZEN rule) → candidate follow-up SPEC.

- **[SPEC-V3R6-DOCS-COVERAGE-001](.moai/specs/SPEC-V3R6-DOCS-COVERAGE-001/spec.md)** — docs-site 4-locale skill-count reconciliation (31→32 + humanize) + ko/ja structural rewrite + en/zh in-body fictional-name cleanup (Tier L, v0.2.0, plan-phase 2026-06-18, run-phase 2026-06-18, sync-phase 2026-06-18). Fourth SPEC of "Sprint 14 Docs-v3" cohort; owns the skill-count axis deferred from DOCSITE-001 §E. Canonical count = 32 (`find internal/template/templates/.claude/skills -maxdepth 1 -mindepth 1 -type d | wc -l` → 32) = 1 umbrella router + 31 specialized skills in 6 categories (Foundation=4 / Workflow=10 / Domain=9 [incl. `moai-domain-humanize` added 2026-06-15] / Reference=5 / Meta-Harness=2 / Design=1). Three correction classes × 4-locale parity: (1) **count correction** — "31"→"32" across 5 page-families × 4 locales (~18 files); (2) **ko/ja structural rewrite** — `advanced/skill-guide.md` carried a pre-v3 fictional taxonomy (9 categories incl. Language/Platform/Library/Tool/Framework + 37 nonexistent skill-name refs each, 74 total) → rewritten to the canonical 6-category structure matching en/zh (74 fictional names eliminated, Reference/Meta-Harness/Design introduced); (3) **en/zh in-body cleanup** — correct 6-category headers but 10/11 residual fictional names in conceptual illustrations (Mermaid nodes / Skill() code / ASCII trees / frontmatter examples) → replaced or rephrased (21 names eliminated). Locale-complete fictional-name inventory: en:10 + ko:37 + ja:37 + zh:11 = 95 total. **11/11 AC PASS** (10 MUST + 1 SHOULD; AC-001 PASS-WITH-DEBT — REQ-002 mandates "1 umbrella + 31 specialized" so the digit "31" legitimately appears 1× per locale as the correct sub-count, NOT a stale total; baseline stale totals en:12/ko:10/ja:7/zh:11 = 40 all eliminated) — orchestrator independently re-verified all ACs via per-locale digit-boundary-anchored grep (`(^|[^0-9])` prefix per `feedback_digit_boundary_locale_grep_4parity`). D-new-1 Meta header slash/hyphen normalization (`Meta/Harness` slash form unified across 4 locales, AC-007 6/6). Go code change: 0 (docs-only). Neutrality: docs-site content project-owned, not template-distributed. plan-auditor iter-1 FAIL **0.62** → iter-2 PASS **0.88** (monotonic +0.26, Tier L threshold 0.85; iter-1 BLOCKING D1 = ko locale has the SAME 9-category fictional taxonomy as ja but plan treated ko as count-patch-only — orchestrator independent 4-locale grep caught it, manager-spec missed it; iter-2 fix = REQ-005 ja+ko unified, AC-006 `for loc in ja ko`, M2 ko structural rewrite + orchestrator-augmented REQ-009/AC-011 for en/zh in-body). Run-phase worktree push `b35caaaf2` (11 AC) + iter-2 SPEC artifact backfill `085e8ecc5`. sync-phase orchestrator-direct (GLM manager-docs spawn context-limit fallback per `feedback_glm_orchestrator_direct_sync_mx`). **Forward-looking (out of scope, acceptance §D.4)**: docs-truth.md §6 "Skill Count (32)" axis addition — separate sync-phase commit / follow-up.

- **[SPEC-V3R6-SESSION-ID-ATTRIBUTION-REPAIR-001](.moai/specs/SPEC-V3R6-SESSION-ID-ATTRIBUTION-REPAIR-001/spec.md)** — Session-ID attribution dead-feature repair (multi-session coordination Layer 2 race-attribution) (Tier M, v0.1.0, run-phase 2026-06-17, sync-phase 2026-06-17). Repairs the `source_session_id: <UUID>` attribution chain that paste-ready resume messages (session-handoff.md Block 2) and auto-memory `project_*.md` depend on. **3 pillars**: **P1 (M1, GATING)** — root cause `internal/hook/types.go:323` `HookOutput.Data json:"-"` was never serialized → the SessionStart handler's `data` map never reached Claude Code → `additionalContext` was structurally unreachable (the 91% environment-fallback dead-feature). Companion `moai session doctor` diagnostic (3 root-cause candidates: empty `input.SessionID`, hook wrapper silent-exit, registry write failure) + empty-SessionID stderr warning (AC-WPR-001/002/003/004 GATE). **P2 (M2+M3)** — `moai session current` subcommand (reads side-channel `.moai/state/current-session-id.txt`, survives `/clear`/compaction) + SessionStart `additionalContext` UUID injection via `hookSpecificOutput.AdditionalContext` (strictly additive, AC-RDP-001..006). **P3 (M4+M5)** — fallback doctrine canonicalization: non-canonical `source_session_id` variants eliminated to 2 canonical surfaces (the environment-fallback string + the `<UUID from moai session current>` happy-path slot); `.moai/specs/` self-referential grep pollution surfaced as a new defect class `feedback_self_referential_grep_pollution` (AC-FBC-001..005). **M6** resume-template citation + final verification. Files: `internal/cli/session.go`, `internal/hook/{types,session_start}.go`, `internal/session/registry.go` + doctrine edits (session-handoff.md / moai.md §8 + template mirrors, byte-parity). **18/18 AC PASS** (15 MUST + 3 SHOULD) — go build/vet exit 0, golangci-lint 0 issues, spec-lint clean, template neutrality + mirror parity PASS, in-scope package tests green (session/cli/hook/template). plan-auditor PASS-WITH-DEBT 0.86 iter-2 (Tier M threshold 0.80, Direction A 3-pillar). Pre-existing residual (OUT OF SCOPE): `internal/statusline` `TestCollectMemory` baseline failure at clean `12e20d190` (STATUSLINE-PRESET-RETIRE leftover, not caused by this SPEC). **Honest residual-risk**: `additionalContext` lost after `/clear` (side-channel file persists → `moai session current` re-reads); headless `-p` without hooks → canonical fallback. sync-phase manager-docs spawn context-limit failed (3rd GLM occurrence incl. WORKFLOW-EFFORT-MAP-001 + SESSION-HANDOFF-SSOT-ALIGN-001 siblings) → orchestrator-direct fallback.

- **[SPEC-V3R6-WORKFLOW-EFFORT-MAP-001](.moai/specs/SPEC-V3R6-WORKFLOW-EFFORT-MAP-001/spec.md)** — Purpose-driven model+effort selection for dynamic Workflow agent() calls, Agent Teams role_profiles, and session-handoff ultracode re-set (Tier M, v0.2.0, run-phase 2026-06-17, sync-phase 2026-06-17). Establishes the SSOT mapping a Workflow `agent()` purpose → `(model, effort)` pair, plus the declarative `effort` field on Agent Teams role_profiles, plus the purpose-conditional `/effort ultracode` re-set line in session-handoff Block 1. **M1**: `.claude/rules/moai/workflow/dynamic-workflows.md` (+ template mirror) gains a "Purpose-driven model+effort selection" section — 7-row taxonomy table (read-only→low, agentic→xhigh, verify→high/xhigh, synthesize→inherit), `agent()` `opt.effort` schema, REQ-WEM-002 explicit-purpose rule, codemaps-extract.js worked example. **M2**: `.claude/workflows/codemaps-extract.js` line 54 `agent()` call carries `effort: 'low'` co-located with `agentType: 'Explore'` (read-only-extract → low). **M3**: `.moai/config/sections/workflow.yaml` (+ template mirror) `role_profiles` gains the declarative `effort` field ×7 (researcher=low, analyst=medium, architect=xhigh, designer=medium, implementer=xhigh, tester=medium, reviewer=high) — byte-aligned across both files. **M4**: `session-handoff.md` (+ template mirror) Block 1 gains the purpose-conditional `/effort ultracode` re-set line + Field-by-Field sub-bullet + Anti-Patterns entry (emitted ONLY when the next SPEC's plan declares workflow fan-out; ultracode is NOT restored by `ultrathink.` per dynamic-workflows.md). **M5**: `output-styles/moai/moai.md` §8 render surface updated to match the session-handoff Block 1 edit (+ template mirror). **Decision #1 (no Go coupling)**: the `effort` YAML field is declarative-only — `RoleProfileEntry` carries no `Effort` field, so the YAML cannot couple to Go even if a reader existed; `internal/` Go changes = 0. **13/13 AC PASS** (AC-WEM-001..012 incl. 009a/009b mirror parity) — all 3 doctrine mirror pairs (dynamic-workflows / session-handoff / moai.md local↔template) 0-diff, template-neutrality PASS (0 SPEC-ID/REQ/Audit leakage under template paths), no Go build/lint surface touched. **Multi-session race recovery**: run-phase (M1-M5, `685a60751`) was FF-merged to local main but never pushed; a parallel SESSION-ID-ATTRIBUTION session pushed `538fcaea6` and this tree reset main to origin/main, marooning the 5 commits on `backup/wf-effort-map-001`; sync-phase re-integrated via merge `e8f3e58dd` (session-handoff.md ×2 conflict resolved by combining the EFFORT-MAP ultracode sub-bullet with SESSION-ID-ATTRIBUTION's refined Block 2). plan-auditor PASS-WITH-DEBT 0.84 iter-2 (Tier M threshold 0.80; 3 NEW MINOR residual D1-new/D2-new/D3-new accepted). Residual debt: D1-new git-diff PRIMARY for AC-WEM-006, D2-new dynamic-workflows.md ~28KB file size, D3-new iter/rev naming in plan history.

- **[SPEC-V3R6-SESSION-HANDOFF-SSOT-ALIGN-001](.moai/specs/SPEC-V3R6-SESSION-HANDOFF-SSOT-ALIGN-001/spec.md)** — session-handoff.md (SSOT) ↔ moai.md §8 (render surface) drift unification + hybrid recurrence-mitigation mechanism (Tier S, v0.1.0, run-phase 2026-06-17, sync-phase 2026-06-17). Track 2 of the 3-track session-handoff defect analysis (track 1 = SPEC-SESSION-HANDOFF-ALIGN-001 tree-pair drift). Closes 4 real drifts between the two FILES within one tree (both trees byte-identical throughout): (1) **D3** — migrate retired `<wave>` filename token + memory-filename examples to `<sprint>` per sprint-round-naming.md AP-SRN-004; preserve generic-English-noun `wave` in prose (multi-SPEC waves / current wave). (2) **D5a** — disambiguate the bare "Pre-emit self-check" label colliding across 3 sites (session-handoff.md paste-ready budget / moai.md §8 localization render / moai.md §8 handoff-template completeness) with surface-owned concern-name qualifiers; the template-completeness site keeps its 9-item count. (3) **D6** — add explicit English-canonical-fallback localization rule for ISO-639 locales not in the en/ko/ja/zh table, scoped to the Localization Table region in both files. (4) **D9** — consolidated `### Anti-Pattern Index` in session-handoff.md with forward links to the 3 existing detail catalogues (general / AP-D-001..005 / AP-V-001..004). **Hybrid recurrence-MITIGATION mechanism** (thesis downgraded from "prevention" to "mitigation + visibility" per plan-auditor falsification): `<!-- render-only, not canonical -->` marker in moai.md §8 + self-check sentinel in BOTH files verifying the other surface's structural invariant (locale column count, label qualifiers). Honest residual-risk: surfaces drift to a reading editor but does NOT mechanically prevent it; the only mechanical catch is a deferred Go lint rule (out of scope, new-code=0). **D5b withdrawn** — iter-1's "label all-9 vs 8-items" defect was a windowed-grep false-positive (actual count is 9 at L682-L690, label already correct); REQ/AC-SHA-004 deleted, 9→7 REQs/ACs renumbered clean. **7/7 AC PASS** (6 MUST + 1 SHOULD, all baseline-fail-verified, no vacuous-pass) — byte-parity both files both trees (session-handoff 28381B / moai 53021B), template-neutrality-leak test PASS (SPEC-ID leak self-caught + fixed mid-run), spec-lint 0 error, new Go code 0. plan-auditor iter-1 FAIL 0.62 → iter-2 PASS-WITH-DEBT 0.84 (monotonic +0.22; D5b false-positive resolved by independent 3-method re-measurement). sync-phase manager-docs spawn context-limit failed → orchestrator-direct fallback (track-1 precedent). **defer 후속**: Go lint rule for mechanical SSOT↔render drift detection → 별도 code SPEC; track 3 (D4 source_session_id 91% fallback dead-feature) → SPEC-V3R6-SESSION-ID-ATTRIBUTION-REPAIR-001.

- **[SPEC-SESSION-HANDOFF-ALIGN-001](.moai/specs/SPEC-SESSION-HANDOFF-ALIGN-001/spec.md)** — session-handoff.md template↔local drift closure + mirror-parity enrollment + i18n/dedup debt (Tier M, v0.2.0, run-phase 2026-06-17, sync-phase 2026-06-17). 6-dimension 구조 분석(7-agent workflow, 19 findings)이 도출한 3축 정렬: (1) **drift closure** — local(314)/template(209) 105-line delta를 0으로 붕괴(양쪽 324/324 byte-identical). local에만 갇혀 있던 generic doctrine 2종을 중립화 port — **Diet Constraints**(Block 2 ≤4 refs / Block 4 ≤200 chars / Block 5 single action / Block 6 ≤2 lines / AP-D-001..005 / 8-item Pre-emit self-check) + **V0 Abort Gate**(`ps aux` raw-count false-abort 봉쇄, lsof canonical commands, AP-V-001..003)가 이제 user project에 배포됨. (2) **mirror-coverage gap** — session-handoff.md를 `rule_template_mirror_test.go`에 enroll(workflow/ 17개 중 유일 major-drift outlier였음); 앞으로 local↔template drift CI 감시. (3) **i18n/dedup** — 4-locale header translation table SSOT 자급화, Block 1 skeleton verb `<entering verb>` placeholder화(moai.md §8 `entering`와 정합), cut-line marker spec 4× 중복을 SSOT 단일화, 3개 anti-pattern catalogue cross-link, Trigger #1 model-label drift(Opus 4.7 → cwm.md pointer) 제거, V0 "Cross-pollination 이력" 5-bullet → 1-line lesson(AP-D-002 self-violation 해소). 3 stale SPEC-ID 라인 local→template realign. **17/17 AC PASS**(13 MUST + 4 SHOULD) — `go test ./internal/template/` green(mirror + neutrality leak), `go vet` 0, `golangci-lint` 0, diff=0. plan-auditor iter-1 FAIL **0.74** → iter-2 PASS-WITH-DEBT **0.84**(monotonic +0.10). lifecycle-sync-gate.md는 local-only(internal SPEC-ID 포함, clean doctrine 아님) → EXCL-006 별도 follow-up SPEC로 carve-out. **defer 후속**: 16 in-sync workflow/ sibling bulk mirror enrollment → FL-1 별도 SPEC.

- **[SPEC-CC2178-MODEL-POLICY-REPAIR-001](.moai/specs/SPEC-CC2178-MODEL-POLICY-REPAIR-001/spec.md)** — CC 2.1.178 Model-Policy Repair: 3-Axis Cost Routing Alignment (model × effort × cycle_type) + Phantom-Map Cleanup (Tier M, v0.3.0, run-phase 2026-06-16, sync-phase 2026-06-16). CC 2.1.163→2.1.178 연구(`.moai/research/cc-update-2.1.163-to-2.1.178.md`)에서 도출한 3축 비용 모델 정렬 + 4개 결함(D1–D4) 수리. **M1**: research 재검증 — `[1m]` entitlement-inheritance 제약 STILL-ACTIVE(CC 2.1.178, #36670 OPEN), per-agent pinning 금지 유지 → Default-model 라우팅이 유일 안전 레버 확인. **M2**: `internal/template/model_policy.go` phantom-map cleanup — `agentModelMap` 19→5 항목(16 archived phantom 키 제거 + `manager-develop`/`builder-harness` 추가), `agentEffortMap` 6→5 항목(3 phantom 제거, plan-auditor/sync-auditor high→xhigh 동기화). **M2.1**: D11 dead domain-walker fix(REQ-MPR-019, walker-target `[]string{"moai"}` Option A 흡수). **M3**: `internal/config/cycle_type.go` 신규 — `ResolveCycleType(harnessLevel, explicitPin)` 심볼 + dispatch table(minimal→ddd, standard/thorough→tdd, unknown→tdd, explicitPin wins); `.moai/config/sections/quality.yaml` `cycle_type_routing` 문서 섹션 추가. **M4**: CC 2.1.175 cost lever 배선 — `internal/template/templates/.claude/settings.json.tmpl`에 `"model": "sonnet"`, `"availableModels": ["sonnet","opus","haiku"]`, `"enforceAvailableModels": true` 추가. **M5**: `.claude/rules/moai/development/model-policy.md`(+ template mirror) `[1m]` Re-Verification + Default-Model Cost Lever 2 섹션 추가(byte-identical mirror, 내부 SPEC ID 비참조로 neutrality 준수). **M6**: Trust-but-verify 검증 증거 + §E.2/§E.3 audit-ready signal. **14/14 AC PASS**(12 MUST + 2 SHOULD, 0 fail) — `go test ./internal/config/ ./internal/template/` green, golangci-lint 0, cross-platform build darwin exit 0, spec-lint 0 error(StatusGitConsistency 1 WARNING = sync-phase 전환 예상). 2 pre-existing failure(`internal/statusline` TestCollectMemory_AutoCompactScaling, `internal/cli` TestDoctorPermission_NoMatchTrace)는 본 SPEC 변경 패키지 미접촉 무관(base `9d018697d`에서도 동일 fail 입증). README/docs-site 영향 없음(신규 settings 키는 template-managed 값으로 사용자 직접 편집 대상 아님 — settings surface 문서 미갱신 정당). plan-auditor iter-3 PASS-WITH-DEBT **0.84**(Tier M threshold 0.80, D11 CRITICAL walker-absorb 등 4 defect 해소). defer 후속: effort-map full retirement → `SPEC-CC2178-EFFORT-MAP-RETIREMENT-001`, task-triage → `SPEC-CC2178-TASK-TRIAGE-001`(3축 scope 통제 목적).

- **[SPEC-CC2178-DOCS-ALIGN-001](.moai/specs/SPEC-CC2178-DOCS-ALIGN-001/spec.md)** — CC 2.1.169→2.1.178 Tier 1 docs-only 기능 — 규칙·docs-site 4-locale 정합화 (Tier S, v0.1.0, run-phase 2026-06-16, sync-phase 2026-06-17). CC 2.1.163→2.1.178 연구(`.moai/research/cc-update-2.1.163-to-2.1.178.md`) P4/P5의 9개 Tier 1 docs-only 항목을 MODEL-POLICY-REPAIR-001 sibling로 분리·정합화. Go 코드 0; `.claude/rules/moai/`(template source + mirror) + docs-site 4-locale(ko/en/ja/zh) markdown에 한정. **M1**(permissions + skills discovery): `Tool(param:value)` wildcard permission syntax(REQ-DA-001), nested `.claude/` closest-wins 선행 규칙(REQ-DA-002), nested `.claude/skills` 로딩(REQ-DA-003), `disableBundledSkills` + `--safe-mode` 토글(REQ-DA-004) — settings-management.md/skill-authoring.md/agent-authoring.md source+mirror + docs-site settings-json.md/skill-guide.md/agent-guide.md 4-locale. **M2**(hooks + agent governance): `post-session` lifecycle hook(REQ-DA-005), subagent `disallowedTools` MCP 서버-수준 강제(REQ-DA-006), auto-mode pre-launch classifier(REQ-DA-007) — hooks-system.md/orchestration-mode-selection.md source+mirror + docs-site hooks-reference.md/hooks-guide.md/agent-guide.md 4-locale. **M3**(session resume): `/cd` cache-preserving resume를 Block 0 new-terminal 경로의 보완(대체 아님)으로 문서화(REQ-DA-008) — session-handoff.md mirror-direct(intentional-DIFFER 보존) + docs-site statusline.md/moai-sync.md 4-locale. **10/10 AC PASS**(5 MUST: AC-DA-001/002/008/009/010 + 5 SHOULD: AC-DA-003/004/005/006/007) — 4-locale parity(`scripts/docs-i18n-check.sh` 본 SPEC 페이지 0 errors; 6 page × 4 locale = 24 file 존재 확인), ZERO Go 코드(AC-DA-010, 본 SPEC commit 범위 `8b5d5d49d..827228819`에 `.go` 0개, `embedded.go` 빌드 산물 제외), template mirror parity(5 source-first PARITY, session-handoff.md intentional-DIFFER 유지), neutrality CI(`TestTemplateNeutralityAudit`) PASS. accuracy-over-completeness: MoAI가 미연결한 CC 옵션(post-session hook 미배선, `disableBundledSkills` 미 emit, param-scoped permission rule 미 emit)을 "옵션으로 존재" framing으로 명시. README 영향 없음(내부 docs 정합화, 기능/API 변경 0). plan-phase audit-ready.
- **[SPEC-V3R6-HARNESS-RUNTIME-RECOVERY-001](.moai/specs/SPEC-V3R6-HARNESS-RUNTIME-RECOVERY-001/spec.md)** — Runtime recovery doctrine (Tier M, v0.3.0, run-phase 2026-06-19, sync-phase 2026-06-19). Policy-layer doctrine codifying the withheld-recoverable-error set, cheapest-first recovery ladder, circuit-breaker invariants, and Recovery-Signal Carve-Out for anti-death-spiral protection (book1 ch03/ch06 grounding). **7 sections**: (1) withheld-recoverable-error set `{PTL, max_output_tokens, media_size, compact-failure}`; (2) 4-rung cheapest-first ladder (in-turn self-correction → paste-ready+clear → worktree restart → abort+preserve); (3) 5 circuit-breaker invariants (max-consecutive-failures, hasAttemptedReactiveCompact no-self-loop, compact-can-PTL last-resort, abort-closes-ledger, narrative-consistency); (4) Recovery-Signal Carve-Out (documentation-only policy — while a turn is recovering from a compact/PTL/max_output/media_size/compact-failure, Stop/PostToolUse hooks SHOULD exit 0 to avoid the error→stop-hook-blocks→retry→error death-spiral); (5) cross-references; (6) agent consult-the-doctrine obligation; (7) anti-patterns. **3 doctrine surfaces**: NEW `.claude/rules/moai/workflow/runtime-recovery-doctrine.md` (SSOT), `agent-common-protocol.md` §Hook Invocation Surface carve-out (render surface), `zone-registry.md` CONST-V3R6-001 entry (anti-death-spiral invariant naming). **M4 verification**: `moai spec lint` 0 errors (1 transient `StatusGitConsistency` WARNING = branch-state artifact, resolves on push); recovery-ladder grep confirms each of `reactive-compact`, `death-spiral`, `withheld-recoverable`, `circuit-breaker` returns ≥1 hit in `.claude/rules/moai/`. No Go code added (policy-layer scope only; `internal/recovery/` absent per AC-RR-009 Non-Goals). **11 ACs (9 MUST + 2 SHOULD)**: all satisfied. plan-auditor PASS-WITH-DEBT 0.88 iter-2 (Tier M threshold 0.80; D1 §E→Out of Scope + h3 sub-section, D2 `era: V3R6` frontmatter, D3 REQ-RR-010 reframed to recovery-ladder vocabulary, D4 REQ-RR-006/007 documentation-only, D5 §H Sprint 15 P1a queue, D6 non-canonical `related_specs:` removed, D7 REQ-RR-011/012 added).

- **[SPEC-HARNESS-CLI-COVERAGE-001](.moai/specs/SPEC-HARNESS-CLI-COVERAGE-001/spec.md)** — internal/cli/harness test coverage 77.9% → 93.0% (test-only, self-harness D3) (Tier S, v0.1.0, run-phase 2026-06-16, sync-phase 2026-06-16). Self-Harness 로드맵 D3 coverage 상향: `moai harness` CLI 표면(execute/install/propose 동사)의 테스트 커버리지를 77.9%에서 93.0%으로 증대. 신규 테스트: `internal/cli/harness/execute_cmd_test.go` (NewExecuteCmd RunE 클로저 success/error 분기), `internal/cli/harness/install_cmd_test.go` (NewInstallCmd RunE success/getwd/filepath.Abs 분기), `internal/cli/harness/propose_run_test.go` (runPropose non-dry-run write path + default-flag fallback). 프로덕션 non-test `.go` 파일 무수정 (test-only 제약 C1). 함수별 도달: NewInstallCmd ≥87.5% (reachable ceiling, 3 residual: os.Getwd/filepath.Abs/MarkFlagRequired panic), NewExecuteCmd ≥69.2% (reachable ceiling, os.Exit body 3 + panic 1 residual), runExecuteCommand ≥90%, RunInstall 100%, runPropose ≥90%. 프로덕션 경계 (AskUserQuestion 호출 금지, C-HRA-008) 무회귀 — `TestPropose_NoAskUserQuestion` PASS (경계 가드 강화). 기존 fail-close 무회귀 (`TestRunExecute_RegressionGateMeasuresProjectRoot`) PASS. **12/12 AC PASS** (AC-HCC-001..012, 전부 MUST; AC-HCC-002/006은 Option A reachable-ceiling 재조정 후 PASS) — coverage internal/cli/harness 93.0% total (package baseline 77.9%→93.0%, +15.1pp), cross-platform build (darwin/windows amd64) exit 0, golangci-lint 0, `go test ./...` green, t.TempDir 격리 enforcement, frontier reachable-ceiling documentation (os.Exit body 미커버 정당화). 무관 27개 grandfathered SPEC history-preserved. plan-auditor PASS-WITH-DEBT **0.86** (iter-1, Tier S threshold 0.75); AC realignment(Option A, 사용자 승인) 후 AC-HCC-002/006 target을 reachable ceiling로 재설정(measured 87.5%/69.2% PASS). 본 SPEC은 새 기능 추가 안 함 — 사용자 CLI 경계의 회귀 안전망 강화 + harness 프로덕션 caller(SPEC-HARNESS-APPLY-EXECUTE-001) 배선 후속의 질량 확보.

- **[SPEC-PROGRESS-MARKER-CANON-001](.moai/specs/SPEC-PROGRESS-MARKER-CANON-001/spec.md)** — progress.md §E 섹션 마커 컨벤션 정합화 (convention B) + era.go 주석 정정 (Tier M, v0.1.0, run-phase 2026-06-16, sync-phase 2026-06-16). progress.md의 §E 섹션 의미가 두 canonical 문서에서 충돌하던 것(`lifecycle-sync-gate.md` 워크드예제 §E.2=Sync vs `manager-develop.md`/`manager-docs.md` ownership §E.2=Run Evidence)을 **convention B로 단일화**: §E.1 Plan / §E.2 Run Evidence / §E.3 Run AR / §E.4 Sync AR / §E.5 Mx AR. 3개 정합 편집 — (1) `internal/spec/era.go` 3개 `//` 주석 사이트(L33/L91/L120)를 "§E.2 = run-evidence start marker / §E.5 = Mx-completion marker / 분류는 문자열-존재(`strings.Contains`) 기반" 으로 정정(**동작 보존**: `ClassifyEra` grep 로직·`go test ./internal/spec/...` byte-identical, comment-only diff EMPTY 입증), (2) `lifecycle-sync-gate.md` 워크드예제 §E.2 Sync→§E.4 Sync 정렬(H-4 비회귀: region 내 §E.2/§E.5 literal 유지), (3) `manager-spec.md`(+ template mirror) progress.md §E.1~§E.5 스켈레톤 생성 지시 추가(미래 §F.* ad-hoc drift 차단; 단 §E.2~§E.5 placeholder 채우기 권한은 부여 안 함 — manager-develop/manager-docs 소유 유지). 신규 프로덕션 코드 0 (era.go 주석 텍스트만 변경). 기존 27개 §F.* 사용 SPEC은 grandfather 역사보존(retro-edit 금지). **13/13 AC PASS** (AC-PMC-001..012 + AC-PMC-004b; 12 MUST + AC-PMC-012 SHOULD→MUST 승격) — comment-only diff gate(AC-PMC-005) EMPTY, run-evidence phrase count 4(≥3), lifecycle conv-B 정렬(§E.2 Sync=0 / §E.4 Sync=1), region-scoped H-4 비회귀 guard, mirror parity OK, `go test ./internal/spec/...` 비회귀(ok 6.2s), `make build` catalog.yaml 재생성. plan-auditor 0.84→0.86→**0.91 PASS** (iter-3, monotonic +0.07, skip-eligible; iter-2 D2 over-reach[self-stripping negative grep + L122 모순]를 iter-3 축소 정정으로 신규 결함 0 수렴). docs-site/README 영향 없음 (내부 dev-convention canonicalization).
...
- **[SPEC-STOP-EVIDENCE-WRITER-001](.moai/specs/SPEC-STOP-EVIDENCE-WRITER-001/spec.md)** — Record-time Evidence Writer — Activates the Stop Evidence Gate in Production (Tier M, v0.1.0, run-phase 2026-06-16, sync-phase 2026-06-16). **Direct successor to SPEC-STOP-EVIDENCE-GATE-001 (IMP-02/03)**: GATE-001 built the dormant evidence gate + session-ledger inference layer + the `IsTestPass`/`IsTestFail`/`PathKind` `omitempty` fields on `telemetry.UsageRecord`, but shipped as a no-op because no writer populated those fields. This SPEC closes that gap by adding **a record-time evidence writer** that genuinely **ACTIVATES the gate in production**. The writer expands the `postToolHandler.Handle` PostToolUse flow to record new `UsageRecord`s on (1) **Bash branch**: test-command evidence (`classifyTestCommand` + `IsTestPass`/`IsTestFail` binary classification), (2) **Edit/Write branch**: path-kind evidence (`classifyPathKind` → `PathKind` ∈ {code-change, docs-only, unknown}). Architecture per design.md: two pure classifiers + one assembler (`buildEvidenceRecord`) + one writer (`logEvidence`, fail-open) + one additive `if` block in `Handle`. **PRESERVE GATE-001**: session_ledger.go / stop.go read path unchanged; activation is write-side only. **10/10 AC PASS** (AC-SEW-001..010, all MUST; AC-SEW-009 = falsifiable gate-activation proof) — zero direct file-write primitives (reuse `telemetry.RecordSkillUsage`), ≤5s budget (in-memory classify + single append), C-HRA-008 subagent boundary clean (no AskUserQuestion in hook code), behavior preservation (existing observers untouched, HookOutput unchanged), fail-open (slog.Warn on error, never returns error, preserves skip semantics), AC-SEW-009 proves gate fires in production (code-change + test-fail → finding; test-pass → nil) with no opt-in flag. Run-phase: TDD M1-M6 complete, full suite green (go test, go vet, golangci-lint 0), cross-platform build (darwin/windows amd64) exit 0, coverage internal/hook 85%+.
