...
- **SPEC-OPUS47-COMPAT-001 status drift fix** (`in-progress → completed`, v0.2.0 → v0.3.0): plan-auditor's 2026-04-24 v0.2.0 HISTORY entry stated "status draft → completed (PR #672/#673 merged)" but the frontmatter remained `status: in-progress`. Corrected the drift. Zero code changes (work already merged). Enables v3.0.0-rc1 release notes to cite the SPEC as base infrastructure for "Claude Code v2.1.110/111 + Opus 4.7 compatibility".
...
- Walker fix가 차단하던 substring noise가 가리던 **real status drift 7건 표면화** — SPEC-V3R4-STATUS-DRIFT-FOLLOWUP-002 (예정)로 별도 처리. SDF-001 → FOLLOWUP-002 precedent 적용. v3.0.0-rc1 release timing에 영향 없음.
