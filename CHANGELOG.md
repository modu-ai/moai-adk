...
- **CWE-732 / CWE-552 (P0-4)**: `internal/migrate/hook_cleanup.go::atomicWrite` was a misnamed wrapper around `os.WriteFile`, breaking the atomic-write contract implied by its name. Two callers (`hook_cleanup.go:109` for `.claude/settings.json`, `:121` for archive) could corrupt user configuration on crash or signal mid-write. Replaced body with canonical tmp+rename pattern (`CreateTemp` + `Write` + `Sync` + `Close` + `Rename` + explicit `Chmod`) from `internal/runtime/persist.go:62-85`, signature preserved. `@MX:WARN` + `@MX:REASON` annotation added per mx-tag-protocol mandatory-fields. Six regression tests added covering all 6 acceptance scenarios (happy-path, replace-existing, perm-preservation, mid-write error, rename error, caller-level `CleanupUserSettings` regression). Run-phase verification: 7/8 ACs binary PASS + AC-AWR-007 CONDITIONAL (package coverage 77.6%; `atomicWrite` function 59.1% exceeds reference baseline `runtime/persist.go::atomicWrite` 50.0% by +9.1pp — deeper structural coverage requires the deferred `pkg/atomicio` mocking framework per EXCL-AWR-003 Tier L). Cross-platform PASS (macOS native, GOOS=windows amd64, GOOS=linux amd64). Race detector PASS. NEW lint=0 (delta-only). [65df2f7e0]
- Source SPEC: SPEC-V3R5-ATOMIC-WRITE-001 (Tier S, status `implemented` v0.2.0). Plan-auditor PASS 0.918 (Tier S threshold 0.75, margin +0.168). 1-pass success — Section A-E delegation template + W3 meta-analysis lessons.
