...
- **[SPEC-SEC-HARDEN-001](.moai/specs/SPEC-SEC-HARDEN-001/spec.md)** — Security & Concurrency hardening — 5 HIGH defects (Tier L, v0.1.0, run-phase 2026-06-13, sync-phase 2026-06-13). Five HIGH-severity security and concurrency defects across internal packages: **M1** (internal/permission) — command-chain shell-injection bypass via unquoted `:*` prefix-match (CWE-78); added `hasUnquotedShellSeparator` quote-aware scanner + guard on the `:*` branch (sync-audit follow-up **D1**: unterminated-quote bypass containment — an open quote at end-of-string now denies, conforming to the design's err-toward-deny invariant; `TestMatches_UnterminatedQuoteBypass` 8-case RED→GREEN, `f25772d1c`). **M2** (internal/permission) — conflict resolution tie-breaker reversed deny-now-wins on equal specificity (correct policy); added best-effort audit log to `.moai/logs/permission.log`. **M3** (internal/cli/worktree + internal/tmux) — credential argv leak on `moai worktree --team` GLM/CG path (CWE-214); `ANTHROPIC_AUTH_TOKEN` routes through `tmux.SessionManager.InjectSensitiveEnv(ctx, key, value) error` (NEW **single additive interface method**, §F.5 carve-out) instead of argv, no fallback on injection failure. **M4** (internal/lsp/hook) — LSP regression tracker data race under `RLock` write; converted to `Lock` in `GetBaseline()`. **M5** (internal/resilience) — circuit breaker half-open multi-entry + unrecovered goroutine panic crashing process; added `halfOpenInFlight` single-permit guard + `defer recover()` on transitionTo goroutine. 27/28 automated AC PASS + 1 OBSERVATIONAL RED (M5-004 panic-crash validated pre-fix by manual observation, post-fix automated via AC-SEC-M5-005); coverage M1 89.5% / M2 90.2% / M3 84.1% / M4 90.3% / M5 98.6%; cross-platform build (darwin / windows amd64) + `go test -race` green; golangci-lint 0. plan-auditor 0.91 PASS (Tier L threshold 0.85). 5 files modified: internal/permission/{stack,conflict}.go, internal/cli/worktree/tmux_integration.go, internal/tmux/session.go, internal/lsp/hook/tracker.go, internal/resilience/circuit.go + 5 per-milestone `*_sec_harden_test.go` + rewritten conflict_test.go. sync-auditor (Tier L 4-dimension skeptical audit) **PASS-WITH-DEBT 0.79** → D1 fixed (M1 Security dimension invariant-clean); D2 (shell redirect operators `>`/`>>`/`<`) and D3 (`${IFS}` word-split) documented as known lexical-scan limitations deferred to a fast-follow.
