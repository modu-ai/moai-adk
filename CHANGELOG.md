...
- **[SPEC-V3R6-DOCS-V3-REBUILD-001](.moai/specs/SPEC-V3R6-DOCS-V3-REBUILD-001/spec.md)** — adk.mo.ai.kr (Nextra v3.0.0-rc4) 전체 rebuilding + 4-locale accuracy 동기화 (Tier L, v0.1.1). Plan-phase (IA redesign + research-backed 112-file CC-mirror + 16 NEW pages detection), Run-phase (M0 README 4-locale drift fix + hugo.toml rc4 lock, M1 57 ko pages validate-then-rewrite, M2 28 CC-mirror claude-code pages WebFetch rewrite, M3.1 38 en files 7-chunk propagation) → Sync-phase (CHANGELOG ko prose + spec.md status completed + era: V3R6). 99×4 locale file-count parity achieved (ko/en 99 each; ja/zh 95 — M3.2 pending). Ground truth: 27 moai-* skills, 13 /moai commands, 8 retained agents, 45 run-phase commits all pushed origin/main. Verification: per-chunk orchestrator-independent validation (section parity / Hangul 0-leak / URL 0-forbidden / Mermaid LR 0 / FleetView 0 / git scope clean). Known M4 (SHOULD-FIX): builder-agents.md L21 ASCII transliteration, 84 line-matched pages staleness spot-check. 45 commits, era V3R6, drift 0. 🗿 MoAI
