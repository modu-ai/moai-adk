...
- **[SPEC-V3R6-SESSION-LEGACY-COVERAGE-001](.moai/specs/SPEC-V3R6-SESSION-LEGACY-COVERAGE-001/spec.md)** — `internal/session` 패키지 test coverage 보강 (Tier S minimal, run-phase commit `a095bce09`): baseline 77.7% → 85.8% (+8.1%p) 달성, zero 프로덕션 코드 변경 (test-only 2 artifact: CHANGELOG.md + 4 SPEC frontmatter sync 제외). **4개 test file 변경** (1 NEW + 3 EXTEND): `internal/session/hydrate_test.go` 신규 202L (22 test case, registry initialization + reader correlation + entry type-assertion), `internal/session/registry_test.go` +194L (5 coverage-gap test case, concurrent register/deregister + edge case), `internal/session/registry_lock_windows_test.go` +115L (7 coverage-gap test case, Windows platform lock=EACCES simulation), `internal/session/registry_lock_unix_test.go` +60L (3 coverage-gap test case, Unix EAGAIN retry + timeout). **Coverage improvement breakdown**: hydrate 0%→95.2% (+95.2%p, 21 NEW lines per 202 total = comprehensive new module), registry 63.2%→88.4% (+25.2%p, gap fill), lock_windows 0%→100% (+100%p, NEW platform gap), lock_unix 68.4%→84.2% (+15.8%p). **Cross-platform verification**: Windows amd64 `go build ./internal/session/...` ✓, Linux x86_64 race detector ✓, Darwin arm64 ✓. **AC verification** (7/7 mandatory PASS): AC-SLCO-001 coverage ≥85.0% PASS (현 85.8%) / AC-SLCO-002 zero production `.go` mutations PASS / AC-SLCO-003 `t.TempDir()` enforcement PASS (0 forbidden temp patterns) / AC-SLCO-004 race detector clean PASS / AC-SLCO-005 C-HRA-008 subagent boundary PASS (0 AskUserQuestion) / AC-SLCO-006 Windows cross-build PASS / AC-SLCO-007 OTEL t.Setenv ban PASS (0 OTEL env var setup). **Quality gates**: golangci-lint 0 NEW issues / go vet 0 / code review 0 findings. **Race absorption (L52 case 12 NEW)**: parallel session SPEC-V3R6-WORKFLOW-PLAN-GEARS-ALIGN-001 + SPEC-V3R6-FOUNDATION-CORE-GEARS-ALIGN-001 concurrent run-phase during M1+M2 commits; pre-spawn fetch `0 0` + path-specific add L46 + pre-commit staging assertion L59 disciplines 결과 0 cross-attribution leakage. **Plan-auditor verdict**: iter-1 PASS 0.88 (Tier S threshold 0.75, +0.13 margin, skip-eligible ≥0.90 미충족). **Tier classification**: S minimal (4 _test.go files + 2 SPEC artifacts CHANGELOG+frontmatter = 6 files, <5 file cap 준수, test-only scope ≤300 LOC guidance). 4 files +571/-0, run-phase 1-pass success. B12 13th CHANGELOG self-test PASS (grep count 1 match, AC count 7 match, 4 file paths verified).
