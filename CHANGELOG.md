...
## [Unreleased] — SPEC-V3R4-HARNESS-003: Embedding-Cluster Classifier (Tier-2 Pattern Aggregation Upgrade)

### Added

- **SPEC-V3R4-HARNESS-003 — Embedding-Cluster Classifier (Tier-2 Pattern Aggregation Upgrade)**: usage-log.jsonl의 패턴 집계 레이어에 Stage-2 SimHash 클러스터 분류기를 추가합니다. Wave A: `AggregatePatterns` 내 Stage-2 호출 seam + `ClassifierConfig` 구조체 stub + backward compatibility 보존 (`Stage2Enabled` 기본값 false). Wave B: 순수 Go Charikar SimHash 64-bit 지문 (`SimHash64`), Hamming 거리 (`bits.OnesCount64`), feature string 빌더 (PII guard: `PromptContent` 완전 제외). Wave C: Union-Find (path compression + union-by-rank) 기반 Hamming 클러스터링, EventType 파티셔닝, append-only JSONL 감사 로그 (`cluster-merges.jsonl`, `hamming_distances` CAP=20, `truncated` 필드). Wave D: `internal/config` 패키지에 `LearningConfig` + `ClassifierConfig` 독립 정의, `yaml.TypeError` fallback (REQ-HRN-CLS-018), `harness.yaml` `learning.classifier` 블록, 1k 이벤트 벤치마크 (~6.6ms/op), AC-HRN-CLS-009 end-to-end PII 검증, 5개 회귀 가드. 14/14 AC PASS, coverage 88.3% (harness) / 94.3% (safety), lint 0건, MX 6 tags. stdlib-only (외부 의존성 0건). 4-wave TDD (RED-GREEN-REFACTOR) delivery.

### Added (English)

- **SPEC-V3R4-HARNESS-003 — Embedding-Cluster Classifier (Tier-2 Pattern Aggregation Upgrade)**: Introduces a Stage-2 SimHash cluster classifier into the pattern aggregation layer of `usage-log.jsonl`. Wave A: Stage-2 call seam inside `AggregatePatterns` + `ClassifierConfig` struct stub + backward compatibility preserved (`Stage2Enabled` zero-value = false). Wave B: Pure-Go Charikar SimHash 64-bit fingerprint (`SimHash64`), Hamming distance via `bits.OnesCount64`, feature string builder with PII guard (complete exclusion of `PromptContent`). Wave C: Union-Find clustering (path compression + union-by-rank) with Hamming threshold, EventType partitioning, append-only JSONL audit log (`cluster-merges.jsonl`, `hamming_distances` CAP=20, `truncated` flag). Wave D: `LearningConfig` + `ClassifierConfig` defined independently in `internal/config` package, `yaml.TypeError` fallback (REQ-HRN-CLS-018), `harness.yaml` `learning.classifier` block, 1k-event benchmark (~6.6ms/op), AC-HRN-CLS-009 end-to-end PII validation, 5 regression guards. 14/14 AC PASS, coverage 88.3% (harness) / 94.3% (safety), 0 lint issues, 6 MX tags. Stdlib-only (0 external dependencies). 4-wave TDD delivery.
