...
- **[SPEC-COMPLETION-MARKER-RETIRE-001](.moai/specs/SPEC-COMPLETION-MARKER-RETIRE-001/spec.md)** — Retire dormant completion markers + persistent_mode subsystem (Tier M, v0.1.0, run-phase 2026-06-15, sync-phase 2026-06-15). Removes coupled dead/dormant completion-marker emission and the persistent-mode subsystem (`defaultCompletionMarkers` consts + `persistent_mode.go` package + `CompletionConfig`/`MarkersConfig` structs + marker-related YAML fields + 4-locale docs-site references). The moai loop replacement path now signals completion via natural-language sentence detection instead of synthetic marker tokens. **Design decision**: Option (a) adopted — drop marker emission entirely; replace `/moai loop` exit signal with explicit "Your harness has completed" or "Request completed" prose from the Agent output (observability preserved via loop-exit prose parsing). Run-phase milestones: M1 Go runtime removal (`defaultCompletionMarkers` const + persistent_mode.go deletion), M2 config struct/YAML cleanup (CompletionConfig removal), M3 test retirement (marker assertion count corrections), M4 output-style + template restyle (Completion* field drops), M5 docs-site 4-locale atomicity (en/ko/ja/zh marker reference removal), M6 traceability + close. 11/11 MUST-FIX AC PASS (AC-CMR-001..011); 2 SHOULD-FIX deferred (AC-CMR-012 loop output-surface test seam, AC-CMR-013 placeholder prose flexibility). Coverage loop 77.3% / moai-adk total 87.0% (no regression). Cross-platform build (darwin / windows amd64) exit 0. Subagent boundary (C-HRA-008) verified 0 AskUserQuestion invocations in `internal/loop` / `internal/output`. Closes the persistent-mode subsystem exit strategy (Option a).
