...
- **[SPEC-PROJECT-HARNESS-BRIDGE-001](.moai/specs/SPEC-PROJECT-HARNESS-BRIDGE-001/spec.md)** — `/moai project` interview clarity-scoring + machine-readable `harness-spec.yaml` bridge to harness generation (Tier M, v0.1.1, `.claude/skills/moai/workflows/project/*` + `.moai/config/sections/interview.yaml` — doc-only, no Go code, foundation SPEC of the 3-SPEC "Project-Harness Pipeline" Epic). **(1) Adaptive clarity-scored interviews** — Phase 0.3 (new-project, `mode-detection.md`) and Phase 1.5 (existing-project, `codebase-analysis.md`) interviews converted from a static fixed 3-round × 3-question sequence to a clarity-driven adaptive loop reusing the `plan/clarity-interview.md` 0-10 scale mechanism: `clarity_threshold: 4` (from `interview.yaml`) is the interview ENTRY floor, sufficiency exit at score ≥ 8 (skip remaining rounds), abandon at score ≤ 3, up to `project.max_rounds: 3`; each interview gained a new Round 4 (Verification, Surfaces, and Sharing) eliciting 4 extended axes — `verification` (test/e2e command), `ui_surface` (has-ui | headless), `external_systems` (DB/API/service list), `team_sharing` (solo | team-shared) — with the existing-project variant pre-filling axes it can infer from Phase 1 codebase analysis rather than re-asking. **(2) Machine-readable `harness-spec.yaml` bridge** — new Phase 3.2 in `doc-generation.md` reads `.moai/project/interview.md` (written by the Phase 0.3/1.5 interviews) and emits `.moai/project/harness-spec.yaml`, an 8-field schema (`domain`, `goal`, `constraints`, `scope`, `verification`, `external_systems`, `ui_surface`, `team_sharing`) with OVERWRITE re-run semantics matching `interview.md`; `meta-harness.md` Phase 5.1 and `harness-build-entry.md` Phase 1 now compose their harness-generation request from `harness-spec.yaml` + `product.md`/`structure.md`/`tech.md` instead of discarding the interview's structured answers and re-eliciting them. **(3) Config surface** — `.moai/config/sections/interview.yaml` gained a config-declared `project.additional_axes` block (the 4 extended-axis names + descriptions) consumed by both interview hosts. The NO-SPEC scope guard in `mode-detection.md` (prohibiting any write under `.moai/specs/`) is explicitly preserved and re-verified — `harness-spec.yaml` is written only to `.moai/project/`. **14/14 AC PASS** (acceptance.md SSOT; all doc-only grep/diff/`make build`/`moai init` sandbox verification per plan-phase design — test-coverage-style ACs are N/A, no Go code touched). Template-First byte-parity verified across 6 file-pairs (5 skill files + interview.yaml, local↔template `diff -q` PARITY OK ×6), `make build` exit 0, `grep -rn SPEC-PROJECT-HARNESS-BRIDGE-001 internal/template/templates/` = 0 (neutrality clean). Run-phase M1-M4 landed as a consolidated commit (`9f1f0f53b`) executed in a Claude Code L1 worktree and integrated to `main` by the orchestrator, plus a `sync_commit_sha`-adjacent SHA backfill in progress.md (`28985a492`), Mode 5 sub-agent sequential (interdependent Template-First mirror-parity milestones). Cross-platform build green (`go build ./...` + `GOOS=windows GOARCH=amd64 go build ./...` exit 0), `go test ./internal/template/...` green; whole-repo `go test ./...` has one PRE-EXISTING unrelated flake (`TestDoctorPermission_NoMatchTrace` in `internal/cli`, shared cobra global-state parallel-contention — passes deterministically in isolation; changeset is 12 markdown/yaml files, zero Go, `internal/cli` untouched). README.md / docs-site 4-locale sync explicitly OUT OF SCOPE per spec.md §Out of Scope — deferred follow-up (no user-facing CLI surface change; this SPEC touches only internal `/moai project` workflow skill prose + config). Sync-phase 3-phase close (this commit — `in-progress → implemented → completed` merged into the single sync commit + `updated:` refresh). 🗿 MoAI
