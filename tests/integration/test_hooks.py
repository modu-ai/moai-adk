...
sys.path.insert(
    0,
    os.path.join(
        os.path.dirname(__file__), "..", "src", "templates", ".claude", "hooks", "moai"
    ),
)
...
class TestDevelopmentGuideGuard(unittest.TestCase):
    ...
    def test_simplicity_check_basic(self):
        ...
        # DevelopmentGuideGuard import 시도
        try:
            sys.path.insert(
                0,
                os.path.join(
            ...
            from constitution_guard import DevelopmentGuideGuard

            guard = DevelopmentGuideGuard(self.test_dir)
            ...
            self.skipTest("DevelopmentGuideGuard not available")
