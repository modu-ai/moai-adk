...
class TestQuestionReduction:
    ...
    def test_total_questions_in_tab_1(self):
        ...
        tab1 = schema['tabs'][0]
        total = sum(
            len(batch['questions']) for batch in tab1['batches']
        )
        assert total == 10
    ...
    def test_question_reduction_rate(self):
        ...
        schema = load_tab_schema()
        tab1_questions = sum(
            len(batch['questions']) for batch in schema['tabs'][0]['batches']
        )
...
class TestAtomicSaving:
    ...
    def test_config_save_creates_backup(self):
        ...
        with patch.object(manager, '_create_backup') as mock_backup, \
             patch.object(manager, '_write_config'):
        ...
        for tab in schema['tabs']:
            for batch in tab.get('batches', []):
                ...
                assert len(questions) <= 4, \
                    f"Batch {batch['id']} has {len(questions)} questions"
        ...
        for tab in schema['tabs']:
            for batch in tab.get('batches', []):
                ...
                assert len(header) <= 12, \
                    f"Header '{header}' is {len(header)} chars"
            ...
            for batch in tab.get('batches', []):
                for question in batch.get('questions', []):
                    ...
                    assert 2 <= len(options) <= 4, \
                        f"Question {question['id']} has {len(options)} options"
