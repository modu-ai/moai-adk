"""
Comprehensive test coverage for MoAI task detector.
...
class TestMoAIDetectorInitialization:
    """Test MoAIDetector initialization and setup."""
...
class TestActiveTaskDetection:
    ...
    def test_detect_active_task_returns_moai_task(self):
        """Test that detect_active_task returns MoaiTask instance."""
        # Arrange
        detector = MoAIDetector()

        with patch.object(detector, "_is_cache_valid", return_value=False):
            with patch.object(detector, "_read_session_state") as mock_read:
                mock_read.return_value = MoaiTask(command="test_command", spec_id="SPEC-001", stage="RED")

                # Act
                result = detector.detect_active_task()

        # Assert
        assert isinstance(result, MoaiTask)
    ...
    def test_detect_active_task_uses_cache_when_valid(self):
        ...
        # Arrange
        detector = MoAIDetector()
        cached_task = MoaiTask(command="cached_command", spec_id="SPEC-999", stage="REFACTOR")
    ...
    def test_detect_active_task_reads_when_cache_invalid(self):
        ...
        # Arrange
        detector = MoAIDetector()
        fresh_task = MoaiTask(command="fresh_command", spec_id="SPEC-002", stage="GREEN")
    ...
    def test_detect_active_task_updates_cache(self):
        ...
        # Arrange
        detector = MoAIDetector()
        fresh_task = MoaiTask(command="fresh", spec_id="SPEC-003", stage="SYNC")
