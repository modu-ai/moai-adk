...
class TestTagMigrationTool:
    ...
    def test_should_validate_data_integrity_during_migration(self):
        ...
        corrupted_json_data = {
            ...
            "index": {
                ...
                "INVALID:CATEGORY-001": [  # 존재하지 않는 카테고리
                    {"file": "test.md", "line": 1, "context": "@INVALID:CATEGORY-001"}
                ],
                "REQ:DUPLICATE-TAG": [
                    {"file": "file1.md", "line": 10, "context": "@REQ:DUPLICATE-TAG"},
                    {"file": "file2.md", "line": 10, "context": "@REQ:DUPLICATE-TAG"},
                ],
            },
            "references": {
        ...
        migration_result = self.migration_tool.migrate_json_to_sqlite(
            validate_data=True, strict_mode=True
        )
        ...
        assert (
            not self.temp_db.exists()
            or self.migration_tool.get_database_tag_count() == 0
        )
    ...
    def test_should_handle_large_dataset_migration_with_progress_reporting(self):
        ...
        for i in range(2000):
            ...
            if i % 10 == 0 and i > 0:
                prev_tag = (
                    f"{categories[(i - 1) % len(categories)]}:LARGE-DATASET-{i - 1:05d}"
                )
        ...
        assert total_time < 30.0, (
            f"마이그레이션 시간 {total_time:.2f}초가 목표 30초를 초과"
        )
    ...
    def test_should_create_and_restore_backups_during_migration(self):
        ...
        backup_info = backup_manager.create_backup(
            json_path=self.temp_json, description="마이그레이션 전 백업"
        )

        # 마이그레이션 중 의도적 실패 시뮬레이션
        with patch.object(
            self.migration_tool, "_validate_migration_result"
        ) as mock_validate:
            mock_validate.side_effect = Exception("마이그레이션 검증 실패")

            migration_result = self.migration_tool.migrate_json_to_sqlite(
                create_backup=True, auto_rollback=True
            )
    ...
    def test_should_support_incremental_migration_for_updates(self):
        ...
        initial_json = {
            ...
            "index": {
                "REQ:INITIAL-001": [
                    {"file": "req.md", "line": 1, "context": "@REQ:INITIAL-001"}
                ],
                "DESIGN:INITIAL-001": [
                    {"file": "design.md", "line": 1, "context": "@DESIGN:INITIAL-001"}
                ],
            },
        ...
        updated_json = {
            ...
            "index": {
                ...
                "TASK:NEW-001": [
                    {"file": "task.md", "line": 1, "context": "@TASK:NEW-001 새 작업"}
                ],
                "TEST:NEW-001": [
                    {"file": "test.md", "line": 1, "context": "@TEST:NEW-001 새 테스트"}
                ],
            },
            "references": {
        ...
        incremental_result = self.migration_tool.migrate_json_to_sqlite(
            mode="incremental", preserve_existing=True
        )
    ...
    def test_should_provide_detailed_migration_statistics_and_reporting(self):
        ...
        migration_result = self.migration_tool.migrate_json_to_sqlite(
            detailed_reporting=True, generate_report=True
        )
    ...
    def test_should_support_custom_migration_plugins(self):
        ...
        plugin_test_json = {
            ...
            "index": {
                "CUSTOM:CUSTOM-TAG-001": [
                    {
                        "file": "custom.md",
                        "line": 1,
                        "context": "@CUSTOM:CUSTOM-TAG-001 커스텀 TAG",
                    }
                ],
                "REQ:NORMAL-TAG-001": [
                    {
                        "file": "normal.md",
                        "line": 1,
                        "context": "@REQ:NORMAL-TAG-001 일반 TAG",
                    }
                ],
                "CUSTOM:SHORT": [
                    {
        ...
        migration_result = self.migration_tool.migrate_json_to_sqlite(
            plugins=[custom_processor], strict_validation=True
        )
