...
class TestCheckOption:
    ...
    def test_check_shows_update_available(self, runner, mock_project_dir):
        ...
        with patch('moai_adk.cli.commands.update._get_current_version') as mock_current, \
            ...
            assert ("Update available" in result.output or
                    "update available" in result.output.lower() or
                    "0.6.1" in result.output and "0.6.2" in result.output)
    ...
    def test_check_shows_already_updated(self, runner, mock_project_dir):
        ...
        with patch('moai_adk.cli.commands.update._get_current_version') as mock_current, \
            ...
            assert result.exit_code == 0
            assert ("up to date" in result.output.lower() or
                    "Already" in result.output or
                    "already" in result.output)
