...
class TestMetadataCompliance:
    ...
    REQUIRED_FIELDS = ['name', 'description', 'version', 'modularized', 'last_updated', 'allowed-tools', 'compliance_score']
    RECOMMENDED_FIELDS = ['modules', 'dependencies', 'deprecated', 'successor', 'category_tier',
                          'auto_trigger_keywords', 'agent_coverage', 'context7_references', 'invocation_api_version']
    ...
    def test_consolidates_multiple_source_skills(self, skill_metadata):
        ...
        assert 'Consolidates' in description or 'consolidate' in description.lower() or \
               'SPEC' in description or 'spec' in description.lower(), \
...
class TestProgressiveDisclosure:
    ...
    def test_has_implementation_guide_section(self, skill_content):
        ...
        assert any(section in skill_content for section in sections), \
               "Must have core implementation section"
    ...
    def test_has_advanced_section(self, skill_content):
        ...
        assert any(section in skill_content for section in sections), \
               "Must have advanced patterns section"
...
class TestModuleStructure:
    ...
    def test_expected_modules_present(self, skill_path, skill_metadata):
        ...
        if skill_metadata.get('modularized', False):
            ...
            assert len(module_files) >= len(expected_modules), \
    ...
    def test_modules_have_valid_content(self, module_files):
        ...
        for module in module_files:
            ...
            assert re.search(r'^#{1,6} ', content, re.MULTILINE), \
                   f"Module {module['name']} missing headers"
            # Each module should have meaningful content
            assert len(content) > 100, \
                   f"Module {module['name']} too short (<100 chars)"
...
class TestConsolidationValidation:
    ...
    def test_no_function_loss_indicated(self, skill_metadata):
        ...
        assert not skill_metadata.get('deprecated', False), \
               "Consolidated skill itself should not be deprecated"
...
class TestAgentCoverage:
    ...
    def test_agents_are_valid_format(self, skill_metadata):
        ...
        for agent in agents:
            ...
            assert agent.startswith('spec-') or agent.startswith('implementation-') or \
                   agent.startswith('plan') or agent.startswith('tdd-'), \
