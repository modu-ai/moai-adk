...
`/alfred:9-update` 커맨드를 사용하여 MoAI-ADK 패키지와 템플릿을 안전하게 업데이트합니다.
...
- **❌ Critical (치명적)**:
  ...
  권장 조치:
  1. 롤백 (권장): moai restore .moai-backup/2025-10-11-15-30-00
  2. 무시하고 진행 (위험)
...
| 업데이트 유형 | 호환성 | 설명 | 예시 |
|------------|-------|------|------|
| **Patch** | 완전 호환 | 버그 수정, 문서 개선 | v0.2.17 → v0.2.18 |
...
4. **Phase 3 종료 후 보고**
   ...
   선택:
   1. 재시도 (실패한 파일만)
   2. 백업 복원 (moai restore .moai-backup/2025-10-11-15-30-00)
   3. 무시하고 진행 (권장하지 않음)
   ```
