...
## title: EARS 语法详细指南 description: 掌握 MoAI-ADK 中使用的 EARS（Easy Approach to Requirements Syntax）语法，编写清晰、结构化的需求文档

# EARS 语法详细指南

EARS（Easy Approach to Requirements Syntax）是一种结构化的需求描述语法，它提供了 5 种清晰的模式来描述不同类型的需求。在 MoAI-ADK 中，EARS
语法是 SPEC 文档的核心组成部分，确保需求描述的一致性和可测试性。
