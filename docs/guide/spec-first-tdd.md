...
MoAI-ADK는 SPEC 우선 TDD 방법론을 엄격히 따릅니다. 이 가이드는 EARS 방법론을 활용한 명세 작성부터 다중 언어 TDD 구현, 고급 리팩토링 기법까지 체계적으로 다룹니다.
...
```mermaid
graph TD
    A["RED: 실패하는 테스트 작성"] --> B{"테스트 실패?"}
    B -->|Yes| C["GREEN: 최소 구현"]
    B -->|No| A
    C --> D{"테스트 통과?"}
    D -->|No| C
    D -->|Yes| E["REFACTOR: 코드 개선"]
    E --> F{"테스트 통과?"}
    F -->|No| E
    F -->|Yes| G["커밋"]
    G --> H["다음 기능"]
...
- 시스템은 감사 로그를 기록해야 한다
- 시스템은 다국어 지원을 제공해야 한다
- 시스템은 접근성 표준(WCAG 2.1)을 준수해야 한다
- 시스템은 RESTful API를 제공해야 한다
- 시스템은 실시간 알림 기능을 지원해야 한다
- 시스템은 데이터 백업 기능을 제공해야 한다
- 시스템은 사용자 프로필 관리를 지원해야 한다
- 시스템은 검색 기능을 제공해야 한다
- 시스템은 파일 업로드/다운로드를 지원해야 한다
- 시스템은 사용자 권한 관리를 제공해야 한다
- 시스템은 이메일 알림 기능을 지원해야 한다
- 시스템은 API 버전 관리를 제공해야 한다
- 시스템은 헬스 체크 엔드포인트를 제공해야 한다
```
...
- WHEN 3회 연속 로그인 실패 시, 시스템은 계정을 15분간 잠금 처리해야 한다
- WHEN 새 사용자가 가입하면, 시스템은 환영 이메일을 발송해야 한다
- WHEN 결제가 완료되면, 시스템은 주문 확인 알림을 보내야 한다
- WHEN 재고가 임계값 이하로 떨어지면, 시스템은 관리자에게 알림해야 한다
- WHEN 파일 업로드가 완료되면, 시스템은 바이러스 스캔을 실행해야 한다
- WHEN 비밀번호 재설정 요청 시, 시스템은 인증 링크를 이메일로 발송해야 한다
- WHEN API 요청이 실패하면, 시스템은 재시도 로직을 실행해야 한다
- WHEN 데이터베이스 연결이 끊기면, 시스템은 자동으로 재연결을 시도해야 한다
- WHEN 사용자가 프로필을 업데이트하면, 시스템은 변경 이력을 기록해야 한다
- WHEN 구독이 만료되면, 시스템은 갱신 알림을 발송해야 한다
- WHEN 보안 위협이 감지되면, 시스템은 관리자에게 즉시 알림해야 한다
- WHEN 대량 데이터 처리가 시작되면, 시스템은 진행 상태를 업데이트해야 한다
- WHEN 시스템 오류가 발생하면, 시스템은 오류 리포트를 생성해야 한다
```
...
- WHILE 무료 체험 기간일 때, 시스템은 제한된 기능만 제공해야 한다
- WHILE 관리자 모드일 때, 시스템은 모든 사용자 데이터 접근을 허용해야 한다
- WHILE 오프라인 모드일 때, 시스템은 로컬 캐시 데이터를 사용해야 한다
- WHILE 디버그 모드일 때, 시스템은 상세한 로그를 출력해야 한다
- WHILE 읽기 전용 모드일 때, 시스템은 모든 쓰기 작업을 거부해야 한다
- WHILE 베타 테스트 기간일 때, 시스템은 실험적 기능을 표시해야 한다
- WHILE 고부하 상태일 때, 시스템은 요청 처리를 제한해야 한다
- WHILE 데이터 마이그레이션 중일 때, 시스템은 새 요청을 대기열에 추가해야 한다
- WHILE 세션이 활성화된 상태일 때, 시스템은 활동을 추적해야 한다
- WHILE 프로덕션 환경일 때, 시스템은 최소한의 로그만 기록해야 한다
- WHILE 테스트 환경일 때, 시스템은 모의 데이터를 사용해야 한다
- WHILE 재해 복구 모드일 때, 시스템은 백업 서버로 트래픽을 라우팅해야 한다
```
...
- WHERE 클라우드 스토리지가 연동되면, 시스템은 자동 동기화를 제공할 수 있다
- WHERE API 키가 제공되면, 시스템은 외부 서비스 통합을 지원할 수 있다
- WHERE 소셜 로그인이 설정되면, 시스템은 OAuth2 인증을 제공할 수 있다
- WHERE 알림 설정이 활성화되면, 시스템은 푸시 알림을 보낼 수 있다
- WHERE 고급 검색 모드가 선택되면, 시스템은 복잡한 쿼리를 지원할 수 있다
- WHERE 자동 저장이 활성화되면, 시스템은 주기적으로 변경사항을 저장할 수 있다
- WHERE 협업 모드가 활성화되면, 시스템은 실시간 공동 편집을 지원할 수 있다
```
...
- 파일 업로드 크기는 10MB를 초과하지 않아야 한다
- 데이터베이스 쿼리 응답 시간은 500ms 이하여야 한다
- 동시 접속자 수는 10,000명을 지원해야 한다
- 시스템 가동 시간은 99.9% 이상이어야 한다
- 데이터는 암호화된 상태로 전송되어야 한다
- 사용자 비밀번호는 평문으로 저장되지 않아야 한다
```
