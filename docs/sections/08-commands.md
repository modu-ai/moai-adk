...
| 명령어              | 기능                      | 설명                                    |
| ------------------- | ------------------------- | --------------------------------------- |
| `/awesome:1-commit` | **한국어 커밋 메시지** 📝 | Git 변경사항을 분석하여 **한국어 커밋 메시지**로 자동 커밋 |
| `/util:commit`      | 영어 커밋 메시지          | Git 변경사항을 분석하여 영어 커밋 메시지로 자동 커밋     |
...
## awesome:1-commit - 한국어 커밋 메시지 생성

**🇰🇷 한국어 커밋 메시지 전용 도구**로 모든 커밋 메시지를 한국어로 자동 생성합니다.

### 주요 특징

- **🔤 한국어 설명**: 모든 커밋 메시지 제목과 본문을 한국어로 작성
- **📋 스마트 분석**: Git 변경사항을 논리적으로 그룹화하여 여러 커밋으로 분할
- **✨ 이모지 지원**: Conventional Commit 형식에 이모지 추가
- **🛡️ 안전 장치**: 민감 정보 자동 감지 및 차단

### 사용법

```bash
# 기본 사용 - 모든 변경사항 한국어 커밋
/awesome:1-commit

# 특정 파일만 한국어 커밋
/awesome:1-commit src/ docs/

# 미리보기 모드 (실제 커밋하지 않음)
/awesome:1-commit --dry-run

# 단일 커밋으로 강제 처리
/awesome:1-commit --single "전체 변경사항 통합"
```

### 한국어 커밋 메시지 예시

```bash
✨ feat(auth): 사용자 인증 모듈 추가
🐛 fix(api): 토큰 만료 처리 로직 수정
📝 docs: API 사용법 가이드 업데이트
♻️ refactor(core): 데이터베이스 연결 구조 개선
🔧 chore: 프로젝트 의존성 버전 업데이트
```

### 영어 버전과의 차이점

| 구분 | `/awesome:1-commit` | `/util:commit` |
|------|---------------------|----------------|
| **언어** | 🇰🇷 한국어 필수 | 🇺🇸 영어 |
| **설명** | "사용자 인증 모듈 추가" | "add user authentication system" |
| **대상** | 한국 개발팀 | 글로벌 팀 |
| **정책** | 모든 텍스트 한국어 강제 | 영어 표준 |
