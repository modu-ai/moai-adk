...
install_binary() {
    ...
    # Setup Claude Code settings for Agent Teams support
...
setup_claude_settings() {
    ...
    local backup_file="$HOME/.claude/settings.json.moai-backup"
    ...
    if command -v jq &> /dev/null; then
        # Use jq to add/update settings
        # Add env variables
        jq --arg key "ENABLE_TOOL_SEARCH" \
           --arg value "1" \
           '.env[$key] = $value' \
           "$settings_file" > "$settings_file.tmp" && mv "$settings_file.tmp" "$settings_file"

        jq --arg key "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS" \
           --arg value "1" \
           '.env[$key] = $value' \
           "$settings_file" > "$settings_file.tmp" && mv "$settings_file.tmp" "$settings_file"
        ...
        # Add teammateMode
        jq '.teammateMode = "auto"' \
           "$settings_file" > "$settings_file.tmp" && mv "$settings_file.tmp" "$settings_file"

        print_success "Added Agent Teams settings to Claude Code settings"
    else
        # Fallback: manual JSON editing using sed
        # This is a simple fallback that adds the settings to an existing JSON file
        ...
        # Add env variables (after "env": {)
        if ! grep -q '"ENABLE_TOOL_SEARCH"' "$settings_file"; then
            sed -i.bak '/"env": {/a\    "ENABLE_TOOL_SEARCH": "1",' "$settings_file"
            rm -f "$settings_file.bak"
        fi

        if ! grep -q '"CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS"' "$settings_file"; then
            sed -i.bak '/"ENABLE_TOOL_SEARCH"/a\    "CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS": "1"' "$settings_file"
            rm -f "$settings_file.bak"
        fi
