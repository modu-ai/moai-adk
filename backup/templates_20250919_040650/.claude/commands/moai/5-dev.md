---
description: 코드 구현
argument-hint: <TASK-ID|SPEC-ID|"all"> [--coverage-target=80] [--force-tdd]
...
### 기능 개요
- `.moai/specs/` 디렉터리의 모든 작업 분해된 SPEC(tasks.md 존재)을 자동 감지
- 각 태스크별로 Red-Green-Refactor 구현을 병렬 실행 (**최대 10개 동시 실행**)
- **code-generator + test-automator 에이전트**가 의존성 없는 구현 작업을 병렬 분산 처리
- TDD 사이클을 보장하며 전체 커버리지 통합 보고서 생성

### 실행 과정
```mermaid
flowchart TD
    A[/moai:5-dev all 실행] --> B[SPEC 디렉터리 스캔]
    B --> C[tasks.md가 있는 SPEC 선별]
    C --> D[Ready 상태 태스크 추출]

    D --> E[의존성 분석 및 병렬 그룹 생성]
    E --> F[각 태스크별 code-generator + test-automator 에이전트 호출]
    F --> G[병렬 실행: TDD-001, TDD-002, TDD-003...]
    G --> H[RED-GREEN-REFACTOR 사이클 강제]
    H --> I[커버리지 통합 및 전체 완료 보고]
```
...
처리된 태스크 목록:
├── SPEC-001: 12개 태스크 → 구현 완료 ✅ (커버리지: 89%)
├── SPEC-002: 18개 태스크 → 구현 완료 ✅ (커버리지: 92%)
├── SPEC-003: 8개 태스크 → 구현 완료 ✅ (커버리지: 85%)
├── SPEC-004: 15개 태스크 → 구현 완료 ✅ (커버리지: 87%)
└── SPEC-005: 10개 태스크 → 구현 완료 ✅ (커버리지: 91%)

TDD 사이클 통계:
- 총 태스크 수: 63개
- RED 단계 완료: 63개 (100%)
- GREEN 단계 완료: 63개 (100%)
- REFACTOR 단계 완료: 63개 (100%)
- 전체 커버리지: 88.7% (목표: 80% 달성)

총 처리 시간: 18분 32초
병렬 처리 효과: 단일 처리 대비 80% 시간 단축 (최대 10개 동시 실행)
전문 에이전트 활용: code-generator + test-automator가 독립적으로 각 태스크 구현

다음 단계:
> /moai:6-sync  # 전체 문서 동기화 및 TAG 인덱스 업데이트
```

### 고급 사용법
```bash
# 높은 커버리지 목표
/moai:5-dev all --coverage-target=95
...
# 강제 TDD 모드
/moai:5-dev all --force-tdd
```

### 의존성 관리
- **순차 실행**: 의존성이 있는 태스크는 자동으로 순차 처리
- **병렬 실행**: 독립적인 태스크는 최대 10개까지 동시 처리
- **자동 대기**: 의존성 태스크 완료 시까지 후속 작업 대기
