...
### 언어 및 운영 원칙
- 모든 대화/문서/PR은 한국어로 작성
- 변경은 작고 안전하게. 가정과 대안 비교(최소 2가지)는 PR 본문에 기록
...
### Pull Request 규칙
- 브랜치/제목/리뷰 흐름은 Git 워크플로우(@.claude/memory/git_workflow.md)를 따른다
- PR 설명과 체크는 공통 체크리스트(@.claude/memory/shared_checklists.md) 기반으로 관리한다
- 필요한 경우 팀별 템플릿을 별도 파일로 만들어 임포트(예: @docs/pr-template.md)
