...
- 모든 명령은 Constitution 5원칙과 16-Core TAG 추적성을 준수하도록 설계되었습니다.
...
- `.moai/memory/constitution.md`: 범용 헌법 (기본 `simplicity_threshold = 5`, Waiver 제도 포함)
- `.claude/settings.json`: `defaultMode = acceptEdits`, 고위험 작업은 ask/deny로 분리
- `.moai/config.json`: Personal/Team Git 전략, 체크포인트 정책, 헌법 임계값 관리

**헌법 5원칙 요약**
1. **Simplicity** – 권장 모듈 ≤ 5, 예외 시 Waiver 기록
2. **Architecture** – 라이브러리/계층 분리, DIP 준수
3. **Testing** – TDD + 커버리지 ≥ 85%
4. **Observability** – 구조화 로깅, PII 마스킹, 감사 추적
5. **Versioning** – MAJOR.MINOR.BUILD, 추적 가능한 Git 이력
