...
- 픽스처는 합성 이벤트다. 실제 `go test -json`이 내는 패키지 수준 pass 이벤트의 필드 구성(예: `Output` 없이 `Elapsed`만)과 다를 수 있다. 추가한 이벤트는 기존 `fail` 패키지 이벤트와 같은 모양을 따랐다.
- 픽스처 하나로 모든 변이를 구별하는 것은 원리적으로 불가능하다. 이번 보강은 감사가 찾은 두 변이를 닫았을 뿐이다.
