#!/bin/bash
# t541 reproduction prototype — NOT doctrine. Surfaces cross-artifact ordering candidates
# between a plan's milestone order and an acceptance file's ordering clauses.
# usage: order-verb-prototype.sh <plan.md> <acceptance.md>
plan="$1"
acc="$2"
...
awk '
  ...
  { sub(/\r$/, "") }
  FILENAME == ARGV[1] {
    if (match($0, /^#+/) && substr($0, RLENGTH + 1) ~ /^[ \t]+M[0-9]+([^0-9]|$)/) {
