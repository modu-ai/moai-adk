...
>
> **정정 2 (plan-audit D5 수용)**: `version` 키 실패를 "조용히/경고 없이"로 적은 서술도 **틀렸다**.
> `--json` 스트림에 파일·필드·행·열을 명시한 파싱 오류가 실린다. 내가 error 아이템을 읽지 않고
> 단정했다. **실제로 무음인 것은 프로젝트 레벨 미발화(§3.2)뿐**이다.
...
### 3.1 최상위 `version` 키를 넣으면 파일 전체가 죽는다
...
- ~~`"matcher": "*"` 가 원인으로 보인다(추정)~~ **→ 3차 실측에서 철회.** matcher 는 `"*"`·`"Bash"` 둘 다
  정상 발화한다. **진짜 원인은 최상위 `"version": 1` 키**로, 파일 전체를 무력화한다(단일 변수 A/B).
  무음은 아니다 — `--json` 스트림에 `unknown field \`version\`, expected \`description\` or \`hooks\`` 오류가 실린다.
  확정 근거: `precondition-measurement-round3.md` §4

**M0 §1 의 "미지의 이벤트 이름은 조용히 무시된다"보다 넓은 실패 양식이다**: 알 수 없는 최상위 키는 그
항목이 아니라 파일 전체를 죽인다. M4 생성기의 화이트리스트 검증은 **이벤트 이름뿐 아니라 필드 집합까지**
덮어야 한다. 허용 최상위 키는 오류 메시지가 명시한 `description`·`hooks` 둘뿐이다.
...
3. **설정 형태 검증이 M3/M4 의 실질 위험**이다 — 프로젝트 레벨 미발화는 무음이므로, 어댑터가 만든 배선이
