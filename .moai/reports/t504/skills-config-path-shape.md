...
### RQ1 — `[[skills.config]]`는 로드되는가 → **DEAD (load-ignored)**
...
동일. R 셀 전 사본(`lab/backup-config.toml`)은 만들어졌으나 복원이 일어날 일이 없었다(무효 확인 no-op). 실험 전체에서 실사용자 `~/.codex`에 기록한 셀은 없다(격리 셀 6+3건은 전부 스크래치 홈, R만 읽기).
...
- **t506 (유령 49건 정리)**: 삭제가 스킬 로딩에 미치는 영향 = **0** (로드 무시 표면이므로). 삭제는 순수한 서류 정리다. 단 삭제 시 `enabled` 필드 보유 엔트리만 골라 안전하게 지우면 되고, 반대로 **`enabled`가 없는 엔트리를 새로 쓰는 사고가 생기면 그 사용자의 codex 전체가 깨진다**(F1) — 정리 도구는 이를 유지해야 한다.
