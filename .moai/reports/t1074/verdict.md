...
The factory-only logical-lane/current-endpoint broker is locally verified by AC-FMH-001 through AC-FMH-010 and AC-FMH-015. The overall SPEC verdict remains **FAIL** because AC-FMH-011 through AC-FMH-014 require Claude subscription turns that are currently unavailable.
...
- Codex live log: `ac10.jsonl` records separate real Codex contexts, distinct live owner identities, the nonce/receipt exchange with payload omitted here, `acknowledged=2`, and a passing exact JQ predicate.
...
| AC-FMH-010 | PASS | `ac10.jsonl`: distinct Codex owner PIDs, two acknowledgements, exact gate `true` |
