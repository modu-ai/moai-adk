...
1. **"Wrong test name" — RETRACTED.** The original finding claimed `TestCatalogHashParity` "does not exist"; that claim was an UNSCOPED absence assertion: the lane probed only `./internal/template/`, where the name is indeed absent, and overreached from that to non-existence. The lead's correction (measured on the develop tree) locates it at `internal/spec/catalog_hash_test.go:108` — a DIFFERENT package — and the lane has since re-run `go test ./internal/spec/ -run 'TestCatalogHashParity'` in a worktree at develop `ddc3be6fa` (pre-t441) and observed it FAIL (on entry `moai-ref-seo`; the drift set differs per tip and per absorbed base — at this branch's base `9b89b8c5b` the internal/spec set was 35 entries and the internal/template entry was `sync-auditor`). Both guards named by the executor and the lead are real and both fail on stale catalog entries: `TestCatalogHashParity` (internal/spec) and `TestManifestHashFormat` (internal/template, `CATALOG_HASH_UNSTABLE: sync-auditor stored f1b4487f… ≠ computed 545d03d9…`, observed this run). Lesson recorded per verification-claim-integrity §1.1 surface 3: an absence claim names its sweep scope or it is a hypothesis.
