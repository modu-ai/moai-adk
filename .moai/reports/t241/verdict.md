...
**C1~C4 는 운영자 승인으로 발행됐다**(2026-08-28): C1 → **t341**, C2 → **t342**, C3 → **t343**, C4 → **t344**. t342 에는 아래 면제 판별식이 [HARD] 조건으로 실렸고, 이 판정의 Gaps 에 적은 탐지 한계(grep 형태로는 `origin/` 없이 표현된 미고정 주장을 못 잡는다)도 산출물에 명시하라는 조건이 붙었다. t344 에는 착수 전 §A.5 의 현재 문면을 읽고 형식 일반으로 올릴지 이미 닫힌 것으로 볼지 먼저 판정하라는 단서가 붙었다.

**C5 는 주인이 없다** — 아래 행의 사유대로 t333 이 덮지 않으며, 다음 라운드 운영자 판정 대기다.
