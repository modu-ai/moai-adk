...
**이 정정이 잡힌 경로를 남긴다** — 판정 방법 자체가 이 카드의 대상이기 때문이다. 코퍼스를 잘못 연 것은 판정자이고, 서로 모르는 채 훑던 수집 레인 **둘이 각각** 그것을 잡아 반환했다. 판정자는 그 지적을 받아들이기 전에 `git log -1 --format="%h authored=%ad committed=%cd"` 를 직접 돌려 재현했고, 그 뒤에 5건을 뺐다. 수집 레인의 지적도 검증 대상이라는 것이 이 경로의 요지다 — 같은 라운드에서 감사관의 판정 하나가 실측으로 뒤집힌 사례도 있다(t303 F5: 인접 CHANGELOG 항목이 마커를 달고 있다는 주장이 거짓).
...
| C4 | 장부 지표 결함 | 예측을 "감사 지적 0건" 으로 쓰면 규칙이 잘 들을수록 성적이 나빠 보인다. 다음 장부는 **채택까지 생존 건수**를 예측으로 쓸 것 | **t333 이 채택함** — 발생/채택생존 표를 SPEC 에 싣고, 모든 발현 기대치가 둘 중 무엇을 재는지 명시하도록 [HARD] 로 박았다(lane-17 회신, 2026-08-28) |
| C5 | 규칙의 발현 자체 | 이 판정이 보여준 것은 정책 층 규칙에 발현 관측이 없다는 것이다. 규칙이 인용되는지·적용되는지를 무엇이 확인하는가 | **t333 산출물에는 들어가지 않는다 — 카드가 여전히 필요하다.** lane-17 이 산출물을 CI 가드로 유지하고(정책 층까지 넓히면 Tier M 이 부풀고, 워크플로에는 실행 기록이 있는데 규칙에는 없어 기전이 다르다) C5 는 자기 `## Out of Scope` 에 후속 후보로 이름만 싣는다. 다만 (a) 의 기대치 저장 형태를 감시 대상 하드코딩이 아니라 **데이터**로 요구해, 두 번째 종류를 나중에 담을 수 있게 열어뒀다 |
