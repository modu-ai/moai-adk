...
- 독립 감사(`audit.md` F2)가 측정한 바로는 `C\:/` 계열 규칙이 CC 2.1.283에서 `\:` 를 역슬래시 그대로 맞춘다. 그래서 실제 `C:/` 명령은 구·신 어느 형태로도 막히지 않는다. 이 카드는 범위를 보존했을 뿐 그 결함을 고치지 않았다. `del`, `rmdir`, `Remove-Item` 형제 규칙도 같은 문제일 것으로 추정되며, 재지 않았다.
