...
> **[캡처 시점 주의 — sync-audit F1]** 이 절의 diff 출력과 문서별 증감 표는 **§④-b의 되돌리기 이전**에 뜬 것이며 트리 핀이 없다. 그래서 표시된 `+` 줄 중 8줄(전부 fold 판정 단위의 산문)은 배포된 문서에 존재하지 않고, 문서별 줄 수도 현재 트리와 어긋난다(예: entry-points 135 표기 / 104 실측). 캡처 당시에는 참인 출력이었고 제거된 줄은 §④-b가 전수 명명하므로 사실은 같은 파일 안에서 복원 가능하다. **현재 트리 기준 값이 필요하면 이 절이 아니라 §④-b와 배포 문서를 직접 읽을 것.**
