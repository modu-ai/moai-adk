...
3. **M3**: 다섯 후보군 84건 전부가 근거를 갖는다 — **뮤턴트 판정 81건 + seam 부재 3건**,
   ...
   **[읽는 법] 이 수는 씨앗 12 자리를 측정한 결과를 구성원 84 건에 귀속한 것이다 — "12 자리 측정 · 47 구성원
   귀속"으로 읽는다. 47 번 잰 것이 아니다.** 씨앗은 전부 도달성 프로브 발화 후 변이했고(무도달 5a 는
   제외·무판정), 구성원은 같은 seam 을 지나므로 판정을 물려받는다(§4 Gaps, §7 도달성 구분).
