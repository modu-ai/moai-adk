...
| 항목 | 값 | 전사본 |
|---|---|---|
| 측정 기준 커밋 | `8d1618ed4`+ (실행 시점 트리) | L24-26 |
| 작업 트리 | 스크립트가 자기 산출물만 수정 | L28-33 |
| t88(M4) 포함 | `git merge-base --is-ancestor 7b217da7c HEAD` rc 0 | L35-36 |

---

## M-1. `moai codex` 부재 (카드 전제) — L38-56

도움말 **전문** 대상 대소문자 무시 검색이 0건, `grep -c` rc 1(매치 없음). LAUNCH COMMANDS 는 `cc` / `glm` / `cg` 셋뿐(L43-52). 소스 쪽 유일한 히트는 `internal/cli/hook.go:221` 의 `codex-review-gate` — `moai hook` 하위 커맨드다(L53-55).

## M-2. auth 상태가 항상 `unknown` — 원인 확정 — L57-118

- `codex login status` → `Logged in using ChatGPT`, rc 0 (L58-60).
- 그 문구는 **stdout 0바이트 / stderr 24바이트** — 전량 stderr (L62-68).
- `realCodexRunner.run` (L70-83) 이 `cmd.Stderr = &bytes.Buffer{}` 로 stderr 를 버리고 stdout 만 반환.
- `classifyCodexAuth` (L85-104) 가 빈 문자열을 받아 `codexAuthUnknown` 으로 fail-open.
...
프로브 공유 표면 셋 (L106-117): `codex_setup` MCP 도구, `moai web` 콘솔 카드(`codex_state.go` / `schemaform.go` / `fieldsets_templ.go`), 신설될 런처.
...
## M-2b. 더 나은 auth 원천 — `auth_mode` 필드 — L119-169

- `codex doctor` 는 auth 를 구조화해 안다: `stored auth mode  chatgpt` (L120-127).
- 기계 판독 형태 — `checks["auth.credentials"].details` (L129-146).
- **런처가 부를 수 없다**: 커밋된 이 전사본에서 `codex doctor --json` 이 **31초** 걸렸다 (L19-21). 이것이 유일한 증거값이다. (같은 명령을 여러 번 돌리며 대략 17초에서 46초 사이로 갈리는 것을 봤지만 **그 실행들의 전사본은 커밋돼 있지 않으므로 근거로 쓰지 않는다.**) 판단 — 대화형 리드아웃이 매번 이만큼 기다릴 수는 없다 — 은 31초 하나로 충분히 서고, 초 단위 정확도에 의존하지 않는다.
- 무거운 이유는 doctor 자신의 진단이 말한다 (L148-150): `state.rollout_db_parity: rollout files and state DB thread inventory differ`.
- doctor 가 읽는 원본 파일은 즉시 읽힌다 (L152-168): `auth_mode = 'chatgpt'`, `OPENAI_API_KEY populated: False`, `tokens present: True`, `tokens non-empty values: 4`.
...
## M-3. codex CLI 가 이미 제공하는 표면 — L170-211

`Commands:` 절 전문이 전사본에 있고(L171-206), 항목 수는 **28** 이다(L208-210). 이전 판은 24로 적었다 — 눈으로 센 값이었고 틀렸다.

본 SPEC 이 참조하는 것은 셋: `app`(데스크톱 앱 기동, 없으면 설치 관리자), `doctor`(설치·설정·auth·런타임 진단), `login`(로그인 관리).

## M-4. moai 쪽 Codex 배선 표면 (t88 M4) — L212-251

- `.codex/hooks.json` · `.codex/config.toml` (L213-216)
- 에이전트 TOML **11종** (L218-234). 이전 판의 12는 `ls | wc -l` 이 별칭 긴 형식의 `total` 행을 함께 센 결과였다.
- 생성기 `moai init --agent claude|codex|both` (L236-238), 진단 `checkCodexWiring` (L240-242), 런타임 `--harness codex` (L244-246)
- 이 저장소 자체는 `.codex/` 없음, rc 1 (L248-250). 미배선이 기본 상태다.

## M-5. CODEX_HOME 을 읽는 런타임 코드는 0건 — L252-255

출력 없음 + rc 1 = 매치 0건. `| wc -l` 로 세지 않았다 — 파이프 앞이 죽어도 0이 나와 부재의 증거가 되지 못한다.

## M-6. 기존 런처 3종의 공통 경로는 재사용 불가 — L256-271

`cc` / `cg` / `glm` **셋 다** `unifiedLaunch` 로 수렴한다 (L262-266). 본체(`unifiedLaunchDefault`)는 `.claude/settings.local.json` 변형 + Claude 프로필 해석 + `claude` exec 이므로, 다른 바이너리인 codex 는 얹을 수 없다. 공유 가능한 것은 `spawnLaunch` (L268-270).

## M-7. `codexCommandRunner` 구현체는 3개 — L272-278
