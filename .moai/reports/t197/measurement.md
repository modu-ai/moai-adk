...
| 작업 트리 | 실행 시점의 더티 항목은 프로브 자기 산출물 2건뿐 | L28-31 |
| 측정 대상 브랜치 | `WT-codex-launcher` | L283-285 |
| t88(M4) 포함 | `git merge-base --is-ancestor 7b217da7c HEAD` rc 0 | L33-34 |

**미측정 (Gap)**: 위 더티 목록은 실행 **후** 한 번만 찍혔다. 실행 **전** 스냅샷이 없으므로 "이 두 항목이 이 스크립트 때문에 더티가 됐다" 는 인과는 측정되지 않았다 — 관측된 것은 실행 후 더티 항목이 프로브 자기 산출물 2건뿐이라는 사실까지다. 종전 판의 "스크립트가 자기 산출물만 수정" 은 이 인용이 뒷받침하지 않는 서술이었고 철회한다.
