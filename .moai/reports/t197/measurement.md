...
| 파일 | 역할 |
|---|---|
| `probe.sh` | 측정 스크립트. **자기완결** — 측정 대상 바이너리를 스스로 빌드하고 doctor JSON 도 스스로 만든다. 선행 조건이 깨지면 중단한다 |
...
| 항목 | 값 | 전사본 |
|---|---|---|
| 측정 기준 커밋 | `01f5c531b` | L24-26 |
| 작업 트리 | 스크립트가 자기 산출물 2개만 수정 | L28-31 |
...
- **런처가 부를 수 없다**: 이 실행에서 `codex doctor --json` 이 **18초** 걸렸다 (L19-21). 앞선 측정에서는 46초였다 — 캐시 상태에 따라 갈리므로 하나의 값으로 못 박지 않는다. **두 값 모두 런처의 대화형 리드아웃에는 과하다** 는 것이 판단 근거이고, 그 판단은 초 단위 정확도에 의존하지 않는다.
...
## M-7. `codexCommandRunner` 구현체는 3개 — L270-277
...
- `moai init --agent codex` 를 실제 미배선 프로젝트에 돌린 결과 (REQ-CL-015/016 은 run-phase 에서 관측)
