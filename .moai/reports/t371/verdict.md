...
2. develop 흡수
3. **좌표 통일 후 라벨 찍기** — `plan.md` 의 `lint.go` 좌표를 `:101` `:169` `:170` 전부 한 트리에서 재측정한 뒤, `:16` 라벨을 **커밋 직전 `git rev-parse origin/develop` 재판독값**으로 찍는다. 순서를 뒤집어 라벨을 먼저 찍으면 문서가 스스로에 대해 거짓을 말한다 (어느 값을 찍어도 그 시점엔 절반이 틀리므로 — 아래 3겹 표)
3.5 **교차검산 — 스윕보다 먼저.** `:101` 의 err skip 좌표와 `:170` 의 err skip 좌표는 같은 코드 위치이므로 재측정 후 **일치해야 한다**. 갈리면 재측정이 틀린 것이다. 스윕은 좌표가 옳다는 전제 위에서 도므로 검산이 선행한다
