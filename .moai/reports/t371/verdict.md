...
그 줄은 "기준 트리: … develop `1e5199b88` 기준" 이다. 이것은 사건의 기록이 아니라 **이 문서의 전방 좌표들이 어느 트리에서 재는가**를 선언하는 귀속 라벨이다. 좌표는 흡수(`35bc0715f`)에서 이미 갱신됐으므로, 라벨만 `1e5199b88` 에 고정하면 **라벨이 가리키는 트리와 좌표가 사는 트리가 갈린다** — VCI §2 의 미귀속 주장이 된다. 바로 위 `:17` 이 진짜 역사 앵커이고 그래서 D3 표에 없다는 대비가 판별식이다. 재개 시 이 줄은 좌표를 재는 그 트리로 함께 옮긴다.
