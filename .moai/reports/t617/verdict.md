...
4. `TestSessionStart_DeferredScanDoesNotBlockReturn` 은 단독 실행에서 이 카드의 테스트 파일이 있든 없든 똑같이 통과했다. 이 카드가 원인이라는 근거는 관측되지 않았다.
...
4. **불안정 테스트의 원인은 확정하지 않았다.** hook 패키지 전체 실행과 부하 조건은 다시 재지 않았다(카드 지시대로 범위를 제한했다). `-count=1` 로 여러 번 따로 돌리라는 지시는 가드 때문에 `-count=5` 한 번으로 바꿨다.
