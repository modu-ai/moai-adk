...
x5·x6·x7·m3·m5는 자기가 건드린 합계 칸 하나만 달라지고, 나머지 칸은 정답과 같다. m1·m2는 합계 줄을 바꾸지 않는다. 행 하나만 바꾼다(독립 감사 F4 반영).

위 코드 블록의 `…`는 긴 경로를 줄인 표기다. 명령 원문 출력은 독립 감사 보고서(`audit.md`)에 실려 있다.
...
- 독립 감사(F1)가 이 부분을 추적했다. census가 출력 이벤트를 읽는 곳은 세 곳이다. 실패 테스트 본문, FAILED PKG 본문, notice다. 셋 모두 이번에 추가한 통과·skip 테스트의 출력과는 관계가 없다. 따라서 현재 결과에는 영향이 없다. 다만 `TestAlphaFailsToo`의 본문은 1줄이다. 실제 스트림이었다면 3줄(`=== RUN`, 메시지, `--- FAIL`)이었을 것이다.
- 픽스처 하나로 모든 변이를 구별할 수는 없다. 이번 보강은 감사가 찾은 세 변이를 닫았을 뿐이다. 독립 감사가 추가 변이 9개를 돌렸고, 그중 7개가 살아남았다.
  - 패키지를 빼고 테스트 이름만으로 세는 변이(F2): 서로 다른 두 패키지에 같은 이름의 테스트가 없기 때문이다.
  - 정렬(`sort -u`)을 빼는 변이(F3): 픽스처 이벤트가 이미 정렬된 순서로 놓여 있기 때문이다.
  이 카드 범위 밖이며 후속 후보다.
