...
- `TestToolPolicyEscapeGuard_DriftCheckAloneMissesIt`는 「드리프트 검사가 이 변이를 놓친다」는 현재 동작을 통과 조건으로 고정한다(감사 F4). 나중에 드리프트 검사가 이 형태를 잡도록 바뀌면 이 테스트는 실패한다. 그때는 결함이 아니라 공백이 닫혔다는 신호이므로, 테스트를 지우거나 뒤집는다.
- 원본 층 가드는 Bash 도구 규칙만 본다(감사 F2). Bash 외 도구 규칙에서 `\:`가 규칙을 무력화하는지는 측정하지 않았다. 앞으로 `PowerShell(...)` 규칙이 원본에 들어오면 같은 가드가 필요할 수 있다.
