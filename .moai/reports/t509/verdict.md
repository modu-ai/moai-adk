...
## 23. 창 집행 — 충돌 5건은 **어느 한쪽 통째로가 답이 아니었다**

### 23.1 [리드 정정] 34/15 대 60/19의 차이는 **단위가 아니라 트리**였다

§22.3에서 나는 「2 대 34」를 **단위 불일치**로 진단했다. 리드가 같은 단위로 재측정했다(트리 `0b1e27877`):

```
strings.Index(                55 사이트 / 18 파일
  body|html|page 한정          34 사이트 / 15 파일     ← 34/15 는 사이트/파일 한 쌍이다
panelHTML(                     2 사이트 /  1 파일
```

내 워크트리(run 후) 값은 `strings.Index(` 60/19 · `panelHTML(` 12호출부/3파일이었다.

**차이의 원인은 트리다 — 내 수리가 `panelHTML` 호출부를 늘렸다.** 34/15는 단위가 섞인 것이 아니라 **사이트/파일 한 쌍**이었고, 리드가 그것을 「소비자 2개」와 나란히 둔 것이 비교를 어긋나게 했을 뿐이다.

**어느 쪽 측정도 틀리지 않았다. 틀린 것은 트리를 안 밝힌 인용이다.** 이 리포가 「측정에는 잰 대상을 붙인다」로 적어둔 형태이고, 나는 그것을 **단위 문제로 오진**했다. 진단이 틀렸어도 **결론(34/15를 경계로 쓰면 안 된다)은 살아남는다** — 다만 이유가 다르다: 단위가 섞여서가 아니라 **다른 트리의 값이고, 그 필터가 t527이 아직 만들지 않은 판별식이기 때문**이다.

리드가 카드를 고쳤다: **착수 레인은 55/18(원시 상위집합)에서 시작해 자기 판별식으로 좁히고, 결과가 34/15와 다르면 다른 것이 맞다.**

### 23.2 충돌 5건 — 셋 다 다른 해법

창 시점 tip: `origin/develop` = 로컬 develop = **`5d08342e4`**(t506·t526 포함). 흡수에서 **충돌 5파일**.

**CHANGELOG.md — 동일 슬롯, both-keep.**
양쪽이 `### Added` 맨 위에 항목을 하나씩 넣었다. **고르는 것이 아니라 둘 다 남긴다** — 마커 3줄만 지우고 본문은 양쪽 보존. 확인: `SPEC-WEB-CODEX-PANEL-001` 1 · `SPEC-CODEX-GHOST-SKILLS-PRUNE-001` 1.

**README 4본 — 교차 선택.** 이쪽이 까다로웠다. 한 훙크에 **인접한 두 행**이 들어 있고 **각 행의 최신본이 서로 반대편에 있었다**:

| 행 | 최신 쪽 | 이유 |
|---|---|---|
| `moai clean` | **develop** | t506이 `--codex-skills`를 추가했다. 내 쪽은 그 이전 문안 |
| `moai web` | **내 쪽** | 내가 11탭 → 14탭으로 고쳤다. develop은 아직 11탭 |

**어느 한쪽을 통째로 고르면 반드시 하나를 되돌린다.** develop을 고르면 탭 수가 11로 회귀하고, 내 쪽을 고르면 `--codex-skills`가 사라진다. 그래서 훙크마다 **develop의 `clean` 행 + 내 `web` 행**으로 조립했다.

검증: 마커 잔존 **0**(5파일) · `--codex-skills` 4/4 · **`11탭` 잔존 0** · 14탭 4/4.

로케일 패리티: 4본 모두 414행이 같은 문장 구조다. `ja`만 산문에 수를 명시하는 **기존 표현 차이**이고 사실은 일치한다(14). 내 병합이 만든 것이 아니다.

### 23.3 이 절이 안 본 것

- **리드의 55/18·34/15를 내가 재현하지 않았다.** 트리(`0b1e27877`)가 다르고, 내 트리에서 잰 60/19·12/3만 내 것이다.
- README `ja` 414행의 "14"가 정확히 무엇을 세는지 **전문을 읽지 않았다**. 낡은 11이 없다는 것까지만 확인했다.
- 병합 트리 재측정은 이 절 **다음**이다.

---

## 24. 종결 — develop 착지, 그리고 리드의 자기 정정 2건

### 24.1 착지

**develop 병합 `de76249e5`** · 병합 트리 `697502c4c0a423874c4ef25de015b9fec062d676` · **트리 동일성 델타 0** · 흡수 `a3ed91ff8` · 재측정-병합 사이 커밋 **0** · 미푸시 **27**. 리드 재측정 일치, 카드 `done`.

병합 델타 **29파일**: 구현/테스트 11 · SPEC 4 · 판정서 5 · 문서 9.

### 24.2 [리드 자기 정정] 첫 검증이 두 번 틀렸고, 둘 다 내 보고를 의심하게 만들 뻔했다

리드가 먼저 밝혔다. 둘 다 **오늘 이 저장소가 반복해서 본 형태**다.

1. **잘못된 트리에서 쟀다.** README 검증을 **primary 체크아웃**에서 돌렸는데 그 트리는 686커밋 뒤처져 있다. 결과 `--codex-skills` **0** · `11탭` 잔존 **1** — 내 보고와 정면으로 어긋나 보였다. develop 워크트리에서 다시 재니 **4/4 · 0**으로 내 값과 일치했다.
2. **트리 동일성에 잘못된 피연산자를 썼다.** 흡수 SHA `a3ed91ff8`을 재측정 트리로 잡아 「불일치」를 냈다. 올바른 비교는 **병합 커밋과 그 2번 부모(카드 tip)**이고, 둘 다 `697502c4c`로 성립한다.

**어느 쪽도 내 보고의 결함이 아니라 리드 측정의 결함이었다.** 그리고 리드가 그것을 **의심을 제기하기 전에 스스로 잡아 밝혔다** — 이 카드에서 나와 리드가 번갈아 한 일이고, 그 습관이 없었으면 「lane-1 보고가 틀렸다」가 한 번 기록됐을 것이다.

### 24.3 README 교차 선택이 이 창에서 가장 위험했다

리드 평: 오늘 창들 중 가장 위험한 자리. 이유는 **git이 제안하는 세 해법이 전부 틀렸기** 때문이다.

- `ours` → `--codex-skills` 소실
- `theirs` → 탭 수 11로 회귀
- `both-keep` → 같은 행이 두 번(표가 깨진다)

**답은 행 단위 조립**이었고, 그것을 **검증으로 닫았다**(`--codex-skills` 4/4 · `11탭` 0 · 로케일 414행 구조 4본 동일).

> **git이 제안하는 해결이 의미상 옳다는 보장이 없다.** 훙크가 충돌했다는 것은 두 쪽이 같은 자리를 건드렸다는 뜻일 뿐, **각 행의 최신본이 같은 쪽에 있다는 뜻이 아니다.**

### 24.4 후속 카드 2장으로 넘어갔다

**t530(문서 정확성 스윕)** — 내가 넘긴 두 건(`cli-reference/web.md`의 "9개 탭", 낡은 스크린샷)을 담았고, 리드가 성질을 [HARD]로 박았다:

> 이 결함은 「숫자 하나가 틀렸다」가 아니라 **「같은 사실을 여러 문서가 각자 세고 있다」**이다. 문서가 11이라 적었을 때 실제는 14였고 `feedback`·`gate`가 이 카드들과 무관하게 이미 빠져 있었다 — **드리프트가 한 번이 아니라 누적됐다.**

처방도 그 성질을 따른다: **수를 문서에서 빼거나**(탭 이름 나열), 뺄 수 없으면 **`consoleTabs()` 길이와 문서 수치를 비교하는 가드**를 붙일 것. **지금 그 가드가 없다.**
스크린샷은 **재생성 절차가 어디에도 없다는 것 자체가 별개 결함일 수 있다**고 판정 대상으로 남았다.

**t527(앵커 스윕)** — 미러가 develop에 있으니 이제 회귀를 **미러 존재 하에서** 잴 수 있다. 카드는 **55/18(원시 상위집합)에서 시작**하도록 고쳐졌다.

### 24.5 이 카드가 남긴 것 — 요약

**산출**: `moai web`에 codex 읽기 전용 미러 탭. 새 설정 키 0 · 새 저장 경로 0 · 새 파일(config) 0.

**공정**: plan-audit **0.69 → 0.84 → 0.89** (3회) → kickoff → run(AC 14/14, 뮤턴트 8/8 RED) → sync → Amendments v0.4.0 → 창.

**찾은 것**: 공허한 통과 **2건**(원본, 그리고 **그 수리가 되살린 것**) · SPEC 반증 **5건**(전부 첫 실행에서) · 명시되지 않은 결합 1건이 실제로 파손 · 문서 드리프트 누적 1건.

**내 오류**(전부 원문 보존 + 정정 병기): 질문을 좁혀 답하고 해결됐다고 보고 **3회** · `templ` cwd 오탐 자작 · 흡수 값 오기 · **감사 창 안에서 두 번째 작성자** · 34/15 단위 오진.

**되풀이된 교훈 하나**: 이 판정서는 §8→§9, §13.2→§16, §22.3→§23.1에서 **원문을 지우지 않고 정정을 나란히 붙였다.** 틀린 경위가 지워지면 다음 사람이 같은 자리에서 같은 문장을 다시 쓴다.
