# MUTANT m2 (card t524): the Group 4 verb with heading-form definition collection dropped.
# Expected: fixture D reports COLLECTED: 0 and GAP instead of UNCOVERED: REQ-FIXD-003.
spec="<new-spec.md>"
acc="<acceptance.md>"
if [ ! -r "$spec" ]; then
  ...
  awk -v accstate="$accstate" '
    ...
    { sub(/\r$/, "") }
    FILENAME == ARGV[1] && ($0 ~ deflist || $0 ~ defrow) {
      match($0, id)
