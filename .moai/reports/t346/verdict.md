...
- **본 수리만으로 develop CI 는 초록이 되지 않는다.** 위 계열 2·3·4·5가 남는다 (수리 소관 카드는 t349 · t352 · t326 · t354 — 표의 t322·t306 은 결함을 **도입한** 카드이고, 그 뒤 수리 카드가 따로 발행됐다) — 다른 레인의 착지 판정(CI 판독) 봉쇄는 t346 착지로 해소되지 않는다.
...
- **CI 델타 = gap (취소로 판정 미도달). 판독은 리드 인계.** 착지 후 `develop` head 두 개의 `CI` 잡이 모두 `completed / cancelled` 로 끝났다 — `61d5d56c9` (CI·SPEC Lint·Graph Freshness·CodeQL 전부 cancelled), `3fcbf8e3a` (CI·CodeQL cancelled; lsel-leak-guard·Graph Freshness·SPEC Lint 는 success 이나 doctor 계열이 사는 잡이 아니다). 원인은 레인 9개가 같은 `develop` 으로 수렴하면서 각 push 가 동시성 그룹으로 앞선 실행을 취소하는 것 — 병합 큐가 이어지는 동안에는 어떤 head 도 판정에 도달하지 못한다. **취소는 통과도 실패도 아니므로 doctor 9종 소거를 이 실행들로 주장하지 않는다.** 리드가 병합 큐가 마르고 head 가 조용해진 시점에 한 번 읽어 계열별로 배분하기로 인계(2026-08-28).
