...
| #1659 | t512 | `reply-1659.md` (t512 초안 + 선행 코멘트 배포 약속과의 정합화 + 오늘 관측 반영) |
...
## Addendum — 리드 게이트 차단 1건 수리 (2026-09-08)

- **리드가 게시를 보류**: 초안 5개가 존재하지 않는 릴리스 번호(v3.2.0)를 외부 사용자에게 약속 — 번호는 미정이고, 릴리스 자체가 t204 배포 게이트로 보류 중(운영자 지시 2026-08-24).
- **전제 재측정 (본 트리)**: `pkg/version/version.go` `Version = "v3.1.3"`(컴파일 기본값), 태그 최신 `v3.1.2` — v3.1.3 태그조차 부재. 본 카드 초안 7파일에서 3.2.0 언급 16건 전수 확인.
- **수리**: 게시용 본문 전체에서 릴리스 번호 제거 → 착지 병합 SHA 명시 + 「이 수리를 싣는 릴리스는 아직 커팅되지 않았습니다 — 나오는 대로 이 이슈에 알려드리겠습니다」로 교체. 재테스트 문구는 「릴리스가 실리면 이 이슈에 알려드리겠습니다」로 통일. 내부 각주의 역사적 인용도 번호 없는 형태로 정리.
- **게시 절차 갱신**: 리드 낭독 → 본 수리 diff 보고 → **운영자 최종 승인** → 게시 (리드 지시대로 gh 게시 명령은 아직 실행하지 않음).
