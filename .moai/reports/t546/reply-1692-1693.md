...
confirmed as a defect and it is fixed on the development line, shipping in v3.2.0.
...
We'd appreciate it if you could re-test on v3.2.0; we'll keep this issue open until
you confirm.

## §2 — GH #1693 (status locator reports `Status`/`Notes` table rows as SPEC status)

Also confirmed and fixed on the same line (v3.2.0) — these two reports described two
...
We'd appreciate a re-test on v3.2.0.
