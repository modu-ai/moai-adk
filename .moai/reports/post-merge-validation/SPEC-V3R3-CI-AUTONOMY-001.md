...
- AC-CIAUT-020 30일 grace window (2026-05-09 ~ 2026-06-08) 활성화 — 수동 debug 개입 감소 측정 예정

## DoD Checklist Result

| SPEC-Level DoD 항목 (acceptance.md §11) | 상태 | 근거 |
|---|----|---|
| **AC-CIAUT-001** `make ci-local` 16개 언어 자동 감지 | PASS | PR #785 commit 39d53f2d4 + coverage 92.1% |
| **AC-CIAUT-002** POSIX sh polling + 30min guard | PASS | PR #788 commit d48676292 + shell tests 9/9 |
| **AC-CIAUT-003** Mechanical vs semantic failure classification | PASS | PR #790 (expert-debug logic) + PR #791 (SSoT) |
| **AC-CIAUT-004** Required CI check SSoT (`.github/required-checks.yml`) | PASS | PR #791 commit 311f27a2a |
| **AC-CIAUT-005** Worktree snapshot/verify/restore primitives | PASS | PR #792 commit 8760b89cd + 87.6% coverage |
| **AC-CIAUT-006** Off-protocol reminder in `moai status` | PASS | PR #794 + PR #795 (cwd anchor fix) |
| **AC-CIAUT-007** i18n validator dual-mode oracle | PASS | PR #793 commit 78929d058 + 85% coverage |
| **AC-CIAUT-008** BODP 3-signal + 8-row matrix | PASS | PR #794 commit 02bed9c14 + 85.9% coverage |
| **AC-CIAUT-009** Template-First mirror + `moai update` sync | PASS | All 8 PRs verified for mirror completeness |
| **AC-CIAUT-010** 16-language neutrality verification | PASS | T1 + T7 언어 편향 검증 (CLAUDE.local.md §15) |
| **AC-CIAUT-011** No release/tag auto-trigger | PASS | go.mod/release info 미수정, tag push 0건 |
| **AC-CIAUT-020** 30-day grace window + manual validation plan | PASS | Window active 2026-05-09~06-08, baseline 3-4 debug/PR |
...
향후 `lessons.md`에 추가 검토할 lesson 5건:

### Lesson #15 후보 — Auto-merge race + post-merge fix orphan brunch

**Rule**: PR이 자동 머지된 후 동일 branch에 추가 commit하면 main에 반영되지 않습니다. Fix는 별도 PR (origin/main 기반 fix brunch + cherry-pick 또는 새로운 feature brunch)로 처리하세요.

**Why**: PR #794 auto-merge 후 PR #795 (same brunch) commit이 main에 반영되지 않음. Brunch HEAD가 auto-merge 후 고정되어 새로운 push가 "up-to-date" 상태로 처리됨.

**How to apply**: Auto-merge가 예상되는 PR은 매우 마지막 commit 이후 추가 fix가 필요한 경우, 병렬로 새로운 feature/hotfix brunch를 origin/main에서 파일 것. Stacked brunch 권장.
...
### Lesson #18 후보 — `make build && make install` 후 binary 재시작 필수

**Rule**: Go binary 변경 후 `moai version` (commit hash 검증) 수행 및 Claude Code 재시작하세요. Binary hot-reload 없음.

**Why**: CLAUDE.local.md §1에 기존 명시되어 있으나, Wave 6/7에서 cli/worktree changes 후 hook 동작 이상으로 재시작 필요한 사례 반복.

**How to apply**: Binary 변경 후 매번 `which moai` → `moai version`으로 commit hash 확인. Expected hash와 불일치 시 `make install` 재실행 후 Claude Code 종료/재시작.
...
5-PR sweep replay 측정은 SPEC 머지 후 30일 grace window (2026-05-09 ~ 2026-06-08) 동안 SPEC 작성자 + 1 reviewer가 수동으로 진행합니다. 측정 목록 (acceptance.md §10 AC-CIAUT-020 표):
...
- [ ] lesson 후보 #15-19 사용자 검토 후 `~/.claude/projects/{hash}/memory/lessons.md` 추가
