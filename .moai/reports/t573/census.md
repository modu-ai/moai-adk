...
- O-1 이행 (plan-audit review-2): AC-009 판독 표(§4)의 행 수와 **유니크 파일 토큰 수**를 교차 확인했다 — §4 표 행 계수와 `sort -u` 파일 토큰 계수가 모두 95로 일치한다 (§4 말미 실측 — 검증 명령은 §4 말미 기록, 2026-09-08 명령 정정 참조).
...
**O-1 유니크 토큰 교차 확인** (2026-09-08 명령 정정 — sync-audit F1): 본 표의 파일 행 수와 유니크 파일 토큰 수는 다음 검증된 명령 쌍으로 계수한다 (트리 `7b86ac8ab` 실측):

- 행 수: `awk '/^## §4 /,/^## §5 /' .moai/reports/t573/census.md | /usr/bin/grep -c '^| SPEC-'` → **95**
- 유니크 파일 토큰: `awk '/^## §4 /,/^## §5 /' .moai/reports/t573/census.md | /usr/bin/grep '^| SPEC-' | /usr/bin/grep -oE 'SPEC-[A-Za-z0-9-]+/[a-z]+\.md' | sort -u | wc -l` → **95** (중복: 0 — 중복 행 슬랙 0)

**정정 (errata, 2026-09-08)**: 이 자리에 이전에 기록된 명령 `grep -c '^| \.moai'` 는 실제로 **0** 을 낸다 — §4 표 행은 `| .moai` 가 아니라 `| SPEC-` 로 시작하기 때문이다 (sync-auditor 적발, 레인 재현). 실질(95행 · 유니크 95 · 중복 0)은 독립 검증으로 성립해 왔고 잘못된 것은 기록된 명령뿐이다. **유니크 단위가 SPEC 디렉터리 토큰(66)이 아닌 이유**: 코퍼스가 SPEC 당 최대 3종(acceptance·spec·plan)을 담으므로 단위는 파일 토큰 `SPEC-<id>/<file>` 이다 — 디렉터리 토큰으로 세면 한 SPEC 의 3종이 하나로 뭉개져 95 를 재현할 수 없다.
