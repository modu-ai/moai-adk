...
server.shutdown()
result['success']=len(result['requests'])>=2 and any(x['tool_references'] for x in result['requests'][1:]) and result.get('final_marker_seen',False)
(base/'claude-tool-surface-result.json').write_text(json.dumps(result,indent=2)+'\n')
