...
if '--mcp' in sys.argv:
 ...
 raise SystemExit
result={'claude_version':'2.1.269','requests':[],'errors':[],'cli_events':[]};lock=threading.Lock();child_counts={}
def refs(v):
 out=[]
...
class Handler(http.server.BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  with lock:result['requests'].append({'path':self.path,'tools':tools,'tool_references':refs(d.get('messages',[])),'identity':identity(d,self.headers)});idx=len(result['requests'])
  agent=self.headers.get('x-claude-code-agent-id')
  if idx==1:
   blocks=[{'type':'tool_use','id':'toolu_fixture_agent_'+str(i),'name':'Agent','input':{'description':'Synthetic child '+str(i),'prompt':'Use ToolSearch to discover fixture echo, then reply CHILD_'+str(i)+'_OK. Do not execute echo.','subagent_type':'Explore'}} for i in [1,2]];stop='tool_use'
  elif agent:
   child_counts[agent]=child_counts.get(agent,0)+1
   if child_counts[agent]==1:
    blocks=[{'type':'tool_use','id':'toolu_search_'+agent,'name':'ToolSearch','input':{'query':'select:mcp__fixture__echo','max_results':1}}];stop='tool_use'
   else:blocks=[{'type':'text','text':'CHILD_IDENTITY_OK'}];stop='end_turn'
  else:blocks=[{'type':'text','text':'PARENT_IDENTITY_OK'}];stop='end_turn'
  ...
  for bi,block in enumerate(blocks):
   events.append(('content_block_start',{'type':'content_block_start','index':bi,'content_block':dict(block,**({'input':{}} if block['type']=='tool_use' else {'text':''}))}))
   ...
   events += [('content_block_delta',{'type':'content_block_delta','index':bi,'delta':delta}),('content_block_stop',{'type':'content_block_stop','index':bi})]
  events += [('message_delta',{'type':'message_delta','delta':{'stop_reason':stop,'stop_sequence':None},'usage':{'output_tokens':20}}),('message_stop',{'type':'message_stop'})]
...
with tempfile.TemporaryDirectory(prefix='moai-claude-tool-surface-') as temp:
 ...
 env.update(CLAUDE_CONFIG_DIR=str(home),ANTHROPIC_API_KEY='synthetic-local-fixture',ANTHROPIC_BASE_URL='http://127.0.0.1:'+str(server.server_port),CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC='1',ENABLE_TOOL_SEARCH='true')
 cmd=['/Users/goos/.local/bin/claude','--print','--verbose','--output-format','stream-json','--max-turns','3','--permission-mode','default','--settings','{"disableAllHooks":true}','--strict-mcp-config','--mcp-config',json.dumps({'mcpServers':{'fixture':{'command':sys.executable,'args':[str(pathlib.Path(__file__).resolve()),'--mcp']}}}),'--','Delegate concurrently to two Explore Agents. Each discovers fixture echo using ToolSearch without executing echo, then replies. Finally reply PARENT_IDENTITY_OK.']
...
server.shutdown()
result['child_request_counts']=child_counts
result['success']=len(child_counts)==2 and all(n>=2 for n in child_counts.values()) and result.get('final_marker_seen',False)
(base/'agent-identity-parallel-result.json').write_text(json.dumps(result,indent=2)+'\n')
