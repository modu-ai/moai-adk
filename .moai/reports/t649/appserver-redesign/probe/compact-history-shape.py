...
if '--hook' in sys.argv:
 ...
 row={k:data.get(k) for k in ['hook_event_name','session_id','trigger','source'] if k in data};row['time_ns']=time.time_ns();row['kind']='hook';row['transcript_path_present']=bool(data.get('transcript_path'));row['input_keys']=sorted(data);row['prompt_id']=data.get('prompt_id');row['summary_sha256']=hashlib.sha256(data['compact_summary'].encode()).hexdigest() if isinstance(data.get('compact_summary'),str) else None;row['summary_length']=len(data.get('compact_summary',''))
...
class Handler(http.server.BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  shape['phase']=phase;shape['messages_full']=d.get('messages',[]);shape['system_full']=d.get('system',[]);shape['header_names']=sorted(k.lower() for k in self.headers if k.lower() not in ['authorization','x-api-key']);shape['prompt_id']=self.headers.get('x-claude-code-prompt-id')
  with lock:result['requests'].append(shape);idx=len(result['requests'])
  block={'type':'text','text':{'seed':'SEED_ASSISTANT_BLUEBIRD42','compact':'COMPACT_SUMMARY_BLUEBIRD42','followup':'FOLLOWUP_DONE'}[phase]};stop='end_turn'
...
with tempfile.TemporaryDirectory(prefix='moai-claude-tool-surface-') as temp:
 ...
 for phase in ['seed','compact','followup']:
  call=list(cmd)
  call[-1]='Remember the synthetic fixture marker BLUEBIRD42. Reply briefly.' if phase=='seed' else '/compact' if phase=='compact' else 'NEW_QUERY_PINE73: recall the fixture marker.'
  if phase!='seed':
...
server.shutdown()
result['success']=len(result['invocations'])==3 and result['invocations'][1]['exit_code']==0 and all(any(e.get('hook_event_name')==name for e in result.get('ordered_events',[])) for name in ['PreCompact','PostCompact','SessionStart'])
(base/'compact-history-shape-result.json').write_text(json.dumps(result,indent=2)+'\n')
