...
class Handler(http.server.BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  with lock:result['requests'].append({'path':self.path,'tools':tools,'tool_references':refs(d.get('messages',[]))});idx=len(result['requests'])
  search=next((t.get('name') for t in tools if t.get('name')=='ToolSearch'),None)
  if idx==1 and search:
   block={'type':'tool_use','id':'toolu_fixture_search','name':search,'input':{'query':'select:mcp__fixture__echo','max_results':1}};stop='tool_use'
...
with tempfile.TemporaryDirectory(prefix='moai-claude-tool-surface-') as temp:
 ...
 env.update(CLAUDE_CONFIG_DIR=str(home),ANTHROPIC_API_KEY='synthetic-local-fixture',ANTHROPIC_BASE_URL='http://127.0.0.1:'+str(server.server_port),CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC='1',ENABLE_TOOL_SEARCH='false')
...
server.shutdown()
result['success']=bool(result['requests']) and any(t.get('name')=='mcp__fixture__echo' and t.get('input_schema') for t in result['requests'][0]['tools']) and not any(t.get('name')=='ToolSearch' for t in result['requests'][0]['tools']) and result.get('final_marker_seen',False)
(base/'claude-tool-surface-disabled-result.json').write_text(json.dumps(result,indent=2)+'\n')
