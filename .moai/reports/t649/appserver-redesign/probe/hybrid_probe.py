...
try:
 ...
 t=rpc('thread/start',{'model':'gpt-5.6-sol','cwd':str(base.resolve()),'ephemeral':True,'environments':[],'approvalPolicy':'never','sandbox':'read-only','baseInstructions':'Use native dynamic ToolSearch to discover echo, then use moai_claude_tool only for the discovered late echo tool. Never dispatch ToolSearch through moai_claude_tool. Obey each actual schema exactly. No other tools. This is a synthetic integration test.','dynamicTools':[{'type':'function','name':'ToolSearch','description':'Discover a late synthetic tool by exact select query.','inputSchema':{'type':'object','properties':{'query':{'type':'string'}},'required':['query'],'additionalProperties':False},'deferLoading':False},{'type':'function','name':'moai_claude_tool','description':'Dispatch an available Claude tool by exact name and arguments. Tool availability and actual schema come from the current turn and tool results.','inputSchema':{'type':'object','properties':{'name':{'type':'string'},'arguments':{'type':'object'}},'required':['name','arguments'],'additionalProperties':False},'deferLoading':False}]})
 thread=t['thread']['id'];result['thread_started']=True;result['reported_model']=t.get('model')
 u=rpc('turn/start',{'threadId':thread,'environments':[],'effort':'low','input':[{'type':'text','text':'Call the native ToolSearch dynamic tool with query select:fixture_echo. Never call ToolSearch through the dispatcher. Read the discovered schema, call fixture_echo with marker MOAI_HYBRID_OK, then reply only its result. fixture_echo is not available until discovered.'}]});turn=u['turn']['id']
 while time.monotonic()-started<60:
  x=read(max(.1,min(30,60-(time.monotonic()-started))));method=x.get('method');params=x.get('params',{})
  if method:result['events'].append(method)
  if method=='mcpServer/startupStatus/updated':
   result.setdefault('mcp_startup_shapes',[]).append({k:(len(v) if isinstance(v,list) else {str(j):len(z) if isinstance(z,list) else type(z).__name__ for j,z in v.items()} if isinstance(v,dict) else v if isinstance(v,(bool,int)) or k in ['status','state'] else type(v).__name__) for k,v in params.items()})
  if method=='item/tool/call':
   ...
   if len(result['tool_calls'])==1 and name=='ToolSearch' and args=={'query':'select:fixture_echo'}:
    ok=True;payload=json.dumps({'tool_reference':{'type':'tool_reference','tool_name':'fixture_echo'},'tool':{'name':'fixture_echo','input_schema':{'type':'object','properties':{'marker':{'type':'string','const':'MOAI_HYBRID_OK'}},'required':['marker'],'additionalProperties':False}},'invocation':'Call fixture_echo through moai_claude_tool because it is newly discovered.'})
   elif len(result['tool_calls'])==2 and name=='moai_claude_tool' and args=={'name':'fixture_echo','arguments':{'marker':'MOAI_HYBRID_OK'}}:
    ok=True;payload='MOAI_HYBRID_OK'
 ...
 result['success']=result.get('turn_status')=='completed' and result.get('dispatch_validations')==[True,True] and 'MOAI_HYBRID_OK' in result['final_texts']
 ...
 (base/'hybrid-result.json').write_text(json.dumps(result,indent=2)+'\n')
