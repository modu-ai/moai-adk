...
try:
 while time.monotonic()-start<55:
  ...
  if not sent and time.monotonic()-start>7:
   os.write(master,b'Use ListAgents to discover peer sessions. Do not send any messages. If a peer named lead appears, reply exactly MOAI_PEER_LEAD_FOUND; otherwise reply MOAI_PEER_MISSING.\r');sent=True
  if '⏺MOAI_PEER_LEAD_FOUND' in compact:result['reply_seen']=True;break
