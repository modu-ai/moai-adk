...
if '--mcp' in sys.argv:
 ...
 raise SystemExit
result={'claude_version':'2.1.269','requests':[],'errors':[]};lock=threading.Lock()
def refs(v):
 out=[]
...
class Handler(http.server.BaseHTTPRequestHandler):
 ...
 def do_POST(self):
  ...
  with lock:result['requests'].append({'path':self.path,'tools':tools,'tool_references':refs(d.get('messages',[])), 'lead_seen': 'lead [a250a7]' in json.dumps(d.get('messages',[]))});idx=len(result['requests'])
  search=next((t.get('name') for t in tools if t.get('name')=='ListAgents'),None)
  if idx==1:
   block={'type':'tool_use','id':'toolu_fixture_search','name':'ListAgents','input':{}};stop='tool_use'
  else:block={'type':'text','text':'CLAUDE_TOOL_SURFACE_OK'};stop='end_turn'
...
with tempfile.TemporaryDirectory(prefix='moai-claude-tool-surface-') as temp:
 ...
 if os.environ.get('PROBE_SHARED')=='1': (home/'sessions').symlink_to(pathlib.Path.home()/'.moai/claude-profiles/mo.ai.kr/sessions',target_is_directory=True)
 ...
 cmd=['/Users/goos/.local/bin/claude','--print','--verbose','--output-format','stream-json','--max-turns','3','--permission-mode','default','--settings','{"disableAllHooks":true}','--strict-mcp-config','--mcp-config',json.dumps({'mcpServers':{}}),'--','Use ListAgents to list local peer sessions, then reply CLAUDE_TOOL_SURFACE_OK. Do not send any messages.']
 ...
 try:out,err=p.communicate(timeout=45);result['exit_code']=p.returncode;result['final_marker_seen']=b'CLAUDE_TOOL_SURFACE_OK' in out
...
server.shutdown()
result['lead_seen']=any(x['lead_seen'] for x in result['requests']);result['success']=len(result['requests'])>=2 and result.get('final_marker_seen',False)
(base/('shared.json' if os.environ.get('PROBE_SHARED')=='1' else 'isolated.json')).write_text(json.dumps(result,indent=2)+'\n')
