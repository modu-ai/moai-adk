...
func productionGatewayHandlerFactory(raw json.RawMessage) (http.Handler, error) {
	...
	factory, err := newGatewayHandlerFactory(gatewayFactoryDependencies{
		...
		Limits:           translate.Limits{PolicyProfile: policy, MaxBodyBytes: 1 << 20, MaxEventBytes: 1 << 20, MaxOutputBytes: 8 << 20},
		MeasureInput:     gateway.EstimateInputTokens,
		AnthropicVersion: "2023-06-01",
		Broker:           broker,
		RefreshVerifier:  verifyGPTCredentialOwnership,
		SendOptions:      auth.SendOptions{WriteTimeout: 30 * time.Second, PollInterval: 100 * time.Millisecond, MaxBodyBytes: 1 << 20},
	})
	if err != nil {
		return nil, err
	}
