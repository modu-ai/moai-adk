...
## 팩토리 모드 — 같은 -k, 워커 수를 붙이면

{{< callout type="info" >}}
{{< icon flash primary >}} **소속 가치**: 다중 세션 오케스트레이션 · 토크노믹스
{{< /callout >}}

칸반이 "역할 3개가 한 카드를 단계별로 나르는" 모양이라면, **팩토리 모드**(Factory Mode)는 "번호 붙은 워커 N개가 카드 여러 장을 동시에 나르는" 모양입니다. 진입은 같은 `-k` 토큰에 **숫자를 붙이는 것**으로 끝납니다 — 새 플래그가 아니라 같은 스위치의 두 번째 해석입니다.

```bash
# 리드 — 워커 4짜리 팩토리 런 (worker-1..worker-4 실행 명령을 알려 줍니다)
$ moai cc -k 4
...
리드가 하는 일이 칸반 리드와 다릅니다. 칸반 리드가 한 카드의 단계 사이를 조율한다면, 팩토리 리드는 **백로그 큐를 폴링해** 운영자가 고른 카드를 **빈 워커에 배분**합니다. 카드 클래스에 따라 배분 방식이 갈립니다 — A/B클래스(단순 변경·원인 미확립 결함)는 워커 하나에 통째로 맡기고, C클래스(설계 변경)는 `plan → run → sync` 직렬 3단계 경로로 보냅니다. 워커가 한창 일할 때는 새 카드를 굳이 깨우지 않고, 슬롯이 비는 순간에 맞춰 분배합니다(스태거 팬아웃 — 동시 기동 경쟁을 피하는 지연 분배).
