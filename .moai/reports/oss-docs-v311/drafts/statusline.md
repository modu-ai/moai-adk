...
```text
🤖 Opus │ 🧠 xhigh·t │ ♻️ 87% │ 🔅 cc v2.1.212 │ 🗿 v3.1.0 │ ⏳ 4h 52m │ 💬 MoAI
...
- **넷째 줄(조건부) — 누구로서, 얼마나 쌓여 있는가**: 세션 이름(🏷️), 에이전트 이름(👤), 백로그 현황(🔄 `TODO: 진행 중 / 대기`), 열린 이슈·PR 수(🔀)를 보여 줍니다. 칸반·팩토리 동반 세션처럼 이름 있는 세션에서 자연스럽게 붙고, 관측 원본이 없으면 그 세그먼트는 줄어들며 전부 비면 줄 자체가 생략됩니다. 세션 이름을 강조 표시하는 자리이기도 해서, 터미널 여러 개를 열어 두고 어떤 창이 어느 역할인지 헷갈릴 때 첫 신호가 됩니다.
...
리포지토리와 브랜치는 하나의 통합 세그먼트로 렌더됩니다. `owner/name` 부분은 Claude Code v2.1.145부터 stdin에 추가된 `workspace.repo`에서 오고, 브랜치는 로컬 git에서 읽습니다. 리포 표시에는 📡 글리프가 붙고 브랜치와는 ASCII 파이프(`|`)로 이어져, 두 값이 합쳐지면 "어느 리포의 어느 브랜치에서 일하고 있는가"가 한눈에 들어옵니다. worktree(연결된 별도 작업 디렉터리)에서 작업 중일 때는 브랜치 앞에 `[WT]` 표시가 붙어 일반 체크아웃과 구분됩니다.
...
statusline:
  ...
  segments:
    ...
    github: true             # 🔀 열린 이슈 / PR
```

열여섯 개 키가 정식 설정 스키마입니다. 리포지토리를 뜻하는 `owner/name` 부분은 `git_branch` 세그먼트 안에서 함께 렌더되는 열일곱 번째 요소로, 스키마 밖이라 개별 토글은 없습니다. 세션 라인의 세 키(`session`·`backlog`·`github`)는 이 16키 스키마와 별개로, 설정에 적지 않아도 기본 켜짐으로 렌더됩니다 — 관측 원본(세션 이름, 백로그 큐, GitHub 캐시)이 없으면 해당 세그먼트는 조용히 생략됩니다. 과거의 이름 붙은 프리셋(full/compact/minimal)은 폐기되었으므로, 원하는 조합은 세그먼트 단위로 직접 켜고 끄면 됩니다.
