...
| Q-14 | `git branch --no-contains HEAD` (space-separated value; same arity pin; `--color`/`--abbrev`/`--column` ATTACHED value forms covered by the same rule — attached `=<value>` only; their space-separated token is a creation operand, pinned deny by M-31) | allow (inferred-pending-M1) | allow |
