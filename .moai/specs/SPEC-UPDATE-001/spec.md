...
MoAI-ADK는 모든 에이전트, 스킬, 룰을 프로젝트 로컬 .claude/ 디렉터리에 설치합니다. 이로 인해 다음과 같은 문제가 발생합니다:
...
REQ-001: 시스템은 모든 설치 모드에서 .moai/ 디렉터리 구조를 유지해야 한다.
...
REQ-010: WHEN 사용자가 moai init을 실행하면 THEN 시스템은 .moai/ 디렉터리 존재 여부를 확인해야 한다.

REQ-011: WHEN .moai/ 디렉터리가 없으면 THEN 시스템은 "moai init . 으로 초기화하세요" 안내 메시지를 표시해야 한다.
...
REQ-030: IF 프로젝트가 Global 모드로 설정되어 있으면 THEN .claude/ 디렉터리에는 hooks, settings.json, settings.local.json만 존재해야 한다.
