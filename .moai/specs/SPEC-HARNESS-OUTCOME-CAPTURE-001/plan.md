...
**Live fail-closed structure (the pseudocode above is simplified)**: the real rollback branch (`applier.go` (6a), lines ~384-396) has TWO fail-closed early-returns BEFORE the terminal `return &ApplyRegressionError{...}` — a `RestoreSnapshot` failure and a `writeLineage` failure each return their own wrapped error. The `recordOutcome` emit lands ONLY on the terminal rolled-back exit (reached after `writeLineage` succeeds) and on the symmetric terminal kept exit (reached after `baselineStore.Save` + `writeLineage` both succeed). A hard rollback-failure or lineage-write-failure surfaces its own Apply error and emits NO `apply_outcome` event — those are Apply failures, not clean rolled-back outcomes, so they are correctly excluded from the outcome substrate (run-phase wires the emit at the terminal exit of each branch, not before the fail-closed guards).
