...
2. **Stamp reachability HARD**: never refresh this PR's codemaps stamp against branch-local HEAD; threshold-crossing churn restamps via `--commit "$(git merge-base HEAD origin/main)"` (expected unnecessary — churn ≪ 40).
...
- E3 Workflow guard dry-run — each of the three guard branches (non-ancestor RED / ancestor GREEN / no-anchor SKIP) executed as local shell exactly as the step's script reads, outputs quoted (the YAML itself ships syntax-validated but unexecuted; first live execution is the delivering PR's own run — expectation stated there, never pre-claimed as observed).
...
- One new step in `.github/workflows/graph-freshness.yml`, positioned after `actions/checkout@v7`, before setup-go/build. Shell implements exactly: read tracked provenance → empty/no-anchor → print skip-with-reason, exit 0 → object missing (`git cat-file -e <sha>^{commit}`) → `::error` naming sha, exit 1 → ancestry vs `origin/${{ github.base_ref }}` fails → `::error` naming sha + base ref, exit 1 → else pass line. On push events the step runs unchanged (base_ref empty ⇒ existence clause still discriminates; conditional anatomy decided by the step's own `if`, no duplicate step).
