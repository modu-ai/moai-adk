...
# SPEC Review Report — 5차 (0.1.4 최종 plan-phase 회차): SPEC-TODO-HOME-TEMP-GUARD-001

> **1·2·3·4차 기록은 위에 그대로 있다.** 이 절은 추가이며 덮어쓰기가 아니다.

Iteration: 5 (Tier M 상한 2 **초과** — 운영자가 명시 조건 아래 개설한 마지막 plan-phase 회차)
**범위(운영자 지시)**: D18(정정된 열거) · D19(교체된 판정식과 그 RED 조건) · **unfold**(REQ-THG-009 분리와 Tier M) ·
카드가 자가 신고한 2건(D20 처분 · `queueRootInsideTemp` 범위 밖 판정) · D21 · D22 · AC-SA-011 판정 ·
경계 보고 명세 · **감사 중 도착한 `§D.0`**(리드 통지, §0). **D17은 범위 밖이며 여전히 열린 채다 —
이번에도 등급 매기지 않는다.** 1-3차에서 닫힌 축과 4차가 닫은 D14·D16은 재론하지 않는다.

Verdict: **FAIL** — **막는 것은 단 하나(D23)**이고, 점수는 임계를 넘는다. 아래 §9·§12가 그 구분을 명시한다.
Overall Score: **0.89** (Tier M 임계 0.80 — **점수로는 통과**. FAIL은 점수가 아니라 blocking 1건에서 나온다)

측정 기준 트리: `.claude/worktrees/t536` · HEAD `075dd8d4b` **+ `acceptance.md`의 미커밋 16행**(§0).
아래 인용·계수는 전부 이 감사 실행 중 이 트리에서 직접 관측했다. 셸 `grep`은 ugrep 래퍼라 **모든
계수·부재 주장에 `/usr/bin/grep`을 썼고**, 심볼 열거는 `-i`로 돌렸다. 절 인용은 **절의 자기 구분자로
경계** 지었다(`sed -n '/^## 5\./,/^## 6\./p'`) — 추측한 줄 번호로 자르지 않았다. 어디서도 잘린 창을
쓰지 않았다.

M1 Context Isolation: 카드가 제시한 수리 주장은 하나도 그대로 채택하지 않았다. D18은 **두 축을 다시
실행**하고 거기에 더해 **요구사항 축(리졸버에 도달하는 생산 소비자 전수)**으로 독립 재유도했다.
D19는 7건 **전부의 본문을 직접 판독**했다. AC-SA-011은 형제 SPEC 산출물에서 직접 인용했다.
`§D.0`의 존재 이유로 제시된 lint 전제도 **소스에서 직접 확인**했다(§0.2).

---

## 0. 감사 창 안에서 트리가 움직였다 — 무엇이 영향을 받았는지 명시한다

리드가 통지한 대로, 수리 회차의 후행 작업이 감사 도중 `acceptance.md`에 한 번 더 썼다. **감사자가
그 사실을 스스로 발견한 것이 아니라 통지로 받았고**, 그 순서 자체를 기록한다.

### 0.1 어느 판독이 편집 이전이었는가 — 명시한다

| 판독 | 시점 | 결과 |
|---|---|---|
| `acceptance.md` 전문 (`cat -n`, 199행) | **편집 이전** | 본문 판정 전부가 여기서 나왔다 |
| `wc -l acceptance.md` = 199 | **편집 이전** | 계수 |
| `git diff -- acceptance.md` | 편집 이후 | 추가분을 여기서 처음 봤다 |
| `sed -n '13,45p'`(§D + §D.0), 재계수 215행, 좌표 재유도 | 편집 이후 | 아래 정정 |

**내용 판정에는 영향이 없다. 좌표에는 있다.** `git diff`가 보인 훙크는 **순수 추가 1개**(+16행,
`§D` 매트릭스 직후)이며 기존 행의 수정·삭제가 0이다 — 그래서 편집 이전 본문에서 유도한 판정
(AC-THG-001 갈래 (c), AC-THG-003 픽스처, D14 정정 기록, DoD)은 그대로 성립한다. 바뀐 것은 **§D 매트릭스
이후 모든 줄 번호가 +16**이라는 것뿐이고, 이 보고서의 좌표는 **편집 이후 파일에서 재유도해 적었다**:
갈래 (c) = `acceptance.md:65-68`(종전 `:49-52`), 이음매 검증 절 = `:98`(종전 `:82`), 판별식 직접 단언 절
= `:99`, D14 정정 기록 = `:164`, DoD 첫 줄 = `:203`. **매트릭스 행 `AC-THG-003`은 `:19`로 불변**이다
(삽입 지점보다 위).

**이 회차가 여기서 배운 것(요구사항 아님, 기록).** 「줄 인용도 HEAD 읽기처럼 낡는다」가 파일 안에서도
성립한다 — 같은 파일의 **위쪽에 추가된 16행**이 아래쪽 인용을 전부 무효화했다. 감사 창 안의 쓰기는
그 자체로 프로세스 결함이며(리드가 그렇게 보고했다), 이번에는 순수 추가라 판정이 살아남았을 뿐이다.
훙크가 수정이었다면 내 인용 다섯 개는 **정확히 인용된 옛 텍스트**가 됐을 것이고 그것은 누락된 편집과
같은 증거를 낸다.

### 0.2 `§D.0` 판정 — **건전하다. 다만 드리프트 방어는 한쪽 방향뿐이다**

**⑴ 제시된 전제를 소스에서 확인했다(요약을 논거 자리에 세우지 않았다).**
`internal/spec/lint_coverage_sibling.go:114`가 형제 `acceptance.md` 전문에 대해
`ExtractRequirementMappings`를 돌리고, 그 함수의 섹션 정규식(`internal/spec/ears.go:128`)은
`(?i)maps\s+(REQ-[A-Z0-9-]+(?:\s*,\s*REQ-[A-Z0-9-]+)*)` — **`maps REQ-…` 형태만** 잡는다. 마크다운 표
칸은 원리상 읽히지 않는다. **전제 성립.**

**⑵ 「선행 결함이지 unfold가 만든 것이 아니다」 — 확인했다.** 커밋된 블롭에서 직접 셌다:
`git show HEAD:…/acceptance.md | /usr/bin/grep -c 'maps REQ-'` → **0**. 현재 파일 → **8**. 즉 0.1.3에서도
`maps` 선언은 0이었으므로 `REQ-THG-001..008`에 대해 같은 findings가 났을 것이고, `REQ-THG-009`는
거기 합류했을 뿐이다. **주장 성립.**

**⑶ 매트릭스와 §D.0이 일치하는가 — 8행 전수 대조했다. 일치한다.**
001→001 · 002→002,003 · 003→005,009 · 004→004 · 005→006,007 · 006→002 · 007→001 · 008→007,008.
합집합 = REQ-THG-001..009 **전부**. 산출 AC 없는 요구사항 0건. AC 개수 8 불변(매트릭스 8행 · §D.1
표제 8 · §D.0 8줄).

**⑷ 의도된 중복이 건전한가 — 순이득이다. 그러나 대칭이 아니다(D28, optional).**
§D.0의 자기 논거("둘 다 두고 갈라지면 감사가 본다")는 **한 방향에서만 기계적으로 참이다**:
- 매트릭스를 고치고 §D.0을 잊으면 → `CoverageRule`이 `CoverageIncomplete`로 **발화한다**(기계 포착 ✓).
- **§D.0을 고치고 매트릭스를 잊으면 → lint는 통과하고 매트릭스만 낡는다.** 기계는 표를 못 읽으므로
  포착 주체가 감사자뿐이다 — 그리고 **그 방향이 정확히 D21이 났던 방향**이다(매트릭스 행이 실제 산출
  요구사항과 어긋남).

즉 중복은 D21형 결함을 **막지 않고, 그것이 났을 때 감사자에게 두 번째 대조 자료를 준다.** 그것도
가치이지만 논거가 주장하는 것보다 약하다.
**그리고 더 나은 형태가 실재하고 비용이 0에 가깝다**: `ExtractRequirementMappings`는 파일 **전문**에
대한 정규식이므로 **표 칸 안의 `maps REQ-…`도 잡는다**(표 인식이 아니라 텍스트 매칭이다 — 소스 판독으로
확인). 매트릭스의 「요구사항」 칸을 `maps REQ-THG-005, REQ-THG-009` 형태로 적으면 **자리가 하나가 되어
드리프트할 곳이 사라진다.** 중복을 유지할 이유(사람이 읽는 표의 가독성)와 저울질할 항목이며 이번
카드가 반드시 할 일은 아니다 — optional로 남긴다.

**⑸ 「0 findings」의 공허 여부 — 리드의 논거는 성립하지만 내가 재현하지는 않았다(Gap).**
같은 명령이 한 실행 전에 이 SPEC에서 9를 냈다는 것이 빈 스윕이 아니라는 증거로 제시됐고, 그 형태는
옳다(대상이 있었음을 같은 도구로 보였다). **다만 나는 `moai spec lint`를 돌리지 않았다** — 이 트리에서
빌드한 바이너리로 재야 도구 출처 귀속이 서는데(판정 빌드와 판정 대상 트리), 감사 창에서 빌드를 돌리지
않았다. 전제는 소스로 확인했고 계수는 블롭으로 확인했으므로 판정에 공백은 없다.

**⑹ 코퍼스 전역 잔여(47건 중 43건) — 이 카드에서 쓸지 않는 처분이 옳다.** 그것은 이 SPEC의 가드와
무관한 **린트 규약과 산출물 관례 일반**의 문제이고, 이 카드에서 쓸면 접촉 범위가 43개 SPEC으로
번진다(D8의 정신과 정면으로 어긋난다). 후속 카드 후보이며 발행은 리드 소관 — 이 SPEC이 이미 같은
부류(공허한 초록 반복 · 판독 도구 사고 · `queueRootInsideTemp` 삼중 구현)에 쓴 처분과 동일하고,
이유도 같다.

---

## 1. unfold — **실질이다.** REQ-THG-009는 존재를 단언하고, exported 절은 실제로 구속된다

**⑴ 형식.** `spec.md:117` REQ-THG-009는 Ubiquitous 형이 성립한다 — 주어("The temp-root set … and the
discriminant that reads it"), 술어("shall be reachable through a package-level injection seam exported
from `internal/kanban`"). 뒤따르는 "so that …"과 "Absent the seam …"은 목적·근거절이고 규범 술어가
아니다. 실측: REQ 9건 연속·중복 0(`REQ-THG-001..009`), AC 8건 불변.

**⑵ 「사용」이 아니라 「존재」를 단언하는가 — 그렇다.** 규범 술어는 **도달 가능성(reachability)과
exportedness**이며 둘 다 생산 패키지의 성질이다. 「테스트가 그것을 쓴다」는 목적절에 있고 의무가
아니다. 그래서 이 요구사항은 픽스처가 하나도 없는 상태에서도 위반 판정이 가능하다 — 이음매가 없으면
그 자체로 위반이다. 접힌 형태에서는 불가능했던 성질이다.

**⑶ 「어떤 AC도 눈치채지 못한다」가 해소됐는가 — 그렇다.** `acceptance.md:19` 매트릭스 행이
`REQ-THG-005, REQ-THG-009`를 가리키고(D21), `:98`이 그 AC가 이음매의 검증임을 본문에 적으며, `§D.0`이
기계 판독 형식으로 한 번 더 적는다(§0.2). 세 자리가 서로 일치한다.

**⑷ exported 절이 load-bearing인가 — 그렇다. 그리고 D19 5행이 그것을 만든다.**
`internal/web/todo_section_test.go:193`의 사본은 `internal/web` 패키지 안에서 `TempOriginReason(base)`를
불러야 하고, `internal/cli/todo_queue_root_test.go:296`의 사본도 같다. 패키지 사설이면 두 사본 모두
판정식을 세울 수 없다. 즉 exported는 취향이 아니라 **판정식의 전제**다 — 수리의 주장과 일치한다.

**⑸ 갈림길 기록의 정직성 — 정직하다.** `spec.md:180`의 취소선(`~~현재 판정: REQ 8 / AC 8, Tier S 유지.~~`)
뒤에 `**리드 판정 2026-09-08: 반론 채택.**`과 분리 근거·D9 선례 기각 사유가 함께 있고, 진 논거(접기가
정직한 이유)의 본문이 **지워지지 않고 읽을 수 있는 상태로** 남아 있다. `plan.md:17`도 같은 형태다
(`~~S 유지 근거~~ [0.1.3까지의 논거 — 보존]`). 정리된 기록이 아니라 **뒤집힌 기록**이다: 무엇이
주장됐고 무엇이 그것을 뒤집었는지가 한 자리에서 읽힌다.

---

## 2. D18 — **닫혔다.** 두 축은 재현되고, 결과는 **요구사항 축으로도 살아남는다**

### 2.1 인용된 방법의 재현 (this run, this tree)

```
/usr/bin/grep -rlEi 'resolveTodoQueueRoot|BacklogPathForRoot' internal/ --include='*.go'      → 23
  … --include='*_test.go'                                                                     → 15
/usr/bin/grep -rlE 'newTodoCmd|newTodoStore|readTodoQueue|todoBodyFor|liveTodoQueueRootReason' \
     internal/ --include='*_test.go'                                                          → 10
union(테스트 파일)                                                                            → 22
```

`plan.md:38-40`의 §C 기대값 23 / 10과 §C.1의 합집합 22가 **정확히 재현된다.** 제외 표의 17건을 빼면
남는 파일 5건(`todo_root_test.go`·`todo_root_contract_test.go`·`todo_queue_root_test.go`·
`todo_axisa_guard_test.go`·`todo_section_test.go`)이고, 14개 행이 전부 그 5건 안에 있다 — 산술이 맞는다.

### 2.2 제외 판정의 검증 (표본이 아니라 기준으로 봤다)

- **git 픽스처 계열** — `todoFixture`(`internal/cli/todo_test.go:60-67`) 본문 실측: `t.TempDir()` +
  `t.Setenv("CLAUDE_PROJECT_DIR", root)` + **`initGitRepo(t, root)`**. 주석이 스스로 "queue-root
  resolution goes through git"라 적는다. → `primaryCheckoutRoot`가 답하므로 가드 미도달. **기준 성립.**
- **명시 루트 계열** — `BacklogPathForRoot(<명시 루트>)`만 부르고 리졸버를 지나지 않는다. 표본 판독
  (`todo_analysis_test.go:160` `todoBacklogPath(root)`)에서 성립.
- **구조 검사 · 심볼 목록 · 렌더 결과 계열** — 각각 기준대로다. `todo_section_test.go`를 파일이 아니라
  **행 단위**로 `:81`·`:133`·`:155`만 제외하고 `:193`을 C행에 남긴 것도 정확하다.

### 2.3 요구사항 축의 독립 재유도 — **행은 하나도 늘지 않는다. 다만 방법은 아직 자기 결과를 낳지 못한다**

「심볼이 아니라 요구사항으로 훑으라」는 지시대로, 두 축을 쓰지 않고 **리졸버에 도달하는 생산 소비자
전수**를 유도했다:

```
/usr/bin/grep -rn 'ResolveTodoQueueRoot' internal/ --include='*.go' | (테스트 제외)
  → internal/web/todo_queue_read.go:33   (readTodoQueue)
  → internal/cli/todo.go:72              (resolveTodoQueueRoot)
```
...
| 경로 | 좌표 | 축 2 시드에 있는가 |
|---|---|---|
| `newTodoStore` | `internal/cli/todo.go:78` | 있다 |
| **`todoLandedRef`** | `internal/cli/todo.go:88-89` | **없다** |
| **`todoLandedRefResolved`** | `internal/cli/todo.go:97-98` | **없다** |
| `readTodoQueue` ← `buildTodo` ← **`handleTodo`** | `internal/web/todo_view.go:32` ← `internal/web/screens.go:155` | 라우트 경유는 **없다** |

그 미포함 경로로 리졸버에 도달하면서 **두 축의 합집합 22건에 들어오지 않는** 테스트 파일 5건을
찾았다: `internal/cli/{todo_analysis,todo_undone,todo_edit_move,todo_landing}_test.go`(전부
`todoLandedRef`/`todoBacklogPath` 경유)와 `internal/web/todo_route_test.go`(`/todo` 라우트 경유).

**다섯 건 전수 판독 결과 — 영향 행은 늘지 않는다.** 앞의 넷은 전부 `todoFixture(t)`(= git 저장소)를
쓰므로 가드에 도달하지 않고, `todo_route_test.go`는 상태 코드·네비게이션 행·아이콘 등 **렌더 형태만**
단언해 `todo_section_test.go:81` 계열과 같은 기준으로 제외된다. **그러므로 §C.1의 14건은 요구사항
축으로 재유도해도 그대로다** — 이것이 D18을 닫는 근거다.

**남는 것은 귀속뿐이다(D24, optional).** 「전수」의 근거로 적힌 두 축은 여전히 자기 결과를 재유도하지
못한다: 위 5건이 리졸버에 도달하는데 두 축 어디에도 잡히지 않고 제외 기준 표에도 이름이 없다.
D20이 고치려던 결함(결과가 방법보다 넓거나 좁다)이 **범위만 줄어든 채 살아 있다.** 실질 피해는 0으로
측정됐으므로 optional로 남긴다.

### 2.4 「전수가 아니었다」의 파급 재검토 목록 — **거의 완전하다**

카드가 나열한 5건(C행 판정식 뒤집힘 · `AC-WTQ-008` 유지 · §5 재인용 · 패키지 수 · Tier)에 더해,
0.1.3의 열거에 기댄 다른 기재가 있는지 훑었다. 하나 더 있다 — **열거에서 유도된 계수가 갱신되지 않은
자리 다섯 곳**(D25). 그밖에 열거에 의존한 판단은 발견하지 못했다: D8의 접촉 금지 목록은 *변경* 대상이라
무관하고(카드 주장과 일치), 마일스톤 구조·AC 개수·범위 경계는 열거와 독립이다.

---

## 3. D19 — 표를 **7행 전수로** 검증했다. **6행은 옳고, 7행째가 성립하지 않는다**

7건 전부의 본문을 좌표로 열어 단언문을 직접 읽었다. 「실측 단언」 칸은 내가 읽은 것이다.

| # | C행 사본 | 실측 좌표 | 실측 단언 | 0.1.3 판정식 | 0.1.4 판정식 | 표와 일치? |
|---|---|---|---|---|---|---|
| 1 | `PureFallbackWritesNothing` | `todo_root_test.go:124` (표 `:125`) | mtime 불변(`:142-146`) + `os.Stat(fallbackRoot)`가 **`IsNotExist`**(`:147-149`) | 불성립(부재 단언) | **성립** | **일치** |
| 2 | `ReadThroughToProjectLocal` | `:155` (표 `:156`) | `got != dir → Fatal`(`:163-165`) + `BacklogPathForRoot(got)`로 3항목 로드 | 불성립(대상이 가드 반환값과 동일) | **성립** | **일치** |
| 3 | `AdoptionLandsWhereConsumersRead` | `todo_root_contract_test.go:14` | `os.Stat(BacklogPathForRoot(root))` 성공(`:23`) + bare-join 부재 | **성립(유일)** | **성립(두 겹)** | **일치** |
| 4 | `AdoptingAndPureResolversAgreeWhenAdoptionFails` | `:40` | `adopting == pure`(`:59-62`) + stat 성공(`:64-67`); 픽스처가 `MkdirAll`을 **고의로 실패**시킨다(`:49-57`) | 불성립 | **성립** | **일치** |
| 5 | `TodoSectionReadsThroughToProjectLocalQueue` | `internal/web/todo_section_test.go:193` (표 `:194`) | 렌더 3행 + mtime 불변 + `fallbackRoot` **부재**(`:216-218`) | 불성립 | **성립 — 단 exported 요건이 여기서 구속** | **일치** |
| 6 | `SilentOnHomeFallbackFixture` | `internal/cli/todo_queue_root_test.go:296` (표 `:298`) | `liveTodoQueueRootReason() != "" → Fatal`, 즉 **침묵**(부재형) | 불성립 | **성립** | **일치** |
| 7 | `AxisACanaryHomeSweep_TodoFamily` | `internal/cli/todo_axisa_guard_test.go:69` | 자식 실행의 `--- PASS` 계수 > 0(`:117-119`) + canary HOME 아래 엔트리 0(`:121-124`) | 불성립(부재 단언) | **성립이라고 적혀 있으나 — 성립하지 않는다** | **불일치 → D23** |

1-6행: 표의 실측·판정이 전부 내 판독과 일치한다. 특히 3행이 유일하게 0.1.3 판정식으로도 성립한다는
것, 5행에서 exported 요건이 구속된다는 것 둘 다 확인했다.

### D23 (blocking, major) — C행 7행의 처분이 **원리상 구성 불가능**하다. 이음매는 프로세스를 건너지 않는다

— `plan.md:201`(M2 종료 조건) · `plan.md:93`(C행 처분) · `plan.md:190`(D19 표 7행) ·
`internal/cli/todo_axisa_guard_test.go:110`(자식 프로세스 기동) — Severity: **major** — Class: **blocking**

**기제(실측).** `TestAxisACanaryHomeSweep_TodoFamily`의 판정 대상은 **자기 프로세스가 아니다**:
`:110` `cmd := exec.Command("go", "test", "./internal/cli/", "-run", "TestTodo", "-count=1", "-v")`로
**별도 프로세스**를 띄우고, 그 자식이 canary HOME 아래에 무엇을 만드는지를 센다. 그런데 REQ-THG-009가
고정한 이음매는 **패키지 수준 주입 이음매**이고(`spec.md:117`), `plan.md:169`가 그 형태를 못박는다 —
"`HomeDirFn`의 것과 같은 급 … 패키지 수준 함수 변수 + 테스트 헬퍼". **Go 패키지 변수는 `exec.Command`가
띄운 자식 프로세스에 전달되지 않는다.**

**귀결이 둘인데 둘 다 나쁘다.**
- 부모에서 이음매를 스텁하고 `TempOriginReason(canary)`가 `isTemp == false`라고 단언하면 **문장은
  선다.** 그러나 그 단언은 **자식의 어떤 픽스처에 대해서도 아무 말을 하지 않는다** — 자식은 스텁이
  없는 새 프로세스이므로 여전히 전부 임시 base에서 돈다. 즉 **판정식이 서지만 공허하다.**
- 자식의 base를 비임시로 만들려면 이음매가 **환경변수로 건너가야** 하는데, REQ-THG-009도 M1도 그
  형태를 요구하지 않는다. 현재 명세 아래에서는 **구성 자체가 불가능하다.**

**왜 blocking인가.** `plan.md:201` M2 종료 조건은 「C행 **7건** 사본이 판별식 단언을 들고 PASS」를
요구한다. 그 조건은 7행에 대해 **충족될 수 없거나, 공허하게 충족된다.** 이 SPEC은 같은 자리에서
공허한 판정문을 세 번(D2·D10·D14) 냈고, 4차가 **공허한 판정식을 공허한 판정식으로 바꾸려던 것**을
막았다(D19). 남은 1/7을 그대로 두면 run-phase는 「선다」는 쪽으로 갈 공산이 크고 — 부모 쪽 한 줄은
쓰기 쉽다 — 그 순간이 네 번째다. 본 SPEC 자신의 문장이 이 판정의 근거다: **"실패할 수 없는 판정식은
판정식이 아니다"**(`plan.md:194`).

**Required fix — 둘 중 하나. 어느 쪽도 두 줄 편집이다.**
(i) **이음매를 프로세스 경계 너머로 넓힌다** — REQ-THG-009(또는 M1의 형태 절)에 「환경으로도 선언
가능」을 더해 자식이 상속하게 한다. 그러면 7행의 사본이 실제로 서고 판정식이 판별력을 갖는다. 대가:
생산 코드가 환경변수를 읽게 되고, 그것은 `HomeDirFn` 선례보다 넓은 표면이다.
(ii) **7행의 처분을 「사본」에서 「수용된 손실」로 바꾸고 그 자리를 명시한다** — 이 행은 비임시 사본을
가질 수 없으므로 `runTodo` 게이트의 증거이기를 그친다는 것을 **적고**, 그 커버리지가 어디로 가는지를
이름 부른다. 갈 곳은 이미 쓰여 있다: **M3 경계 보고**(`plan.md:206-215`)의 항목 2가 정확히 "t422 게이트와
t536 가드는 서로 다른 층이고 후자가 전자의 우회를 덮는다"를 요구한다. 그리고 `plan.md:201`의 「C행
7건」을 「C행 6건 + 7행은 손실 기록」으로 고친다.

권고는 **(ii)** 다. 생산 표면을 넓히지 않고, 이미 쓰인 산출물이 그 자리를 받으며, 이 SPEC이 반복해
대가를 치른 실패 계열(공허한 판정식)을 **손실로 명시하는 쪽**으로 닫는다.

---

## 4. RED 조건 — **1-6행에 대해 진짜로 붉어진다.** M1 절은 init-snapshot 구멍을 실제로 닫는다

**⑴ RED가 실제로 발생하는가 — 그렇다(1-6행).** 기제를 따라가면: 사본의 base는 `t.TempDir()`이고,
Go의 `t.TempDir()`은 정의상 `os.TempDir()` 아래에 만든다. 생산 루트 집합은 `os.TempDir()`을 포함한다
(REQ-THG-002). 따라서 이음매 스텁을 제거하면 `TempOriginReason(base)`는 `isTemp == true`를 보고하고,
새 단언 `isTemp == false`는 **그 줄에서 FAIL한다.** 부재 단언도 아니고 가드 반환값과의 동어반복도
아니다 — 4차가 6/7을 무너뜨린 두 기제 어느 쪽에도 걸리지 않는다.

**⑵ 실패 방향이 안전한가 — 그렇다.** 정규화가 깨진 구현에서는 `isTemp`가 false로 나와 이 실연이
**붉어지지 않는다.** 그때는 M2 종료 조건(「RED 실연 출력이 §E.2에 있다」)이 **충족되지 않아** 멈춘다 —
조용히 초록으로 넘어가는 방향이 아니다. fail-safe다.

**⑶ M1 절이 init-snapshot 구멍을 닫는가 — 닫는다.** `plan.md:172` M1 종료 조건은 「**이음매가 호출
시점에 읽힌다는 증거** — 스텁을 건 뒤 `TempOriginReason` 판정이 **바뀌는 것**」을 요구한다. 스텁은
언제나 패키지 초기화 **이후**에 걸리므로, 구현이 init 시점에 집합을 값으로 복사했다면 판정은 바뀌지
않고 그 증거가 산출되지 않는다 — M1이 거기서 멈춘다. 이것은 D19가 명시적으로 지목한 형태
(`plan.md:199`: "구현이 집합을 호출 시점이 아니라 init 시점에 값으로 복사해 이음매가 사실상 안
듣게 되면")를 **M2가 아니라 M1에서** 잡는다는 뜻이고, 순서상 옳다 — M2의 사본이 서기 전에 걸린다.

**남는 위험(결함 아님, 기록).** ⑶의 증거는 「판정이 바뀐다」는 관측이므로, 구현이 스텁을 **부분적으로**
듣는 형태(예: 일부 루트만 이음매 경유)는 통과할 수 있다. AC-THG-002/006이 생산 집합 쪽을 겨누므로
전면 공백은 아니지만, 「부분 이음매」는 어느 AC도 겨누지 않는다.

---

## 5. AC-SA-011 — **인용 넷을 전부 확인했다. 결론을 지지한다**

형제 SPEC 산출물에서 직접 재측정했다(절 인용은 자기 구분자로 경계, `sed -n '/^## 5\./,/^## 6\./p'`).

- **§5의 위임 조항** — `SPEC-STATE-ANCHOR-001/spec.md` **108-116행**(실측: `## 5.` = 108, `## 6.` = 117).
  표제가 "미결 설계 결정 — 운영자 소관(**본 SPEC에서 구현하지 않는다**)"이고, 본문이 「거부하는 안 /
  유지하는 안」 둘을 놓고 고르지 않는다. **일치.**
- **§5:115** — 그 절의 마지막 문단이 한 줄 안에 **둘 다** 담는다: "이 결정은 본 SPEC의 어느 REQ에도
  영향을 주지 않는다" · "본 결정의 구현은 **별도 후속 카드로 발행하는 것이 적절하다**." **일치.**
  그리고 `sed -n '100,112p'`가 이 줄에 **네 줄 못 미친다**는 것도 산술로 확인된다 — §8 ⑵의 기제 서술은 참이다.
- **REQ-SA-011의 두 번째 가지** — `spec.md:106` 원문: "… **a mutant that produces no pollution shall be
  reported alongside the one that does, naming the guard boundary it reveals.**" **일치, 축자 인용이다.**
- **테스트가 그 요구사항을 되인용한다** — `internal/cli/todo_axisa_guard_test.go:174` `t.Fatalf` 문구에
  "REQ-SA-011: a pollution-less mutant is a finding, not a pass"가 실제로 있다. **일치.**

**넷이 결론을 지지하는가 — 지지한다. 그리고 한 조각이 카드의 서술보다 더 강하다.** 두 번째 가지의
"**alongside the one that does**"는 「오염을 낸 뮤턴트도 함께」를 요구한다 — 가드 착지 후 그 짝은
현재 트리에 없다. 그런데 M3 경계 보고의 항목 3(대조군: 같은 뮤턴트를 `412c8cb14`에서 돌리면 오염이
재현된다)이 **정확히 그 짝**이다. 즉 t536의 산출물은 두 번째 가지를 부분이 아니라 **문장 그대로**
이행한다. 이것은 카드가 명시하지 않은 정합이고, 판정을 강화한다.

**철회가 아닌가 — 아니다.** 그 SPEC은 한 글자도 바뀌지 않았고(`status: completed` 실측), 판정은
t536 자신의 §8에 있다. `acceptance.md:23`의 증거 칸을 「t536 이전 트리에 시각 고정된 기재」로 성격
규정한 것도 정확하다 — 그 칸은 과거 트리의 관측 기록이지 현재 트리에 대한 주장이 아니다.
**잔여(기록만)**: 그 SPEC만 읽는 독자는 자기 AC의 증거 칸이 시각 고정됐다는 사실에 도달할 경로가
없다(역참조 부재). 그러나 「그 SPEC을 수정하지 않는다」가 구속이므로 이 잔여는 **선택된 것이 아니라
부과된 것**이다. 결함으로 세지 않는다.

### §8의 판독 사고 3건 기록 — **기록한 것이 옳다**

세 인스턴스(`spec.md:183-187`)가 전부 참이다: ⑴ 대소문자 grep(실측 `/usr/bin/grep -c 'func Resolve'
internal/cli/todo.go` = **0** 재현), ⑵ 잘린 창(§5 = 108-116 vs 창 100-112 — 산술 확인), ⑶ 원본 대신 요약.
**옳게 기록한 이유 둘.** 첫째, 세 사고의 공통 모양(판독 도구가 결론을 정했다)은 **다음 편집자에게
쓰이는 교정**을 낳는다 — 「인용은 절의 자기 구분자로 경계 짓는다」. 이번 회차가 그 교정을 실제로 썼고,
그래서 §5 인용을 재현할 수 있었다(그리고 §0.1이 보이듯 **줄 번호 인용은 파일 내부 삽입에도 낡는다** —
같은 교훈의 확장이다). 둘째, **⑵는 리드 자신의 오류다.** 자기 오류만 빼고 감사자 오류만 남기는 기록이
흔한 실패인데, 여기서는 둘 다 있고 **행위자 이름이 양쪽 모두 익명**이다 — 선택적 보호가 아니라 일관된
처리이며, SPEC 본문은 행위자 귀속의 자리가 아니다. **정리된 기록이 아니라 정직한 기록이다.**

---

## 6. 자가 신고 2건 — **둘 다 판정한다**

**⑴ D20 처분 — 부분적으로만 닫혔다(D24, optional).** 각 행에 「축」 칸이 붙었고(실측: A·C 표에 전부
있다), 제외 기준 표가 5부류로 실려 있다 — 요구된 형태 그대로다. 그러나 §2.3이 보인 대로 두 축은
여전히 자기 결과를 재유도하지 못한다. 결과는 내가 요구사항 축으로 재유도해 **같음을 확인**했으므로
실질 피해 0이고, 귀속만 남는다.

**⑵ `queueRootInsideTemp`를 요구사항으로 만들지 않은 것 — 옳다.**
- **관측 자체는 참이다.** `internal/cli/todo_queue_root_test.go:236-253`을 읽었다: 양쪽에
  `EvalSymlinks`(존재 시), `underDir`(`:255-263`)의 `filepath.Rel` 기반 구성요소 포함, 형제 접두사
  면역(주석이 스스로 `"/tmp/x" vs "/tmp/xy"`를 든다). **REQ-THG-002/003이 고정한 규칙과 같다.**
- **범위 밖이 옳은 이유 셋.** ⓐ 그것은 `_test.go` 안의 **비노출 심볼**이고 다른 패키지다 — 통합하려면
  D8이 고정한 접촉 패키지 경계(`internal/kanban` + `internal/cli` 안내 1행)를 넘는다. ⓑ 두 구현은
  **목적이 다르다**: 하나는 픽스처 격리 가드(테스트 축), 하나는 생산 가드 — 나중에 정당하게 갈라질
  수 있는 축이다. ⓒ 규칙 자체는 REQ-THG-003이 못박으므로 드리프트가 **요구사항 수준에서 검출된다.**
- **덧붙일 관측(결함 아님).** REQ-THG-009가 `TempOriginReason`을 exported로 만드는 순간
  `queueRootInsideTemp`를 그 호출 한 줄로 바꾸는 비용이 거의 0이 된다 — 통합의 적기는 이 카드가
  **아니라 이 카드 직후**다. SPEC이 이미 「별도 카드 후보, 발행은 리드 소관」으로 처분했으므로
  추가 요구 없다.

---

## 7. D21 · D22 · 경계 보고 — **셋 다 닫혔다**

**D21 — 닫혔다.** `acceptance.md:19` 매트릭스 행이 `REQ-THG-005, REQ-THG-009`다(실측). `:98`이 본문에서
같은 것을 적고, `§D.0`이 기계 판독 형식으로 한 번 더 적는다(§0.2에서 8행 전수 대조). 셋이 일치한다.

**D22 — 닫혔다. 그리고 「오늘 RED」 주장이 참이다.** `acceptance.md:65-68`이 갈래 (c)를 **And 한 줄**로
더했고 AC 개수는 8로 불변이다(실측). RED 주장을 코드에서 확인했다:
`internal/kanban/todo_root_test.go:203-217` `TestResolveTodoQueueRoot_HomeUnresolvableWritesNothing`이
`dir := t.TempDir()` + `HomeDirFn = func() (string, error) { return "", os.ErrNotExist }`로 정확히 그
교차를 밟고, **`want := filepath.Join(dir, ".moai", "state", "todo")`**를 단언한다 — 즉 현재 트리는
교차에서 `base/.moai/state/todo`를 돌려준다. (c)가 요구하는 `base`와 **다르다**. **오늘 RED가 맞다.**

**경계 보고 명세 — run-phase가 검사 가능한 것을 낼 만큼 충분하다.** 세 항목이 `plan.md:207-215`에
있고 각각 기계 검사 가능한 형태다: ⑴ 뮤턴트의 정확한 형태 + 명령 + 출력 전문, ⑵ 어느 경계인가
(t422 게이트 = 테스트 축 / t536 가드 = 리졸버 층, 후자가 전자의 우회를 덮는다 —
`todo_axisa_guard_test.go:150`의 뮤턴트가 `runTodo`를 우회하지만 리졸버는 우회하지 못한다는 사실과
일치한다. **판독으로 확인**), ⑶ 대조군. 쓰는 시점이 run-phase임도 명시돼 있다 — 이번 감사에서 그
파일의 부재를 결함으로 세지 않는 근거다.

**대조군이 가드를 격리하는가 — 실질적으로는 격리한다. 다만 더 강한 대조군이 같은 트리 안에 있다
(D26, optional).** 기재된 대조군은 **트리 간**이다(`412c8cb14` vs HEAD). 두 실행의 차이는 엄밀히
「가드」가 아니라 「t536의 변경 전체」다 — 안내 1행과 테스트가 함께 들어 있다. 실질적으로는 디렉터를
만들거나 막는 생산 변경이 가드뿐이므로 결론은 선다. 그러나 **같은 트리 안의 대조군**이 더 강하고,
이 SPEC이 이미 갖고 있다: **판별식을 상수 `false`로 무력화한 뮤턴트**(AC-THG-007)를 우회 뮤턴트와
**겹쳐** 돌리면 오염이 돌아온다 — 트리 차이라는 교란 없이 「오염 부재의 원인은 리졸버 층 가드 하나」가
나온다. 한 줄 추가이며 기재된 ⑶을 대체하지 않고 보강한다.

---

## 8. 이번 회차 결함 목록 (구조화)

D23. **C행 7행의 처분이 구성 불가능 — 이음매가 프로세스 경계를 건너지 않는다** —
`plan.md:201`(M2 종료 조건) · `plan.md:93` · `plan.md:190`(D19 표 7행) ·
`internal/cli/todo_axisa_guard_test.go:110` — `TestAxisACanaryHomeSweep_TodoFamily`는
`exec.Command("go","test",…)`로 **자식 프로세스**를 판정 대상으로 삼는데, REQ-THG-009의 이음매는
패키지 수준 변수(`plan.md:169`)라 자식에 전달되지 않는다. 그 결과 「비임시 사본 + 판별식 직접 단언」은
구성 불가능하거나(자식을 비임시로 못 만듦), 부모 쪽에 세우면 **공허하다**(자식의 픽스처에 대해
아무 말도 못 함) — Severity: **major** — Class: **blocking** — Required fix: (i) 이음매를 환경 경유로도
선언 가능하게 넓히거나, **(ii) 권고** — 7행의 처분을 「수용된 손실」로 바꾸고(그 행은 `runTodo` 게이트의
증거이기를 그친다) 그 커버리지가 M3 경계 보고 항목 2로 감을 적은 뒤, `plan.md:201`의 「C행 7건」을
「C행 6건 + 7행 손실 기록」으로 고친다.

D24. 「전수」의 방법 귀속이 여전히 자기 결과를 낳지 못한다 — `plan.md:57-61`(두 축 표) ·
`plan.md:118` — `todoLandedRef`/`todoLandedRefResolved`(`internal/cli/todo.go:88`·`:97`)와 `/todo` 라우트
(`internal/web/screens.go:155` → `todo_view.go:32`) 경유로 리졸버에 도달하는 테스트 파일 5건
(`internal/cli/{todo_analysis,todo_undone,todo_edit_move,todo_landing}_test.go`,
`internal/web/todo_route_test.go`)이 두 축의 합집합 22건 **밖**이고 제외 기준 표에도 없다.
**실질 피해 0 — 다섯 건 전수 판독 결과 넷은 git 픽스처, 하나는 렌더 형태 단언이라 영향 행이 늘지
않는다(§2.3)** — Severity: minor — Class: **optional** — Required fix(선택): 축 2 시드에 두 심볼을
더하고 라우트 경유를 축 3으로 적은 뒤, 위 5건을 제외 기준 표에 기존 두 부류로 등재한다.

D25. 8→14 정정의 잔여 계수 — `plan.md:14`·`plan.md:17`(「4패키지」) · `plan.md:93`(「4개 패키지」) ·
`plan.md:142`(「C행 **5건**」) · `plan.md:172`(「§C.1의 **8건**」) · `spec.md:175`(「공허화 **5건**」) ·
`spec.md:193`(「3 → 4」) — ⑴ **패키지 수는 3이지 4가 아니다**: 14개 행의 패키지는 `internal/kanban` ·
`internal/cli` · `internal/web` 셋뿐이다(행 전수 귀속으로 확인). `spec.md:193`의 괄호가 그 오산을
스스로 드러낸다 — "(`internal/cli`가 두 갈래로 들어온다)"는 **축이 둘**이라는 뜻이지 패키지가 둘이라는
뜻이 아니다. 그 수치가 Tier 판정을 「강화한다」는 문장의 근거로 쓰였다(Tier M 자체는 REQ 9 > Tier S
상한 8만으로 성립하므로 판정은 흔들리지 않는다). ⑵ 나머지 넷은 8→14 정정 이전 계수가 남은 것이다
(M2 종료 조건 `plan.md:201`은 7·14로 **옳게** 갱신돼 있다) — Severity: minor — Class: **optional** —
Required fix(선택): 「4패키지」→「3패키지」, 「C행 5건」→「7건」, 「§C.1의 8건」→「14건」.

D26. 경계 보고의 대조군이 트리 간이라 t536 변경 전체와 교란된다 — `plan.md:214-215` —
같은 트리 안의 대조군(판별식 상수 `false` 뮤턴트를 우회 뮤턴트와 겹침)이 교란 없이 같은 결론을 낸다 —
Severity: minor — Class: **optional** — Required fix(선택): ⑶에 같은-트리 대조군 한 줄을 **보강으로**
더한다(트리 간 대조군을 대체하지 않는다).

D27. 산출물 상태 불일치(기록) — `acceptance.md`가 **미커밋 수정 상태**다(감사 창 안에서 도착; §0).
내용은 매트릭스와 **일치하며 결함이 아니다** — Severity: minor — Class: **optional** —
Required fix: 커밋하거나, 커밋하지 않기로 했다면 그 사실을 진행 기록에 남긴다.

D28. `§D.0`의 드리프트 방어가 한쪽 방향뿐이다 — `acceptance.md:26-38` — 매트릭스를 고치고 §D.0을
잊으면 lint가 발화하지만(기계 포착), **§D.0을 고치고 매트릭스를 잊으면 lint는 통과하고 매트릭스만
낡는다** — 그리고 그 방향이 정확히 D21이 났던 방향이다. `ExtractRequirementMappings`는 파일 전문에
대한 정규식이므로 **표 칸 안의 `maps REQ-…`도 잡는다**(소스 판독 확인) — 매트릭스 「요구사항」 칸을
그 형태로 적으면 자리가 하나가 되어 드리프트할 곳이 사라진다 — Severity: minor — Class: **optional** —
Required fix(선택): 중복을 유지하되 한쪽이 파생임을 명시하거나, 매트릭스 칸을 `maps` 형태로 바꿔
§D.0을 없앤다.

**D17(3차, optional)은 여전히 열려 있다** — 표준 지시에 따라 이번에도 범위 밖이며 등급 매기지 않았다.

---

## 9. Must-Pass Results / 카테고리 점수

- [PASS] **MP-1 REQ 번호 일관성** — `REQ-THG-001..009` 연속, 중복 0, 자리수 일관(실측:
  `/usr/bin/grep -c '^- \*\*REQ-THG-' spec.md` = 9, 고유 id 9).
- [PASS] **MP-2 GEARS 형식(요구사항 층)** — 이번 회차가 새로 판정하는 것은 REQ-THG-009뿐이며
  Ubiquitous("The `<subject>` shall …") 형이 성립한다(`spec.md:117`). **판정 층은 `spec.md`의 `REQ-XXX`
  항목이다** — `acceptance.md`의 Given-When-Then은 검증 층이므로 여기서 감점하지 않았다(Group 4 소관).
  1-3차가 닫은 다른 8건은 재론하지 않았다.
- [PASS] **MP-3 YAML frontmatter** — 12필드 전수 실재(`id`·`title`·`version:"0.1.4"`·`status: draft`·
  `created`/`updated` ISO·`author`·`priority: P2`·`phase`·`module`·`lifecycle: spec-anchored`·`tags` CSV)
  + `tier: M`·`era`·`related_specs`. 거절되는 snake_case 별칭(`created_at`/`updated_at`/`labels`/`spec_id`) 0건.
- [N/A] **MP-4 언어 중립성** — 단일 언어(Go) 프로젝트 내부 카드. 템플릿 구속 콘텐츠 아님.
- [PASS] **MP-5 D7 교차-SPEC** — 참조 2건 전부 실재하고 둘 다 `status: completed`(실측). retired/
  superseded/archived 아님. 그럼에도 두 건 모두에 명시적 화해 판정이 `spec.md §8`에 있다. BLOCKING 0건.
- [PASS] **MP-6 D8 크로스 플랫폼** — `syscall` 문자열 0건(spec/plan/acceptance 전부, `/usr/bin/grep -c`).
- [N/A] **MP-7 clarification gate** — `plan.md`에 `[NEEDS CLARIFICATION` 0건, `research.md` 부재(Tier M).

| Dimension | Score | Rubric Band | Evidence |
|---|---|---|---|
| Clarity | 0.90 | 0.75-1.0 | 요구사항 9건 모두 단일 해석. 감점은 D25의 잔여 계수 — `plan.md:142`가 「C행 5건」, `:172`가 「8건」이라 같은 문서의 `:201`(7·14)과 순간 모순된다 |
| Completeness | 0.85 | 0.75-1.0 | 전 절 실재, Out of Scope H3 5개 각각 `-` 항목 보유, Gaps 9항. 감점은 M2 종료 조건 한 행이 충족 불가(D23) |
| Testability | 0.85 | 0.75-1.0 | AC-THG-001 (a)(b)(c) 전부 이진 판정 가능, RED 조건이 명시·검증됨(§4), 뮤턴트+대조군 명세 존재. 감점은 C행 7행(D23)과 대조군 교란(D26) |
| Traceability | 1.00 | 1.0 | REQ 9건 전부 산출 AC 보유(매트릭스 + `§D.0` 기계 선언, 8행 전수 대조에서 일치). 존재하지 않는 REQ를 가리키는 AC 0건 |

**Overall: 0.89** (조화평균 0.897 — 보수적으로 0.89). **Tier M 임계 0.80을 넘는다.**

## 10. 회귀 점검 (4차 결함)

- **D18 — RESOLVED.** 두 축 재현 + **요구사항 축 독립 재유도로 행이 늘지 않음을 확인**(§2). 누락됐던
  2건이 A행에 실렸고, 감사자가 제안한 `todo_axisa_guard_test.go:69`도 C행에 실렸다. AC-SA-011 교차
  판정도 `AC-WTQ-008`과 같은 급으로 있다(§5).
- **D19 — PARTIALLY RESOLVED.** 7행 중 **6행에서 새 판정식이 성립하고 RED 조건도 실제로 붉어진다**(§3·§4).
  7행째는 성립하지 않는다 → **D23으로 승계**. 정체(stagnation) 아님: 이 표면은 0.1.4가 처음 연 것이고
  (7행 자체가 이번 판본에 신규 등재), 4차가 지적한 4건은 전부 실제로 고쳐졌다.
- **D20 — PARTIALLY RESOLVED.** 축 표기와 제외 기준 표는 이행됐고, 방법의 자기 재유도만 남았다 → D24.
- **D21 — RESOLVED.** §7 (그리고 `§D.0`이 세 번째 자리로 보강 — §0.2).
- **D22 — RESOLVED.** §7, RED 실재까지 코드에서 확인.

## 11. 측정 기록 (Claim / Evidence / Baseline-attribution / Gaps / Residual-risk)

**Claim A** — §C.1의 14건은 요구사항 축으로 재유도해도 그대로다.
**Evidence** — 생산 소비자 전수(`/usr/bin/grep -rn 'ResolveTodoQueueRoot' internal/ --include='*.go'`
→ `internal/web/todo_queue_read.go:33`, `internal/cli/todo.go:72`) → 한 단계 전개 → 두 축 밖 소비자
3종(`todoLandedRef`·`todoLandedRefResolved`·`/todo` 라우트) → 그 경유 테스트 파일 5건 전수 판독
(`todoFixture` 본문 `internal/cli/todo_test.go:60-67`의 `initGitRepo`, `todo_route_test.go`의 렌더 단언).
**Baseline-attribution** — this run, this tree, HEAD `075dd8d4b`(+ `acceptance.md` 미커밋 16행; 이 주장은
`internal/` 소스에만 의존하므로 그 훙크와 무관하다).
**Gaps** — 실행으로 확인하지 않았다(가드 부재로 원리상 불가). 판독 유도다.
**Residual-risk** — 구현이 술어를 더 좁은 지점에 넣으면 파급이 줄 수 있다(SPEC이 이미 §E.2 기록을 요구).

**Claim B** — D19 표 7행 중 6행이 실측과 일치하고 7행째가 성립하지 않는다.
**Evidence** — 7개 테스트 본문 전수 판독(§3 표에 좌표와 단언문). 7행의 기제는
`internal/cli/todo_axisa_guard_test.go:110`의 `exec.Command("go","test",…)` 직접 판독 +
`plan.md:169`의 이음매 형태 규정(패키지 수준 함수 변수) 대조.
**Baseline-attribution** — this run, this tree, HEAD `075dd8d4b`.
**Gaps** — 사본이 아직 없으므로 실행으로 반증하지 못했다. 판정은 「사본이 구성 가능한가」에 대한
것이고, 그것은 이음매의 형태와 판정 대상의 프로세스 경계에서 결정된다.
**Residual-risk** — run-phase가 이음매를 환경 경유로 확장하면 7행도 성립한다. **그러나 현재 명세가
그것을 요구하지 않는다** — 그것이 결함이다.

**Claim C** — AC-SA-011 판정의 인용 넷이 전부 참이다.
**Evidence** — `SPEC-STATE-ANCHOR-001/spec.md` §5를 자기 구분자로 경계 지어 인용(108-116),
REQ-SA-011 축자 인용(`spec.md:106`), `acceptance.md:23`·`:152`, `status: completed`, 테스트의 되인용
(`todo_axisa_guard_test.go:174`). **Baseline-attribution** — this run, this tree.
**Gaps** — 그 SPEC의 `plan.md` §D11(§5가 참조하는 구속 조건)은 읽지 않았다 — 판정에 쓰이지 않았다.
**Residual-risk** — 없음으로 본다. 네 조각이 독립적으로 같은 방향이다.

**Claim D** — `§D.0`의 존재 이유로 제시된 lint 전제가 참이고, 그 finding은 선행 결함이다.
**Evidence** — `internal/spec/lint_coverage_sibling.go:114`(형제 파일 전문에 `ExtractRequirementMappings`)
+ `internal/spec/ears.go:128`의 섹션 정규식(`maps\s+(REQ-…)`) 직접 판독; 커밋된 블롭 계수
`git show HEAD:…/acceptance.md | /usr/bin/grep -c 'maps REQ-'` → **0**, 현재 파일 → **8**.
**Baseline-attribution** — this run, this tree.
**Gaps** — **`moai spec lint`를 직접 돌리지 않았다.** 도구 출처 귀속(이 트리에서 빌드한 바이너리)을
세우려면 빌드가 필요한데 감사 창에서 빌드를 돌리지 않았다. 「9 → 0」이라는 수치는 리드의 보고이며
내가 재현한 것이 아니다 — 다만 전제(정규식)와 계수(0 → 8)는 내가 직접 쟀다.
**Residual-risk** — 설치된 바이너리가 트리보다 오래됐다면 리드가 본 9라는 수치는 다른 규칙집합의
산물일 수 있다. 판정(§D.0이 옳은 형태다)은 그 수치에 의존하지 않는다.

**Gaps (이번 회차 전체)** — ⑴ `plan.md:38`의 **344는 이번에도 재측정하지 않았다**(워크트리 격리가 실
HOME을 겨눈 `find`를 거절한다). SPEC이 이미 Gap으로 등재하고 run-phase 재측정을 fail-safe로 설계했다.
⑵ **리눅스 셀 미측정** — SPEC이 스스로 Gap 등재, CI 소관. ⑶ 판별식이 없으므로 **어떤 실동작도 실행
검증하지 못했다** — run-phase 소관이며 AC가 겨눈다. ⑷ 뮤턴트를 걸어보지 않았다 — 공유 트리에서
고의 훼손을 하지 않는 규율에 따른다. ⑸ `moai spec lint` 미실행(Claim D). ⑹ D17은 지시에 따라
판정하지 않았다. ⑺ **감사 창 안에서 트리가 한 번 움직였다**(§0) — 순수 추가였으므로 판정은
살아남았고, 좌표는 편집 이후 파일에서 재유도했다.

---

## 12. Recommendation

**운영자의 implementation-kickoff 게이트에 올릴 수 있는 상태에 매우 가깝다 — 그러나 아직 아니다.**
막는 것은 **D23 한 건**이고, 그것은 이 SPEC이 세 번 대가를 치른 바로 그 실패 계열(공허한 판정식)의
남은 1/7이다. 점수(0.89)는 임계(0.80)를 넘고 다른 blocking은 없다. **이 FAIL은 점수가 아니라 그 한
조항에서 나온다** — 운영자가 「부채를 안고 수용(PASS-with-debt)」을 고를 때 무엇을 안는지가 정확히
하나로 특정된다는 뜻이다.

**막는 것 (1건)**

1. **D23 — `plan.md:201` M2 종료 조건의 「C행 7건」.** 7행(`TestAxisACanaryHomeSweep_TodoFamily`,
   `internal/cli/todo_axisa_guard_test.go:69`)은 자식 프로세스를 판정 대상으로 삼고, REQ-THG-009의
   이음매는 패키지 수준 변수라 프로세스를 건너지 않는다. **닫는 조건**: (ii) 그 행의 처분을 「수용된
   손실」로 바꾸고 커버리지가 M3 경계 보고 항목 2로 감을 적은 뒤 `plan.md:201`을 「C행 6건 + 7행 손실
   기록」으로 고친다 — 두 줄. 또는 (i) 이음매를 환경 경유로 넓힌다(생산 표면이 넓어진다).

**운영자가 수용을 고를 경우 — run-phase에서 이 한 조항이 만들 위험은 하나다.** 구현자가 7행에 대해
부모 쪽 `TempOriginReason` 단언 한 줄을 쓰고 「7건 전부 판정식을 들었다」로 기록하는 것. 그 한 줄은
**자식의 어떤 픽스처에 대해서도 아무 말을 하지 않으므로** 네 번째 공허한 초록이 된다. 수용한다면
그 사실을 진행 기록에 미리 적어 run-phase가 그 자리에서 「손실」을 쓰게 하는 것이 최소 방어다.

**재량 (5건, 전부 optional)** — D24(전수 귀속) · D25(잔여 계수, 「4패키지」→3 포함) ·
D26(같은-트리 대조군 보강) · D27(미커밋 `acceptance.md`) · D28(`§D.0`의 한쪽 방향 방어).

**코퍼스 전역 잔여(47건 중 43건의 `maps` 선언 부재)** — 이 카드에서 쓸지 않는 처분에 **동의한다**(§0.2 ⑹).

**run-phase에서 계속 지켜볼 것 (D23이 닫히든 수용되든 동일)**

1. **RED 실연이 실제로 붉어진 출력** — `progress.md §E.2`에 명령·출력 전문. 요약("실패를 확인했다")은
   증거가 아니다. 이 SPEC은 같은 자리에서 세 번 공허했다.
2. **M1의 「호출 시점에 읽힌다」 증거** — 스텁 전후로 `TempOriginReason` 판정이 **바뀌는** 출력.
   이것이 없으면 init-snapshot 구현이 C행 사본 전부를 조용히 공허하게 만든다.
3. **보존 이관 5건의 원 단언이 지워지지 않았는지** — 특히 `AdoptsLocalQueue`(AC-WTQ-008 산출)와
   `FallbackAdoptsExistingLocalQueue`. 「새 동작에 맞춰 고쳐 쓰기」가 조용한 철회의 경로다.
4. **C행 사본이 판별식 단언을 실제로 들고 있는지** — 그리고 7행에 대해 무엇을 했는지가 §E.2에
   명시되는지(손실인지, 부모 쪽 한 줄인지).
5. **경계 보고의 대조군이 실제로 돌았는지** — `412c8cb14`에서의 오염 재현 출력이 파일 안에 있는지.
   없으면 「뮤턴트가 그냥 안 돌았다」와 구별되지 않는다.
6. **리눅스 셀 판정을 macOS 통과로 대체하지 않는지** — CI 매트릭스 출력을 §E.2에 인용.
7. **`~/.moai/todo` 계수 344의 진입 재측정** — 다르면 멈추고 보고(SPEC 설계대로).
8. **매트릭스와 `§D.0`이 갈라지지 않는지** — 한쪽만 고치는 편집이 들어오면 D28의 약한 방향이 발화한다.

**Tier**: `M`으로 유지가 옳다. REQ 9건은 Tier S 상한 8을 넘으므로 판정은 계수 논쟁(D25의 「4패키지」)과
독립적으로 성립한다. 산출물 3파일은 이미 Tier M 집합과 일치한다.

**점수 회귀 없음**: 0.75 → 0.875 → 0.8125 → (4차 부분 감사, 미산출) → **0.89**. STOP 조항 미발동.
