...
- REQ-CONT-001-001: The CLI shall produce all non-zero exit codes through the existing `ExitCoder` mechanism mapped at the main.go boundary, and RunE/PostRunE bodies shall not call `os.Exit` — removing the 11 call sites across 8 files (grep-verified against the live tree, comment lines excluded): `astgrep.go:119`; `hook.go:231` and `:362`; `spec_drift.go:74`; `harness/execute.go:333`; `hook_pre_push.go:196`; `spec_lint.go:70` and `:96`; `migrate_agency.go:650` and `:652`; `agentlint/workflow_lint.go:158`. The `hook_pre_push.go:196` site is a removal target (NOT an approved exception): the inline comment at line 194 claims "This is the ONLY os.Exit site" and lines 102-103 document `decideExit` as a pure decision function, but the boundary mapping is performed via `os.Exit(2)` instead of an `ExitCoder` return — converting it to `return ExitCoder{2}` lets the main.go boundary map the code AND lets defers run, and REQ-CONT-001-006 (stderr routing) is applied before the return. Approved exceptions OUTSIDE the 8-file removal scope: `launch_exec_windows.go:37,41` and `update.go:487` — both perform Windows process replacement (re-execing the binary as a child because `syscall.Exec` is unavailable on Windows, so `os.Exit` after child success is the standard boundary), parallel to the `main.go` ExitCoder boundary.
...
- REQ-CONT-001-003: When `moai spec status --sync-git` would mutate git state (spec_status.go:205-235), the CLI shall gate the mutation on the `--yes` flag (the canonical non-interactive flag, wired at spec_status.go:40 as the `autoConfirm` parameter to `syncGitSpecStatuses` and gated at `:205`) and shall abort with a diagnostic in a non-TTY context instead of blocking on `fmt.Scanln`. The `--confirm` flag (`syncConfirm`, registered at spec_status.go:63) is a dead flag — registered but never read (only `syncYes` is passed to `syncGitSpecStatuses` at `:40`; `syncConfirm` has zero readers) — and shall be removed; `--yes` is the sole non-interactive gate.
