...
- [ ] `DetectProjectType(root string) (ProjectType, error)` 구현
  - 디렉터리 구조 기반 분류 로직 (cmd/ -> cli, public/ -> web-app, api/ -> api, 기본 -> library)
- [ ] 테이블 기반 단위 테스트 작성 (`detector_test.go`)
  - testdata/ 디렉터리에 각 언어/프레임워크별 샘플 프로젝트 구조 구성
  - 에지 케이스: 빈 디렉터리, 혼합 언어, 감지 불가 프로젝트
...
- [ ] 테이블 기반 단위 테스트 작성 (`methodology_detector_test.go`)
  ...
  - testdata/ 디렉터리에 각 시나리오별 샘플 프로젝트 구조
...
- [ ] `Validate(root string) (*ValidationResult, error)` 구현
  ...
  - `CLAUDE.md` 존재 여부 확인
  - 필수 하위 디렉터리 확인 (config/sections/, specs/, memory/)
...
| `initializer.go` | 초기화 오케스트레이션, 디렉터리 생성, 의존 모듈 호출 | ~350 |
