...
| AC-CDS-007 | MUST | `go build ./...`; `go vet ./...`; `go test ./... -count=1` | build `0`, vet `0`; suite identical to baseline (119 ok, same 2 pre-existing `internal/hook` failures) | **PASS-WITH-DEBT** — see Pre-existing baseline failure above |
