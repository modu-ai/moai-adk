...
- **P2 증강**: harness `learning` 블록, git-strategy `merge_method`, security sandbox 리스트 키, mx 정책 맵/리스트, observability hook_metrics(실측 결과 no-op)를 기존 seam/typed 경로 재사용으로 콘솔 표면에 정합화.
...
| F6 | `security.sandbox.{network_allowlist,env_scrub_extra}` | **행동적**: sandbox 패키지 소비 (`internal/sandbox/docker.go` allowlist append, `profile.go`, `env.go` scrub; `SecuritySandbox` @MX:ANCHOR fan_in≥3 — loader→launcher) | read-only raw view ×2 (리스트 값 — 기존 `security.extra_dangerous_bash_patterns` raw view 선례) |
...
| F9 | `observability.hook_metrics.{output_path,slow_hook_threshold_ms}` | **이미 노출** (schema_sections.go observability seam 필드 실측) | no-op 확인 완료; 회귀 핀 AC만 추가 |
...
- **REQ-WC14-010**: 콘솔은 3개 mode profile(manual/personal/team) 각각의 `git_strategy.<mode>.merge_method`를 닫힌 enum select 편집 필드로 노출하고(shall), 기존 git-strategy typed 저장 경로(dirty-flag Save)로 영속화해야 한다.
...
- **REQ-WC14-020**: 콘솔은 `security.sandbox.network_allowlist`와 `security.sandbox.env_scrub_extra`를 읽기 전용 raw view로 렌더해야 한다(shall).
...
REQ ↔ AC 매핑은 acceptance.md §C AC Matrix의 매핑 컬럼이 SSOT다. 총 16 REQ / 21 AC.
