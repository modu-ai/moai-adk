...
# Plan — SPEC-WEB-CONSOLE-014 (v0.1.0)
...
- **authoring 시점(2026-07-10) 실측**: `.moai/specs/SPEC-WEB-CONSOLE-013/`는 디스크에 **아직 존재하지 않는다** (forward reference — 같은 감사 리포트에서 분기된 Track). run 진입 pre-flight C-1이 013의 존재+완결(또는 orchestrator의 명시적 순서 해제)을 확인하고, 미충족 시 blocker report로 반환한다.
...
| 파일 | 변경 성격 |
|------|-----------|
| `internal/settings/schema_sections.go` | merge_method 3필드 추가(typed), learning.auto_apply 편집 FieldDef 철거 + ReadOnlyDisplayFields 추가, RawViewBlocks 5건 추가(learning.tier_thresholds, learning.rate_limit, security.sandbox.network_allowlist, security.sandbox.env_scrub_extra 2건, mx 2건), mx 렌더 그룹 등재 |
...
- **B10**: `learning.auto_apply` 편집 필드 **철거**는 기존 노출의 축소 — TUI bridge parity(TestTUIRendersSchemaFieldSet 등)와 웹 라운드트립 테스트(TestApplySchemaEditsAllFieldsRoundTrip)가 해당 필드를 순회 중이면 함께 갱신 필요 (REQ-WC14-061의 "양방향" 의미).
...
3. **C-3 B1/B2 반증 탐색**: `grep -rn "MaxPerWeek\|CooldownHours" internal/ --include="*.go" | grep -v _test` 재실행 — enforcement 소비자 신규 발견 시 blocker. `grep -rn "pre_allowlist\|session_rules" internal/config internal/permission --include="*.go"` 재실행 — struct 바인딩 신설 발견 시 blocker.
...
- 가드 테스트(M1)는 **현행 코드에서 즉시 GREEN이어야 정상** — regression pin/characterization이며 TDD RED 대상이 아니다. RED가 나오면 그 자체가 현행 노출 결함 발견 → blocker report.
- 편집 필드 추가(M3)는 TDD: 라운드트립 테스트 선행 RED → 구현 GREEN.
- i18n 키는 en 기준 작성 후 ko/ja/zh 번역 — 누락 locale 금지 (AC-060).

## §E Self-Verification

- E1: acceptance.md §C AC Matrix 21건 PASS/FAIL 표 (명령 verbatim 출력 첨부, 파일-redirect 계약 준수).
...
- **M2 — learning 정직화**: REQ-WC14-001/002/003. auto_apply 편집 철거→read-only 표시(+설명 i18n), tier_thresholds/rate_limit raw view. M1 denylist에 learning.auto_apply 항목이 이미 있으므로 철거 즉시 가드 정합.
- **M3 — merge_method**: REQ-WC14-010/011. 3필드 select(정렬된 파생 옵션) + typed 라운드트립/거부 테스트 (TDD).
- **M4 — raw views (security + mx)**: REQ-WC14-020/030. RawViewBlocks 4건 + mx 렌더 그룹(B4 판정 반영) + TestMXRawViewRendered.
