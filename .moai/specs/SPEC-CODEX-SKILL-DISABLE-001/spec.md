...
- **REQ-CSD-013** (Event-driven) — **When** 발행이 완료되면, 지목된 스킬은 codex의 스킬 노출에서 **실제로 사라져야** 한다. 엔트리가 문법적으로 올바르게 쓰였다는 것만으로는 이 조항을 만족하지 않는다 — 게이트가 realpath 비교이므로, 올바른 모양의 엔트리도 다른 실파일을 가리키면 아무것도 하지 않는다.
