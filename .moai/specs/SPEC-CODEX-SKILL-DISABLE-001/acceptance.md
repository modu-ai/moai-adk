...
> **[예산 초과 — 리드 판정 대기] 현재 17개, Tier M 상한 16개.** `grep -o 'AC-CSD-[0-9]*' … | sort -u | wc -l` → **17**. 기준을 넓혀 수를 맞추지 않았다: 그 수법이 감사 D-N1이 지적한 손상(수는 줄고 한 기준이 서명해야 할 표면은 늘어남) 그 자체이므로, 리드 지시대로 줄이는 대신 보고한다. 초과분의 후보와 판단은 리드 보고에 적었다.
