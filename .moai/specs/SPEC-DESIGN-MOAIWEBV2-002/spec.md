...
- **REQ-MWA-002** (Ubiquitous): The console UI shall not render a theme-toggle control: the `themeToggle` button is removed from `board.templ` and `root.templ`, the `icon-sun`/`icon-moon` SVGs are removed from `icons.templ`, and the `theme.aria` i18n key is removed from all 4 locales in `i18n.js`.
- **REQ-MWA-003** (Event-driven): **When** a console page loads, the client runtime (`app.js` and the `<head>` FOUC snippet rendered by `root.templ`) shall neither read nor write any theme persistence key (`moai-console-theme`) nor set a `data-theme` attribute on the document element.
...
- **REQ-MWA-006** (Event-detected): **When** a status-token TEXT usage in the console fails WCAG AA contrast (≥ 4.5:1 normal text) after the swap — pre-measured: `.banner--success` text at 2.45:1 — the usage site shall darken via `color-mix(in srgb, var(--color-<status>), var(--color-ink) <N>%)` (or equivalent usage-scoped darkening) until AA passes; the token value itself shall not change.
...
- **REQ-MWA-011** (Event-detected): **When** license verification finds that Goorm Sans Code does not permit embedding/subset redistribution, the run shall halt Group 3, keep the current OS fallback mono stack, and return a structured blocker report to the orchestrator (no unverified license claim is committed).
...
| AC-MWA-003 | REQ-MWA-003 | `moai-console-theme` + `data-theme` absent from `app.js` / `root.templ` |
| AC-MWA-004 | REQ-MWA-004 | dark-absence test inversion green |
...
| AC-MWA-007 | REQ-MWA-007/014 | `git diff --exit-code -- docs-site/static/` clean; hugo build warning-free |
...
| AC-MWA-013 | REQ-MWA-013 | Go-side diff restricted to `*_templ.go` / `*_test.go` / `.templ` / `assets/` |
