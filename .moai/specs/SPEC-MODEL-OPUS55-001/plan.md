...
| 9 | agent-authoring.md:219, prompting-best-practices.md:35 | "`xhigh` for coding/agentic, minimum `high` for intelligence-sensitive" | KEEP | Per-agent / per-prompt escalation guidance, not a default claim; consistent with a `medium` session default | — (P6 does not match) |
