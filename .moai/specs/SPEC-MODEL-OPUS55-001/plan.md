...
- **K4 — `max` in settings (changed at run, commit `eb629efb5`; amendment 0.1.2).** The model-config page says "`max` isn't accepted as a level in either key" (settings `effortLevel` / `CLAUDE_CODE_EFFORT_LEVEL`). Shipped behavior: a resolved `max` is never written to the injected `--settings` `effortLevel`; it is passed as the session-scoped `--effort max` launch argument on the general path (`crosssession_settings.go`) and the kanban path (`kanban_settings.go`); `low`/`medium`/`high`/`xhigh` stay on settings; `CLAUDE_CODE_EFFORT_LEVEL` is untouched; an operator-supplied `--effort` wins. Required precedence (REQ-OP55-007 (b)): an operator `--effort` anywhere in argv — before or after `--`, space or `=` form — suppresses injection. At `0512e6e5f` the after-`--` case is not yet honored (sync-audit F1, fix scheduled in run); AC-OP55-007b is RED until it lands.
