...
**REQ-KR-012** — The `AC-FM-*` acceptance-criterion identifiers appearing in test comments shall **not** be renamed. They are citations to the acceptance criteria of the closed `SPEC-FACTORY-MODE-001`; rewriting them would break traceability to a record this SPEC does not amend.
...
**REQ-KR-020** — **When** template source has been edited, the implementer shall run `make build` and shall commit the regenerated `internal/template/catalog.yaml`. Renaming a file inside `.claude/skills/moai/` changes that skill directory's content hash, which the catalog records; an uncommitted catalog leaves the committed tree stale and breaks the parity check in CI.
...
Two residues are invisible to this pattern by construction, and each carries its own criterion:
