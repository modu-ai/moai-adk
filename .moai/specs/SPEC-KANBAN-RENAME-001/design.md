...
**Residual risk — a weakened assertion at constant line count is not mechanically detectable, and this is where the preservation claim is thinnest.** Recorded at v0.4.0 because the v0.3.0 rationale for `AC-KR-012`'s count-invariance check named this hazard and then did not answer it. Two measurements bound the exposure. The criterion's first command carries a trailing `grep -viE 'kanban|factory'`, and of the **226** `t.Error`/`t.Fatal` lines across the six surface test files, **22 — 9.7%** — carry one of those tokens and are discarded; the v0.3.0 text claimed the filter discarded *every* assertion line, which is false and would have justified a broader remedy than the situation needs. The second and third commands compare `+` and `-` counts, which is filter-independent and catches an outright deletion or addition anywhere in the 226. Neither reaches a **weakened predicate at constant line count** — an assertion whose comparison is loosened in place contributes one `+` and one `-` exactly as a rename does, leaving the counts equal. `AC-KR-011` cannot reach it either: a weakened assertion leaves the suite green by construction, which is the whole reason it is dangerous. `REQ-KR-013` traces only to those two criteria, so the intersection — a loosened predicate on one of the 22 filtered lines — is uncovered by command and closed by human review of the 22, or not at all. The exposure is accepted rather than engineered away: the mechanical alternatives are a semantic diff of assertion predicates, which does not exist here, or a hand-maintained inventory of all 226 lines, which is a second corpus to keep correct and would drift from the first.
...
**A fourth blind spot, measured and closed at v0.4.0: test function *names*.** The pattern is scoped to production identifiers, path fragments, and mode prose, and a test function name carries none of them — `TestCC_FactoryWritesStateRecord` matches no `$TOK` alternative, because `internal/factory` is a path, `parseFactoryFlag` is a production symbol, and `[Ff]actory [Mm]ode` needs a space that a CamelCase identifier does not have. So the completion grep of `AC-KR-021` reads `0` on a tree whose test names still announce the old mode. `REQ-KR-011` is what binds those names, and its two criteria reach them only through their `-run` patterns: measured at HEAD `d39e3cdc6`, **sixteen** test functions carry a `Factory` token (`cc_test.go` 9, `glm_test.go` 1, `cg_test.go` 2, `launcher_blockcap_infinite_test.go` 3, `factory/record_test.go` 1, `factory/revision_test.go` 0) and the patterns of `AC-KR-001` and `AC-KR-005` select **seven**. Nine were watched by nothing at all — a wider gap than the third blind spot above, which cost two lines of one file.

**Rejected — extending `$TOK` with a `Test.*Factory` alternative.** It would work, and it would put a test-name concern inside the pattern `spec.md` §D.1 declares the sole definition of the completion surface — a pattern already copied byte-identically into `AC-KR-021` and drift-checked by `AC-KR-027`. Widening it makes every future change to a test-naming convention a change to the completion criterion. `AC-KR-001` gains a bare-word grep bounded to the six surface test files instead (baseline **16**, target **0**), which is the same instrument choice the third blind spot made and rests on the same ground: the tree-wide false-positive population that forces `$TOK` to be token-scoped does not exist across six named files, where all sixteen baseline matches are Kanban Mode names. `plan.md` M1 step 6 enumerates the nine invisible ones, because a criterion that fires at M4 is a late signal for work done at M1.
