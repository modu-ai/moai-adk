...
### AC-CONTRACT-001 — location, version, spec_id, card

**Given** a fixture SPEC directory `SPEC-FIXTURE-001` whose `contract.yaml` has `schema_version: 1`, `spec_id: SPEC-FIXTURE-001`, and `card: t1234`,
**When** the contract is loaded by SPEC ID,
**Then** it decodes without error and `show --json` reports `"card": "t1234"`; **and given** the same file with `spec_id: SPEC-OTHER-001`, verify reports `spec_id_mismatch`; **and given** `schema_version: 2`, verify reports `schema_invalid`; **and given** each of: no `card` key, `card: ""`, `card: ../x`, `card: a/b`, `card: -x`, and a 65-character card, **then** verify reports `card_invalid`; **and given** `card: FEAT_7-b` (a non-numeric scheme), **then** no `card_invalid` is reported.
...
**Then** all four digests are identical 64-character lowercase hex strings; **and when** the digest is computed for C with `budget.turns` changed from 60 to 61, **then** it differs; **and when** it is computed for C with `card` changed from `t1234` to `t1235`, **then** it differs; **and when** the `signature` block is added or changed, **then** the digest is unchanged.
...
**Then** exit 1 and `reasons` contains `contract_digest_mismatch`; **and when** `acceptance.sha256` in the contract is edited to match a tampered `acceptance.md`, **then** `reasons` contains `contract_digest_mismatch`; **and when** only `card` is edited (`t1234` → `t1235`), **then** `reasons` contains `contract_digest_mismatch`; **and when** `card` is edited and `signature.contract_sha256` is also set to the new digest without recomputing the seal, **then** `reasons` contains `signature_seal_mismatch` (the card → digest → seal chain of `design.md` § Card Field).
**And given** the untampered human-signed fixture and the untampered `llm` receipt-signed fixture from AC-CONTRACT-015, **when** verify runs, **then** each is `signed-valid` with `reasons` `[]`. **And when** exactly one signature field is edited **without** recomputing the seal — each of `signed_at`, `head_sha`, `operator.email`, `batch_id`, `supersedes`, `acceptance_sha256`, `method`, `signer_kind`, and, on the receipt fixture, `receipt.provenance` (`file` → `store`) and `receipt.path` — **then** each verify exits 1 with `reasons` containing `signature_seal_mismatch`. **And when** the edit is made **with** the seal recomputed: (a) on the receipt fixture, only `method` changed to `interactive-tty` — **then** `reasons` contains `signature_inconsistent`; (b) on the receipt fixture, `receipt.provenance: store` (any value other than `file`) — **then** `reasons` contains `signature_inconsistent`; (c) on the human fixture, `signer_kind: llm` — **then** `reasons` contains `signature_inconsistent`; (d) on the human fixture, `signature.acceptance_sha256` set to a different 64-hex value — **then** `reasons` contains `signature_acceptance_mismatch`; (e) residual-risk control — on the receipt fixture, `method: interactive-tty`, `signer_kind: human`, the `receipt` block deleted, and the seal recomputed — **then** verify reports `signed-valid`: a complete keyless re-seal is not detectable by verify, as spec.md §C.6 and §H state.
...
**And given** config `mode: contract` and a receipt at the fixed path whose inputs equal `show --json`'s `signable_contract_sha256`, the acceptance hash, and the plan-audit report hash, **when** `sign --signer <effective_decider> --receipt <that path>` runs for each of: (r1) `kickoff.decider: llm`, `requested_decider: llm`, `effective_decider: llm`, `llm_answer` `approve`, `outcome: approve`; (r2) no `kickoff.decider` key (derived `llm`) and the same receipt as r1; (r3) `kickoff.decider: llm+jev`, `workflow.jev.enabled: false`, `requested_decider: llm+jev`, `effective_decider: llm`, `fallback: {applied: true, reason: jev_disabled}`, no `jev_answer`, `llm_answer` `approve`, `outcome: approve`,
**Then** each exits 0 without calling the confirmation reader, the signature has `method: receipt`, `receipt.sha256` equal to the receipt file's SHA-256, `receipt.provenance: file`, and `signer_kind: llm`, each output carries the REQ-CONTRACT-019 notice, and verify exits 0; **and when** one byte of the r1 receipt file is then changed, **then** verify exits 1 with `reasons` containing `receipt_mismatch`. (r3 is the recorded fallback: no Jev answer is an input, so the interim A1 rule of spec.md §C.8 does not apply; the signature activates nothing, spec.md §C.6.)
...
**And given** config `mode: contract`, `workflow.jev.enabled: true`, and — unless stated otherwise — `kickoff.decider: llm` with an otherwise valid `llm` receipt (`requested_decider` and `effective_decider` `llm`, `llm_answer` `approve`, `outcome: approve`), receipt-path invocations whose receipts have: (h) an `llm_answer` without `reason_refs`; (i) with `kickoff.decider: llm+jev`, `requested_decider: llm+jev`, `effective_decider: llm`, and `fallback.applied: false` (a silent fallback); (j) an `inputs.acceptance_sha256` that differs from the file; (k) with `kickoff.decider: llm+jev`, `effective_decider: llm+jev` and no `jev_answer`; (l) with `kickoff.decider: llm+jev`, a fallback to `llm` whose `fallback.reason` is `jev_timeout` (not in the closed set); (m) `outcome: approve` with `llm_answer` `reject` (under `kickoff.decider: llm+jev`, with `jev_answer` `approve` at confidence 0.71); (n) `outcome: human`; (o) a receipt path outside `.moai/specs/SPEC-FIXTURE-001/kickoff-receipt.json`; (p) `outcome: reject` with `llm_answer` `reject`; (q) with `kickoff.decider: llm`, `requested_decider: llm+jev`, `effective_decider: llm`, and `fallback: {applied: true, reason: jev_call_failed}` (a fallback the configuration did not request); (r) configured `kickoff.decider: jev`; (s) `effective_decider: llm` with a `jev_answer` present; and (t) with `kickoff.decider: llm+jev`, `requested_decider` and `effective_decider` `llm+jev`, `llm_answer` and `jev_answer` both `approve` (jev confidence 0.71), and `outcome: approve`,
**When** sign runs for each,
**Then** they refuse with, respectively, `receipt_invalid`, `receipt_signer_mismatch`, `receipt_input_mismatch`, `receipt_invalid`, `receipt_invalid`, `receipt_invalid`, `receipt_requires_human`, `receipt_invalid`, `receipt_rejected`, `receipt_signer_mismatch`, `kickoff_decider_jev_sole`, `receipt_invalid`, and `receipt_requires_human`. Cases (h)–(s) are structural or consistency refusals from `design.md` § Kickoff Receipt field rules (or the configuration check of REQ-CONTRACT-015 for (r)); none depends on the A3-owned cross-check rules. Case (t) tests the A1 state of the interim rule (spec.md §C.8, REQ-CONTRACT-024) and is binary on A1's own tree: while A3's principle amendment has not landed, a receipt in which Jev actually answered is refused even when both approve. A3 owns the replacing test: when A3's amendment lands, A3 removes this refusal and replaces case (t) with a test of its cross-check result. Case (m) is the "a Jev approval never suffices alone" check (field rule 9).
...
**Then** exit 0 and the output parses as one JSON object containing the keys `spec_id`, `schema_version`, `state`, `valid`, `reasons`, `contract_sha256`, `recorded_contract_sha256`, `signable_contract_sha256`, `acceptance`, `actions`, `card`, `push_requires_lease`, `terminal`, `effective_never`, `scratch`, `frozen_files`, `second_review`, `mode`, `budget`, `signature`; `effective_never` contains `internal/x/**`, `.moai/specs/SPEC-FIXTURE-001/contract.yaml`, and `.moai/specs/SPEC-FIXTURE-001/acceptance.md`; `scratch` equals `[".moai/state/verify/**"]`; `frozen_files` equals, as a set, `["**/CLAUDE.md", "**/CLAUDE.local.md", ".claude/rules/moai/core/moai-constitution.md", "internal/x/**"]` (every element a glob; the bare strings `CLAUDE.md` and `CLAUDE.local.md` are absent); `terminal` is `false`; **and given** the unsigned draft, `state` is `unsigned` and `effective_never` omits the two SPEC files; **and given** the fixture's `spec.md` frontmatter `status: completed`, and separately `status: "archived"`, **then** `terminal` is `true`, and with `status: draft` it is `false`; **and** plain `show` (no `--json`) prints each schema section heading.

### AC-CONTRACT-019 — configuration defaults and fail-safe

**Given** a `workflow.yaml` with no `autonomy` section,
**When** the configuration is loaded,
**Then** mode is `guided`, `batch_sign` false, `second_review` `required`, `push_develop` false, budget `60/40/2`, the effective `kickoff.decider` `human`, and `kickoff.jev_min_confidence` `0.50`; **and given** `mode: contract` with no `decider` key, **then** the effective decider is `llm`; **and given** `mode: contract` with `decider: human`, **then** it is `human` (an explicit value wins); **and given** each of `mode: yolo`, `second_review: sometimes`, `decider: oracle`, `jev_min_confidence: 1.5`, **then** the value falls back to `guided`, `required`, `human`, `0.50` respectively and a warning naming that key is emitted; **and given** `mode: contract`, `second_review: advisory`, `decider: llm+jev`, `jev_min_confidence: 0.6` with `workflow.jev.enabled: false`, **then** those values are returned and no configuration error is reported (the Jev-side fallback is A3's decision, recorded in the receipt, AC-CONTRACT-015 r3); **and given** `decider: jev`, **then** loading succeeds, the reader reports a configuration error naming `workflow.autonomy.kickoff.decider` and stating that Jev is never a sole decider, the returned decider is not `human` (no fallback), and a human-path `sign` still succeeds while the receipt path refuses with `kickoff_decider_jev_sole` (AC-CONTRACT-016 r).
...
**When** it is decoded,
**Then** `workflow.autonomy.mode` is `guided`, `workflow.autonomy.contract.second_review` is `required`, the decoded `workflow.autonomy.kickoff` section has no `decider` key (and `grep -nE '^ *decider:' internal/template/templates/.moai/config/sections/workflow.yaml` prints no line), and the configuration reader's effective decider for the template as shipped (`mode: guided`) is `human`.
