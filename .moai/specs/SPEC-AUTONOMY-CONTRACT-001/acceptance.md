...
**And given** a second unsigned draft, config `mode: contract`, `kickoff.decider: llm`, and a receipt at the fixed path whose inputs equal `show --json`'s `signable_contract_sha256`, the acceptance hash, and the plan-audit report hash, with `requested_decider: llm`, `signer: llm`, and one `llm` decision `approve` (in A1 only an `llm` signer can be accepted; a deciding Jev answer routes to a human, spec.md §C.8), **when** `sign --signer llm --receipt <that path>` runs, **then** exit 0, the confirmation reader is never called, the signature has `signer_kind: llm`, `method: receipt`, `receipt.sha256` equal to the receipt file's SHA-256, and `receipt.provenance: file`, and verify exits 0; **and when** one byte of the receipt file is then changed, **then** verify exits 1 with `reasons` containing `receipt_mismatch`.
**And given** a third unsigned draft, config `mode: contract`, `kickoff.decider: jev`, `workflow.jev.enabled: false`, and a receipt with `requested_decider: jev`, `signer: llm`, `fallback_reason: jev_disabled`, and one `llm` decision `approve` (the recorded Jev-to-LLM fallback), **when** `sign --signer llm --receipt <that path>` runs, **then** exit 0 and the signature has `signer_kind: llm` and `method: receipt`. **And given** a fourth unsigned draft, config `mode: contract` with no `kickoff.decider` key, and a receipt with `requested_decider: llm`, `signer: llm`, and one `llm` decision `approve`, **when** the same command runs, **then** exit 0 (the derived decider under `contract` is `llm`).
...
**And given** config `mode: contract`, `workflow.jev.enabled: true`, `jev_min_confidence: 0.50`, `kickoff.decider: llm` unless stated otherwise, and receipt-path invocations whose receipts have: (h) a missing `reason_refs`; (i) with `kickoff.decider: jev`, `requested_decider: jev`, `signer: llm`, and no `fallback_reason` (a silent fallback); (j) an `inputs.acceptance_sha256` that differs from the file; (k) with `kickoff.decider: jev`, `requested_decider: jev`, `signer: jev`, and one `jev` decision `approve` at confidence 0.71; (l) with `kickoff.decider: jev`, a fallback to `llm` with `fallback_reason: jev_low_confidence` whose `jev_attempt.confidence` is 0.71 (not below the minimum); (m) with `kickoff.decider: jev`, a fallback to `llm` with `fallback_reason: jev_disabled` while `workflow.jev.enabled` is true; (n) one `llm` decision `escalate`; (o) a receipt path outside `.moai/specs/SPEC-FIXTURE-001/kickoff-receipt.json`; (p) one `llm` decision `reject`; and (q) with `kickoff.decider: llm`, `requested_decider: jev`, `signer: llm`, and `fallback_reason: jev_call_failed` (a fallback the configuration did not request),
**When** sign runs for each,
**Then** they refuse with, respectively, `receipt_invalid`, `receipt_signer_mismatch`, `receipt_input_mismatch`, `receipt_requires_human`, `receipt_invalid`, `receipt_invalid`, `receipt_requires_human`, `receipt_invalid`, `receipt_rejected`, and `receipt_signer_mismatch`. Case (k) routes to a human by rule 0 (a deciding Jev answer, `design.md` § Agreement Rule); (l) and (m) fail the fallback evidence checks in `design.md` § Kickoff Receipt; (i) and (q) show that a fallback is accepted only when recorded and only from a configured `jev` decider.
