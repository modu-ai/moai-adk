...
## §F. Resolved Questions (re-audit RESOLVED — folded from file evidence)

- **OQ-1** (RESOLVED): The orchestrator trust-but-verify batch has NO structural "about to re-run command X" preamble today. `agent-common-protocol.md` § Parallel Execution binds the batch as an orchestrator-composed single-turn multi-Bash call, and `agent-common-protocol-reference.md` owns the verbatim 7-command list; re-execution is implicit Bash, with NO literal preamble token to intercept. **Decision:** A9 attaches as a **doctrinal switch the orchestrator applies when composing the batch** — consult `moai verify check --key-current` first; on all-three attribution match (snapshot key + recorded command + recorded output) consume the §E evidence; on any mismatch fall back to re-execution (REQ-SPD-009). A9 is NOT a mechanical hook intercepting a preamble; it is a composition-time doctrinal switch.
- **OQ-2** (RESOLVED): The Tier-aware ceiling lives in `harness.yaml`. `harness.yaml` already owns the per-level `plan_audit.max_iterations` map; the per-Tier `max_iterations` map sits alongside the per-level one in the same file (single config authority). `plan-auditor.md` § Retry Loop Contract's hard-coded `3` is a **consumer-side reference value, NOT the SSOT** — the contract consults the Tier-resolved value from `harness.yaml` at spawn time rather than reading its own literal. **Decision:** harness.yaml = SSOT for the per-Tier ceiling; plan-auditor.md = consumer that resolves the Tier value and compares against it.
