...
**Current phase**: run-phase (M6 complete — all 14 ACs PASS)
**Tier**: M
...
| M6 | Quality Verification + Cross-Platform Build + Coverage | completed | 2026-05-23 | 2026-05-23 | 10-command parallel verification batch executed in single turn after rebase onto origin/main (which inherited SPEC-V3R6-CI-BASELINE-DRIFT-001 lint 27→0 fix). All 10 commands PASS: (1) `go test ./internal/tmux/... ./internal/cli/worktree/...` exit 0, (2) coverage tmux 81.4% + worktree 83.7%, per-file new files: handoff_guidance.go 87.5/100%, swarm_registry.go 100/100/90%, team_launch.go 100/100%, team_launch_posix.go 90.9 (launchP3) /100 (launchP1P2)% / 0% (defaultTmuxNewWindow, real-tmux-only adapter), cg_detect.go 94.1/100% — all ≥87.5% except real-tmux-only adapters (matches M2/M3 syscall.Exec uncoverable pattern), (3) C-HRA-008 grep `internal/cli/worktree/ internal/tmux/` returns 0 non-comment matches, (4) `go vet ./...` clean, (5) `golangci-lint run --timeout=2m` 0 issues (1 errcheck on cg_detect.go:65 self-introduced fixed via `_, _ = fmt.Fprintln(...)`), (6-8) cross-platform `GOOS={darwin,linux,windows} GOARCH=amd64 go build ./...` all exit 0, (9-10) template mirror diffs (SKILL.md + worktree-integration.md) both exit 0 (byte-identical). |
...
| AC-WTL-011 | Lint baseline: 0 NEW violations | `golangci-lint run --timeout=2m` (diff vs baseline) | **PASS (M6 2026-05-23)**: After rebase onto origin/main (which absorbed SPEC-V3R6-CI-BASELINE-DRIFT-001 lint 27→0 fix) and self-introduced errcheck fix on `internal/tmux/cg_detect.go:65` (Fprintln return discarded via `_, _ =` assignment), `golangci-lint run --timeout=2m` reports **0 issues**. Net delta vs origin/main baseline (post-DRIFT-001): 0 NEW lint findings introduced by this SPEC. |
| AC-WTL-012 | Test coverage ≥85% for new files | `go tool cover -func=cover.out` per-file threshold | **PASS (M6 2026-05-23)**: `go test -coverprofile=/tmp/m6-cover.out ./internal/tmux/... ./internal/cli/worktree/...` produces: tmux package 81.4%, worktree package 83.7%. Per-new-file (functions ≥85% threshold): cg_detect.go IsCGMode 94.1% + hasGLMEnv 100%, handoff_guidance.go printHandoff 87.5% + printHandoffWithError 100%, swarm_registry.go swarmRegistryDir 100% + WriteSwarmEntry 90% + patternToMode 100%, team_launch.go String 100% + decidePattern 100%, team_launch_posix.go launchP3 90.9% + launchP1P2 100%. Uncovered: `defaultTmuxNewWindow` 0% (real-tmux-only OS adapter, mocked via `tmuxNewWindowFn` injection in tests — same pattern as M2 syscall.Exec uncoverable line, accepted per R3 mitigation). All test-coverable functions exceed the 85% threshold. |
| AC-WTL-013 | Skill body + template mirror byte-identical | `diff -r .claude/skills/moai-workflow-worktree/ internal/template/templates/.claude/skills/moai-workflow-worktree/` | **PASS (M5 + re-verified M6 2026-05-23)**: `diff .claude/skills/moai-workflow-worktree/SKILL.md internal/template/templates/.claude/skills/moai-workflow-worktree/SKILL.md` → exit 0 (zero output). `diff .claude/rules/moai/workflow/worktree-integration.md internal/template/templates/.claude/rules/moai/workflow/worktree-integration.md` → exit 0 (zero output). Both pairs byte-identical via `cp` mirror operation. Pre-M5 baseline already had zero drift; M5 preserved invariant; M6 re-verified post-rebase. REQ-WTL-011 (Template-First mirror discipline) satisfied. |
| AC-WTL-014 | BODP audit trail preserved | grep `writeWorktreeAuditTrail` + dispatchTeamLaunch ordering inspection | **PASS (M6 2026-05-23)**: `internal/cli/worktree/new.go` line 174 (`writeWorktreeAuditTrail(repoRoot, specID, branchName, effectiveBase)`) is invoked BEFORE line 217 (`dispatchTeamLaunch(cmd, repoRoot, specID, branchName, wtPath)`). The `--team` flag adds a downstream dispatch but does NOT skip or bypass the BODP audit trail write. Verified by `grep -n "writeWorktreeAuditTrail\|dispatchTeamLaunch" internal/cli/worktree/new.go`. Existing BODP tests (pre-existing in `new_test.go`) continue to PASS post-M6, confirming the `.moai/branches/decisions/<branch>.md` artifact is generated for every worktree creation including `--team` invocations. |
