...
amendment_of: SPEC-AGENT-PARALLEL-OPT-001
---
...
| 0.13.0 | 2026-07-25 | manager-spec | **AC-APO-072 / 072b RED/GREEN 왕복 직접 관측 — PASS-WITH-DEBT 2건 해소**(in-place amendment, `amendment_of` 자기참조). close 시점의 두 MUST AC는 첫 항(정적 존재 확인)만 기계 검증되고 왕복 절반이 **"템플릿 트리 변형이 필요해 read-only 감사 범위 밖"** 이라는 사유로 유예돼 있었다. 본 세션이 격리 worktree(`c7309aeb6` 기준, dirty 0)에서 5-stage 왕복을 **실행**해 그 사유를 무효화했다 — STAGE 0 GREEN baseline(`ok  github.com/modu-ai/moai-adk/internal/template`) → STAGE 1 `redgreen-probe.js`(C1 토큰 `SPEC-V3R6-REDGREEN-PROBE`)를 `internal/template/templates/.claude/workflows/`에 심고 `TestTemplateNoInternalContentLeak` **FAIL**(`class=C1-spec-id-prefix`) → STAGE 2 probe 제거 후 GREEN → STAGE 3 `hns-release-update-run.js` 심고 `TestSplitHarnessNamespaceNoLeak` **FAIL**(sentinel `SPLIT_HARNESS_NAMESPACE_LEAK`) → STAGE 4 probe 제거 후 GREEN → STAGE 5 worktree 청결 0. 두 probe는 매처를 **먼저 판독한 뒤** 저작해 RED가 공허하지 않음을 보장했다(`internal_content_leak_test.go`의 C1 클래스 패턴 `\bSPEC-(V3R[2-6]\|AGENCY\|WORKTREE)-[A-Z0-9-]+\b` — always-on, strict 티어 아님 — 및 `leakTextExtensions`의 `.js` 등재, `split_namespace_test.go`의 `splitHarnessAgentPrefixes`). 목표 매트릭스 **54 PASS / 0 FAIL / 2 PASS-WITH-DEBT → 56 PASS / 0 FAIL / 0 PASS-WITH-DEBT**. `progress.md` §E.4 매트릭스 갱신과 재-close(`in-progress → completed`)는 manager-docs 소관이라 본 편집 범위 밖이다. `acceptance.md`도 0.12.0 동반 상향. |
