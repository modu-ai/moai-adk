...
**REQ-PFW-005 (Ubiquitous):** The gate-off child invocation shall use the CI invocation shape — the package glob `./internal/hook/perf/...` without `-short` — rather than a hand-narrowed `-run` selector, because that is the shape under which CI rewrites the fixtures today (§A.3); and the guard shall build that child's environment **explicitly**, carrying the sentinel and nothing else, rather than inheriting the parent's, so that a `MOAI_HOOK_PERF_UPDATE` value set in the parent cannot reach the negative leg and turn the guard red for a reason that is not a regression.

**REQ-PFW-006 (Ubiquitous):** The guard shall restore the captured original bytes of both fixtures on **every** exit path — including the path on which its own assertion fails — so that a RED guard leaves the two tracked files byte-identical and never becomes a fresh instance of the defect it exists to detect; and the guard shall not be `t.Parallel()`, because that restore contract assumes its cleanup fires before any later test in the package begins.
