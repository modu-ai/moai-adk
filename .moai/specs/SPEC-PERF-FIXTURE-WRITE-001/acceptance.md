...
(d) 는 그 자리를 **도달 가능한** 격리 요구로 채운다. 부모가 `MOAI_HOOK_PERF_UPDATE=1` 을 세운 채 가드가 도는 경로는 (c) 가 막지 않으며, 이 패키지의 기존 관용구(`harness_test.go:243` 의 `cmd.Env = append(os.Environ(), …)`)를 그대로 베끼면 게이트가 **음성** 자식으로 새어 가드가 회귀가 아닌 이유로 붉어진다. **막아야 할 것은 상속 자체가 아니라 뺄셈 없는 상속이다** — REQ-PFW-005 는 부모 환경을 물려받되 지목된 변수를 **빼도록** 요구하며(허용목록 열거는 금지), 이 행은 그 결과만 본다: 그 상황에서 가드가 **통과하는지**. 게이트를 빼지 않은 구현은 실패한다.
...
**When** 가드가 부모 환경을 물려받되 지목된 변수(`MOAI_HOOK_PERF_UPDATE`·`MOAI_HOOK_PERF_SKIP`·`GOFLAGS`)를 **빼고** 센티널을 붙여(양성 다리에만 게이트를 더한다) 자식을 띄운다 —
