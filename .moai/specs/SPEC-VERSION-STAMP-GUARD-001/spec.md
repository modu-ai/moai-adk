...
| 0.5.0 | 2026-09-01 | manager-spec | plan-audit iter-3 **PASS-WITH-DEBT 0.90**(`.moai/reports/t388/plan-audit-iter3.md`)의 부채 정리. 문안 수정만이며 요구·수락은 6/6 불변, `spec.md` 본문은 이 HISTORY 행과 `version` 외에 바뀌지 않았다. **F1**(minor/blocking, `plan.md` M2.1): 두 조항이 같은 트리에 서로 다른 항목 수를 함의하던 것을 닫았다 — 유령은 M2.1에서 `**Version Stamps:**` 아래에 그대로 두므로 항목이 **8건**이고, 그것이 `parsed=8` 핀의 출처임을 [HARD]로 적었다. **F2**(minor/blocking, `plan.md` §B): D8의 인용 정정이 `spec.md`에만 반영돼 있던 것을 `항목 71-74·77-78행`으로 맞췄다(재측정 — 75행 공백, 76행 라벨). **F4**(minor/optional, `acceptance.md` AC-VSG-006): 네 갈래로 흩어져 있던 grep 범위를 「Files Requiring Version Sync」 절 전체로 한 번만 고정하고 판정 명령을 실행 가능한 형태로 교체했다. **F5**(minor/optional, `plan.md` §D): 리터럴을 못박는 설계에서 유일하게 안 박혀 있던 앵커 문자열을 **앵커 리터럴 계약**으로 신설했다(`**Version Stamps:**` / `**Release Artifacts:**`). **F6**(minor/optional, `acceptance.md` 문서 핀): 「측정 시점 `origin/develop`과 동일」이라는 브랜치 등가 주장을 제거하고 측정 트리 SHA 단일 핀으로 바꿨다 — 브랜치는 움직이고 등가는 조용히 썩는다. 유효성 확인을 대상 경로 diff로 교체했다. **F3**은 plan-phase 산출 커밋(`c6aed3c36`)으로 전반부가, `progress.md` §E.1의 완료 신호 기록으로 후반부가 닫혔다. **F7**은 잔여 기록이라 수리 없음. 재측정 1건: `origin/develop`이 `2c18091d1`로 더 나아갔으나 측정 대상 8경로는 `9328a5242`와 빈 diff이고, `CHANGELOG.md`만 29줄 늘었다(존재만 판정하므로 무영향) |
