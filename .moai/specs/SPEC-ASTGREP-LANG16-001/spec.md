...
| 2026-08-25 | 0.7.0 | **Plan-audit iterations 4 (FAIL 0.84) and 5 (PASS 0.88) — three mechanical closures.** **N1**: the standing PRESERVE proof and AC-A16-008 baselined against `origin/main`, a ref that had moved ten commits past this tree, so the command reported `18 files changed, 23 insertions(+), 2825 deletions(-)` before any work existed — false on arrival and worse with every upstream commit. Re-pinned to `294b4b6ab` at all five assertion sites, where the same command returns empty, with a note at each site recording why a branch name cannot baseline an invariant and instructing that a rebase re-measure §H and re-pin. Provenance mentions of `origin/main` (§A.6, `research.md`, `progress.md`) are kept deliberately: their subject *is* the mainline, so pinning them would weaken the claim. **N2**: §H row 019's guard list now names the `catalog.yaml`-unchanged clause, with the rule that a guard is green now and must stay green, so no guard is what makes the row red. **N3**: `design.md`'s matrix example carried a bare `rule-tests/` path contradicting the relocation; it now reads `$R`. No requirement or criterion text changed; counts stay 23 REQ / 19 AC. |
