...
- 2026-09-26 (sync-phase note, card t1201) — **REQ-CX2-008 is narrowed by SPEC-CODEX-RESUME-SCOPE-001**: `resume_last` thread selection is now scoped by the optional `thread_id` / `work_key` inputs, and an unscoped `resume_last` over a registry recording more than one distinct thread is refused with `resume_ambiguous`. This SPEC's requirement text is left unchanged; the narrowing is owned by SPEC-CODEX-RESUME-SCOPE-001.
- 2026-08-11 (run-phase, targeted amendment, v0.6.0) — **Closes one gap the live protocol probe found, and nothing else.** The probe (`progress.md` §E.2 § Live protocol verification, HEAD `4a059f8b1`, codex-cli 0.146.1) executed the production session path and found that `item/fileChange/requestApproval` is a server→client **request** — it carries an `id` and expects a response — while `awaitCodexTurnReview` (`internal/cli/mcp_codex.go:823`, switch at `:843`) dispatches only on `turn/started` / `item/completed` / `turn/completed` and drops every other line. Codex then holds the turn at `activeFlags:["waitingOnApproval"]`; the observed turn did not return within 120 s. A separate check (`grep -n 'context.WithTimeout\|context.WithDeadline\|context.Background' internal/cli/codex_task.go internal/cli/mcp_codex.go internal/cli/codex_job_control.go` → no matches) confirmed the `codex_task` path imposes no deadline of its own, so the stall is bounded only by whatever context the MCP host supplies — possibly none. Reachability is high rather than exotic: every `codex_task` turn without a write grant runs `readOnly`, the distributed default for `workflow.codex.task.allow_write` is `false`, and any such turn where the model decides to edit a file lands here. This is a **gap, not a defect against spec** — no existing requirement or criterion claims approval handling, which is precisely why an amendment is the right instrument. Two requirements are ADDED (**REQ-CX2-016** — answer client-bound requests, denying file-change approval on a turn that did not opt into writes; **REQ-CX2-017** — bound a `codex_task` turn with a deadline the tool itself imposes) with two matching criteria (**AC-CX2-017**, **AC-CX2-018**), and `plan.md` gains **M6** plus three anti-patterns (AP-7, AP-8, AP-9). **No existing requirement or criterion was amended**: REQ-CX2-001..015 and AC-CX2-001..016 are verified as-is and stand untouched. Denial is not merely a defensive default — under REQ-CX2-007's gate it is the semantically correct answer, since a turn running `readOnly` by policy must not acquire write access by answering an approval. Two things this amendment deliberately does NOT do: it specifies no approval-*granting* path (the write opt-in `workflow.codex.task.allow_write` remains the only grant mechanism), and it asserts no wire shape for the response payload — the probe never answered such a request, so the decision field is to be read from the generated protocol schema at implementation time (`plan.md` M6, AP-9). **Budget note**: the Tier M ceiling is 16 requirements and 16 criteria (`spec-workflow.md` § SPEC Complexity Tier); this amendment takes the SPEC to 17 and 18. The overage is recorded as accepted debt rather than resolved by a tier-up, because Tier L would demand `design.md` + `research.md` for a two-function defense — the over-formalization the tier ladder exists to prevent.
...
> Added by the v0.6.0 amendment above. Both requirements below rest on a live observation recorded in `progress.md` §E.2 § Live protocol verification, not on an inference from the schema.

**REQ-CX2-016** (Event-driven) **When** the codex server sends a **client-bound request** during a turn the session driver is reading — a line carrying both a `method` and an `id`, as `item/fileChange/requestApproval` does — the driver shall transmit a JSON-RPC response carrying that same `id` rather than dropping the line; and **where** the turn did not opt into writes under REQ-CX2-007, the response to a file-change approval request shall **deny** the change. Denial is the correct answer under REQ-CX2-007's gate rather than a defensive default: a turn that the tool put in `readOnly` because the project did not opt in must not acquire write access by answering an approval. A client-bound request whose method the driver does not recognize shall receive a JSON-RPC error response (the transport is JSON-RPC 2.0 — every observed line carries `"jsonrpc":"2.0"`, and a `-32600` rejection is already recorded in `progress.md` §E.2), so that an unrecognized request also unblocks the turn rather than stalling it. This requirement introduces **no approval-granting path**: `workflow.codex.task.allow_write` remains the sole write opt-in (REQ-CX2-007), and no per-request approval surface, interactive or configured, is added. Because the driver is shared, the review-gate caller answers such requests too; that path never opts into writes, so its answer is likewise a denial, and `runCodexReviewRPC`'s `(ReviewOutput, error)` contract (C7) is unchanged — the only behavioral difference is in the case whose current behavior is to stall.

**REQ-CX2-017** (Ubiquitous) A `codex_task` turn shall be bounded by a deadline the tool itself imposes, so that a turn whose completion never arrives — because a request went unanswered, or for any other reason — terminates rather than running for the lifetime of a caller-supplied context that may carry no deadline at all. The bound shall be defined in Go per REQ-CX2-015, as a named, test-overridable value in `internal/config/defaults.go` following the `DefaultCodexReviewGateTimeout` precedent, and shall be **distinct from** that review-gate budget so that changing one does not silently move the other. On expiry the foreground form shall return a structured result naming the timeout and the background form shall record the job in a terminal status; neither shall panic, and neither shall wait unbounded. This requirement binds the `codex_task` path only — the Stop-hook review gate keeps its existing caller-supplied 900 s bound (`codex_review_gate.go:87`) unchanged.

## §E. Acceptance Criteria

Enumerated as `AC-CX2-001..018` in `acceptance.md`, bound to milestones M1-M4, cross-cutting, and M6.
