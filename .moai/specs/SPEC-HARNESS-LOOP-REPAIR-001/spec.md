...
3. **Lesson store direction** — **RESOLVED (M5, 2026-07-28, AskUserQuestion): option (a) designate the topic-file convention as the single lesson store; `lessons.md` becomes a legacy/superseded artifact (content not migrated).** The constitution § Lessons Protocol will name the topic-file convention (`feedback_*.md` topic files + the `MEMORY.md` index) rather than `lessons.md`, satisfying REQ-HLR-006 ("resolved in favour of the practiced store") — the practiced store is the 103 `feedback_*.md` topic files, while `lessons.md` is 47 KB stale (mtime 2026-06-17, 40+ days). `moai-memory.md` already describes the topic-file convention as the discovery surface, so the constitution change aligns with existing doctrine. Options (b) migrate-then-retire and (c) restore-as-index were considered and rejected: (b) is labor-intensive for low-value 40-day-stale content; (c) creates a second index (`lessons.md` + `MEMORY.md`) contradicting the single-store principle. `lessons.md`'s 47 KB stays on disk marked `[SUPERSEDED]` (legacy preserved, not deleted).
