...
### AC-TA-004 — `relate` 는 소견만 남기고 카드를 건드리지 않는다

- **Given** `t1`, `t2` 가 queued 이고 `items` 배열 전체의 JSON 스냅샷을 떠 둔 상태에서
- **When** `moai todo relate t2 t1 --relation absorbs --note "t2 covers t1"` 를 실행하면
- **Then** exit 0, `items` 배열이 스냅샷과 바이트 동일하고, `findings` 가 `{subject_id: "t2", related_id: "t1", relation: "absorbs", source: "agent"}` 1건만큼 늘어난다.
- **붉게 만드는 구현**: `absorbs` 를 실제 흡수로 해석해 `t1` 을 drop 하거나 `t2` 텍스트에 병합하는 구현 — 독트린이 이름으로 금지한 바로 그 동작.
- **기능 부재 시**: `relate` 동사가 없어 exit ≠ 0 → 실패.
...
### AC-TA-011 — `unreviewed` 표시는 붙고, 또 걷힌다 (양방향)

- **Given** `t5`↔`t1` 기계 `near-duplicate` 소견만 있고 같은 쌍에 에이전트 소견이 없는 상태에서
- **When** ① `moai todo list` 를 실행하고, ② `moai todo relate t5 t1 --relation replaces` 후 다시 `moai todo list` 를 실행하면
- **Then** ①의 해당 소견 줄은 `unreviewed` 를 담고, ②의 같은 줄은 담지 않는다.
- **붉게 만드는 구현**: 표시가 아예 없는 구현(①에서 실패), 한 번 붙으면 절대 안 걷히는 구현(②에서 실패). 한쪽만 주장했다면 다른 쪽 구현이 통과했을 것이다.
- **기능 부재 시**: ①에서 실패.

### AC-TA-012 — 기존 파일이 그대로 읽히고, 새 파일이 옛 로더에서 그대로 읽힌다

- **Given** `findings` 키가 없는 이 기능 이전 형식의 `backlog.json` 픽스처에서
- **When** `moai todo list --json` 을 실행하고, 이어서 `moai todo add "new card"` 를 실행한 뒤 그 결과 파일을 **구 스키마 구조체**(5필드 항목 + `version` + `last_seq`)로 디코딩하면
- **Then** 첫 실행이 exit 0 이고 기존 항목 5필드가 바이트 동일하게 왕복하며 `findings` 가 빈 배열로 렌더되고, 구 스키마 디코딩이 에러 없이 성공하며 항목 수가 맞는다.
- **붉게 만드는 구현**: `findings` 부재를 에러로 취급하는 구현, 항목 스키마를 건드려 구 로더를 깨뜨리는 구현(REQ-TODO-013 위반).
- **기능 부재 시**: `findings` 가 빈 배열로도 안 나와 실패.

### AC-TA-013 — 새 동사 전부가 headless 다

- **Given** stdin 이 닫힌 비대화형 셸에서
- **When** `add`, `analyze`, `relate`, `unrelate`, `why`, `list` 를 각각 실행하고, `moai todo --help` 를 확인하고, `grep -rn "AskUserQuestion" internal/cli/` 를 돌리면
- **Then** 여섯 명령이 블로킹 없이 exit 0 또는 1 로 끝나고, `--help` 가 `analyze` / `relate` / `unrelate` / `why` 를 모두 나열하며, grep 결과가 0건이다.
- **붉게 만드는 구현**: 대화형 확인을 넣은 구현(블로킹 또는 EOF 에러), REQ-TODO-014 위반.
- **기능 부재 시**: `--help` 에 새 동사가 없어 실패 — grep 0건 단독이면 부재 시에도 통과했을 것이므로 `--help` 양성 대조가 이 AC를 지탱한다.
...
- AC-TA-001 ~ AC-TA-015 전부 PASS, 각 PASS 가 실제로 실행된 명령의 출력에 귀속될 것.
