...
sync_audit:
  ...
  tier: M
  threshold: 0.80
  must_pass_firewall: not_triggered
  blocking_findings: 0
  optional_findings: 9
  report: ".moai/reports/t166/sync-audit.md"
docs_surfaces_touched:
  - ".claude/skills/moai-domain-svg-infographic/SKILL.md (red-flag row 12 — SVG074 clause appended)"
  - ".claude/skills/moai-domain-svg-infographic/references/authoring.md (§2.5 — SVG074 coverage-note paragraph)"
  - "internal/template/templates/.claude/skills/moai-domain-svg-infographic/** (byte-identical mirror of both)"
  - "internal/template/catalog.yaml (regenerated by make build)"
  - "CHANGELOG.md ([Unreleased] — Summary paragraph + Added entry)"
readme_docs_site_changes: none_required
changelog_entry_position: "[Unreleased] → ### Added, first entry"
frontmatter_status_transitions:
  spec_md: "draft → completed (updated: 2026-08-22 → 2026-08-23)"
  plan_md: not_applicable_no_frontmatter_block
  acceptance_md: not_applicable_no_frontmatter_block
  progress_md: not_applicable_no_frontmatter_block
  in_progress_transition: skipped_at_run_phase_collapsed_into_this_close
verification:
  self_test_runner:
    command: "node .claude/skills/moai-domain-svg-infographic/scripts/test-check-svg.mjs"
    observed: "42/42 fixtures matched, rc=0"
    run: post_sync_edit
  mirror_parity:
    command: "diff -rq --exclude=.moai .claude/skills/moai-domain-svg-infographic internal/template/templates/.claude/skills/moai-domain-svg-infographic"
    observed: "no output, rc=0"
    run: post_sync_edit
  make_build:
    command: "make build"
    observed: "catalog.yaml updated successfully (12899 bytes), go build rc=0"
    run: post_sync_edit
  go_test_template:
    command: "go test ./internal/template/..."
    observed: "ok internal/template 22.062s; ok internal/template/agentemit 0.441s; rc=0"
    run: post_sync_edit
  go_vet_template:
    command: "go vet ./internal/template/..."
    observed: "rc=0"
    run: run_phase_M3_not_re_executed_at_sync
  template_neutrality:
    command: "grep -rn 'SPEC-|REQ-|CLAUDE.local' internal/template/templates/.claude/skills/moai-domain-svg-infographic/"
    observed: "0 matches"
    run: post_sync_edit
  changelog_b12_self_test:
    duplicate_grep: "grep -c 'SPEC-SVG-GEOMETRY-CHECKS-001' CHANGELOG.md → 0 before emission"
    ac_count_match: "grep -oE 'AC-([A-Z0-9]+-)*[0-9]+' acceptance.md | sort -u | wc -l → 16 (non-zero, matches the 16 named in the entry)"
    path_verification: "ls on every path named in the entry → all resolve"
```

Notes bounding the block above:

- `sync_commit_sha` is a placeholder for the same structural reason as §E.3's `run_commit_sha`: the
  block names the commit it is written into. Backfill both at the same time.
- The `draft → in-progress` transition never happened. Run-phase left `spec.md` at `draft`, so
  this close performs `draft → completed` in one step. The record says so rather than implying a
  transition that was never made.
- **Only `spec.md` carries a YAML frontmatter block.** `plan.md`, `acceptance.md`, and `progress.md`
  open directly on their `# ` heading with no frontmatter at all, as they were authored at
  plan-phase. There was therefore no `status:` or `updated:` field on those three to transition, and
  none was invented at sync — adding one would be a body change this phase does not own. The
  four-artifact close is recorded here as one artifact transitioned and three with nothing to
  transition, which is what the tree actually holds.
- The four `post_sync_edit` verifications were re-run **after** the SVG074 documentation edit and
  its mirror, not carried over from run-phase. `go vet` was not re-run at sync — the sync edit
  touches markdown only, and `go test ./internal/template/...` (which compiles the same package)
  passed after it.
- `readme_docs_site_changes: none_required` is an observation, not an assumption. `README.md`,
  `README.ko.md`, `README.ja.md`, `README.zh.md` name `moai-domain-svg-infographic` only as an item
  in two skill lists (lines 404 and 737 in each) and name no diagnostic code; the four
  `docs-site/content/{ko,en,ja,zh}/advanced/skill-guide.md` carry it as a single table row
  describing the skill in one clause. Neither surface documents diagnostics, so neither goes stale
  from this change.

### §E.4 Gaps — what this sync did NOT observe

Carried from the audit's own §Gaps (G1-G9), compressed:

- **G1** — `make build` was not re-run *by the audit* (read-only constraint). It was run in this
  sync commit: `catalog.yaml updated successfully (12899 bytes)`, and the only diff it produced is
  the one hash the skill edit accounts for.
- **G2** — the full `go test ./...` suite was not run. Local policy forbids it; the whole-package
  verdict belongs to CI.
- **G3** — **no CI verdict exists. The branch is unpushed, there is no PR and no workflow run, so
  the darwin/windows matrix and the full-package suite are still outstanding.** §D.11 item 5 ("CI is
  green on the pushed branch") is UNMET at this commit and must be read separately before merge.
- **G4** — only 3 fixtures' coordinates were recomputed by hand (`c2-mask-at-6`, `c4-attach-at-12`,
  `c4-edge-exactly-120`) plus one mutation test; the other 38 rest on the runner's exact-set
  assertion.
- **G5** — no row-by-row crosswalk between §D.9's 44 rows and the fixture set.
- **G6** — `render.mjs` (the headless-Chromium PNG path) was never executed. Untouched by this SPEC,
  unverified all the same.
- **G7** — no linter and no coverage measurement for `.mjs`; the project has no JS toolchain. The
  42-fixture set stood in for a coverage number that was never measured.
- **G8** — no dependency audit tool was run. There are zero dependencies, so absence was confirmed
  rather than scanned.
- **G9** — no falsifying case was constructed for F5; the float-boundary flip remains a hypothesis.

### §E.4 Follow-up candidates — the eight optional findings NOT fixed here

F6 (SVG074 undocumented outside the script header) is the one finding this sync closes. The other
eight are recorded so they are not lost:

- **F1 [medium]** — any nearer decorative or label-mask `<rect>` intercepts arrival binding and
  silences `SVG072`. The audit report carries a reproduction, but **no fixture pins it**, so nothing
  in the harness would catch its regression. Highest-value follow-up of the eight.
- **F2 [low]** — `SVG071` is emitted once per (mask, later-rect) pair, so one clipped label can
  report several times.
- **F3 [low]** — a single `<text>` carrying a `transform` is enough to fire `SVG074`, which reads as
  a coverage loss larger than it is.
- **F4 [low]** — the runner de-duplicates code sets, so multiplicity defects such as F2 are
  structurally invisible to the harness and always will be under exact-set comparison.
- **F5 [low]** — boundary comparisons carry no epsilon; a float-representable value at exactly the
  threshold could flip. Unfalsified (G9).
- **F7 [info]** — AC-SGC-015's command wording matches 3 pre-existing lines this SPEC never touched,
  so the tree-wide form of the grep false-positives while the changed-lines form is clean.
  **`acceptance.md`'s body is not editable at sync-phase**; this is recorded as a follow-up only.
- **F8 [low]** — connector detection keys on a literal `fill="none"` attribute, so an equivalent
  declaration by CSS or by a presentation-attribute inherited from a group is not seen.
- **F9 [low]** — diagrams drawn with `<line>`, `<polyline>`, or `<circle>` instead of `<path>` are
  transparent to every geometry check.
