...
- **Trigger RESOLVED (Q3, v0.2.1; trigger-point command names corrected v0.2.2): on-touch at `moai session register` and `moai session list`.** A periodic background tick (option a) was rejected — adds background-loop lifecycle complexity this SPEC should not own. Piggybacking on session-exit (option c) was rejected — misses crashed-session worktrees. On-touch reuses existing invocation points, fires when the user is present, and the "stale until next moai invocation" latency is acceptable for a default-OFF opt-in feature. See design.md §C.3. **(v0.2.2 correction: the v0.2.1 names `moai worktree list` / `moai session start` were non-existent — `worktree list` is intentionally retired per `internal/cli/worktree/root_test.go:61`, and `session start` never existed per `internal/cli/session.go:53-59`; the replacement pair `session register` + `session list` are both verified-existing CLI commands, making the "reuses existing invocation points" rationale actually true. See spec.md HISTORY §A v0.2.2.)**
...
- **Q3 — PR-merge cleanup trigger (RESOLVED — on-touch; trigger-point command names corrected v0.2.2).** Trigger is on-touch at `moai session register` and `moai session list`. Periodic background tick rejected (lifecycle complexity); session-exit piggyback rejected (misses crashed-session worktrees). Pinned in design.md §C.3, plan.md M8, AC-SW-022. **(v0.2.2 correction: the v0.2.1 names `moai worktree list` / `moai session start` were non-existent in the codebase — `worktree list` is retired, `session start` never existed. The corrected pair `session register` + `session list` are both existing CLI commands (`internal/cli/session.go:66` register, `:132` list), making the "reuses existing invocation points" rationale accurate. The Q3 rationale shape — on-touch, fires when user present, stale-until-next-invocation acceptable — is PRESERVED; only the command names change.)**
