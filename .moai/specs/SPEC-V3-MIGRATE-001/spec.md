...
priority: High
phase: "Phase 7 - Migration Tool + User Docs"
...
dependencies:
  ...
  - SPEC-V3-SCH-001
  - SPEC-V3-SCH-002
related_gap: [gm#149, gm#150, gm#151]
...
- 개별 migration step (M01-M05)의 로직 구현 — 각각 별도 SPEC(V3-CLN-001, V3-CLN-002, V3-CLN-003)이 담당
...
- 개별 migration step의 내부 로직 (SPEC-V3-CLN-001/002/003, SPEC-V3-MIG-002 담당)
...
핵심 기준:
- `moai migrate v2-to-v3 --dry-run`이 실제 파일 변경 없이 M01-M05 예상 변경 내역을 출력 (master §5.1 예시와 동일 형식).
- `moai migrate v2-to-v3 --yes`가 backup 생성 후 M01-M05 순차 적용 → template refresh → schema auto-repair → summary report 순서로 실행.
- Pre-flight check가 binary version < v3.0.0 시 실패하고 exit code 2 반환.
- 이미 v3.x인 프로젝트에서 재실행 시 exit code 0과 "already migrated" 메시지 출력.
- Rollback(`--rollback <ts>`)이 `.moai/backups/v2-to-v3-<ts>/`에서 파일들을 원위치로 복원.
- `--only M01` 플래그가 M01만 적용하고 다른 step은 skip.
- `--rollback`과 `--yes` 동시 지정 시 `MIGRATE_V2V3_INCOMPATIBLE_FLAGS` 반환.
- SIGINT/SIGTERM 수신 시 SPEC-V3-MIG-001 runner에 시그널 전파, exit code 130/143.
- Summary report가 `.moai/reports/migration-v2-to-v3-<timestamp>.md`에 저장되고 REQ-MIGRATE-001-008 필드를 모두 포함.
- 4-locale 문서(`docs-site/content/{ko,en,ja,zh}/migration/v3.md`)가 release 시 모두 존재하고 CLAUDE.local.md §17 규칙 준수.
- User-modified 파일은 template refresh 단계에서 건드려지지 않음.
...
- **SPEC-V3-MIG-002** (Initial migration set M01-M05): M01-M05 구현체가 존재해야 본 SPEC이 step 3(migration runner)을 실행할 수 있음.
- **SPEC-V3-CLN-001** (Template drift resolution): M01/M03/M04 구현체 제공.
- **SPEC-V3-CLN-002** (Legacy code removal): M05 구현체 제공.
- **SPEC-V3-CLN-003** (Agency archival + docs drift): M02 구현체 제공.
