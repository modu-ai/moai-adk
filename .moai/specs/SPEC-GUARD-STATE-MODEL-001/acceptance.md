...
**Given** the populated manifest; **When** each entry is checked for locator, expected events, window, and measured quantity; **Then** BOTH hold — (a) no entry is missing any of the four, and (b) an entry deliberately constructed without a measured quantity is **rejected with a named error** rather than defaulted.
...
- **Green path:** M2. Passing output is all six values observed across the 7 cases (rows 3 and 5 both yield `OK`).
...
**Given** a workflow file present on disk with no manifest entry; **When** the evaluator runs; **Then** ALL hold — it classifies `UNDECLARED`, appears in the output, is counted, and **the run does not report an all-clear**.
...
**Given** an evaluator whose workflow-file enumeration returns **zero files** (simulated wrong working directory, then a permissions failure), with every manifest entry otherwise classifying `OK`; **When** the run completes; **Then** BOTH hold — (a) the run does **not** report an all-clear, and (b) the enumerated-files count reads `0` in the same output.
...
- **Why clause (b):** the audit's answer to "is any requirement inert?" found no inert requirement but **two inert fields** — counts that were required, rendered, and consumed by nothing. Field-level inertness inside a live requirement is the residue this clause exists to prevent, and the enumerated-files count added by AC-GSM-012 is a count with a consumer by construction (REQ-GSM-010's guard reads it).
...
- **The state table is total over the conditions it enumerates, which is not the same as total over reality.** AC-GSM-007 proves every row classifies and AC-GSM-008 proves every value is reachable; neither proves the seven rows exhaust what the mechanism can encounter. That is the same class of claim that failed three times in the predecessor — the improvement is that an eighth condition now shows up as a missing row rather than as a sentence nobody wrote, not that its absence is impossible.
