...
- **REQ-GSM-006** — The evaluator shall decide every entry it encounters by the **state table below, which is normative**. The table is the artifact; the prose around it describes it and does not extend it.
  ...
  **How the enumeration was derived, so a reader can re-run it rather than trust the table.** The mechanism has exactly two inputs — a manifest of declared entries, and a disk enumeration of workflow files — so the condition space is the product of two axes, and each axis is partitioned by successive binary splits until every branch terminates:
  ...
  - **Entry axis** (a declared manifest entry): does its kind have a reader? (no → row 1) · did the query return a result? (no → row 2) · are there qualifying runs? (yes → inside the window? → rows 3/4) · (no → does the declared condition excuse the absence? → rows 5/6) · **is the subject still on disk?** (no → row 8).
  ...
  **Row 2 carve-out.** A query returning *not-found* for a subject the disk enumeration confirms is gone classifies `ORPHANED` (row 8), never `UNRESOLVED` (row 2). Row 2 covers a query that could not be completed and may succeed later; row 8 covers one that is answering correctly about a subject that no longer exists.
  ...
  | 6 | Query returned zero qualifying runs; the entry's declared condition does not account for the absence | `UNKNOWN` | `warn` | Look again with a longer window; retention may have consumed the history | M2 — **needs M1's release-cycle-conditional field**, the same field row 5 declares: rows 5 and 6 are the two branches of one test, so a missing field does not leave row 6 empty — it makes row 5 collapse into it |
  ...
  | 8 | A manifest entry is declared and the workflow file it names is **no longer on disk** | `ORPHANED` | `warn` | Reconcile the manifest — delete the entry, or restore the subject | M2 — needs the disk enumeration **and** the entry→disk direction of the set comparison (REQ-GSM-008) |
...
- **REQ-GSM-010** — The evaluator shall not report an all-clear while its successfully-queried count is zero, **or its workflow-file enumeration returned zero files, or its enumerated count is implausible against its declared count** — where implausible means the two differ by more than the manifest's own `UNDECLARED` and `ORPHANED` findings account for. Both inputs to the set comparison can degrade, and an enumeration that silently returns empty — wrong working directory, a path constant, a permissions failure — makes `UNDECLARED: 0` indistinguishable between "census complete" and "the evaluator never looked".
