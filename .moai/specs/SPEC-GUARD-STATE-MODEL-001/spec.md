...
- **REQ-GSM-012** — The result shall satisfy the published contract (§B): every entry carries exactly one classification, and **exactly one value — `OK` — denotes *nothing to report***. This SPEC may add, rename, or re-decide classifications provided that clause continues to hold, and the consuming SPEC cannot verify it, so it is verified here.
