...
- 2026-09-25 — AC-HDS-014 판정식 교체 (v0.3.4, 제자리 개정). 독립 sync 감사 F1: REQ-HDS-008
  의 판정식이 **통과 불가**였다. `go test ./internal/cli/... ./internal/hook/... -timeout 120s`
  는 이 머신에서 `internal/cli` 단독 약 1200 초이므로 이 SPEC 이 무엇을 했든 타임아웃으로
  죽고, 감사가 그대로 실행해 `panic: test timed out` 을 받았다. run-phase 는 `-timeout 40m`
  로 치환해 통과시키고 그 치환을 행에 공개했으므로 정직했지만, **문언대로의 기준이 충족된
  것은 아니었다** — 그 기준이 재고 있던 것은 의무가 아니라 머신의 스위트 벽시계다.
  타임아웃을 40 분으로 늘리는 수리는 채택하지 않았다(그만큼 느슨한 경계는 아무것도 변별하지
  못하고, 여전히 무관한 패키지의 실행시간에 판정을 묶는다). 대신 감사 자신이 실측한 모양 —
  **싱크 구현 파일에 대한 대기 원시요소 그물 + 양성 대조** — 으로 교체했다. 40 분 스위트
  실행이 exit 0 으로 끝났다는 기록은 `progress.md` §E.2 의 보조 증거로 남는다. AC 수 16
  불변이므로 AC 스냅숏 재생성은 필요 없고, 면제 4 건 · 두 단 12 건 구성도 불변이다(면제
  사유만 「패키지 전체 스위트」→「grep + 대조」로 바뀐다). 요구사항 15건 문언 불변.
