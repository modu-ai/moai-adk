# SPEC-HOOK-DIAG-SINK-001 — 인수 기준 (v0.3.4)

> **개정 사유 (v0.3.4)**: 독립 sync 감사 F1. AC-HDS-014 의 판정식이 **통과 불가**였다 —
> `go test ./internal/cli/... ./internal/hook/... -timeout 120s` 는 이 머신에서 `internal/cli`
> 단독 약 1200 초이므로, 이 SPEC 이 무엇을 했든 타임아웃으로 죽는다. 즉 그 기준은
> REQ-HDS-008("경계 없는 대기 금지")이 아니라 머신의 스위트 벽시계를 재고 있었다. 판정식을
> **싱크 구현 파일에 대한 대기 원시요소 그물 + 양성 대조**로 교체했다. AC 수는 16 그대로이고,
> 면제 4 건 · 두 단 12 건 구성도 변하지 않는다(면제 사유만 「패키지 전체 스위트」→「grep + 대조」로
> 바뀐다).
...
3. **[HARD] 테스트 기반 판정은 반드시 두 단이다 — 존재 확인이 먼저다.**
   ...
   **면제는 네 건뿐이고, 전부 테스트 이름을 선택자로 쓰지 않는다**: AC-HDS-011(grep —
   미수정 트리에서 이미 exit 1 로 변별), AC-HDS-013(크로스 빌드), AC-HDS-014(grep + 양성 대조 —
   v0.3.4 에서 스위트 전체 실행을 대체했다), AC-HDS-016(grep — 주석 갱신
...
**Given** 구현이 완료된 트리에서,
**When** 싱크 구현 파일(`internal/cli/hook_sink.go`)에 대기 원시요소 그물을 걸고, 같은 그물을
대기가 실재하는 파일에 양성 대조로 걸면,
**Then** 싱크 쪽은 **0 건**이고 대조 쪽은 **1 건 이상**이다.

```
WAIT='time\.Sleep|time\.After|time\.Tick|\.Wait\(\)|sync\.WaitGroup|<-|select[[:space:]]*\{|exec\.Command|net\.Dial|http\.(Get|Post|Do)'
grep -cE "$WAIT" internal/cli/hook_sink.go            # = 0  (부재 팔)
grep -cE "$WAIT" internal/hook/session_start.go       # >= 1 (양성 대조)
```

기준선(HEAD `4ef2a4bab`): 부재 팔 **0**, 대조 팔 **8**. 판정: 부재 팔 = 0 **그리고** 대조 팔
≥ 1. 부재 팔이 1 이상이면 FAIL 이며 그 매치가 훅의 5초 예산 안에서 경계를 갖는지 먼저 따진다.
대조 팔이 0 이면 **그물 자체가 눈이 멀었다는 뜻**이므로 부재 팔의 0 은 판정 근거가 되지 못한다
(판정 원칙 5). `grep -c` 는 0 건일 때 종료코드 1 을 내므로 두 팔은 종료코드가 아니라 **출력 수**로
판정한다.

> **왜 스위트 전체 실행이 판정식이 아닌가.** v0.3.3 의 판정식은
> `go test ./internal/cli/... ./internal/hook/... -count=1 -timeout 120s` 였는데, 이 머신의
> `internal/cli` 는 단독으로도 약 1200 초가 걸린다 — 그 명령은 이 SPEC 이 무엇을 했든
> **통과할 수 없고**, 실제로 감사가 그대로 실행해 `panic: test timed out` 을 받았다. 즉 그
> 판정식은 REQ-HDS-008 의 의무가 아니라 **머신의 스위트 벽시계**를 재고 있었다. 타임아웃을
> 40 분으로 늘리는 수리는 채택하지 않는다 — 그만큼 느슨한 경계는 사실상 아무것도 변별하지
> 못하고, 여전히 무관한 패키지의 실행시간에 판정을 묶는다. 40 분 스위트 실행이 exit 0 으로
> 끝났다는 기록은 `progress.md` §E.2 의 **보조 증거**로 남고, 판정식은 REQ-HDS-008 이 실제로
> 말하는 범위 — 싱크 기록 경로 — 만 겨눈다. (판정 원칙 3 의 두 단 면제 — 테스트 이름을
> 선택자로 쓰지 않는다.)
