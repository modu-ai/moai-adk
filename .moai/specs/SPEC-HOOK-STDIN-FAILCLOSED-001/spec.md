...
- **REQ-HSF-001** (Event-driven + Unwanted): **When** 훅 디스패처가 결정 이벤트의 stdin 을 파싱하지 못하면(`ReadInput` 이 오류를 돌려주는 모든 경우 — 형식 불량, 잘림, 5 MiB 상한 절단, 중첩 깊이 초과 포함), the hook dispatcher **shall** 그 이벤트의 fail-closed 거부를 stdout 에 쓰고 exit 0 으로 끝내며, 어떤 핸들러에도 디스패치하지 않는다. 단 REQ-HSF-013 의 술어가 참인 (하네스, 이벤트) 쌍은 REQ-HSF-012 를 따른다. The hook dispatcher **shall not** 이 거부를 통과로 바꾸는 실행 시점 스위치(환경 변수, `.claude/settings*.json`, `.moai/config/` 키 등 어떤 출처든)를 둔다 — 0.3.x 의 탈출 장치는 Pre-flight 5 에서 「추가 후 제거」로 우회되는 것이 측정돼 0.4.0 에서 없앴다(§F.2). 지속적인 파싱 실패에서의 복구는 moai 갱신이나 훅 비활성화로 하며, 운영자 문서에 적는다.
...
- **REQ-HSF-010** (Ubiquitous): The fail-closed 출력의 사유 문구 **shall** 비어 있지 않으며, (a) `fail-closed` 라는 표시, (b) 원인이 stdin 파싱 실패라는 고정 문구, (c) 운영자 문서 식별자(고정 문자열)를 싣고, 그 밖의 안내는 싣지 않는다(Codex 는 `TranslateCodex` 가 이 사유를 고정 문구로 감싼다 — §B.4). 사유 문구는 모델이 읽는 면이므로(PreToolUse 의 `permissionDecisionReason` 등), 복구 절차(moai 갱신, 훅 비활성화)는 싣지 않고 운영자 문서에만 둔다. 빈 사유는 Codex 에서 거부 자체를 무효로 만든다(`fabc33812:decision.go:171-175`).
