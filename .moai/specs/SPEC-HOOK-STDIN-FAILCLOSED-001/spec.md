...
3. **Kickoff Approval 은 받았고(0.3.4), 구현은 t1099 착지 뒤에 시작한다.** 운영자가 2026-09-24 t1152 레인의 AskUserQuestion 에서 승인했다(판정 내용은 plan.md §F M0, 기록은 progress.md §E.1 「Kickoff 판정」). 구현 커밋은 t1099 가 develop 에 착지하고 이 브랜치가 그것을 흡수한 뒤 시작한다. 그 시점에 위 2의 심볼 diff 와 함께 다음 설계 전제를 다시 확인하고 progress.md 에 남긴다: (a) 설치된 codex-cli 버전이 Q2 측정 버전(0.156.1)과 같은지, 다르면 같은 방법으로 Q2 를 다시 재는지, (b) `runAgentHook` 의 action → 이벤트 매핑(`internal/cli/hook.go:469-481`)이 바뀌지 않았는지, (c) t1099 흡수 트리의 `writeCodexFailClosed` 본문과 두 진입점의 파싱 실패 분기가 환경 변수·설정 파일을 읽지 않는지 — acceptance.md AC-HSF-003(b3) 의 전제다. 읽는다면 (b3) 의 검사 범위를 먼저 고친다(0.4.0 에서 종전 (c) 「설정 파일 형식이 REQ-HSF-007 의 전제와 같은지」를 대체).
...
| 사유 문구를 통한 복구 절차 노출 | 거부 사유는 모델이 읽는다. 사유에 훅 비활성화 같은 복구 절차가 실리면, 설정 파일을 쓸 수 있는 모델에게 가드 전체를 끄는 방법을 알려 주는 셈이다 | REQ-HSF-010 이 사유에 원인과 운영자 문서 식별자만 싣게 한다. AC-HSF-001(e4) 가 카나리와 복구 절차 문구(`disableAllHooks`)의 부재를 단언한다 |
