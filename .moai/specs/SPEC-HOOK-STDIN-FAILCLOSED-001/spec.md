...
tier: M
partially_superseded_by: [SPEC-HOOK-STOP-PARSE-CAP-001]
related_specs:
...
| 0.4.0 | 2026-09-25 | manager-spec (card t1152) | Pre-flight 5 결과에 따라 탈출 장치를 없앴다. 측정(`.moai/reports/t1152/preflight5.md`, 로컬 증거): Claude Code 2.1.281 비대화형 `claude -p` 에서 세션 중 `settings.local.json` `env` 에 추가한 키가 다음 훅 호출부터 훅 환경에 전달되고, 키를 지운 뒤에도 그 세션의 훅 환경에 남았다(A1 실행, 12:00:04–12:00:06 기록 3줄이 `local_declares:false` 인데 탐침 값 `on`). 판정 1 「유지한다」 — 0.3.x REQ-HSF-007 의 출처 제약은 「추가 후 제거」로 우회된다. 운영자 재판정(2026-09-25, t1152 레인 AskUserQuestion): **탈출 장치 없이 결정 이벤트는 언제나 거부한다**(Codex Stop 면제는 그대로). 지속적인 파싱 실패(예: 호스트 형식 변화)는 moai 갱신이나 훅 비활성화(`disableAllHooks`)로 복구하고 운영자 문서에 적는다. 거부 사유는 원인(stdin 파싱 실패)과 운영자 문서 식별자만 싣는다. 처분: REQ-HSF-007·016 삭제(번호 유지, 「삭제됨」 표시), REQ-HSF-001(실행 시점 스위치 금지 절 추가)·010·011 개정, AC-HSF-006 [RETIRED]·AC-HSF-010 [RETIRED] 삭제(번호 유지), AC-HSF-001(e4)·003(b3)·(c6) 개정·신설, §D·§F.1·§F.2 갱신. 남은 변형 A2·B1·B2 는 판정 1 이 우선해 돌리지 않았다. 예산 사용 2/6(모델 호출 0회 인증 실패 4건은 운영자 판정으로 제외). 살아 있는 REQ 14, AC 11 + GATE. |
| 0.4.1 | 2026-09-25 | manager-spec (card t1152) | plan-audit 6회차 FAIL(0.87, `.moai/reports/t1152/plan-audit-iter6.md`)의 차단 결함 N19~N21 과 선택 결함 N22·N23 반영. 운영자가 N19~N21(+N23) 수정 뒤 좁은 7회차 감사로 연장했다(2026-09-25, t1152 레인 AskUserQuestion). N19·N22: acceptance.md AC-HSF-003(b3) 에 기록 싱크의 루트 해석 허용 목록(`resolveHookProjectRoot` 호출, 인자가 `config.EnvClaudeProjectDir` 인 `os.Getenv`)을 두고, 범위를 `internal/cli` 안 직접 호출의 전이적 폐포로 넓히고, 한계 문장을 실제 회피 경로(메서드·함수 값·다른 패키지·금지 목록 밖 판독 원시)로 고쳤다. N20: 삭제된 두 AC 식별자의 모든 출현 바로 뒤에 `[RETIRED]` 를 붙이고 짧은 별칭을 정식 식별자로 풀어 썼다 — AC 계수기 결과 11(`TestACCounterFullCorpusMatchesBaseline` 의 `absent-from-snapshot … COUNT 11`). N21: AC-HSF-001(e4) 를 사유 문자열 전체의 바이트 단위 일치로 바꾸고 복구 안내 덧붙임 RED 변이를 필수로 더했다. N23: plan.md M0 의 미래형 Pre-flight 5 문장을 지웠다. REQ·AC 수 변화 없음(살아 있는 REQ 14, AC 11 + GATE). |
| 0.4.2 | 2026-09-26 | manager-spec (card t1152) | Q2 를 버전에 묶인 관측으로 남긴다는 리드 판정(2026-09-26, kanban 리드)을 한계로 기록했다. Q2 는 codex-cli 0.156.1 에서 쟀고(`.moai/reports/t1152/q2-codex-stop-cap.md`), run 시점의 설치본은 0.157.0 이다(progress.md §E.2 「spec.md §F.1 의 3」 (a)). 재측정하지 않는 이유: 라이브 실행은 운영자의 Codex 사용량에 달려 있고, 결과가 달라져도 바뀌는 곳은 술어 `internal/codexadapter/stop_cap.go` `HostLacksStopBlockCap` 하나라 판정을 되돌리는 비용이 작다. 0.157.0 재측정은 리드가 후속 카드로 발행한다. §F.2 에 행 하나를 더하고 §F.1 3(a)·plan.md §C 3 을 이 판정에 맞췄다. REQ·AC 문구·수·상태 변화 없음(살아 있는 REQ 14, AC 11 + GATE). |
| 0.4.3 | 2026-09-26 | manager-spec (card t1272) | 낡은 측정 서술을 정정했다(t1230 sync-audit F6). Claude 호스트의 Stop 상한이 「측정되지 않았다」는 서술을 t1230(`.moai/reports/t1230/verdict.md`)과 t1272(`.moai/reports/t1272/verdict.md`, 둘 다 로컬 증거) 측정으로 바꿨다: 기본 상한에서 JSON block + exit 0 도 exit 2 와 같이 훅 9회 실행·9회 차단 뒤 턴이 끝났고, 그 상한은 도구 사용 없이 이어진 차단에만 걸리는 것으로 보이며(해석), 칸반·팩토리·무한 goal 세션에 주입되는 200 에서는 턴 상한 30 까지 끊기지 않았다. 고친 곳: REQ-HSF-009, §D 「상한의 측정과 Codex 의 다른 동작 형태」 첫 항목, §D 「핸들러 실패 경로」 둘째 항목, §F.2 「Stop 루프 — Claude」 행, plan.md §B.1 Q2 의 Claude 쪽 항목과 §B.2 Q8. REQ-HSF-001 에는 Claude Stop 상한 예외를, REQ-HSF-010 에는 Claude 하네스 문구 대체 표시를 **포인터로만** 달았다 — 두 요구의 본문과 이 SPEC 의 상태(`completed`)는 그대로다. 새 동작(moai 자체 상한 N=8, 사유 문구 개정)은 SPEC-HOOK-STOP-PARSE-CAP-001 이 정한다. AC 문구·수 변화 없음. |
...
- **REQ-HSF-001** (Event-driven + Unwanted): **When** 훅 디스패처가 결정 이벤트의 stdin 을 파싱하지 못하면(`ReadInput` 이 오류를 돌려주는 모든 경우 — 형식 불량, 잘림, 5 MiB 상한 절단, 중첩 깊이 초과 포함), the hook dispatcher **shall** 그 이벤트의 fail-closed 거부를 stdout 에 쓰고 exit 0 으로 끝내며, 어떤 핸들러에도 디스패치하지 않는다. 단 REQ-HSF-013 의 술어가 참인 (하네스, 이벤트) 쌍은 REQ-HSF-012 를 따른다. The hook dispatcher **shall not** 이 거부를 통과로 바꾸는 실행 시점 스위치(환경 변수, `.claude/settings*.json`, `.moai/config/` 키 등 어떤 출처든)를 둔다 — 0.3.x 의 탈출 장치는 Pre-flight 5 에서 「추가 후 제거」로 우회되는 것이 측정돼 0.4.0 에서 없앴다(§F.2). 지속적인 파싱 실패에서의 복구는 moai 갱신이나 훅 비활성화로 하며, 운영자 문서에 적는다. **[0.4.3 포인터]** Claude 하네스 Stop 에서 연속 파싱 실패가 8회를 넘은 뒤의 응답은 SPEC-HOOK-STOP-PARSE-CAP-001(REQ-SPC-001~003)이 정한다 — 그 상한은 실행 시점 스위치가 아니라 코드 상수다.
...
- **[삭제됨 — 0.4.0, Pre-flight 5 결과] REQ-HSF-007.** 0.3.x 의 탈출 장치(셸 환경 변수 + 프로젝트·로컬 설정 `env` 선언 시 무시) 요구였다. Pre-flight 5 가 호스트의 「추가 후 제거」 값 유지를 측정해 출처 제약이 우회됨을 보였고, 운영자가 탈출 장치를 두지 않기로 재판정했다(§F.2, HISTORY 0.4.0). 이 번호는 감사 보고서의 인용을 살리기 위해 비워 둔다. 대신하는 금지 절은 REQ-HSF-001 에 있다.
...
- **REQ-HSF-009** (Ubiquitous): The Stop 이벤트의 fail-closed 출력 **shall** `stop_hook_active` 의 값에 의존하지 않는다 — 파싱이 실패한 페이로드에서 그 값을 읽을 수 없기 때문이다. Claude 하네스에서 반복 차단의 상한은 호스트의 Stop 차단 상한이 정하며, 그 의존 사실과 근거 등급을 코드 주석(@MX:WARN)에 남긴다. **(0.4.3 정정 — 측정됨)** Claude Code 2.1.283 기본 상한에서 JSON `decision:"block"` + exit 0 차단도 exit 2 와 같이 동작했다 — 훅이 9번 실행돼 9번 차단한 뒤 턴이 끝났다(t1230). 다만 이 상한은 도구 사용 없이 이어진 차단에만 걸리는 것으로 보이며(t1272 B팔: Stop 16회, 마지막 무도구 구간 9회 — 해석, 업스트림 미확인), 칸반·팩토리 세션과 실행 시점에 무한 goal 이 걸린 세션에는 런처가 상한 200 을 주입해 파싱 실패 루프가 턴 상한 30 까지 끊기지 않았다(t1272 A팔). 증거는 `.moai/reports/t1230/verdict.md`, `.moai/reports/t1272/verdict.md`(로컬). 따라서 호스트 상한은 이 루프의 한계로 충분하지 않으며, moai 자체 상한은 SPEC-HOOK-STOP-PARSE-CAP-001 이 정한다(§F.2, plan.md 추적 항목 Q8). Codex 하네스에는 상한이 없다는 측정 결과(plan.md Q2)에 따라 Codex 의 Stop 은 fail-closed 하지 않는다(REQ-HSF-012).
- **REQ-HSF-010** (Ubiquitous) **[0.4.3 — Claude 하네스 문구는 SPEC-HOOK-STOP-PARSE-CAP-001 REQ-SPC-010 이 대체한다. 아래 본문은 기록으로 남기며, Codex 하네스에는 계속 적용된다]**: The fail-closed 출력의 사유 문구 **shall** 비어 있지 않으며, (a) `fail-closed` 라는 표시, (b) 원인이 stdin 파싱 실패라는 고정 문구, (c) 운영자 문서 식별자(고정 문자열)를 싣고, 그 밖의 안내는 싣지 않는다(Codex 는 `TranslateCodex` 가 이 사유를 고정 문구로 감싼다 — §B.4). 사유 문구는 모델이 읽는 면이므로(PreToolUse 의 `permissionDecisionReason` 등), 복구 절차(moai 갱신, 훅 비활성화)는 싣지 않고 운영자 문서에만 둔다. 빈 사유는 Codex 에서 거부 자체를 무효로 만든다(`fabc33812:decision.go:171-175`).
- **REQ-HSF-011** (Unwanted): The fail-closed 출력, 면제의 stderr 줄과 영속 기록 **shall not** 파싱에 실패한 페이로드의 원문이나 그 일부를 싣는다. 입력에서 유래한 정보로 허용되는 것은 파싱 오류 메시지(`encoding/json` 오류가 인용하는 한 글자·숫자 리터럴 수준)와 stdin 바이트 수뿐이다. 모델이 제어하는 `tool_input` 이 사유면·기록면으로 새어 나가지 않게 하기 위함이다.
...
- **[삭제됨 — 0.4.0, Pre-flight 5 결과] REQ-HSF-016.** 0.3.x 에서 탈출 장치의 설정 판독 실패를 불인정하던 요구였다. 탈출 장치가 없어져 대상이 사라졌다(REQ-HSF-007 과 같은 사유). 번호는 비워 둔다.
...
- Stop 체인의 루프 상한 설계(t1099 `@MX:WARN` 이 M2d 의 몫으로 명시한 것). MoAI 자체 Stop 상한(plan.md Q1 의 선택지 A2)은 이 SPEC 에서 채택되지 않았다 — Codex 의 Stop 은 면제 술어로 처리한다(REQ-HSF-012·013). (0.4.3) Claude 하네스 Stop 의 파싱 실패 루프에 대한 moai 자체 상한은 리드 판정 D1(2026-09-26)으로 SPEC-HOOK-STOP-PARSE-CAP-001 이 도입한다.

### Out of Scope — 상한의 측정과 Codex 의 다른 동작 형태

- Claude 호스트의 Stop 상한이 JSON `decision:"block"` + exit 0 에도 걸리는지의 측정. 이 SPEC 은 그것을 독트린 근거로만 두고 잔여 위험으로 추적했다(plan.md 추적 항목 Q8). Kickoff 를 막지 않았다. **(0.4.3 정정)** 이 측정은 이후 카드 t1230·t1272 가 수행했다 — 결과는 REQ-HSF-009 와 §F.2 「Stop 루프 — Claude」 행에 있다. 이 SPEC 의 run-phase 가 그 측정을 한 것은 아니다.
...
- 모델이 `.claude/settings*.json` 이나 `.moai/config/` 를 편집하는 것 자체를 막는 가드. 0.4.0 부터 이 SPEC 에는 fail-closed 를 끄는 설정면 스위치가 없으므로(REQ-HSF-001) 설정 편집이 이 SPEC 의 거부를 직접 끄는 경로는 없다. 다만 호스트 자체의 `disableAllHooks` 는 설정 쓰기로 모든 훅을 끌 수 있는 면이며, 이 SPEC 이전부터 있던 면이다(§F.2). `internal/hook/pre_tool.go` 에 설정 쓰기 가드가 있는지는 plan-audit 의 grep 1회로 찾지 못했을 뿐 부재가 확정되지 않았다.

### Out of Scope — 탈출 장치 (0.4.0 에서 제거)

- 결정 이벤트의 fail-closed 를 실행 시점에 끄는 스위치. 0.3.x 는 셸 환경 변수 방식의 탈출 장치를 두었으나, Pre-flight 5 가 「추가 후 제거」 우회를 측정해 운영자가 제거를 판정했다(HISTORY 0.4.0, §F.2). 이 SPEC 은 대체 스위치를 새로 설계하지 않는다 — 다시 필요해지면 호스트의 설정 전파 방식과 무관한 출처를 요구하는 별도 카드로 다룬다.
- 운영자 문서의 복구 절 본문. 이 SPEC 은 사유 문구가 가리킬 문서 식별자만 정하고, 문서 본문(moai 갱신, `disableAllHooks`, Codex 쪽 훅 비활성화 방법)은 sync-phase 에서 쓴다.
...
3. **Kickoff Approval 은 받았고(0.3.4), 구현은 t1099 착지 뒤에 시작한다.** 운영자가 2026-09-24 t1152 레인의 AskUserQuestion 에서 승인했다(판정 내용은 plan.md §F M0, 기록은 progress.md §E.1 「Kickoff 판정」). 구현 커밋은 t1099 가 develop 에 착지하고 이 브랜치가 그것을 흡수한 뒤 시작한다. 그 시점에 위 2의 심볼 diff 와 함께 다음 설계 전제를 다시 확인하고 progress.md 에 남긴다: (a) 설치된 codex-cli 버전이 Q2 측정 버전(0.156.1)과 같은지, 다르면 같은 방법으로 Q2 를 다시 재는지 — 0.4.2: run 시점 설치본은 0.157.0 이었고 리드 판정으로 재측정하지 않는다(§F.2 「Q2 의 버전 한정」), (b) `runAgentHook` 의 action → 이벤트 매핑(`internal/cli/hook.go:469-481`)이 바뀌지 않았는지, (c) t1099 흡수 트리의 `writeCodexFailClosed` 본문과 두 진입점의 파싱 실패 분기가, acceptance.md AC-HSF-003(b3) 의 범위(`internal/cli` 안 직접 호출의 전이적 폐포) 안에서 허용 목록(`resolveHookProjectRoot` 호출, 인자가 `config.EnvClaudeProjectDir` 인 `os.Getenv`) 밖의 환경 변수·파일을 읽지 않는지 — (b3) 의 전제다. 읽는다면 run 은 거기서 멈추고 허용 목록을 spec 개정으로 늘린다(0.4.1). (0.4.0 에서 종전 (c) 「설정 파일 형식이 REQ-HSF-007 의 전제와 같은지」를 대체).

### F.2 위험

| 위험 | 설명 | 대응 |
|---|---|---|
| Stop 루프 — Claude | 파싱 실패가 지속되면 매 Stop 이 차단된다. 파싱 실패 상태에서는 `stop_hook_active` 도 `session_id` 도 읽을 수 없어 가드식 조기 반환이나 세션별 횟수 계산이 불가능하다 | (0.4.2 까지) 호스트 상한(8회 연속 차단 후 해제, `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`)이 루프를 끊는다고 가정했다. **(0.4.3 정정 — 측정됨, 가정은 조건부로만 성립)** Claude Code 2.1.283, `claude -p --model haiku`, 팔당 1회: (1) 상한 변수 unset 에서 JSON block + exit 0 도 exit 2 와 같이 훅 9회 실행·9회 차단 뒤 턴이 끝났다(t1230 2차). (2) 그러나 실제 `moai hook stop` 경로를 쓴 t1272 B팔에서는 Stop 이 16회 불렸고 마지막 무도구 구간이 9회였다 — 기본 상한은 도구 사용 없이 이어진 차단만 세는 것으로 보인다(해석, 업스트림 미확인). 모델이 도구를 계속 쓰면 기본 상한에서도 루프 길이에 위쪽 한계가 없다. (3) 칸반·팩토리 세션과 실행 시점 무한 goal 세션에는 런처가 `CLAUDE_CODE_STOP_HOOK_BLOCK_CAP=200` 을 주입하며(`internal/cli/launcher_blockcap_infinite.go`), 그 조건의 t1272 A팔은 Stop 23회·무도구 차단 17회 이상에서도 끊기지 않고 턴 상한 30 에서 끝났다. 또 두 팔 모두 모델이 fail-closed 사유를 받은 뒤 `.claude/settings.json` 과 훅 스크립트를 편집하려 했다(`-p` 기본 권한 모드라 거부됨). 증거: `.moai/reports/t1230/verdict.md`, `.moai/reports/t1272/verdict.md`(로컬). 대응은 SPEC-HOOK-STOP-PARSE-CAP-001(moai 자체 상한 N=8, 사유 문구 개정)이 맡는다 |
...
| Q2 의 버전 한정 — 0.157.0 미재측정 (0.4.2) | Codex Stop 면제(REQ-HSF-012·013)의 전제인 「Stop 연속 차단 상한 없음」은 codex-cli 0.156.1 에서만 쟀다(`.moai/reports/t1152/q2-codex-stop-cap.md`). run 시점에 설치된 codex-cli 는 0.157.0 이며(progress.md §E.2 「spec.md §F.1 의 3」 (a)), 이 버전에서는 재지 않았다. 0.157.0 에서 상한이 새로 생겼다면 면제는 불필요한 fail-open 한 쌍을 남기고, 상한이 여전히 없다면 면제는 그대로 옳다 — 어느 쪽인지 지금은 모른다 | 리드 판정(2026-09-26, kanban 리드): 재측정하지 않고 버전에 묶인 관측으로 남긴다. 근거는 둘이다 — 라이브 실행은 운영자의 Codex 사용량에 달려 있고, 결과가 달라져도 바뀌는 곳은 술어 `internal/codexadapter/stop_cap.go` `HostLacksStopBlockCap` 하나라 되돌리는 비용이 작다. 0.157.0 재측정은 리드가 후속 카드로 발행한다. 그 결과가 상한 존재로 나오면 그 술어와 §B.2 행 3 을 개정한다 |
...
| 지속적 파싱 실패로 인한 잠김 | 호스트 형식 변화처럼 지속적인 원인이면 결정 이벤트가 매번 거부된다 — PreToolUse 는 모든 도구 호출을, UserPromptSubmit 은 모든 프롬프트를 막아 세션을 쓸 수 없다. 0.4.0 부터 이 SPEC 안에는 이 잠김을 푸는 스위치가 없다(REQ-HSF-001) | 복구는 둘 가운데 하나다: (1) 호스트 형식을 따라잡은 moai 로 갱신한다(`moai update` 의 바이너리 갱신 경로), (2) 훅을 끈다 — Claude Code 는 설정의 `disableAllHooks`. Codex 쪽 훅을 끄는 방법은 이 SPEC 에서 재지 않았고 운영자 문서가 정한다. 차단 사유가 원인(stdin 파싱 실패)과 운영자 문서 식별자를 알리므로(REQ-HSF-010) 운영자는 문서에서 이 복구 절을 찾는다. 잠김은 소리가 나고 우회는 소리가 나지 않는다 — 이 비대칭이 fail-closed 를 택하는 근거이며, 스위치를 없앤 대가로 복구 비용이 「환경 변수 하나」에서 「갱신 또는 훅 전체 비활성화」로 커졌다. 운영자 재판정(HISTORY 0.4.0)이 이 대가를 받아들였다 |
| 호스트 형식 변화의 파급 | `6a3603274` 가 막으려던 상황, 즉 새 호스트 버전에서 페이로드 형식이 바뀌어 매 호출이 파싱 실패하는 경우, 결정 이벤트 전부가 거부된다 | 관측 이벤트는 보존되므로 세션 시작·종료·관측 경로는 영향이 없다. 복구는 위 「지속적 파싱 실패로 인한 잠김」 행의 두 경로다. 기록(REQ-HSF-008)이 원인 진단의 근거가 된다 |
| 탈출 장치를 두지 않은 이유 — 「추가 후 제거」 값 유지 (측정) | 0.3.x 는 셸 환경 변수 탈출 장치에 「프로젝트·로컬 설정 `env` 에 같은 키가 선언돼 있으면 무시」라는 출처 제약을 걸었다. 제약은 훅 실행 순간의 설정 파일 상태만 본다. Pre-flight 5(`.moai/reports/t1152/preflight5.md`, 로컬 증거, Claude Code 2.1.281, 비대화형 `claude -p`, 격리 `/tmp` 프로젝트)가 이 제약의 성립 조건을 쟀다: 양성 대조 C 는 셸 환경의 탐침을 두 기록 모두에서 봤고, A1(세션 안에서 `settings.local.json` `env` 에 키 추가 → 키 삭제)은 추가 직후 PostToolUse 부터 탐침 `on` 을 봤으며, 키를 지운 뒤의 기록 3줄(12:00:04 PostToolUse, 12:00:06 PreToolUse·PostToolUse)이 `local_declares:false` 인데도 탐침 `on` 이었다. 셸 환경에는 키가 없었다(첫 기록 `probe:""`). 즉 호스트는 세션 중 추가된 설정 `env` 를 훅 환경에 반영하고, 키를 지운 뒤에도 유지한다 — 판정 1 「유지한다」 | 운영자 재판정(2026-09-25): 탈출 장치를 없앤다(REQ-HSF-007·016 삭제, REQ-HSF-001 금지 절). 관측 범위: 이 호스트·이 버전·`-p` 형태·`settings.local.json` 면의 키 삭제 변형 한 번. A2(파일 삭제)·B1·B2(세션 시작 전파)는 판정 1 이 우선해 돌리지 않았다 — 한 변형의 유지만으로 출처 제약이 우회됨이 서므로 결론은 바뀌지 않는다. 예산 사용 2/6(모델 호출 0회 인증 실패 4건은 운영자 판정으로 예산에서 제외, 증거 파일 §1) |
...
| 사유 문구를 통한 복구 절차 노출 | 거부 사유는 모델이 읽는다. 사유에 훅 비활성화 같은 복구 절차가 실리면, 설정 파일을 쓸 수 있는 모델에게 가드 전체를 끄는 방법을 알려 주는 셈이다 | REQ-HSF-010 이 사유에 원인과 운영자 문서 식별자만 싣게 한다. AC-HSF-001(e4) 가 사유 문자열 전체를 정해진 형태와 바이트 단위로 비교하고(복구 안내 덧붙임 변이가 필수 RED), (e5) 가 카나리와 복구 절차 문구(`disableAllHooks`)의 부재를 단언한다(0.4.1). 상수 값 자체에 든 안내는 리뷰 몫이다 |
| 복구 경로 `disableAllHooks` 의 성질 | `disableAllHooks` 는 그 설정 범위의 모든 훅(MoAI 의 관측 훅 포함)을 끈다 — 잠김을 푸는 대가로 이 SPEC 이 지키려는 가드 전체가 꺼진다. 또 설정 파일을 쓸 수 있는 모델도 같은 키를 쓸 수 있다. 이 면은 이 SPEC 이 새로 여는 것이 아니라 호스트가 원래 제공하는 것이고, 탈출 장치를 없앤 뒤에도 그대로 남는다 | 잔여 위험으로 기록한다. 설정 쓰기 자체를 막는 가드는 범위 밖이다(§D). 운영자 문서의 복구 절은 잠김을 푼 뒤 훅을 다시 켜는 절차를 함께 적는다(sync-phase) |
