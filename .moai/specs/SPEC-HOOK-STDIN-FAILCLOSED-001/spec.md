...
- **REQ-HSF-005** (Ubiquitous): The hook dispatcher **shall** 하네스 모드를 stdin 을 읽기 **전에** 결정하여, 파싱 실패 분기가 발화하는 시점에 하네스 모드가 이미 알려져 있게 하고, 잘못된 `--harness` 값은 stdin 을 읽기 전에 0 이 아닌 종료로 거부한다. 두 진입점의 현재 동작은 다르다. `runHookEvent` 는 지금도 잘못된 값을 0 이 아닌 종료로 거부하므로(`internal/cli/hook.go:292`), 이 요구는 그 거부를 stdin 판독 앞으로 옮길 뿐이다. `runAgentHook` 은 지금 `--harness` 를 읽지 않아 잘못된 값을 무시하고 성공한다(`harnessModeIsCodex` 의 비테스트 호출처는 `hook.go:292` 하나뿐). 따라서 `runAgentHook` 의 하네스 판독과 잘못된 값의 거부는 이 요구가 **새로 도입하는 동작**이며, 유효한 stdin 과 잘못된 `--harness` 로 부른 agent 호출은 성공에서 실패로 바뀐다 — 보존 동작이 아니라 의도된 계약 변경이다(REQ-HSF-014, AC-HSF-012).
