...
| `huh field_confirm.go:260-265` (확인 내부 빈줄) | huh v2.0.3 `field_confirm.go:270-274` — 제목 뒤 고정 `"\n\n"`(`!c.inline`일 때) | **잔여 결함** — 테마로 제거 불가, `Confirm.Inline(bool)`(`:136`)이 유일한 우리 쪽 지점 |
...
| (신규 발견) 영어 고정 고지문 | `internal/cli/profile_setup.go:31` `acceptEditsConfirmationLine` — stderr 고지, grep 앵커 토큰 계약(REQ-CCI-006/AC-CCI-006) | REQ-TRI-006 대상 — 현지화 시 앵커 토큰 보존 |

## §C Pre-flight (M1 착수 조건)

1. `go test ./internal/cli/wizard/ -run 'TestLayout|TestOptionColumn|TestDrawInit' -count=1` — plan 단계 기준 통과 확인(이미 수행, progress.md §D).
...
- 확인 필드 내부 간격 축소(REQ-TRI-003) — 인라인 모드 등 라이브러리 설정 지점 적용, 캡처 프레임으로 빈 행 ≤1 확인.
...
- huh v2.0.3 소스: 모듈 캐시 `charm.land/huh/v2@v2.0.3/field_confirm.go`(:53 기본 정렬, :136 Inline, :270-274 고정 빈 줄, :361 WithButtonAlignment).
