...
- D3: 확인 필드 내부 고정 빈 줄의 유일한 우리 쪽 제어점은 라이브러리 인라인 모드(huh v2.0.3 `field_confirm.go:136`)로 판독 — M2 에서 캡처로 확정.
...
- 확인 내부 빈 줄: huh v2.0.3 `field_confirm.go:270-274` — `!c.inline`일 때 제목 뒤 `"\n\n"` 고정 출력(테마로 제거 불가 → REQ-TRI-003).
...
|---|---|---|---|
| 1 | 렌더 정렬·간격 기존 검사가 이 트리에서 통과 | `go test ./internal/cli/wizard/ -run 'TestLayout\|TestOptionColumn\|TestDrawInit' -count=1` | `ok github.com/modu-ai/moai-adk/internal/cli/wizard 0.607s` |
...
| 6 | 라이브러리 기본 정렬·고정 빈 줄 존재 | huh v2.0.3 `field_confirm.go` 판독 | `:53 buttonAlignment: lipgloss.Center`, `:270-274` `"\n\n"` 출력부, `:136 Inline`, `:361 WithButtonAlignment` |
