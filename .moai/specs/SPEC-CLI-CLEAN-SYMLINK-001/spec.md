...
- 2026-08-22 (감사 iter-1 반영, v0.1.1): plan-audit **FAIL 0.92**(must-pass MP-1~MP-7 전부
  PASS, blocking 2건). D1 — §B 서두의 dangling 하위-카운트 "배치 2곳/두 진입 팔"이 자기
  표(FX-3a/3b/3c)·3형제 산물과 모순되던 것을 **3배치**로 정정(본 SPEC이 [HARD]로 못박은
  AP-2 드리프트 계급의 자체 사고). D2 — 교차 계약의 "보유"를 **렌더링 후 기록 경로**로
  고정(루트 1이 `settings.json.tmpl`로만 보유 — 원시 판독은 도시에 §1.1(b)으로 반증).
  D3 — AC-CSL-003에 진행줄 단언 추가. D4 — AC-CSL-001 RED 라벨 귀속 정정(재실행은 코드
  추적). REQ/AC/형태 수치 불변(12/11/5).
...
픽스처 형태는 5종(도시에 §1.3 매트릭스가 형태×분기 근거 데이터; dangling은 배치 3곳 —
비-글롭 뿌리 / 글로브 매치 이름 / `.moai/config` 뿌리 — 이 같은 형태의 진입 팔이다. 이
중 실측은 2개 배치(Run D), config 팔은 코드 추적 — 도시에 §2.1·gap 4):
