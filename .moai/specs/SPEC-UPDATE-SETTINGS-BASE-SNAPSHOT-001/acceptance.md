...
| REQ-USB-005 (승격 시점과 조건) | AC-USB-005, AC-USB-006, AC-USB-016 |
| REQ-USB-006 (안 바꾼 leaf 에 템플릿 값) | AC-USB-001, AC-USB-006 |
...
| REQ-USB-011 (새 키 전달 유지) | AC-USB-006, AC-USB-010, AC-USB-016 |
...
"RED-now" 는 이 SPEC 을 구현하기 전 트리(`81c1d58f9`)에서 해당 테스트가 **값 단정에서** 실패해야 한다는 뜻이다. 컴파일 오류는 RED 로 치지 않는다. 아직 없는 시접이 필요한 기준은 **RED-stub** 으로 표기한다. 시접의 틀만 만들고 동작을 비워 둔 상태에서 값 단정으로 실패하는 것을 뜻한다.
...
- **RED 이유** — 유도 base 에서는 "사용자만 바꿈"으로 끝나 충돌 줄이 없다. 값 단정은 오늘도 통과한다.
- **green 경로** — M3.
...
- **Given** `makeScenarioA` 기반 프로젝트가 있다. 사용자 `.claude/settings.json` 에는 사용자만 가진 키 `"userOnly": true` 와 폐기 v2 deny 항목 1개가 있다. 확정본 자리에는 표식 바이트 `{"marker":"prior"}` 를 미리 심는다. 테스트 배포기 대역(plan.md M4 확장)은 배포 때 렌더 `R`(`userOnly` 없음, 폐기 deny 없음)을 `.claude/settings.json` 에 쓰고, 병합 호출 직전 확정본 바이트를 기록하는 관측 콜백을 제공한다. 홈은 `homeSeamSpy` 로 주입한다.
- **When** `runCleanReinstall` 을 실행한다.
- **Then** 네 가지를 단정한다.
  - (i) 병합 직전 관측한 확정본 바이트가 `{"marker":"prior"}` 다.
  - (ii) 병합 직전 대기본 바이트가 `R` 이다.
  - (iii) 흐름이 끝난 뒤 확정본 바이트가 `R` 이다(D5 경우 1, 병합이 결과를 씀).
...
- **When** plan.md Decision D8 의 흐름 시접으로 두 사이클을 돌린다.
...
- **반증** — 두 하위 테스트 각각에서 M-06c 와 M-06t 는 `newKey` 단정이 RED 여야 한다. base 와 updated 가 같아져 새 키가 사용자 삭제로 읽히기 때문이다.
- **green 경로** — M3.

### AC-USB-007 — 대기본 기록 지점 세 곳이 배포 성공 뒤, 첫 재기록 전에 무조건 실행된다 · **RED-now**

- 패키지·테스트: cli(슬롯) · `TestSettingsSnapshot_WriteSites` (하위 테스트 `init`, `template_sync`, `clean_reinstall`)
- **Given** 각 흐름을 실행할 테스트 하네스가 있다.
  - `clean_reinstall` 은 AC-USB-005 하네스를 쓴다.
  - `init` 은 새 디렉터리(settings.json 없음)에 렌더를 쓰는 배포기 대역을 쓰며, `tool-policy.yaml` 이 있어 자율성 번들이 settings.json 을 고치는 조건이다.
  - `template_sync` 는 백업 경로가 비는 조건(`configBackupPath == ""`)과 차는 조건 두 셀이다.
- **When** 각 흐름을 실행한다.
- **Then** 모든 셀에서 흐름이 끝난 뒤 확정본 바이트가 배포 대역이 쓴 렌더와 같다(D5 경우 1·6·8, 모두 승격). `init` 셀에서는 자율성 번들이 고친 뒤의 settings.json 과 다르다.
- **행동 검사를 세울 수 없는 흐름의 대체 기준.** 소스 위치 검사로 대신하되, 조건을 "기록 호출이 배포 호출의 성공 경로와 같은 블록 안의 무조건 문장"으로 조인다. 이 경우에도 M-07d·M-07e 가 RED 여야 하며, 대체를 택한 사실과 이유를 `progress.md` §E.2 에 적는다.
- **RED 이유** — 기록이 어느 흐름에도 없다.
- **반증** — M-07a, M-07b, M-07c, M-07d(`template_sync` 에서 기록을 `if configBackupPath != ""` 블록 안으로 옮김 → 비는 조건 셀 RED), M-07e(`init` 에서 기록을 `executor.Execute` 오류 분기 안으로 옮김 → RED).
- **green 경로** — M4.

### AC-USB-008 — 대기본 기록·승격 실패는 흐름을 막지 않고 구별되는 경고를 남긴다 · **RED-now**

- 패키지·테스트: cli(슬롯) · `TestSettingsSnapshot_WriteFailureDoesNotBlock` (하위 테스트 `clean_reinstall`, `helper`)
- **Given** AC-USB-005 하네스를 쓰되 `.moai/cache/template-snapshot/claude` 자리에 일반 파일을 심는다. 대기본 기록은 실패하고, sections 스냅숏 기록은 영향을 받지 않는다.
- **When** `runCleanReinstall` 을 실행한다(`clean_reinstall`). 세 흐름이 함께 부르는 기록 헬퍼를 직접 호출한다(`helper`).
- **Then** 반환 오류는 `nil` 이다. 출력에서 `settings-snapshot-write-failed:` 로 시작하는 줄이 정확히 한 번 나온다. `Warning: template snapshot write failed:` 로 시작하는 줄은 0개다. `.claude/settings.json` 은 폴백 병합 결과로 기록된다.
- **REQ-USB-010 의 init·template_sync 적용 범위.** 세 흐름은 같은 헬퍼를 부르며, 그 호출 사실은 AC-USB-007 이 행동으로 확인한다. 이 둘의 조합을 init·template_sync 의 비차단 근거로 삼는다.
- **RED 이유** — 대기본 기록이 없어 문구 단정이 실패한다.
- **반증** — M-08, M-08s.
- **green 경로** — M4.
...
- **Then**
  ...
  - 셀 B: `HasSnapshot == false` 이고, `destDir/sections/` 의 바이트가 `SaveTemplateDefaults(otherDir)` 결과와 같다(임베디드 폴백).
...
- **반증** — M-11. 가짜 스냅숏을 base 로 찾는 뮤턴트는 값을 새 렌더로 뒤집는다.
...
- **반증** — M-14("건너뜀과 무관하게 배포 뒤 디스크의 `.claude/settings.json` 을 읽어 기록"). 두 기존 파일 셀에서 대기본이 `U` 로 생겨 RED 여야 한다.
- **green 경로** — M4.
...
### AC-USB-016 — 승격 규칙: 흐름 끝의 살아 있는 파일이 렌더를 반영할 때만 승격한다 · 셀별 표기

- 패키지·테스트: 흐름 시접이 사는 곳(merge 또는 backup) · `TestSettingsSnapshotFlow_PromotionRule` (하위 테스트 `c1` … `c8`, 번호는 plan.md Decision D5 경우 번호)
- **공통 Given.** 이전 흐름의 렌더 `R1 = {"a":1}` 가 확정본으로 있다. 이번 흐름의 렌더는 `R2 = {"a":2,"K":1}` 이다(값 변경 `a` 와 새 키 `K`). 다음 흐름의 렌더는 `R3 = {"a":2,"K":1}` 이다.
- **When** 흐름 시접으로 이번 흐름을 셀 조건대로 실행하고, 셀에 따라 다음 흐름을 한 번 더 실행한다.

| 셀 | 이번 흐름의 조건 | Then — 이번 흐름 뒤 확정본 | Then — 다음 흐름 뒤 | 표기 |
|---|---|---|---|---|
| c1 | 사용자 `{"a":1,"u":1}`, 병합이 결과를 씀(살아 있는 파일 ≠ 렌더 바이트) | `R2` | — | RED-stub |
| c2 | 사용자 파일이 JSON 손상 `{"a":`, 병합이 통째 보존. 다음 흐름 전 사용자가 파일을 `{"a":1}` 로 고침 | `R1` 유지, 대기본 없음 | `K == 1` (새 키 도착) | 이번 흐름: RED-stub(대기본 폐기 단정) / 다음 흐름: guard |
| c3 | 사용자 파일이 `R2` 와 바이트 동일 | `R2` | — | RED-stub |
| c4 | 배포 뒤 병합 전에 중단(살아 있는 파일 = `R2`), 복원 없음 | 확정본 `R1` 그대로, 대기본 `R2` 남음 | 다음 흐름 시작 판정으로 확정본 `R2`. 다음 흐름 뒤 `a == 2`, `K == 1` | RED-stub |
| c5 | c4 와 같이 중단한 뒤, 다음 흐름 전 사용자 파일 `{"a":1}` 이 되돌려짐(복원 모사 — 실제 `--restore` 동작은 M1 검증 항목) | 확정본 `R1` 그대로, 대기본 `R2` 남음 | 다음 흐름 시작 판정으로 대기본 폐기, 확정본 `R1` 유지. 다음 흐름 뒤 `K == 1` | 다음 흐름: RED-stub(대기본 폐기 단정) |
| c6 | init 이 렌더 `R2` 를 씀, 이후 자율성 번들 모사가 `a` 를 `3` 으로 고침 | `R2` | — | RED-stub |
| c8 | update, 사용자 settings.json 이 원래 없음(백업 항목 없음) | `R2` | — | RED-stub |

- D5 경우 7(init 이 기존 파일을 건너뜀)은 AC-USB-014 가 담당한다.
- **반증**

| 뮤턴트 | RED 가 나야 하는 셀 | 이유 |
|---|---|---|
| M-D5a — 항상 승격 (리드 요구) | c2 다음 흐름, c5 다음 흐름 | 보존 흐름의 렌더 `R2` 가 확정본이 되어, 다음 흐름에서 `K` 가 base 에는 있고 사용자 파일에는 없으므로 사용자 삭제로 읽혀 `K` 가 도착하지 않는다 |
| M-D5b — 병합이 결과를 쓸 때만 승격 | c4, c6, c8 | 병합이 없거나 중단된 흐름에서 승격하지 못해 이전 렌더→이번 렌더 사이 변경이 사용자 변경으로 읽힌다 |
| M-D5c — 정상 종료 흐름을 바이트 동일로 판정 | c1, c6 | 병합 결과와 번들이 고친 파일은 렌더와 바이트가 달라 승격되지 못한다 |
| M-D5d — 남은 대기본을 판정 없이 버림 | c4 | 중단 뒤 살아 있는 파일이 렌더인데도 승격되지 않는다 |
| M-D5e — 중단 흐름의 승격을 중단 시점에 결정 | c5 | 중단 시점의 살아 있는 파일은 렌더라 승격되고, 뒤이은 복원을 보지 못한다 |
| M-D5f — 다음 흐름이 새 대기본을 기록한 뒤 남은 대기본을 판정 | c4 | 남은 대기본이 새 대기본으로 덮여 판정 대상이 사라진다 |
...
| M-06t | template_sync 형 순서(배포 단계 호출 안)에서 확정본을 새 렌더로 덮음 | AC-006 `split_deploy_then_restore_order` |
| M-07a | init 의 기록을 `ApplyAutonomyTierBundle` 뒤로 옮김 | AC-007 `init` |
| M-07b | template_sync 의 기록 삭제 | AC-007 `template_sync` |
| M-07c | clean_reinstall 의 기록을 `stripRetiredV2DenyEntries` 뒤로 옮김 | AC-007 `clean_reinstall` |
| M-07d | template_sync 의 기록을 `if configBackupPath != ""` 블록 안으로 옮김 | AC-007 `template_sync` 백업 경로가 비는 셀 |
| M-07e | init 의 기록을 `executor.Execute` 오류 분기 안으로 옮김 | AC-007 `init` |
| M-08 | 기록 실패를 흐름 오류로 반환 | AC-008 |
...
| M-D5b | 병합이 결과를 쓸 때만 승격 | AC-016 c4·c6·c8 |
| M-D5c | 정상 종료 흐름을 바이트 동일로 판정 | AC-016 c1·c6 |
| M-D5d | 남은 대기본을 판정 없이 버림 | AC-016 c4 |
| M-D5e | 중단 흐름의 승격을 중단 시점에 결정 | AC-016 c5 |
| M-D5f | 새 대기본 기록 뒤 남은 대기본 판정 | AC-016 c4 |

각 관측은 명령, 출력 원문, 종료 코드, 트리 SHA(뮤턴트 적용 트리 포함)를 `progress.md` §E.2 에 남긴다.

## §F Edge Cases

- **버전 일치로 update 를 건너뜀** — 배포 자체가 없어 대기본도 기록되지 않고, 확정본은 그대로다. 이전 흐름이 남긴 대기본이 있으면 배포를 건너뛰는 흐름에서도 판정할지는 run 단계가 흐름 시접 배치로 정한다. 어느 경우든 판정 결과는 D5 규칙을 따른다.
...
- **복원이 settings.json 을 되돌리지 않는 경우** — 중단 뒤 복원을 해도 살아 있는 파일이 렌더로 남아 D5 경우 4 와 같아진다(승격). M1 검증 결과에 따라 AC-USB-016 c5 의 이름만 조정한다.
...
- §E 뮤턴트 22개의 RED 관측이 `progress.md` §E.2 에 있다.
