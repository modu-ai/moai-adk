...
출처 증명 없이 디스크의 현재 파일을 다시 읽는 방식은 금지한다(M-14). **근거 2(매니페스트 기록 + 해시)를 우선한다(감사 N-11).**

- **근거 2 (우선).** 배포 뒤 매니페스트 항목이 `TemplateManaged` 이고, 기록된 해시가 배포가 쓴 렌더 바이트의 해시와 같을 때만 "썼다"로 본다. 건너뛴 파일은 기록이 없던 경우 `UserCreated` 로 기록되고, 이미 사용자 소유였던 경우 그 기록이 남는다(`deployer.go:243-253`).
- **대기본 바이트의 출처 (리드 승인 2026-09-11).** 대기본은 디스크의 `.claude/settings.json` 바이트를 읽어 만든다. 다만 매니페스트 항목의 출처가 `template_managed` 이고 그 `template_hash` 가 읽은 파일의 SHA-256 과 같을 때만 읽는다. 그 해시는 배포기가 렌더해 쓴 바이트의 해시이므로, 이 조건을 통과한 바이트가 곧 배포기의 렌더다. 두 조건 가운데 하나라도 어긋나면 기록하지 않는다. 배포기 반환값에서 렌더 바이트를 직접 받는 경로는 택하지 않는다. 그 경로는 `internal/template` 을 바꿔야 하는데, 이 모듈은 이 SPEC 의 범위 밖이다.
...
- 배포 뒤 매니페스트 출처 증명(`template_managed` + `template_hash` 일치) 없이 디스크 파일을 읽어 대기본을 만들지 않는다(M-14).
