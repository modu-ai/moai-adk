...
artifacts: spec.md + plan.md + acceptance.md + progress.md (Tier M) + spec-compact.md — v0.1.1
card: t619
baseline: worktree `.claude/worktrees/t619`, branch `WT-toolpolicy-drift`, plan 작성 HEAD `c7b8d110b`, 1회차 감사 HEAD `b2cbfd207`, `git merge-base HEAD develop` → `d1b61005d20967fdbd970ec7ec734c6d14f29dc3`
...
- D8 관련 가드 실측: `trap` 포함 명령, heredoc 으로 python/bash 에 스크립트를 넘기는 명령, 변수로 계산된 스크립트 경로를 python 에 넘기는 명령이 모두 워크트리 세션 가드에 거부됨. 리터럴 절대 경로의 스크래치 스크립트 호출은 통과(항목 173 → 172, 복원 sha 일치, 없는 도구 지정 시 종료 1).
- 재감사 대기.
