...
> **v0.2.0 refresh**: AC-ADC-002 / AC-ADC-003a / AC-ADC-003b 는 REQ-ADC-002/003 과 함께 #1142로
> 해소되어 제거되었다 (번호 공간은 추적성 보존을 위해 비워둠). 아래는 잔여 활성 AC.

| AC | REQ | 검증 명령 | 기대 결과 |
|----|-----|-----------|-----------|
| AC-ADC-001 | REQ-ADC-001 | `ls .moai/config/astgrep-rules/go-hardcoding.yml 2>&1` | `No such file or directory` (orphan 부재) |
| ~~AC-ADC-002~~ | ~~REQ-ADC-002~~ | — | ✅ RESOLVED by #1142 (ruleDirs `[go, security]` 정렬) |
| ~~AC-ADC-003a~~ | ~~REQ-ADC-003~~ | — | ✅ RESOLVED by #1142 (utils 제거) |
| ~~AC-ADC-003b~~ | ~~REQ-ADC-002~~ | — | ✅ RESOLVED by #1142 (ruleDir 2개) |
| AC-ADC-004 | REQ-ADC-004 | `for d in cpp flutter java javascript python r rust scala swift typescript; do ls -d .moai/config/astgrep-rules/$d 2>/dev/null; done \| wc -l \| tr -d ' '` | `0` (빈 stub 10개 전부 부재) |
...
| AC-ADC-006 | REQ-ADC-006 | `grep -c 'SPEC-' .moai/config/astgrep-rules/sgconfig.yml` | `0` (내부 SPEC-ID 부재; v0.2.0 타깃 = `SPEC-GATE-ASTGREP-REPAIR-001` 접두 strip) |
...
| AC-ADC-009b | REQ-ADC-009 | `find .moai/config/astgrep-rules/security -type f \| wc -l \| tr -d ' '` | `5` (security 실제 룰 5개 보존, credentials.yml 포함) |
...
### 시나리오 1 — 정리 후 config-mode 파싱 (정상 경로)

- **Given** 로컬 `sgconfig.yml`의 `ruleDirs`가 `[go, security]`로 정렬(#1142 완료)되고, orphan + 빈/데모
  stub 디렉터리가 제거되며, 주석의 내부 SPEC-ID가 strip된 상태에서,
- **When** maintainer가 `sg scan --config .moai/config/astgrep-rules/sgconfig.yml .`을 실행하면,
- **Then** ast-grep 엔진은 누락 ruleDir 오류 없이 설정을 파싱하고 정상 종료한다 (AC-ADC-007).

### 시나리오 2 — 병렬 세션 레이스 방어 (go/security 보존)

- **Given** run-phase pre-flight가 `go/`·`security/` dirty 여부와 `security/credentials.yml` untracked
  여부를 재실측한 상태에서 (레이스 신호 있으면 STOP),
- **When** 정리 run-phase가 orphan 1개 + 15개 stub/demo 디렉터리(빈 10 + 데모 5)를 경로-한정 `git rm`으로 삭제하면,
- **Then** `go/` 5파일과 `security/` 5파일(credentials.yml 포함)은 변경 없이 보존되고,
  삭제는 명시된 경로(orphan 파일 1 + 디렉터리 15)에만 적용된다 (AC-ADC-009a/009b).
...
- [ ] 활성 AC 전부 PASS: AC-001, AC-004, AC-005, AC-006, AC-007, AC-008, AC-009a/b, AC-010
      (AC-002/003a/003b는 #1142 해소로 제거됨 — 실제 명령 출력 관측 증거 첨부)
- [ ] 시나리오 1~3 통과
- [ ] `git status`에서 삭제가 정확히 한정됨: orphan 파일 1 (`go-hardcoding.yml`) + 디렉터리 15
      (빈 stub 10 + 데모 stub 5). tracked 파일 총 31개 = orphan 1 + .gitkeep 15 + 데모 boilerplate 15.
- [ ] 템플릿 트리 diff 0 확인
