...
- 따라서 `sandbox` 축의 `enforced` 매핑은 하위 에이전트 역할에 대해 근거를 잃는다. 이 문서는 그 축을 "강제 불가(부모 세션 sandbox 상속)"로 고쳐 적는다. 방출기의 축 계약과 사용자 대면 문구는 이 카드의 커밋 `17bfceaa8`에서 고쳤다. `agents-codex.yaml`의 `axis: sandbox` 행은 `mapping: UNSUPPORTED`, `basis: measured`로 바뀌었고 이유(하위 에이전트는 부모 세션의 sandbox를 물려받으며 역할 파일의 `sandbox_mode`는 적용되지 않음)가 함께 적혔다. `AGENTS.md.tmpl`의 `audit-verdict-file` 행과 두 감사 역할 TOML의 부록도 실제 동작을 적는다. Codex의 감사 역할은 부모 세션의 sandbox를 따르고, 역할 파일의 read-only는 강제되지 않으며, 판정 파일은 반환된 원문으로 부모가 쓴다. 방출되는 `sandbox_mode` 값은 계약 값 그대로 남는다. 후속 카드 t1143에 남은 일은 감사자를 최상위 read-only 실행 경로로 띄워 read-only를 실제로 강제하는 것이다.
