...
- LIVE AC(AC-DHR-012, AC-DHR-018, AC-DHR-023)는 결정적 AC와 따로 집계한다. LIVE 미실행은 `NOT_RUN`, 예산 초과 중단은 `ABORTED`로 적고 해당 설계 기준을 PASS로 올리지 않는다. LIVE 판정은 테스트 pass만이 아니라 증거 줄의 양성 증거(시도 증거, 양성 대조, 호출 수 ≤ 예산)를 요구한다.
- LIVE 증거 줄 형식: 테스트가 표준 출력에 한 줄로 `<TAG> <json>`을 찍는다(`fmt.Println`, `t.Log` 아님). 판정식이 그 줄을 JSON으로 읽는다.
...
**Then** 각 건은 설계 표의 분류(`completed` / `not-applied` / `diverged`)를 받고, `completed` 건은 manifest에 저널의 provenance가 반영되며, `diverged` 대상은 바이트 그대로 남고, 어떤 경우에도 `.codexwiring-*` 임시 파일이 남지 않는다.

음성: `diverged` 건을 복구가 덮어쓰면 FAIL. P6 건에서 provenance가 두 번 적용되어 부분 기록이 중복되면 FAIL.
...
**Then** 모든 행에서 사용자 소유 파일·부분이 바이트 그대로이고, 고아가 된 배선 파일과 더 이상 배포되지 않는 템플릿 관리 파일은 바이트 그대로 남은 채 보고되며, 배선 파일 보고에는 `moai tool disable codex`가 적힌다. 손상된 `hooks.json`은 건드리지 않는다.
...
### AC-DHR-008 — 폐기 경로 네 곳의 동등성 (REQ-DHR-010)

**Given** `moai codex -w`로 만든 트리 세 개(base에 없는 커밋 보유, 미커밋 변경 보유, 살아 있는 codex lock 보유)와 병합이 끝나고 lock이 없는 트리 하나, 그리고 같은 상태의 Claude 생성 트리 네 개,
**When** `moai worktree clean --stale --apply`, `moai worktree done`, `moai worktree remove`(`--force` 없이), 세션 종료 정리를 각각 적용하면,
**Then** clean·done·세션 종료 정리에서 앞의 세 Codex 트리는 남고 이유가 보고되며, 병합이 끝나고 anchor가 없는 트리만 제거 대상이 된다. `remove`는 lock된 Codex 트리를 거부하고 anchor를 보고한다. 모든 경로에서 Codex 트리의 결과가 같은 상태의 Claude 트리 결과와 같다.

```bash
mkdir -p .moai/reports/t1100 && unset MOAI_KANBAN MOAI_KANBAN_ID MOAI_KANBAN_LABEL MOAI_KANBAN_LEAD_ADDR MOAI_KANBAN_SETTINGS_INJECTED && go test -json ./internal/cli/worktree ./internal/cli -run '^(TestWorktreeDisposalRefusesUnintegratedCodexTree|TestSessionExitCleanupRefusesUnintegratedCodexTree)$' -count=1 -timeout=600s > .moai/reports/t1100/ac008.jsonl; jq -se '([.[]|select(.Action=="pass" and .Test=="TestWorktreeDisposalRefusesUnintegratedCodexTree" and ((.Package//"")|endswith("/internal/cli/worktree")))]|length)==1 and ([.[]|select(.Action=="pass" and .Test=="TestSessionExitCleanupRefusesUnintegratedCodexTree" and ((.Package//"")|endswith("/internal/cli")))]|length)==1 and ([.[]|select(.Action=="pass" and ((.Test//"")|test("^TestWorktreeDisposalRefusesUnintegratedCodexTree/(clean|done|remove)$")))]|length)==3 and ([.[]|select(.Action=="fail" or .Action=="skip")]|length)==0 and ([.[]|select((.Output//"")|test("NOT_RUN|ABORTED"))]|length)==0' .moai/reports/t1100/ac008.jsonl
```
...
**When** (i) 12개 역할마다 한 번씩 nonce를 되돌려 달라는 작업을 주고, 그중 `workspace-write` 역할 하나에는 탐침 파일 쓰기를 함께 시키며(양성 대조), (ii) 부모 lane 오케스트레이터 세션을 두 번 띄워 각각 `plan-auditor`와 `sync-auditor` 하위 에이전트가 탐침 파일 쓰기를 시도하게 하면,
**Then** 증거 줄 `AC012_EVIDENCE`에 codex 버전, 12개 역할 이름과 각 역할이 되돌린 nonce(로드의 양성 증거), 양성 대조 탐침 파일의 존재와 해시, 두 감사 역할의 쓰기 시도 출력(sandbox 거부 문구를 포함한 명령 출력)과 탐침 파일 부재, 호출 수가 기록되고, 호출 수가 14 이하이며, 출력에 `Ignoring malformed agent role definition`이 없다. MCP 경유 부작용은 이 AC가 증명하지 않으며 `UNSUPPORTED`로 남는다. `mission-governor`와 `super-advisor`의 read-only 강제는 이 AC가 측정하지 않는다.
...
```bash
mkdir -p .moai/reports/t1100 && unset MOAI_KANBAN MOAI_KANBAN_ID MOAI_KANBAN_LABEL MOAI_KANBAN_LEAD_ADDR MOAI_KANBAN_SETTINGS_INJECTED && MOAI_CODEX_ROLE_LIVE=1 go test -json ./internal/cli -run '^TestCodexRoleLiveLoadAndReadOnly$' -count=1 -timeout=1800s > .moai/reports/t1100/ac012-live.jsonl; jq -se '([.[]|select(.Action=="pass" and .Test=="TestCodexRoleLiveLoadAndReadOnly")]|length)==1 and ([.[]|select(.Action=="fail" or .Action=="skip")]|length)==0 and ([.[]|select((.Output//"")|test("NOT_RUN|ABORTED|Ignoring malformed agent role definition"))]|length)==0 and ([.[]|select((.Output//"")|test("^AC012_EVIDENCE "))|.Output|sub("^AC012_EVIDENCE ";"")|fromjson] as $e | ($e|length)==1 and ($e[0].codex_version|test("^[0-9]")) and ($e[0].invocations<=14) and (($e[0].roles|length)==12) and ([$e[0].roles[]|select(.nonce_returned==.nonce_sent)]|length)==12 and ($e[0].positive_control.exists==true) and (($e[0].write_attempts|length)==2) and ([$e[0].write_attempts[]|select(.attempt_output!="" and .denied==true and .probe_exists==false)]|length)==2)' .moai/reports/t1100/ac012-live.jsonl
```

### AC-DHR-013 — Codex 감사 역할 read-only 예외의 범위 (REQ-DHR-015)

**Given** 생성기 입력(`agents-codex.yaml`)과 neutral 원본 `.claude/agents/moai/plan-auditor.md`, `sync-auditor.md`(C2),
**When** agent 생성기를 실행하면,
**Then** 생성된 `plan-auditor.toml`과 `sync-auditor.toml`의 `sandbox_mode`가 `read-only`이고, 두 파일에만 Codex 전용 반환문 지시가 있으며, neutral 원본 두 파일과 Claude 쪽 에이전트 정의에는 그 지시가 없고, Codex가 읽는 부모 지시 표면에 "반환문 그대로 판정 파일을 기록한다"는 항목이 있다.
...
**When** 같은 결과가 (a) 두 번 전달되고, (b) 수신자가 결과 적용 후 receipt 전에 멈춰 lease 만료로 재전달되고, (c) 송신 worker가 결과를 보낸 뒤 새 세션 UUID로 재시작해 같은 멱등 키로 다시 보내고, (d) 브로커 프로세스가 결과 적용 직후 재시작한 뒤 재전달되고, (e) (c) 이후 이전 generation 메시지가 다시 전달되고, (f) 같은 attempt에 다른 본문의 결과가 오면,
...
변이: 판정 순서 5를 지운 변이는 (a)·(c)에서 두 번 적용하므로 FAIL해야 한다. 4와 5의 순서를 바꾼 변이는 (e)가 `duplicate`가 되어 FAIL해야 한다. 분기 A에서 범위를 송신자 세션 UUID로 되돌린 변이는 (c)에서 행이 2개가 되어 FAIL해야 한다.
...
### AC-DHR-015 — 낡은 fencing token, superseded 메시지, 같은 attempt 재부여 (REQ-DHR-018, REQ-DHR-020)
...
**Then** generation 1의 모든 시도는 `stale`로 거부되고 dispatch·메시지 행은 바뀌지 않으며, 브로커 status는 그 메시지를 `pending`이 아니라 `superseded`로 센다. 재부여 뒤 generation 2의 결과는 `accepted`이고 attempt는 그대로다. 브로커는 superseded 메시지 body를 generation 2에게 스스로 넘기지 않는다.
...
**Then** 각 조합의 증거 줄 `AC018_EVIDENCE`에 조합 이름, nonce, 적용된 결과 수 1, 늦은 결과의 판정 `stale`, 호출 수(8 이하), 경과 초(900 이하), 정리한 프로세스 pid 목록(비어 있지 않음)이 기록되고, 테스트가 pass한다.
...
```bash
mkdir -p .moai/reports/t1100 && unset MOAI_KANBAN MOAI_KANBAN_ID MOAI_KANBAN_LABEL MOAI_KANBAN_LEAD_ADDR MOAI_KANBAN_SETTINGS_INJECTED && MOAI_FACTORY_LIVE=1 MOAI_FACTORY_LIVE_CASE=cardflow-claude-claude go test -json ./internal/cli -run '^TestFactoryLiveCardFlowClaudeClaude$' -count=1 -timeout=1000s > .moai/reports/t1100/ac018-live-claude-claude.jsonl
```

```bash
unset MOAI_KANBAN MOAI_KANBAN_ID MOAI_KANBAN_LABEL MOAI_KANBAN_LEAD_ADDR MOAI_KANBAN_SETTINGS_INJECTED && MOAI_FACTORY_LIVE=1 MOAI_FACTORY_LIVE_CASE=cardflow-codex-codex go test -json ./internal/cli -run '^TestFactoryLiveCardFlowCodexCodex$' -count=1 -timeout=1000s > .moai/reports/t1100/ac018-live-codex-codex.jsonl
```

```bash
unset MOAI_KANBAN MOAI_KANBAN_ID MOAI_KANBAN_LABEL MOAI_KANBAN_LEAD_ADDR MOAI_KANBAN_SETTINGS_INJECTED && MOAI_FACTORY_LIVE=1 MOAI_FACTORY_LIVE_CASE=cardflow-claude-codex go test -json ./internal/cli -run '^TestFactoryLiveCardFlowClaudeCodex$' -count=1 -timeout=1000s > .moai/reports/t1100/ac018-live-claude-codex.jsonl
```

```bash
unset MOAI_KANBAN MOAI_KANBAN_ID MOAI_KANBAN_LABEL MOAI_KANBAN_LEAD_ADDR MOAI_KANBAN_SETTINGS_INJECTED && MOAI_FACTORY_LIVE=1 MOAI_FACTORY_LIVE_CASE=cardflow-codex-claude go test -json ./internal/cli -run '^TestFactoryLiveCardFlowCodexClaude$' -count=1 -timeout=1000s > .moai/reports/t1100/ac018-live-codex-claude.jsonl
```

판정(네 파일을 한 번에):

```bash
jq -se '([.[]|select(.Action=="pass" and ((.Test//"")|test("^TestFactoryLiveCardFlow(ClaudeClaude|CodexCodex|ClaudeCodex|CodexClaude)$")))]|length)==4 and ([.[]|select(.Action=="fail" or .Action=="skip")]|length)==0 and ([.[]|select((.Output//"")|test("NOT_RUN|ABORTED"))]|length)==0 and ([.[]|select((.Output//"")|test("^AC018_EVIDENCE "))|.Output|sub("^AC018_EVIDENCE ";"")|fromjson] as $e | ($e|length)==4 and ([$e[].case]|sort)==["claude-claude","claude-codex","codex-claude","codex-codex"] and ([$e[]|select((.nonce|length)>0 and .applied_results==1 and .late_result_outcome=="stale" and .invocations<=8 and .elapsed_seconds<=900 and (.cleaned_pids|length)>0)]|length)==4)' .moai/reports/t1100/ac018-live-claude-claude.jsonl .moai/reports/t1100/ac018-live-codex-codex.jsonl .moai/reports/t1100/ac018-live-claude-codex.jsonl .moai/reports/t1100/ac018-live-codex-claude.jsonl
```
...
**Then** 정상 흐름만 `true`이고 나머지 다섯은 `false`다.
...
run의 첫 마일스톤에서, factorymsg 스키마를 바꾸기 전 트리에서 한 번 실행하고 그 jsonl을 증거로 보존한다. 스키마를 바꾼 뒤 다시 실행한 결과로 이 측정을 대신하지 않는다.

**Given** 바뀌지 않은 브로커 저장소, 송신 lane slot 하나, 수신 lane 하나,
**When** 송신자가 멱등 키 K로 보내고, 같은 slot을 새 세션 UUID로 재등록해 generation이 오른 뒤 같은 요청을 K로 다시 보내고, 수신자가 claim하면,
**Then** 증거 줄 `IDEM_SCOPE_REPRO`에 결과(`reproduced` 또는 `not-reproduced`), 저장소가 연 unique 제약 문자열, K를 가진 메시지 행 수, 수신자가 claim한 서로 다른 메시지 ID 수, 송신자 generation 두 값이 기록되고, 결과가 수치와 맞는다(`reproduced` ⇔ 행 수 ≥ 2 그리고 claim한 ID 수 ≥ 2). 측정 시점의 HEAD는 `.moai/reports/t1100/ac020-head.txt`에 따로 남긴다.

```bash
mkdir -p .moai/reports/t1100 && git rev-parse --short HEAD > .moai/reports/t1100/ac020-head.txt && go test -json ./internal/factorymsg -run '^TestIdemScopeRestartReproduction$' -count=1 -timeout=120s > .moai/reports/t1100/ac020.jsonl; jq -se '([.[]|select(.Action=="pass" and .Test=="TestIdemScopeRestartReproduction")]|length)==1 and ([.[]|select(.Action=="fail" or .Action=="skip")]|length)==0 and ([.[]|select((.Output//"")|test("NOT_RUN|ABORTED"))]|length)==0 and ([.[]|select((.Output//"")|test("^IDEM_SCOPE_REPRO "))|.Output|sub("^IDEM_SCOPE_REPRO ";"")|fromjson] as $e | ($e|length)==1 and (($e[0].outcome=="reproduced") or ($e[0].outcome=="not-reproduced")) and (($e[0].unique_constraint|length)>0) and ($e[0].sender_generations[1] > $e[0].sender_generations[0]) and (($e[0].outcome=="reproduced") == ($e[0].rows>=2 and $e[0].claimed_ids>=2)))' .moai/reports/t1100/ac020.jsonl
```
...
**Then** 증거 줄 `AC023_EVIDENCE`의 두 항목마다 감사 역할의 쓰기 시도가 거부되었고(AC-DHR-012의 `write_attempts`와 같은 실행), 판정 파일이 있으며, 세션 기록에서 꺼낸 하위 에이전트 반환문의 sha256과 판정 파일의 sha256이 같고, 반환문에 그 실행의 nonce가 들어 있다.

세션 기록에 하위 에이전트 반환문이 남지 않으면 테스트는 `NOT_RUN`을 찍고, 이 AC는 `NOT_RUN`이다(반환문을 얻을 수 없으면 원문 일치를 판정하지 않는다).

```bash
jq -se '([.[]|select(.Action=="pass" and .Test=="TestCodexRoleLiveLoadAndReadOnly")]|length)==1 and ([.[]|select(.Action=="fail" or .Action=="skip")]|length)==0 and ([.[]|select((.Output//"")|test("NOT_RUN|ABORTED"))]|length)==0 and ([.[]|select((.Output//"")|test("^AC023_EVIDENCE "))|.Output|sub("^AC023_EVIDENCE ";"")|fromjson] as $e | ($e|length)==1 and (($e[0].audits|length)==2) and ([$e[0].audits[].role]|sort)==["plan-auditor","sync-auditor"] and ([$e[0].audits[]|select(.write_denied==true and .verdict_file_exists==true and (.returned_sha256|length)==64 and .returned_sha256==.verdict_file_sha256 and .returned_contains_nonce==true)]|length)==2)' .moai/reports/t1100/ac012-live.jsonl
```
