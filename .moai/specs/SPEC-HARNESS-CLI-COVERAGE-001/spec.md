...
| 파일:라인 | 함수 | 현재 커버리지 | 목표 |
|-----------|------|--------------:|------|
| `install.go:98` | `NewInstallCmd` | 33.3% | ≥90% (최대 단일 gap) |
| `install.go:60` | `RunInstall` | 80.0% | 100% |
| `execute.go:298` | `NewExecuteCmd` | 46.2% | ≥90% (os.Exit 분기 제외) |
...
| `countApplyOutcomeKept(t, usageLogPath)` | `execute_e2e_test.go:74` | usage-log.jsonl apply_outcome(kept) 라인 카운트 |
...
- **REQ-HCC-006**: `internal/cli/harness` 테스트 스위트는 `NewExecuteCmd`(execute.go:298) 함수
  커버리지를 **≥ 90%**로 달성한다(os.Exit 분기는 §D.4 documented residual로 제외).
...
- 함수별 목표 충족: `NewInstallCmd`/`runExecuteCommand`/`NewExecuteCmd`/`runPropose` ≥90%, `RunInstall` 100%
