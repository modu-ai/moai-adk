...
- **§E.3 `run_commit_sha` 백필이 자기 커밋이 아니라 `3720f06db`(sync 백필 커밋) 안에 들어갔다.** 그 슬롯은 run 단계 소관이고 본 세션의 편집 대상이 아니었으나, run 소유자가 같은 파일에 쓴 hunk 가 본 세션의 편집과 staging 사이에 워킹 트리에 도착해 같은 커밋에 실렸다. 경로 단위 확인으로는 갈라지지 않는다 — 두 세션이 **같은 파일**을 편집했으므로 경로 층위에서는 구별 자체가 불가능하고, 변경 목록은 양쪽 모두에서 `progress.md` 한 줄로 똑같이 보인다.
    - **그 커밋의 메시지는 「§E.3 의 placeholder 를 건드리지 않았다」고 적는데, 같은 커밋의 diff 가 그것을 반증한다.** 트레일러보다 이쪽이 더 날카로운 결함이다 — 나중에 누가 무엇을 했는지 복원하려는 독자가 신뢰하는 것이 커밋 메시지이기 때문이다. 커밋은 이미 착지해 불변이므로 **고치지 않고 여기 기록한다.**
    - 따라서 `Authored-By-Agent: manager-docs` 트레일러가 run 소유자의 hunk 를 manager-docs 에 잘못 귀속시킨다. **기계적 파급은 없다**: 그 커밋은 `spec.md` 를 한 바이트도 건드리지 않아 `OwnershipTransitionRule` 이 읽을 상태 전이가 존재하지 않고, 스키마의 D3 예외는 SHA 백필이 애초에 소유권 침범이 아니라고 단정한다.
    - **내용 자체는 옳다** — 리드가 HEAD 에서 두 슬롯을 직접 읽어 확인했다(`run_commit_sha: f47a88d02` 410행, `sync_commit_sha: 5eb3e493f` 471행).
    - **원인은 리드가 같은 턴에 같은 파일·같은 트리로 쓰기 주체 둘을 배차한 것이다**(트리당 쓰기 주체 하나 규율). 레인의 staging 층위에서 잡을 수 있는 지점이 아니었다.
