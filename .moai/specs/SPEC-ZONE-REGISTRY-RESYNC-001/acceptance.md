...
| AC-ZRR-002 | REQ-ZRR-001, 003, 005 | 로컬 **1회 적중 24** / 0회 76 / 2회 이상 1 / 자기참조 0 | 97 (비은퇴) / 0 / 0 / 0 / 은퇴 면제 4 |
| AC-ZRR-003 | REQ-ZRR-001, 003, 013 | 템플릿 **1회 적중 24** / 2회 이상 1 (부가 조건 동일) | 97 (비은퇴) / 0 (+은퇴 면제 4) |
...
**RED (`294b4b6ab`)**: 1회 적중 `24`, 0회 `76`, 2회 이상 `1`(`CONST-V3R2-002` = `TRUST 5`, 3회), 자기참조 `file:` `0`. **GREEN 은 1회 적중 97 (비은퇴) / 0회 0 / 2회 이상 0 / 자기참조 0 / 은퇴 clause 면제 4.** 집계 기준은 v0.5.0 C안(`spec.md` §1.2)으로 바뀌었다 — `[SUPERSEDED …]` 은퇴 마커를 단 4건(`CONST-V3R2-021..024`)은 clause 가 불변 감사 기록이라 verbatim 요구에서 면제되어 다섯째 버킷 `은퇴 clause 면제`로 보고하며, RED 값은 `294b4b6ab` 의 역사적 측정치로 그대로 둔다.
...
**GREEN 은 AC-ZRR-002 와 같은 기준으로 1회 적중 97 (비은퇴) / 0회 0 / 2회 이상 0 / 자기참조 0 / 은퇴 clause 면제 4** — 집계 기준 변경은 v0.5.0 C안(`spec.md` §1.2)을 따르고, RED 값은 `294b4b6ab` 의 역사적 측정치로 그대로 둔다.
...
- **And** 은퇴 4건(`CONST-V3R2-021..024`)의 `anchor:` 도 이 101건에 **포함된다**(v0.5.0 C안 — `spec.md` §1.2). 이들의 anchor 는 사라진 절이 아니라 후계 절(`CLAUDE.md` §14)을 가리키며, anchor 검사가 `[SUPERSEDED …]` 마커의 안내 포인터가 계속 해석 가능함을 검증한다 — 그래서 이 AC 의 101/101 은 은퇴 엔트리를 제외하지 않는다.
