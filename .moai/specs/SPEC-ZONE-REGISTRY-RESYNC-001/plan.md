...
| **AC-007 (부분 순회)** | 가드에 제외 목록·조기 반환을 넣어 일부만 검사한다. 변이가 제외 밖 엔트리에 떨어지면 가드는 정상적으로 붉어져 통과 | 007 의 "평가 엔트리 수 = 101(두 미러 각각)" 단언 + 변이 대상 고정(명시 ID 1 + 무작위 1) |
...
| AC-ZRR-005 (매처 불변) | 매처는 그대로 두고 **가드 쪽**에 제외 목록을 넣는다 | 007 의 평가 엔트리 수 단언(101×2) — 변이 위치에 의존하지 않는 유일한 방어. 변이 주입만으로는 제외 밖에 떨어질 때 통과한다(위 합동 행) |
