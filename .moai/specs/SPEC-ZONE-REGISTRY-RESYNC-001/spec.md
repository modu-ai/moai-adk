...
| 0.5.0 | 2026-08-25 | manager-spec | §1.2 충돌 해소 — 사용자가 C안 확정(은퇴 엔트리는 clause 면제·anchor 유지; 측정 근거 명시, 트리 9ba1e308d — progress.md §F 계보). REQ-ZRR-001 live 한정 / REQ-ZRR-002 전체 명시 / REQ-ZRR-007 은퇴 anchor-only 명시, AC-ZRR-002/003 기대치 97 clause + 101 anchor로 갱신, plan M1 종료조건 동기화(97/97 비은퇴) |
...
- **REQ-ZRR-002** (Ubiquitous) — The zone registry shall carry, for every entry **including retired entries**, an `anchor:` value that resolves to a heading present in the file named by that entry's `file:` (retired entries keep the anchor check; see §1.2).
...
- **REQ-ZRR-007** (Event-driven) — **When** an edit to the rules tree or to the registry breaks any entry's clause or anchor, the guard shall fail the pull request containing that edit. For retired entries, only anchor breaks apply — their clause is exempt per §1.2 (option C).
