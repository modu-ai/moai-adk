...
priority: Medium
phase: "Phase 6b - Tier 2 Polish"
module: "internal/output/, .claude/output-styles/, outputStyles loader"
dependencies:
  - SPEC-V3-SCH-001
related_gap: [gm#165, gm#166, gm#167, gm#168, gm#169, gm#175, gm#176, gm#177, gm#178, gm#179, gm#180, gm#181, gm#182]
...
핵심 기준:
- `.claude/output-styles/moai-default.md` 파일이 CC 호환 frontmatter를 갖추고 파싱 성공한다.
- `internal/output/Renderer.Diff()`가 `diff --git` 형식으로 Git-compatible 출력을 생성한다 (unit test).
- `internal/output/Renderer.ValidationError()`가 REQ-OUT-001-006 스키마 준수 YAML/JSON lines를 생성한다.
- `moai doctor output-style`이 잘못된 frontmatter를 탐지하고 exit code 1로 종료한다.
- TTY 환경에서 StatusIcon이 ANSI colored, non-TTY에서는 plain glyph만 출력된다.
- `NO_COLOR=1` 환경에서 모든 ANSI escape이 생략된다.
- 기존 MoAI output-style 기능(SPEC-First DDD, TRUST 5 reference)이 변환 후에도 보존된다.
- CI 환경(CI=true)에서 Progress 라인이 JSON lines 형식으로 emit된다.
...
- 본 SPEC의 모든 요구사항 ID는 `plan.md` 마일스톤(Wave 5 작성)과 `acceptance.md` 시나리오로 역참조된다.
