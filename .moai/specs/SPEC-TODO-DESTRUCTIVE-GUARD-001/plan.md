...
Add `Archived []BacklogArchiveEntry` as a **single** additive top-level `BacklogRecord` field, following the `findings` precedent — always rendered as an array, never null, never omitted, absent in older files. Each entry carries the card row, the **position** it held in `Items`, and the findings that named it, each with the position it held in `Findings` (`BacklogArchivedFinding`). Add the two `IF NOT EXISTS` tables to `backlogDDL` — `archived_items` and `archived_findings`, both carrying `position` — and their load/save mapping. **Do not touch `backlogSchemaVersion`.**

> **Shape correction (post-implementation, lead-approved).** This milestone was drafted as two parallel top-level fields, `Archived []BacklogItem` plus `ArchivedFindings []BacklogFinding`, with no positions. That sketch does not satisfy REQ-TDG-002: `done` splices a row out of the *middle* of the queue, so an `undone` that appends returns the same SET of cards in a different ORDER — a record that reads correctly and is not the record the operator had. Carrying the index per row closes that class for one integer, and nesting each card's findings inside its own entry keeps "restore the card and everything recorded about it" one move instead of two arrays that must be kept in step. Landed shape: `internal/kanban/backlog_store.go` — `BacklogArchiveEntry` / `BacklogArchivedFinding`, `ArchiveCard` / `RestoreCard`. Every invariant this milestone was written to protect is intact: an additive top-level field, new `IF NOT EXISTS` tables, `backlogSchemaVersion` still `"1"`, the 5-field `BacklogItem` contract and the 3-value state enum untouched. `spec.md` §B.1's additive-vs-fourth-state ruling is unchanged.
