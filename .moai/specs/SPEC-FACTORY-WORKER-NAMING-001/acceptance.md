...
- **AC-005** Given `internal/hook/session_start_factory_i18n.go`, When `grep -c 'lane-'` is run on it post-M3, Then the count is 0, and each of the four locales (en/ko/ja/zh) carries the worker-axis wording in lockstep (grep for `worker-1..worker-` returns one hit per locale block).
