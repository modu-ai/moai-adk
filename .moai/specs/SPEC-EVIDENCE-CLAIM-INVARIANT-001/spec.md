...
| AC-ECI-003 | "no unobserved-verification-claim" invariant 명문 + 두 결속 표면(오케스트레이터 self-report + manager-agent §E) 모두 명명 | `grep` keyword: `unobserved`, `Evidence absent`, `orchestrator self-report`(또는 동치), `manager`+`§E`(또는 `E1`..`E7`) |
...
| AC-ECI-006 | template mirror 중립성 — 내부 누출 0 | `grep -E 'SPEC-[A-Z]\|feedback_\|/Users/\|CLAUDE.local\|[0-9a-f]{7,40} commit' <mirror>` 결과 0 |
| AC-ECI-007 | 기존 규칙으로의 cross-reference ≥4개 (내용 복제 없음) | `grep -c` cross-ref 경로: `agent-common-protocol.md`, `moai-constitution.md`, `manager-develop-prompt-template.md`, `verification-batch-pattern.md`, `output-styles/moai/moai.md` 중 ≥4 |
