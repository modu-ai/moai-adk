...
| P16 | Card → SPEC link in the queue store (resolver layer (a)) | **Yes** | `internal/kanban/backlog_store.go:81` `SpecID *string` with JSON tag `spec_id` on each queue item; absent spec id is JSON null (`:69`) | The resolver reads the card's `spec_id` read-only; a card without one resolves not-armed (REQ-AE-002). |
