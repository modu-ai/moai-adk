...
| P3 | Constitution zone registry | **Yes (doc + loader)** | `.claude/rules/moai/core/zone-registry.md:33-36` (`zone_class` enum, Frozen → `canary_gate`); runtime loader `internal/constitution/loader.go:78` `LoadRegistry`, zone filter `:47` `FilterByZone` | A1 derives `frozen_files` from the registry Frozen list supplied through this loader (A1 `design.md` § Frozen Files item 1, line 200 at `65e0a9167`). REQ-AE-007 consumes that derived list, obtained once at arming (design.md §C.6); the detector does not read the registry itself. |
...
| P15 | A1 contract schema | **On branch `WT-contract-schema`, not on `develop`** | SPEC-AUTONOMY-CONTRACT-001 v0.5.1 `design.md` and `spec.md` at commit `65e0a9167`, read with `git show 65e0a9167:<path>`; derived fields `design.md:183-187`; `card` field `design.md:22`, `:124`, § Card Field `:148-163`; store path `design.md:89`, `spec.md:94-96`; verify-from-PreToolUse `spec.md:480`; projection owner A2b at `spec.md:78` and `design.md:469`, with two stale A2 sentences at `spec.md:178` and `design.md:241` | Every contract-reading requirement keeps the tag 「A1 plan-audit 통과본으로 재확인」; R8, R9, and R10 closed in spec.md §F.2; the two stale sentences are an A1-lane item. |

## §C — Gaps in this measurement

- The A1 pin `65e0a9167` (v0.5.1) was read with `git show` from this worktree, together with the
  earlier pins `67a2f55cb` and `6d98ca466`; the line numbers cited above are from `65e0a9167`.
  The §F.1 row check against `65e0a9167` follows plan-audit iteration 4's table
  (`.moai/reports/t1235/plan-audit-iter4.md`) and this run's greps for the `card`, store-path,
  derived-field, and PreToolUse lines; every other row was not re-read line by line here.
