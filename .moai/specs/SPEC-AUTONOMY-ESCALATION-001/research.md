...
| P3 | Constitution zone registry | **Yes (doc + loader)** | `.claude/rules/moai/core/zone-registry.md:33-36` (`zone_class` enum, Frozen → `canary_gate`); runtime loader `internal/constitution/loader.go:78` `LoadRegistry`, zone filter `:47` `FilterByZone` | A1 derives `frozen_files` from the registry Frozen list supplied through this loader (A1 `design.md` § Frozen Files item 1, line 200 at `25283ebf8`). REQ-AE-007 consumes that derived list, obtained once at arming (design.md §C.6); the detector does not read the registry itself. |
...
| P17 | Home-layout store root (for the contract store) | **Yes** | `internal/kanban/state_dir.go:26-40` `StateDirForRoot`: `$MOAI_HOME` when absolute, else `~/.moai`, then `db/<homestate.ProjectKey(root)>/todo`; `MOAI_HOME` constant `internal/config/envkeys.go:22` | The contract store `$MOAI_HOME/db/<project-key>/contract/` resolves its root and key the same way (lead ruling 09-26 (4) #2); the detector reuses that resolution rather than re-deriving it. |
| P15 | A1 contract schema | **On branch `WT-contract-schema`, not on `develop`** | SPEC-AUTONOMY-CONTRACT-001 v0.5.2 `design.md` and `spec.md` at commit `25283ebf8`, read with `git show 25283ebf8:<path>`; derived fields `design.md:183-187`; `card` field `design.md:22`, `:124`, § Card Field `:146-163`; store path `design.md:89`, `spec.md:95-97`; verify-from-PreToolUse `spec.md:484`; projection owner A2b at `spec.md:79`, `:179`, `design.md:241`, `:470` | Every contract-reading requirement keeps the tag 「A1 plan-audit 통과본으로 재확인」; R8, R9, and R10 closed in spec.md §F.2. |

## §C — Gaps in this measurement

- The A1 pin `25283ebf8` (v0.5.2) was read with `git show` from this worktree; the line numbers
  cited above are from it. Its difference from `65e0a9167` (v0.5.1) was read with `diff`: wording
  only — the interim Jev rule and the Kickoff notice stated conditionally, the provenance comment
  at `design.md:89`, and the remaining A2 → A2b attributions (`spec.md:179`, `design.md:227`,
  `:241`, `:431`, `:492`); no §F.1 field changed. The §F.1 row check at `65e0a9167` follows
  plan-audit iteration 4's table (`.moai/reports/t1235/plan-audit-iter4.md`); at `25283ebf8` this
  run re-read the `card`, store-path, derived-field, Frozen Files, PreToolUse, and projection lines
  by grep; every other row rests on that diff showing no change.
