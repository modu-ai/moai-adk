...
renumbering is recorded in the commits that made it (`8c9ee29b7`, `fe118c4d1`). v0.4.3 changes no
numbers: AC-AE-018 gains case (d) tail truncation, AC-AE-019 a concurrent same-card clause, and
AC-AE-020 case (d) empty log (plan-audit iteration 5 R1, R2).
...
- **AC-AE-017** (maps REQ-AE-017) — **Given** card `t9001` armed as in AC-AE-016, **When** in three separate fixtures (a) the SPEC's `status` becomes `completed` and a checkpoint runs, (b) the contract's `card` field is changed to `t9002` (re-signed) and a Write follows, and (c) a second in-progress SPEC's contract gains `card: t9001` and a Write follows, **Then** each writes exactly one `detection-disarmed` record with `contract_ref` `disarm:terminal-status`, `disarm:card-mismatch`, and `disarm:card-mismatch` respectively, each followed by one `disarmed` entry in `<card log>`; and **When** in fixture (a) the contract file is then also deleted and a further Write follows, **Then** no new record and no new `disarmed` entry is written and that record's `occurrences` becomes 2. 「A1 plan-audit 통과본으로 재확인」
  ...
  - RED-now: L2. Green path: M5.
- **AC-AE-018** (maps REQ-AE-017) — **Given** card `t9001` armed, `<card log>` for `t9001` showing it armed with a valid chain, **When** in four separate fixtures (a) the `armed` value is deleted from `<card state>` by a Bash command and a Write follows, (b) `<card state>` bytes are rewritten by a Bash command without a matching `state` entry and a Write follows, (c) one line in the middle of `<card log>` is altered, and (d) the last `armed` entry and every line after it are cut from the tail of `<card log>` while `<card state>` keeps its `armed` value, and a Write follows, **Then** each writes exactly one `detection-disarmed` record with `contract_ref: disarm:state-tamper` and one `disarmed` entry, in (a) no path reads the card as disarmed before that record is written — the mutant control for a trigger read from the state file, which would write no record at all — and in (d) the tail-truncated log, whose remaining chain is valid and which no longer shows the card armed, is still judged `state-tamper` from the unaccounted `armed` value and the state-file digest comparison. 「A1 plan-audit 통과본으로 재확인」
  - Test: `TestStateTamperJudgedFromCardLog` in `internal/escalation`.
  - Case (d) closes plan-audit iteration 5 R2 (tail truncation) — derived from the design text; first observed in run milestone M5.
  - RED-now: L2. Green path: M5.
- **AC-AE-019** (maps REQ-AE-017) — **Given** cards `t9001` and `t9002` both armed, each with its own `<card log>` and a valid chain, **When** hooks for `t9002` append fifty entries to `t9002`'s log — arming, state, not-armed, and one full disarm-and-rearm cycle — interleaved with hooks for `t9001`, **Then** `t9001`'s log chain verifies, `t9001` writes no `detection-disarmed` record and no `state-tamper` of any kind, `t9001`'s log contains no entry written on behalf of `t9002`, and the two log files have the names `t9001.log.jsonl` and `t9002.log.jsonl` exactly; and **When** two PostToolUse hooks for `t9001` run concurrently, each counting one operation, **Then** `t9001`'s chain verifies, no `detection-disarmed` record and no `state-tamper` is written, the operations counter in `<card state>` rises by exactly 2, and `<card state>` matches the latest `state` entry. 「A1 plan-audit 통과본으로 재확인」
  - Tests: `TestOtherCardLogDoesNotAffectThisCard` and `TestSameCardConcurrentHooksKeepChain` in `internal/escalation`.
  - The concurrent clause closes plan-audit iteration 5 R1 (same-card append race) — derived from the design text; first observed in run milestone M5.
  - RED-now: L2. Green path: M5.
- **AC-AE-020** (maps REQ-AE-017) — **Given** a worktree `t9001` whose `<card log>` does not show the card armed, **When** in four separate fixtures a Write is processed with (a) the log absent, no `<card state>`, and no escalation record for `t9001`, (b) the log absent and a `<card state>` carrying an `armed` value, (c) the log absent, no `<card state>`, but a `detection-disarmed` record under `.moai/reports/t9001/escalation/`, and (d) the log present but empty (zero bytes) and a `<card state>` carrying an `armed` value, **Then** (a) writes no `detection-disarmed` record and proceeds as a never-armed card (resolution and a `not-armed` or arming entry in a new log), while (b), (c), and (d) each write exactly one `detection-disarmed` record with `contract_ref: disarm:state-tamper` and leave a `<card log>` whose first entry after the tamper is `disarmed`. 「A1 plan-audit 통과본으로 재확인」
  - Test: `TestMissingCardLogReading` in `internal/escalation`.
  - Case (d) closes plan-audit iteration 5 R2 (emptied log) — derived from the design text; first observed in run milestone M5.
  - RED-now: L2. Green path: M5.
