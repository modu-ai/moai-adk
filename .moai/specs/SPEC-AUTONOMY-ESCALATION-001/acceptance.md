...
- **AC-AE-003** (maps REQ-AE-002) — **Given** mode `contract` and a SPEC with no `contract.yaml`, **When** a checkpoint fires, **Then** no escalation report is written and the audit log gains exactly one `not-armed` line naming the SPEC. 「A1 스키마 확정 후 재조정」
...
- **AC-AE-006** (maps REQ-AE-005) — **Given** a contract recording the sha256 and AC count of `acceptance.md`, **When** one byte of `acceptance.md` changes and a checkpoint runs, **Then** one `acceptance-change` report is written, and with the file unchanged no report is written. 「A1 스키마 확정 후 재조정」
  - RED-now: L2. Green path: M2.
- **AC-AE-007** (maps REQ-AE-005) — **Given** an `acceptance.md` whose AC count differs from the contract's `ac_count`, **When** a checkpoint runs, **Then** the report names both the recorded and the observed count. 「A1 스키마 확정 후 재조정」
  - RED-now: L2. Green path: M2.
- **AC-AE-008** (maps REQ-AE-006) — **Given** a contract invariant `go test ./internal/spec/...`, **When** a Bash call with that exact command exits 1, **Then** an `invariant-violation` (`command`) report is written, while the same command exiting 0 and a different command exiting 1 each write none. 「A1 스키마 확정 후 재조정」
  - RED-now: L2. Green path: M3.
- **AC-AE-009** (maps REQ-AE-007) — **Given** a caller identity other than the harness learner, **When** a Write targets `CLAUDE.md`, **Then** an `invariant-violation` (`frozen-file`) report is written and the existing harness-learner deny tests in `internal/hook` still pass unchanged.
...
- **AC-AE-014** (maps REQ-AE-010, REQ-AE-019) — **Given** recorded inputs (a) an `audit_multi` result with `disagreement_flag: true`, (b) a first verdict PASS and a second review FAIL, (c) a local verification pass and a recorded CI failure on the same head, **When** a checkpoint runs, **Then** each writes a `contradictory-evidence` report, while an `audit_multi` result with `disagreement_flag: null` writes no report and lists the flag under not-observed.
  - RED-now: L2. Green path: M5.
- **AC-AE-015** (maps REQ-AE-011) — **Given** a contract whose `actions` permits push to `develop` only, **When** Bash calls `git push origin main`, `git push origin v1.0.0`, `git tag v1.0.0`, and `gh release create v1.0.0` are processed at PreToolUse, **Then** four `irreversible-action` reports are written before execution, while `git push origin develop` writes none. 「A1 스키마 확정 후 재조정」
  - RED-now: L2; `Bash(git tag:*)` sits in the settings allow block (research.md P8), so nothing observes it today. Green path: M5.

## §D — Operational trips

- **AC-AE-016** (maps REQ-AE-012) — **Given** a budget of `operations: 3`, **When** a fourth write tool call is observed, **Then** a `budget-exceeded` report names `operations` with observed 4 and limit 3, and with no contract budget the `budget_default` value applies. 「A1 스키마 확정 후 재조정」
  - RED-now: L2. Green path: M5.
...
- **AC-AE-021** (maps REQ-AE-017) — **Given** the same observation tripping twice, **When** both are processed, **Then** exactly one report exists and its occurrence count is 2.
  - RED-now: L2. Green path: M2.
- **AC-AE-022** (maps REQ-AE-020) — **Given** a contract whose `escalate_on` omits `ownership-move`, **When** an out-of-scope Write and an over-budget operation occur, **Then** no `ownership-move` report is written, the audit log records the class as disabled by the contract, and a `budget-exceeded` report is still written. 「A1 스키마 확정 후 재조정」
  - RED-now: L2. Green path: M5.
...
- Every criterion tagged 「A1 스키마 확정 후 재조정」 is re-read against the landed A1 schema before M2.
