...
per-test temporary directory, so no fixture touches the operator's real store.
...
fifth acceptance reason and the before-signing case, 010 gains a contract-store write
(v0.4.1), 018 renamed from `contract-void` to `detection-disarmed`); new 019 (terminal status, card
mismatch, state tamper, second disarm reason); 019 → 020; 020 → 021; 021 → 022; 022 → 023;
023 → 024 (rewritten: first observation verified at PreToolUse). v0.4.1 changes no numbers:
AC-AE-013 loses its receipt case, AC-AE-010, AC-AE-018, and AC-AE-019 name the contract store,
and AC-AE-022 no longer assumes a decider value. Earlier renumbering
...
- **AC-AE-009** (maps REQ-AE-008, REQ-AE-012) — **Given** a signed contract with `ownership.write: [internal/foo/**, .moai/specs/<ID>/**]` and `ownership.never: [internal/foo/secret/**]`, **When** Writes target `internal/bar/x.go`, `internal/foo/secret/y.go`, `.moai/specs/<ID>/contract.yaml`, and `.moai/specs/<ID>/acceptance.md`, **Then** four `ownership-move` records are written, the second naming the `never` line and the last two naming the post-signing immutability from `effective_never`, while a Write to `internal/foo/z.go` writes none. 「A1 plan-audit 통과본으로 재확인」
  ...
  - RED-now: L2. Green path: M3.
- **AC-AE-010** (maps REQ-AE-013, REQ-AE-022) — **Given** the AC-AE-009 contract plus `ownership.scratch: [tmp-build/**]`, a worktree directory named `t9001`, and every exemption root determined, **When** Writes target `.moai/reports/t9001/verdict.md`, `.moai/state/x.json`, a file under the OS temporary directory, a file under the session scratchpad, a file under the auto-memory store, `tmp-build/out.txt`, `<contract store>/escalation/t9001.json`, and an absolute path outside the worktree root covered by none of these, **Then** the first six write no record and the last two each write one `ownership-move` record, the contract-store write being judged as an outside-root path that no `ownership` glob matches; and with the scratchpad root undeterminable, the same outside-root write writes no record and is listed under `not_observed`. 「A1 plan-audit 통과본으로 재확인」
...
- **AC-AE-017** (maps REQ-AE-016) — **Given** a contract with `budget.audit_retries: 0` and one plan-audit verdict file recording FAIL, and a contract with `budget.audit_retries: 2` and sync-audit verdict files recording FAIL at iterations 1 and then 3, **When** a checkpoint runs after each file, **Then** the first case writes one `audit-fail-at-retry-cap` record, and the second writes none after iteration 1 and one after iteration 3. 「A1 plan-audit 통과본으로 재확인」
  ...
  - RED-now: L2. Green path: M5.
- **AC-AE-018** (maps REQ-AE-017) — **Given** card `t9001` armed against a contract recorded in `<contract store>/escalation/t9001.json`, **When** in three separate fixtures (a) the contract's `signature` block is removed and a PreToolUse Write follows, (b) the contract file is deleted and a PreToolUse Write follows, and (c) a byte outside `signature` changes so verify reports `contract_digest_mismatch` and a commit checkpoint runs, **Then** each fixture writes exactly one `detection-disarmed` record whose `contract_ref` is `disarm:signature-invalid`, `disarm:contract-absent`, and `disarm:signature-invalid` respectively, that record is written before any `not-armed` line, the detector audit log in the contract store records the disarming, a later out-of-scope Write writes no `ownership-move` record, and an over-budget operation still writes a `budget-exceeded` record. 「A1 plan-audit 통과본으로 재확인」
  - Test: `TestDisarmContractLossWritesOneRecord` in `internal/escalation`.
  - RED-now: L2. Green path: M5.
- **AC-AE-019** (maps REQ-AE-017) — **Given** card `t9001` armed as in AC-AE-018, **When** in four separate fixtures (a) the SPEC's `status` becomes `completed` and a checkpoint runs, (b) the contract's `card` field is changed to `t9002` (re-signed) and a Write follows, (c) a second in-progress SPEC's contract gains `card: t9001` and a Write follows, and (d) the card state file in the contract store is rewritten by a Bash command and a Write follows, **Then** each writes exactly one `detection-disarmed` record with `contract_ref` `disarm:terminal-status`, `disarm:card-mismatch`, `disarm:card-mismatch`, and `disarm:state-tamper` respectively; and **When** in fixture (a) the contract file is then also deleted and a further Write follows, **Then** no new record is written and that record's `occurrences` becomes 2. 「A1 plan-audit 통과본으로 재확인」
...
- Every row of spec.md §F.1 and every criterion tagged 「A1 plan-audit 통과본으로 재확인」 is re-checked against the A1 final (`67a2f55cb`, or its successor if A1 lands later) before M2; requests R8 and R9 are either satisfied or recorded as open in progress.md §E.2 (R10 is closed).
