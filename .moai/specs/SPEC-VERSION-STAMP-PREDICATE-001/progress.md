...
M0 수치와 상수는 최종 트리에서 재확인: 스윕 **27** · 등록부 28 · stamp 7 — 병합 트리 재측정(R-7)은
통합 창의 몫이며 기대값은 27로 고정.

> [정정 2026-09-01, sync-audit D1] 이 블록의 초판은 스윕을 「34(불변)」로 적었다. 34는
> `051f209b0` 핀 트리의 값이고, M4(golden 6건, 96bfa0c99)와 M5(문서 토큰 행 1건, 8c71cd423)를
> 거치면 최종 트리의 실측은 **27**이다 — `8c71cd423`과 `e9704b73c` 양쪽에서 감사자와 lane이
> 각각 측정해 일치했다. 검사는 스윕 개수 상수를 의도적으로 갖지 않으므로 코드 영향은 없고,
> CHANGELOG 항목은 34를 `051f209b0` 기준으로 정확히 귀속하고 있다.
