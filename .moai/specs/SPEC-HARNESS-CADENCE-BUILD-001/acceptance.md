...
| AC-HCB-034 | HCB-034 | MUST | Boundary grep: `grep -rn 'AskUserQuestion\|mcp__askuser' internal/cli/harness/ \| grep -v _test.go \| grep -v '^[^:]*:[0-9]*:[ \t]*//'` → 0 matches; invocation-surface grep: `grep -rn 'mcp__' internal/cli/harness/ \| grep -v _test.go` → 0 matches (the CLI never CALLS Cron tools). Scope: binds invocation surfaces only — user-facing notice STRING literals naming a mechanism token (e.g. `CronDelete` in the remove notice, REQUIRED by AC-HCB-032) are exempt: they are emitted text, not tool calls |
...
| AC-HCB-051 | HCB-051 | MUST | Bounded window: `awk '/^#+ /{f=($0~/Schedule Retrofit/)} f' harness-build-entry.md` → window names `moai harness edit` as the path-discovery surface; ordering asserted mechanically: `grep -n` first-match line numbers within the window satisfy recurrence-token < manifest-edit-token (`moai harness edit`) < registration-token (strictly increasing); both trees |
