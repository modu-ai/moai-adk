...
- **A-06**: Python 전작의 `.moai/` 디렉터리 구조와 하위 호환성을 유지해야 한다.
...
시스템은 항상 배포 전 모든 파일 경로에 `filepath.Clean()`을 적용하고, 프로젝트 루트 디렉터리 내부로의 containment check를 수행해야 한다.

**[REQ-T-005] 경로 순회 방지 (Unwanted)**

시스템은 `../` 또는 절대 경로를 통한 프로젝트 루트 외부 경로 접근을 허용하지 않아야 한다.

**[REQ-T-006] 디렉터리 자동 생성 (Event-Driven)**

WHEN 배포 대상 경로의 상위 디렉터리가 존재하지 않을 경우 THEN 필요한 디렉터리를 자동으로 생성해야 한다.
