...
```gherkin
Given .moai/config/sections/ 디렉터리에 유효한 YAML section 파일들이 존재하고
...
```gherkin
Given .moai/config/sections/ 디렉터리에 user.yaml만 존재하고
...
Given 프로젝트 루트에 config.json 파일이 존재하고
  ...
  And .moai/config/sections/ 디렉터리가 존재하지 않을 때
...
Given 프로젝트 루트에 config.json이 존재하지 않고
  And .moai/config/sections/ 디렉터리에 YAML 파일들이 정상 존재할 때
When DetectLegacy(projectRoot)가 호출되면
...
### Scenario 16: 빈 디렉터리에서 로드

`.moai/config/sections/` 디렉터리가 비어 있을 때 전체 default 값으로 초기화되는지 검증한다.

```gherkin
Given .moai/config/sections/ 디렉터리가 존재하지만 YAML 파일이 하나도 없을 때
When ConfigManager.Load(projectRoot)가 호출되면
Then error는 nil이어야 하고
  And 반환된 Config는 NewDefaultConfig()와 동일해야 한다
```

### Scenario 17: 디렉터리 자체가 없는 경우

`.moai/config/sections/` 디렉터리가 존재하지 않을 때의 동작을 검증한다.

```gherkin
Given 프로젝트 루트에 .moai/config/sections/ 디렉터리가 존재하지 않을 때
When ConfigManager.Load(projectRoot)가 호출되면
Then error는 nil이어야 하고
  And 반환된 Config는 NewDefaultConfig()와 동일해야 한다
```
...
```gherkin
Given 현행 Python MoAI-ADK 프로젝트의 .moai/config/sections/ 디렉터리를 test fixture로 복사했을 때
