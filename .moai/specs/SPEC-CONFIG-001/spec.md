...
| 환경 변수 | 대상 설정 | 기본값 |
|-----------|-----------|--------|
| `MOAI_CONFIG_DIR` | `.moai/` 디렉터리 위치 | 프로젝트 루트의 `.moai/` |
...
- A1: 프로젝트 루트 디렉터리에 `.moai/config/sections/` 디렉터리가 존재한다고 가정한다. 존재하지 않을 경우 default 값으로 초기화한다.
...
시스템은 **항상** 프로젝트 루트의 `.moai/config/sections/` 디렉터리에서 YAML section 파일들을 읽어 typed Go struct로 deserialize해야 한다.
