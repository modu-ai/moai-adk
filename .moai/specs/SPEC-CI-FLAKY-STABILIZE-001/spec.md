...
| 0.4.0 | 2026-07-25 | manager-spec | sync-audit F1(가드 깊이, Medium) 반영. REQ-CFS-004에 **순회 깊이 요구**를 가산 — `warmUpDone`을 **유지한 채** 방문 노드 수 신호 `warmUpVisited`를 추가하여, "호출은 유지하고 본문의 재귀 루프만 무력화하는" 변형까지 결정론적으로 봉쇄한다. 비교 대상 노드 수는 **`TestMain`에서 warm-up 직후·`m.Run()` 이전에** 별개 구현으로 산출해 보관하며 가드 테스트는 비교만 한다 — 가드 시점 재계산은 전역 트리가 실행 도중 6 노드 자라(cobra `help`/`completion` 자동 등록) 올바른 구현에서도 `182` vs `188` 오탐 FAIL을 만들기 때문이다(구현 중 실측). 별개 구현 · warm-up 직후 직렬 실행 두 불변은 그대로 보존된다. `acceptance.md`에 `AC-CFS-004c`(본문 무력화 왕복 검증) 신설 + §D.3b 근거 기록 + §G 종료 게이트 1항 추가. §H 잔여 위험 2의 방어 경계를 *호출 제거 한정* → *호출 제거 + 본문 무력화*로 확장하고 "가드 본문 재계산 복귀" 리팩터 위험을 3번째 한계로 추가. 신호를 교체하지 않고 추가했으므로 기존 AC-CFS-003/004/004b/005 판정은 그대로 유효하다. 결함 2는 변경 없음 |
...
**추가 요구 (0.4.0 — 순회 깊이 신호)**
**When** 가드 테스트가 실행되면, 가드는 warm-up이 **실행되었음**(`warmUpDone`)과 warm-up이 **트리 전체를 순회했음**(방문 노드 수 `warmUpVisited`가 warm-up 시점에 산출된 트리 노드 수와 일치함)을 **모두** 어서션해야 한다(shall). `warmUpVisited`는 warm-up 재귀가 노드를 방문할 때마다 증가하는 테스트 스코프 카운터이며, `warmUpDone`과 동일하게 그 증가 구문은 **`warmUpCommandTree` 함수 본문 안**(재귀가 실제로 노드를 방문하는 지점)에 위치해야 하고(shall), `TestMain`에서 대입하거나 증가시켜서는 안 된다(shall not).

판별 기준(0.3.0 대비 확장): warm-up **호출을 유지한 채 본문의 재귀 루프만 제거**했을 때에도 가드가 실패해야 한다. `warmUpDone` 단독으로는 이 변형에서 신호가 `true`로 남아 가드가 통과한다 — sync-audit 실측으로 확인되었으며, 그 상태에서 20회 중 1회 레이스가 재발했다. 따라서 `warmUpDone`(실행 여부)과 `warmUpVisited`(순회 깊이)는 서로 다른 변형을 봉쇄하며 **둘 다** 필요하다.

비교 대상 노드 수는 **`TestMain`에서 warm-up 호출 직후·`m.Run()` 호출 이전에** 산출되어 패키지 스코프 변수에 보관되어야 하며(shall), **가드 테스트 안에서 재계산해서는 안 된다**(shall not). 근거: 전역 커맨드 트리는 실행 도중 자란다 — cobra의 Execute 경로가 `InitDefaultHelpCmd` / `InitDefaultCompletionCmd`로 `help` + `completion`(+ 셸 4종 자식) = 6 노드를 전역 루트에 덧붙이며, 이를 유발하는 테스트가 가드보다 먼저 실행된다. 가드 시점의 재계산은 warm-up이 순회한 트리가 아닌 더 나중의 트리를 재므로, **올바른 구현에서도 오탐 FAIL을 만든다**(실측: `visited 182` vs `holds 188`, 차이가 정확히 6).

노드 수의 산출 함수는 `warmUpCommandTree`와 **별개 구현**이어야 하며(shall) — 같은 순회 코드를 재사용하면 두 수가 항상 함께 움직여 구조적으로 반증 불가가 된다 — warm-up 완료 **이후 직렬로** 실행되어야 한다(shall). `TestMain` 배치는 `m.Run()` 이전이므로 엄격히 단일 goroutine이며, 이 두 불변(별개 구현 · warm-up 직후 직렬 실행)을 모두 보존한다.
...
2. **TestMain warm-up은 미래 기여자가 삭제하거나 무력화할 수 있다.** REQ-CFS-003(의도 주석) + REQ-CFS-004(도달 가능성 + 순회 깊이 가드) + REQ-CFS-006(인벤토리 가드)로 완화한다. REQ-CFS-004 가드는 초판의 정렬 기반 가드(구조적으로 항상 통과)와 달리 다음 **두 변형**에서 결정론적으로 실패한다 — (a) warm-up **호출 제거**: `warmUpDone`이 `false`로 남아 탐지(AC-CFS-005), (b) 호출은 유지한 채 **본문의 재귀 루프만 무력화**: `warmUpVisited`가 독립 산출 노드 수와 불일치하여 탐지(AC-CFS-004c, 0.4.0 신설).

   **0.3.0까지의 방어 경계는 (a) 호출 제거에 한정**되어 있었고 (b)는 통과했다 — sync-audit에서 실측으로 확인되었다(호출 유지 + 재귀 루프 제거 상태에서 스위트 전부 통과, 그럼에도 20회 중 1회 레이스 재발). 0.4.0이 이 경계를 (a)+(b)로 확장한다.

   남는 한계는 세 가지다. 첫째, 가드 테스트 자체가 함께 삭제되면 방어가 무력화된다. 둘째, (a)·(b) 밖의 임의 변형 — 예컨대 재귀 순회는 그대로 두고 각 노드의 `Commands()` 호출만 제거하는 형태(방문 수는 유지되므로 두 신호 모두 통과) — 은 여전히 미탐지다. 셋째, **노드 수 산출을 가드 테스트 본문으로 되돌리는 "단순화" 리팩터가 오탐을 재도입한다** — 전역 트리가 실행 도중 6 노드 자라므로 올바른 구현에서 `182` vs `188` FAIL이 되살아난다(§D.3b). 이 세 번째 항목은 소스의 `DO NOT "simplify" this back into the guard test body` 주석으로만 억제되어 있으며, **주석은 산문일 뿐 기계적 가드가 아니다** — 이를 강제하는 테스트·린트는 존재하지 않는다.
