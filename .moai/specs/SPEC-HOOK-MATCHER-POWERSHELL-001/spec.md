...
| 0.2.2 | 2026-09-26 | manager-spec | Decisions D1, D2, D6 resolved (source: lead, 2026-09-26 (operator-delegated scope)). D1 = (A): separate PreToolUse block with matcher `PowerShell`, existing `Write\|Edit\|Bash` matcher and `agent_model_matcher_test.go` untouched (REQ-HMP-001). D2 = (A): unclassifiable → allow and write one audit-log line whose reason states `unclassifiable` (REQ-HMP-010, AC-HMP-009). D6 = (A): wrapper Risk-Amplifier warning stays Bash-only (REQ-HMP-002, AC-HMP-013). REQ count (16) and AC count (13) unchanged. |
...
`internal/template/agent_model_matcher_test.go` asserts the exact string `"Write|Edit|Bash"` is present and "must not be renamed or widened" (its purpose: the Agent matcher must be a separate block), and builds a RED fixture on the same literal. Widening the line-58 matcher breaks both; this is decision D1, resolved (A) — a separate `PowerShell` block, so the line-58 matcher and this test stay untouched (source: lead, 2026-09-26 (operator-delegated scope)).
...
| 14 | `internal/template/templates/.claude/hooks/moai/handle-pre-tool.sh.tmpl:44` (local `.claude/hooks/moai/handle-pre-tool.sh:39`) | Bash Risk-Amplifier warn block, gated by `grep -q '"tool_name"[[:space:]]*:[[:space:]]*"Bash"'`; comment at :43 says "matcher scope is Write\|Edit\|Bash" | Counts `\|\|`, `&&`, `\|`, `;`, backtick, `$(` in the command and warns on stderr above a soft cap of 5 (warn-only, never blocks) | **(b)** — backtick is PowerShell's escape and line-continuation character and `$(` is a PowerShell subexpression, so the same count means something different on PowerShell text | Decision D6 resolved (A): stays Bash-only (REQ-HMP-002; source: lead, 2026-09-26 (operator-delegated scope)). The comment is updated in the same delivery to state the matcher scope and why PowerShell is excluded |

## §B Requirements (GEARS)

### B.1 Registration

- **REQ-HMP-001** — The distributed settings template and the local `.claude/settings.json` shall register `handle-pre-tool.sh` for PreToolUse calls of the PowerShell tool in addition to Bash, as a separate PreToolUse registration whose matcher is `PowerShell`, leaving the existing `Write|Edit|Bash` matcher string unchanged (decision D1), and the embedded template shall be rebuilt so that the binary distributes the change.
- **REQ-HMP-002** — The pre-tool shell wrapper (template `handle-pre-tool.sh.tmpl` and its local copy `handle-pre-tool.sh`) shall keep its Bash Risk-Amplifier subcommand-count warning Bash-only and shall not emit it for PowerShell calls (decision D6), and its matcher-scope comment shall name the matcher delivered by REQ-HMP-001 and state why PowerShell is excluded.
