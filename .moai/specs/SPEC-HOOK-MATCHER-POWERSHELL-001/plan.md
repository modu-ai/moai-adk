...
- D1 = (A): `agent_model_matcher_test.go` is not edited; the `Write|Edit|Bash` line stays byte-identical.
- Wrapper (REQ-HMP-002): RED test drives the rendered wrapper with a stub `moai` on `PATH` and a PowerShell payload whose command has more than 5 metacharacters; assert the warning is absent (D6=A), plus a Bash control asserting the unchanged warning. Update the template `.sh.tmpl`, the local `.sh`, and the matcher-scope comment together.
...
- RED then GREEN per D2 outcome for each construct M0 found unclassified, on both the branch guard and the integration lock. D2=(A): allow + exactly one line in the guard's named log (branch guard → `branch-guard-audit.log`; integration lock → `integration-lock-audit.log`) whose reason field contains the literal word `unclassifiable`. `pwsh -EncodedCommand` with a payload that decodes to a harmless command still takes the D2 path (proves no decoding). A Bash control (`eval "git switch x"`) keeps its current outcome.
...
| D1=(B) silently drops the Agent-widening guard's intent | Closed: D1 resolved (A); the guard test is not edited |
| D6=(B) false-warns on escaped PowerShell lines | Closed: D6 resolved (A); PowerShell calls receive no warning |
