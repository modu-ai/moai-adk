...
| D2-encoded | `-EncodedCommand` / `-enc` / `-ec` / `-e` | — | Not a decision: unclassifiable regardless of payload, never decoded, handled by D2 (spec REQ-HMP-010) | Fixed |
...
- **LIVE arms A and B (payload capture, REQ-HMP-016).** Run M5 arms A and B now: they need only a logger, not the branch binary. If the captured `tool_input` lacks a string `command`, or `tool_name` is not `PowerShell`, stop and return a blocker report before M1.
...
- Mutants after GREEN, each must turn the test red: (m1) remove the PowerShell registration; (m2) register `PowerShell` on a different PreToolUse script (for example move it onto the `handle-harness-observe.sh` entry) while `handle-pre-tool.sh` lacks it.
