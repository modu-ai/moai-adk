...
> SSOT for the tester gate. 8 ACs. Tier S. All verification commands run from project root
> under go1.26.4.
...
| AC-GDU-007 | REQ-GDU-008 — scope guard | `git diff --name-only` | only `go.mod` + `go.sum` (any `.go` edit noted in progress.md per REQ-GDU-007; expected none) |
| AC-GDU-008 | REQ-GDU-008 — no new dependency | compare `go mod edit -json \| jq '.Require[].Path'` pre vs post | no net-new module path in `require` (transitive tidy churn allowed) |
...
- [ ] AC-GDU-001 through AC-GDU-008 all PASS (AC-GDU-005 evaluated against the §D.1
      exclusion list).
...
- **EC-1 — tidy proposes a charmbracelet/x/* indirect bump**: if `go mod tidy` itself
  ...
  and not a force-bump. The AC-GDU-007 scope guard still holds (diff stays in go.mod/go.sum).
