...
| 2026-09-02 | plan-audit iter-3 PASS-WITH-DEBT 0.875 (STOP signal, measurement-surface score basis) → final fold-in D22-D29; verdict accepted with recorded debt; no further audit iteration |
...
The `moai graph report` subcommand shall emit a markdown report to the fixed rotating path `.moai/reports/graph-report.md` under the resolved project root — a REGENERATING derived artifact (always the latest view, never committed, gitignored) containing three deterministically ordered sections: (1) god nodes — packages ranked by distinct fan-in over the edge layers, with code-call targets (persisted bare callee names) normalized to their package directory via the directory proxy defined below before fan-in aggregation (ambiguous or unresolved callees excluded, mirroring §(2)) — highest first, ties broken by node id; (2) surprising connections — INFERRED code-call edges whose endpoints' package directories differ, ranked above same-confidence intra-package edges; (3) import cycles — import-edge cycles reported as strongly connected components (SCCs), each SCC rendered as its canonical member list (a branched SCC can contain no simple cycle through all of its members, so membership — not a cycle — is what is rendered; the SCC count is the section's primary datum; bounded and deterministic; never an exhaustive simple-cycle enumeration). Package identity is derived from each endpoint's source-file directory under the project root (the edge's `Source` carries `file:Function` — `splitCodeNode` gives the file; the package is that file's directory). This is a directory-level identity proxy, not Go package-name resolution: a code-call edge's `Target` persists only a bare callee name, so the callee's package resolves only when exactly one node in the edge set carries that function name — ambiguous or unresolved callees are excluded from section (2) rather than guessed. Two runs over the same tree shall produce byte-identical report files.
