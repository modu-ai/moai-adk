...
- Existing infrastructure to EXTEND: 3-tool MCP registration pattern (`internal/cli/mcp_server.go:486-509` + handlers in `mcp_code_tools.go`), query layer (`internal/graph/codequery.go`), refresh path (`internal/cli/graph_refresh_cli.go`), deferred advisory scans (`internal/hook/session_start.go:680-702`), duration seam (`edgesRefreshClock`), staleness predicate (`edgesRefreshNeeded`), hook handler option pattern (`Option` / `WithSynchronousDeferredScans` — the DI seam for `DeferredEdgesRefresh`).
...
- repo `.gitignore` — add the `graph-report.md` entry so the derived report never appears as untracked noise.
...
Key files:
- `internal/graph/shrink.go` (new) — `DetectUnexplainedShrink(existing, rebuilt []Edge, scannedSources map[string]bool) ShrinkReport`: names removed edges whose source file (from `splitCodeNode(Source)` / the edge's file) BOTH still exists on disk AND lies outside `scannedSources`; empty report = overwrite permitted. A removed source absent from disk is a genuine deletion — never reported as a shrink defect.
...
- `scannedSources` is the file list the rebuild's own extraction loop actually processed (captured inside `BuildWithCodeLayers` at build time). NOT the `SourceFingerprintsForEdges` aggregate — that function returns four doc-side source-SET hashes (`internal/graph/meta.go:37`), not a per-file inventory, and does not cover the Go source tree.
- `internal/graph/meta.go` — the fixed shared `.tmp` path of the meta write (`metaPath + ".tmp"`, meta.go:90) hardened to a per-process suffix (pid-based), closing the two-concurrent-refresh temp race (trivial; part of this milestone).

Tests: injected partial failure (rebuild missing one existing source's edges) → refusal, prior artifact SHA-identical, message names the edges; genuine deletion (source file removed from disk → its edges dropped) → overwrite proceeds; existing-but-unscanned source → refuse; build path refuses with non-zero exit naming the removed edges; both automatic paths and the build path covered; refusal leaves zero writes (byte comparison).
