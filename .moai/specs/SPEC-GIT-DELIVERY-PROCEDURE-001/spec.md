...
| 근거 | 위치 | 내용 |
|---|---|---|
| 무조건 금지 | `internal/template/templates/AGENTS.md:64` (루트 `AGENTS.md:64` 도 같은 문장) | "Never change branch state in the primary checkout." 조건 없음 |
...
따라서 AC-01은 이 저장소의 로컬 규칙과만 부딪히는 문제가 아니라, 배포되는 지침끼리의 모순이다.

실행 안내 위치(`b412f8a33` 기준, 로컬과 템플릿 사본이 같은 줄번호):
...
| 〃 | 119, 121 | Phase D `git checkout main` · `git reset --hard origin/main` |
...
| 〃 | 56, 58 | Step 4 late-branch 종결 `git checkout main` · `git reset --hard origin/main` (계약 원문 — `manager-git.md:129` 가 이 절을 참조) |
| `.claude/skills/moai/workflows/plan/spec-assembly.md` | 336 | Phase 13 late-branch 사전 점검, 수동 `git switch -c feat/SPEC-*` |
| `.claude/skills/moai/workflows/sync/delivery.md` | 323, 324 | Step 3.3.5 기준 브랜치 복귀 `git checkout {main_branch}` 등 |

`manager-git.md:42` 와 `delivery.md:323-324` 는 late-branch 블록 밖의 일반 안내라, OD-1이 어떤 선택지로 결정되더라도 따로 처리 방식이 필요하다(§C.1 표에 표기).
...
| `main_late_branch` | **설정 키가 아니다.** 저장소 전체 검색에서 `manager-git.md:137` 과 생성물 `.codex/agents/moai/manager-git.toml:131` 의 문구에만 나온다. 템플릿 YAML·Go 설정에는 없다 |
...
| `workflow.worktree.auto_merge` | 템플릿 `internal/template/templates/.moai/config/sections/workflow.yaml:58` `false` · Go 기본값 `defaults.go:872` `false` · `types.go:623` 주석 "AutoMerge has no production reader" |
...
| SPEC-WORKTREE-BRANCH-GUARD-001 | completed | primary checkout 가드. §A가 `manager-git.md` Phase D를 primary checkout에서 정당하게 실행되는 예외로 인용했다 |
...
워크트리 문맥 기본 병합 문구는 SPEC에서 나오지 않았다. `git log -S` 추적 결과 커밋 `b7447cb90`(2026-04-03, `sync.md`)에서 들어왔고, SPEC-V3R4-WORKFLOW-SPLIT-001 커밋 `980ccdc56` 이 `delivery.md` 로 옮겼다(분할 이동).
...
`manager-git` 에이전트 정의의 동기화 절은 `git fetch` 가 끝난 뒤에 그 결과를 읽는 `git rev-list --count --left-right` 가 실행되도록 순서를 지시해야 하며, `git fetch` 를 그 결과를 읽는 명령과 서로 독립인 병렬 배치 항목으로 분류해서는 안 된다. 진정으로 독립인 읽기(`git status`, `gh pr checks --json`)는 병렬 배치로 남을 수 있다.

**REQ-GDP-002** (Ubiquitous)
`agent-common-protocol.md` 의 Pre-Spawn Sync Check 는 `git fetch origin main` 과 `git rev-list --count --left-right origin/main...HEAD` 를 순서가 보장되는 형태로 지시해야 한다. 세 번째 명령(`moai session list --json --filter-spec=<SPEC-ID>`)의 병렬 실행 허용과 두 해석 표의 내용은 바뀌어서는 안 된다.

**REQ-GDP-003** (Unwanted)
Pre-Edit Sync Check 절(`agent-common-protocol.md` 342-348행의 확인 블록)은 이 SPEC의 변경으로 수정되어서는 안 된다.
...
범위 파일의 어떤 실행 예시도 `gh pr merge <PR> --squash` 를 실행할 명령으로 제시해서는 안 된다. `manager-git.md:32` 의 기본값 설명 문장은 남아야 한다. Late-branch 절차 블록이 OD-1 결정 뒤에도 남는 경우 그 예시는 `--<merge_method>` 로 적혀야 한다.
...
- Go 코드(`internal/config`, `internal/hook`, `internal/github`) 변경. OD-2 선택지 C가 사용처 분류 갱신을 요구하는지는 결정 뒤 별도로 판단한다.
...
- `sync/doc-execution.md:36` — "worktree contexts default to auto-merge". OD-2 선택지 B·C에서는 반대 기본값을 적은 문장이 된다.
...
- 다음 위치의 `git checkout`·`git switch`·`git reset --hard` 는 금지 목록·경고표·인용 예시·권한 항목·워크트리 내부 안내라서 유지한다: 템플릿 `AGENTS.md` 64-66, `main-checkout-branch-guard.md` 20-22와 detail 파일, `coding-standards.md:141`, `moai-ref-git-workflow/SKILL.md` 144-145, `verification-claim-integrity-detail.md:161`, `settings.json.tmpl:564`, `moai-workflow-worktree/modules/troubleshooting.md` 176·179.
...
- OD-2 선택지 C에서 지침 문구의 설정 읽기가 사용처 분류 테스트의 "reader"로 인정되는지 확인하지 않았다.
- OD-1 선택지 2가 `AGENTS.md` 바이트 예산에 주는 영향을 측정하지 않았다(현재 루트 15,277바이트, 템플릿 16,936바이트).
- `manager-git.md` 는 `rule_template_mirror_test.go` 의 바이트 동일 대상에서 빠져 있다(같은 파일 주석). 사본 일치는 이 SPEC의 diff 기준으로만 확인된다.
- `sync/doc-execution.md` 템플릿 사본의 36행 문구 위치는 로컬 사본에서만 읽었다.

---

## §F 수용 기준

`acceptance.md` 가 기준(AC-GDP-001~016)과 검증 명령, 양성 대조를 담는다.
