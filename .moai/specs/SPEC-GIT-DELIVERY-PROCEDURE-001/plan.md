...
- 재현 근거: `.moai/reports/t622/repro.md` 와 같은 디렉터리의 sweep 파일들(추적 파일).
...
2. **`manager-git.md` 는 바이트 동일 테스트 대상이 아니다.** `internal/template/rule_template_mirror_test.go` 의 `lateBranchMirroredPaths` 주석이 제외 사유를 적어 두었다. 현재 두 사본은 diff exit 0 이지만, 일치는 AC-GDP-013으로만 보장된다.
...
2. 기준 트리 사본 반출: `git show $BASE:<경로> > .moai/reports/t622/run/base-<이름>` (범위 파일 로컬·템플릿 전부, `delivery.md` 포함).
...
- **범위.** §D(spec.md)의 "카드 범위 밖에서 관측한 형제"는 리드 판단 없이 고치지 않는다.
...
- E4: `go test ./internal/template/ -run '…' -v -count=1` 출력과 PASS 줄 수(0이면 판정 불가).
...
- 편집:
  - `manager-git.md` L·T 156 — `git fetch` 를 먼저 끝내고, 그 뒤에 진정 독립인 읽기(`git status`, `git rev-list`, `gh pr checks --json`)를 병렬로 둔다는 순서로 문장을 고친다. `git fetch` 를 "independent" 목록에서 뺀다.
  - `agent-common-protocol.md` L·T Pre-Spawn Sync Check 292·296·299 — Pre-Edit Sync Check 347행과 같은 형태(`git fetch origin main 2>&1; git rev-list …` 한 명령)로 순서를 보장한다. 세 번째 명령과 두 해석 표는 그대로 둔다. 347행은 건드리지 않는다.
...
- 어느 선택지든 `manager-git.md:42`(Checkpoint Rollback)와 `delivery.md:323-324`(기준 브랜치 복귀)는 late-branch 블록 밖이라 별도 처리 방식이 필요하다. 선택지 2는 조건으로 덮을 수 있고, 선택지 1·3은 두 자리를 따로 정해야 한다.
...
- 절차:
  ...
  3. 사본 diff, `go test ./internal/template/ -run 'TestRuleTemplateMirrorDrift|TestLateBranchTemplateMirror|TestTemplateNoInternalContentLeak' -v -count=1`.
  4. 템플릿 diff에서 SPEC ID·날짜 추가 여부 검사.
