...
tags: "kanban, bootstrap, topology, dispatch, plan, recorded-baseline, sole-writer, role-resolution, admission-mechanism"
tier: L
...
**B.4 Out-of-band note — CLOSED at v0.6.0: the admission mechanism exists, and it is a CLI verb.** This note recorded that `spec.md` §A.4 states entry into `plan` to be an operator act rather than a dispatch — `backlog` has no owning session (`REQ-KB-012`), so no completion report exists for the lead to act on, and a lead that admitted cards on its own initiative would be generating work rather than scheduling it — while no SPEC in the family stated the **mechanism**. It named three candidate shapes: a CLI verb, a hand edit of board state, or an import from an issue tracker.

**It is the first of the three.** Measured: `/moai todo` (`.claude/skills/moai/workflows/todo.md`, `.claude/commands/moai/todo.md`), with state at `.moai/state/kanban/backlog.json`. Its own text states the property this note derived independently — "`backlog` has no owning session, so nothing dispatches work into it — admission is always an operator act, and this is the surface for it" — and `.claude/rules/moai/workflow/kanban-dispatch.md` § Entry into the board is an operator act names it as the surface and restates that the lead never self-admits. The verb set is `/moai todo "<text>"` to append, bare `/moai todo` to list, `done <n>` to remove, and `next` to present the queue through `AskUserQuestion` for the operator's pick. `[HARD]` on that last one: "The pick is the operator's. Do not preselect, do not reorder by inferred priority, and do not append a 'start the top one' default."

**Nothing in this plan changes.** The closure is recorded, not absorbed. The reason this note gave for recording rather than owning still holds in full — the lead's loop begins at the first dispatchable column and is complete without the mechanism — and the mechanism landed **outside** the SPEC family rather than inside it, so no requirement here gains a consumer and none is added. This SPEC is at 25 of 25 requirements and 30 of 25 criteria; the closure costs neither.

**One thing the closure did not settle, and it is recorded rather than assumed shut.** This note observed that `SPEC-KANBAN-BOARD-001` "defines the card record (`REQ-KB-004`) and which moves are legal (`REQ-KB-008`) but never says who creates a card". `/moai todo` still does not answer that: its documentation states it is "**Not a board** — Column position lives with the lead and the SPEC status, not in this file", and an item carries `spec_id: null` until the operator picks it and the lead dispatches it into `plan`. So the queue holds *intent* and the board holds *cards*, and whether a queued item occupies the `backlog` **column** — which `SPEC-KANBAN-BOARD-001` §A.2 explicitly contemplates holding a card with no `spec.md` at all — is stated by neither surface. That residual is recorded on the board sibling, whose `REQ-KB-004` decides what a card is: `SPEC-KANBAN-BOARD-001` v0.5.0 §A.9 carries it as a reported disagreement rather than a resolved one, on the ground that a queue upstream of a `backlog` column and a queue that *is* the `backlog` column are both coherent readings and nothing measured chooses between them.

The reason this note was written at all is unchanged and is worth keeping after the closure: the same silence — a transition nobody states because each document assumes a neighbour states it — is exactly what produced the two ownership gaps this SPEC has repaired twice, and the residual above is the same shape one layer down.
...
| The declaration's **carrier** is left to run-phase, **except where one already exists**, in which case it is adopted (v0.7.0) | The SPEC cannot be read to learn where the role lives, and two implementations could differ. Accepted: no measurement here favours the launch command over the registry over the discovery output, and fixing it would be fixing a decision on taste. Narrowed at v0.7.0 in one case only: `SPEC-KANBAN-BOARD-001` `REQ-KB-025` establishes a carrier conditionally, because this SPEC is not on its critical path and its `REQ-KB-017` refusal would otherwise fail open — so where that has happened, this implementation adopts rather than choosing afresh (REQ-KS-006, AC-KS-030 fifth conjunct). Bought: the openness survives where it was argued for, and the one case that would produce a second declaration is closed. The obligation sits here because it cannot be enforced there — that SPEC's uniqueness scan runs before this SPEC exists, and its adopt-if-landed branch is unreachable since this SPEC depends on it. Cost: nothing; an in-place widening, ceiling unchanged at 25 of 25 and 30. |
| Entry into `plan` is an **operator act**; the mechanism was left unowned, and is **owned as of v0.6.0 — outside this family** | The board cannot be started without a human, which is unchanged and deliberate. Bought at v0.5.0: the hole was written down (§B.4) instead of being assumed closed by whichever SPEC the reader happened to be in. Paid off at v0.6.0: `/moai todo` is the mechanism, and it is the CLI-verb shape §B.4 named first of three. It landed outside the SPEC family, so no requirement here gains a consumer and the 25-of-25 ceiling is untouched. One residual is recorded rather than declared shut — whether a queued item occupies the `backlog` **column** or sits upstream of it — and it is carried on the board sibling, whose `REQ-KB-004` decides what a card is. |
