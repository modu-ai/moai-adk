...
The selector half holds, and holds for a specific mechanical reason: `setGLMEnv` writes the backend into the **process** environment, and the launcher then replaces the process with `claude` via `syscall.Exec`, which inherits it. The backend therefore travels with the launched session and not through any file — the GLM path's own comment records that `settings.local.json` injection was deliberately removed so the backend would *not* leak into later `claude` invocations. Interleaved launches do give each worker its own backend, which is what §A.8 needs.
...
- **The interleaved sequence is tolerable**, because none of the mutated project-global state determines the backend of a session that has already been exec'd. A worker launched under GLM stays on GLM when a later `moai cc` resets `team_mode`.
...
Board state lives in a file under the **primary checkout's** `.moai/state/kanban/`, resolved from any worktree as the parent of the directory reported by `git rev-parse --git-common-dir` — measured here as `/Users/goos/MoAI/moai-adk-go/.git`, whose parent is the primary root, and the same discriminant the repository's branch guard already uses. That rule is `REQ-KB-005` and belongs to the board sibling.
...
The third is quorum accounting, which is REQ-KS-012's criterion and is not re-checked here; what §D.5's expiry row now rests on is that "which roles answered" is read from declarations, so an implementation counting labels would report a role as answered on the strength of an operator's backend choice.
...
- `internal/cli/launcher.go` — `applyCCMode` and `applyGLMMode`, the mode application the printed guidance's commands run, and `launchClaudeDefault`, whose `syscall.Exec` is the mechanism by which a per-session backend survives into the launched session. Their project-global mutations — tmux session environment, `.claude/settings.local.json`, `team_mode`, and `worker-`-prefixed worktree removal — are the measured reason §A.8 states a consequence rather than assuming per-session isolation, and the reason REQ-KS-003 re-measures the launcher surface (`research.md` §J).
