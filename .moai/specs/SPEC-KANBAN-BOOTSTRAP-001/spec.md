...
title: "Kanban session topology, bootstrap, and the pointer-only dispatch protocol (KANBAN M1)"
version: "0.7.0"
status: draft
created: 2026-08-10
updated: 2026-08-14
...
tags: "kanban, bootstrap, topology, dispatch, cross-session-messaging, quorum, backend, entry-switch, sole-writer, role-resolution, admission-mechanism"
tier: L
...
- **v0.7.0** (2026-08-14) — One in-place widening, requested by the plan audit of `SPEC-KANBAN-BOARD-001` v0.5.0 and landed here because here is where it is enforceable. **No requirement added, no criterion added**: 25 of 25 and 30, unchanged and re-measured.

  **The obligation that BOARD could state but not enforce.** That SPEC's v0.5.0 borrowed the *carrier* of this SPEC's role declaration — the contract stays `REQ-KS-006`'s, which fixes no carrier by its own terms — because this SPEC is not on its critical path and its `REQ-KB-017` refusal would otherwise read nothing and fail open. Its §A.8 then asserted, in prose, that a later-landing implementation of *this* SPEC adopts that carrier rather than defining a parallel one: "one declaration in the system, or the borrow has failed." Nothing enforced it, and nothing on that side could. This SPEC is named in that SPEC's `dependencies:`, so its adopt-if-landed branch is unreachable at its own run-phase, and its uniqueness scan runs at its M1 — before this SPEC exists — so it can only ever find the declaration it wrote itself. The fork can only be introduced from this side, by an implementer reading this document, who until now found the carrier choice explicitly free.

  **`REQ-KS-006` widened in place**, the third time this requirement has absorbed a contract rather than costing one (v0.3.0 the declaration itself, v0.5.0's sibling move on `REQ-KS-019`, now this): where BOARD's run-phase has already established a carrier under its `REQ-KB-025`, this SPEC's implementation adopts it and defines no second declaration. The widening **fixes no carrier and narrows no property** of the contract above it — the run-phase openness §A.13 argues for is preserved exactly where it was argued for, the choice when there is none, and closed only where a choice would produce a second datum. `AC-KS-030` gains a fifth conjunct deciding it, with a positive control, since the other four are all satisfied by a second declaration that agrees. §A.13 records the narrowed case.

  **No `dependencies:` edge was added**, and the absence is deliberate for the fourth revision running: BOARD carries no edge to this SPEC, this SPEC carries one to BOARD, and reversing or doubling that would close the cycle `SPEC-KANBAN-BOARD-001` §A.4a refuses. A contract obligation stated in requirement text imposes no landing order, which is the whole reason it is the shape this family uses.

- **v0.6.0** (2026-08-14) — Targeted amendment, one item, **no requirement added, no criterion added, and no requirement or criterion amended**: 25 of 25 requirements and 30 criteria, unchanged. This is not a re-plan of this SPEC; it closes a gap this SPEC itself recorded, and the closure landed outside the SPEC family.

  **The admission mechanism `plan.md` §B.4 recorded as unowned now exists, and it is a CLI verb.** That note said entry into `plan` is an operator act while no SPEC in this family stated the **mechanism**, and named three candidate shapes: a CLI verb, a hand edit of board state, or an import from an issue tracker. Measured, it is the first: `/moai todo` (`.claude/skills/moai/workflows/todo.md`, `.claude/commands/moai/todo.md`), state at `.moai/state/kanban/backlog.json`, with `.claude/rules/moai/workflow/kanban-dispatch.md` § Entry into the board is an operator act naming it as the surface and independently restating the property this SPEC derived — the lead never self-admits.

  **spec.md is touched because §C asserted something that is now false.** Its exclusion entry read that the mechanism "is owned by no SPEC in this family today", and the gap was recorded as unowned rather than excluded on ownership grounds. That statement has an expiry the document could not see, and an exclusion resting on a false premise is a defect rather than a stale note: a reader arriving at §C would conclude the hole is open and either work around it or fill it a second time. The entry is rewritten to exclude on the ground that now holds — **owned, outside this family** — which changes the exclusion's reason and not its scope. Nothing this SPEC requires, verifies, or delegates moves.

  **One residual is recorded rather than declared shut.** §B.4 also observed that `SPEC-KANBAN-BOARD-001` "never says who creates a card", and `/moai todo` does not settle that: its own documentation states it is "Not a board", and an item carries `spec_id: null` until the operator picks it and the lead dispatches it into `plan`. Whether a queued item occupies the `backlog` **column** — which that sibling's §A.2 explicitly contemplates holding a card with no `spec.md` at all — is stated by neither surface. The residual is carried on the board sibling, whose `REQ-KB-004` decides what a card is, and is reported there (v0.5.0 §A.9) as a disagreement rather than resolved, because a queue upstream of a `backlog` column and a queue that *is* that column are both coherent and nothing measured chooses. Recording it here rather than closing it silently is the same discipline §B.4 was written under.

- **v0.5.0** (2026-08-11) — Plan-audit delta repair, one finding, **no requirement added and no criterion added**: 25 of 25 requirements and 30 criteria before and after, both re-measured in this worktree. The finding is that v0.4.0's defect (4) released more than it had to.

  **v0.4.0 released the vacancy area on a premise that held for only half of it.** Its stated ground was that owning any part "would need a new obligation, not a widening of an existing one", because the area is "a runtime lifecycle property (detect a vacancy, then either re-quorum, refuse dispatch, or surface it) and no requirement here is the natural host for it". That reading treats detection as a separate act. It is not one, for the dispatch case: the lead resolves "the session whose declared role owns the card's column" on **every** dispatch, so where the owning role has no occupant the lead already holds that observation — it is the null result of the selection `REQ-KS-019` governs, arrived at by the same lookup, at the same moment. Requiring the lead to report that result instead of swallowing it is a widening of REQ-KS-019's own subject, and the SPEC had already made exactly this move in the other direction: v0.3.0 widened `REQ-KS-006` in place from addressability to addressability-plus-role-declaration rather than spending a requirement on it.

  **What is now owned.** `REQ-KS-019` gains a **where**-clause: where no session in the launched set declares the role owning a **dispatchable** card's column, the lead addresses no session for it and surfaces both the unoccupied role and the waiting card. The safety property — never dispatching into a vacancy, and reporting it rather than stalling — is the §A.5 argument applied after bootstrap rather than at it, converting a hang into a fault.

  **The criterion judges the report, not the refusal, and the distinction is the whole point (§D.12).** The refusal half is already entailed by REQ-KS-019's first clause — "dispatch **only** to the session whose declared role owns the column" permits no dispatch when none does — so an implementation that does nothing at all satisfies it. That do-nothing reading is precisely the failure the clause exists to rule out. `AC-KS-019`'s new conjunct therefore fails a run in which nothing is addressed **and** nothing is surfaced. This is also why the clause carries a surfacing obligation its neighbour — the refusal for a board-reported not-dispatchable card — does not: that verdict is the board's own recorded state and is already legible to an operator reading the board, whereas §C records that in the vacancy case the board is "unaffected and correct", so a silent refusal there withholds the only signal there was.

  **What the entry now releases, narrowed rather than deleted.** Two items, and the second is stated as a limit rather than smoothed over. **Recovery** — re-establishing an occupant, by re-quorum, targeted relaunch, or operator act — is untouched by this widening and remains a new obligation. **Detection independent of a dispatch attempt** likewise: REQ-KS-019 observes only what its own selection returns, so a role falling vacant while its column holds no dispatchable card goes unreported. That case is benign rather than missed — where no dispatchable card waits for the role, nothing is stalled — so the reported set and the costly set coincide, which is what makes the dispatch-time check sufficient for the stall and insufficient for monitoring. `plan.md` §B.6 and the §E decision row are rewritten to match, and the three-way decision recorded there for the next revision is now two-way, one option having been taken.

  **Budget position after the repair, re-measured in this worktree:** requirements 25 of 25, criteria 30 against 25 — both unchanged, the five-criterion overflow remaining v0.3.0's.
...
**One case narrows that openness, added at v0.7.0.** `SPEC-KANBAN-BOARD-001` left this SPEC's landing off its critical path — its `REQ-KB-017` refusal reads this declaration and would otherwise fail open — and its `REQ-KB-025` therefore establishes a carrier for it, conditionally, where none exists. Where that has happened, this SPEC's run-phase **adopts** that carrier rather than choosing afresh (REQ-KS-006). The openness is preserved exactly where it was argued for — the choice of carrier when there is none — and closed only where a choice would produce a second declaration. The obligation is stated on this side because this is the side that can honour it: that SPEC declares this one in no `dependencies:` edge, its adopt-if-landed branch is unreachable at its own run-phase since this SPEC depends on it, and its uniqueness scan runs before this SPEC exists. An obligation placed there would bind nobody who was reading.
...
**REQ-KS-019** — The lead shall dispatch a card only to the session whose declared role owns the card's column, shall not dispatch to a session chosen because it is idle, and shall address no session for a card the board reports not dispatchable; **where** no session in the launched set declares the role owning a dispatchable card's column, the lead shall address no session for that card and shall surface both the unoccupied role and the card awaiting it — the refusal alone being already entailed by the first clause of this requirement, so that the report is the part that is not, and a vacancy, unlike the board's own not-dispatchable verdict, being legible nowhere on the board (§D.12, §C); and a dispatch shall confer no board-write authority on its recipient, the decision that a card *may* move being the board's (`REQ-KB-008`, `REQ-KB-009`) and the act of writing that move being the `lead`'s alone (`REQ-KB-017`).
...
- **The mechanism by which an operator admits a card from `backlog` into `plan`.** §A.4 states that the admission is an operator act rather than a dispatch — `backlog` has no owning session, so no completion report exists for the lead to act on — and that is the whole of this SPEC's claim. This SPEC's dispatch loop begins at the first dispatchable column and is complete without the mechanism.

  **Excluded on different grounds as of v0.6.0: it is owned, and owned outside this family.** Through v0.5.0 this entry read that the mechanism "is owned by no SPEC in this family today" and pointed at `plan.md` §B.4, where the gap was recorded rather than resolved. It is resolved now, by the first of the three shapes that note named: `/moai todo` (`.claude/skills/moai/workflows/todo.md`, `.claude/commands/moai/todo.md`), with state at `.moai/state/kanban/backlog.json`, and `.claude/rules/moai/workflow/kanban-dispatch.md` § Entry into the board is an operator act naming it as the surface. The exclusion's **scope is unchanged** — this SPEC still owns none of it, requires nothing of it, and verifies nothing about it — while its **reason** changes from an open hole to a surface belonging to another layer. An exclusion resting on a premise that has since become false is not a stale note but a defect: a reader arriving here would conclude the hole is open and either work around it or fill it a second time.

  **One residual stays open and is carried elsewhere.** Whether a queued item occupies the `backlog` **column** or sits upstream of the board is stated by neither surface — `/moai todo` describes itself as "Not a board", and an item carries `spec_id: null` until the lead dispatches it into `plan`. That question is `SPEC-KANBAN-BOARD-001` `REQ-KB-004`'s, since it decides what a card is; it is reported on that sibling (v0.5.0 §A.9) rather than answered here, because answering it would be this SPEC deciding a neighbour's subject on preference.
...
### Out of Scope — re-establishing an occupant for a role vacated after bootstrap
...
- **Restoring a role whose session dies once bootstrap has completed.** Quorum in this SPEC is scoped to bootstrap and only to bootstrap: REQ-KS-007 waits for it at the entry switch and REQ-KS-012 bounds that wait, and neither is evaluated again afterwards. Nothing in this SPEC — and, measured, nothing in either sibling — re-establishes a role that falls vacant later.
  ...
  **What moved at v0.5.0, and why it was a widening after all.** This entry previously released the whole area, on the stated ground that owning any of it "would need a new obligation, not a widening" because no requirement here was its natural host. That ground was wrong about one half. The lead resolves "the session whose declared role owns the card's column" on **every** dispatch; the vacancy is not a separate thing to go and detect, it is the null result of the selection REQ-KS-019 already governs — the lead holds the observation at the moment it selects. Requiring it to report that empty result rather than swallow it is a widening of REQ-KS-019's own subject, and it costs no requirement, exactly as v0.3.0's widening of REQ-KS-006 from addressability to addressability-plus-declaration did. The **safety** property is therefore now owned: REQ-KS-019 refuses to dispatch into a vacancy and surfaces it, converting the stall into a fault (§D.12).

  **What is still unowned.** Two things, and neither is a widening of anything here.

  1. **Recovery** — re-establishing an occupant for the vacated role. Surfacing a vacancy says a role is empty; it does not fill it. Whether the answer is a re-quorum over the full role set, a targeted relaunch of the one missing role, or an operator act with no automation at all is undecided, and each is a new runtime-lifecycle obligation rather than a widening.
  2. **Vacancy detection independent of a dispatch attempt** — a liveness watch that observes a session's death when it happens rather than when the next card needs it. REQ-KS-019 observes only what its own selection returns, so a role that falls vacant while its column holds no dispatchable card is not reported. That is a genuine limit and not a hole: where no dispatchable card is waiting for the role, nothing is stalled either, so the two conditions coincide and the dispatch-time check reports every vacancy that is actually costing anything. It reports no others, which is precisely why monitoring remains unowned.

  **Why recovery is not owned here.** This SPEC is at the Tier L requirement ceiling of 25, measured in this worktree at the time of writing — `grep -cE '^\*\*REQ-KS-[0-9]{3}\*\*' spec.md` returns 25 — and every repair in this revision and the three before it is an amendment in place for that reason. The ceiling is reported rather than absorbed, exactly as the criteria overflow is (§B).

  **What a reader who hits this should do.** The symptom is no longer silent: a dispatchable card whose owning role is unoccupied now produces a named report rather than the **hang rather than a fault** that §A.5 argues about. Read that report as what it says — a role has no live session — and not as a board defect or a dispatch bug. The recovery available today is the one §A.5 already prescribes for a failed bootstrap: launch the missing session and re-run the entry switch, which re-establishes quorum over the full role set. That is a workaround and not a design; the family's next revision should decide the owner of recovery, and `plan.md` §B.6 records the decision it faces.
...
### D.12 The vacancy refusal is judged on the report, not on the silence (REQ-KS-019)

The vacancy clause added at v0.5.0 has two halves, and only one of them is observable. The refusal half is already entailed by the requirement's first clause — "dispatch a card **only** to the session whose declared role owns the card's column" permits no dispatch at all when no session declares that role — so an implementation that does nothing whatsoever satisfies the refusal. That is exactly the free-reading the clause exists to rule out: given no target, do nothing, and let the card sit. The observable separating a correct implementation from that one is the **report**, so `AC-KS-019`'s vacancy conjunct requires both halves and fails a run in which nothing is addressed and nothing is surfaced. A criterion checking only that no session was addressed would pass against the silent stall it was written to catch.

This is why the clause carries a surfacing obligation that its immediate neighbour — the refusal for a card the board reports not dispatchable — does not, and the asymmetry is principled rather than an inconsistency. The board's not-dispatchable verdict is the board's own recorded state, legible to any operator reading it; the refusal there withholds an action whose reason is already visible. An unoccupied role is visible on the board nowhere: §C records that in the vacancy case the board is "unaffected and correct". A silent refusal there withholds the only signal there was.

The check fires only where a dispatch is attempted, and that limit is stated rather than smoothed over (§C). Where the vacated role's column holds no dispatchable card, no selection runs and nothing is reported — and nothing is stalled either, so the reported set and the costly set coincide. Detecting a vacancy before a card needs the role is monitoring, which this requirement does not perform and does not claim to.
