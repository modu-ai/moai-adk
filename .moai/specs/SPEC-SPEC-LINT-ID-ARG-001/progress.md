...
### 계획 산출물 개정 (v0.6.0, 2026-09-08)

- **성격**: run-phase가 종료하며 `§E.3` manager-spec 후속 표에 올린 **4건(F-B1 · F-B2 · F-B3 · F-B4)을 리드 승인 아래 닫은 개정**이며 manager-spec이 수행했다. 범위: `acceptance.md`(기준 11 · 기준 12 · AC-SLI-004a · §D.1 신설) + `spec.md`(§D 포인터 한 줄 + HISTORY) + 이 절. **코드·테스트·`§E.2`·`§E.3`은 손대지 않았다.**
- **[HARD] 네 건 전부 run-phase가 자기 소관 밖이라 판단해 올린 것이다.** 주 ③은 「`acceptance.md`는 이 에이전트의 소관이 아니므로 §E.3에 F-B1로 올린다」로 명시했다. 발견 → 상신 → 리드 승인 → manager-spec 개정의 경로가 돌았다.

**F-B1 — 기준 11(AC-SLI-008)의 모집단 정의가 실행 불가능했다.**

- **무엇이 깨졌는가**: `0b1e27877..HEAD`는 이 브랜치가 `origin/develop`을 두 번 흡수한 뒤 **285 커밋**이다(run-phase 실측, 주 ③). 흡수된 남의 커밋 수백 개가 모집단에 들어와 **잔여 집합이 결코 비지 않고**, 문자 그대로 따르면 남의 커밋 수백 개의 `git show`를 인용해야 통과다 — **통과할 수 없는 기준이었다.**
- **[HARD] 왜 v0.4.0에는 보이지 않았는가**: 그 시점 `HEAD == base`라 범위가 **0건**이었고, 빈 범위 위에서는 「잔여 집합이 비어 있다」가 자동으로 참이다. **공허하게 성립하던 정의**이며, 이 카드가 다른 자리에서 네 번 만난 공허한 초록과 같은 모양이다.
- **처분**: `--no-merges origin/develop..HEAD`로 교체. 나머지 절차(각 줄 판독 · 파이프 안에서 세지 않기 · 크기 비교하지 않기)는 **바뀌지 않는다**. **옛 정의를 지우지 않고 병기**했고 두 수(285 vs 8)를 나란히 실었다.
- **함께 적은 것**: **다음 카드도 이 자리를 밟는다** — `origin/develop` 흡수는 사고가 아니라 긴 카드의 정상 사건이므로, 「고정 base SHA로부터의 범위」로 카드 커밋을 정의하는 어떤 기준도 첫 흡수와 함께 조용히 실행 불가능해진다. 옳은 표현은 **`origin/develop`으로부터의 도달 불가능성**이다. **잡지 못하는 것도 적었다**: 새 정의는 측정 시점의 `origin/develop`에 상대적이므로 **착지 후에는 소급 재실행되지 않는다**(실행 창은 착지 전).
- **run-phase의 8줄 판독은 그 기록에 귀속시켰다** — 이 회차에서 다시 돌리지 않았다.

**F-B2 — 기준 12(AC-SLI-009 갈래 ②)의 부채를 기준 안에 기록했다.**

- 상환된 것과 미상환분을 갈랐다. 상환: 18패키지 양쪽 `ok`, 두 핵심 패키지가 **캐시 없이** 돌았음(`internal/cli` 404.484s → 482.526s · `internal/kanban` 147.176s → 145.593s), 뮤턴트 M11 CAUGHT. 미상환: **다섯 호출부의 실행 관측** — 채취본이 패키지 단위 `ok`만 실어 **이 AC가 스스로 막으려 한 모양**(「셀렉터가 0건을 골라도 초록은 초록으로 보인다」)이 그대로 남았다.
- **[HARD] 왜 소급 상환이 불가능한지를 구조로 적었다**: 요구가 **전/후 두 트리 대조**로 쓰여 있어 전 트리 채취 창이 M-B1 착지와 함께 닫혔다. **부재 가드를 두 시점의 대조로 쓰면 그 기준은 실행 창을 한 번만 갖는다** — 이 카드가 이 자리에서 치른 대가다.
- **상환의 모양도 적었다**(후속 카드 소관): 후 트리 단독 **도달성 관측**(`go test -v -run`의 `=== RUN`/`--- PASS` 줄에 다섯 호출부를 덮는 테스트 이름이 나타남). 무변형 대조가 아니라 도달성이 상환 대상이다.
- **`PASS`를 완료로 읽지 않게 하는 것이 이 항목의 목적**이며, 원 요구 문구는 지우지 않았다.

**F-B3 — M4의 `NOT CAUGHT`가 가드 결함이 아님을 문서화했다.**

- 관측 축자를 실었다: `mutation landed in source: yes` · `tests matched by selector: 1` · `rc=0 => NOT CAUGHT`. **변형이 착지했고 셀렉터가 1건을 골랐다** — 즉 「뮤턴트가 도달하지 못했다」가 아니라 **「도달했는데 동작이 바뀌지 않았다」**이며, 그 둘은 같은 `NOT CAUGHT`를 찍으므로 구별해 적었다.
- 원인은 **방어 두 겹**: 선행 배제를 벗겨도 앵커된 모양 검사가 그 경로를 기각한다. 비공허성은 **M4b가 세운다**(`rc=1 => CAUGHT`).
- **이 AC가 그리는 경계도 적었다**: 「경로 형태 대조군의 **관측 가능한 동작**이 변하지 않는다」를 재지 「선행 배제 분기가 **살아 있다**」를 재지 않는다. **`NOT CAUGHT` 기록은 지우지 않는다**(§A 규칙 5 — 그것이 가드의 경계를 그린다).
- **[HARD] 라벨 정정 — 조용히 고치지 않는다.** `§E.3` F-B3 행은 대상을 「기준 8」로 적었으나 AC-SLI-004a는 **기준 6-7**에 있고 기준 8은 AC-SLI-005다(재유도: `acceptance.md`의 `### 기준 6-7 — 대조군 무변형, 양쪽 (REQ-SLI-004)` · `### 기준 8 — 앵커 불변식 (REQ-SLI-006)`). 실질(M4 ↔ AC-SLI-004a)은 어긋나지 않으므로 개정은 AC-SLI-004a 자리에 넣었고, **`§E.3` 원문은 manager-develop 소관이라 손대지 않았다.**

**F-B4 — `acceptance.md §D.1` AC ↔ 검증 수단 전수 대응표 신설.**

- **부재가 실제로 오독을 만들었다**: AC 식별자는 소스 주석에만 있고 증거 파일은 테스트 이름만 실어, 배차 단계에서 **AC-SLI-001b가 미판정으로 오독됐다**. 그 관측 자체는 참이었고(보고 파일이 다른 축을 적는다) **침묵은 기준의 부재가 아니었다.**
- **표의 `파일:줄`은 이 회차에서 직접 재유도했다 — 옮겨 적은 것이 아니다**: `/usr/bin/grep -n '^func Test' internal/cli/spec_lint_test.go`(파이프 없음), 트리 `.claude/worktrees/t518` · HEAD **`358405d7c`** — 12개 테스트 전부가 `§E.2` AC 판정표의 줄 번호(HEAD `a4fbaeb82` 판독)와 **일치**했다. 앵커를 표 옆에 붙여 **줄 인용이 HEAD 읽기처럼 낡는다**는 사실을 적었다.
- **테스트가 없는 네 칸(008 · 009 갈래 ② · 010 갈래 ①)에도 이름을 붙였다** — 빈칸으로 두면 다음 독자가 다시 「미판정」으로 읽는다. 그 셋은 결손이 아니라 검증 수단의 성질(git 이력 단언 / 두 트리 대조 / 문서 존재 단언)이다.
- **판정은 옮겨 적지 않았다** — 표가 담는 것은 대응뿐이고, PASS / PASS-WITH-DEBT의 정본은 `§E.2` AC 판정표다. 두 곳에 두면 갈라진다. `spec.md §D`에는 포인터 한 줄만 더했다.

**세 수치는 움직이지 않았다 — 재유도로 확인했다.**

- AC 식별자 **13** · REQ **9** · lint 경고 **9**(rc=0, `0 error(s), 9 warning(s)`). `§D.1`이 기존 AC 식별자를 다시 언급하지만 재유도가 **유일 집합**을 세므로 이동이 없다. 증거를 **추적 경로**로 반출했다: `.moai/reports/t518/manager-spec-v08/lint-id-arg.txt`(트리·HEAD·명령 헤더 포함).

**이번 회차에 변경한 아티팩트**: `acceptance.md` · `spec.md` · `progress.md`(이 절). **`plan.md`는 내용 변경이 없어 버전을 올리지 않았다**(v0.5.0 유지) — v0.4.0의 「내용 없는 버전 상승」 간극을 반복하지 않는다는 이 파일의 규율이며, 의도된 무변경으로 적는다.

**do-not-disturb 무이동 확인**: `AC-SLI-005`의 픽스처 `SPEC-A-1`과 그 선택 근거 미변경 · 감사 대안 `SPEC--1`의 근처-실패 기록(§A.1) 미변경 · I-1의 「귀속되지 않은 커밋 집합 직접 판독」 절차 자체는 **유지**(고친 것은 모집단 정의뿐이며 개수 대조 폐기 논거와 반례 병기는 그대로다) · 명령 ①·②와 `-- internal/cli/` 한정 미변경 · Tier M 재분류의 실질 근거 4항(§G #5) 미변경 · `spec_status.go:18` 미통합 처분과 「동명-이의 재사용 드리프트」 명명 미변경 · **`§E.2`·`§E.3` 전체 미변경**(manager-develop 소관).
...
### 감사 시점 추가 관측 — 닫힌 뒤에 채워진 두 간극 (2026-09-08 독립 sync-audit)

**[HARD] 이 절의 관측은 내 측정이 아니고, 이 카드가 닫히던 시점에는 존재하지 않았다.** 위 YAML의 `mutants_never_fired: 1`과 `ac_pass_with_debt_count: 1`은 그 시점의 정직한 기록이며 **그대로 둔다** — 「증거 없이 닫았고, 뒤에 독립 감사자가 채웠다」가 사실이고, 나중에 증거가 생겼다는 것이 없었다는 사실을 지우지 않는다. 아래는 감사의 관측을 귀속과 함께 옮겨 적은 것이다.

**① `AC-SLI-008`의 계측기 비공허성 — 감사가 이력 안전한 catch-all 대조군으로 쟀다.**

뮤턴트는 **여전히 발화되지 않았다**(요구된 변형이 착지 커밋 훼손 = 공유 브랜치 이력 재작성이므로). 감사가 쓴 것은 다른 형태의 대조군이다: `git show --name-only --format= fe75ec5c8 | grep -c '^internal/spec/'` → **2**, 같은 탐침이 `2cfccd4eb` → **0**, `5d495bafc` → **0**. 계측기가 `internal/spec`을 실제로 보고함이 보였으므로, 명령 ①·②의 두 0은 **공허한 0이 아니다**. 증거: `.moai/reports/t518/audit/ac-sli-008-control.txt`(커밋 `4bb4cb7d2`에 추적됨). **주 ④의 「미발화」 기록은 유효하다** — 감사가 세운 것은 뮤턴트 발화가 아니라 계측기의 비공허성이며, 둘은 같은 것이 아니다.

**② `AC-SLI-009` 갈래 ②의 부채 — 감사가 후 트리 단독 형태로 상환 측정을 냈다.**

상환 형태는 이 §E.2 주 ⑤가 스스로 지정한 것과 같다(전 트리 창은 닫혔으므로 후 트리 단독). 감사 보고: `ValidateSpecID` 여섯 호출부 전부 **count > 0**, 오류 분기 포함. 증거: `.moai/reports/t518/audit/ac-sli-009b-callsites.txt`. 같은 보고가 `go test ./internal/kanban/` rc=0 · 커버리지 86.5%를 함께 냈으나 **그 두 값은 위 증거 파일에 실려 있지 않다**(그 파일은 블록별 count만 싣는다) — 감사 보고문에서 옮긴 값이며 나는 재실행하지 않았다.

- **오독 주의**: 같은 파일의 `internal/kanban/status_read.go:106.18,108.3 1 0`은 호출부가 아니라 그 앞의 빈 문자열 가드 블록이다. 호출부는 `109.2,109.54`(count 1)이고 오류 분기는 `109.54,111.3`(count 1)이다. 이 0을 호출부 미실행으로 읽지 않는다.
- **[HARD] 판정은 바꾸지 않는다.** `AC-SLI-009` 갈래 ②는 착지한 대로 **PASS-WITH-DEBT**로 남고, 위 YAML의 `ac_pass_with_debt_count: 1`도 그대로다. **부채가 상환됐는지의 판정은 감사자와 리드의 몫이며, 자기 이전 작업에 대해 내가 내리는 자기평가가 아니다.** 이 절이 세우는 것은 「상환 측정이 존재한다」까지다.

**범위 밖**: 감사의 나머지 자문은 이 절이 다루지 않는다. 전문과 증거는 `.moai/reports/t518/audit/`(추적 11파일).

## §E.4 Sync-phase Audit-Ready Signal

```yaml
sync_complete_at: 2026-09-08
sync_commit_sha: a90f3d024          # 이 §E.4 를 실은 sync 커밋. 커밋은 자기 SHA 를 담을 수 없어 placeholder 로 착지한 뒤 backfill 한다
