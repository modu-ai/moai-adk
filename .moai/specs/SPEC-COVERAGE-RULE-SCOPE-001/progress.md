...
**Gap — 증거 디렉터리에서 관측됐으나 설명되지 않은 파일 수 변동.**

리드가 같은 턴 안에서 증거 점검을 두 번 돌렸을 때 `.moai/reports/t362/`의 파일 수가 **24**로
읽혔고, 잠시 뒤 같은 호출에서의 디렉터리 목록은 **21**(현재의 올바른 집합)이었다. **유실은
없다** — 21개는 baseline과 파일 단위로 일치하고, `find -maxdepth 1 -type f`가 하위 디렉터리
없이 21개를 확인한다.

리드는 사라진 3개가 이 카드의 C2 픽스처 스크래치 정리였을 것으로 추정했다. **그 추정은 맞지
않는다. 내 진술은 이 관측을 닫지 못한다.**

내가 사실로 말할 수 있는 것:

- C2 스크래치는 **내 것이 맞다.** `.moai/reports/t362/c2/{mirror,nomirror}/SPEC-COVERAGE-RULE-SCOPE-001/spec.md`
  (파일 2개, 디렉터리 5개)를 C2 대조 측정을 위해 만들었고, 측정 직후 `rm -rf .moai/reports/t362/c2`로
  지웠다.
- **그러나 그 정리는 커밋 `130846ab2` 스테이징 전에 끝났다** — 이 카드의 다섯 커밋 중 **첫 번째
  이전**이고, 리드가 관측한 "마무리 중" 창보다 네 커밋 앞선다. 커밋된 적도 없다
  (`git log --all --diff-filter=A -- '.moai/reports/t362/c2*'` 빈 출력).
- 그 이후 나는 `.moai/reports/t362/` 아래에 **일시 파일을 만들지 않았다.** 이후의 테스트 실행은
  기존 파일 2개(`m1-corpus-measurement.txt`, `m2-gate0-decomposition.txt`)를 제자리에서 덮어썼을
  뿐, 생성·삭제가 아니다.

따라서 **24→21의 원인은 이 카드가 설명하지 못한다.** 관측은 실재하고, 최종 집합은 온전하며,
원인은 미상이다. 편한 설명(내 정리였다)을 받아들여 닫는 편이 장부는 깨끗해지지만, 시점이 맞지
않으므로 **그 설명은 거짓이다.** 열어 둔다.
