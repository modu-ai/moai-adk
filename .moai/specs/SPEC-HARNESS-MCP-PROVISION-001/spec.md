...
| 2026-07-11 | 0.1.3 | Plan-audit iter-2 fixes (11 REQ / 16 AC). (D1 MUST-FIX) The `/moai project` ROUTER (`project.md`) was out of scope — plan.md §F M2 now edits it (+ template mirror) to register Phase 3.6 in BOTH the Phase Routing Table and the Invocation Flow diagram, plus a user-approved backfill of the missing Phase 3.2 row (drift already shipped by BRIDGE-001); new AC-HMP-016 pins router registration + ordering + Phase 3.2 backfill; AC-HMP-011 parity loop extended to the router pair. (D2) AC-HMP-012/007 prohibition filter widened with `NOT ` so the parenthetical-negation phrasing plan.md itself prescribes no longer false-FAILs. (D3) AC-HMP-015 gained a second reverse delta on the H2 heading `(the 5 artifact types)`; plan.md §F M3 enumerates all six stale-count sites. (D4) Struck the stale `moai harness doctor` from acceptance.md §E Tested bullet. (D5) Corrected the false BRIDGE-001 `draft` dependency-state claim (it is `completed`; gate satisfied). (D6) AC-HMP-004/013 row greps guard the absent-file non-scalar case. No REQ / scope change; AC 15 → 16. | manager-spec |
