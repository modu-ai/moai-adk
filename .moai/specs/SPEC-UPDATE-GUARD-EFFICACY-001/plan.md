...
> **실제 결과 — M3는 코드 변경이 불필요했다 (run-phase 사후 정정).** 위 두 항목 모두 근거가 없었다.
>
> - **"재구성"이 불필요한 이유**: sentinel HOME 절차는 **판정 측**(AC-UGE-007의 셸 명령)에 있지 테스트 소스에 있지 않다. 기존 `update_home_radius_test.go`는 이미 `userHomeDirFn` 주입으로 격리하고 있어 그 절차를 그대로 만족한다. plan-phase에서 이 판정을 실제로 돌려 `--- PASS` + `diff-exit=0`을 관측했으면서도, 계획 문장은 재구성을 전제한 채 남아 있었다.
> - **"반경 확대"가 불필요한 이유**: 세 호출부는 **삭제를 하지 않는다**(spec.md REQ-UGE-004 정정 주석 — `homeDir`은 렌더링 입력으로만 흐른다). 확대할 반경이 존재하지 않는다. 이 문장은 §A에서 이미 정정한 "삭제 반경 누수" 전제의 잔재였다 — 정정을 §A에만 적용하고 §F까지 전파하지 않은 누락이다.
>
> M3의 실질 산출물은 `acceptance.md` 측 판정 설계뿐이며, 프로덕션·테스트 코드 변경은 0건이다.
