...
- 개발자 이름 포함 메시지
- 다음 명령어 안내 (/alfred:0-project, /alfred:1-plan)
...
describe("Phase 2: 개발자 정보 수집", () => {
  it("Git user.name을 기본값으로 제안해야 한다", async () => {
    ...
    expect(inquirer.prompt).toHaveBeenCalledWith(
      expect.arrayContaining([expect.objectContaining({ default: "홍길동" })])
    );
  });
  ...
  it("Git user.name이 없으면 빈 문자열을 기본값으로 사용해야 한다", async () => {
    ...
    expect(inquirer.prompt).toHaveBeenCalledWith(
      expect.arrayContaining([expect.objectContaining({ default: "" })])
    );
  });
...
describe("설치 프롬프트 통합 테스트", () => {
  ...
  it("Git 미설치 시 에러 처리", async () => {
    // Given
    (execCommand as jest.Mock).mockRejectedValue(
