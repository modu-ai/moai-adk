...
이 SPEC 은 `.agents/skills/` 에 미러를 **만들기만 하고 지우지 않는다.** `ManagedCleanTargets` 등록, dangling 링크 인식, pre-clean 백업 정책은 전부 승계 SPEC 소관이다(전출 목록은 HISTORY iter-6).

**따라서 이 SPEC 만으로는 다음을 보장하지 못한다 — 정면으로 적는다.**

> 스킬이 개명되거나 은퇴하면, 그 미러는 사용자 프로젝트의 `.agents/skills/` 에 **영구 잔존한다.**

이것은 이 SPEC 이 원래 막으려고 출발했던 바로 그 실패 형태이며(§A.9 가 서술하는 오염), 축소된 판본은 그것을 막지 못한다. 각주가 아니라 여기 본문에 적는 이유는, 이 SPEC 만 읽는 사람이 "미러가 배포되니 수명 관리도 된다"고 읽을 여지를 없애기 위해서다.
