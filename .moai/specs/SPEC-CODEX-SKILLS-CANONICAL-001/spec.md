...
판별 **기준**은 코드에 이미 있다 — `backupThenRemove` 의 파일 분기가 `templateCarries` 로 "템플릿이 같은 경로를 가졌는가"를 묻고, 그렇다면 백업 없이 지운다. 같은 기준을 미러에 적용한다: **이번 실행이 다시 만들 미러는 백업하지 않고, 다시 만들지 않을 항목은 백업한다.** REQ-CSC-010 이 이 형태다.

[HARD] **다만 "기준이 있다"가 "그대로 쓸 수 있다"는 뜻은 아니다.** `templateCarries` 분기는 `!info.IsDir()`, 즉 **파일 대상에만** 걸린다. 미러는 링크이거나 디렉터리이므로 실제로는 `templateManagedPaths` 경로를 타고, 그쪽은 템플릿에 `.agents/` 가 없어 항상 공집합이다(§A.7). 따라서 미러용 판별 분기는 **새로 작성해야 한다** — 재사용이 아니다. plan M3 이 그 작업을 소유한다.

(iter-4 까지 이 자리에는 위 기준의 *사유* 문장이 코드 주석에 적혀 있다는 식으로 쓰여 있었는데, 확인해 보니 주석은 동작만 적고 그 사유 문장을 담고 있지 않다. 인용 형태를 버리고 사실에 맞췄다.)
