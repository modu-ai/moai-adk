...
open_followups:
  - "AC-GFR-016 (final codemaps stamp main-reachable + no branch-HEAD restamp) — local halves RESOLVED via a recorded main-reachable refresh (2026-08-27, CR #1665 3865025115): the update-merge `c890d6abe` (origin/main) was followed by commit `b9dd6a9e4` ('fix(graph): reanchor codemaps stamp to main tip after update-merge (t279)', provenance.json only) restamping to `410da655f` — measured this run: `git merge-base --is-ancestor 410da655f39d1d3731abf2f247e8e5353a9d0de5 origin/main` → exit 0. This supersedes the sync-time state recorded above (M0 stamp `c9eed8ac6…` carried unchanged); REQ-GFR-014's 'unless a recorded, main-reachable refresh was required' clause is exactly this case, and no branch-HEAD restamp occurred. The PR-head half (`moai graph check` green in CI on the delivering PR) remains manager-git's PR gate."
