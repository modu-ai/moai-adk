...
- Commit chain: `fe575d8f4` (mode log) → `371b8799c` (M1) → `58bb7d8ba` (M2) → `e70d6acde` (M3) → `bc9192411` (M4 amendment) → `f32e9a346` (close) → `c1ed9baae` (M4 close evidence + §E.3) → `ffd98485c` (sync commit — the §E.4 `sync_commit_sha` names this commit) → `7b127b958` (D3 backfill of that SHA) → `c890d6abe` (update-merge of origin/main) → `b9dd6a9e4` (stamp re-anchor — the recorded main-reachable refresh REQ-GFR-014 allows). Lineage reconciled 2026-08-27 against the branch (CR #1665 3865025100): every SHA above verified present via `git show -s`; `mx_commit_sha: (this commit)` in the predecessor's progress.md resolves to `f32e9a346` (the close commit that appended it, per the M4 row-3 record).
...
open_followups:
  - "AC-GFR-016 (final codemaps stamp main-reachable + no branch-HEAD restamp) — local halves RESOLVED via a recorded main-reachable refresh (2026-08-27, CR #1665 3865025115): the update-merge `c890d6abe` (origin/main) was followed by commit `b9dd6a9e4` ('fix(graph): reanchor codemaps stamp to main tip after update-merge (t279)', provenance.json only) restamping to `410da655f` — measured this run: `git merge-base --is-ancestor 410da655f39d1d3731abf2f247e8e5353a9d0de5 origin/main` → exit 0. This supersedes the sync-time state recorded above (M0 stamp `c9eed8ac6…` carried unchanged); REQ-GFR-014's 'unless a recorded, main-reachable refresh was required' clause is exactly this case, and no branch-HEAD restamp occurred. The PR-head half (`moai graph check` green in CI on the delivering PR) remains manager-git's PR gate; live main-side stamp health is tracked by separate cards (t291/t292) — this row records the branch-local measurement above, not a live main-side claim."
  - "RESOLVED (`7b127b958`, D3 — 2026-08-27, CR #1665 3865025100): sync_commit_sha backfill — the field above now names `ffd98485c`, the actual sync commit `docs(SPEC-V3R6-GRAPH-FRESHNESS-002): sync-phase artifacts (t279)`"
```
