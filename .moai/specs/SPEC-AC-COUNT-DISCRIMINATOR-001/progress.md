...
- **왜(운영자 판정)**: run-phase 8건 전부 PASS 뒤 `origin/develop` 흡수에서 깨끗한 머지가 수트를 붉게 만들었다 — `ac_count_clause_test.go:430`, `SPEC-TODO-DESTRUCTIVE-GUARD-001/acceptance.md: counts 16 but is absent from the snapshot`. **신규 `acceptance.md` 추가 속도는 하루 여러 건이다** — 재유도 명령과 관측 트리(`947f5cffb`)는 `spec.md` §4 REQ-ACD-006 주석에 있고, **수는 얼리지 않는다**(브랜치와 함께 움직인다 — 개정 당일 한 시간 안에 값이 옮겨 갔고, 얼린 수를 그대로 두었다면 감사 N7 이 고친 결함을 되들이는 것이었다). 설계대로면 무관한 카드의 평범한 SPEC 저작이 하루에 여러 번 이 테스트를 붉히고 매번 손 재생성을 요구한다 — 큐에 이미 열려 있는 CI-상시-레드 카드 2건과 같은 부류다. 이 SPEC 이 겨눈 병은 **이미 잰 파일의 조용한 과다 계상 회귀**이고 그 탐지는 손대지 않았다
...
- **함께 정합화한 인용 4곳**: `spec.md` §3.5 규칙 1(+ "판정 규칙은 넷") · `spec.md` §4 REQ-ACD-006 글롭 주석 · `research.md` §B(글롭을 얼린 이유) · `design.md` §C.2 판정표. **넷 다** 종전 `"어떤 차이든 실패"` 를 근거로 인용하고 있었다 — 글롭을 얼린 논거 자체는 유지된다(글롭 변경은 **이미 잰** 파일을 사라짐으로 만들 수 있고, 그것은 여전히 실패다)
