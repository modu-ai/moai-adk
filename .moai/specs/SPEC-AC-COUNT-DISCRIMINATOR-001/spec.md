...
|---------|------|--------|-------------|
| 0.5.0 | 2026-08-28 | manager-spec | **운영자 판정에 따른 조항 개정 1건 — 재계획이 아니다.** REQ-ACD-006 과 AC-ACD-006 의 **실패 표면을 좁혔다**: 스냅샷에 **없는** 파일은 **보고하되 실패시키지 않는다**. 스냅샷에 **있는** 파일의 개수·상태별 집계·정지 상태 변화는 **종전 그대로 실패**하고, 스냅샷에 있는데 글롭이 더는 잡지 않는 **사라짐**도 **종전 그대로 실패**한다. 좁힌 근거(운영자, `origin/develop` 실측): 신규 `acceptance.md` 추가 속도가 **하루 여러 건**이라는 것 — 재유도 명령과 관측 트리는 §4 REQ-ACD-006 주석에 있고 **수는 얼리지 않는다**(감사 N7 이 고친 결함과 같은 형태). 설계대로 내보내면 무관한 카드의 평범한 SPEC 저작이 이 테스트를 하루에 여러 번 붉게 만들고, 매번 손 재생성을 요구한다(큐에 이미 열려 있는 CI-상시-레드 카드 2건과 같은 부류). 이 SPEC 이 없애려는 병은 **이미 잰 파일의 조용한 과다 계상 회귀**이며 그 탐지는 손대지 않았다. **이음매 1건을 명시적으로 풀었다**: 종전 AC-ACD-006 항목 5(d) "스냅샷에 없던 정지는 언제나 실패" 와 새 2번이 **새 파일이 정지할 때** 만난다 → **부재를 먼저 판정한다**(세든 정지하든 보고·비실패). (d) 는 자기가 겨눈 회귀 — **`COUNT` 로 기록된 파일이 정지하기 시작함** — 으로 좁혔다. **보고는 요구 출력**으로 못 박았다: 보고 없는 좁힘은 검사를 끈 것과 구별되지 않는다. 함께 정합화: `spec.md` §3.5(규칙 1 개정 + 규칙 4 신설) · §4 REQ 주석 · `research.md` §B 글롭 인용 · `design.md` §C.2 판정표(행 3개 개정·신설). 요구 8 / 수락 8 불변, Tier L 불변. **plan-audit 재실행 없음** — PASS 0.91(iter-3)은 이 개정 **이전** 판에 대한 판정이며, 재감사는 불필요로 판정됐다 |
...
판정 규칙은 넷이다(4번은 0.5.0 개정에서 더해졌다).

1. **판정 대상은 상태의 *변화*이고, 변화는 스냅샷이 이미 잰 파일에만 있다.** 스냅샷이 `COUNT` 인 파일이 `HALT` 로 바뀌면 실패, `HALT` 이던 파일이 `COUNT` 로 바뀌어도 실패한다(정규화가 일어났는데 스냅샷이 따라오지 않은 것이다). 스냅샷에 기록돼 있는데 글롭이 더는 잡지 않는 파일도 실패다 — **사라짐**은 이미 잰 관측이 없어진 것이다. **스냅샷이 아예 담지 않은 파일은 세든 정지하든 보고 대상이며 실패 대상이 아니다**(0.5.0 개정, REQ-ACD-006).
...
4. **부재 파일의 정지도 보고하고 실패시키지 않는다 — 부재를 먼저 판정한다.** 1번의 "기록된 파일" 조건과 종전의 "미기록 정지는 언제나 실패" 는 **새로 추가된 파일이 정지할 때** 만난다. 결정은 **부재 우선**이다: 스냅샷이 담지 않은 파일은 처음 보는 관측이므로 회귀일 수 없고, 회귀가 아닌 것을 실패로 부르면 1번이 지키려는 신호가 잡음에 묻힌다. 그 파일은 다음 재생성에서 `COUNT` 또는 `HALT` 항목으로 편입되고, 편입된 뒤부터 1·2·3번이 그 파일을 지킨다. **보고는 그 사이의 유일한 가시성이므로 선택이 아니다** — 부재 파일마다 경로와 이번 실행의 관측을 출력하지 않는 구현은 2번의 건너뛰기 금지를 우회한 것과 같다.
...
**REQ-ACD-006** — The verifier shall additionally run the extracted counter across every `acceptance.md` matched by the depth-1 glob `.moai/specs/*/acceptance.md`, excluding `.moai/specs/_archive/`, and compare the per-file result against a committed baseline snapshot that records each file's live count and its per-state identifier tallies; the corpus size shall be re-derived at run time rather than asserted as a frozen literal. While a file is recorded in the snapshot, the verifier shall fail on any difference in that file's live count, in its per-state tallies, or in its recorded state in either direction. While a file is recorded in the snapshot but is no longer matched by the glob, the verifier shall fail. When the verifier detects a file that the glob matches but the snapshot does not record, the verifier shall report that file's path together with this run's observation — `COUNT <n>` or `HALT <identifiers…>` — and shall not fail. The snapshot shall record a halting file as a first-class `HALT` entry naming the halting identifiers, its owning SPEC, and the reason the halt remains, and the verifier shall not exclude a halting file from the corpus nor record it as a zero count.

> 계수기의 정확성을 주장하지 않고 매 실행 재유도한다 — 카드가 물은 "판별자 자신을 무엇이 검증하는가" 의 답이 이 조항이다.
>
> **부재를 실패시키지 않는 이유(0.5.0 개정, 운영자 판정)**: 종전 문면은 "어떤 차이든 실패" 였고, **새로 추가된 파일**도 차이이므로 실패했다. 신규 `acceptance.md` 추가 속도 아래에서 그것은 무관한 카드의 평범한 SPEC 저작이 이 검증자를 **하루에 여러 번** 붉게 만들고, 그때마다 손 재생성을 요구한다는 뜻이다. **그 속도는 얼리지 않고 재유도한다**(REQ-ACD-008 의 규율은 이 SPEC 자신의 근거에도 적용된다 — 감사 N7 이 같은 결함을 한 번 고쳤다):
>
> ```bash
...
> ```
>
> 이 명령을 `origin/develop` `947f5cffb`(2026-08-28)에 돌려 관측한 값은 **7일 60 / 3일 29 / 24시간 6** 이었다. **값은 브랜치가 움직이면 함께 움직인다** — 실제로 개정 당일 한 시간 안에 `59 / 28 / 5` 에서 이 값으로 옮겨 갔다. 운영자 판정이 딛고 선 것은 특정 수가 아니라 **자릿수(하루 여러 건)** 이며, 그 자릿수가 뒤집히려면 위 명령이 하루 0-1 건을 내야 한다. 그래서 **실패 표면을 좁혔다**: 이 요구가 없애려는 병은 *이미 잰 파일*의 조용한 과다 계상 회귀이고 그 탐지는 그대로다. 사라짐(기록된 파일이 글롭 밖으로 나감)도 그대로 실패한다 — 그것은 이미 잰 관측이 없어진 것이다. **좁힘이 검사 종료와 구별되는 지점은 보고 의무다** — 부재 파일은 매 실행 경로와 관측이 출력되며, 그 출력이 없으면 좁힘은 검사를 끈 것과 구별할 수 없다.
>
> **글롭을 문면에 못 박는 이유**: 같은 트리에서 세 모집단이 동시에 참이다 — 깊이 1 이 602, 재귀가 603(`.moai/specs/_archive/SPEC-SKILL-001/acceptance.md`), 실측이 낸 값이 601(이 SPEC 자신의 파일 이전). 스냅샷의 판정 기준이 "기록된 파일은 어떤 차이든 실패 · 기록된 파일이 글롭 밖으로 나가면 실패" 이므로, 저자와 나중 독자가 모집단을 다르게 읽으면 **테스트가 무엇을 단언하는지가 조용히 달라진다** — 좁게 읽으면 이미 잰 파일이 사라짐으로 잡혀 실패하고, 넓게 읽으면 잡힌 파일이 부재 보고로 조용히 흡수된다. 그래서 수를 얼리지 않고 글롭을 얼린다. **이 논거는 0.5.0 의 좁힘 뒤에도 그대로 선다** — 좁힌 대상은 처음 보는 파일이고, 글롭 해석이 흔드는 것은 **이미 잰** 파일이기 때문이다.
>
