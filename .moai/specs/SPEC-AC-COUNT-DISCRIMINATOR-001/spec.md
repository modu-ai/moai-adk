...
>
> **부재를 실패시키지 않는 이유(0.5.0 개정, 운영자 판정)**: 종전 문면은 "어떤 차이든 실패" 였고, **새로 추가된 파일**도 차이이므로 실패했다. 실측한 추가 속도 — `origin/develop` 기준 **7일 59건 / 3일 28건 / 최근 24시간 5건** — 아래에서 그것은 무관한 카드의 평범한 SPEC 저작이 이 검증자를 하루 대여섯 번 붉게 만들고, 그때마다 손 재생성을 요구한다는 뜻이다. 그래서 **실패 표면을 좁혔다**: 이 요구가 없애려는 병은 *이미 잰 파일*의 조용한 과다 계상 회귀이고 그 탐지는 그대로다. 사라짐(기록된 파일이 글롭 밖으로 나감)도 그대로 실패한다 — 그것은 이미 잰 관측이 없어진 것이다. **좁힘이 검사 종료와 구별되는 지점은 보고 의무다** — 부재 파일은 매 실행 경로와 관측이 출력되며, 그 출력이 없으면 좁힘은 검사를 끈 것과 구별할 수 없다.
>
