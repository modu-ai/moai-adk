...
| 1.5.0 | 2026-08-28 | manager-spec | **plan-audit iter-3 (terminal) repair — PASS-WITH-DEBT 0.845, regressed from 0.8625, STOP raised.** The governing finding is not any single defect: the previous iteration **declared a sweep it did not run**, and three of this round's findings were reachable by re-running a command the artifact set already quotes. A declared sweep that was not run is an unobserved-verification claim — this SPEC's own subject, performed by its author, at the artifact layer. Every repair below names the command that confirms it. **D12** — the falsified baseline of 7 survived on `plan.md`'s implementation-facing surface, because the v1.4.0 note declared the milestone map untouched and it was; an implementer would have held 7 and discovered the discrepancy only when AC-GDL-010(b) failed. Now **4 session-start handlers**, matching the criterion in figure and definition, and swept: the figure appears nowhere else. **D13** — §B.3 quoted `grep -i binlag` above output containing `session_start_binary_lag.go`, a line that pattern cannot produce (the filename reads `binary_lag`); the pattern is now `grep -iE 'binlag|binary_lag'` and every quoted line reproduces. **D14** — §A.10's stability guidance was **inverted and measurably so**: it used a moving left operand and told readers to trust the left-hand count. `origin/develop` has advanced 11 past `ec15ec2cd`, so every documented left-hand value now reads 78 while every right-hand value reproduces. Both operands are pinned now and the whole block reproduces. **D15** — prose read "carries 9" above a block measuring `67 10`; corrected. **D16** — "All seven are recorded" against 8 bullets, replaced with the derivation form the sibling adopted for exactly this. **D17** — the D11 mirror covered one of the producing side's three contract clauses; an entry carrying two classifications matches the clean value and **under-fires**, which is how the fold route failed one clause inward. REQ-GDL-013's enumeration is now exhaustive over the contract's negations and AC-GDL-013 carries five fixtures. **D18** — the seam's only mechanical instrument was unsatisfiable: bare `OK` matches 79 files under `internal/hook/`. New `acceptance.md` §D.2 specifies it in full — case-sensitive, word-anchored, scoped to the deliverable's own files, comment lines excluded — with each property's measurement. **D19** — the schedule invariant counted files, so a job added inside an existing scheduled workflow satisfied it; a schedule-reachable-job clause closes that. Counts unchanged at 13 REQ / 13 AC. |
...
The two trees are **diverged**, not merely offset. **The durable claim is the divergence, not the count.** Both operands are pinned to SHAs below, because a count against a moving ref falsifies itself as soon as that ref advances. Measured at `9c543b99a`:

```
$ git merge-base --is-ancestor 9c543b99a ec15ec2cd && echo ancestor || echo "NOT ancestor"
NOT ancestor
$ git rev-list --count --left-right ec15ec2cd...9c543b99a
67	10
```

`ec15ec2cd` carries 67 commits this branch does not, including all of t326's landed work; this branch carries 10 it does not, all of them this card's SPEC authoring. **Reading this tree for a t326 surface returns "No such file or directory" for a feature that exists** — a stale-tree reading that reports a landed feature as absent, which is the class this SPEC has now paid for repeatedly.

**Two corrections to this block, recorded rather than quietly applied, because both are the failure it exists to prevent occurring inside it.**

*First*, an earlier draft labelled its figures "measured on `091966c55`" while citing `37263c222` — a commit that did not exist at the tree it named, and whose count does not reproduce there.

*Second, and it inverted the guidance a reader was told to follow*: the block used `origin/develop` as its left operand and then advised treating **the left-hand count as stable and the right-hand as valid only at the named SHA**. The opposite is true, and it is measurable. Pinning the right operand freezes the right-hand count; nothing freezes the left while it names a moving ref. Re-measured against `origin/develop` as it stands now, every left-hand value has moved and every right-hand value reproduces:

| SHA | as documented | against `origin/develop` now |
|---|---|---|
| `9c543b99a` | `67 10` | `78 10` |
| `37263c222` | `67 9` | `78 9` |
| `091966c55` | `67 7` | `78 7` |

The cause is exact: `origin/develop` has advanced **11** commits past `ec15ec2cd` (`git rev-list --count ec15ec2cd..origin/develop` → `11`), and 67 + 11 = 78. A reader following the old guidance would have trusted the volatile number and discounted the stable one. The block now pins **both** operands, so the whole of it reproduces rather than only half.
...
That is the entire contract. **No requirement, design section, or milestone in this SPEC names the vocabulary, states how many values it has, names any of them, or decides any entry into any of them.**

The scope of that claim is stated precisely, because one place necessarily breaks it: `acceptance.md` §D.2, the instrument that *enforces* the seam, must name the tokens it forbids in order to search for them. That is not coupling — the instrument runs **against** the deliverable and is not part of it, so a vocabulary change edits the instrument's pattern and touches no requirement, no design section, and no shipped line. The falsification test at the artifact level therefore excludes §D.2 and the HISTORY row that describes it, and the exclusion is stated here rather than applied silently. The test that actually governs the deliverable is §D.2 itself, run over the deliverable's source.
...
```
$ git ls-tree --full-tree -r --name-only origin/develop | grep -iE 'binlag|binary_lag'
internal/binlag/binlag.go
internal/binlag/binlag_test.go
internal/cli/binary_lag_test.go
internal/hook/session_start_binary_lag.go
internal/hook/session_start_binary_lag_test.go
```

*An earlier draft quoted this with the pattern `grep -i binlag` and an eliding `…`, listing `session_start_binary_lag.go` among the results — a line that pattern cannot produce, because the filename contains `binary_lag`, not `binlag`. The narrow pattern returns exactly two lines and the `…` elided nothing. The claim was true and the citation was not, which is the failure mode §A.10 governs, occurring inside a citation §A.10 governs. The command now produces every line quoted under it.*
...
- **REQ-GDL-013** — Where the consumed result **violates the contract of REQ-GDL-001 in any respect**, the harness shall render an advisory reporting the contract violation, and shall **not** report an all-clear. The violations are exactly the negations of the contract's clauses, and the enumeration is exhaustive rather than illustrative: **(i)** an entry carries a number of classifications other than exactly one — none, or more than one; **(ii)** the clean-value designation is absent, null, or designates more than one value. Clause (i) is not decorative — an entry carrying two classifications will match the clean value on comparison and be treated as nothing-to-report, so it **under-fires** rather than failing loudly. On such a result the clean/non-clean partition has no referent, so REQ-GDL-004's condition cannot be evaluated; the path of least resistance is to identify nothing as non-clean and therefore render nothing, which is green about a set the mechanism never learned (§A.0) at the consumer's layer. This mirrors the producing SPEC's own guard, which refuses an all-clear while its queried count is zero or its enumeration returned nothing, for the same reason and in the same shape.
...
**Every bullet above** is recorded in `acceptance.md` §D.7 as residual risk, not as a solved problem. Stated as a derivation rather than a number: an earlier draft said "all seven" and a bullet was added without updating it, which is the transcribed-count failure the sibling SPEC adopted a derivation to avoid and this SPEC's own `plan.md` applies to its milestone map. The claim is one-directional — §D.7 carries additional criterion-level bullets with no counterpart here, and that asymmetry is intended.
