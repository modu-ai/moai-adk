...
**Given** a manifest entry whose per-workflow query returns zero runs **and whose declared expectation does not excuse the absence** (it is not a release-only entry sitting outside its release cycle); **When** the evaluator classifies it; **Then** the classification is `UNKNOWN`, and the output states that the window is retention-bounded rather than asserting the guard never fired.
...
**Given** the evaluator and the harness; **When** it emits a result, and separately in **two** degraded runs — (i) one where every query **fails**, and (ii) one where every query **succeeds and returns zero runs** for entries whose declared expectation does not excuse the absence; **Then** ALL THREE hold — (a) every result carries a measurement timestamp and all five coverage counts (declared, queried, unknown, unreadable, undeclared), (b) run (i) does not report an all-clear and its `queried` count reads `0`, and (c) **both run (i) and run (ii) render an advisory to the operator**, even though in neither is any entry `STALE` or `UNDECLARED`, and in run (ii) `queried` equals `declared`.
...
- **Mutant (c) — this SPEC's own subject inside the deliverable, and it took two passes to key correctly.** Run (i): an evaluator whose every forge query fails classifies every entry `UNKNOWN`, so nothing is `STALE`, nothing is `UNDECLARED`, and a harness triggered only on those two states **renders nothing** while (a) and (b) pass. Run (ii) is the same end state down a branch the first repair did not enumerate: every query **succeeds** and returns empty, so `queried == declared == N` and a trigger keyed on `queried < declared` does not fire either — yet the sweep learned nothing about any guard, and REQ-GDL-010 permits an all-clear because the successfully-queried count is `N`, not zero. Both runs are now caught by one condition, not two: REQ-GDL-013 fires on **any entry not `OK`**, and both runs classify every entry `UNKNOWN`.
- **Why the fixture is two runs and not one:** run (i) alone was what the first repair asserted, and it is satisfied by an implementation keyed on query failure — the proxy that missed run (ii). Run (ii) is the discriminator between "the queries returned" and "the sweep learned something", which is the distinction the requirement now turns on. The clause binds the **advisory**, not the result object, because (a) and (b) bind the result and nothing carried indeterminacy across that boundary.
...
**Given** the M1 manifest schema; **When** an entry of a second, non-workflow kind is added — a watched subject carrying a different `kind` and locator, with no GitHub workflow behind it; **Then** BOTH hold — (a) the manifest reader accepts the entry's shape, counts it in `declared`, includes it in the reported entry list, and classifies it `UNREADABLE`, and (b) accepting it required no change to the manifest schema, the classification vocabulary, or the `measures` vocabulary.
...
- **Why clause (a) names `UNREADABLE` and not `UNKNOWN`, in two moves:** the first draft required the entry to report "as `UNKNOWN` at minimum", which reused `UNKNOWN` for a second, incompatible meaning — REQ-GDL-007 defines it as retention-bounded absence whose implied action is *look again with a longer window*, meaningless for an entry whose kind simply has no reader. The repair excluded that state from `UNKNOWN` and named no classification at all. That closed the conflation and opened a hole: REQ-GDL-009, closed in the same revision, requires **every** entry to take exactly one value from its set, and this entry could take none — `UNKNOWN` forbidden by REQ-GDL-007, `OK` and `STALE` both requiring a run set that does not exist, `UNDECLARED` defined as a workflow file with no entry, which is the inverse on both counts. `UNREADABLE` is the fifth value that makes the state space total. **The lesson is in the sequence, not the outcome:** narrowing one state without giving the excluded case somewhere to go is not a fix, it is a relocation, and the two revisions that did it landed in the same commit as the requirement that made it a contradiction.
