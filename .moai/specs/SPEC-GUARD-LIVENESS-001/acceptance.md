...
Budget: Tier M ≤ 16 acceptance criteria. **Count: 12** — still well under, which is the point of the reduction.
...
**Given** the merged deliverable and an evaluation result carrying a non-clean entry; **When** the session-start path and the hook package are inspected; **Then** BOTH hold — (a) the advisory's text is emitted through the **existing** contributor helper on that surface, called from within an **already-registered** session-start handler — verified by a call-site check on the deliverable's own code, and (b) the count of non-test files under `internal/hook/` registering a session-start handler is **unchanged** from its measured baseline.
- **RED-now:** no advisory exists on `091966c55` (AC-GDL-001's measurement) — no call site to check. Red for absence. The composition target and the baseline are measured on `origin/develop` at `ec15ec2cd`: `internal/hook/session_start_binary_lag.go` carries the contributor helper (its own comment records it as the fifth joiner), and the baseline count is **7** non-test files — `auto_update.go`, `handoff_inject.go`, `registry.go`, `session_start.go`, `session_start_binary_lag.go`, `session_start_compact.go`, `types.go`.
- **Green path:** M2. Passing output is a call to the existing helper from an existing handler, and a handler-file count still reading 7.
