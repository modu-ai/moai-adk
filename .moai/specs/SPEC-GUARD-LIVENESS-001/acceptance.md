...
**Given** three results that each violate REQ-GDL-001's contract — (i) the clean-value designation is **absent**, (ii) it is **null**, and (iii) it designates **more than one** value — each otherwise well-formed and carrying entries; **When** the harness processes each; **Then** BOTH hold — (a) each run renders an advisory naming the contract violation, and (b) **no run reports an all-clear**, including the case where every entry would classify clean under some guess.
