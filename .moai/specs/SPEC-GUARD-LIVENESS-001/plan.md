...
- **B-1 — the self-application constraint.** A watcher that cannot prove its own firing is forbidden. Answered by making the evaluator pull-based rather than scheduled (`spec.md` §D). The answer relocates the regress; it does not eliminate it, and §D says so.
...
- Render `STALE` and `UNDECLARED` entries as a non-blocking advisory (REQ-GDL-013).
- The advisory carries the age of the measurement it reports (REQ-GDL-014).

Flips: AC-GDL-010, AC-GDL-011.
