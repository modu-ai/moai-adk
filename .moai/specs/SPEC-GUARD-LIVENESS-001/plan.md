...
- Join the **existing** session-start additional-context block by calling the established contributor helper from an already-registered handler; register no new handler (REQ-GDL-010). The baseline to hold: **4 session-start handlers** — types whose `EventType()` returns `EventSessionStart` — measured on `origin/develop` at `ec15ec2cd`, matching AC-GDL-010(b) verbatim in both figure and definition.

  *An earlier draft of this bullet held **7** "non-test files under `internal/hook/`", the figure and the definition that AC-GDL-010's own repair had already falsified — `session_start_binary_lag.go` is one of those 7 and registers no handler. The repair corrected the criterion and declared the milestone map untouched, which is exactly why the plan kept the wrong number: an implementer follows the plan, holds 7, and discovers the discrepancy only when the criterion fails.*
