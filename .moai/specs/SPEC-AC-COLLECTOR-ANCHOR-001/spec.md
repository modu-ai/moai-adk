...
같은 실패 부류 — **코퍼스에 맞지 않게 형성된 판별식이 거짓 수치·거짓 분류를 낸다** — 가 이 카드를 재는 동안 다섯 번 재현됐다. 4·5행은 감사 iteration 2(`.moai/reports/t528/plan-audit-2.md`, D1'·D2')가 잡았다.
...
**이 카드가 고치려는 결함이 정확히 이것이다** — 코퍼스가 실제로 쓰는 모양에 맞지 않는 앵커. 그것을 측정하는 동안 같은 부류를 다섯 번 재생산했다. 이 기록은 자기비판이 아니라 **증거다**: `acceptance.md`의 뮤턴트 규율과 대조군 규율이 의례가 아니라는 가장 강한 근거가 여기 있다. 판별식은 자연히 이렇게 어긋나며, 어긋난 판별식은 **자신 있게 통과한다.**

#### 다섯 번 모두 같은 방법으로 잡혔다

다섯 중 어느 것도 **수를 다시 읽어서** 잡히지 않았다. 다섯 다 **코퍼스의 실제 줄 또는 산출물 원문을 열어서** 잡혔다(4행은 `/usr/bin/grep` 전수 열람, 5행은 프로브 소스와 출력 파일 직독). 그래서 이것을 규율로 승격한다 — `plan.md` §D: **코퍼스 수치는 그 뒤의 표본 줄을 읽기 전까지 채택하지 않는다.**
...
- `internal/cli/spec_view.go` — `moai spec view <ID> --acceptance`. blind인 100개 SPEC에 대해 지금 `No acceptance criteria found in <ID>`를 출력한다. **이것이 이 카드의 일차 정당화다.**
...
결과는 이렇다. **`moai spec lint`를 아무리 돌려도 이 두 오류는 0으로 관측된다** — 실제로 발화하든 안 하든. 그러므로 lint 실행으로 이 위험을 재려는 계측은 판정 불가능하며, 종전 판본의 AC-ACA-001-010 셋째 절이 정확히 그 자리에 있었다(감사 D3). 반대로 **CLI에서는 두 오류가 `default`로 떨어져 명령 전체를 실패시킨다.** 넓힘이 어떤 SPEC에서 중복 ID를 새로 만들면, 그 SPEC의 `moai spec view --acceptance`는 오늘의 `No acceptance criteria found`(무해)에서 **하드 에러**로 바뀐다 — 이 카드가 고치겠다고 지목한 바로 그 명령이 기준선보다 나빠지는 방향이다.

**이 카드는 그 방향을 계측만 하고 수리하지 않는다**(운영자 판정 2026-09-08). 수치가 0이 아니면 카드를 멈추고 보고하며, `internal/cli/spec_view.go`는 편집하지 않는다(REQ-ACA-001-013, §4).
...
- REQ-ACA-001-013: **Where** 넓힘 이후 어떤 `spec.md`의 `moai spec view <ID> --acceptance`가 기준선에 없던 하드 에러(`parse error:`)를 내면, 그 건수를 실측해 보고해야 하며(shall) 이 카드 안에서 수리해서는 안 된다(shall not) — `internal/cli/spec_view.go`는 편집하지 않는다.
...
- **CLI 하드 에러의 현재 건수.** §1.3의 치명 경로는 **코드 판독**에 근거한다. 현재 코퍼스에서 실제로 몇 건이 그 경로에 빠지는지 `moai spec view`를 코퍼스 전체에 돌려 재지 않았다. 넓힘 전후 둘 다 run 단계가 잰다(REQ-ACA-001-013).
...
- **`moai spec view`의 실제 코퍼스 전역 실행.** 중복 재료가 0이라는 것은 쟀지만, CLI를 코퍼스 전체에 돌려 하드 에러 건수를 직접 센 적은 없다. M7이 잰다.
...
- `internal/cli/spec_view.go:72` — `moai spec view --acceptance` (사용자에게 보이는 결함 지점)
