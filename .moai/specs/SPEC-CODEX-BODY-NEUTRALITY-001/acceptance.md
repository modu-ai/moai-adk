...
- **`manager-lead` 자기 스폰 4줄.** `subagent-spawn` 은 능력 존재로 확정돼 결속행이 없으므로, 존재하지 않는 행을 가리키는 참조는 만들지 않고 **본문에 능력 이름과 대체 행동을 직접 적는다.** 이것은 선택지가 아니라 유일한 처분이며, AC-CBN-013 이 그 결과를 양성으로 잡는다(착수 전 0). 앞 라운드의 이 항목은 「M1 이 행을 만들지 않았으면 건너뛴다」로 읽혀 REQ-CBN-009 의 한 부류가 아무 판정 없이 빠져나갈 수 있었다 — 그 허용을 제거한다.
