...
**M7 refinement follow-up (allowlist token + opt-out):** the initial M7 allowlist used `${CLAUDE_PROJECT_DIR:-$PWD}` (copying the governance-gate pattern), but `$PWD` is NOT a registered renderer passthrough token — it broke `TestRetiredWrapperTemplates_Renderable` (4 retired wrappers) and `TestManifestHashFormat`/`TestCatalogHashParity` (hash drift). Refined to `$CLAUDE_PROJECT_DIR` (passthrough-safe; when the env var is unset the prefix check simply doesn't match, falling to the default branch — acceptable). A second refinement added `/dev/null` to the allowed set after `TestHookWrapper_OptOutStderrLog` surfaced the legitimate operator opt-out contract. catalog.yaml regenerated (`gen-catalog-hashes.go --all`) after each wrapper-content change. Post-refinement: whole-repo `go test ./...` 0 FAIL, build/lint clean.
