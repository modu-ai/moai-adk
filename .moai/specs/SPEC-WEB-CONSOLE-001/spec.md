...
| 0.3.0 | 2026-09-10 | manager-spec | In-place amendment (card t613): the REQ-WC-009 Host check is widened from mutating methods to every method on every route, static assets included. The accepted Host set is stated as implemented (ASCII case-insensitive `localhost`, all of `127.0.0.0/8`, `::1` with or without brackets, IPv4-mapped loopback), and `localhost` matching becomes ASCII case-insensitive. The original requirement text is retained verbatim. See § Amendments and the supersession block under REQ-WC-009. |
...
- **2026-09-10 (in-place amendment, v0.2.0 → v0.3.0)** — **The REQ-WC-009 Host-check method scope is widened from `POST`/`PUT`/`PATCH` to every request on every route, `/static/` assets included.** A foreign or absent `Host` is now rejected with HTTP 403 on reads as well as writes. `status: completed` is deliberately unchanged: this amends the contract of a closed SPEC. It does not reopen the SPEC, and no run-phase work is re-entered under this SPEC ID.
  ...
  - **Accepted Host set**: stated in REQ-WC-009 as amended exactly as the code accepts it — `localhost` in any ASCII letter case, all of `127.0.0.0/8`, `::1` with or without brackets, and IPv4-mapped loopback, each with or without a port. The one change to the set is that `localhost` matching becomes ASCII case-insensitive (RFC 4343); without it, widening the gate to reads would have turned previously served `Host: LOCALHOST` reads into 403. Non-ASCII case-fold spellings stay rejected. The trailing-dot form `localhost.` is currently rejected and left undecided.
  - **Explicitly preserved, unchanged**: the `Sec-Fetch-Site` same-origin CSRF gate of SPEC-INTERNAL-SECURITY-001 REQ-SEC-002, which stays scoped to state-changing routes. The GLM key reveal defense (`/glm-key/reveal` is POST-only, and its handler re-checks loopback). The "no token auth, no session store, no CSRF-token infrastructure" boundary of REQ-WC-009 and § Exclusions.
...
**When** the Console receives a request of any HTTP method on any route (dynamic pages, form endpoints, and static assets under `/static/` alike), the Console **shall** verify that the request `Host` header names a loopback origin, with or without a port. The accepted set is: the host name `localhost` with its ASCII letters in any case (e.g. `LOCALHOST`, `LocalHost:<port>`), host-name case-insensitivity being ASCII-only per RFC 4343; any IPv4 address in `127.0.0.0/8` (e.g. `127.0.0.1`, `127.1.2.3`); `::1`, with or without brackets; and IPv4-mapped loopback (`::ffff:127.x.y.z`, with or without brackets). **When** the `Host` header names anything else or is absent, the Console **shall** reject the request with HTTP 403, serving none of the route's content and performing no state change. Rejected values include a non-ASCII case-fold spelling of `localhost` (e.g. the U+017F LATIN SMALL LETTER LONG S spelling), `0.0.0.0`, any other IP address, any other host name including look-alikes such as `localhost.attacker.example.com`, and an empty or absent `Host`. The trailing-dot form `localhost.` is currently rejected; whether it should be accepted is not decided by this amendment. The purpose is to prevent DNS-rebinding reads as well as DNS-rebinding/CSRF writes from other origins. The `Sec-Fetch-Site` same-origin CSRF gate (SPEC-INTERNAL-SECURITY-001 REQ-SEC-002) **shall** remain scoped to state-changing routes and is not widened by this requirement. As in the original, no token auth, session store, or CSRF-token infrastructure is introduced.
