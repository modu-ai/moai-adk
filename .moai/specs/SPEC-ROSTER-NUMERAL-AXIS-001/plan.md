...
- [ ] Re-derive the digit-axis and word-axis populations IN-RUN against the tree being changed;
      never reuse the §A plan-phase figures as the run baseline.
...
- **Consequence, stated plainly**: the registry grows from its current 30 `Site` rows to roughly
  45. That growth is the deliverable's visible cost — each new row is one previously-silent
  stale claim made enumerable.
...
### M6 — Template-First mirroring and doc comments

- Any `.claude/` ↔ `internal/template/templates/.claude/` pair touched in M5 is edited on both
  sides in the same commit; neutrality rules hold on the template side.
- Package doc comment gains a short section naming the second axis and why it exists, in the
  register `check.go`'s head comment already uses.

## §F Risks

| Risk | Shape | Mitigation |
|---|---|---|
| Allowlist becomes the subject matter | Each exempt row is prose a reviewer must read; too many and the guard reports its own exemptions | D1 bounds the population at ~75 hits; an exempt row needs a reason, which prices it |
