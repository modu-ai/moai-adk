...
priority: Low
phase: "Phase 1 - Foundation (ships via M05) + direct source edits"
module: "internal/cli/, internal/template/embed.go, coverage artifacts"
dependencies:
  - SPEC-V3-MIG-001
related_gap: [gm#186, gm#187, gm#188, gm#189, gm#193]
...
- `.moai-backups/` 폴더 제거 — SPEC-V3-CLN-003(M02 agency archival)이 담당
- `.agency/` 관련 정리 — SPEC-V3-CLN-003이 담당
...
**M05 (Migration Step — MigrationStep 구현체 `m05_legacy_cleanup.go`):**
- `internal/cli/glm.go.bak` 삭제
- `internal/cli/worktree/new_test.go.bak` 삭제
- `coverage.out`과 `coverage.html`이 30일 이상 오래된 경우에만 삭제 (fresh coverage는 유지)
- 삭제 전 `.moai/backups/<timestamp>/legacy/`로 스냅샷 복사 (rollback 경로)
...
- `.moai-backups/` 폴더 및 `.agency/` 관련 파일 정리 (SPEC-V3-CLN-003 담당)
...
- 영향 파일 (신설):
  - `internal/core/migration/steps/m05_legacy_cleanup.go`
  - `internal/core/migration/steps/m05_test.go`
...
**REQ-CLN-002-001**
The M05 migration step **shall** 프로젝트 루트 기준 다음 파일 목록을 삭제 대상으로 조회한다:
- `internal/cli/glm.go.bak`
- `internal/cli/worktree/new_test.go.bak`
...
**REQ-CLN-002-002**
The M05 migration step **shall** 삭제 전에 각 대상 파일을 `.moai/backups/<ISO-8601-timestamp>/legacy/`로 복사하여 rollback 경로를 확보한다.

**REQ-CLN-002-003**
The M05 migration step **shall** SPEC-V3-MIG-001의 `MigrationStep` 인터페이스를 구현하며 `Version()`, `ID()` (e.g., "M05-legacy-cleanup"), `Description()`, `IsIdempotent() bool = true`, `PreConditionsMet()`, `DryRun()`, `Apply()`, `Rollback()` 메서드를 모두 제공한다.
...
핵심 기준:
- `internal/cli/glm.go.bak`과 `internal/cli/worktree/new_test.go.bak`이 v3.0 release 이후 저장소에서 제거됨.
- `coverage.out`과 `coverage.html`이 `.gitignore`에 추가되고, stale한 경우 M05에서 제거됨.
- `internal/template/embed.go:8-12`의 주석이 실제 동작과 일치하도록 재작성됨 (REQ-CLN-002-004의 정확한 문구).
- `internal/cli/deps.go:151-186`에 AutoUpdateHandler compose pattern 주석이 추가됨.
- `.gitignore`가 존재하고 legacy 패턴을 포함함.
- M05 dry-run이 실제 삭제 없이 예상 작업만 출력.
- M05 rollback이 `.moai/backups/<timestamp>/legacy/`에서 파일 복원 가능.
- Regression test가 embed.go 주석 문구를 검증.
- `make build` 성공 및 `go test ./...` 통과.
- `git status`가 clean한 상태에서 M05 재실행 시 no-op으로 동작.
...
- SPEC-V3-CLN-003 (Agency archival + docs drift): M02와 함께 실행되지만 scope가 분리됨 — CLN-003은 `.agency/` 및 docs-site 정리 담당.
