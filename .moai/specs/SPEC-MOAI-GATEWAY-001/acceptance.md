...
> **철회됨 (2026-09-16).** 이 SPEC이 서술하는 기능은 운영자 결정으로 철회됐고, 아래 수용 기준은 그 시점의 기록일 뿐 현재 유효한 의무가 아니다 — 집합 전체가 검증 불가가 된 이유는 이 문서 맨 끝 `## 처분 기록 — 수용 기준 전체의 검증 불가 (2026-09-16 철회)`에 있다.
...
## 처분 기록 — 수용 기준 전체의 검증 불가 (2026-09-16 철회)

이 SPEC이 규정한 기능은 2026-09-16 운영자 결정으로 철회됐다(카드 t857, 커밋 `2d25a88eb` —
`feat(cli)!: withdraw the moai gpt gateway and GPT-in-Claude-Code path`). 자세한 경위와 종결 상태의
근거는 `spec.md` §I에 있다.

그 결과로 **이 문서의 수용 기준은 개별 항목이 아니라 집합 전체가 구성상 검증 불가(unverifiable
by construction)** 상태가 됐다. 측정 대상 자체가 트리에 없기 때문이다. 현재 트리(HEAD `df59ad18c`)의
측정 결과는 다음과 같다.

- `internal/gateway` 부재 — `ls`가 실패한다(양성 대조: `internal/` 아래 70개 패키지는 정상 조회).
  철회 커밋이 이 경로의 147개 `.go` 파일을 제거했다.
- `internal/orchestration` 부재 — 카드 t854가 그 안의 유일한 파일을 제거하면서 디렉터리가 사라졌다.
- `naming-migration-manifest.json` 부재 — 어느 커밋에도 존재한 적이 없고 트리에도 없다(`find` 무출력).

검증 불가는 "실패"가 아니고 "PASS 아님"도 아니다. 판정을 내리려면 피시험체가 있어야 하는데,
그 피시험체가 제거됐으므로 어느 방향의 판정도 성립하지 않는다. 이 문서 서두가 밝힌 Gap 규율과 같은
성격이되, 원인이 환경 제약이 아니라 대상의 부재라는 점이 다르다.

이 절은 처분을 기록할 뿐 **개별 AC의 문언을 고치거나 표시하지 않는다.** §A~§E의 AC-* 항목은
"당시 무엇을 검증하기로 했는가"의 기록으로 그대로 남는다.
