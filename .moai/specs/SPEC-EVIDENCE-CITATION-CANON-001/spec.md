...
Tier M 임계 0.80을 상회하고 재감사 상한에 도달해 종결 판정을 받았다(`.moai/reports/t375/plan-audit-iter2.md`). 남은 부채 4건을 이 판에서 닫았다: **N1** 증거 기반이 이 카드 자신의 plan-close를 견디도록 모집단 배제를 도입하고 그 근거를 §1.1.2에 적었다(배제 대상은 이 카드의 산출 디렉터리 **둘** — SPEC 디렉터리와 `.moai/reports/t375/`; 후자 누락과 그 적발 경위는 §1.1.1). **D1 잔여** 하한이 원리상 못 하는 하위트리 소실 검출을 집합 상등 단언으로 옮겼다. **D7 잔여** 허용목록 단위 단언에 판정 명령을 붙였다. **N2·N3·N4·N5** 단위·순서 미끄러짐 넷을 정리했다 — 셋이 이 SPEC이 입법하는 바로 그 형태였다.
...
- **REQ-ECC-001** (인용 증거 경로는 추적돼야 하며 정본 위치는 `.moai/reports/<card-id>/`) —
  좁혀진 대체 문언: 추적되는 인용 대상은 **판정서 파일** `.moai/reports/<card-id>/verdict.md`
  하나이며, 그 디렉터리의 다른 산출물에 대해서는 추적성이 주장되지 않는다.
- **REQ-ECC-003** (인용 전 추적 경로로 반출) — 좁혀진 대체 문언: 판정을 결정한 줄은 판정서 안에
  적고 그 파일을 인용한다. 판정서가 아닌 산출물은 스크래치에 남고 **인용하지 않는다** — 이는
  REQ-ECC-003 후반부(반출하지 않기로 한 것은 인용하지 않는다)와 같은 방향이다.
- **승계되지 않는 것**: REQ-ECC-002, 004~011은 그대로다.
...
아래 수치는 전부 스크립트 2개가 만든다. 스크립트가 곧 출처이고, 재측정은 그것을 실행하는 것이다. 두 파일은 이 SPEC과 같은 plan-close 커밋으로 추적 집합에 들어갔다.
...
| 측정 항목 | 값 | 출처 |
|---|---|---|
| 이 문자열을 담은 **추적** `*.md` 파일 (이 카드 산출물 제외) | **184** | `measure_citations.py` |
...
### 1.1.2 [HARD] 모집단은 이 카드 자신의 산출 디렉터리를 배제한다 — 그래야 추적 집합 논거가 끝까지 성립한다

추적 집합을 고른 논거에는 유효 기간이 있다. **이 카드가 만들어 내는 문서는 전부 `.moai/state/verify` 문자열을 담는다** — 그 경로에 관한 문서이므로 담지 않을 수가 없다. 그래서 plan 단계가 닫혀 그것들이 추적 집합에 들어가는 순간 `git grep`이 세기 시작하고, 위 표는 이 카드 자신의 정상적인 다음 동작 하나로 스테일해진다. §1.1.1이 고백한 결함과 같은 계열이고, 기제만 명령 오기가 아니라 **시간**이다.

그래서 두 스크립트는 모집단에서 이 카드의 **산출 디렉터리 둘**을 배제한다(`OWN_DIRS`, 각 스크립트 36행·필터 1행):

```
.moai/specs/SPEC-EVIDENCE-CITATION-CANON-001/   ← SPEC 산출물
.moai/reports/t375/                              ← 이 카드의 보고·증거 디렉터리(감사 보고 포함)
```

**둘 다 같은 이유로 들어간다**: 이 인용들을 논하는 문서는 필연적으로 그 인용을 담는다. 감사 보고서가 특히 그렇다 — 결함을 지목하려면 결함이 있는 경로를 그대로 옮겨 적어야 하므로, 구체 인용을 여럿 담는다.

배제가 실제로 값을 고정하는지 plan-close 커밋 **이후**에 확인했다(이 트리):

| 입력 모집단 | 입력 파일 수 | 스캔 | 출현 | 구체 | 파일 | 경로 |
|---|---|---|---|---|---|---|
| 추적 집합 (배제 적용) | 188 | **184** | **515** | **346** | **124** | **231** |

배제된 4개는 SPEC 3개와 `.moai/reports/t375/plan-audit-iter1.md`다. 배제가 없으면 같은 트리에서 185 / 523 / 350 / 125 / 232로 전부 이동한다.

**이 배제는 측정을 좁히는 것이 아니라 측정 대상을 정의하는 것이다.** 부채는 이 카드가 오기 **전부터** 있던 문서들이고, 그것을 세는 수가 이 카드의 진행 상태에 따라 흔들리면 애초에 부채의 크기를 말할 수 없다.

primary 체크아웃 `.moai/state/verify/` 실체: 최상위 디렉터리 **124개** — 그중 1개는 `snapshots`(§1.4의 기계 저장소)이므로 **세션 디렉터리는 123개**다. 파일 **905개**, **13 MB**(`find … -maxdepth 1 -mindepth 1 -type d | wc -l`, `find … -type f | wc -l`, `du -sh`). 대조군으로 이미 추적 중인 `.moai/reports/`: 추적 파일 **981개**(`git ls-files .moai/reports | wc -l`, t375 트리, plan-close 이후 — 그 커밋이 이 카드의 보고 7개를 더했다), primary 디스크 전체 **2,421 파일 / 29 MB**(미추적 포함).
...
- 184·532는 **추적 집합**(`git grep`)에 대해 옳고, 적혀 있던 **작업 트리 명령**(`grep -r`)에 대해 틀렸다. plan-close **이전** 같은 트리에서 후자는 187·562를 냈다 — 차이는 당시 미추적이던 SPEC 산출물 3개였다. (이 두 수는 그 시점에 고정된 기록이다. 지금 같은 명령을 돌리면 커밋으로 모집단이 바뀌어 다른 값이 나오며, 그것이 §1.1.2가 존재하는 이유다.)
- 131은 어느 명령으로도 재현되지 않았다. 지목된 증거표에서 재도출하면 97이 나왔다.

**같은 자리에서 두 번째로 미끄러졌다 — 그것도 적는다.** §1.1.2의 배제는 처음에 SPEC 디렉터리 하나만 담았다. "어느 파일이 이 경로를 인용하겠는가"를 **머리로 헤아려** 쓴 목록이었고, `.moai/reports/t375/plan-audit-iter1.md`를 빠뜨렸다 — 그 경로의 결함을 지목하는 감사 보고서라 구체 인용을 여럿 담는데도 그랬다. plan-close 커밋 **직후 재측정**이 그것을 잡았다(185 / 523 / 350 / 125 / 232 — 다섯 값 전부 이동). 그래서 배제는 이제 디렉터리 둘을 담는다.

교훈이 §1.1.1의 것과 같다: **적어 둔 명령이 기억해 둔 의도를 이긴다.** 한 카드 안에서 두 번 나왔고, 두 번 다 재측정이 잡았으며, 두 번 다 추론은 잡지 못했다.
...
```
X1=':!.moai/specs/SPEC-EVIDENCE-CITATION-CANON-001/*'
X2=':!.moai/reports/t375/*'
git grep -o '\.moai/state/verify'  -- '*.md' "$X1" "$X2" | wc -l   # 532
git grep -o '\.moai/state/verify/' -- '*.md' "$X1" "$X2" | wc -l   # 515
```

두 명령이 §1.1.2의 배제를 pathspec으로 다시 표현한다. 배제 없이 돌리면 plan-close 이후 580 / 548이 나온다 — 이 카드 자신의 문서가 세어지기 때문이고, 그 값은 표의 515와 대조할 수 있는 값이 아니다.
