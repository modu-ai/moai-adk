...
### M1 추가 조항 — 반출 의무의 나머지 반쪽 (리드 추가 지시)

M1 규칙 본문에 **"반출하지 않기로 한 것은 인용해서는 안 된다"**를 추가했다. 이유는 예시가 아니라 경계에서의 모순이다: 선택 기준(REQ-ECC-005)이 "일부는 스크래치에 남긴다"고 정하므로, **"인용 전 반출"만으로는 남긴 것을 가리키는 인용을 막지 못한다.** 규칙을 충실히 따른 사람이 미귀속 주장을 만들어 낼 수 있는 자리다.

살아 있는 실례가 이 카드 안에 있다 — `internal/hook/evidence_writer_zeroexec_test.go:10`(이 트리에서 축어 확인):

```
// v22.14.0. Raw captures live under .moai/state/verify/t341/.
```

t341은 §3.1이 이미 다루는 카드, 즉 338 KB 전사를 **일부러 반출하지 않기로 결정한** 그 카드다. 인용이 겨누는 대상이 "아직 반출 안 된 것"이 아니라 **"옳게, 의도적으로 남겨진 것"**이다. t341 레인이 한 처리(판정문 Residual-risk에 손실 위험 기재)가 곧 이 조항이 요구하는 행동이다.

반영 위치: `agent-common-protocol.md` § Parallel Execution 불릿 1문장 + `agent-common-protocol-reference.md` § Evidence export obligation 문단 1개(선택 기준 바로 뒤), 양쪽 미러 동반. 경로 리터럴을 새로 넣지 않았으므로 허용목록은 9개 그대로다(가드 재실행 rc=0).

### 알려진 잔여 (이 카드가 닫지 않는다)

- **확장자 열거가 경계의 잘못된 모양이다** — 누출이 `.md` / `.go` / `.txt` / `.html` 넷에서 관측됐다. 상세·측정 위치·`origin/develop` 귀속은 plan.md §D.6.1. **가드의 초록은 `.md` 표면에 대해서만 참이다.**
- **`.go` 센서스는 13이고, 그 13은 배제가 있어야 나온다.** 이 카드의 가드 픽스처가 추적 집합에 들어온 뒤 `git grep -n '\.moai/state/verify' -- '*.go' | wc -l`은 **21**을 낸다(가드 파일 단독 8건). 기존 코퍼스 값 13은 `':!internal/template/evidence_citation_guard_test.go'` 배제 시의 값이다. **§1.1.1이 기록한 실패가 세 번째로 반복됐고, 이번에도 재측정이 잡았다.** spec.md §5의 명령-값 쌍이 이 배제를 적지 않아 지금은 어긋나 있다 — 본문 소관이 아니라 리드에게 보고한다. 상세 plan.md §D.6.2.
- **착지·종결된 `SPEC-HIERARCHICAL-TEAM-001`이 통과한 인수 기준 안에 거짓 전제를 담고 있다**(`acceptance.md:9` + `progress.md:25` AC-FOLD-002=PASS, `status: completed`). 지금까지 가장 강한 실례이며, 닫으려면 **착지한 SPEC 개정**이 필요하다 — 문서 정리가 아니다. 그대로 둔다. 상세 plan.md §D.6.3.
