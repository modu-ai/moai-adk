...
- **장부와 해시 연결 (D6, D7).** 모든 시작 검사·LIVE 호출·정지는 `.moai/reports/t1172/ledger.json` 한 파일에 행으로 적는다. M1-a의 `startup` 네 행과 원자료 12개 파일도 보존하고 이후 상한에 포함한다. 네 행은 장부의 첫 네 행이며, `jq -cS '.[0:4][]' ledger.json | shasum -a 256`의 고정 결과는 `4e9b04acb14ed10b7122f1740fdcfaec147a91698bdebe2867a43123d6cb73ef`이다. 원자료 12개는 `startup/` 바로 아래의 네 픽스처별 `.jsonl`·`.err`·`.launches.json`이고, 경로 정렬한 `shasum -a 256` 출력 전체의 고정 sha256은 `c8653794d64014487c9f05d3e54f07e4b5fb61cf783f5e18f6460bb0e6a4b937`이다. 행 필드: `kind`(`startup`·`live`·`stop`·`invalidate`), `fixture`(`disc-control`·`disc-treatment`·`car010`·`car011`; `invalidate` 행은 묶음 이름 `disc`·`car010`·`car011`), `label`(이월 LIVE 행은 호출 라벨 — D6), LIVE 행만 `attempt`(1부터 시작하는 정수 시도 번호, 재실행마다 1 증가), `temp_root`·`fixture_root`·`fixture_sentinel`(시작 검사·LIVE 행; `temp_root`는 하네스가 받은 `t.TempDir()` 절대 경로이고 `fixture_root`는 그 아래다, D-N13), `started_ns`·`ended_ns`(정수 epoch 나노초 — 문자열 시각 비교를 쓰지 않는다), `argv`(배열), `exit`, `bound_by`(`test` 또는 `launcher`), `inputs_sha256`(`codex_home_config`·`project_config`·`argv`·`prompt`·`root_state`·`moai_binary`), 새 시작 검사 행만 고유 `startup_id`(`attempt-<n>/<fixture>-<started_ns>`), `passed`, `stdout_sha256`·`stderr_sha256`·`launches_sha256`, 새 시작 검사·LIVE 행만 `export_id`(`export-<exported_ns>`)·`export_sha256`(그 반출의 매니페스트 원문 바이트 해시), `provider_base_url`·`auth_file_present`·`env_auth_present`(인증 환경 변수가 시작 검사 프로세스 환경에 있었는지), LIVE 행만 대응 `startup_id`, `auth_sha256_before`·`auth_sha256_after`, `stop` 행만 `reason`, `invalidate` 행만 `attempt`·`recorded_by: "lead"`·`recorded_ns`·`reason`. `inputs_sha256`은 **그 호출이 실제로 쓴 바이트**를 해시한 값이고, 판정식은 이것을 반출본의 해시와 비교한다. 반출 매니페스트는 픽스처 묶음마다 하나(`discriminator/export-manifest.json`, `car010/export-manifest.json`, `car011/export-manifest.json`)이고 `exported_ns`와 반출 파일별 sha256을 담는다. 반출마다 묶음별 `exports/<export_id>/export-manifest.json` 고유 사본을 남기고 행의 `export_id`·`export_sha256`로 결속한다. 다시 반출하면 최신 매니페스트를 새로 쓰되(`exported_ns`가 바뀐다) 이전 반출 사본은 보존한다. 각 사본은 해당 반출 뒤의 시작 검사 전에 만들며 같은 시도 안에서 다시 반출해도 다른 ID로 보존한다. 팔 diff 메타(`arm-diff.meta.json`)는 `written_ns`를 담는다. 판별 증거 `evidence.json`은 그 판정이 읽은 시도 번호 `attempt`와 `recorded_pids`·`killed_pids`(하네스가 기록한 pid와 실제로 종료한 pid)를 담고, 팔마다 `user_turns`(그 팔 `--json` 출력의 `turn.started` 수, 호출당 사용자 턴 1개 상한의 증거, D-N16)를 담는다.
- **재실행 (D-N1).** 리드가 한 시도를 `INVALID`로 판정하면, 다음 시도 전에 리드가 장부에 `invalidate` 행(`fixture`는 묶음 이름, `attempt`는 무효가 된 번호, `recorded_ns`는 그 시도의 마지막 끝과 다음 시도의 첫 시작 사이)을 적는다. 다음 시도는 번호를 1 올린다. 묶음마다 재실행은 한 번(시도 번호 최대 2). 판별 결과 파일(`evidence.json`·`outcome.txt`·`live.jsonl`)과 시작 검사 원자료는 마지막 시도의 것이고, 이전 시도의 것은 `attempt-<n>/` 아래로 옮겨 보존한다. 장부 행은 지우지 않는다. 판정식은 묶음별로 마지막 시도의 결과만 채택하고, 무효 시도가 기록 없이 남아 있으면 거짓이다. 과거 무효 시도 행과 원자료는 각 행의 시도별 반출 사본에 대조하며, 마지막 시도만 현재 매니페스트에 대조한다.
- **재반출과 시작 검사 순서 (D-N2).** 픽스처 입력이 바뀌면 다시 반출하고(매니페스트 `exported_ns` 갱신), 그 뒤 그 픽스처의 시작 검사를 다시 돌린 다음에야 LIVE를 시작한다. 새 시작 검사는 LIVE에 쓰이지 않더라도 모두 `startup/<startup_id>.jsonl|.err|.launches.json` 고유 경로에 원자료를 남기며 기존 파일을 덮어쓰지 않는다. AC-CPP-004는 신규 시작 검사 행 전체를 원자료·해시·성공 조건에 대조한다. LIVE는 `startup_id`로 자신의 시작 검사를 지목한다. AC-CPP-004는 그 행의 동일 fixture·두 루트, `passed: true`, 대응 원자료의 thread/event/error/기동 결과와 해시, 해당 시도의 반출 사본 이후 시작, 두 설정의 그 사본 해시 일치, LIVE 이전 완료를 모두 확인한다. 마지막 유효 시도의 사본만 현 반출본과 같아야 한다. 이전 시작 검사 행(예: M1-a의 car010)은 기록으로 남지만 다른 임시 루트의 LIVE 성공 근거로 쓰지 않는다.
...
- 시작 검사(REQ-CPP-003)는 LIVE가 아니다. `ledger.json`에 `kind: startup`으로 적고 **모든 단계·시도를 합한 절대 상한 16회**를 적용한다: M1-a 4회 + M1-b의 새 임시 픽스처 4회 × 최대 2시도 + car010 1회 × 최대 2시도 + car011 1회 × 최대 2시도. `t.TempDir()`가 테스트 뒤 없어지므로 M1-a 행을 M1-b의 성공 근거로 재사용하지 않는다. 상한은 M1-a 행을 지우거나 다른 종류로 바꿔 회복할 수 없으며, 다음 호출로 17회가 되면 시작하지 않고 `ABORTED`를 기록한다. 호출당 `timeout 20`(AC-CPP-014는 정리 여유 5 s를 더해 25 s까지 허용). 모델 끝점에 닿지 않는 것이 조건이다. 비모델 `error` 항목 밖의 `item.*` 이벤트가 한 번이라도 보이면 즉시 멈추고 `ABORTED`로 적는다.
...
- 이번 상한 개정은 이전 plan-audit의 `startup <= 8` 합성 판정을 그대로 이월할 수 없게 한다. 첫 LIVE 전에 AC-CPP-004·AC-CPP-014의 현재 판정식으로 16행 참·17행 거짓과 LIVE/시작 검사 임시 루트 불일치 거짓을 다시 재고, 수정된 판정식과 코드 상한을 plan-auditor에게 증분 재감사받는다. M1-a의 기존 네 행은 삭제·재분류하지 않고 재감사 입력에 포함한다.
...
- M1-b (LIVE 2): 다른 테스트 프로세스가 만든 새 `t.TempDir()` 안에 네 픽스처를 다시 만들고 반출·시작 검사를 재실행한다(M1-a 장부 네 행은 그대로 보존·상한 포함). 현재 LIVE가 쓰는 임시 루트와 픽스처 루트가 같은 선행 시작 검사 행이 없으면 LIVE를 시작하지 않는다. 판별 프로브 두 호출 뒤 `outcome.txt` 작성.
