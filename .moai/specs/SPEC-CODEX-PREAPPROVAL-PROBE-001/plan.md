...
- **재반출과 시작 검사 순서 (D-N2).** 픽스처 입력이 바뀌면 다시 반출하고(매니페스트 `exported_ns` 갱신), 그 뒤 그 픽스처의 시작 검사를 다시 돌린 다음에야 LIVE를 시작한다. 새 시작 검사는 `startup/<startup_id>.jsonl|.err|.launches.json` 고유 경로에 원자료를 남기며 기존 파일을 덮어쓰지 않는다. LIVE는 `startup_id`로 자신의 시작 검사를 지목한다. AC-CPP-004는 그 행의 동일 fixture·두 루트, `passed: true`, 대응 원자료의 thread/event/error/기동 결과와 해시, 해당 시도의 반출 사본 이후 시작, 두 설정의 그 사본 해시 일치, LIVE 이전 완료를 모두 확인한다. 마지막 유효 시도의 사본만 현 반출본과 같아야 한다. 이전 시작 검사 행(예: M1-a의 car010)은 기록으로 남지만 다른 임시 루트의 LIVE 성공 근거로 쓰지 않는다.
