...
exports = {'disc-control': (base / 'discriminator/export-manifest.json', 'control'), 'disc-treatment': (base / 'discriminator/export-manifest.json', 'treatment'), 'car010': (base / 'car010/export-manifest.json', ''), 'car011': (base / 'car011/export-manifest.json', '')}
starts = {r.get('startup_id'): r for r in rows if r.get('kind') == 'startup' and r.get('startup_id')}
assert len(starts) == len([r for r in rows if r.get('kind') == 'startup' and r.get('startup_id')])
...
for live in lives:
    ...
    assert ident.split('/')[1].startswith(fixture + '-')
    assert start['fixture'] == fixture and start['temp_root'] == live['temp_root'] and start['fixture_root'] == live['fixture_root']
    assert start['passed'] is True and start['ended_ns'] < live['started_ns']
    manifest_path, arm = exports[fixture]
...
음성·변이(plan 단계에서 확인, verdict §Plan iter-3): `item.completed`(`agent_message`) 1개, car011 `.err` 없음, car010 `auth_file_present: true`, `env_auth_present: true`, `CODEX_HOME` 설정에 `model = "gpt-5"`, 프로젝트 설정에 `model = "gpt-5"`, 시작 검사 인자에 `-c model="gpt-5"`, 죽은 제공자를 정의만 하고 선택하지 않음, car011 시작 검사가 그 LIVE 뒤에 끝남, car010 마지막 시작 검사의 설정 해시가 반출본과 다름, car010 재반출 뒤 시작 검사를 다시 돌리지 않음, LIVE 행 인자에 한 토큰짜리 `--model=gpt-5`·`-mgpt-5`·`-c model="gpt-5"`(verdict §Plan iter-4, D-N14) → 모두 `false`. plan-audit iter 2의 `car010-restart`(M1-a 행은 옛 해시, 재반출 뒤 새 행) → `true`. 비모델 `error` 항목 1개만 있는 경우 → `true`(의도된 허용). 이번 개정의 추가 변이: 시작 검사 17행, LIVE와 시작 검사의 `temp_root` 또는 `fixture_root` 불일치, LIVE가 지목한 시작 검사의 `passed: false`, 현 반출과 다른 설정 해시, M1-a 원래 첫 행 변경, 대응 원자료 내용 또는 해시 변경 → 모두 `false`. 합성 양성은 `true`(M1-a 원본 4행 보존, 새 고유 ID 시작 검사와 LIVE 연결).
