...
| `discriminator/export-manifest.json` | `exported_ns`(정수), `files`(`"<팔>/<파일>"` → sha256, 14개). 다시 반출하면 새로 쓴다 |
| `car010/export-manifest.json`, `car011/export-manifest.json` | 같은 형식, `files`는 `"<파일>"` → sha256 7개. car010은 채택 갈래에서 M2 뒤에 다시 반출한다 |
...
| `ledger.json` | 시작 검사·LIVE·정지·무효 기록 전부를 담는 한 배열. 행 필드는 `plan.md` §D3 "장부와 해시 연결"(`attempt`, `fixture_root`, `fixture_sentinel`, `env_auth_present` 포함) |
...
**Given** §A의 증거 배치와 `plan.md` §D의 상한(판별 2회), **When** 대조 팔과 처치 팔을 차례로 한 번씩 돌리면, **Then** 호출 수 2, 중단 없음, 반출·차이·시작 검사 통과가 증거에 기록되고, 대조는 `REFUSED`이며 오류 문구가 거부 원문과 같고, 처치는 `REFUSED`(거부 원문) 또는 `RAN`이며, `RAN`이면 결과 문구가 moai 접두사 `codex_role_audit: codex audit sync-auditor: `로 시작하고 이어 `destination rejected: ` 또는 `working root rejected: `가 온다. 두 팔 모두 자식 프로세스 0개·launch record 0개이고, `outcome.txt`가 처치 결과와 같고, 로그인 파일 해시가 전후 같으며, 증거의 `attempt`가 장부의 판별 마지막 시도 번호와 같고, 종료한 pid(`killed_pids`)가 모두 기록한 pid(`recorded_pids`) 안에 있다.
...
**Given** `AGENTS.md.tmpl`의 `audit-verdict-file` 행, **When** 행을 읽고 지시면 테스트와 중립성 검사를 돌리면, **Then** 행에 REQ-CPP-008이 고정한 문장 "When the call is refused with `MCP tool call requires approval, but approval policy is never`, do not skip the audit and do not fall back to `spawn_agent`; return a blocker that quotes the refusal text"가 바이트 그대로 있고, `requires approval`은 행에 정확히 한 번 나오며, 반대 뜻 구절 `skip the audit and fall back`과 `no blocker`(대소문자 무시)가 없고, `internal/template/agentemit` 테스트에 실패가 없으며, 이 브랜치가 템플릿에 더한 줄에 SPEC ID·카드 ID·날짜·로컬 절대 경로(`/Users/`, `/private/var/`)·9자 이상 16진 문자열(커밋 SHA 모양)이 없다. 그 밖의 중립성 항목은 CI 가드(`template-neutrality-check`)가 맡는다.

```bash
grep -c '^| audit-verdict-file |' internal/template/templates/AGENTS.md.tmpl | grep -qx 1 && grep '^| audit-verdict-file |' internal/template/templates/AGENTS.md.tmpl > .moai/reports/t1172/ac-cpp-010-row.txt && grep -Fq 'When the call is refused with `MCP tool call requires approval, but approval policy is never`, do not skip the audit and do not fall back to `spawn_agent`; return a blocker that quotes the refusal text' .moai/reports/t1172/ac-cpp-010-row.txt && grep -oF 'requires approval' .moai/reports/t1172/ac-cpp-010-row.txt | grep -c . | grep -qx 1 && ! grep -Fq 'skip the audit and fall back' .moai/reports/t1172/ac-cpp-010-row.txt && ! grep -Fiq 'no blocker' .moai/reports/t1172/ac-cpp-010-row.txt && unset MOAI_KANBAN MOAI_KANBAN_ID MOAI_KANBAN_LABEL MOAI_KANBAN_LEAD_ADDR MOAI_KANBAN_SETTINGS_INJECTED MOAI_KANBAN_BACKEND MOAI_FACTORY_WORKER MOAI_FACTORY_WORKERS && go test -json ./internal/template/agentemit/... -count=1 | jq -se '([.[]|select(.Action=="fail")]|length)==0 and ([.[]|select(.Action=="pass" and .Test==null)]|length)>=1' | grep -qx true && git diff --name-only develop...HEAD -- internal/template/templates | grep -q . && ! git diff develop...HEAD -- internal/template/templates | grep -E '^\+[^+]' | grep -Eq 'SPEC-[A-Z][A-Z0-9-]*-[0-9]{3}|\bt[0-9]{3,4}\b|20[0-9]{2}-[0-9]{2}-[0-9]{2}|/Users/|/private/var/|\b[0-9a-f]{9,40}\b' && echo true
```

plan 단계 결과(verdict §Plan iter-3): 현재 트리에 그대로 적용 → 출력 없음(행에 고정 문장이 없고 템플릿 변경도 없다 — run 전 RED). `unset` 앞까지의 행 판정 부분을 합성 템플릿 트리에 적용 → 고정 문장을 덧붙인 현재 행 `true`, 현재 행 그대로 `false`, plan-audit iter 2의 의미 반전 행(`polarity-row.txt`) `false`, 고정 문장과 반전 문장을 함께 둔 행 `false`.
...
**Given** 반출된 car010·car011 입력, car010 부모 세션 출력, `adoption.txt`, **When** 이것들을 보면, **Then** 채택 갈래(`default`·`opt-in`)에서는 car010 사용자 층 decoy 표 안에 `enabled_tools = ["spec_progress"]`가 정확히 한 번 있고, car010 프로젝트 설정에 도구별 표가 정확히 한 번 있으며, 부모 세션의 `codex_role_audit` 호출이 1회 이상이고 그 서버가 모두 `moai`이며, car010 매니페스트의 파일 해시가 실제 반출 파일과 같고, car010 마지막 시도의 테스트 호출 행(`bound_by: test`)이 모두 매니페스트 반출 뒤에 시작했고 설정·인자·프롬프트·루트 상태 해시가 반출본과 같다(`not-adopted`에서는 AC-CAR-010을 돌리지 않으므로 이 부분을 보지 않는다). 모든 갈래에서 car011 입력 7종이 비어 있지 않고, car011 매니페스트와 마지막 시도 LIVE 행이 같은 방식으로 반출본과 맞으며, `m1-route/route.txt`가 t1143 반출본 사본과 바이트 같으며 값이 `mcp`다.

```bash
{ grep -qx 'not-adopted' .moai/reports/t1172/adoption.txt || { awk '/^\[/{s=($0=="[mcp_servers.decoy]")} s' .moai/reports/t1172/car010/inputs/codex-home-config.toml | grep -cx 'enabled_tools = \["spec_progress"\]' | grep -qx 1 && grep -cx '\[mcp_servers\.moai\.tools\.codex_role_audit\]' .moai/reports/t1172/car010/inputs/project-config.toml | grep -qx 1 && jq -se '[.[]|select(.type=="item.completed" and .item.type=="mcp_tool_call" and .item.tool=="codex_role_audit")] as $c | ($c|map(select(.item.server=="moai"))|length)>=1 and ($c|map(select(.item.server!="moai"))|length)==0' .moai/reports/t1172/car010/parent.jsonl | grep -qx true && shasum -a 256 .moai/reports/t1172/car010/inputs/* > .moai/reports/t1172/car010/inputs.sha && jq -e --rawfile s .moai/reports/t1172/car010/inputs.sha --slurpfile l .moai/reports/t1172/ledger.json '($s|split("\n")|map(select(length>0)|capture("^(?<h>[0-9a-f]{64})  .*/inputs/(?<p>.+)$"))|map({(.p):.h})|add) as $fs | .exported_ns as $e | ($l[0]|map(select(.kind=="live" and .fixture=="car010"))) as $r | ($r|map(.attempt)|max) as $fa | ($r|map(select(.attempt==$fa and .bound_by=="test"))) as $fin | .files==$fs and ($fs|length)==7 and ($fin|length)>=1 and ($fin|all(.started_ns > $e and .inputs_sha256.codex_home_config==$fs["codex-home-config.toml"] and .inputs_sha256.project_config==$fs["project-config.toml"] and .inputs_sha256.argv==$fs["argv.txt"] and .inputs_sha256.prompt==$fs["prompt.txt"] and .inputs_sha256.root_state==$fs["root-state.txt"]))' .moai/reports/t1172/car010/export-manifest.json >/dev/null; }; } && [ "$(for f in codex-home-config.toml project-config.toml repo.txt moai-build.txt argv.txt prompt.txt root-state.txt; do [ -s ".moai/reports/t1172/car011/inputs/$f" ] && echo ok; done | grep -c ok)" = 7 ] && shasum -a 256 .moai/reports/t1172/car011/inputs/* > .moai/reports/t1172/car011/inputs.sha && jq -e --rawfile s .moai/reports/t1172/car011/inputs.sha --slurpfile l .moai/reports/t1172/ledger.json '($s|split("\n")|map(select(length>0)|capture("^(?<h>[0-9a-f]{64})  .*/inputs/(?<p>.+)$"))|map({(.p):.h})|add) as $fs | .exported_ns as $e | ($l[0]|map(select(.kind=="live" and .fixture=="car011"))) as $r | ($r|map(.attempt)|max) as $fa | ($r|map(select(.attempt==$fa))) as $fin | .files==$fs and ($fs|length)==7 and ($fin|length)>=1 and ($fin|all(.started_ns > $e and .inputs_sha256.codex_home_config==$fs["codex-home-config.toml"] and .inputs_sha256.project_config==$fs["project-config.toml"] and .inputs_sha256.argv==$fs["argv.txt"] and .inputs_sha256.prompt==$fs["prompt.txt"] and .inputs_sha256.root_state==$fs["root-state.txt"]))' .moai/reports/t1172/car011/export-manifest.json >/dev/null && cmp -s .moai/reports/t1172/m1-route/route.txt .moai/reports/t1172/plan-checks/t1143-route.txt && grep -qx 'mcp' .moai/reports/t1172/m1-route/route.txt && echo true
```

음성·변이(plan 단계에서 확인): decoy 표에 `enabled_tools` 없음 → `false`, 부모의 `codex_role_audit` 호출 서버가 `decoy` → `false`, `route.txt` 없음 → `false`, car011 LIVE 행의 프롬프트 해시가 반출본과 다름 → `false`. `not-adopted` 갈래 → car010 부분을 건너뛰고 `true`.
...
- 모든 시작 검사·LIVE 행의 `fixture_root`가 `/`로 시작하는 절대 경로이고 `/.claude/worktrees/`를 담지 않으며, `fixture_sentinel`이 `true`다.
