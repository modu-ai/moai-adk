...
| `car010/export-manifest.json`, `car011/export-manifest.json` | 같은 형식. `files`는 공통 5종(`"<파일>"`)과 호출 라벨별 2종(`"<라벨>/argv.txt"`, `"<라벨>/prompt.txt"`) → sha256, 모두 `5 + 2×라벨 수`개. car010은 채택 갈래에서 M2 뒤에 다시 반출한다 |
