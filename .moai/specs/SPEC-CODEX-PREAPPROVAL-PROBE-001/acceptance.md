...
- `startup` 행은 M1-a 네 행을 보존한 채 모든 단계·시도를 합쳐 16개 이하이고, 17번째 시작 검사는 시작 전에 거부된다.
