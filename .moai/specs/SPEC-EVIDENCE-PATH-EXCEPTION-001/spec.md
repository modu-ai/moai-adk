...
- **REQ-EPE-006** [UNRESOLVED — 운영자 소관, plan 단계에서 해소되지 않는다] — 「판정서」 집합의
  경계는 두 독법 중 하나로 확정돼야 한다(`shall`): (A) 감사 판정서 계열 전체, (B) `verdict.md`
  단독. **이 SPEC은 어느 쪽도 확정하지 않는다.** §3이 두 독법의 측정 수치와 각각의 귀결을
  나란히 싣고, §3.3이 권고 독법과 그 근거를 적는다 — 권고는 결정이 아니다.
  plan 단계는 이 축이 열린 채로 닫힌다.
...
> **[HARD] 이 절은 결정하지 않는다.** 폭은 2026-09-14 운영자 지시의 예외 폭이므로 운영자
> 소관이다. 레인이 리드에 에스컬레이트했고 답은 아직 없다. 이 절은 **두 독법의 수치와 귀결을
> 나란히 싣고** 권고 독법과 그 근거를 적는다. plan 단계는 이 축이 열린 채로 닫힌다.
...
- **이 SPEC 자신의 증거가 반출돼 있지 않다.** `.moai/reports/t1039/lane-measurements.md`와
  ...
  이것이 이 결함의 **6번째 실사례**다. 카드가 착지하기 전까지 이 기록들은 저작 트리 밖에서
  해소되지 않으며, **워크트리를 폐기하면 유일본이 사라진다.**
- §1.4의 primary 체크아웃 수치는 다른 세션이 쓰는 트리의 스냅샷이며 드리프트한다.
- §5의 누수·깊이 측정은 픽스처 측정이다. 제자리 재측정 전까지 결론이 아니다(REQ-EPE-008).
- REQ-EPE-006의 폭은 **미해소**다. 영향 범위는 §3.4가 두 등급으로 전수 열거한다.
  plan 단계는 이 축이 열린 채로 닫힌다.
