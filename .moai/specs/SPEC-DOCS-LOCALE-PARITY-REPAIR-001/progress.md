...
| AC-011 | `git diff --name-only bce6d7e08..HEAD` (검증식 결함 — E.2.3 참조) | — | 의도된 대상 한정 10파일 · CHANGELOG t538 항목 1 | PASS(의도된 대상 한정) |
...
**(2) AC-011 검증식 결함 — 수리하지 않음(리드 판단).**
검증식 `git diff --name-only bce6d7e08..HEAD` 에 "변경 파일 집합이 10개로 한정" 이라는 기준을 건 형태는 쓰인 그대로는 도달 불가능하다. 브랜치 전체 diff 에는 베이스 이후 커밋된 plan 페이즈 아티팩트 7종이 필연적으로 포함되므로, `bbc37b729` 에서 이 식은 10이 아니라 17을 잰다.
10파일 기준은 **의도된 대상** 위에서는 충족된다 — `git diff --name-only bce6d7e08..HEAD -- docs-site CHANGELOG.md` 가 정확히 명명된 집합 10파일을 낸다(e2e en/zh + doctor ko/ja/zh + skill-guide ×4 + CHANGELOG). CHANGELOG 쪽 절반도 충족된다: `/usr/bin/grep -c "t538" CHANGELOG.md` → 1, 항목은 `[Unreleased]` 최상단 `### Added` 에 놓였다.
AC-008 과 달리 이 개정은 옛 읽기에서의 판정을 바꾼다. 그래서 카드 안에서 개정하지 않고, 리드에게 미해결 발견으로 보고한다. AC-011 은 의도된 대상 한정 PASS 로 기록하며, 검증식 결함은 sync/audit 소관 미해결 항목으로 §E.3 에 명시한다.
...
ac_matrix:
  ...
  AC-009: PASS
  AC-010: PASS
  AC-011: PASS-on-intended-subject
red_now_acs: [AC-001, AC-002, AC-003, AC-006, AC-007, AC-008]
...
open_items:
  ...
  - id: AC-011-expression-defect
    status: open-not-repaired
    owner: lead / sync-audit
    detail: "git diff --name-only bce6d7e08..HEAD 에 10파일 기준을 건 형태는 도달 불가 — 브랜치 전체 diff 가 plan 아티팩트 7종을 필연 포함해 bbc37b729 에서 17을 잰다. 의도된 대상 한정(-- docs-site CHANGELOG.md)에서는 정확히 10. 옛 읽기에서 판정을 바꾸는 개정이므로 카드 안에서 수리하지 않고 미해결로 보고."
```
