...
- `fully-autonomous` (sandbox-only, opt-in): `defaultMode: bypassPermissions` + sandbox/container proof required, sync-gate advisory-only, commit gate OFF, subagent lifecycle hooks dormant — with `deny` rules mechanically binding at every tier (non-bypassable) and `ask` rules advisory under `bypassPermissions` (see §C.1 binding note); the fully-autonomous safety boundary is the verified sandbox + the deny list. The unattended-loop tier.
...
Claude Code permission semantics: `deny` rules are mechanically enforced at every tier (block, non-bypassable — this is the only list that binds under `fully-autonomous`); `ask` rules are treated as auto-approved under `bypassPermissions` (the prompt is skipped because all prompts are skipped) and bind only at `semi-auto` (`defaultMode: default`) and `automatic` (`defaultMode: auto`); `allow` rules are always auto-approved. This is corroborated internally by `.claude/rules/moai/development/coding-standards.md` §Bash Risk-Amplifier Doctrine (3), which imposes an agent-side "explicit confirmation EVEN in `bypassPermissions`" obligation precisely because the runtime does NOT enforce `ask` under bypass. Consequence: the SPEC's framing that fully-autonomous safety rests on "deny/ask + sandbox" is corrected to "the verified sandbox + the `deny` list"; `ask` is advisory-only under `fully-autonomous`. The rule-set enumeration (REQ-004) is unchanged — only the per-tier binding semantics are clarified.
