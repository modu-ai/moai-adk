...
canary_compliance_check:
  ...
  ac_ibx_009_scope_guard_resync: "re-measured at sync close (acceptance §D.4): git diff --name-only 918bacd2c..HEAD -- internal/cli/update/ internal/template/ → 0 rows (grep cross-check 0 matches, this run); independently re-measured by sync-audit at e7dc2627d against 131daa290 → zero matches for internal/cli/update/ and the template tree (relayed verdict)"
