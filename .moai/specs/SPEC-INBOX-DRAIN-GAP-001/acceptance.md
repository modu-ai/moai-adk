...
- Marker directory created between the stat and the rotate → worst case one rotation lands on a would-be curator machine. Measured post-rotation dynamics (`drain.sh:64-75`): the no-op branch preserves the stale offset (`offset_after: $OFFSET_BEFORE`) while the fresh live file's line count stays at or below it, so the wrapper no-ops until the fresh file outgrows the stale offset; when the drain resumes it slices `tail -n +(offset+1)`, so stubs in the fresh file at or below the stale offset are permanently unread by the curator — a bounded loss (≤ one cap generation ≈ 4.2k lines per the t259 figures in spec.md §B), not self-healing. Residual risk recorded, not guarded; the collector deliberately never touches `drain-offset.json` (NFC-4 — t259 offset semantics).
- Marker directory deleted on a curator machine → the cap arms (marker absent) and the next over-cap append rotates unread stubs into archives the curator will not read; exposure is bounded to the gap before the next session start, where `session_drain.sh:85` (`mkdir -p "$STATE_DIR"`) recreates the state dir and the wrapper drain resumes ownership. Consistent by definition — drain absent means the cap owns the inbox — not a t259 violation.
