...
priority: P2
phase: "v3.1.0 target"
module: "internal/template/templates/.claude (skills/moai/workflows/e2e.md, agents/moai/e2e-tester.md, commands/moai/e2e.md.tmpl) + local .claude siblings"
...
| Agent pair `e2e-tester.md` (local vs template) | byte-identical (`diff` exit 0) | byte-identical after edits |
...
### Group B — e2e-tester per-OS recipes (both trees)

- **REQ-DNE-100** (Ubiquitous): The e2e-tester `### desktop-native` stub shall be replaced with per-OS recipe subsections (macOS / Windows / Linux), each carrying: default + fallback toolchain, install commands, version probes, accessibility-permission prerequisites, and artifact conventions, in BOTH trees.
...
- **REQ-DNE-103** (Event-driven): **When** the macOS Accessibility (TCC) permission is not granted to the executing terminal/host process, the e2e-tester shall surface the System Settings grant path and return a structured blocker report — it shall not silently fail and shall not prompt the user.
...
- **C-2** [HARD] NO new agent file — reuse e2e-tester. Adding an agent breaks the CI pins (`expectedAgentCount = 10` in `catalog_tier_audit_test.go`) and the 66-cell tier matrix (`model_policy_test.go`). Body-only edits require no `catalog.yaml` entry/count changes and no `model_policy.go` changes (the mechanical `hash:`-line regen of existing `catalog.yaml` entries via `make build` is permitted — REQ-DNE-302 carve-out).
- **C-3** [HARD] Thin Command Pattern preserved: command body <20 non-empty LOC.
- **C-4** [HARD] Subagent boundary: e2e-tester never prompts users; missing prerequisites (Accessibility/TCC permission not granted, toolchain absent) produce blocker reports / orchestrator-surfaced install commands per the parent REQ-E2E-006 pattern.
