...
This section is to be filled by manager-develop during M3:

- **Helper file placement choice** (inline vs new files): _[fill at M3]_
- **`package quality` env var literal vs import** (CLAUDE_PROJECT_DIR literal chosen because): _[fill at M3]_
- **slog.Warn vs slog.Info for cwd_fallback**: _[fill at M3 — recommend Warn per REQ-HCWA-008]_
- **REQ-HCWA-011 inventory file**: 1 location per test file with line numbers, or aggregate count only: _[fill at M3]_
- **Other deviations from plan**: _[fill at M3]_
