...
- **sync_commit_sha**: `fe1fa0508` — the sync commit cannot cite its own SHA, so it landed first with the standard placeholder `pending-backfill-sync`, resolved in this backfill commit (spec-frontmatter-schema.md § SHA placeholder backfill exemption).
