...
| 12 AC-LNC independent verifiability | 12 MUST-PASS AC (AC-LNC-001 through AC-LNC-012) have grep/test/file-existence commands per acceptance.md §B. AC-LNC-012 retrospectively transitioned SOFT → MUST in v0.1.6 reconcile (PR #1048 generic-cover via `isUserOwnedNamespace()` REQ-UNP-009 pattern at `internal/cli/update.go:1156-1228`). AC-LNC-006 binding reverted to REQ-LNC-011 only in iter-3 (REQ-LNC-014 deleted). AC count 11 → 12. | confirmed (plan-auditor iter-3 + v0.1.6 retro-reconcile) |
...
| AC-LNC-012 | PASS | `grep -nE "REQ-UNP-009: user direct-added agents" internal/cli/update.go` + `git log --oneline --grep "UPDATE-NAMESPACE-PROTECT" \| head -5` (PR #1048 `767bc04a4` confirmation) | REQ-LNC-009 runtime invariant (`moai update` PRESERVE `.claude/agents/local/`) is empirically covered by `isUserOwnedNamespace()` REQ-UNP-009 generic pattern at `internal/cli/update.go:1156-1228`: `.claude/agents/<custom>` where `<custom>` ∉ {core, expert, meta, harness} and NOT prefixed with {moai-, moai, manager-, expert-, builder-, evaluator-} → returns `true`. `local` segment matches trivially. PR #1048 merged 2026-05-23 covers the behavior BEFORE LNCO-001 plan-phase began (2026-05-25). v0.1.6 retro-reconcile: SOFT → MUST PASS. |

**Definition of Done assessment**: 12/12 MUST-PASS AC verified PASS (post-v0.1.6 reconcile — AC-LNC-012 transitioned SOFT → MUST retrospectively per PR #1048 generic-cover via `isUserOwnedNamespace()` REQ-UNP-009 pattern). Run-phase complete per spec.md §D Definition of Done.
...
run_status: PASS  # 12/12 MUST-PASS AC (post-v0.1.6 reconcile: AC-LNC-012 retrospectively transitioned SOFT → MUST per PR #1048 isUserOwnedNamespace REQ-UNP-009 generic-cover); pre-existing unrelated test failures NOT my scope
ac_pass_count: 12
ac_fail_count: 0
ac_deferred_count: 0  # AC-LNC-012 retrospectively transitioned SOFT → MUST PASS in v0.1.6 reconcile per PR #1048 generic-cover
...
| 0.1.6 | 2026-05-25 | orchestrator | retro-reconcile | AC-LNC-012 retrospective SOFT → MUST PASS transition per Sprint 10 lane B 2nd SPEC entry L62 paste-ready memo verification discovery (2026-05-25 post-Mx). Ground truth: SPEC-V3R6-UPDATE-NAMESPACE-PROTECT-001 was authored 2026-05-23 (`ff93ccdbb` plan-phase, Tier M, 4 artifacts) AND implemented as PR #1048 (`767bc04a4` feat — "User-Owned Namespace Protection + Backup Standardization") BEFORE LNCO-001 plan-phase began (2026-05-25). The Go function `isUserOwnedNamespace()` (`internal/cli/update.go:1156-1228`) introduces REQ-UNP-009 generic clause covering `.claude/agents/<custom>` for non-{core,expert,meta,harness} and non-{moai-,moai,manager-,expert-,builder-,evaluator-} prefixes → `local` segment matches trivially → returns `true` → `moai update` PRESERVE behavior holds. REQ-LNC-009 runtime invariant empirically covered without explicit `local/` enumeration in UPDATE-NAMESPACE-PROTECT-001 spec.md body. Changes: §E.1 row 12 description updated (12 MUST-PASS, retro-reconcile annotation); §E.2 AC-LNC-012 row DEFERRED → PASS + verification command updated from forward-ref grep to inline REQ-UNP-009 cite + PR #1048 commit confirmation; §E.2 Definition of Done assessment "11/11" → "12/12"; §E.3 yaml metrics run_status PASS-WITH-DEBT → PASS, ac_pass_count 11 → 12, ac_deferred_count 1 → 0. Files changed (3): acceptance.md (frontmatter v0.1.2 → v0.1.3, §A row 12 severity + leader paragraph + §B AC-LNC-012 section + §D DoD item 1 + HISTORY v0.1.3), progress.md (this — frontmatter v0.1.5 → v0.1.6 + §E.1 + §E.2 + §E.3 + HISTORY v0.1.6), MEMORY.md (new entry + new memory file `project_aclnc012_retrospective_reconcile.md`). 0 .go modification (strict Strategy A scope). Doc-gap (UNP-001 spec.md body does NOT explicitly enumerate `.claude/agents/local/`) anchored as SHOULD-FIX low-priority chore in MEMORY.md — no separate follow-up SPEC required. L62 paste-ready memo verification 자체 validated (3rd case after WORKFLOW-PLAN + LNCO-001 plan-phase). L65 NEW (proposed): **retrospective AC transition pattern** — when a SOFT (deferred) AC's substantive verification anchor is found to be empirically covered before the deferred follow-up SPEC's body explicitly enumerates the case, transition SOFT → MUST PASS via retro-reconcile chore rather than waiting for body-level enumeration. |
