...
- **Given** 런처 분류기를 **디스크 내용과 무관하게** 지정한 상태를 반환하도록 스텁
- **When** 실제 `.codex/` 는 완전(S6 배치)한데 스텁이 `not wired` 를 반환하게 하면
- **Then** 제안이 **뜬다** — 구현이 디스크가 아니라 분류기의 반환값을 따랐다는 증거.
- **And Given** 실제 `.codex/` 는 비어 있는데(S2 배치) 스텁이 `wired` 를 반환하게 하면
- **Then** 제안 **없이 기동한다** — 반대 방향으로도 반환값을 따른다.
- **And** 이 두 칸을 여섯 상태 전부에 대해 돌려, 게이트 결과가 **디스크가 아니라 반환 상태의 함수** 임을 고정한다.
...
- **And** E1 에서 계약 쓰기 함수의 호출 횟수가 **0** 이다 — 생성기가 실패했는데 계약을 이어 쓰지 않는다.

## AC-CI-011 — 경로 봉쇄: 심링크 탈출 (REQ-CI-011)

스냅샷 판정만으로는 **심링크가 프로젝트 밖에 쓰는 것을 못 잡는다** — 링크 자체는 프로젝트 안에 있기 때문이다. 구조로 판정한다.

- **Given** 프로젝트 밖 임시 파일에 `SENTINEL-OUTSIDE-3k3` 를 심고, 프로젝트 안의 `AGENTS.md` · `CLAUDE.md` · `CLAUDE.local.md` **각각** 을 그 파일로 향하는 심링크로 만든 fixture 3종
- **When** 초기화를 실행하면
- **Then** 세 경우 모두 **바깥 파일이 바이트 무변경** 이다 (링크를 통해 쓰지 않았다).
- **And** 두 하네스 어느 진입 파일에서도 `SENTINEL-OUTSIDE-3k3` 에 도달하지 않는다 (링크를 통해 읽어 import 하지 않았다).
- **And** 각 경우가 비성공 종료 + 해당 경로를 명명하는 진단으로 끝나고, exec 0회다.
- **And Given** 프로젝트 **안** 을 가리키는 심링크 fixture
- **Then** 같은 판정이 적용된다 — 일반 파일이 아니면 경로 밖이든 안이든 거부한다 (판정 기준이 `Lstat` 상 일반 파일 + 해석 경로 봉쇄이지, 목적지가 어디인지가 아니다).
