...
| 0.3.0 | 2026-08-25 | 확인 감사 D1 · D2 반영 (두 건 모두 문서 간 모순). D1: `plan.md` §F 검증 레시피가 같은 라운드에 신설된 AC-CI-009 를 위반(금지된 `-run 'Codex'` 이름 정규식)해 패키지 통째 `-v` + 서브테스트 계수 + windows 봉쇄 실행으로 재작성. D2: 정의 2 의 `--spawn` 전칭 선언을 네 AC 가 지키지 않아, 기동 단언을 담은 행렬(AC-CI-011 포함)에는 축을 곱하고(45 → 180칸) 담지 않은 셋은 근거를 적어 명시적으로 제외했으며, 그 제외를 지탱하는 단언을 AC-CI-004 수락에 칸 증가 없이 얹었다. 계수 재산정: 219 → 354, windows 하한 27 → 108. REQ 무변경 |
