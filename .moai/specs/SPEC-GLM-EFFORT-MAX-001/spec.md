...
|---------|------|--------|-------|
| 0.1.0 | 2026-08-22 | manager-spec | Initial plan-phase draft (Tier S, card t175). Ground truth: `.moai/reports/t175/measurements.md` (committed measurement record) + direct code reads at worktree HEAD. Carries two `[NEEDS CLARIFICATION]` markers (session-default ratification, template doc-block scope) resolved via plan.md before Implementation Kickoff Approval. |
...
### §1.3 Cross-SPEC conflict (load-bearing)

**SPEC-GLM-EFFORT-REBALANCE-001** (status: `in-progress`, phase "v3.1.0 target") REQ-GER-004 requires `SessionGLMReasoningState()` to return `reasoning-high` — its Change (2) stepped the value down from `max` to `high`, and that step-down has already landed in the tree (the current hardcode + rewritten cost-policy comment are its product). This SPEC's REQ-GEM-002 **reverses REQ-GER-004** on the operator's newer directive. The reversal is lead-ratified at Implementation Kickoff Approval (plan.md §D-1, `[NEEDS CLARIFICATION]` marker); reconciliation of REBALANCE's own frontmatter/body is Out of Scope here (§5).
...
**REQ-GEM-002** (State-driven): **While** a GLM-backed session derives its session-global reasoning state with no effort preference set, the deriver `SessionGLMReasoningState()` shall return the `max` reasoning state (thinking enabled, `reasoning_effort: max`) — superseding the `high` floor and reversing SPEC-GLM-EFFORT-REBALANCE-001 REQ-GER-004, per the operator's directive (lead-ratified; plan.md §D-1).

**REQ-GEM-003** (Capability gate): **Where** the `glmReasoningHigh` package variable becomes unreachable from both the collapse and the session default, the implementation shall remove that variable, and shall retain the `GLMStateHigh` and `GLMReasoningEffortHigh` constants (the settings-widget domain and the z.ai wire vocabulary keep `high` as a selectable stored state — `GLMReasoningStateNames()` and `internal/settings/schema_sections.go` continue to reference them).
...
**REQ-GEM-005** (Ubiquitous): The cost-policy comment on `SessionGLMReasoningState` (`:182-199`) shall be rewritten to record the operator directive superseding the `high`-floor rationale (citing the t127 trivial-spawn measurement); the UNVERIFIED shim-consumption comments (`glm_effort_overlay.go:194-196`, `internal/cli/glm.go:386-391,:414-417`) shall be updated to cite the t175 measured finding (shim honors the `thinking` parameter; top-level `reasoning_effort` ignored); and the collapse-documentation block in the template mirror `internal/template/templates/.moai/config/sections/llm.yaml` (`:16-23`) shall state the new mapping, also correcting its pre-existing stale "thinking off" floor line (scope rider: plan.md §D-2).
...
**AC-GEM-003** — Given the post-change tree, when `go build ./...` and `golangci-lint run ./internal/template/...` run, then both are clean — no unused-variable finding for `glmReasoningHigh` (removed), and `GLMStateHigh`/`GLMReasoningEffortHigh` still present and referenced (`GLMReasoningStateNames()` at `:136`; `internal/settings/schema_sections.go:184`).
...
### Out of Scope — SPEC-GLM-EFFORT-REBALANCE-001 reconciliation

- This SPEC reverses REBALANCE's REQ-GER-004 on the operator's newer directive, but does NOT edit REBALANCE's frontmatter (`partially_superseded_by`), body, or progress — REBALANCE is `in-progress` in another lane and its disposition (amend / partial-supersede / close-as-overtaken) is the lead's call after ratification.
