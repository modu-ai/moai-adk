...
| 0.1.2 | 2026-09-02 | manager-spec | §1.2 measured ground-truth correction (in place; status stays `completed`). The §1.2 first bullet and the REQ-GEM-005 parenthetical stated the t175 direction ("shim honors `thinking`, ignores top-level `reasoning_effort`") as MEASURED; that direction was superseded by the t225 measurement (2026-08-24, null-controlled differential on the same Anthropic-compat endpoint) and directly re-measured by card t442 on 2026-09-02 (36 API calls, 2 models, n=3-5 per condition: the endpoint reads top-level `reasoning_effort`; a thinking-budget request object shows no depth scaling; the effect direction of `low` is model-dependent, so no per-value ordering is asserted). The t175 record itself is preserved (not deleted) at `.moai/reports/t175/measurements.md`. Re-measurement evidence: `.moai/reports/t442/verdict.md`. |
...
- **AC-MTP-032b residual is now MEASURED**: the z.ai Anthropic-compat shim READS the top-level `reasoning_effort` request field (measured 2026-09-02 by card t442 — 36 API calls, 2 models, n=3-5 per condition: material reasoning-depth shifts on both glm-5.3-flash and glm-5.3; corroborated by the earlier null-controlled differential of t225, 2026-08-24; full verdict at `.moai/reports/t442/verdict.md`), while a `thinking` budget request object shows NO depth scaling (budget 512 vs 4096 flat-to-inverted on flash: median thinking-chars 1221 vs 698), and `max` ≈ omit-default holds on both models. The pre-revision wording of this bullet recorded the superseded t175 finding ("the shim honors `thinking` and IGNORES a top-level `reasoning_effort` field") — that direction is not the current truth and this SPEC asserts no per-value ordering or grade semantics, because the effect direction of `low` is model-dependent (flash: low < null; glm-5.3: low > null); the t175 record itself is preserved at `.moai/reports/t175/measurements.md`. The live session env carries `ANTHROPIC_REASONING_EFFORT=high`, matching the code hardcode (injection verified); the env→reasoning chain rides the `reasoning_effort` channel now measured effective, so the session's own thinking-block responses remain indirect end-to-end evidence that the injection is the live reasoning channel this collapse rides on.
...
- **llm.yaml `glm.effort` block is stored-only** — no code reads it at runtime (`internal/settings/schema_sections.go:192-204` documents the fields as store-only; the single runtime channel is the session-global `ANTHROPIC_REASONING_EFFORT`). Its `reasoning-max` label's channel assumption (a z.ai-native `reasoning_effort` field) is CONFIRMED by the later measurements (t225, 2026-08-24; card t442, 2026-09-02) — the top-level `reasoning_effort` field is the live reasoning channel, so the stored-only block's label matches the measured channel (probe P2's contrary reading is superseded along with the §1.2 first bullet's correction). The recorded config-code divergence note (stored-only, no runtime reader) stays accurate and unchanged.
