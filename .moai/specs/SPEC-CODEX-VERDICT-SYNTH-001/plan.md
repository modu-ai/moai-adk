...
### C.5 [비결정] 보수 채택 순위 테이블 — 요구사항 아님

현 구현은 대입 순서로 보수 채택을 달성하고 있고, 실측표에 반례가 없다(spec.md §A.5). 순위 테이블(`verdictRank`/`moreConservative`)로 바꾸는 것은 **선택 사항**이며, C.1 의 fall-through 수정이 신호를 셋으로 늘리는 과정에서 자연스럽게 필요해지면 그때 도입한다. 필요 없으면 도입하지 않는다 — 결함이 아닌 것을 고치는 데 감사 예산을 쓰지 않는다.
...
- RED 먼저: acceptance.md §B corpus 를 순회하는 속성형 테스트. 사전구현 트리에서 C1·C5·C7 이 실패해야 한다

### M2 — 점수 표기 인식기 (REQ-CVS-002)
...
| 위험 | 완화 |
|---|---|
| native clean-pass 를 깨뜨려 Stop 훅 게이트가 정상 변경을 막음 | AC-CVS-003. M1 RED 에 포함 |
...
**안티패턴 3 — 감지만 하고 관대한 쪽 채택**: 불일치를 기록만 하고 `pass` 를 반환하는 구현. AC-CVS-004 가 **채택값** 을 걸어 잡는다.
