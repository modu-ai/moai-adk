...
| `status` is read from the card's branch | measured, §B / `spec.md` §A.4a | branch resolved via `SPEC-KANBAN-WORKTREE-001` `REQ-KW-003`; no branch name is derived here; a card with no branch reads the primary checkout |
...
- Record the creating process in the board-wide lock artifact and provide the bounded clearing operation, conditioned on that process being positively observed absent **and** on the artifact still being the one inspected. Construct the release-and-recreate interleaving; a clear that completes through that window is the defect (REQ-KB-023, `spec.md` §D.12).
...
- Read each card's `status` from the card's **branch** by blob read, without a checkout, resolving the branch through `SPEC-KANBAN-WORKTREE-001` `REQ-KW-003` and deriving no branch name here; fall back to the primary checkout only where the card has no branch; observe committed state only (REQ-KB-020, `spec.md` §A.4a). This lands **before** the table below, which is unusable without it: with a primary-side read, every card in `run` pairs as `(run, draft)` and the table refuses it.
