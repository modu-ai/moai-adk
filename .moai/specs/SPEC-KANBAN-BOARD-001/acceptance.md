...
tags: "kanban, board, acceptance, given-when-then, verification, sole-writer, atomicity"
tier: L
---
...
**AC-KB-002** (REQ-KB-005) — *Given* a session running inside a worktree, *when* the board root is resolved, *then* it equals the primary checkout's path recorded by `plan.md` §C commands 2 and 2b — the parent of the **absolute** common git directory — and the board file resolves beneath that root's `.moai/state/kanban-board/` through **this package's own** path-segment constant, distinct from `internal/factory/record.go`'s per-tree `stateDirSegments`, which is not referenced from the board package at all. Decided by comparing the resolved value against the recorded one, and by scanning the board package for a reference to the session-record constant, which must yield none.
...
**AC-KB-022** (REQ-KB-020) — *Given* a card in the `run` column whose **branch** carries `status: in-progress` while the primary checkout's copy of the same `spec.md` still reads `status: draft`, *when* the board reconciles the card, *then* it reads `in-progress`, reports the card **consistent** and dispatchable, and does **not** report it inconsistent on the strength of the primary-side value; and *given* a card with no branch — a `backlog` or `plan` card, and equally a card whose branch was deleted after merge — *then* its `status` is read from the primary checkout and judged by the same table; and *given* a transition written into a worktree's working tree but not committed, *then* the board does not observe it.
...
*Given* a board-wide lock artifact left behind by a process that has terminated, *when* the clearing operation is invoked, *then* the artifact is removed, the removal is reported, and a subsequent board mutation acquires the lock. *Given* an artifact whose recorded process is **live and holding the lock**, *when* the same operation is invoked, *then* nothing is removed and the operation reports the refusal — the age of the artifact having no bearing on either outcome. *Given* an artifact inspected by the clearing operation and then, **before the removal**, released by its owner and re-acquired by a live process, *when* the removal step proceeds, *then* it aborts and the re-acquired lock survives.
...
- All 24 criteria executed, with each one's command and verbatim output recorded in `progress.md` §E.2.
...
**Reconciliation.** 23 requirements, `REQ-KB-001` … `REQ-KB-023`, each appearing exactly once in the left column. 24 criteria, `AC-KB-001` … `AC-KB-024`, each appearing at least once in the middle column. Both sit under the Tier L ceiling of 25, with two requirements and one criterion to spare — reported rather than smoothed, because Tier L is the top tier and a further finding of this size would have to be reported rather than absorbed.
...
- How the `lead` role is elected, assigned, or handed over, and by what mechanism a session's role declaration is carried. AC-KB-017's runtime half **reads** the declaration of `SPEC-KANBAN-BOOTSTRAP-001` `REQ-KS-006` and judges the refusal; it judges neither the declaration's carrier nor its correctness, and who becomes the `lead` is `REQ-KS-004`.
