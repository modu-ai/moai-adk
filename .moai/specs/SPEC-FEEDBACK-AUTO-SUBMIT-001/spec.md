...
- **2026-08-22** v0.5.0 — plan-audit iter3 **FAIL 0.863**(Tier L 임계 0.85 — 집계는 넘겼으나 미해소 선행 결함 조항이 구속), must-pass 7/7. **Tier L 재시도 상한(3회) 도달 → 운영자 에스컬레이션**, 결정은 **"기계적으로 증명된 2건만 지금 고치고 나머지는 M6 부채로 문서에 박아 넘긴다"**. 이번이 마지막 plan-phase 개정 회차이며 재감사는 없다. 점수 추이 0.75 → 0.837 → 0.863(단조 상승), iter1 블로킹 11건 중 10건 종료.
  - **지금 고친 것 — AC-F-019의 ④⑤ 토큰 교체(증명된 공허)**. 감사관이 미변경 트리에서 실측: `grep -c 'feedback-draft'` → **1**(소스/미러 양쪽), `grep -c 'conversation_language'` → **8**. 즉 두 관측은 **스킬 본문을 한 글자도 고치지 않아도 통과**했다 — ④의 1건은 `feedback.md:40`의 *기존* 초안 저장 줄이지 REQ-9의 실패-부류 분기가 아니고, ⑤의 8건은 이슈 제목·템플릿 헤더 언어 규칙이지 게이트 라벨 의무가 아니다. 관측이 아니라 장식이었다. 교체: ④ → `queue.json`, ⑤ → `label`↔`conversation_language` **동일 줄** 공존 검사(`label` 단독은 라벨 매핑 6건에 걸려 벡터가 못 된다). **둘 다 base에서 0/0 실측 확인.** 더불어 **"기준 토큰은 base에서 0을 반환해야 한다"는 규칙 자체**를 AC 옆에 [HARD]로 적고 `git show <base>:<path>` 확인 명령을 함께 남겼다 — 이번 실수의 원인이 그 확인의 부재였기 때문이다.
  - **M6 부채로 이관한 것 2건**(고치지 않음, 문서에 박음): **(i) ②의 앵커 부재** — `grep -c -- '--title'` 가 파일 어디의 `--title` 이든 잡으므로, 본문만 스크럽하고 `gh issue create --title` 을 쓰는 구현이 통과한다(스크러버가 제목을 못 받은 채로). **(ii) ③의 한국어 리터럴** — 보조 검사가 영어 전용 + 템플릿 미러 대상을 한국어 토큰으로 겨냥한다. 둘 다 **스킬 절의 실제 모양을 알아야 제대로 재작성**할 수 있어 M6 착수 시점이 맞는 자리다. 감사 인용 문단을 **영어 원문 그대로** `acceptance.md`의 AC-F-019 바로 위와 `plan.md` M6 두 곳에 넣고, 지우지 말라는 [HARD]와 "M6의 Exit는 그때까지 관측이 아니라 주장"이라는 시점 조건을 함께 걸었다.
  - ③에 대해: 같은 형태를 형제 SPEC(`SPEC-TODO-ENABLE-FLAG-001`)에서는 이미 진단해 제거했는데(행동 왕복 관측에 위임) **같은 커밋이 여기엔 남겼다** — 그 사실을 부채 문단에 적어 M6 재작성 시 형제의 해법을 참고하도록 했다.
  - 선택 항목(N3~N8)은 상한 회차 판단으로 여전히 미처리.
