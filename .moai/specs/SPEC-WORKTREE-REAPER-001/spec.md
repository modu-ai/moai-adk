...
- **2026-09-02** — requirement revision v0.4.1 (card t231, requirement-text change only; 25 requirements and 28 criteria unchanged, no id added or renumbered). **REQ-WR-016 revised**: an unreadable anchor (lock) source is neither an abort nor a silent success — the caller-facing `clean` command shall complete its report (keeping every tree, anchors undetermined) and end with the distinguished preservation signal, process exit code 2. The gap this closes was measured, not assumed: `cmd/moai/main.go` resolves an intentional exit code only through the `ExitCoder` chain (`internal/cli/exitcode.go`), so every error return from `clean` collapsed to exit 1 and a degraded run was indistinguishable from a clean one by exit status — the t209 CR OD-2 request ("exit 2 on lock-source-unreadable") was correctly ruled unwireable at the time and this revision makes it wireable. The non-blocking clause is unchanged and still binds `prMergeCleanup` (internal consumer; its callers receive a function return, not a process exit), and its covering criteria (AC-WR-024, AC-WR-024b) are untouched. Implementation: `cleanMergedWorktrees` aborts with the signal; `cleanStaleWorktrees` and `reportStaleWorktrees` complete their report/JSON first, then return it via the shared `ExitCodeError` vehicle (`Detail` field added; guard wording unchanged when unset).
...
- **REQ-WR-016** (Ubiquitous) — The sweep shall not abort its caller on any
  failure; every failure path shall remain a non-blocking notice, and no
  failure path shall remove a tree the sweep could not judge. An unreadable
  anchor (lock) source is neither an abort nor a silent success: the
  caller-facing `clean` command shall complete its report — keeping every
  tree, with anchor states reported as undetermined — and end with the
  distinguished preservation signal, process exit code 2, so a degraded run
  is never mistaken for a clean one by exit status alone. (Revision, card
  t231, 2026-09-02: exit 2 was previously unreachable — any error return
  collapsed to exit 1 at cmd/moai/main.go. The non-blocking clause is
  unchanged and still binds the internal `prMergeCleanup` consumer, whose
  caller-facing surface is a function return, not a process exit; what the
  revision forbids is destroying work or silently succeeding, not
  distinguishing the outcome.)
