...
- **(iii) 합성 프로세스 트리 — 끼어든 셸을 건너뛴다**: 변수를 비우고, 조상 표를 두 모양으로 차례로 주입한다 — 트리 1: moai 400 → bash 300 → bash 200 → 호스트 100(명령 이름 `claude`), 트리 2: moai 401 → bash 201 → 호스트 100. 트리 1 로 파손 Stop 한 번, 트리 2 로 파손 Stop 한 번을 부르면 두 호출이 **같은 셈 기록**을 쓰며 그 연속 횟수가 2 다. 이 시험은 해석을 상수로 대신하지 않고 실제 조상 탐색을 합성 표 위에서 돌린다(plan.md §C 6).
  **필수 RED 변이(이음매 안)**: 주입된 이음매 안에서 래퍼 셸을 건너뛰지 않는 구현 — 주입된 조상 표의 첫 부모를 그대로 열쇠로 쓰는 구현 — 은 트리 1 에서 300, 트리 2 에서 201 을 열쇠로 삼아 기록이 둘로 갈리고 각 연속 횟수가 1 이 되어 빨개진다.
  **정적 검사(이음매 밖)**: 원시 `os.Getppid()` 를 쓰는 변이는 주입된 조상 표를 읽지 않고 테스트 프로세스의 실제 부모를 읽으므로 두 트리에서 같은 값을 내 위 시험이 초록으로 남는다. 그래서 따로 잡는다 — `grep -c 'os.Getppid' internal/cli/hook_stop_parse_cap.go internal/cli/hook_stdin_failclosed.go` 가 두 파일 모두 0 이다. 형식은 AC-SPC-005 의 정적 검사와 같다: 대상은 정확히 이 두 파일이고, 둘째 파일이 없으면 판정 불가로 보고한다(0 을 통과로 읽지 않는다).
...
**Given** plan.md M5a 의 상한 선언(1회, `--max-turns 2`, `timeout -k 10 120`)과 AC-SPC-015 와 같은 배포 래퍼 사슬(대리 스크립트는 유효 stdin 을 그대로 통과), **When** 워크트리 빌드 바이너리로 `claude -p` 를 한 번의 복합 호출 `unset CLAUDE_CODE_SESSION_ID MOAI_SESSION_PID && timeout -k 10 120 claude -p … --output-format json`(plan.md M5a) 으로 1회 실행해 Stop 이 한 번 이상 불리면 — 바깥 세션에서 물려받은 두 변수는 같은 호출 안에서 지워지므로 훅 환경에 남은 값은 중첩 호스트가 스스로 찍은 것뿐이다 — **Then** (a) 대리 스크립트의 측정 로그에 `hook stop` 호출마다 `CLAUDE_CODE_SESSION_ID` 의 상태(`set-nonempty` / `set-empty` / `unset`)와 값이 한 줄씩 남아 있고 — 이 로그는 `exec` 직전의 같은 프로세스에서 쓰였으므로 `moai hook stop` 프로세스의 환경이다 — (b) 상태가 `set-nonempty` 인 모든 줄의 값이 **그 실행의** `session_id`(같은 실행의 결과 JSON `session_id`, 또는 같은 호출이 받은 유효 stdin 페이로드의 `session_id`)와 같다. 값이 존재한다는 것만으로는 통과가 아니다 — `set-nonempty` 이지만 값이 그 실행의 `session_id` 와 다르면 이 AC 는 실패이며, 그 값은 1순위 열쇠의 증거로 쓰지 않는다. 상태가 `set-empty` 나 `unset` 이면 (b) 는 해당 줄이 없어 성립하고, 그 관측값(호스트가 훅 환경에 세션 id 를 찍지 않는다)을 그대로 기록한다. 스크럽 없이 얻은 `set-nonempty` 는 증거로 쓰지 않는다. 관측값은 progress.md §E.2 에 원문으로 남기고 spec.md §A.4 의 「관측하지 않았다」 문장을 D-NEW-1 로 갱신할 입력이 된다. 열쇠 종류(세션/프로세스)와 moai 의 열쇠 선택의 대응은 이 AC 가 아니라 AC-SPC-007 (i)(ii) 가 단위 수준에서 고정한다.
**RED 경로**: 현재 코드에는 열쇠 해석이 없으므로(원장 L5) 이 측정은 M1 이 REQ-SPC-014 를 구현한 뒤에만 의미가 있다. 이 AC 의 판정 자료(대리 스크립트 로그와 결과 JSON)는 moai 의 출력이 아니라 호스트가 훅 환경에 무엇을 찍는지이므로, RED 는 코드 변이가 아니라 값 불일치(예: 스크럽을 빼 바깥 세션 값을 물려받은 실행)로만 관측된다. 실행하지 못하면 Gap 이 아니라 blocker 로 보고한다.
