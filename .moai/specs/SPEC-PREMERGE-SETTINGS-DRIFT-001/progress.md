...
`63e8e900d` (M1) touched two doctrine files under `.claude/rules/moai/workflow/`: `kanban-dispatch.md` (+2 lines, the acquire-precondition mention) and `kanban-dispatch-detail.md` (the fuller mechanism description). Both carry a mirror under `internal/template/templates/.claude/rules/moai/workflow/`, and both mirrors were edited in the same M1 commit (confirmed above by `git diff --stat 256b30fa5..HEAD`, which lists all four paths, and by the `diff -q` byte-identity check in `canary_compliance_check`). `CLAUDE.local.md` is deliberately local-only per `CLAUDE.local.md` §2 (Local-Only Files list) and carries no template counterpart — its M1 addition needs none. No embedded-template content changed beyond the two rule-file mirrors, so `make build` was not required for this sync commit (the `.claude/rules/` tree is deployed as source files under `internal/template/templates/`, not regenerated by `make build`; `make build` only recompiles the Go binary with the `//go:embed all:templates` directive picking up the current template-source bytes at compile time — verified: `git diff --stat` shows no `internal/template/embed.go` or catalog change, and no new template file was added that would need a fresh embed).

### Attribution for consumed (not re-executed) §E.2 run-phase evidence

Sync phase did not re-run the `internal/kanban` / `internal/cli` / `internal/config` test suites §E.2 already exercised. That is an attributable-diff-check consumption (`agent-common-protocol.md` § Parallel Execution → Attributable diff-check doctrinal switch), not an omission, and the attribution is measured here rather than asserted:

```
$ git diff --stat 63e8e900d..HEAD
 .../SPEC-PREMERGE-SETTINGS-DRIFT-001/progress.md   | 144 ++++++++++++++++++++-
 .../specs/SPEC-PREMERGE-SETTINGS-DRIFT-001/spec.md |   4 +-
 CHANGELOG.md                                       |   1 +
 3 files changed, 144 insertions(+), 5 deletions(-)
```

`63e8e900d` is the M1 implementation commit the §E.2 evidence was measured against. Every commit since (`f1df50d52`, `dc67db6f7`, and this sync phase's own commits) touches only `progress.md`, `spec.md` frontmatter, and `CHANGELOG.md` — zero `.go` files, zero test fixtures, zero production code. The §E.2 AC matrix, mutant table, and coverage figures therefore remain attributable to the current HEAD without re-execution.
