...
Version: 0.6.0 | Tier: L | Status: draft | 26 requirements · 36 acceptance-criterion leaves (over the Tier L ceiling of 25 — declared, see `spec.md` §D)
...
- `go test ./...` exits 0 **except for the single scope-bounded pre-existing failure** `TestRunHarnessObserveStop_ProposeChainAutoRuns`. Judged by `go test ./... 2>&1 | grep '^--- FAIL'` returning exactly that one line; a second distinct `--- FAIL` line is a FAIL. The exclusion rests on provenance (red at the pre-SPEC baseline `7171880a9`; `git diff --name-only 7171880a9..HEAD -- internal/` touches no file in its code path), not on a root-cause diagnosis — see `acceptance.md` AC-FM-025 § Scope bound (v0.6.0) for the runnable form, the environment-sensitivity contradiction, and why fixing the test is deferred to a separate SPEC.
...
- Mirror all five edited/new `.claude/` files into `internal/template/templates/.claude/`, stripped of internal tokens; `make build`.
...
- **AP-16 — Writing an acceptance criterion whose baseline was never measured (v0.3.0; third shape added v0.5.0, fourth added v0.6.0).** Four shapes have now been found in this SPEC: an absence grep run against a tree with pre-existing matches (AC-FM-025's mirror clause), a presence grep whose pattern already matches (AC-FM-007), — found during run-phase, after M5 landed — **a coverage floor set above the package's actual starting point** (AC-FM-025's coverage conjunct: 90% asserted on a package measured at 76.3-76.4%), and **a green-suite assertion made against a repository that already has a red test** (AC-FM-025's `go test ./...` exits-0 conjunct, with `TestRunHarnessObserveStop_ProposeChainAutoRuns` red at the pre-SPEC baseline). Measure in §C pre-flight, assert a delta against the measured value, and never let "fixing" a criterion mean editing unrelated shipped files. The third shape is the one that survives plan-audit most easily, because a coverage number reads as a policy target rather than as a claim about this tree; the fourth survives for the same reason and one worse — "all tests pass" reads as so obviously correct that nobody checks whether it is true of the starting tree. Two corollaries learned while correcting them: when a measured baseline is noisy, anchor the delta to the **lowest** observation — a floor set at the highest observation fails on jitter and is unfalsifiable in the opposite direction; and when bounding around a pre-existing failure, name the **individual failing test**, never a failure count or a blanket "known failures" clause, or the bound becomes a hiding place for the next regression.
