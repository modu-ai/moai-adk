...
**And (known limitation, pinned — not a pass condition for survival)** a project name init accepts but does NOT store verbatim — one carrying a YAML backslash escape (typed `a\\b`, a literal `a\nb`, `Kim \"Goos\"`), which the unescaped `project.yaml.tmpl` stores as a different string — is erased to `""` by the update while `user.name` survives; this is pinned so a change in either direction is visible, and the follow-up is to escape identity values in the init project template. **And** hand-edited unrenderable names survive the clean-reinstall path, whose call site uses the same identity loader.

**Verification command**:
```bash
go test ./internal/config/ -run '^TestLoadIdentity_ReturnsStoredValueVerbatim$' -count=1 -v
go test ./internal/cli/ -run '^(TestIdentityLoader_AcceptedNamesRenderVerbatim|TestUpdateForce_UnrenderableIdentityNamesSurvive|TestUpdateForce_InitOriginNamesSurvive|TestCleanReinstall_InitOriginNameSurvives|TestCleanReinstall_HandEditedUnrenderableNamesSurvive|TestUpdateForce_KnownLimitation_InitEscapedProjectName)$' -count=1 -v
# Expected: both exit 0; output contains the seven top-level '--- PASS:' lines
# (TestUpdateForce_KnownLimitation_InitEscapedProjectName pins the limitation; TestCleanReinstall_HandEditedUnrenderableNamesSurvive covers the clean-install loader call site)
```
