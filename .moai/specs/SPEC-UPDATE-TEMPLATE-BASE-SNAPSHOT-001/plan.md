...
>
> **Companion change (same fix, not part of D4)**: the update render contexts now carry `ProjectName` / `UserName` loaded from the existing config (`internal/config/loader_identity.go` `LoadProjectName` / `LoadUserName`, precedent `LoadGitMode`), so unchanged identity renders identically and the merge never has to rely on reading a name as a customization against the snapshot. The loaders return the stored value verbatim; the update's accept rule (`internal/cli/update_identity.go` `loadUpdateIdentity`) then keeps a stored name only if the embedded `project.yaml.tmpl` / `user.yaml.tmpl`, rendered through the same renderer the update deploys with (a context carrying the name, the project root, and the version), renders without error AND the output parses back to exactly that name — otherwise the render carries `""`; an empty name passes through. Because init renders names the same way, every name init stored verbatim — the stored value equals what the render produces for it — is kept, so the snapshot BASE equals both the new render and the user's value. A dropped name is one no render could have written (a hand edit such as `$TEAM`, `{{.Version}}`, `"`, `\`, a raw newline), so the snapshot BASE differs from the user's value — and that difference is precisely the condition under which the merge keeps the user's value as a customization instead of taking the empty render (AC-TBS-027). *(Rewritten 2026-09-24, card t1139: the earlier character filter over-rejected names init had written — e.g. `cost$5`, `a{{b`, a ZWJ emoji — whose BASE equalled the user's value, so the merge could not keep them as customizations.)*

> **Known limitation (card t1139, sync-audit iter3 N3; not mitigated)**: the guarantee covers names init stored verbatim, not every name init accepts. `project.yaml.tmpl` renders `name: "{{.ProjectName}}"` without escaping, so a project name containing a YAML backslash escape (typed `a\\b`, a literal `a\nb`, `Kim \"Goos\"`) is stored by init as a DIFFERENT string that does not round-trip. The accept rule drops it, the update renders `""`, and because the snapshot BASE equals that stored value, the merge reads the user value as unchanged and takes `""` — `project.name` only; `user.name` survives because init rewrites `user.yaml` with a different writer. This is not a regression: before the fix every update erased every name. Pinned by `TestUpdateForce_KnownLimitation_InitEscapedProjectName` (`./internal/cli/`), which fails loudly once init stops altering such names. Follow-up: escape identity values in the init project template so the stored value round-trips.
>
