...
### C.1b run 단계에서 확정된 두 가지 (근거는 `spec.md` 가 보유)

| 결정 | 요지 | 근거 위치 |
|---|---|---|
| INIT 게이트의 인자 | `-w` 가 있어도 게이트에는 **프로젝트 루트**를 넘긴다. 자식의 `Dir` 만 워크트리를 따른다 — 배선은 프로젝트의 속성이고, 링크된 워크트리에 `.codex` 사본이 있다는 보장이 없어 워크트리를 넘기면 배선된 프로젝트를 미배선으로 오판해 기동을 막는다 | `spec.md` §D.4 REQ-CLV-009 옆 normative note |
| `-C, --cd` 미사용 | codex 에 `-C/--cd` 가 **있지만** 쓰지 않는다. 합성 토큰을 argv 에 넣지 않는다는 REQ-CLV-004 와 같은 규율이며, 작업 디렉터리는 `exec.Cmd.Dir` 로 넘긴다 | `spec.md` §B B9 |

두 항목 모두 **이유가 규칙과 같은 자리에** 있다. 커밋 본문이나 레인 보고서에만 있으면 다음 독자가 조용히 뒤집고 스스로 일관성을 고쳤다고 믿는다 — REQ-CLV-008 의 `moai cc` 비대칭 기록과 같은 이유로 SPEC 본문에 올렸다.
