...
|---|---|---|---|
| AC-HYG-001-001 | REQ-HYG-001-001 | `N=$(go test ./internal/cli/ -list 'Characterization' \| grep -c '^Test'); echo "matched=$N"; test "$N" -ge 17 && go test ./internal/cli/ -run 'Characterization' -count=1` && `MAX=$(for f in internal/cli/update.go internal/cli/update_*.go; do case "$f" in *_test.go) continue;; esac; wc -l < "$f"; done \| tr -d ' ' \| sort -rn \| head -1); echo "max_production=$MAX"; test "$MAX" -le 1200` | Characterization suite PASS with a non-vacuity guard: the matched-test count must be ≥ 17 (`matched=17` observed post-split — 11 pre-existing + the 6 added by M1). The count assertion is load-bearing: `go test` exits 0 on a selector matching nothing (`[no tests to run]`), so a selector that silently stops matching — a rename, a deleted file — FAILS this AC instead of passing vacuously. Second half: every update-cluster **production** file ≤ 1,200 lines; `max_production=1005` (`update.go`) post-split. `_test.go` files are excluded by the `case` filter — the ceiling governs production files, and `update_test.go` (2,901 lines) is out of its scope. |
