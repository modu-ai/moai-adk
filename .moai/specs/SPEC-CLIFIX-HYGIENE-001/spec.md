...
| 0.2.1 | 2026-07-30 | manager-spec | In-flight AC-wording correction (run-phase, status stays `in-progress`; no implementation change). Two `acceptance.md` §D rows asserted more than the tree satisfies, so the written verification was narrowed to the scope actually implemented and guarded. **AC-HYG-001-003**: the old row ran a broad `grep -rn 'ANTHROPIC_\|CLAUDE_' internal/cli` and expected "env-name literals outside `envkeys.go` reduced to 0"; that grep in fact returns 137 lines (57 raw env-name literals), because M3 (`2e0d75585`) deliberately scoped the work to the three GLM env-var NAMEs. What actually ships and is guarded: `config.GLMEnvVarSet()` (`internal/config/envkeys.go:122`), the parity test `TestGLMEnvSetParity`, and the AST gate `TestNoBareGLMEnvVarLiteralsInCLIProduction` (`internal/cli/glm_env_parity_test.go:66`). The row now asserts exactly that; the `ANTHROPIC_*` family and `CLAUDE_PROJECT_DIR` are recorded as out of scope. **AC-HYG-001-005**: the old `web_port*.go` glob also matched `internal/cli/web_port_test.go`, so the command returned 1 file against an expected 0; all four production files are clean. The row now resolves its file list through `git ls-files` with a `_test.go` filter (rg's `-g` glob does not filter explicitly-passed paths), keeping the expected outcome at 0, and records that the residual Hangul lives in test comments and `t.Fatalf`/`t.Errorf` strings — governed by `code_comments`, not `error_messages: en` — under the existing §C `Out of Scope — Broader i18n sweep`. The user approved narrowing the AC wording to the real scope rather than widening the implementation. No REQ text, no other AC row, and no `.go` file changed. |
| 0.2.2 | 2026-07-31 | manager-spec | AC-HYG-001-001 verification-command correction (sync-phase, no implementation change). The row's first half ran `go test ./internal/cli/ -run 'UpdateCharacterization' -count=1`, a selector that matches **zero** tests — there is no `TestUpdateCharacterization*`; M1's tests are named `TestCleanLegacyHooks_*Characterization` and `TestRunAgencyMigrationAdapter_*Characterization`. Go exits 0 on a `-run` selector that matches nothing (`ok … [no tests to run]`), so the command passed vacuously on any tree, including one with the characterization net deleted outright. The **substantive verification always existed** and always passed: 17 `Characterization` tests in `internal/cli` (11 pre-existing + 6 added by M1), green both pre- and post-split, recorded in `progress.md` §E.2 — only the AC's written command was wrong, never the underlying evidence. The corrected row runs `-run 'Characterization'` behind a matched-count assertion (`test "$N" -ge 17`), so a selector that silently stops matching — a rename, a deleted file — now FAILS the AC rather than passing. The second half (file-size ceiling) was rescoped: its loop included `_test.go` files, which legitimately exceed 1,200 lines (`update_test.go` = 2,901), while the ceiling governs production files; the loop now filters test files and the expected outcome states the satisfied post-split figure (`max_production=1005`, `update.go`) rather than the stale pre-split 1,905 baseline. The defect was surfaced by manager-docs during sync-phase and recorded in `progress.md` §E.2 Gaps + the CHANGELOG follow-ups before this correction — it was never silently passed. No REQ text, no other AC row, and no `.go` file changed. |
