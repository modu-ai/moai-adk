...
5. 변경 범위 unit/race/vet 후 t1074 regression과 MCP catalog invariant(총수는 `.claude/rules/moai/core/moai-mcp-tools.md`가 정하고 `internal/mcp/catalog_test.go`의 `wantCatalogSize`가 고정한다. 쓰기/읽기 구성은 `internal/mcp/catalog.go`의 도구별 `WriteCapable` 표시가 정하고 `TestMoaiMCPTools_WriteCapableSet`이 고정한다)를 실행한다.
