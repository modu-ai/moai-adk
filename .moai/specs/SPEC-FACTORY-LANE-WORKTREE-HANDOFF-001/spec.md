...
| Version | Date | Change |
|---|---|---|
| 0.5.12 | 2026-09-25 | 카드 t1170. 카드 t1143(SPEC-CODEX-AUDIT-READONLY-001)이 MCP catalog를 39개(쓰기 15, 읽기 24)로 올려 AC-FLH-015 Then 절의 `36 total/14 write/22 read` 리터럴이 현재 값과 어긋났다. 새 리터럴로 다시 고정하지 않고, catalogue SSOT(`.claude/rules/moai/core/moai-mcp-tools.md` + `moai-mcp-tools-catalogue.md`, `internal/mcp/catalog_test.go`가 고정)를 참조하도록 바꿨다. 다음 catalog 변경이 이 SPEC에 다시 드리프트를 만들지 않게 하기 위해서다. 같은 리터럴을 쓰던 plan.md 형제 문장도 같은 방식으로 맞췄고, spec.md의 plan 시점 관측(36/14/22)은 이력으로 남기고 현재 개수는 SSOT가 정한다는 괄호만 덧붙였다. 판정식(AC-FLH-015 bash 블록)과 AC 개수는 바꾸지 않았다. progress.md의 관측 기록도 손대지 않았다. 카드 t1145의 SPEC 본문에서 옛 catalog 개수 36/14/22를 적은 여섯 곳은 t1145가 develop을 흡수할 때 스스로 고치며, 여기서는 바꾸지 않았다. 같은 버전 안의 후속 수정(delta plan-audit `.moai/reports/t1170/plan-audit.md`, PASS-WITH-DEBT 0.86): D1 — 규칙 파일은 총수(39)만 선언하므로, 총수의 출처는 `moai-mcp-tools.md`와 `wantCatalogSize`로, 쓰기/읽기 구성의 출처는 `internal/mcp/catalog.go`의 `WriteCapable` 표시와 `TestMoaiMCPTools_WriteCapableSet`으로 나눠 적었다(acceptance.md·plan.md·spec.md 괄호). D3 — research.md의 「현재 분류는 write 14/read 22」를 plan 시점 관측으로 주석했다. D4 — t1145 관련 문장을 「36/14/22 유지」 문구 여섯 곳에서 옛 개수를 적은 여섯 곳으로 고쳤다(그 문자열 자체가 있는 곳은 두 곳뿐이다). |
...
- 현재 MCP catalog는 단위 검증에서 36개(쓰기 14, 읽기 22)로 관측됐다(plan 시점 관측이다. 현재 총수는 `.claude/rules/moai/core/moai-mcp-tools.md`가, 쓰기/읽기 구성은 `internal/mcp/catalog.go`의 `WriteCapable` 표시가 정하며, 카드 t1143이 39개(쓰기 15, 읽기 24)로 올렸다). 이 SPEC은 새 MCP tool을 기본 해법으로 추가하지 않는다.
...
- REQ-FLH-014(LIVE cross-harness proof)와 AC-FLH-012·013은 0.5.11에서 이 SPEC 범위 밖(§ Out of Scope — LIVE cross-harness proof)으로 옮겨 t1145에 넘겼다. 이 SPEC은 이 요구사항을 충족한다고 주장하지 않는다. 근거는 progress.md § M5 lane record다.
...
### Out of Scope — LIVE cross-harness proof (카드 t1145로 이관, 0.5.11)

- 리드 결정 (a)(sync-audit `.moai/reports/t1082/sync-audit-deep.md` F1)에 따라 REQ-FLH-014(Live cross-harness proof)와 이를 검증하는 AC-FLH-012·013을 이 SPEC 범위에서 빼고 카드 t1145에 넘겼다. 이 SPEC은 이 요구사항을 충족한다고 주장하지 않는다.
- REQ-FLH-013의 LIVE 검증 다리(AC-FLH-012·013)도 함께 넘어갔다. 이 SPEC 안에서 REQ-FLH-013은 AC-FLH-011로 검증한다.
- 두 AC의 원문(Given/When/Then, 명령, jq predicate, 기대 결과)은 acceptance.md § Exclusions에 그대로 있다.
- 이관 전 추적표 행은 `` | § REQ-FLH-013 | AC-FLH-011, AC-FLH-012 (`NOT_RUN → t1145`), AC-FLH-013 (`NOT_RUN → t1145`) | ``와 `` | § REQ-FLH-014 | AC-FLH-012 (`NOT_RUN → t1145`), AC-FLH-013 (`NOT_RUN → t1145`) — t1145까지 미충족 | ``였다.
- 이관 전 요구사항 원문(제목 `REQ-FLH-014 — Live cross-harness proof`)은 아래 인용과 같다.
