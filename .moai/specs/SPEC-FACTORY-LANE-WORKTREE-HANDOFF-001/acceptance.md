...
| AC-FLH-019 | REQ-FLH-018 | `TestFactoryLaneHandoffRebindVsUserPromptRegisterRace` | 비종결 handoff 동안 같은 owner·새 session UUID의 UserPromptSubmit registration이 별도 핸들에서 경합해도 tombstone·receipt 없는 endpoint 이동 0, 단조 generation, 패자의 이름 있는 거부다. |
...
AC-FLH-017/018 두 행은 위 subject tree가 아니라 t1074 흡수 후 HEAD `1487f97a0`에서 2026-09-23에 측정했다. AC-FLH-019 행은 HEAD `d28ed9aa4`에서 2026-09-23에 측정했다. 세 행 모두 RED 이유는 named test 부재다.
...
- In every forced and unforced run, while the handoff is non-final, no committed transaction other than R's rebind changes the endpoint. Every endpoint change during that window is accompanied by exactly one tombstone and one BOUND receipt. The generation never decreases across observed commits. Each unforced run ends in exactly one of the (i) or (ii) end states.

Each forced interleaving must be observed exactly as forced — the recorded start order of the two write transactions and the commit order must equal the forced order. An order that was not actually observed is a FAIL, never a vacuous PASS. The separate-handle requirement and its three-part proof from AC-FLH-018 apply verbatim to H and R, including the shared-pair guard probe and the `RACER_HANDLES_DISTINCT=2` log line. The test runs under `-race`.
