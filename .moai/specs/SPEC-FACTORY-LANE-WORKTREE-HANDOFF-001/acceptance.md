...
**Given** existing t1074 broker/roster/receipt/SessionStart/MCP catalog fixtures plus filesystem/process inventory, **when** handoff additions run, **then** legacy behavior remains passing, the MCP catalog total and write/read split equal the counts declared by the catalogue SSOT (`.claude/rules/moai/core/moai-mcp-tools.md` + `moai-mcp-tools-catalogue.md`, pinned by `internal/mcp/catalog_test.go`) and the registered server matches it — this SPEC adds no MCP tool, and a count change is authorized only by a card that updates that SSOT — and exactly zero new broker DB roots, daemons, private sockets, or parallel message stores exist.
