...
- **handoff에 production 진입 경로가 없다 (0.5.10, 알려진 한계).** handoff controller와 재기동 reconciler는 테스트 밖에서 부르는 곳이 없다. 그래서 이 SPEC이 착지해도 handoff는 운영자가 쓸 수 있는 기능이 아니고, 쓸 수 있으려면 카드 t1145가 필요하다. REQ-FLH-014는 t1145까지 충족되지 않으며 AC-FLH-012·013은 `NOT_RUN → t1145`다. 자세한 내용은 spec.md § Known limitation과 progress.md § M5 lane record에 있다.
