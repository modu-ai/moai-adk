...
| 3 | SessionStart bind | `BindLaunchPending` (`store.go:392`) | `internal/hook/factory_messages.go:84` | pending row만 CAS로 bound 전환, 그 외 no-op(`store.go:419`). t1074 best-effort 선행 결합자, 순서 무관. 호출 session UUID에 tombstone이 하나라도 있으면(generation 무관) 행 판독 전에 `STALE_ENDPOINT`로 거부하고 무쓰기(이 SPEC, REQ-FLH-010) |
...
**Tombstone된 session의 launcher 재결합 처리 선택 (REQ-FLH-010, 결정 D1=(b)).** 델타 plan-audit이 관측한 부활 경로는 이렇다. handoff로 결합된 owner가 죽은 뒤 launcher가 `RegisterLaunchPending`(`store.go:435`)으로 새 launch-pending 행을 commit하면, 이전 session을 resume한 프로세스의 SessionStart가 `BindLaunchPending`(`store.go:446`)으로 tombstone된 UUID를 그 행에 결합했다(호출부 `internal/hook/factory_messages.go:53` `registerFactoryHookPeer`의 SessionStart 분기). `BindLaunchPending`은 tombstone을 읽지 않고, stale 분류기 `staleOrUnregistered`(`handoff_bind.go:421`)는 `(session_uuid, generation)` 쌍으로 맞추므로 새 generation의 같은 UUID를 놓친다. 리드 결정으로 REQ-FLH-010을 좁히지 않고 이 경로를 막는다.

- 판독 위치와 키: `BindLaunchPending`은 자기 write transaction을 시작한 직후, 행을 읽고 바꾸기 전에 `sessionTombstoned`(`handoff_bind.go:500`, `session_uuid`만으로 조회)로 tombstone을 읽는다. 행이 launch-pending인지와 무관하게 tombstone이 있으면 거부한다. 거부 여부가 행 상태에 따라 갈리지 않으므로, SessionStart와 UserPromptSubmit이 어떤 순서로 발화하든 같은 session은 같은 결과를 받는다(UserPromptSubmit은 이미 `refuseTurnRegistrationDuringHandoff`에서 같은 판독으로 거부된다).
- 거부 오류: 새 sentinel을 만들지 않고 `STALE_ENDPOINT`(`StaleEndpointError`)를 재사용한다. 호출자에게 필요한 정보가 turn 등록 경로의 거부와 같기 때문이다. 이 session은 영구히 교체됐고, lane의 현재 endpoint는 redirect에 있다. 이 시점의 현재 endpoint는 launcher 등록이 만든 launch-pending 행이므로, 비공개 토큰 규칙에 따라 redirect의 session UUID는 빈 값이고 generation만 담긴다(`currentEndpoint`, `handoff_bind.go:89`).
- 거부 뒤 launch-pending 행의 종착 상태: 거부 transaction은 아무것도 쓰지 않으므로 행은 launcher 등록이 commit한 그대로다(비공개 토큰·generation·PID·process-start 불변). 이 행은 거부된 프로세스가 owner로 쥐고 있고, 그 owner가 t1074 live-owner 규칙으로 current가 아니게 되면 다음 launcher 등록이 t1074 dead-owner 재등록으로 대체한다. 세 선택지 가운데 이것이 가장 단순하고 다른 규칙과 부딪히지 않는다. 행을 지우는 rollback은 slot을 빈 상태로 만들어 다음 등록이 generation 1부터 다시 시작하게 하므로(`RegisterPeer`의 `p.Generation < 1` 보정) REQ-FLH-017의 「generation이 줄지 않는다」를 깨고, 과거 tombstone의 `(session_uuid, generation)` 쌍과 generation이 겹칠 수도 있다. 행을 「닫힌」 상태로 바꾸는 선택은 peers에 없는 새 상태와 스키마 변경을 요구한다. 「거부는 무쓰기」라는 점에서 이 SPEC의 다른 모든 거부와 모양이 같다는 것도 근거다.
- REQ-FLH-017 종착 불변식과의 정합: 고아는 owner가 current가 아닌데도 남은 launch-pending 행이다. 거부된 행은 살아 있는 owner가 쥐고 있고 owner가 사라지면 대체되므로 고아가 아니다. 같은 행은 결합되지 않은 채 lane의 유일한 현재 endpoint이므로 「결합된 현재 endpoint 정확히 하나」 조항에도 걸린다. REQ-FLH-017 본문은 이 경우를 두 조항에 똑같이 걸리는 예외 하나로 적었다. owner가 current인 동안 lane의 현재 endpoint는 정확히 하나이고 그것은 결합되지 않은 이 행이며, owner가 current가 아니게 되면 새 launcher 등록이 대체한다.
- 운영자 신호: SessionStart hook은 store의 이 거부를 SessionStart 분기의 일반 오류 처리(`factory messaging degraded: <err>`)로 흘리지 않고, turn 등록 거부와 같은 `factoryHandoffRegistrationNotice`(`internal/hook/factory_handoff_bind.go:92`)의 endpoint-replaced 안내문을 세션 additional context에 싣는다. `moai factory` 상태 표면의 lane roster는 그 lane의 `binding_state`를 `launch_pending`으로 보고한다(값은 `store.go:41` `BindingLaunchPending = "launch_pending"`, 판정은 `laneRoster` `store.go:926`; 기존 동작, 변경 없음). 기존 안내문(`factory_handoff_bind.go:92`)의 문구는 `factory endpoint replaced STALE_ENDPOINT: slot=<slot> is current at <session> generation <n>; this session receives no factory messages`뿐이다. 이 session이 factory 메시지를 받지 않는다고만 알리고, session을 끝낸 뒤 launcher로 다시 시작하라는 조치는 담고 있지 않다. 이 SPEC은 문구를 바꾸지 않는다. 그 조치는 sync 단계 사용자 문서가 안내한다. `<session>` 자리에는 redirect의 session UUID가 들어간다. 현재 endpoint가 launch-pending 행이면 비공개 토큰 규칙에 따라 그 값이 빈 문자열이어서 `is current at  generation 4`처럼 빈칸 두 개로 렌더된다(델타 plan-audit 탐침 관측). 현재 endpoint가 handoff로 결합된 행이면 그 session UUID가 들어간다(예: `is current at post-cd-uuid generation 3`, 0.5.8 개정 탐침 관측).
...
- **거부된 resume session이 살아 있는 동안 lane은 주소를 받지 못한다 (결정 D1=(b)).** launch-pending 행이 current인 동안 `ResolveLane`과 송신 검증은 `ErrEndpointLaunchPending`을 돌려주고, 새 handoff는 REQ-FLH-016에 따라 `ENDPOINT_LAUNCH_PENDING` NACK를 받으며, 새 launcher 등록은 t1074 live-owner 규칙으로 거부된다. 운영자가 그 session을 끝내야 lane이 풀린다. 두 번째 writer가 생길 여지를 남기기보다 이쪽을 택한 것이 REQ-FLH-010의 결정이다. 신호는 SessionStart 안내문과 roster의 `launch_pending` 상태뿐이며 자동 종료 수단은 없다. 안내문은 조치를 지시하지 않고, 이 경우 session 자리가 빈 값으로 렌더된다(`is current at  generation <n>`). 그래서 운영자가 안내문만 보고 무엇을 해야 하는지, 어느 endpoint가 현재인지 알기 어렵다. 빈 session 렌더링은 비공개 토큰 규칙을 지키는 기존 동작이며, 이 SPEC은 이를 새 요구로 만들지 않고 잔여 위험으로 남긴다. roster의 `launch_pending`도 정상적인 launch 대기와 구별되지 않는다.
...
- **handoff에 production 진입 경로가 없다 (0.5.10, 알려진 한계).** handoff controller와 재기동 reconciler는 테스트 밖에서 부르는 곳이 없다. 그래서 이 SPEC이 착지해도 handoff는 운영자가 쓸 수 있는 기능이 아니고, 쓸 수 있으려면 카드 t1145가 필요하다. REQ-FLH-014와 AC-FLH-012·013은 0.5.11에서 이 SPEC 범위 밖으로 옮겨 t1145에 넘겼다(spec.md § Out of Scope — LIVE cross-harness proof). 자세한 내용은 spec.md § Known limitation과 progress.md § M5 lane record에 있다.
...
| BOUND로 tombstone된 session UUID가 launcher resume 뒤 SessionStart `BindLaunchPending`으로 결합을 시도 | 같은 transaction에서 session UUID만으로 tombstone을 읽고(generation 무관) `STALE_ENDPOINT` 거부, 무쓰기. launch-pending 행은 launcher 등록이 commit한 그대로 두고, owner가 current가 아니게 되면 새 launcher 등록이 대체. 같은 session의 UserPromptSubmit도 `STALE_ENDPOINT`. SessionStart 안내문(기존 endpoint-replaced 문구, 조치 지시 없음)과 roster `launch_pending`이 운영자 신호. 이미 BOUND인 행에서도 같은 거부, 무쓰기 |
