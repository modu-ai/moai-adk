...
- REQ-FLH-017 「고아 launch-pending 없음」과의 정합: 고아는 owner가 current가 아닌데도 남은 launch-pending 행이다. 거부된 행은 살아 있는 owner가 쥐고 있고 owner가 사라지면 대체되므로 고아가 아니다. REQ-FLH-017 본문에 이 경우를 명시했다.
- 운영자 신호: SessionStart hook은 지금처럼 `factory messaging degraded: <err>`를 내지 않고, turn 등록 거부와 같은 `factoryHandoffRegistrationNotice`(`internal/hook/factory_handoff_bind.go:92`)의 endpoint-replaced 안내문을 세션 additional context에 싣는다. `moai factory` 상태 표면의 lane roster는 그 lane을 `binding_state=launch-pending`으로 보고한다(`laneRoster`, `store.go:926`; 기존 동작, 변경 없음). 안내문이 운영자에게 요구하는 조치는 이전 session을 resume하지 말고 그 session을 끝낸 뒤 launcher로 새 session을 시작하는 것이다.
