...
Claude Code sub-agent가 매번 fresh context로 spawn되는 본 프로젝트 환경에서, "auto memory mount" 환상을 포기하고 orchestrator가 sub-agent prompt 작성 시 SPEC progress, related memory, domain lessons를 명시 주입하는 표준 정책을 신설한다. 5KB 토큰 cap + 우선순위 정책으로 context overflow를 방지한다.
...
- 5KB 토큰 cap per agent invocation 명시
...
- A2: 5KB cap은 sub-agent 토큰 예산의 ~5%로 합리적 fit
...
- **REQ-CI-006**: WHEN `.moai/specs/<SPEC-ID>/progress.md` exists for the active SPEC, THE ORCHESTRATOR SHALL inject its content into the sub-agent prompt subject to the 5KB cap.
- **REQ-CI-007**: WHEN injected context approaches the 5KB cap, THE ORCHESTRATOR SHALL apply truncation per the priority order, retaining the highest priority entries first.
...
- **REQ-CI-011**: IF the cumulative injected context exceeds the 5KB cap after applying priority order, THEN THE ORCHESTRATOR SHALL truncate the lowest-priority entries first and emit a non-blocking note in the prompt.
...
- **REQ-CI-013**: THE ORCHESTRATOR SHALL NOT inject raw memory file contents that exceed the 5KB cap (truncation per REQ-CI-011 mandatory).
...
- C3: 5KB cap은 default; 사용자 설정 가능 여지 명시 (`.moai/config/sections/observability.yaml.context_injection.cap`)
