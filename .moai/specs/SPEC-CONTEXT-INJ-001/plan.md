...
orchestrator의 sub-agent spawn prompt 작성 시 context 명시 주입 정책 표준화. 5000-token cap + 3-tier 우선순위 (progress.md > MEMORY.md > domain lessons). 코드 변경 없이 정책 문서 + cross-ref만으로 일관성 확보.
...
- [ ] 5000-token cap의 적정성 검토 (sub-agent 평균 초기 토큰 measure)
...
- [ ] `.claude/rules/moai/development/context-injection.md` 신규 작성
  ...
  - §2 5000-token Token Budget Cap 명시 (unit: tokens, see SPEC §1)
...
- [ ] 정책 문서 ~400-500 tokens 범위로 압축 (~1.5-2KB chars)
...
**Exit Criteria**: SKILL.md cross-ref + 5000-token cap 명시
...
- [ ] `.claude/rules/moai/development/context-injection.md` §5에 권장 schema 작성:
  ...
  - Related SPECs
  ```
- [ ] 250-750 tokens (1-3KB chars) 권장 사이즈 명시
- [ ] 권장 강조: hard rule 아님
...
- [ ] 5000-token cap 준수 sample 5 sub-agent 호출 측정
...
If sum > 5000 tokens → truncate from lowest priority

## 7. Anti-Patterns
- Injecting raw secrets
- Cross-agent private memory without consent
- Injecting >5000 tokens
...
.claude/rules/moai/development/context-injection.md (5000 tokens cap, 3-tier priority).
...
|------|-------------|--------|-----------|
| 5000-token cap이 부족하여 핵심 누락 | Medium | High | priority + truncation 전략 + 사용자 설정 가능 cap |
...
- **OQ1** (5000-token cap LSP 검증): 불가 (텍스트 정책), 자동화는 후속 SPEC
