...
> 공허성 봉투(감사 D3): 이 검사가 보완하는 기존 테스트는 기수를 단언한다 — `internal/template/agentemit/golden_test.go:284-285` 의 `if count != 11 { … want 11 }`(측정: 48c35a4d4; 이 트리 이 실행에서 `ls internal/template/templates/.codex/agents/moai/*.toml | wc -l` → `11` 로 재확인). 새 검사에 대응 조항이 없으면, 추출이 부분 성공해 일부 경로만 비교해도 통과하고, 바이너리 부재 시 "비교 0건 → exit 0" 이 규정상 허용된다. 둘 다 이 SPEC 이 근절하겠다고 선언한 실패 유형 그 자체다.
