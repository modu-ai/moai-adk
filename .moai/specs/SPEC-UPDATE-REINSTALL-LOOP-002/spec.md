...
| 0.4.0 | 2026-07-31 | Post-run documentation revision (D10, D13, D14, D16, D17), documentation-only — no Go source, test, or template file changed; all four run-phase milestones already landed green. **D16 (major)** closed: `REQ-RIL2-003` accepted `v` **or** `V`, which contradicted both `AC-RIL2-001`'s nine-row table and the §A residue-free widening table (each pinning `V2.5.0` to `Signal 1 = false`) and would have widened the destructive path that `NFR-RIL2-001` forbids without exception. The shipped implementation is lowercase-only (`internal/cli/v2_detection.go:225` `case major == 2 && prefix == 'v':`), so the requirement text was wrong, not the code; REQ-RIL2-003 is narrowed to the lowercase `v` prefix, `REQ-RIL2-006` now explicitly claims the uppercase-`V`-prefixed major-2 case, and §A prose, §G risk row 1, and plan.md §A D1 are aligned. **D17 (minor)** closed: `REQ-RIL2-022`'s "second run removes zero" is not literally true for an `.agency/`-carrying tree, because `.agency.archived` is itself a `defs.DeprecatedPaths` entry (`internal/defs/dirs.go:140`) and the migration copies rather than moves (`internal/cli/migrate_agency.go:244`); the REQ now scopes its zero-removal claim to paths present before the first run and records the bounded three-run `.agency` convergence with its cause (the migration is gated on `.agency/` existing, `migrate_agency.go:195`), and `AC-RIL2-012` states its non-`.agency` fixture scope. The behaviour is accepted; no code change. **D10 (minor)**: `acceptance.md` §A items reordered 1,2,5,3,4 → 1,2,3,4,5 by relocating item 5 to the tail without renumbering, so the sole `§A.N` cross-reference (§C.2's `§A.4` git-stash pointer) still resolves. **D13 (minor)**: the "verbatim" `stripRetiredV2DenyEntries` grep block was missing its 8th line (`68:\treturn nil`); the command was re-run against the current tree and the true output restored. **D14 (minor)**: `NFR-RIL2-004` rewritten from a noun phrase into shall-form with content unchanged. |
...
The last two columns are why REQ-RIL2-003 is scoped to the lowercase-`v`-prefixed form alone (see §D.1). Both `2.5.0` and `V2.5.0` on a residue-free tree are `IsV2=false` today — the pre-change rule was the case-sensitive literal `strings.HasPrefix(v, "v2.")`, which matched neither; making either form Signal-1 positive would flip it to `true` and send a project with no legacy residue into the destructive full reinstall — precisely what NFR-RIL2-001 forbids.
...
- **REQ-RIL2-003** — **Where** the normalized major version is exactly 2 **and** the original string carried a leading lowercase `v`, the version probe shall report Signal 1 positive and shall not report v3-confirmed.
  - **Prefix scope is deliberate, not an oversight — and it is lowercase-only.** Both a bare `2.x` (no prefix) and an uppercase-prefixed `V2.x` are Signal-1 **negative** today and stay negative — see REQ-RIL2-006, which claims both. The pre-change rule was the case-sensitive literal `strings.HasPrefix(v, "v2.")`, so the uppercase form was already negative; admitting it here would be a widening, not a preservation. Making either form positive would flip a residue-free project from `IsV2=false` to `IsV2=true`, which NFR-RIL2-001 forbids (§A, residue-free table — the `V2.5.0` row is `Signal 1` false, `IsV2` false today). The asymmetry against REQ-RIL2-007 (which does accept the bare `3.0.1` and the uppercase `V3.0.1`) is intentional: widening *v3-confirmed* narrows the destructive path, whereas widening *Signal 1* broadens it. Only the second direction is constrained.
  - **Cost of the narrowing is nil in practice.** A genuine v2 project is v2 precisely because it carries v2 residue, so Signals 2/3 detect it regardless of the version string. The narrowing removes only the case "bare `2.x` or uppercase `V2.x` version with no residue whatsoever", which is not a v2 project in any observable sense.
  - **Rejected alternative:** granting NFR-RIL2-001 an explicit exception for `major == 2` and letting the bare or uppercase-prefixed form be Signal-1 positive. Rejected because it converts a monotonicity invariant into a case-by-case judgement, and because the inputs it admits (residue-free `2.5.0` / `V2.5.0`) are the exact shape of the false positive that triggers a destructive reinstall.
...
- **REQ-RIL2-022** — **When** `moai update` runs twice consecutively on a v3-confirmed project with residue, the second run shall remove zero deprecated paths **that were present in the tree before the first run**. A deprecated path created *during* the update sequence by the `.agency/` migration pre-step is excluded from this count; such a path is removed by the following run, and the sequence still terminates (see the `.agency` convergence note below).
  - **`.agency` three-run convergence, and why it is not the #1243 loop.** `.agency.archived` is itself a `defs.DeprecatedPaths` entry (`internal/defs/dirs.go:140`) and the agency migration **copies** rather than moves (`internal/cli/migrate_agency.go:244` `r.copyDir(agencyDir, archiveDir, tx)`), so both `.agency/` and `.agency.archived/` exist after run 1. Run 1 removes `.agency/` (present in the pre-migration snapshot); run 2 removes `.agency.archived/` (now in the snapshot); run 3 removes zero. The sequence terminates because the migration pre-step is gated on `.agency/` existing (`migrate_agency.go:195` returns early when it is absent), so run 2 cannot re-create the archive. Every removal is backed up first (REQ-RIL2-015). This is categorically different from the net-zero infinite loop of issue #1243, where the removal count never reaches zero because the same path is removed and restored every run. The behaviour is accepted as-is; no code change is in scope.
...
- **NFR-RIL2-004** — Every Go file this change set adds or edits shall use a `snake_case.go` filename, shall wrap errors with `fmt.Errorf("…: %w", err)` rather than `%v` or string concatenation, and shall carry English comments and godoc.
...
|---|---|---|
| Normalization misclassifies a genuine v2 project as v3 | False negative — needed migration skipped | REQ-RIL2-003 pins lowercase-`v`-prefixed major 2; the table-driven test asserts the `v2.5.0` row. A bare `2.x` and an uppercase `V2.x` both deliberately fall to REQ-RIL2-006 (Signal 1 negative), where Signals 2/3 still detect the residue every genuine v2 project carries |
