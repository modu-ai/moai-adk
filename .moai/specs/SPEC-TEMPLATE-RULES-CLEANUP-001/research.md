...
- **lessons #N / W# (전수 실측 완료 — plan-audit iter-1 D4 정정)**: 광패턴 `\bW[0-9]` 전수 grep 결과 W#-토큰 5개 라인 = `agent-common-protocol.md:274`(W3 meta-analysis), `manager-develop-prompt-template.md:71`(lessons #21 W0 fix), `:91`(W1/W2), `:221`(W3 케이스), `:222`(W3에서 ~10분); lessons 4개 라인 = `manager-develop-prompt-template.md:71`, `session-handoff.md:291`(#13,#12), `:332`(#14), `spec-workflow.md:51`(#13). 감사 원문의 `mdpt:132` / `acp:409-413`은 실측상 토큰이 아닌 이미-일반화된 프로즈(phantom) — 열거에서 제거.
- **내부 날짜 9건**: `design/constitution.md:12-15`(HISTORY), `:423-424`(footer "Last Updated: 2026-05-20"/"Relocated: 2026-04-20"), `worktree-integration.md:44,46` + `spec-workflow.md:175,269,416` + `session-handoff.md:330`(반복 "2026-05-17 policy"), `zone-registry.md:629-630`(2026-05-04/05-09) — 전건 grep 실측. 추가 실측(plan-audit iter-1 D1): `moai/NOTICE.md:18,91`(2026-04-26)은 rules 트리 **내부**에 위치하나 Out of Scope 이연 대상 — AC-TRC-B4 `--exclude=NOTICE.md` + 가드 (c) allowlist entry로 처리. pedagogical 날짜 3건(`spec-frontmatter-schema.md:190-191` anti-example, `skill-authoring.md:45,89` 형식 예시, `archived-agent-rejection.md:7` 예시 frontmatter)은 라인-컨텍스트 제외 미커버 — design.md §4 사전 등재.
...
- **R2 — C2 백포트는 용어 한정**: 감사가 "누락"으로 기록한 `.moai/research/dive-into-claude-code-archive.md` 참조와 Version/Origin footer의 template 부재는 **sanitized-pair 계약의 의도적 산출** (mirror test 주석 + `sanitized_pair_parity_test.go:71-76` registry 실측). 백포트 대상은 `MoAI`→`moai-adk` 용어 실측 14개 매치 라인(`grep -cE '\bMoAI\b'` — plan-audit iter-1 D8 정정, 감사 원문 "~8곳"은 과소집계) + local 측의 더 중립적인 표현("The orchestrator-interrupt-ledger contract owns...")만. local 측 `CONST-V3R6-001` 토큰은 역방향으로 template에 절대 복사 금지 (REQ-TRC-032).
- **R5 — zone-registry 런타임 소비자 3곳**: `internal/cli/constitution.go:24`(constitutionRegistryRelPath), `internal/cli/doctor.go:580`(not-found 메시지 — graceful), `internal/cli/spec_lint.go:159`(`detectRegistryPath` → 부재 시 `""` 반환 — graceful), `internal/constitution/validator.go`. 템플릿 제거는 사용자 프로젝트의 `moai constitution` 동작에 영향 — scratch 실측 검증 필수 (REQ-TRC-027). constitution 서브커맨드는 4종(list/guard/amend/validate — `constitution.go:35-38` 실측; validate는 registry-load 실패 시 abort 이력)이며 4종 전부 scratch 검증 대상 (plan-audit iter-1 D5). `internal/template/catalog.yaml`에는 zone-registry 미등재 (grep 0 hit) — 카탈로그 변경 불요.
...
- ~~lessons/W# 전수 확정~~ — plan-audit iter-1 D4에서 광패턴 `\bW[0-9]` + `\blessons? #[0-9]+` 전수 실측 완료 (5 W-라인 + 4 lessons-라인; phantom 2건 `mdpt:132`/`acp:409-413` 열거 제거). **해소.**
- ATR/WO/COORD/UNP/LNC/TII prefix allowlist의 정확한 코드 위치 — M1에서 확정 (후보 4파일 좁힘 완료).
- `moai constitution` 4종(list/guard/amend/validate)의 registry-부재 동작 — M3 scratch 실측 전까지 미확정 (doctor/spec_lint는 코드 레벨 graceful 확인; validate는 abort 이력 보유 — D5).
