...
| `workflow/runtime-recovery-doctrine.md` | SANITIZED PAIR | **template 측만** 편집 (백포트도 template 방향) | `sanitizedPairPaths` 8-entry registry |
| `core/askuser-protocol.md`, `core/agent-common-protocol.md` | SANITIZED PAIR (`sanitizedPairPaths` 등재 — plan-audit iter-1 D2 정정, 종전 UNENROLLED 오분류) | template 측 sanitization 편집; identical dual-edit는 normalized parity 보존 시 허용 (`TestSanitizedPairParity` green 필수) | 실측 8-entry registry |
...
| `core/settings-management.md`, `development/sprint-round-naming.md`, `workflow/worktree-integration.md`, `workflow/orchestration-mode-selection.md` | UNENROLLED (기본 동일 편집) | template + local 동일 편집 (드리프트 최소화) | 미래 mirror 등록 용이성 |
...
| # | 리스크 | 완화 |
|---|--------|------|
| R-1 | zone-registry 런타임 소비자 3곳 (`constitution.go:24`, `doctor.go:580`, `spec_lint.go:159`) — 사용자 프로젝트에서 파일 부재 시 동작 | M3에 scratch `moai init` 프로젝트 실측 검증 단계 포함. doctor/spec_lint는 graceful 확인됨; `moai constitution` 4종(list/guard/amend/validate — validate는 registry-load 실패 시 abort 이력) 중 비-graceful 발견 시 최소 Go 수정 추가 (REQ-TRC-027) |
...
| **M3** | Finding B: 중립성 정리 + zone-registry 템플릿 제거 + 참조 정리 + CLI graceful 실측 | template: `askuser-protocol.md`, `settings-management.md`, `sprint-round-naming.md`, `session-handoff.md`(local 동일), `spec-workflow.md`(local 동일), `agent-common-protocol.md`, `manager-develop-prompt-template.md`(template만), `worktree-integration.md`, `design/constitution.md`(날짜), `core/zone-registry.md` **삭제**, `runtime-recovery-doctrine.md`(zone-registry 참조 2곳) | acceptance.md AC-TRC-B1..B8 grep 세트 전부 0-hit; scratch `moai init` 프로젝트에서 `moai constitution` 4종(list/guard/amend/validate) + `moai doctor` graceful 확인 | manager-develop (cycle_type=ddd; graceful 수정 필요 시 해당 부분만 tdd) |
| **M4** | Finding C: 선별 백포트 | template: `spec-frontmatter-schema.md`(§I row), `runtime-recovery-doctrine.md`(용어 실측 14 라인; sanitized 요소 미복사) | `grep -c 'Token Accounting'` ≥1; MoAI-표기 잔존 grep 0; `grep -n '\.moai/research/\|^Version:\|^Origin:\|CONST-V3R'` → 0; `TestSanitizedPairParity` green | manager-develop (cycle_type=ddd) |
