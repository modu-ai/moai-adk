...
### §F.3 The "17 implementation references" figure — reproducible, but not a good AC basis

The brief cited 17 other Go references. It is reproducible with the command the lead subsequently supplied:

```bash
grep -rn '/goal' --include='*.go' internal/ cmd/ pkg/ | grep -v '_test.go' \
  | grep -E 'state/goal|internal/goal|stop-goal|cli/goal\.go' | wc -l
```

Observed: **17**. My earlier statement that the figure was not reproducible from any pattern *I* tried was accurate, but calling it "unattributable" was broader than the evidence supported — the two are different claims, and the narrower one is the true one.

The figure is nonetheless **not** used as an AC basis. A count over a four-alternative grep is brittle: it moves whenever any of the four token families gains or loses an unrelated line, so a drift would fail the AC for a reason unconnected to this SPEC. AC-GSU-027 instead pins two *specific* symbols in the one file where over-deletion would cause harm (`NativeGoalActive` → 2, `native /goal` → 3 in `internal/goal/evaluate.go`), which is the guarantee the brief was asking for and is stable under unrelated change.

### §F.6 A fifth Go surface M7 must own — the v4manifest test fixture

Found at plan-audit iteration 1 (D3).

```bash
sed -n '15,20p' internal/harness/v4manifest/runner_template_test.go
go test ./internal/harness/v4manifest/
```

Observed: line 19 is `{PrimitiveGoal, \`case "/goal"\`}`, and the suite is **`ok`** at baseline. The test asserts both `strings.Contains(RunnerTemplate, tc.marker)` and `strings.Contains(RunnerTemplate, tc.primitive)`. Once M7 sets `PrimitiveGoal = "/moai goal"` and rewrites the dispatch arm, the hard-coded marker `case "/goal"` is absent and the subtest fails — so M7 breaks a currently-green test unless it updates the fixture in the same change.

This was invisible to the original AC matrix: AC-GSU-027's third component excludes `_test.go`, so it would read `0` while the package suite was red. AC-GSU-030 closes it by pinning the suite exit code **and** the updated marker literal.
...
## §G Documentation Measurement — MOVED to `SPEC-GOAL-DOCS-RETIRE-001`

The documentation-layer measurement (50 docs-site files carrying a backticked native-`/goal`; 28 under `claude-code/`, 22 on the MoAI surface; the 13-file affected set; the split-surface line-by-line classification of `autonomous-loops.md`; the four-locale symmetry data) moved with the sync-phase scope after plan-audit iteration 2 emitted STOP. It is re-authored in `SPEC-GOAL-DOCS-RETIRE-001` §B with **per-locale** baselines, which is what closes audit finding N2 (an English-anchored detector that read `en:2 ja:0 ko:0 zh:0` while identical emission content sat in all four locales).

Two results from that measurement are retained here because they bear on this SPEC's own scope:

- **URL-path false positives.** In `docs-site/`, an unbackticked `/goal` search matches link paths such as `/en/cli-reference/goal`. A first pass returned 58 files / 32 `claude-code` pages; the backticked detector returned **50 / 28**. This is the same detector hazard the union form in §E must respect — hence the right-boundary class excluding `/`, `.`, `-`.
- **Locale-invariance is a detector property, not a content property.** A prose-anchored regex can read zero in three locales while the content is present, because only the trigger word is translated. Any detector this SPEC applies to a multi-locale surface must anchor on a code literal or a structural marker. This SPEC has no multi-locale surface after the split, so the constraint is recorded rather than applied.
