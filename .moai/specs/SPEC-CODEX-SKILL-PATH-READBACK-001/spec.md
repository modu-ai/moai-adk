...
- **Doctor side** (`codexStaleSkillFinding`): stat is a DIRECT `os.Stat` (M4; `osStatFn` count in `doctor_codex.go` = 0). No recorder can be placed there. **Adding a seam at ~:857 to manufacture symmetry is FORBIDDEN** — it is production-code change owned by card t563 (lane-7, already dispatched); adding it silently in this card FAILS the DoD (AC-CSRB-007 pins the zero with a positive control).
...
- No `osStatFn` (or any other injection seam) is introduced at `internal/cli/doctor_codex.go` ~:857. AC-CSRB-007 pins the zero with a positive control; adding the seam quietly FAILS this card's DoD.
