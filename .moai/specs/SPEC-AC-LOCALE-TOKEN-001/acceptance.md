...
- 2026-09-08: AC-009 예시 명령 정정 — 기록 오류 수리 (판정 불변).
  - before: `grep -c '^| \.moai' census.md` — 실제 출력 **0** (§4 판독 표 행은 `| .moai` 가 아니라 `| SPEC-` 로 시작한다. 계측기가 표를 못 본 것이지 표가 비어 있는 것이 아니다 — sync-audit F1, 레인 재현)
  - after: `awk '/^## §4 /,/^## §5 /' .moai/reports/t573/census.md | /usr/bin/grep -c '^| SPEC-'` → **95** (트리 `7b86ac8ab` 실측; 유니크 파일 토큰 쌍 명령도 95·중복 0 — census.md §4 말미 기록)
  - 사유: AC-009 의 기준(판독 표 행 수 ≥ 재측정된 A∖B 수)은 두 읽기 모두에서 성립한다 — 의도한 측정(표 행 95 = PASS)과 기록 명령의 문자 출력(0)은 서로 다른 대상을 잡고 있었다. 이는 기준 완화가 아니라 **계측기 전사 결함** 수리다 — t538 AC-008 정정의 작성자-측 안전 조건(옛 읽기와 새 읽기 모두에서 기준 충족)이 성립한다.
  - **이 정정은 판정에 영향을 주지 않는다.** AC-009 는 run-phase 에서 benign 60 · 비-grep 계수 관찰 35 · 왜곡형 0 으로 PASS 판정됐고(progress.md §E.2), 그 판정은 표의 내용을 사람이 판독해 낸 것으로 명령 전사 오류와 무관하다.
  - **승인 귀속: 리드 승인 — 카드 t573 (sync-audit F1 수리)**.
...
- **When** census.md 의 A∖B 판독 표에서 파일 수를 센다: `awk '/^## §4 /,/^## §5 /' .moai/reports/t573/census.md | /usr/bin/grep -c '^| SPEC-'` — §4 판독 표 행 계수 — 파일별 판정(benign / 비-grep 계수 관찰 / 왜곡형)이 한 줄씩 있다.
