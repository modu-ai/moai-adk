...
| AC-CTX-005 | REQ-CTX-005 | `go test ./internal/tui/ -run 'NoColor\|Monochrome\|Detect' -count=1 -v` | PASS — NO_COLOR 우선순위 체인 기존 테스트 green |
...
| AC-CTX-008 | REQ-CTX-008 | `grep -rn '#[0-9A-Fa-f]\{6\}' internal/core/project internal/cli --include='*.go' \| grep -v '_test.go' \| wc -l` | 0 — reporter.go hex 제거 후 마이그레이션 표면 hex-free (baseline: reporter.go 3건) |
| AC-CTX-009 | REQ-CTX-009 | `go test ./internal/tui/... -count=1` + `git diff --stat -- internal/tui/testdata internal/tui/golden` | 스위트 PASS; 골든 변경이 있다면 변경 파일 각각이 progress.md §E.2에 사유와 함께 기재됨 |
| AC-CTX-010 | REQ-CTX-010 | `git diff --stat -- internal/statusline internal/cli/wizard internal/cli/uikit` + `go test ./internal/statusline/... ./internal/cli/wizard/... ./internal/cli/uikit/... -count=1` | statusline/wizard/uikit 소스 무수정(diff 0; 단 wizard 대상 diff는 REQ 외 사유 없을 것) + 테스트 전부 PASS |
...
| AC-CTX-020 | REQ-CTX-020 | `go test ./internal/cli/... ./cmd/... -run 'ExitCode\|ExitCoder' -count=1 -v` | PASS — ExitCoder 체인(worktree verify 0/1/2/3 포함) exit code 보존 |
...
| AC-CTX-023 | REQ-CTX-023 | `grep -n '@MX:DEBT' internal/merge/confirm.go && grep -n '@MX:UPGRADE' internal/merge/confirm.go && go test ./internal/merge/ -count=1` | @MX:DEBT + @MX:UPGRADE(SPEC-CLI-TUX-V3-003 트리거) 존재 + merge 스위트 PASS + design.md §D에 결정 기록 |
