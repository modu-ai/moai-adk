...
| M3 | TG-2 | `internal/web/screen_states_test.go` | 10 (`TestKanban*`, `TestTodo*`, `TestSpecs*`, `TestCloseDebt*`, `TestSpecRow*`, `TestSpecDetail*`, `TestOverview*`) | `ok ... 0.853s` |
| M4 | TG-3 | `internal/web/fieldsets_states_test.go` | 12 (`TestSchema*`, `TestSettingsPanel*`, `TestFieldset*`, `TestPermissionOption*`, `TestAgentFMRow*`, `TestMCPToggle*`, `TestCodex*`, `TestGlmKey*`) | `ok ... 0.607s` |
| M5 | TG-4 | `internal/web/shell_chrome_states_test.go` | 14 (`TestShell*`, `TestNavRow*`, `TestTopbar*`, `TestLiveState*`, `TestSaveCluster*`, `TestProfilePop*`, `TestPanelMeta*`, `TestNoteBanner*`, `TestGauge*`, `TestStageMark*`, `TestStateMark*`, `TestBackendBadge*`, `TestBadgeKinds*`, `TestLaneUnresolved*`) | `ok ... 0.535s` |
| M6 | TG-5 | `internal/web/page_chrome_states_test.go` | 8 (`TestProfileModifiable*`, `TestProfileActionRoutes*`, `TestIconSVG*`, `TestIconAt*`, `TestBanner*`, `TestSaveAction*`, `TestSettingsPage*`) | `ok ... 0.623s` |
...
Classification of the 2,388 residual zero statements (computed from `/tmp/t1079-verdict.out`):

1. **2,238 statements — 1-statement templ codegen error-propagation blocks** (`if err != nil { return err }` after every `WriteString` / `ResolveAttributeValue`). Rendering into a healthy in-memory buffer cannot fail these; converting them requires a failing-writer/child-component injection seam, i.e. product-code changes forbidden by spec §C, or error-path tests that exist only to execute the branch — coverage-filling under spec REQ-003.
2. **~120 statements — 2-statement child-`Render` error propagations** in the `*_templ.go` files (e.g. `fieldsets_templ.go:179`, `screens_templ.go:50`). Same reachability wall as (1): a child render cannot be made to fail without an injected failing component (a seam), or a test whose sole purpose is the error branch (filler).
3. **~30 statements — genuinely reachable logic not targeted by any planned TG**, concentrated in: `screens.go`/`handlers.go` 500-branches (`buildMonitor`/`buildSpecList` failure paths through seams), `events.go` SSE error branches, `glmkey.go`/`schemaform.go`/`profile_crud.go` request-parse error branches, `browser.go`+`assets.go` (6 stmts, excluded per spec §E).

**Conclusion**: 85% would require converting 976 residual statements; ≥2,358 of the 2,388 are error-propagation code unreachable without forbidden product changes or forbidden filler. The honest ceiling of behavioral render assertions at this codegen shape is ≈ **76%** (2,388 − ~30 reachable ≈ 2,358 zero ⇒ 74.9%, rounded against further diminishing returns). The gap is structural (generated-code error propagation), not a test-authoring shortfall. Decision requested per REQ-012: accept the documented shortfall, OR re-plan with a product-side scope (e.g. templ codegen error-block filtering in coverage policy, or injection seams — both out of this SPEC's tests-only envelope).
...
ac_fail_count: 1    # AC-031 (85% not met) — resolved as the REQ-012 documented-shortfall branch, orchestrator decision pending
