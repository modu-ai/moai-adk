...
- **REQ-UPC-008 (State-Driven, clarified v0.2.3):** WHILE no deprecated paths are present in the user project, THE update command SHALL skip the **backup and deletion phases** silently with no backup directory creation. The telemetry phase (REQ-UPC-022) SHALL still execute and emit `.moai/logs/update-cleanup-{ts}.jsonl` with `backup_outcome: "skipped"`, `cleanup_outcome: "completed"`, and `pre_update_suffix2_files: []`. "Silent skip" applies only to backup/deletion side effects, not to observability.
...
- **REQ-UPC-026 (Event-Driven, addresses D-08 R7; extended v0.2.1 D-02-07):** WHEN running on a case-insensitive filesystem (detected at start of cleanup phase via a test-file creation probe — create `.moai-fscase-probe`, attempt to stat with the uppercase variant, then remove), THE update command SHALL match deprecated paths case-insensitively (e.g., `agency.md` and `Agency.md` resolve to the same registry entry). On case-sensitive filesystems, matching SHALL remain exact-case.
  ...
  - **Probe frequency (v0.2.1):** THE probe SHALL execute exactly once per `moai update` invocation, scoped to the project root filesystem. Results from the probe SHALL NOT be cached across invocations (avoids stale state when filesystem is remounted with different case settings between runs).
...
v0.2.0에서 OQ1과 OQ2는 코드 인스펙션을 통해 해소되었다 (`internal/merge/confirm.go:623`; manifest provenance는 `internal/manifest/types.go`의 Provenance enum (`types.go:11-39`) + Manifest struct (`types.go:42-46`) + FileEntry struct (`types.go:49-54`)에 분산 정의).
...
- **OQ4 (resolved 2026-05-04, v0.2.3):** Lock file (`.moai/.update.lock`) 형식은 **JSON 객체** `{"pid": int, "started_at": "ISO8601", "hostname": string}`로 확정. 단순 PID-only 형식 대신 JSON을 채택한 근거: (a) stale lock 감지 시 `started_at`으로 age-based timeout 판정 가능, (b) `hostname`으로 NFS 등 공유 파일시스템 환경에서 cross-machine lock 충돌 방지, (c) 향후 fields 확장(SPEC ID, lock reason 등) 호환성. plan.md T1.3 (lock 파일 생성 로직)이 본 스키마를 사용한다. signal handler (SIGINT/SIGTERM) cleanup은 동일하게 적용 (NFR-UPC-S2).
