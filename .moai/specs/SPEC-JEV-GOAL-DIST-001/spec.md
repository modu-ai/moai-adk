...
- Building any host path for Consumer A's routing inside the goal loop. Host-design question N2 is OPEN (CONSUMERS spec.md §F) with its producer M5 recorded blocked; this SPEC neither answers N2 nor amends CONSUMERS' recorded state.
...
| N1 | Consumer B's verification surface | `SPEC-JEV-CONSUMERS-001` | **SETTLED** (CONSUMERS §F; the foreman-pattern prose surface, landed at M6) | The precedent this SPEC's skill-surface discipline follows. |
| N2 | Which code path hosts Consumer A | Recorded in `SPEC-JEV-CONSUMERS-001` §F; **currently unowned** — CONSUMERS closed `completed` with the M5 block recorded | **OPEN — blocked CONSUMERS M5 and thereby this SPEC's seat (i)** | Seat (i) is withdrawn (§C.1). A future SPEC must answer N2 before any seat-(i) scope is re-admitted; this SPEC does not resolve N2. |
