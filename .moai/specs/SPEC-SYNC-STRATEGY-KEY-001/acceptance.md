...
- **Given** `make build` regenerated the embedded templates and the SPEC-scoped edits landed on both trees, **When** the local surfaces are checked, **Then**:
  1. **Token parity on SPEC-edited regions** (NOT byte-identity): `grep -rn 'spec_git_workflow' .claude/skills/ .moai/config/sections/system.yaml | wc -l` = 0 on the local tree — the dead-key tokens are gone from template AND local alike (the greps of AC-SYK-002 cover the template side).
