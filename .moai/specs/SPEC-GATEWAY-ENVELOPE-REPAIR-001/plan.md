...
## §H [NEEDS CLARIFICATION] (bounded, non-blocking for M1/M2 — must be resolved before M3 exit)

- [NEEDS CLARIFICATION: public-content equality on the failing switch-turn replay] — whether the t703-class client re-encode also alters public content (t703 verdict.md Gaps; never captured the failing request body). Bounded either way: the repair refuses on a Prefix-class mismatch (REQ-EVR-007), so the answer changes only the observed refusal frequency, not the design. Resolvable by the next production occurrence's classified 400 body (t703's diagnosability closure) or an optional run-phase capture.
- [NEEDS CLARIFICATION: transcript envelope retention under client compaction / `--continue` slicing] — whether the family native transcript retains verbatim `redacted_thinking` carriers after Claude Code compaction or session slicing (research synthesis gap 5; unmeasured). Bounded either way: the repair refuses when the source is gone (REQ-EVR-007). Resolvable by a run-phase retention probe against a compacted family transcript.
- [NEEDS CLARIFICATION: scope boundary vs the serial-turn chain-class shape (t707)] — whether this SPEC's scope extends to the `CauseChain` serial-turn shape (family `1f14d174`, spec.md §3.4 row / §4.1) or a follow-up SPEC takes it awaits card t707's reproduction verdict. Bounded the same way as the two items above: this SPEC's repair path is CauseReasoning-scoped (REQ-EVR-004), so the answer changes only scope allocation, never the design. The underlying digest-stability question — does client-side split-record re-serialization (storing `redacted_thinking` and `tool_use` as separate records) change the envelope digest or public-content bytes — cites t707's result when it lands (external prior art: BerriAI/litellm #40288).
...
- Card t707 (lane-5) — serial-turn `CauseChain` 400 reproduction (family `1f14d174`); its verdict is M0's second adjudication input and resolves plan.md §H marker 3.
