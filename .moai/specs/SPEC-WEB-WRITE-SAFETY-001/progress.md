...
**전체 경로 가드 (RED-first, verbatim: `.moai/reports/t517/evidence/RED-f1d-render-polarity.log`)**: `TestHandleSaveUntouchedRenderedBodyLeavesTrackedConfigByteIdentical` — **렌더→파서→게이트 루프 전체**를 통과한다: (1) GET /settings로 실제 렌더를 받아 absent 키의 checked 라디오 값을 읽고 (2) 그 값 + companion으로 POST /save (3) workflow.yaml byte-identical assert. 수리 전 트리에서는 렌더가 OFF를 보여줘 빈 값이 제출되고 게이트가 `todo: enabled: false`를 기록해 FAIL(RED) — ApplySchemaEdits 직접 호출 3건이 이 루프를 못 지나가던 빈틈이 이 테스트가 메운다. 수리 후: 렌더가 ON을 보여주고 그 재제출은 게이트가 스킵 → **byte-identical (GREEN)**.

**RED 채득 이력 (sync-audit F2-f 재채득)**: 최초 채득은 초안 가드 세대(158행 Errorf 단계가 존재하던 시점)라 커맨드·트리 SHA 미포함 — **재채득으로 교체했다**. 절차(감사 실험 B): (1) 커밋 `6639e7ecb` 트리 위에 `fieldsets_templ.go:925`의 수리식만 이전식(`== "true"`)으로 되돌리는 working-tree 오버레이 적용 (2) 커밋 가드 실행 → FAIL, exit 1 verbatim 채득 — 헤더에 커맨드·exit code·트리 상태 명기 (3) 오버레이를 수리식으로 복원 → 같은 커맨드 PASS 확인 (4) `git diff --stat internal/web/fieldsets_templ.go` 무출력으로 오버레이 완전 복원 증명. 채득 로그의 실패 메시지는 결함을 직접 가리킨다: "renderer shows no checked radio for the absent default-ON key — expected ON (value=1) after the polarity repair".
