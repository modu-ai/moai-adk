...
**Then** (a) 정상 판은 10/10 회 스왑 뒤 지표가 발화하고, 매 회 REQ-AFG-016 자기확인 네 다리가 모두 참이어야 한다. (b) M1 과 M2 는 **각각** 10/10 회 적색이어야 한다(사유 `indicator did not fire` 또는 `selector matched nothing`, 스왑 뒤 항목 이름과 함께). (c) 세 판은 같은 조건에서 돌아야 하며, 판정서와 보고서는 증폭 적용 여부와 그 값을 적어야 한다(아래 「측정 순서」). (d) **만료된 대기는 적색이다.** AC-AFG-015 (iii) 의 사본(리스너를 클릭 **뒤에** 붙여 `htmx:afterSettle` 을 놓치는 사본)에서 탐침은 **exit 1** 이어야 하고, 보고서가 스왑 뒤 항목(`popover_after_swap`)을 사유 「afterSettle 대기 만료」와 함께 지목해야 한다. 그 실행에서 스왑 뒤 지표가 발화로 판정되면 이 AC 는 실패다 — 오늘의 `poll` 처럼 만료 뒤 현재값으로 진행하는 대기는 여기서 걸린다(REQ-AFG-007 (3)).
...
- **(iii) (c) 단독 판** — `htmx:afterSwap`·`htmx:afterSettle` 리스너를 클릭 **뒤에** 붙인 사본(plan.md §G 가 안티패턴으로 이름 붙인 형태). 스왑은 실제로 일어나므로 (a)(b)(d) 는 참이고, 이벤트를 놓치므로 (c) 만 거짓이 된다. 같은 실행이 AC-AFG-014 (d) 의 만료 대기 적색도 보인다.
...
- [ ] AC-014 — 「측정 순서」대로 스로틀 12배 단독 측정(정상·M1·M2 각 10회)을 먼저 보존한다. M1·M2 가 각각 10/10 적색이 아니면 2단계로 가서, 증폭 전후 afterSettle 도달 시간 원문(각 10회, 중앙값 비교)과 같은 증폭 조건의 정상 10/10 발화·M1·M2 각 10/10 적색을 보존한다. 만료 사본의 exit 1 + `popover_after_swap` 지목(사유: 대기 만료)도 보존한다. 판정서에 증폭 적용 여부와 값을 기록한다
