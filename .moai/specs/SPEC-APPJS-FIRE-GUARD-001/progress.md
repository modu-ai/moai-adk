...
**Claim** — plan-phase 산출물 4종(spec.md / plan.md / acceptance.md / progress.md)이 작성됐고, 이 SPEC 이 딛고 선 실측값 — 특히 런타임 발화 가드의 **양방향 프로토타입** — 은 이 트리에서 이번 실행으로 측정됐다.
...
- **정방향 프로토타입**(실바이너리 `moai web --port 18441 --no-open --no-reuse` + 로컬 Chrome `--headless=new --remote-debugging-port=9333`): 5 지표 전부 발화, ReferenceError 0건 — 출력 그대로:
...
- 셀렉터 미달 방향(AC-AFG-004)과 skip 사유 방향(AC-AFG-003)은 관측 불가(테스트 미존재) — acceptance.md §D 에 RED-now 부재로 기록했다.
