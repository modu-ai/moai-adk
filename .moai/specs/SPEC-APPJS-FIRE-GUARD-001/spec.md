...
>
> **(3) 의 근거 좌표 (plan-audit iter-2 N3 — 조문에서 이곳으로 옮겼다).** (3) 이 금지하는 의미론은 오늘 탐침의 `poll` 이 쓰는 것이다(`internal/web/testdata/appjs_fire_probe.py:434-443`, 마지막 줄 `return await ev(cdp, expr)`). 구현 좌표는 요구사항 문언이 아니라 근거이므로 이 주석에 둔다. plan.md M8.3 이 같은 좌표를 싣는다.
>
