...
| AC-SWF-013 | REQ-SWF-011 (RT-007-003 supersession) | `go test ./internal/template/ -run TestFallbackChainOrder` AND `grep -n 'GoBinPath' internal/template/hardcoded_path_audit_test.go` | `TestFallbackChainOrder` PASSES asserting the new order (`PATH → resolved-executable → $HOME/go/bin → $HOME/.local/bin`); no residual assertion of a baked `.GoBinPath` branch |
