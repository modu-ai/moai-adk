...
**Second observation pair** (D2 commission — measured 2026-09-26 on HEAD `2286d77c3`, clean tree; single invocations, each recorded with its own four elements):

| ID | Command (as run) | Verbatim stdout | Exit | Tree |
|---|---|---|---|---|
| E-18 | `unset MOAI_KANBAN MOAI_KANBAN_ID MOAI_KANBAN_LABEL MOAI_KANBAN_LEAD_ADDR MOAI_KANBAN_SETTINGS_INJECTED && go test -count=1 ./internal/hook/ -run TestBranchGuardPSForms -o /dev/null` (the `unset` prefix is the repo's mandated one-invocation env-scrub compound, not command sequencing) | `ok  	github.com/modu-ai/moai-adk/internal/hook	0.645s [no tests to run]` | `0` — the empty-sweep hazard itself: the swept set is zero (`[no tests to run]`), so this exit 0 is NOT a pass of any battery (verification-completeness §1.1) | `2286d77c3` |
| E-19 | `sed -n '/## §E.2 Run-phase Evidence/,+2p' .moai/specs/SPEC-HOOK-GUARD-POWERSHELL-FORMS-001/progress.md` | `## §E.2 Run-phase Evidence` / (blank line) / `_<pending run-phase>_` | `0` | `2286d77c3` |

## §C Acceptance criteria (two-cell)

| AC | Requirement | RED-now (cell 1) | Green path (cell 2) |
|---|---|---|---|
| AC-HGF-001 | REQ-HGF-001 | E-01: `git.exe switch probe` allows today. Red because the branch-state scan does not accept the `.exe`-suffixed executable spelling — not because the guard is inert: control E-09 (the wrapper form `cmd /c git switch probe`) proves the same scan still reaches deny verdicts, and the plain form's deny is the branch_guard family's established behavior. | M2 flips it: committed test asserts deny with the `BRANCH_GUARD_VIOLATION:` family; scoped selector `go test -count=1 ./internal/hook/ -run TestBranchGuardPSForms` exits 0. |
...
| AC-HGF-010 | REQ-HGF-008 + REQ-HGF-014 | E-18: the battery's scoped selector sweeps zero tests — `ok … [no tests to run]`, exit 0, at `2286d77c3`. The battery does not exist yet, and the recorded exit 0 is the empty-sweep hazard itself (§1.1), not a pass — exactly the regression window: no failure signal can exist for a test that is not there. | M2-M4 deliver the battery as a committed table-driven test: every §D legit leg asserts allow AND zero new audit lines; it must stay green through every detector change. The test file carries the REQ-HGF-014 isolation contract (each test sets `MOAI_HOME` to a per-test temp dir, no `t.Parallel`, no real MoAI home or repo `.moai/state` access), verified by the same scoped run — which after M2-M4 reports a non-zero swept count. |
...
| AC-HGF-014 (regression-guard) | REQ-HGF-012 | Trivially satisfied at the base (no card commits yet); classification: regression-guard over the card's commit range. | At close, **pre-merge evaluation only** (after the card merges, the merge-base collapses and this form is unusable — gitflow-lane-protocol §8): `CARD_BASE=$(git merge-base develop HEAD); git diff --name-only "$CARD_BASE"..HEAD -- internal/template/ .claude/` is empty — the recompute form, never a literal pinned base (the t543 measured class: after absorbing develop, a literal `19b5321c1..HEAD` range pulls in other cards' commits and fails spuriously). Non-vacuous rule: the empty filtered result counts only when the same range **without** the pathspec lists at least one file (the card's own changes) — an empty range sweeps nothing and asserts nothing. |
...
- **Candidate count**: the loop enumerates **11** spellings; the **12th** candidate is the attached-colon form `-enc:<B64>` — the `:value` shape the parameter-name parser handles — run as its own invocation because the payload is attached to the parameter:
```
printf '%s -> ' '-enc:<B64>'; pwsh -NoProfile -NonInteractive "-enc:$B64" 2>&1 | head -1
```
- **Caps declared up front** (t1152 lesson): 12 candidate spellings — 11 enumerated in the loop plus the attached-colon form as its own 12th invocation — one pass, wall-clock ≤ 10 min, ≤ 2 turns, foreground only, per-invocation `timeout 60`, no background load.
