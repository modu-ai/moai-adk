...
carried_deviations:
  ...
  - "A1 residual — the fill lock's staleness reclaim keeps a test-and-remove window; narrowed, never eliminated. The bound is wider than the §E.2 wording 'exactly one extra child' implies: the `os.Remove` at `internal/hook/session_start_drift_fill.go:329` is unconditional once staleness is judged, so N handlers can each judge the same stale lock, each remove it, and each win a re-claim. The honest bound is one extra COMPUTE, itself bounded by the child's own flock (`internal/spec/drift_fill_lock.go`), which serialises the compute whatever the claim layer admits. Raised as sync-audit F3; corrected here in §E.4 only — the §E.2 sentence is manager-develop's and was left untouched. Stated as a residual, not closed."
