...
test-and-remove. A microsecond-wide race survives, and it is bounded on one axis
only: the child's own flock (`internal/spec.WithDriftFillLock`, non-blocking)
admits at most one COMPUTE — a child that loses it computes and writes nothing —
so no second compute, no torn cache, no data corruption. It does NOT bound the
NUMBER of children: N handlers judging the same lock stale can each remove it and
each win the re-claim, and each extra child costs a process start, a failed flock
and an exit. The comment cites the precedent that
...
carried_deviations:
  ...
  - "A1 residual — the fill lock's staleness reclaim keeps a test-and-remove window; narrowed, never eliminated. The bound is wider than the §E.2 wording 'exactly one extra child' implies: the `os.Remove` at `internal/hook/session_start_drift_fill.go:339` (re-measured on the card t898 tree; the coordinate moved when that comment was rewritten) is unconditional once staleness is judged, so N handlers can each judge the same stale lock, each remove it, and each win a re-claim. The bound splits by axis, and card t898 rewrote all three surfaces to say so: the child's flock (`internal/spec.WithDriftFillLock`, `internal/spec/drift_fill_lock.go:50`, taken non-blocking) admits at most one COMPUTE — a loser returns `ran=false` having computed and written nothing — so no second compute, no torn cache, no corruption; it does NOT bound the NUMBER of children, and each extra one costs a process start, a failed flock and an exit. The earlier correction in this section said 'one extra COMPUTE, itself bounded by the child's own flock', which reads as a count bound the flock does not give — sync-audit F9. Raised as sync-audit F3, refined by F9; §E.2, this bullet and CHANGELOG.md now carry the same split. Stated as a residual, not closed."
  - "spec lint: 0 errors, 22 warnings — 15 CoverageIncomplete plus 7 ModalityUnjudged (15 + 7 = 22). The CoverageIncomplete rule is NOT spec.md-scoped: it reads the sibling acceptance.md, via `internal/spec/lint_coverage_sibling.go` with `internal/spec/lint_coverage_sibling_table.go` for the table form. The cause is the mapping FORMAT — this acceptance.md declares each mapping as a `- **Requirement**: REQ-DCF-001` bullet (line 99 and its siblings), which matches neither the `(?i)maps\\s+(REQ-[A-Z0-9-]+...)` pattern at `internal/spec/ears.go:124` nor the table form the sibling table reader accepts. Coverage itself is genuine: `comm -23` over the REQ id sets of spec.md and acceptance.md prints nothing, 16 ids on each side. The sync auditor proved the mechanism by experiment on a scratch copy — appending one `AC-DCF-001 maps REQ-DCF-001` line to acceptance.md took CoverageIncomplete 15 -> 14 and removed exactly REQ-DCF-001's row. The repair is reformatting the mappings inside acceptance.md, a SPEC-body edit manager-docs may not make; it is manager-spec's. CORRECTION: the first revision of this bullet stated 12 findings and asserted the rule measures spec.md alone — the count was miscounted and the premise was never verified; both were wrong, and sync-audit F1 (`.moai/reports/t871/sync-audit.md`) caught them."
