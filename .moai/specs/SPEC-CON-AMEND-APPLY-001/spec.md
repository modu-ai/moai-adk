...
tags: "constitution, amendment, atomic-apply, evolution-log, rate-limiter, dry-run, registry-resolver, t659"
tier: M
...
Card: **t659** (lane-6). Worktree `.claude/worktrees/t659`, branch `WT-amend-apply`. Code coordinates were read at `034d55c56`; the 0.1.1 revision was authored on `ff11e752f`, whose only change after `7b4d1ac89` is verdict §8 (no code change).
...
6. **The CLI and `Execute` can read different registries.** `runConstitutionAmend` validates `--before` against the registry `resolveRegistryPath` returns (`MOAI_CONSTITUTION_REGISTRY`, then `CLAUDE_PROJECT_DIR`, then cwd — `internal/cli/constitution.go:144-155`), while `Execute` loads `<projectDir>/.claude/rules/moai/core/zone-registry.md` (`pipeline.go:66`). Once this card turns writing on, that split is a write to a file the user never validated (verdict §8, scope addition (a)).
...
## §C Lead rulings encoded (verdict §7 and §8 — authoritative, not reopened)
...
- **G5 — non-dry-run CLI path not tested at CLI level.** Approved by the lead (verdict §8 G5). CLI tests cover dry-run only (AC-CAA-015). The real-apply path through `runConstitutionAmend` is not exercised by any test in this SPEC: it would need a stdin seam (`NewHumanOversight` hardcodes `os.Stdin`, `internal/constitution/human_oversight.go:20-24`) and would create the cwd-relative lock file inside the package directory (§F). The apply itself is covered at `Execute` level (AC-CAA-001, 002, 005, 012, 013, 019, 020, 021, 022). What stays unobserved is only the CLI wrapper around a successful or failed real apply: its output lines and exit status.
...
All §7 and §8 items (Q1–Q5, G1–G5, scope addition (a)) are resolved and encoded in §C; none remains open. One question surfaced while encoding (a) and is not decided here:

1. **G6 — source and log paths under a divergent environment.** REQ-CAA-019 unifies only the registry path, as ruled. The source rule file (`rule.File` joined to `projectDir`, `pipeline.go:192-195`) and the evolution log (`<projectDir>/.moai/research/evolution-log.md`) still resolve against `projectDir`. When `CLAUDE_PROJECT_DIR` names a project other than `projectDir`, the registry comes from one tree and the source file and log from another. In practice `LoadRegistry` also rejects a registry path that escapes `projectDir` (`loader.go:80-88`), so today that setup fails at registry load rather than writing across trees — AC-CAA-022 therefore uses a `MOAI_CONSTITUTION_REGISTRY` path inside `projectDir`. Should the source and log paths follow the resolved project root too, or is the escape rejection the intended boundary?

## §H Cross-References

- `.moai/reports/t659/verdict.md` — repro evidence (§2), lead rulings (§7, §8), follow-up candidates (§7.1, §8.1)
