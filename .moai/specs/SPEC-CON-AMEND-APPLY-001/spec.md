...
All §7, §8, §9, and §11 items (Q1–Q5, G1–G7, scope addition (a)) are resolved and encoded in §C.

**Open — D4 (plan-audit iteration 1).** Whether the containment check of REQ-CAA-021 applies to the shared `LoadRegistry` for all of its non-test callers or only to the amend path (verdict §15.3 lists the callers). It awaits an operator decision. Revision 0.1.5 leaves unchanged the text whose meaning depends on it: §E.2's loader bullet, the rationale sentences of REQ-CAA-021 about commands that load the registry, `plan.md` §D and R-8, and `design.md` §C.3–§C.4.
