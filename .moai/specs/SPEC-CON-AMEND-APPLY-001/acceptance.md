...
Revision 0.1.8 (same as spec.md HISTORY): plan-audit iteration 3 optional findings X1, X3, and X4 applied here by operator decision, without re-audit and without new IDs. AC-CAA-023's parenthetical states that the check runs first on the amend path and the loader's retained refusal would refuse the same path, instead of calling the order a run-phase choice; no wording assertion is added. AC-CAA-025's preservation paragraph names the two blind spots of its `--first-parent --no-merges` range and makes the test run the primary witness. The common rules gain a path-formatting rule (`%s`/`%v`, not `%q`). Existing IDs unchanged.
...
- **Path formatting.** Errors this SPEC adds or changes format paths with `%s` or `%v`, not `%q`, so the path assertions — and the path removal of the numeric-assertion rule — hold on Windows: `%q` doubles every backslash of a Windows path, as the present loader error does (`internal/constitution/loader.go:86`). The loader's own error keeps its present form under D4; the one assertion that reads it (AC-CAA-024 `loader_unchanged` case `absolute_escape`) checks its wording `escapes project dir`, not a path. This is a rule for the errors the tests read, not an acceptance criterion.
...
- **Then** each returns the registry load error, whose text contains the offending registry path `Q/.claude/rules/moai/core/zone-registry.md` in its cleaned absolute form or its symbolic-link-resolved form (the test computes both; the wording around the path is not asserted: the check runs first on the amend path (plan.md M2), and the loader's retained absolute-only refusal would refuse the same path — under D4 both layers refuse this absolute path),
