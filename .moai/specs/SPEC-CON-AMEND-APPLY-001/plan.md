...
| `LoadRegistry` rejects only an absolute registry path escaping `projectDir`; a relative path is not checked, and `applyAmendment` joins `file:` with no check — both closed on the amend path by REQ-CAA-021 (verdict §11); `LoadRegistry` itself keeps this behaviour (operator decision D4) | `loader.go:80-88`, `pipeline.go:192-195` |
| `LoadRegistry` has seven non-test call sites: two on the amend path (`internal/cli/constitution.go:511`, `internal/constitution/pipeline.go:67`) and five that stay unchanged under D4 (`internal/spec/lint.go:114`, `internal/cli/constitution.go:70` and `:160`, `internal/cli/doctor.go:683`, `internal/constitution/validator.go:183`); `TestLinter_AC08_DanglingRuleReference` pairs the relative registry path `../../.claude/rules/moai/core/zone-registry.md` with `BaseDir: "testdata"` | grep at `193136a6a`; verdict §15.3; `internal/spec/lint_test.go:18-22`, `:218-222` |
...
Gaps carried: the non-dry-run CLI path (spec.md §E.4, approved reduction G5). G6 and G7 are resolved (REQ-CAA-020, REQ-CAA-021). D4 from plan-audit iteration 1 is resolved by operator decision (revision 0.1.6, `spec.md` §G): the containment check applies to the amend path only; extending it to read-only callers of `LoadRegistry` is out of scope (`spec.md` §F).
...
BASELINE_SHA=$(git rev-parse HEAD)   # before the first run-phase commit; record the resolved value in progress.md §E.2 (later commands may use it literally); AC-CAA-025's preservation assertion ranges from it
...
/usr/bin/grep -c 'filepath.IsAbs(cleanPath)' internal/constitution/loader.go  # expect 1 (absolute-only containment; stays 1 after run-phase, D4)
...
- [HARD] Lane-local verification is scoped: `go test ./internal/constitution/ -count=1` plus the named `internal/cli` selectors and the one `internal/spec` preservation selector of AC-CAA-025 (`go test ./internal/spec/ -run '^TestLinter_AC08_DanglingRuleReference$' -count=1 -v`), never the full suite. The five non-amend callers of `LoadRegistry` are left to CI's full suite; the `internal/spec` selector is named because it is the one existing test whose expectation D4 is predicted to protect. `internal/cli` runs need the compile slot granted at run-phase and `-timeout 600s`.
...
- Containment (REQ-CAA-020, REQ-CAA-021): one check function — clean, make absolute, resolve symbolic links (nearest existing ancestor for a path not yet created), compare with the resolved root at a separator boundary — called on the amend path only (operator decision D4): by `Execute` and by the registry validation of `runConstitutionAmend`, in both callers on the registry path before `LoadRegistry` reads the file — so a refused registry is never read, which REQ-CAA-020's "no read … shall reach another directory tree" requires — then on every entry's joined `file:` immediately after the registry is loaded, and on the evolution-log path before Layer 1. The order is observable by code review only (`spec.md` §E.4). `LoadRegistry` does not call it and keeps its absolute-only refusal, so its five non-amend call sites are untouched; do not move the check into the loader to save a call site (M-20 variant (iii) is exactly that move). The source rule file and the log keep their `projectDir` joins; the check only admits or refuses them. Baseline-first, predicted from code reading at `54ca2e3b6` and observed only in the baseline commit: AC-CAA-023 RED against current code (`Execute` ignores `CLAUDE_PROJECT_DIR`, loads the registry inside `projectDir`, and returns dry-run success or the stub error instead of a registry load error). AC-CAA-024's seven escape rows are predicted RED the same way, since no check exists for them.
- Exit: AC-CAA-022 GREEN; M-19 RED (on AC-CAA-022 and on AC-CAA-023's divergent rows); AC-CAA-023 subtests `divergent_root_real` and `divergent_root_dry_run`, AC-CAA-024's seven escape rows (real and dry-run), AC-CAA-024 `loader_unchanged`, and AC-CAA-024's CLI case (compile slot) GREEN; the AC-CAA-025 `internal/spec` preservation run GREEN; M-20 (all four variants), M-21 (both variants), M-22 (both variants), and M-23 (all three variants) RED. AC-CAA-023 `same_root_control`, the real cases of AC-CAA-024 `in_root_control`, and AC-CAA-025 `real` turn GREEN at M5, once the apply is wired. Existing `internal/cli` constitution tests re-run with the compile slot.
...
| R-7 | An over-strict containment check refuses legitimate paths — a root reached through a symbolic link (the macOS temp directory is one), or a log not yet created | REQ-CAA-021 resolves both sides and judges a missing path by its nearest existing ancestor; AC-CAA-024 `in_root_control`, AC-CAA-025, M-24. The same platform link bears on the tests beyond M-24: with a fixture base under darwin's `/var`, a mutant that resolves the root but not a candidate (M-23), or compares an unresolved candidate with a resolved root (M-22 (ii)), refuses every candidate, so the rows named for it pass and the mutant dies only on `in_root_control`. AC-CAA-024 and AC-CAA-025 therefore build their base with `filepath.EvalSymlinks(t.TempDir())`, keeping only the intentional links (`B/link`, `P/linkdir`, `P/linked`, `P/.moai/research`), and M-22 (ii) compares against the unresolved `projectDir`; the root reached through a link stays covered by the intentional `B/link` (M-24) |
| R-8 | The containment check is scoped to the amend path only (operator decision D4). Two ways that scope goes wrong: (a) an implementation puts the check inside `LoadRegistry`, which reaches its five non-amend callers — `internal/spec/lint.go:114` (spec lint), `internal/cli/constitution.go:70` (`guard`) and `:160` (`list`), `internal/cli/doctor.go:683`, `internal/constitution/validator.go:183` — and is predicted from code reading to break `TestLinter_AC08_DanglingRuleReference`, whose relative registry path lies outside `BaseDir: "testdata"` and whose load error `NewLinter` swallows (`lint.go:114-118`), so the failure surfaces as a missing finding rather than an error; (b) within the amend path, refusal at registry load also reaches an escaping `file:` in an entry the amendment does not target | (a) the check is called by `Execute` and by the registry validation of `runConstitutionAmend`, never by `LoadRegistry`, which keeps its absolute-only refusal (`spec.md` REQ-CAA-021, §E.2); held by AC-CAA-024 `loader_unchanged` and the AC-CAA-025 `internal/spec` preservation run, both killed by M-20 variant (iii); extending the check to read-only callers is out of scope (`spec.md` §F), a follow-up card candidate recorded by the lead. (b) intended by verdict §11 (load error); the real registry has 0 absolute and 0 `..` `file:` values of 101, so the amend path refuses no current registry (AC-CAA-025) |
