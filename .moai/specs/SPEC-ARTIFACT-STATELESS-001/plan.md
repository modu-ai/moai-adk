...
**번복 지점**: "frontmatter 일체 금지"로 넓히자는 판단이 나올 수 있다. 그 경우 채택 모집단(전 코퍼스 696)에서 대상은 **D1 389 → D2 (미측정)** 으로 늘어난다 — D2는 전 코퍼스에서 재지 않았다(§1.6). 종결 633 모집단으로만 잰 옛 쌍은 D1 362 → D2 417이었고, 그때 차분 55개가 spec.md frontmatter 복제 블록(12~14 필드)이라 별개 결정임이 드러났다. 넓히려면 이 SPEC이 아니라 후속 카드이며, 착수 전에 D2를 전 코퍼스에서 다시 재야 한다.
