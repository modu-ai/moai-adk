...
|---------|------------|--------|--------------------------------------|
| 0.2.0   | 2026-05-18 | orchestrator | Run-phase TDD COMPLETE — PR #980 admin merged. `moai agent lint` 8-rule lint engine (~980 LOC) + 14 ACs PASS + 93.5% coverage + 9 fixture files + agent definition cleanup (manager-quality/evaluator-active Skeptical block 중복 제거, expert-security dead `Agent` token 제거) + §Skeptical Evaluation Stance EVOLVABLE 신설 (zone-registry CONST-V3R2-049) + CI integration (`.github/workflows/ci.yml` `agent lint --strict`). Status `planned → completed`. v3.0.0-rc1 P0 release-blocker 6건 중 2번째 완료. Unblocks ORC-003 (effort matrix LR-03 promotion) + ORC-004 (worktree mandate LR-05 promotion) + MIG-001 (legacy SPEC rewriter validation). |
