...
**Priority High.** Owns `acceptance.md` AC-GDR-012 block (lines ~238-246). Single edit; single commit.

The 5 inline-literal `t=$((t + ...))` lines are refactored into the form below. The run-phase implementer applies this edit to `acceptance.md`; the literal pattern values come from AC-GDR-010's `case` block at `acceptance.md:199-211` (the authoritative source, byte-identical per constraint D2):
...
**Output-format preservation (constraint D6)**: the final `echo "total=$t"` line is byte-identical to the v1.4.0 form, so AC-GDR-006/008/009/011's compound references to `AC-GDR-012 at 0` continue to parse correctly.
