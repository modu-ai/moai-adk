...
| 1.6.0 | 2026-07-27 | In-place amendment (B2-2 + B2-3 hardening — AC-GDR-010 base-current + exemption-list guards). The two SHOULD-FIX findings deferred from plan-audit iteration 2 (`progress.md` B2-2 / B2-3) are closed by extending AC-GDR-010's component (b) with a frozen-base structural-currency assertion (heading-set equality between `e306e21a9:<scoped page>` and the working-tree page) and component (c) with a mechanical exemption-list count check (structured `exempt_detectors: []` declaration replacing prose). **Applied lesson** (`feedback_hypothesis_as_defect`): both are future-risk hardening, NOT current defects — the recorded baseline (`distinct=1,live_min>=1,apt=1` for all 5 detectors; `paired_se:live_min=2`; exemption list empty today) is unaffected and is preserved verbatim. Component (a) symmetry, component (d) aptness, and AC-GDR-012 (the v1.5.0 single-`p=`-source aggregate guard) are NOT touched. Prose-only plan-phase declaration; the `acceptance.md` body refactor is run-phase scope. See `## Amendments` § 2. | manager-spec |
...
### Amendment 2 — B2-2/B2-3 base-current + exemption-list hardening (v1.6.0) — IN-PROGRESS

- **Prior completed version**: 1.5.0
- **prior_completed_sha**: `62df55fab` — the PR #1181 squash-merge that landed the D2 amendment (`version: 1.4.0 → 1.5.0` + `status: in-progress → completed`). Verified this run via `git -C <worktree> log --oneline -- .moai/specs/SPEC-GOAL-DOCS-RETIRE-001/spec.md`, which shows `62df55fab amendment(SPEC-GOAL-DOCS-RETIRE-001): D2 aggregate liveness/aptness guard (v1.5.0) (#1181)` as the most recent commit touching spec.md; `git show --stat --no-patch 62df55fab` confirms it is the D2 squash-merge. The SPEC's own frontmatter (`run_commit_sha: 24c84c56e`, `sync_commit_sha: 2a12e2b7d9aee1b5cdfbfba31d6b28ab5d7312b8`) records the underlying v1.0–v1.4 provenance; `62df55fab` is the v1.5.0 close on main (the D2 sync commit consolidated via squash).
- **Rationale**: AC-GDR-010 currently carries four components — (a) symmetry, (b) liveness against the immutable base `e306e21a9`, (c) asymmetry carve-out (exemption list), (d) aptness (single-`p=` source). Two SHOULD-FIX findings deferred from plan-audit iteration 2 close defense-in-depth gaps that are NOT current defects (the recorded baseline `distinct=1,live_min>=1,apt=1` holds for all 5 detectors, and the exemption list IS empty today) but would silently admit a future regression:
  - **B2-2** — component (b)'s liveness check runs each detector against `e306e21a9:<scoped page>` and asserts non-zero content. If a scoped page is later restructured (path renamed / split / heading skeleton changed), the base path would still exist and still match content — but that content would no longer correspond to what the working tree carries at the same path. Liveness would be stale-true against a non-representative base. There is NO criterion asserting the frozen base is still structurally current for the scoped pages.
  - **B2-3** — component (c)'s empty-exemption-list invariant is asserted by prose ("No detector is currently exempt; the exemption list is empty and any addition must cite the measured content asymmetry"), NOT by a check. A future edit could quietly add an exemption and bypass component (a) symmetry without citing any measured asymmetry. There is NO mechanical guard enforcing the list is empty (or that any addition carries the required justification).

  **Applied lesson** (`feedback_hypothesis_as_defect`): both are future-risk hardening. The recorded baseline is unaffected — the 5 scoped pages (`autonomous-loops.md` ×4 locales, `self-evolving.md` ×4 locales, `handoff.md` ×4 locales) DO still exist at `e306e21a9` with the same heading skeleton as the working tree today, and the exemption list IS empty. The amendment adds defense-in-depth so a future restructuring / exemption-creep is caught at the next AC run rather than silently passing.

- **Scope**: `acceptance.md` AC-GDR-010 component (b) [B2-2] + component (c) [B2-3] ONLY. Component (a) symmetry and component (d) aptness are NOT touched. AC-GDR-001 through AC-GDR-009, AC-GDR-011, AC-GDR-012 body content is NOT touched. The run-phase scope is local to AC-GDR-010's judgment-command block plus its surrounding prose for components (b)/(c) (~10-15 lines of shell + prose). 1 file affected at run-phase (`acceptance.md`). Prose-only plan-phase declaration; no judgment-command target value, recorded baseline, or held-out gate changes — the recorded baseline (`distinct=1,live_min>=1,apt=1` for all 5 detectors; `paired_se:live_min=2`) is preserved verbatim. The run-phase strategy is authored in `plan.md` §I.
- **Out of scope (amendment)**: AC-GDR-001 through AC-GDR-009, AC-GDR-011 body content; AC-GDR-010 components (a) and (d); AC-GDR-012 (the v1.5.0 D2 single-`p=`-source aggregate guard); spec.md §B requirements; plan.md §A-§H (the v1.0–v1.5 plan and D2 strategy, already complete); the 12 sweep-target locale files (already swept); the four retention surfaces; `run_commit_sha` / `sync_commit_sha` provenance fields; `progress.md` §E.2/§E.3/§E.4 (owned by run-phase / sync-phase); the Amendment 1 (D2) row above.
- **Tier**: S (unchanged from v1.5.0 — run-phase scope is local to one AC body in one file; <5 files, <300 LOC, non-constitutional).
- **Plan-phase artifacts**: This commit (frontmatter `version: 1.5.0 → 1.6.0`, `status: completed → in-progress`; HISTORY v1.6.0 row; this Amendments § 2; `plan.md` §I strategy). The `acceptance.md` body refactor is a separate run-phase delegation (manager-spec, per the D-NEW-1 inline-fix pattern).
