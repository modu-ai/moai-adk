...
### 계획 산출물 개정 (v0.8.0, 2026-09-08)

- **성격**: 감사 부채 상환도 운영자 결재의 착지도 아니다. **run-phase가 종료하며 자기 소관 밖으로 올린 후속 3건(`§E.3` manager-spec 후속 표 F-A1 · F-A2 · F-A3)을 리드 승인 아래 닫은 개정**이며 manager-spec이 수행했다. 범위: `spec.md`(§A 표 · §A 정정 병기 · §I 후속 절 · HISTORY) + `plan.md`(§F) + 이 절. **코드·테스트·`§E.2`·`§E.3`은 손대지 않았다** — 뒤 두 절은 manager-develop 소관이다.
- **[HARD] 소관 경계가 이번에도 실제로 돌았다.** F-A1은 M1을 수행한 에이전트가 「`spec.md`는 이 에이전트의 소관이 아니므로 고치지 않았다」고 적고 올린 것이다(`§E.2` M1 「manager-spec 후속」). 발견 → 상신 → 리드 승인 → manager-spec 개정의 경로는 v0.6.0(AC-SLB-012)과 같다.

**F-A1 — `spec.md §A` 축 2 여섯 줄의 「잠정」 해제.**

- 그 표가 스스로 건 조건이 충족됐다: M1이 판독 기준을 고정했고(커밋 `fc02d2542`), 정본은 `measureM1ModalityCensus` 주석 + `§E.2` M1 「[HARD] 판독 기준」 표다.
- **[HARD] 수치는 `§E.2` M1 기록에서 옮겨 적었지, 이 회차에서 재측정하지 않았다.** 재측정하면 세 번째 모집단이 생겨 귀속이 갈라진다 — 그 사실을 `spec.md` 본문에도 [HARD]로 적었다. 앵커는 M1의 것 그대로(HEAD `1e273079b` · 증거 `.moai/reports/t518/m1-census.txt`)이며, **이 개정이 새로 만든 baseline이 아니다.**
- **옛 값을 지우지 않고 나란히 실었고, 증감 0인 행(463 → 463)도 실었다** — 이 카드의 규율(`plan.md §G`)이며 「움직이지 않았다」도 잰 결과다.
- 표가 답해야 할 세 가지를 함께 못박았다:
  - **행 5는 두 판독이 나란히 남는다** — 189(라이브 · 볼드 유지) / 198(볼드 제거). 정본은 189(`judgeModality`가 보는 문자열)이나 **하나를 조용히 고르지 않는다.** 볼드가 움직이는 9행은 어느 관점에서도 무판정으로 떨어지므로 **결론이 관점에 의존하지 않는다**는 사실도 적었다.
  - **행 2 = 행 3 + 행 4 + 3.** 잔여 3행은 **옛 값에도 있었으나**(2,399 vs 2,396) 실을 행이 없어 드러나지 않았다. 이것이 이 표의 실질적 발견이며 회계 정리가 아님을 [HARD]로 표기했다. 접두사별 분해(`THE ` 1,608 · `WHEN ` 412 · `WHILE ` 178 · `WHERE ` 165 · `IF ` 54, 합 2,417)도 함께 실었다.
  - **[HARD] 증감이 분해되지 않는다는 한계를 표 옆에 실었다.** `+18`·`+40`·`+69`는 코퍼스 증가·판독 기준 변경·본문 수정으로 갈리지 않으며(옛 기준이 글로 남아 있지 않다) **M1을 다시 돌려도 회복되지 않는다.** 귀속 가능한 성분은 코퍼스 크기(+14 SPEC) 하나다. **이 한계를 싣지 않으면 재유도된 수의 표는 다음 독자에게 「빼서 원인을 읽어라」로 읽힌다** — 그것이 이 항목을 [HARD]로 세운 이유다.
- **낡은 전방 참조 둘을 정정 병기했다(원문 미삭제)**: `§A`의 「나머지 차이는 여전히 추정이며 **M1이 확정한다**」(시제가 지났다 — 볼드는 9행으로 실측됐고, 그럼에도 옛/새 차를 방법 결함의 크기로 읽지 말 것을 함께 적었다) · `§I`의 「축 2의 §A 표 여섯 줄이 아직 전부 **잠정값**인 것도 이 때문」(두 조건 모두 충족. 결재가 남기는 산출물인 「정확한 판정 불가 건수」는 `ModalityUnjudged` **481**이며, 481은 「깨진 481건」이 아니라 **「의견 없음」의 모집단 크기**임을 명시).

**F-A3 — `plan.md §F`에 M3′(M-A2b) 행 신설.**

- 계획 5개 · 실행 6개였고 M-A2b에 대응하는 행이 없었다. 행을 더하되 **「계획된 마일스톤이 아니다. run-phase 중에 생겼고, 소급해서 처음부터 있었던 것처럼 적지 않는다」**를 행 본문 첫머리에 두었다 — 분리 판단의 출처(`§E.2` M-A2)와 착지 커밋(`fc0540f41`)을 함께 인용한다.
- 표 아래에 **개수·순서가 모두 어긋났다는 사실**을 실었다(실행 순서 M2 → M3 → M3′ → M4 → M1 → M5). **어느 쪽도 조용히 맞추지 않는다** — 전수 대응표는 `§E.3` 「마일스톤 대조」가 정본이고, M1 지연의 대가(§A 증감 분해 불가)를 그 자리에서 다시 가리킨다.

**F-A2 — 계획 밖 도구 결함. 기록만 하고 고치지 않는다.**

- blast-radius 하네스가 `decomposeReportRelPath`로 **추적되는 t362 증거**(`.moai/reports/t362/m2-gate0-decomposition.txt`)에 쓴다. 실측: 재실행이 `wide_extractions_total=1085`를 **4018**로 덮어썼고(`387 insertions(+), 93 deletions(-)`) `git checkout --`로 되돌렸다.
- `spec.md §I`에 **후속 카드 발행 요청**으로 실었다. **이 카드는 고치지 않고 카드도 발행하지 않는다** — 결함이 이 카드보다 앞서고, 반경이 t362의 측정 하네스이며, 큐 변경은 이 에이전트의 소관이 아니다. 함께 적은 것: **재실행하는 누구나 재현한다**(경로가 상수 고정이므로 이 카드의 확장이 만든 결함이 아니다) · **덮어쓰기는 실패 신호를 내지 않는다**(rc=0, 파일은 존재, 내용만 남의 수치로 바뀐다) · 「한 번 되돌렸으니 해결됐다」로 읽지 않는다.

**§A 규칙 8의 세 수치는 움직이지 않았다 — 재유도로 확인했다.**

- AC 식별자 **16** · REQ **14** · lint 경고 **14**. 이 개정은 `### AC-SLB-` 제목을 만들지 않았고 REQ 정의 행도 더하지 않았다. 파이프 없이 rc를 읽었고(`moai spec lint …/spec.md` → rc=0, `0 error(s), 14 warning(s)`), 증거를 **추적 경로**로 반출했다: `.moai/reports/t518/manager-spec-v08/lint-blind-axes.txt`(트리·HEAD·명령 헤더 포함). 규칙 8이 요구하는 대로 **「움직이지 않았다」도 잰 결과로 적는다.**

**이번 회차에 변경한 아티팩트**: `spec.md` · `plan.md` · `progress.md`(이 절). **`acceptance.md`는 내용 변경이 없어 버전을 올리지 않았다**(v0.7.0 유지) — 형제 SPEC이 v0.4.0에서 겪은 「내용 없는 버전 상승」을 만들지 않는다는 이 카드의 규율이며, 부재가 아니라 **의도된 무변경**으로 적는다.

**do-not-disturb 무이동 확인**: `BLIND-AXES-DISCRIMINATOR-GATE` CLOSED와 C-d 선택 근거(재현성, 잔존 집합 크기 아님) 미변경 · `SPEC-INIT-001` 선택된 대가와 「어휘를 넓혀 고치지 않는다」 [HARD] 미변경 · 20/61 재유도와 31·90 철회 기록 미변경 · L1/L2 개수(3 / 16 / 19)와 원문 병기 미변경 · 각 AC의 뮤턴트 기록 미변경 · v0.6.0·v0.7.0의 AC-SLB-012·005 개정 미변경 · **`§E.2`·`§E.3` 전체 미변경**(manager-develop 소관).
...
**[정정 병기 — 행 ①은 기준이 지명한 기구가 아니라 대체 기구로 충족됐고, 표는 그것을 일치로 적었다.]** 독립 sync-audit(2026-09-08, 자문 A1)이 지적한 것이다. `acceptance.md`의 기준은 「① 수리 전 **수집기 경로**(보존된 narrow)」의 실행을 지명하는데, M-A3이 실제로 쓴 것은 **수리 전 바이너리 두 개**(`moai-b7`·`moai-dev`)다. 여섯 행 중 다섯은 기준 문구에 축자로 대응하고, 이 행만 대응하지 않는다.

- **실질은 충족된다.** 같은 앵커 시점의 같은 병합 코퍼스 위에서 재유도값이 둘 나왔고, 감사의 판단으로는 바이너리 경로가 수리 전 코드 전체를 통과시키므로 보존된 narrow 함수 하나만 부르는 것보다 **오히려 강한 증거**다.
- **기준의 문자 그대로의 기구도 따로 수행된다** — 보존된 narrow 경로를 코퍼스에 돌리는 일은 `internal/spec/lint_req_table_test.go:410` `TestTableCollection_CorpusListFindingsUnchanged`가 한다(이 파일·줄의 실재는 이 실행에서 HEAD `4bb4cb7d2` 위에서 직접 확인했다). 즉 **증거가 없는 것이 아니라, 판정표가 기준이 지명한 것과 다른 산출물을 인용했다.**
- **[HARD] 판정은 바꾸지 않는다.** 행 ①은 「충족: 예」 그대로이고 위 표 행도 지우지 않는다 — 감사가 실질을 충족으로 판정했다. 바뀌는 것은 기록뿐이며, **기구 대체를 일치로 적은 것이 결함이었지 대체 자체가 결함은 아니다.**
