...
- **N1 (가드 층 — 숫자 스윕 + 허용 목록)**: 대상 12파일에서 `[0-9]+` 와 **낱말 경계를 가진** 탭 명사
  (`\btabs?\b`/`탭`/`タブ`/`标签页`)가 12글자 이내로 인접한 줄이 **허용 목록과 정확히 일치**한다.
  base 실측 **15행**, 그중 14행은 열거된 자리이고 1행이 허용 대상(`ja/advanced/moai-web-console.md:149`).
  M3 이후 기대값은 허용 목록 1항목뿐이다. 허용 항목은 **줄 번호가 아니라 내용**으로 식별한다
  (`advanced/moai-web-console.md` 안에서 `codex` 토큰을 포함하는 줄) — 편집이 줄 번호를 움직이기 때문이다.
...
셋이 서로를 덮는다. N0 은 오늘의 20자리를, N1 은 새로 들어오는 **숫자** 계수를, N2 는 **이름**과 탭 수 변화를 잡는다.
낱말 표기로 새로 쓰인 계수는 어느 층도 잡지 못한다 — 닫히지 않는 방향이며 `spec.md §7` 에 적었다.
...
| 낱말 수사 축을 **정규식에 넣지 않음** | ko `탭 두 곳`, en `two tabs`, zh `两个标签页`(같은 codex 문단의 다른 로케일) | 넓힌 수사 클래스로 실측했을 때 모두 적중했다 |

마지막 줄이 D9(낱말 커버리지) 에 대한 답이기도 하다. 낱말 축을 열면 오탐이 로케일마다 늘어난다 —
그래서 낱말은 **N0 의 리터럴 집합**으로 닫고, 정규식은 숫자만 본다. 대신 탭 수가 바뀌면 N2 가 붉어지므로
낱말 계수의 노후화는 이름 축에서 검출된다.

### A.5 공허한 초록 방지 — 변이 지점을 파일 부류 × 로케일로 넓힌다
...
서로 다른 **파일 부류**와 서로 다른 **로케일**을 함께 건드려야 한다. run-phase 는 아래 셋을 각각 따로 깨뜨려 보인다.
...
| V3 | `docs-site/content/zh/cli-reference/web.md` 에 `14 个标签页` 를 되살림 | N1 숫자 스윕이 살아 있다 |

추가로 가드는 **자기가 읽은 파일 수를 출력**하고 그것이 선언된 12와 같은지 스스로 단정한다(AC-TCD-011).
읽은 집합의 크기를 판정 전에 세지 않으면, 아무것도 읽지 않은 초록과 전부 읽은 초록이 구분되지 않는다.
세 변이의 FAIL 출력과 되돌린 뒤의 PASS 출력을 모두 `progress.md` §E.2 에 기록한다. (AC-TCD-006)
...
| 가드 허용 목록 | 가드의 숫자 스윕이 잡지만 **고칠 수 없는** 정당한 계수 | **1항목** — ja:149 codex 근거 문장(범위 밖, §4) |
