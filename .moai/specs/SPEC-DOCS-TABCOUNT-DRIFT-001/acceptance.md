...
| AC-TCD-006 | REQ-TCD-004 | 변이 4종(파일 부류 × 로케일 × 수사 표기) | 가드 부재 | M5 |
...
| AC-TCD-012 | REQ-TCD-003, 005, 008 | 낱말 표기 6자리 — 가드 낱말 축 적중 | 6행 적중 | M2+M3 |
...
### AC-TCD-006 — 변이 4종: 파일 부류 × 로케일 × 수사 표기

**RED-now 셀**: 가드 부재. 붉은 이유: 가드가 무엇을 잡는지 보인 적이 없다.

**Given** 가드가 GREEN 인 트리에서,
**When** 아래 네 변이를 **각각 따로** 넣고 가드를 돌린 뒤 되돌리면,
...
| V4 | `docs-site/content/en/advanced/moai-web-console.md` | `fourteen tabs` → `fourteen settings tabs` | N1 **낱말** 축이 살아 있다 |

**Then** 네 변이가 각각 `--- FAIL` 을 내고, 실패 메시지가 **어느 파일의 무엇**이 어긋났는지 적시하며, 되돌린 뒤에는 모두 `ok` 다.
**And** 여덟 출력(FAIL 4 + PASS 4)을 `progress.md §E.2` 에 축어로 남긴다.

V4 가 이 AC 의 핵심이다. 감사가 실제로 성공시킨 변이체이며, 측정은 이렇다 —
...
### AC-TCD-012 — 낱말 표기 6자리에 두 번째 층이 있다

리터럴 층(AC-TCD-002)만으로는 낱말 표기 자리가 재작성 한 번에 빠져나간다. 이 AC 는 그 자리에 **가드의 낱말 축**이
실제로 걸리는지를 본다 — AC-TCD-006 V4 가 변이로 증명한다면, 이쪽은 축 자체의 적중 집합을 단정한다.
