...
- **AC-HPR-002** — **Given** event-input goldens for Stop, each run with the same project configuration and the same environment (codex binary present or absent through the `codexLookPath` seam, `internal/cli/mcp_codex.go:457`, and a stub reviewer on the Claude path):
  ...
  - sync gate: **HEAD not a sync-phase commit with the receipt absent**, sync-phase commit with a failing receipt, with a passing receipt, with the receipt absent, and with the blocking opt-out;
  - codex review gate (member 6): **codex installed, no receipt**; **installed, fresh PASS receipt**; **installed, FAIL receipt**; **stale receipt (HEAD moved after the receipt was written)**; **codex binary missing**; no reviewable change;
