...
| AC-HPR-002 | HOOK-01 | REQ-HPR-002, REQ-HPR-005, REQ-HPR-019 (cap) | golden | `go test -json -count=1 -run '^TestStopChainEffectParityGolden' ./internal/cli/` | make the Codex path return `{}` for the goal-unmet golden → fails; make the Codex goal or sync-gate `unmeasured` case allow the stop → fails; make a Codex multi-review-gate missing-result allow write no discard record → fails; make a Codex multi-review-gate missing-result case block → fails (Claude allows, `multi_review_gate.go:79`); make Codex member 6 allow with codex installed and no receipt → fails (Claude runs the review in-hook, `codex_review_gate.go:78–108`); make Codex member 6 accept a receipt whose HEAD differs → fails; make Codex member 6 block when the codex binary is missing → fails (Claude allows, `:79–80`); make the Codex sync gate require a receipt when HEAD is not a sync-phase commit → fails (Claude allows, `sync-phase-quality-gate.sh:207–218`); make Codex member 6 skip step 2 (`stop_hook_active`) → the codex-installed, no-receipt, `stop_hook_active: true` golden fails (Codex continues, Claude allows at `codex_review_gate.go:71–72`); make the Codex sync gate ignore `stop_hook_active` → the fresh-`fail`-receipt, `stop_hook_active: true` golden fails (Codex blocks, Claude allows at `sync-phase-quality-gate.sh:484–486`); remove the §D3.8 cap → the Nth-consecutive-`unmeasured` golden fails (Codex continues where the golden expects allow); suppress the `unverified` discard record at the cap → the Nth-consecutive-`unmeasured` golden fails (no record, or reason text not naming `unverified`) |
...
| AC-HPR-019 | POL-01 | REQ-HPR-022, REQ-HPR-023, REQ-HPR-024 | unit | `go test -json -count=1 -run '^TestParityVerdictAggregate' ./internal/<registry-pkg>/` | inject each of: skip; empty run; `NOT_RUN`; `UNSUPPORTED`; missing attribution field; config-existence-only evidence; a `pass` action paired with a `NOT_RUN` record; a Stop gate recorded `unverified` by the §D3.8 cap → each makes the aggregate not PASS |
...
- **AC-HPR-002** — **Given** event-input goldens for Stop, each run with the same project configuration and the same environment (codex binary present or absent through the `codexLookPath` seam, `internal/cli/mcp_codex.go:457`, and a stub reviewer on the Claude path):
  - goal: unmet, met, receipt absent, no goal;
  - sync gate: **HEAD not a sync-phase commit with the receipt absent**, sync-phase commit with a failing receipt, with a passing receipt, with the receipt absent, and with the blocking opt-out; **sync-phase commit with a fresh failing receipt and `stop_hook_active: true`** (on the Claude path, a stored `fail` record plus payload for the same HEAD and work tree, in blocking mode);
  - codex review gate (member 6): **codex installed, no receipt**; **installed, fresh PASS receipt**; **installed, FAIL receipt**; **stale receipt (HEAD moved after the receipt was written)**; **codex binary missing**; no reviewable change; **codex installed, no receipt, `stop_hook_active: true`**;
  - multi review gate (member 7): result blocking, result passing, **result missing**;
  - Codex-only cap goldens (design.md §D3.8), for the sync gate on a sync-phase commit and for member 6 with codex installed, each with the receipt absent and `stop_hook_active: false`: **N−1 consecutive `unmeasured` Stops on the same HEAD and working-tree digest**; **the Nth consecutive Stop**; **N−1 Stops, then a fresh receipt, then one more receipt-absent Stop**; **N−1 Stops, then a HEAD change, then one more receipt-absent Stop**;

  **When** each golden is run through the Claude path and the Codex path, **Then** the normalized decision is equal for every `required-gate`, `fail-open-on-missing`, and `goal` member, and the reason class is identical or paired by the design.md §D3.4 mapping. In particular:
  - sync gate with HEAD not a sync-phase commit → allow on both, and the Codex path does not read the receipt; sync-phase commit with the receipt absent → Codex continues the turn (`unmeasured`) and does not allow below the §D3.8 cap;
  - member 6 with codex installed (a continuation is a Stop `block` decision): no receipt → Codex continues (`unmeasured`, reason names the review runner command); fresh PASS receipt → allow; FAIL receipt → block; stale receipt → continues. Claude's path, given the stub reviewer's verdict for the same tree, allows on pass and blocks on FAIL;
  - member 6 with the codex binary missing → allow on both, and the Codex output also carries a discard record and reason text naming the missing reviewer;
  - member 6 with codex installed, no receipt, and `stop_hook_active: true` → allow on both (Claude step 2, `codex_review_gate.go:71–72`; Codex step 2 before any receipt read). This golden is the loop bound for the step-7 continuation; an implementation that omits step 2 continues here and fails it;
  - sync gate on a sync-phase commit with a fresh failing receipt and `stop_hook_active: true` → allow on both (Claude does not re-deliver the stored block, `sync-phase-quality-gate.sh:484–486`); the same input with `stop_hook_active: false` blocks on both;
  - member 7 with the result missing → allow on both (Claude: `multi_review_gate.go:47, :79`), and the Codex output also carries a discard record and reason text naming the missing result.
  - Codex-only cap goldens (declared parity deviation, design.md §D3.8; excluded from the Claude/Codex equality above, run on the Codex path only, with N the M2d constant): after N−1 consecutive `unmeasured` Stops → continue (`unmeasured`); the Nth → allow, with exactly one discard record and reason text naming the gate `unverified`, and the verdict record for that gate reads `unverified`, not PASS; N−1 Stops, a fresh receipt, then a receipt-absent Stop → continue (the count reset to 1); N−1 Stops, a HEAD change, then a receipt-absent Stop → continue (the count reset to 1).
...
- The durable partial-closure markers of spec.md §E are all present at close:
  - plan-phase markers, still in place (written by manager-spec in v0.4.0): the `live-uncertified` token in spec.md frontmatter `tags`, and the spec.md HISTORY line "v0.4.0 (closure mode)";
  - sync-phase markers, written by manager-docs: a progress.md §E.4 statement quoting the not-PASS aggregate verdict, listing the `NOT_RUN` live legs, and naming the follow-up card's id; and a CHANGELOG entry for this SPEC that says "partial (live-uncertified)" and names the same card.
  A close missing any of the four is not done.
