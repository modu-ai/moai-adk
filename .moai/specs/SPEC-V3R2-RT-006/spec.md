...
|---------|------|--------|-------------|
| 0.2.0 | 2026-05-18 | orchestrator | Run-phase COMPLETE — PR #984 admin merged. 35 tasks (T-RT006-01~35) all DONE (T-31 manual tmux QA deferred to sync). 16 ACs all PASS. SystemHookConfig + observabilityOptIn helper + Pattern A gate (4 RETIRE-OBS-ONLY handlers: notification + elicitation×2 + task_created). 26 handler files with Resolution: CATEGORY headers (KEEP/UPGRADE/FIX/RETIRE-OBS-ONLY/COMPOSITE). subagent_stop.go 500ms tmux pane kill timeout (P-H02 CRITICAL bug fix). config_change.go RT-005 Manager.Reload + 20ms debounce. coverage_table.go + doctor_hook.go (~150 LOC) `moai doctor hook` CLI with --json/--trace/--observability flags. 28-row coverage table (27 events + 1 composite). settings.json.tmpl 4 retire entries 제거. Status `in-progress → completed`. v3.0.0-rc1 P0 release-blocker 6건 중 3번째 완료. |
