...
| 0.3.1 | 2026-09-10 | **Scope-criterion left endpoint (t543).** acceptance.md AC-UDD-023 and plan.md §C pre-flight measured "no Go change" and "only the declared write surface" as a name-diff over a literal range pinned at `7f61332ef`, and AC-UDD-021 measured "no template-tree file is modified" as a `diff --stat` and a `log` over the same literal range; each turns false once the branch absorbs the integration branch before re-measuring: the range then holds other work's commits (the t543 reproduction on 2026-09-10 counted 51 Go files from other cards in a tree whose own contribution was 0 Go). All three now derive the left endpoint at read time as `CARD_BASE=$(git merge-base develop HEAD)`, add an unfiltered control (`>= 1`; `0` = not measurable), and state their range lines as pre-merge only, per `.claude/rules/local/gitflow-lane-protocol.md` §8; AC-UDD-021's unstaged check is unchanged, and the acceptance.md §D Definition of Done item that stated the template-tree criterion relative to `7f61332ef` is aligned to AC-UDD-021's read-time, pre-merge form. The v0.3.0 readings are kept as dated references. No requirement, status, or other criterion changed. |
