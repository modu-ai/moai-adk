...
> AC 도메인 접두사 `AC-CSM-NNN` (Tier L 상한 25 중 13). 각 항목은 이항 판정 가능해야 하고 검증 명령을 가진다. GEARS 의무는 spec.md §D(요구 계층)에 있다 — 이 문서는 검증 계층(Given-When-Then).
...
| AC-CSM-008 | REQ-CSM-010 | session_msg 소스에 codex 스폰/잡 토큰이 없다 | `grep -rn "exec.Command\|codex-jobs\|app-server" internal/sessionmsg/ internal/cli/mcp_session_msg.go \| grep -v _test` → 0행 |
...
- [ ] REQ-CSM-001..015 전부 AC에 매핑·이항 판정 (위 매트릭스 + 누락 0 — REQ-CSM-001/004는 AC-CSM-003/002의 전제로 간접 검증: 스토어 경로·하트비트)
