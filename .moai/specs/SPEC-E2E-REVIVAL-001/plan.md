...
- **D-1 — Agent name & placement**: `e2e-tester` at `.claude/agents/moai/e2e-tester.md` (FLAT moai/ layout, matching the 9 existing retained agents). Class: `core/specialist`. Rationale: "Define a custom subagent when you keep spawning the same kind of worker" (Anthropic best practices, already cited in CLAUDE.md §4); e2e execution is a recurring, tool-heavy, output-noisy worker profile that pollutes manager-develop's run-phase context. Alternative rejected: keeping manager-develop delegation (the retired design) — it conflates implementation-cycle ownership with test-execution ownership and forfeits the token-diet of a purpose-built agent.
- **D-2 — Catalog expansion consequence**: adding a 10th MoAI-custom agent BREAKS the literal CLAUDE.md §4 claim "exactly 10 retained agents (9 MoAI-custom + 1 Explore)" present in BOTH trees. The SPEC therefore owns updating: count text, catalog table row, Selection Decision Tree entry (new item 12: "E2E test execution across web/mobile/desktop? Use the e2e-tester subagent"), in both trees. Missing any of the three leaves the agent inert-by-omission. AUDIT-EXTENDED (iter-1 D2): the count-literal surface is larger than CLAUDE.md §4 alone — repo-wide grep (`'10 retained agents|9 MoAI-custom|10-agent'`, re-executed 2026-07-13) measures **12 files / 24 sites** across both trees: CLAUDE.md ×3 sites, agent-authoring.md ×3, agent-patterns.md ×3 (incl. the MoAI-custom name enumeration), model-policy.md ×1, spec-workflow.md ×1, manager-design.md ×1 — each ×2 trees. All are in M3 scope per REQ-E2E-302. Line numbers drift between trees (local agent-authoring.md is offset +1 from the template) → anchor every edit by content token, never line number. Post-change invariance: stale-literal grep over the touched files → 0 (CMD-019-INV). AUDIT-EXTENDED-2 (iter-2 D13, re-derived by executed repo-wide grep 2026-07-13): the TRUE repo-wide surface adds an outer ring — (a) template-distributed skill modules: moai-foundation-core SKILL.md + modules/agents-reference.md (gains an e2e-tester table row) + modules/INDEX.md + moai-foundation-quality SKILL.md (8 sites, TEMPLATE tree); OBSERVED PRE-EXISTING DRIFT: the 4 LOCAL siblings are two generations stale ("8 retained agents / 7 MoAI-custom", 7 sites measured, zero 10/9-family matches) — run-phase normalizes BOTH trees to 11/10; (b) README.md 3 sites; README.ko/ja/zh carry NO ERE-family matches — run-phase reviews their locale-language count claims instead; (c) Go tier-profile display pin → REQ-E2E-305 (data-row + test-pin under the C-7 carve-out; `tierProfileAgentOrder` in model_policy.go, len 10→11, 60→66 cells, comment sweep in model_policy_test.go + internal/web/modelpolicy_test.go); (d) docs-site 10-file inventory — EXPLICITLY DEFERRED (spec.md §E). Widened invariance baselines (measured): CMD-019-INV 19 files → 38 sites; CMD-019-INV-B local 8/7-era → 7 sites; both target 0 post-change.
...
11. Re-run the count-literal surface grep TRULY REPO-WIDE (D-2 / REQ-E2E-302): `grep -rEln '10 retained agents|9 MoAI-custom|10-agent'` over CLAUDE.md, README*, .claude, internal/template, internal/web, docs-site (excluding .moai/specs, .moai/reports, agent-memory, worktrees) — reconcile the 19-file in-scope surface + the 10-file docs-site deferral inventory against HEAD before M3 (measured baselines 2026-07-13: CMD-019-INV 38, CMD-019-INV-B 7; the surface can grow between plan and run)
...
- [HARD] The e2e-tester `tools:` CSV excludes `Agent` and `AskUserQuestion`. Include: Read, Write, Edit, Bash, Grep, Glob, TaskCreate, TaskUpdate, TaskList, TaskGet, Skill (mirror manager-develop's profile minus mcp__context7; final list in design.md §D).
...
- E1: AC matrix PASS/FAIL table (all 28), each row citing an executed command + verbatim-output path
- E2: `go test ./internal/template/...` and `go test ./...` exit 0 (full suite, not just touched packages)
- E3: `golangci-lint run` exit 0
- E4: Subagent-boundary grep — e2e-tester tools line clean of `Agent`/`AskUserQuestion`
...
- E7: `moai spec lint --strict .moai/specs/SPEC-E2E-REVIVAL-001/spec.md` → 0 ERRORS (the lint CLI takes the spec.md FILE; a directory argument fails with `ParseFailure … is a directory` — iter-1 D7). The pre-existing `StatusGitConsistency` WARNING (frontmatter status vs git-implied status from this SPEC's own `feat()` commits) is EXPECTED until sync close and is NOT a gate failure (iter-2 D14).

## §F Milestones

### M1 — e2e-tester agent (template tree) — Priority: High, most design-sensitive

- Author `internal/template/templates/.claude/agents/moai/e2e-tester.md` per design.md §D: frontmatter (name, description with PROACTIVELY + "NOT for:" clause, tools CSV, `model: inherit`, `effort: high`, color, `permissionMode`, `memory: project`, `skills:` ≤2), body (scope, platform-toolchain execution knowledge, token-minimization ladder [HARD], blocker-report protocol, artifact-directory conventions, delegation-return contract).
- Sync byte-identical copy to `.claude/agents/moai/e2e-tester.md`.
...
- Author `internal/template/templates/.claude/skills/moai/workflows/e2e.md` per design.md §E: frontmatter (`user-invocable: false`, triggers, version reset to 3.0.0 lineage), Phase 0 detection (marker matrix §B) → Phase 0.5 selection (orchestrator AskUserQuestion contract — the workflow INSTRUCTS the orchestrator; it never claims the agent prompts) → Phase 1 journey mapping → Phase 2 script creation (per-toolchain file conventions) → Phase 3 execution (CLI-first, bounded output) → Phase 4 recording (native facilities) → Phase 5 report (conversation_language). Embed: platform-toolchain matrix with token-cost column, 3-rung MCP escalation ladder, e2e-tester delegation directives per phase.
- Sync identical copy to local tree.
- Exit: both trees identical; body contains `e2e-tester` delegation in ≥3 phase sections; ≤~450 lines; zero neutrality-class violations.
...
- Count-literal sweep, ring 1 (both trees, REQ-E2E-302): agent-authoring.md (3 sites), agent-patterns.md (3 sites incl. adding `e2e-tester` to the MoAI-custom enumeration), model-policy.md (1 site), spec-workflow.md (1 site incl. named list), manager-design.md (1 site). Content-token anchored edits (line numbers drift +1 between trees for agent-authoring.md).
- Count-literal sweep, rings 2-3 (REQ-E2E-302): moai-foundation-core SKILL.md / modules/agents-reference.md (incl. NEW `e2e-tester` table row) / modules/INDEX.md + moai-foundation-quality SKILL.md — TEMPLATE tree updated to 11/10 AND the two-generations-stale LOCAL siblings (8/7-era, 7 sites) normalized to the same content; README.md 3 sites; README.ko/ja/zh locale-language count-claim review (no ERE-family matches measured — verify in-language claims manually).
- Exit: grep-verifiable deltas per acceptance.md AC-E2E-017…021 (baseline 0 → 1 per surface, both trees) + invariance CMD-019-INV (19 files) → 0 AND CMD-019-INV-B (local 8/7-era) → 0.

### M4 — catalog.yaml + CI-guard constants + make build + embed verification — Priority: Medium (mechanical)

- Manually add `e2e-tester` entry to `catalog.yaml` `core.agents` (name/tier: core/path/version: 1.0.0; hash placeholder).
- Update `expectedAgentCount` 9→10 and `expectedTotal` 37→38 with ledger-style provenance comments.
- REQ-E2E-305 Go display row: add `e2e-tester` to `tierProfileAgentOrder` + its tier-profile entries in `internal/template/model_policy.go`; reconcile the pins in the SAME commit — `model_policy_test.go` length pin 10→11, 60-cell assertion → 66 (2 plans × 11 agents × 3 tiers), "10 retained agents" comments → 11; `internal/web/modelpolicy_test.go` per-plan row expectation 10→11 + comment sweep.
...
- **AP-6 Inert agent**: shipping e2e-tester without the M3 catalog-table/decision-tree/workflow-delegation wiring — grep-green but unreachable (lesson: inert-deliverable reachability).
