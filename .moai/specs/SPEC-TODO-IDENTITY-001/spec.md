---
id: SPEC-TODO-IDENTITY-001
title: "Todo 프로젝트·카드 UUIDv7 영속 identity"
version: "0.1.0"
status: completed
created: 2026-09-12
updated: 2026-09-12
...
tags: "todo, identity, uuidv7, sqlite"
tier: M
related_specs: [SPEC-TODO-UNIFIED-001, SPEC-TODO-RUNTIME-STORE-001]
---

# SPEC-TODO-IDENTITY-001

## HISTORY

| 날짜 | 버전 | 변경 |
|---|---|---|
| 2026-09-12 | 0.1.0 | DELTA-ID-01: 사용자 Tier M/UUIDv7/null 선택과 구현 요청 기록; 정식 post-audit kickoff 대기 |

## 목적과 승인 경계

t648의 다음 최소 identity 단위다. 사용자는 UUIDv7, legacy 명시 null을 확정했고 직전 Tier M/구현 착수 질문에 “구현하고 검증 진행”으로 응답했다. 이는 구현 요청과 Tier M 범위 선택이다. 이후 사용자가 informed override와 Formal Implementation Kickoff Approval을 명시 승인했다. audit_verdict는 BYPASSED이며 독립 plan gate PASS나 품질 검증 면제가 아니다. umbrella23AC 중 identity 기반만 담당하며 전체 완료를 주장하지 않는다.
...
iteration-2 D10–D13 보강 및 최신 RED 증거 반영본이다. Tier M plan audit ceiling=2가 소진되었다. iteration-2 FAIL 0.75 이후 D10–D13 post-audit repair complete이며 independent re-audit unavailable이다. Explicit informed user override 및 formal Implementation Kickoff Approval approved. audit_verdict는 BYPASSED이며 감사 FAIL 원문은 보존한다. 구현/PASS/in-progress가 아니다. exact persistence/검증 경계는 plan.md에 고정한다. backup 파일 복원은 이 child의 transaction rollback AC와 구분한 비차단 후속 작업이며 미검증이다. 기존 감사 FAIL을 PASS로 변경하지 않는다.

## 종료 조건

7 AC의 실제 명령/출력/기준과 독립 감사가 필요하다. 기존 UUID 부재 RED를 version·equality·lifetime·backfill·경쟁 안전성의 실측으로 소급하지 않는다. 전체 umbrella23AC는 후속 누적 감사에서 유지한다.
