...
- **SPEC-V3-SCH-001** — `moai doctor config` subcommand depends on the schema validator registry; the doctor family cannot ship without it.
- **SPEC-V3-MIG-001** — `moai doctor migration` subcommand and `PersistentPreRunE` migration wiring depend on the migration runner framework.
...
- **SPEC-V3-HOOKS-001** — `moai doctor hook` subcommand requires Hook Protocol v2; Related (not Blocked) because CLI can ship with doctor hook subcommand disabled if HOOKS-001 slips.
- **SPEC-V3-AGT-001** — `moai doctor agent` subcommand requires agent frontmatter v2; Related (not Blocked) because CLI can ship with doctor agent subcommand disabled if AGT-001 slips.
