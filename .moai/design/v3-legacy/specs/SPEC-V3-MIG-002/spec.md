...
author: manager-spec
priority: P1 High
phase: "v3.0.0 — Phase 1 — Foundation"
...
- **Owns**: `internal/core/migration/steps/m0{1,2,3,4,5}*.go` — ALL migration step Go implementations including M02 agency archival + docs-site locale sync (absorbed scope).
...
- **M02 — .agency/ Archive** (Version=2, One-shot):
  ...
  - Apply: `docs/archive/agency-v2/{commands,rules,backups}/`로 이동 (delete 아님; canonical archive path per master §3.9)
  - Rollback: archive에서 원 위치로 복원
  - [HARD] Never delete; move only
  - Note: `~/.moai/history/v2.12/` backup-only snapshot은 별도 safeguard로 유지 가능하나 canonical destination은 `docs/archive/agency-v2/`.
...
- **M02 agency archival** — migrate `.agency/` directories to `docs/archive/agency-v2/` with backlink retention (absorbed scope; implementation lives in `m02_agency_archive.go`).
- **docs-site 4-locale sync** — sync `docs-site/content/{en,ko,ja,zh}/` on version bump via explicit migration step (absorbed scope; implementation lives in `m02_agency_archive.go` locale sync helper).
...
- **Diagnostic CLI tooling** (`moai doctor template-drift`, `moai doctor skill-drift`, `moai doctor legacy-cleanup`) — owned by SPEC-V3-CLN-001/002.
...
- 영향 디렉터리 (실행 시점):
  ...
  - `docs/archive/agency-v2/` (M02 canonical archive 대상; master §3.9)
...
- A-002 (High): canonical `docs/archive/agency-v2/` 디렉터리는 M02 실행 시 자동 생성되며, 기존 내용이 있으면 append (never overwrite).
...
- A-005 (Medium): 사용자가 M02 canonical archive 위치(`docs/archive/agency-v2/`)를 직접 참조하지 않는다. 문서화된 복구 경로 외 의존 금지.
...
**When** M02 `Apply()`가 호출되면, the 시스템 **shall** `.claude/commands/agency/*.md` (8 파일), `.claude/rules/agency/constitution.md`, `.moai-backups/`를 canonical `docs/archive/agency-v2/` 경로로 **이동**(move)하고, 원 위치에는 파일/디렉터리를 남기지 않는다.
...
**Where** M02 archive 후 canonical `docs/archive/agency-v2/`에 이미 동일 이름 파일이 존재하면, the 시스템 **shall** `.{N}.bak` suffix를 붙여 충돌 회피한다 (never overwrite).
...
### 5.7 M02 Agency Archival Requirements (absorbed scope)

**REQ-MIG-002-052 (Ubiquitous)**
시스템은 **항상** M02 Apply 시 `.agency/` 하위의 모든 디렉터리(`.claude/commands/agency/`, `.claude/rules/agency/`, `.moai-backups/`)를 canonical `docs/archive/agency-v2/` 경로로 이동하며, 원 위치에는 파일을 남기지 않는다 (canonical archive path per master §3.9).

**REQ-MIG-002-053 (Ubiquitous)**
시스템은 **항상** M02 archive 수행 후 `docs/archive/agency-v2/README.md`에 원본 위치 → archive 위치 매핑 표(backlink retention)를 생성하여 사용자가 archive 위치를 확인할 수 있도록 한다.

**REQ-MIG-002-054 (Event-Driven)**
**When** M02 Apply가 `.agency/` 디렉터리 이동을 완료하면, the 시스템 **shall** 이동한 파일 개수, 총 바이트, archive 경로를 post-migration report에 기록한다.

**REQ-MIG-002-055 (Event-Driven)**
**When** M02 Apply가 호출되고 대상 archive 경로 `docs/archive/agency-v2/`가 이미 존재하면, the 시스템 **shall** 기존 archive 내용을 보존하면서 새 이동 파일만 추가한다 (never overwrite).

**REQ-MIG-002-056 (State-Driven)**
**While** M02 archive 대상 디렉터리가 `UserModified` provenance인 경우, the 시스템 **shall** 해당 디렉터리를 archive 위치로 이동하되 report에 "User-modified archived: <path>" 로그를 emit하여 사용자가 인지할 수 있도록 한다.

**REQ-MIG-002-057 (Unwanted)**
시스템은 M02 실행 시 `.agency/` 경로의 파일을 **삭제하지 않아야 한다**. 이동(move) 전용이며, archive 경로로 이동 실패 시 step은 전체를 rollback한다.

### 5.8 docs-site 4-Locale Sync Requirements (absorbed scope)

**REQ-MIG-002-058 (Ubiquitous)**
시스템은 **항상** M02 Apply 단계에 docs-site 4-locale sync 보조 단계를 포함하며, `docs-site/content/ko/` canonical 기준으로 `en/`, `ja/`, `zh/` 디렉터리의 누락 파일(placeholder)을 탐지한다.

**REQ-MIG-002-059 (Event-Driven)**
**When** M02 locale sync 보조 단계가 실행되면, the 시스템 **shall** `docs-site/content/{en,ja,zh}/migration/v3.md` 4-locale 파일 존재 여부를 검증하고, 누락된 locale에 대해 ko 파일에서 파생한 `_TBD_` placeholder를 생성한다 (CLAUDE.local.md §17.3 준수).

**REQ-MIG-002-060 (State-Driven)**
**While** locale sync 단계가 실행 중인 상태에서, the 시스템 **shall** front matter의 `translation_status: pending` 필드를 자동 삽입하여 release CI가 4-locale SLA 위반을 탐지 가능하도록 한다.

**REQ-MIG-002-061 (Unwanted)**
시스템은 locale sync 단계에서 기존 번역 완료 파일(`translation_status: complete` 또는 미지정 상태)을 **덮어쓰지 않아야 한다**. 누락된 locale만 placeholder로 생성한다.
...
**AC-MIG-002-02** (M02): `.claude/commands/agency/migrate.md` 존재 상태에서 M02 Apply 후 해당 파일이 `docs/archive/agency-v2/commands/agency/migrate.md`로 이동, 원 위치 파일 부재.
...
**AC-MIG-002-09** (Non-destructive archive): M02 Apply 이후 `docs/archive/agency-v2/`에 모든 archived 내용이 존재하고, 원본 소실 없음 (전체 바이트 합 일치).
...
**AC-MIG-002-M02-01**: Given `.agency/` 디렉터리가 존재하는 v2.12 프로젝트, When M02 Apply 완료 Then `docs/archive/agency-v2/` 경로에 모든 `.agency/` 내용이 이동되었고, `docs/archive/agency-v2/README.md`에 backlink 매핑 표가 생성됨 (maps REQ-MIG-002-052, REQ-MIG-002-053).

**AC-MIG-002-M02-02**: Given `docs/archive/agency-v2/`가 이전 실행으로 이미 존재 When M02 Apply 재실행 Then 기존 archive는 보존되며 새 이동 파일만 append됨 (maps REQ-MIG-002-055).

**AC-MIG-002-M02-03**: Given M02 Apply 수행 중 archive 경로로 이동 실패 When step이 실패 Then 원 `.agency/` 파일은 그대로 유지되고 step은 전체 rollback됨 (maps REQ-MIG-002-057).

**AC-MIG-002-LOC-01**: Given `docs-site/content/ko/migration/v3.md`만 존재하고 `en/`, `ja/`, `zh/`가 누락 When M02 locale sync 보조 단계 실행 Then 3개 locale에 `_TBD_` placeholder 파일이 `translation_status: pending` front matter와 함께 생성됨 (maps REQ-MIG-002-058, REQ-MIG-002-059, REQ-MIG-002-060).

**AC-MIG-002-LOC-02**: Given 4-locale 파일이 모두 존재하며 `en/migration/v3.md`가 `translation_status: complete` 상태 When M02 locale sync 실행 Then 기존 `en/` 파일은 변경되지 않음 (maps REQ-MIG-002-061).
...
| R-MIG-002-02 | M02 canonical archive 대상 `docs/archive/agency-v2/`에 동일 이름 파일이 이미 존재 시 충돌 | Low | Medium | `.{N}.bak` suffix 추가 (REQ-MIG-002-031) |
...
- **SPEC-V3-CLN-001/002** (Internal Cleanup SPECs — other writer) — 중복 항목 (M03/M04/M05와 overlap); cleanup SPEC은 본 migration을 reference
...
| REQ-MIG-002-052 | `internal/core/migration/steps/m02_agency_archive.go` (canonical move) | `TestM02CanonicalArchivePath` |
| REQ-MIG-002-053 | `internal/core/migration/steps/m02_agency_archive.go` (README backlink) | `TestM02BacklinkReadme` |
| REQ-MIG-002-054 | `internal/core/migration/report.go` (M02 post-migration report) | `TestM02ReportMetrics` |
| REQ-MIG-002-055 | `internal/core/migration/steps/m02_agency_archive.go` (existing archive append) | `TestM02ExistingArchiveAppend` |
| REQ-MIG-002-056 | `internal/core/migration/steps/m02_agency_archive.go` (UserModified provenance log) | `TestM02UserModifiedArchiveLog` |
| REQ-MIG-002-057 | `internal/core/migration/steps/m02_agency_archive.go` (move-not-delete invariant + rollback) | `TestM02MoveNotDelete`, `TestM02RollbackOnFailure` |
| REQ-MIG-002-058 | `internal/core/migration/steps/m05_docs_locale_sync.go` (ko canonical detection) | `TestLocaleSyncDetectsMissingLocales` |
| REQ-MIG-002-059 | `internal/core/migration/steps/m05_docs_locale_sync.go` (placeholder generation) | `TestLocaleSyncGeneratesPlaceholders` |
| REQ-MIG-002-060 | `internal/core/migration/steps/m05_docs_locale_sync.go` (translation_status pending) | `TestLocaleSyncSetsPendingStatus` |
| REQ-MIG-002-061 | `internal/core/migration/steps/m05_docs_locale_sync.go` (complete-status protection) | `TestLocaleSyncPreservesComplete` |
| AC-MIG-002-01 ~ 12, AC-MIG-002-M02-01 ~ 03, AC-MIG-002-LOC-01 ~ 02 | 해당 REQ 커버리지 전체 | `go test ./internal/core/migration/steps/...` + 10-project corpus e2e |
