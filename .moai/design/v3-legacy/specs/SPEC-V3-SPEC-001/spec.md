...
- **AC-SPEC-001-05**: Child REQ `REQ-<X>-<NNN>` with `override: true` replaces parent's `REQ-<X>-<NNN>`; without `override: true` parent wins and warning is logged. (maps REQ-SPEC-001-012, -030)
- **AC-SPEC-001-06**: A REQ with `traces_to: [REQ-V3-<PARENT>-<NNN>-<NNN>]` passes trace-index validation when parent exists; fails with `ErrTraceTargetNotFound` when parent REQ is absent. (maps REQ-SPEC-001-014)
