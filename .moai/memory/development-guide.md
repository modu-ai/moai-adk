...
### 🎯 완전성 우선 원칙 (Completeness First) - v0.4.0+ 필수

**GitHub Issue #48 회귀 방지**: Alfred와 모든 에이전트는 사용자가 요청한 **모든 작업을 완료**해야 합니다.
...
2. **사용자 요청 존중**
   - 암묵적 생략 금지
   - "모든" "전체" "각각" 등 명시적 요청 존중
   - 예시: "5개 파일 리팩토링" → 5개 모두 처리

3. **생략은 명시적 동의 후**
   - 일부만 처리할 필요가 있으면 먼저 설명
   - 사용자 승인 없이 생략 금지
   - 승인 후에도 이유 기록 필수
...
```
사용자: "5개 파일 리팩토링"
잘못된 응답: "시간 절약을 위해 1개만 처리하고 나머지는 TAG만 달겠습니다"
```

❌ **"나중에 하겠습니다" (사용자 동의 없이)**
```
잘못된 응답: "효율을 위해 나중에 처리하겠습니다"
```

❌ **작업 미완료 상태로 보고**
```
잘못된 보고: "완료했습니다" (실제로는 1개만 처리)
```
...
⚠️ 처리 불가 상황 발생:
현재 상황: [구체적 이유]
영향: file4, file5는 현재 처리 불가

대안 제시:
1. 지금 처리 가능한 3개 파일 먼저 처리
2. [조건 충족 후] 남은 2개 파일 처리
3. 또는: 우선순위 변경
...
- [ ] 사용자 요청 범위 명확히 파악했는가?
- [ ] 5개 요청 = 5개 처리인가?
- [ ] "모두" "전체" "각각" 표현 놓친 것은 없는가?
- [ ] 일부만 처리할 이유가 명시적으로 있는가?
- [ ] 있다면 사용자 승인을 받았는가?

작업 보고 전 확인:
- [ ] 모든 항목이 처리되었는가?
- [ ] 미처리 항목은 명확히 구분했는가?
- [ ] "완료했습니다" ≠ "일부만 했습니다" 구분했는가?
