...
---
#### 8. tag-agent (TAG System Management)
...
- TAG system health checks

**Example Delegation**:
```python
Task(
    subagent_type="tag-agent",
    description="Validate authentication feature TAG chain",
    prompt="""Verify TAG chain integrity for authentication feature:
...
Verify:
- All TAGs present and correctly formatted
- Bidirectional links working
- No orphaned TAGs in chain
- TAG chain follows SPEC → TEST → CODE → DOC sequence

Output: TAG chain validation report with any issues found.
"""
)
```
...
- [`reference.md`](./reference.md) - Complete 19-agent team roster with detailed responsibilities
- [`examples.md`](./examples.md) - Practical agent selection examples and decision trees
