...
iteration: 2 (D1-D6 P0/P1 + D7-D10 P2/P3 모두 반영)
prev_audit: plan-auditor FAIL 0.62 (D1-D10 defects)
classification: VISION_PROPOSAL (pending plan-auditor iteration 2 review)
status: revised, pending audit
---
...
| D10 (Q3 determinism) | ✓ resolved | §3.5 Design Constraint로 promote (deterministic generation 보장) |
...
2. /moai project (Socratic interview + meta-harness 7-Phase)
   ...
   └─ my-harness-* skills/agents 자동 생성 (deterministic — §3.5)
...
### 3.5 [HARD] Deterministic Harness Generation (D10 resolution)

`/moai project`는 동일 프로젝트 입력에 대해 항상 동일한 `my-harness-*` 세트를 생성해야 한다.

**Determinism Requirement** (E3 resolution — LLM determinism scoped):
- 입력: Socratic interview 답변 (text) + project marker files (go.mod, package.json, ...) + 기존 `.moai/config/sections/*.yaml`
- 출력: my-harness-* skills + agents 파일 세트
- 동일 입력 →
  - **Template-rendered portions**: bit-exact identical (file names, frontmatter, structural sections)
  - **LLM-generated bodies**: semantically equivalent (concept-level similarity ≥ 0.95, exact match best-effort)
- **Note**: LLM determinism은 temp=0에도 model version/provider kv-cache state 따라 token-level 차이 발생 가능. 따라서 "bit-exact" 보장은 template-rendered 영역에만 적용 — LLM-generated body는 semantic equivalence target.

**Implementation guarantees**:
1. Generation seed = SHA256(sorted_interview_answers + project_markers_hash)
2. Template selection deterministic (no random sampling) — bit-exact for templates
3. LLM call (메타-harness generation) sets temperature = 0 — best-effort determinism, not contractual
4. Generated files include header `# Generated by meta-harness vX.Y.Z, seed: <seed-hash>, llm-determinism: best-effort`
5. CI test (`meta_harness_determinism_test.go`)가 3 standard input set × 2 runs 비교:
   - Template portions: byte-diff = 0 (PASS criterion)
   - LLM portions: semantic similarity ≥ 0.95 (e.g., embedding cosine similarity, or AST equivalence for structured outputs)

**Implication for CI/team consistency**:
- 같은 프로젝트 clone 후 `/moai project --refresh` → identical harness (team 간 분기 없음)
- Generated harness diff-tracked in git (확정적 변경만)
- `moai harness verify --determinism`이 seed 일치 검증
...
### W4 — SPEC-V3R5-PROJECT-MEGA-001 (P1, polish)

**Scope** — `/moai project` 강화:
- revfactory/harness 7-Phase workflow 채택
- Socratic interview 결과 기반 프로젝트 type 분류
- 분류 결과 → my-harness-* skills + agents 자동 생성
- **Deterministic generation guarantee (§3.5)**:
  - Seed = SHA256(sorted_interview_answers + project_markers_hash)
  - LLM temperature = 0
  - File header `# Generated by meta-harness vX.Y.Z, seed: <hash>`
  - CI test (`meta_harness_determinism_test.go`)
- `/moai project --refresh` 명령 (기존 harness 업데이트 — 새 seed 반영)
- Seed library (`.claude/skills/moai-meta-harness/seeds/`) 초기 baseline 8개 (Go/Node/Python/Rust/React/Vue/Flutter/iOS)
...
- Determinism test: 동일 입력 × 2 runs → template portions bit-exact + LLM bodies semantic equivalence ≥0.95 (per §3.5)
...
- [ ] Generation determinism guaranteed (D10 resolved → §3.5)
