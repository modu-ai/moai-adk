...
| LR-02 | error | error | `Agent` token in `tools:` CSV list. Forbidden EXCEPT for the orchestrator (main session, not an agent file) and `manager-kanban` (sole hierarchical-team coordinator per SPEC-HIERARCHICAL-TEAM-001 REQ-LEAD-001, depth-2 sealed — leaf workers it spawns omit Agent, enforced by the `manager_kanban_depth_test.go` CI guard). |
