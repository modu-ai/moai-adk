# Dev-Only Commands Isolation (Split Harnesses)
...
> **2026-06-22 — SPEC-V3R6-DEV-HARNESS-SPLIT-001**: the three capabilities of the formerly-unified dev-maintainer harness were split into three INDEPENDENT harnesses (`/harness:release-update`, `/harness:github`, `/harness:release`). This reverses the unified-entry decision of SPEC-V3R6-DEV-HARNESS-CONSOLIDATION-001 (which had consolidated the three numeric commands `97-release-update` / `98-github` / `99-release` into one unified entry; that prior consolidation note is preserved below for migration history). The doctrine below tracks the three split harness-namespaced artifacts.

---


### [HARD] 3개 split harness + 관련 산출물은 로컬 moai-adk 개발 전용

세 split harness (`/harness:release-update`, `/harness:github`, `/harness:release` 진입) 와 그 산출물은 모두 moai-adk-go 메인테이너 전용 도구다. 패키지 사용자 프로젝트에는 **절대 배포되어서는 안 된다**. `internal/template/templates/` 어디에도 흔적이 남으면 안 된다.

3개 독립 harness 는 각각 1개 capability 를 직접 호출한다:
- `release-update` — 외부 시스템 추적/동기화 (CC upstream tracker; 구 `97-release-update`). 유일하게 비-상호작용 research fan-out 을 가지므로 Runner + manifest 보유.
- `github` — GitHub 등 외부 플랫폼 워크플로우 (issue/PR; 구 `98-github`). 순수 human-gated specialist — Runner/manifest 없음.
- `release` — production release 워크플로우 (Enhanced GitHub Flow; 구 `99-release`). 순수 human-gated specialist — Runner/manifest 없음.

> **Retired numeric prefixes (historical)**: 구 `97-*` / `98-*` / `99-*` 번호 prefix 슬래시 커맨드 관례는 SPEC-V3R6-DEV-HARNESS-CONSOLIDATION-001 에서 폐지되었다. 세 번호 커맨드는 한때 단일 unified 진입점으로 통합되었다가, SPEC-V3R6-DEV-HARNESS-SPLIT-001 에서 3개 독립 harness 로 분리되었다. dev-only 격리 보호는 번호 grep 에서 harness-namespace embedded-tree-absence 단언으로 이전되었다.

### 배포 금지 파일 목록

3개 split harness 별로 구분 (release-update 만 Runner + manifest 보유; github / release 는 thin command → specialist 직접):

| Split harness | 파일 경로 | 목적 | 격리 이유 |
|---------------|---------|------|----------|
