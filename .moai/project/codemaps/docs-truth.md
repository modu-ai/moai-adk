...
Additional verbs registered across the `internal/cli/` tree (261 non-test `.go` files including subpackages, 201 non-test `.AddCommand(` calls): `hook`, `agent`, `research`, `workflow`, `migrate`, `profile`, `lsp`, `github`, `clean`, `feedback`, `review`, `coverage`, `e2e`, `codemaps`, `design`, `project`, `plan`, `run`, `sync`, `harness`, `session`, `fix`, `gate`, `mx`.
...
**Source:** `moai --help` rendered output + `grep -rn '\.AddCommand(' internal/cli/ --include='*.go' | grep -v _test` (201 non-test calls) + `grep -rhn 'rootCmd\.AddCommand(' internal/cli --include='*.go' | grep -v _test | wc -l` (61 root registrations) + `find internal/cli -name '*.go' ! -name '*_test.go' | wc -l` (261). Re-verified 2026-08-31: 201 / 61 / 261.
