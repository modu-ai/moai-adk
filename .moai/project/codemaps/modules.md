...
| `internal/git` | 8 | `internal/core/git` 위에 얹은 상위 유틸리티. **루트 패키지 자체의 비테스트 import 는 0이고, 최상위 집계 fan-in 1은 전부 하위 `convention`이 받은 것**이다 — 아래 §네거티브 스페이스 | `convention` |
| `internal/core/git` | (core 24 중) | 저수준 리포지터리 프리미티브 — 매니저, 브랜치, 체크아웃, 충돌, 워크트리, 이벤트, 오류 타입. `internal/git`과 `internal/stateanchor`가 이 위에 얹힌다 | — |
...
3. 세션 디렉터리에 대한 리포지터리 해석 — `internal/core/git`의 공통 디렉터리 해석을 재사용하며,
