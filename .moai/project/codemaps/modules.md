...
**현재 갱신 — t1274, worktree `.claude/worktrees/t1274`, 브랜치 `WT-codemaps-refresh4`, base `cf4b82755` (2026-09-26).**
...
| `internal/cli` | 351 | 아래 클러스터 표 참조 | `update`(+`plan`/`deploy`/`merge`/`backup`/`report`), `harness`, `worktree`, `agentlint`, `preference`, `wizard`, `uikit`, `printer`, `specid`, `taskledger`, `pr`, `ptycaptest`, `jev` |
| `internal/hook` | 147 | Claude Code 26종 훅 이벤트의 핸들러 레지스트리와 개별 핸들러. `registry.Dispatch`가 이벤트별 체인을 돌려 `HookOutput`을 만든다. **t1274 판에서 `escalation_observe.go`가 더했다** — `internal/escalation` 감지기의 유일한 비테스트 소비자. `Active` 게이트를 지나 계약 이벤트를 조립해 `Observe` 로 넘기며, 감지기가 관측 전용이라 이 파일도 결정에 개입하지 않는다(card t1235, SPEC-AUTONOMY-ESCALATION-001). **t1238 판: 명령 텍스트를 읽는 모든 가드·증거 분기가 `IsShellTool`(`internal/hook/shell_tool.go` — 닫힌 집합 Bash·PowerShell)을 거친다.** Bash용으로 쓴 가드가 PowerShell 도구로 보낸 같은 명령에서 조용히 빠지지 않게 하려는 것이다. 브랜치 가드와 통합 락이 POSIX식 스캔으로 볼 수 없는 PowerShell 우회 형태(`Invoke-Expression`·`Start-Process` 안의 git, `-EncodedCommand` 페이로드)는 거부하지 않고 허용하되 가드 감사 로그에 `unclassifiable` 한 줄을 남긴다(`internal/hook/powershell_indirection.go` — 파서 추가 없음, 인코딩 페이로드는 디코드하지 않음). **이 판에서 `factory_handoff_bind.go`가 더했다** — SPEC-FACTORY-LANE-WORKTREE-HANDOFF-001 M3 원자적 rebind의 훅 쪽 절반(대화형 lane의 다음 턴 증거를 통한 bind; 헤드리스 절반은 `internal/cli/factory_lane_handoff_bind.go`). 이전 판에서 `subagent_write_guard.go`(SPEC-SUBAGENT-WRITE-SHRINK-GUARD-001, PreToolUse: 서브에이전트의 `Write`가 추적 중인 기존 파일을 바이트 기준 큰 폭으로 축소하면 거부. 판별은 경로 범위가 아니라 파괴성 자체 — 2000바이트 이상(SWG-T1)이던 파일을 25% 이하(SWG-T2)로 줄이는 전체 덮어쓰기. 거부 계층만 `workflow.subagent_write_guard.enabled`에 게이트되고 감지·감사는 무조건 돈다)와 `factory_messages.go`(SessionStart·UserPromptSubmit에서 `internal/factorymsg` peer를 등록하는 훅 쪽 배선)가 더했다. 그 앞 판에서 `internal/hook/session_heartbeat.go`가 더했다 — UserPromptSubmit마다 세션 레지스트리의 `last_heartbeat`를 갱신하는 seam(등록된 세션에만, 모든 실패는 침묵. 실측 근거는 § `entry-points.md` 훅 절) | `quality`, `security`, `mx`(+`complexity`), `memo`(+`taxonomy`), `handoff`, `perf`, `trace`, `testutil` |
...
루트 273개 비테스트 파일(`find internal/cli -maxdepth 1 -name '*.go' -not -name '*_test.go'`)을
...
| `doctor*` | 20 | 진단 — **t1274 판에서 `doctor_settings_defaultmode.go`("Settings DefaultMode" — project/local settings 의 `permissions.defaultMode="bypassPermissions"` 는 CC v2.1.142+ scope 규칙(경고는 2.1.283+)에서 무시되는 죽은 키다. warn 으로 잡고 런처 플래그(`--permission-mode`)나 user-scope settings 를 안내한다. codex-only 프로젝트에서는 claude-surface 체크로 INFO 강등)가 더했다**, config, disk, harness, hook wiring, mcp version, permission, sandbox, skills, worktree base, agentemit embed, codex, jev(게이트·credential·도달성을 읽기 전용으로 확인 — 판정 요청을 보내지 않는다; 도달성은 TCP 접속·종료만 한다), git-strategy workflow 판정(`doctor_git_strategy_workflow.go`). t1238 판에서 `doctor_mcp_provider.go`("MCP Provider Duplicates" — 로컬 MCP 서버와 claude.ai 커넥터의 이름이 달라 둘 다 로드되는 중복; 비공개 내부 파일을 읽으므로 권고 전용·fail-open)가 더했다. binary-lag 판정은 비교 ref가 바이너리보다 오래된 경우(`StatusAhead`)를 OK 대신 WARN으로 보고한다 — 그 비교는 바이너리의 신선도에 대해 아무 말도 하지 못한다 |
| `mcp*` | 23 | 세 갈래. **t1238 판에서 `mcp_worktree_root.go`가 더했다** — `.moai/`를 추적하지 않는 저장소의 링크된 워크트리는 자기 `.moai`가 없어 `project_root` 검증이 거부했는데, git 기반 두 번째 수용 분기(스크럽된 git 검사, 종료 상태·출력 형태로만 판정, 실패 시 거부)와 "config-orphaned" 판정, 그리고 그런 트리에서도 primary의 `workflow.audit.gates`가 구속력을 유지하게 하는 감사 게이트 라우팅을 둔다. `mcp_server.go`(51KB)는 stdio JSON-RPC 서버, `mcp.go`/`mcp_codex.go`(97KB — CLI 최대 파일)/`mcp_glm.go`/`mcp_convergence.go`는 codex·GLM 위임과 다중 모델 감사 수렴, 그리고 `mcp_claude*.go` 5개(`_runner` · `_protocol` · `_process_unix` · `_process_windows` 포함)는 `claude` CLI를 서브프로세스로 띄우는 읽기 전용 `claude_audit` 도구다. 서브프로세스 환경에서 `CLAUDE_CODE_*`·`CLAUDECODE`를 지우고 출력 상한을 둔다. `audit_multi` 수렴도 `mcp_convergence.go`에서 같은 함수를 부른다. **이 판에서 둘이 더했다** — `mcp_factory_msg.go`(`internal/factorymsg` 위의 5개 도구 `factory_msg_{send,list,body,receipt,status}` — 발신자는 현재 세션·프로세스로 귀속되고 본문은 `factory_msg_body`로만 신뢰되지 않는 데이터로 읽힌다)와 `mcp_jev.go`(게이트된 `jev_ask` 1개 — `workflow.jev.enabled` 기본 꺼짐이면 요청 자체를 조립하지 않는다) |
...
| `hook*` | 10 | 훅 디스패처 진입점(`hook.go`, 62KB)과 pre-commit/pre-push 설치. **t1238 판에서 fail-closed seam 둘이 CLI 층에 더했다**(`internal/hook`은 건드리지 않는다) — `hook_stdin_failclosed.go`는 stdin을 JSON으로 파싱하지 못한 결정성 이벤트에 no-opinion 객체 대신 deny를 내고(사유는 고정 원인 + 운영자 문서 경로 `.moai/docs/hook-stdin-fail-closed.md` — 템플릿 원본은 `internal/template/templates/.moai/docs/hook-stdin-fail-closed.md`), 관측 이벤트는 기본 출력을 유지한다. 예외는 Stop 블록 캡이 없는 호스트(Codex)의 Stop 하나로, `codexadapter.HostLacksStopBlockCap`이 단일 판정처다. `hook_codex_failclosed.go`는 `--harness codex`에서 핸들러 타임아웃·오류·파싱 불가 출력을 결정성 이벤트에 한해 fail-closed deny로 답한다 |
...
| `contract*` | 1 | **t1238 판에서 새로 생긴 클러스터.** `moai contract verify|show|sign`(`internal/cli/contract.go`) — 프로젝트 루트·`workflow.autonomy` 설정·constitution 레지스트리·spec.md status를 읽어 평범한 값으로 `internal/contract`(검증)와 `internal/contract/sign`(서명)에 넘기는 얇은 어댑터. 종료 코드: verify 0 유효 / 1 무효, sign 0 서명 / 1 거부, 둘 다 2 사용법·I/O 오류 |
...
| `internal/template` | 33 | `//go:embed all:templates` + `catalog.yaml`. **t1238 판에서 `obligations.go`가 더했다** — 이중 하네스 의무 레지스트리(`Obligation` 행: 요구 동작, Claude·Codex 각각의 적용 경로 또는 `UNSUPPORTED:`/`blocked:`/`unverified:` 표식, 검증 테스트 함수, 대응 AC). 배포기, 렌더러, settings 생성, 스킬 미러, 카탈로그 트리 해시, 모델 정책·프로파일 매트릭스. **배포 뒤편에 두 개의 기계 방출기와 두 개의 미러 보호·복구 seam이 붙어 있다**(§ 템플릿 방출·미러 계열). 임베드 트리는 588개 파일이다. **이 판에서 모델 정책의 `opus` alias 타깃이 `ModelIDOpus5`(`claude-opus-5`)에서 `ModelIDOpus55`(`claude-opus-5-5`, Claude Code v2.1.280+ 필요)로 갈렸다** — 구 id 는 `ModelDeprecatedCanonicalIDs`에 얹혀 여전히 `opus` alias 로 역정규화된다. **미러 보호 seam이 셋째를 얻었다** — `skill_mirror.go`의 `releaseOwnMirrorLink`가 배포기가 `.agents/skills/<skill>/` 아래로 실제 파일을 쓰기 직전, 이전 배포가 같은 경로에 남긴 자기 미러 심볼릭 링크(대상이 `MirrorLinkTarget(skill)`과 정확히 일치하는 것만)를 지운다 — 그러지 않으면 managed clean 이 링크 타깃을 지운 뒤 `MkdirAll`이 "file exists"로 실패한다 | `agentemit`, `commandemit`, `scripts` |
