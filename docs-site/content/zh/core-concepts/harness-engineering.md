...
其中第二个核心是关键创新。AI 递归式自我改进 (RSI) 的现实短期路径不是直接修改模型权重，而是**改进围绕模型的挽具**。MoAI-ADK 走的正是这条路 — 递归改进的不是模型，而是挽具（技能·智能体指令）。
...
该循环在 `/moai loop` 中默认最多重复 5 次(`ralph.max_iterations`),并包含收敛检测(同一错误重复出现时切换替代策略)。如果想直接声明完成条件,可使用 goal 引擎(`/moai goal "<条件>"`)— 会话会自主工作,直到条件满足或达到轮次上限。
...
修改指令本身也是一次实验。编辑规则、智能体、钩子等挽具组件时，适用**决策可观测性**：每次编辑都记录一条可证伪的预测（哪一类失败将不再复发），并且在采纳前必须通过双重检查——**held-in**（该编辑确实捕获了引发它的失败）与 **held-out**（现有防护与测试仍然通过）。被否决的编辑也会留档，避免重复同一次失败的尝试。
