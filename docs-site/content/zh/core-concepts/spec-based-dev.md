...
| 烹饪顺序 | 实现顺序 | 定义按什么顺序进行 |
| 成品照片 | 验收标准 | 定义完成后的样子 |
| 没有"盐少许"这种含糊表达 | 用 GEARS 格式明确表达 | 消除歧义 |
...
SPEC 文档由 **manager-spec 智能体**自动生成。开发者无需背 GEARS 格式，用自然语言发出请求，智能体会自动转换。
...
{{< callout type="info" >}}
  不需要亲自编写 GEARS 格式。向 `/moai plan` 用自然语言发出请求，**manager-spec 智能体会自动转换为 GEARS 格式**。上表是帮助理解转换方式的参考资料。
{{< /callout >}}
