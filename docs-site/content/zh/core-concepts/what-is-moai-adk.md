...
{{< callout type="info" >}}
**一句话概括:** MoAI-ADK 是一个把"将与 AI 的对话作为文档(SPEC)留存、安全地改进代码(DDD/TDD)、自动验证质量(TRUST 5)"的工作 —— **由成本·自我改进·品质把控这三大核心从外部强制** —— 一并完成的智能体开发套件。
{{< /callout >}}
...
只优化成本,品质就会崩塌,随之而来的是返工与调试循环 —— 而返工恰恰是所有 token 支出里最贵的一项。只立品质门禁,每轮会话都会重复同样的失误。没有成本上限就让自主循环跑起来,一个任务失稳就会蚕食配额。三大核心互相支撑 —— **成本**靠**品质**挡住返工才经济地维持,**品质**靠**循环**把通过的样本沉淀为可强制的规则,**循环**靠**成本**门禁在超预算前停下、停在合适的价位。
...
3. **由系统守住预算** —— 追踪 token 使用,在超过阈值前正常停止。
...
用 3 级 Progressive Disclosure 系统进行 token 高效管理。只有技能说明(~100 token)常驻列表中,正文(~5K token)仅在实际调用时加载 —— 是上下文瘦身的核心手段之一。
...
| **low** | 每任务成本最低 —— 大多数 agentic 智能体降到 Opus `medium`（`super-advisor` 保持 `high`），Sonnet 用于单次调用的行和 `e2e-tester` |
