...
MoAI-ADK是为Claude Code打造的 **高性能AI开发环境**。8个专业AI Agent和27个技能协同工作，生产高质量代码。新项目和功能开发默认采用TDD，测试覆盖率低的现有项目自动采用DDD，同时支持Sub-Agent和Agent Teams双重执行模式。
...
MoAI是 **战略编排器**。它不直接编写代码，而是将任务委派给8个专业Agent（7个MoAI自定义 + 1个Anthropic内置 `Explore`）。

### Agent分类

MoAI 由 **8个专业Agent**（7个MoAI自定义 + 1个Anthropic内置）组成：

| Agent | 角色 |
|---------|------|
| **manager-spec** | Plan阶段：SPEC文档生成 |
| **manager-develop** | Run阶段：DDD/TDD实现 |
| **manager-docs** | Sync阶段：文档化及PR创建 |
| **manager-git** | Git工作流及分支管理 |
| **plan-auditor** | SPEC文档的独立审计 |
| **sync-auditor** | 4维度质量评分 |
| **builder-harness** | 动态工具链生成 |
| **Explore** (内置) | 只读代码分析 |
...
flowchart TD
    ...
    subgraph Managers["Manager Agent（4个）"]
        M1["manager-spec\nPlan阶段：SPEC创建"]
        M2["manager-develop\nRun阶段：DDD/TDD实现"]
        M3["manager-docs\nSync阶段：文档"]
        M4["manager-git\nPR创建、Git操作"]
    ...
    subgraph Evaluators["评估Agent（2个）"]
        E1["plan-auditor\n独立SPEC审计"]
        E2["sync-auditor\n4维度质量评分"]
    end

    subgraph Builder["Builder Agent（1个）"]
        B1["builder-harness\n动态工具链生成"]
    end

    subgraph Explore["内置（1个）"]
        X1["Explore\n只读代码分析"]
    end
...
从Plan阶段过渡到Run阶段时，MoAI自动检测当前执行环境 (cc/glm/cg)，显示选择UI供用户在实现开始前确认或更改模式。
...
此门控确保无论环境状态如何，都使用正确的执行模式，防止实现过程中模式不匹配。
...
| 子命令 | 别名 | 用途 | 主要标志 |
|-----------|------|------|-----------|
| `plan` | `spec` | 生成SPEC文档 (GEARS格式) | `--worktree`, `--branch`, `--resume SPEC-XXX`, `--team` |
...
{{< callout type="info" >}}
**v2.7.1 变更**: CG 模式现在是**默认的**团队模式。使用 `--team` 时，除非明确通过 `moai cc` 或 `moai glm` 改变，否则会以CG模式运行。

`moai cg` 使用tmux会话级环境变量隔离来分离Claude Leader和GLM Workers。从 `moai glm` 切换时，`moai cg` 会自动初始化GLM设置。
{{< /callout >}}
...
```bash
/moai fix       # 单次修复：扫描 → 分类 → 修复 → 验证
/moai loop      # 反复修复：直至完成条件满足（最多100次）
```

**Ralph Engine工作方式:**
1. **并行扫描**: 同时运行LSP诊断 + AST-grep + 林特
2. **自动分类**: 将错误分类从1级（自动修复）到4级（需要用户干预）
3. **收敛检测**: 当相同错误重复出现时应用替代策略
4. **完成标准**: 0个错误，0个类型错误，85%+覆盖率
...
| 标准 | 含义 | 验证内容 |
|------|------|----------|
| **T**ested | 测试覆盖 | 85%+覆盖率，特征化测试，单元测试通过 |
| **R**eadable | 易读性 | 清晰的命名规范，一致的代码风格，0个林特错误 |
| **U**nified | 统一性 | 一致的格式化，import排序，项目结构遵循 |
| **S**ecured | 安全性 | OWASP合规，输入验证，0个安全警告 |
| **T**rackable | 可追踪性 | Conventional Commits，问题引用，结构化日志 |

## @MX 标签系统

MoAI-ADK使用 **@MX代码级注释系统** 在AI Agent间传递上下文、不变量和风险区域。

| 标签类型 | 用途 | 添加时机 |
|----------|------|----------|
| `@MX:ANCHOR` | 重要合约 | fan_in >= 3的函数，更改时影响范围广 |
| `@MX:WARN` | 风险区域 | goroutine，复杂度 >= 15，全局状态变异 |
| `@MX:NOTE` | 上下文传递 | 魔法常数，文档缺失，业务规则 |
| `@MX:TODO` | 未完成工作 | 测试缺失，未实现功能 |

@MX标签系统设计为仅标记最危险和最重要的代码。大多数代码无需标签，这是正常的设计。
...
MoAI-ADK根据Claude Code订阅级别为8个Agent分配最优AI模型。在订阅级别使用量限制内最大化质量。

| 策略 | 级别 | Opus | Sonnet | Haiku | 用途 |
|------|------|------|--------|-------|------|
| **High** | Max $200/月 | 23 | 1 | 4 | 最高质量，最大吞吐量 |
| **Medium** | Max $100/月 | 4 | 19 | 5 | 质量和成本的平衡 |
| **Low** | Plus $20/月 | 0 | 12 | 16 | 经济型，无Opus |
...
# 现有项目重新配置
moai update                   # 每个配置步骤交互式提示
```

{{< callout type="info" >}}
默认策略是 `High`。GLM配置隔离在 `settings.local.json` 中（不提交至Git）。
{{< /callout >}}

## 双重执行模式

MoAI-ADK同时提供Claude Code支持的**Sub-Agent**和**Agent Teams**两种执行模式。

```mermaid
flowchart TD
    A["MoAI Orchestrator"] --> B{"执行模式选择"}
...
MoAI-ADK自动分析项目复杂度以选择最优执行模式：

| 条件 | 选择模式 | 原因 |
|------|-----------|------|
| 域名3个或以上 | Agent Teams | 多域协调 |
| 影响文件10个或以上 | Agent Teams | 大规模变更 |
| 复杂度评分7或以上 | Agent Teams | 高复杂度 |
| 其他 | Sub-Agent | 简单可预测的工作流 |

**Agent Teams 模式**使用并行团队驱动开发：
...
```bash
/moai plan "大规模功能"          # 自动：researcher + analyst + architect 并行
...
- **TeammateIdle 钩子**: 团队成员转换为待机状态前LSP质量门验证（错误、类型错误、林特错误）
- **TaskCompleted 钩子**: 当任务引用SPEC-XXX模式时验证SPEC文档存在性
- 所有验证使用优雅降级 - 警告被记录但任务继续进行
{{< /callout >}}

### CG 模式 (Claude + GLM 混合)

CG 模式是Leader使用**Claude API**、Workers使用**GLM API**的混合模式。通过tmux会话级环境变量隔离实现。

```
┌─────────────────────────────────────────────────────────────┐
│  LEADER (当前 tmux 窗格, Claude API)                         │
│  - /moai --team 执行时工作流编排                             │
│  - plan、quality、sync 阶段处理                              │
│  - 无GLM环境 → 使用Claude API                               │
└──────────────────────┬──────────────────────────────────────┘
...
| 命令 | Leader | Workers | tmux必需 | 成本节省 | 使用场景 |
|--------|--------|---------|----------|----------|----------|
...
使用Claude Code的`Task()` API进行顺序Agent委派的既有模式。

- 将任务委派给一个专业Agent并接收结果
- 按Manager → Expert → Quality顺序逐阶段进行
- 适合简单可预测的工作流
...
| 命令 | 说明 |
|--------|------|
| `moai init` | 交互式项目设置（语言/框架/方法论自动感知） |
| `moai doctor` | 系统状态诊断和环保境验证 |
| `moai status` | Git分支、质量指标等项目状态摘要 |
| `moai update` | 更新到最新版本（支持自动回滚） |
| `moai update --check` | 检查更新（不安装） |
...
| `moai worktree clean` | 清理旧worktree |
| `moai worktree go <name>` | 在当前shell中切换到worktree目录 |
| `moai hook <event>` | Claude Code Hook分发器 |
| `moai glm` | 使用GLM API启动Claude Code（成本效益替代方案） |
| `moai cc` | 不使用GLM配置启动Claude Code（纯Claude模式） |
| `moai cg` | 启用CG模式 -- Claude Leader + GLM Workers (tmux窗格级隔离) |
| `moai version` | 显示版本、提交哈希、构建日期 |

## 任务指标日志

MoAI-ADK在开发会话期间自动捕获任务工具指标：

- **位置**: `.moai/logs/task-metrics.jsonl`
- **捕获指标**: 令牌使用，工具调用，耗时，Agent类型
- **目的**: 会话分析，性能优化，成本追踪

任务工具完成时PostToolUse钩子记录指标。使用此数据分析Agent效率并优化令牌消耗。

## 项目结构

安装MoAI-ADK后，您的项目会生成以下结构。
...
| `.claude/skills/` | 包含编程语言、平台特定最佳实践的知识模块 |
| `.moai/specs/` | SPEC文档的保存位置。每个功能有单独的目录 |
| `.moai/config/` | 管理TRUST 5质量标准、DDD/TDD设置等项目设置 |

## 多语言支持

MoAI-ADK支持4种语言。用户用韩语请求时用韩语回应，用英语请求时用英语回应。
...
{{< callout type="info" >}}
**语言设置:** 在 `.moai/config/sections/language.yaml` 中可分别设置对话语言、代码注释语言、提交消息语言。例如，对话用中文，代码注释和提交消息用英语。
{{< /callout >}}

## 下一步

现在您已了解MoAI-ADK的全貌，是时候深入学习每个核心概念了。

- [SPEC基础开发](/core-concepts/spec-based-dev) -- 了解如何将需求定义为文档
- [域驱动开发](/core-concepts/ddd) -- 学习如何安全改进现有代码
- [TRUST 5质量](/core-concepts/trust-5) -- 了解如何自动验证代码质量
- [MoAI内存](/claude-code/context-memory/memory) -- 学习会话间上下文如何保留
