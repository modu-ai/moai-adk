...
MoAI是 **战略编排器**。它不直接编写代码，而是将任务委派给8个专业Agent。
...
MoAI-ADK根据Claude Code订阅套餐为8个Agent分配最优AI模型。在套餐的使用限制内最大化质量。
...
- [MoAI Memory](/claude-code/context-memory/memory) -- 了解跨会话上下文如何保留
