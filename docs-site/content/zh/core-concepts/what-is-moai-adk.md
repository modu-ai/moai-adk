...
MoAI-ADK 是用 **成本 · 自我改进 · 品质把控** 三个轴把 Claude Code 包起来的 **Agentic Development Kit**。同等质量的代码用更少的 token 产出（成本轴，代币经济学），每轮会话跑过都会把观察沉淀成规则，让 harness 越用越好（自我改进轴，智能体循环工程），并用 SPEC 3-phase 与 TRUST 5 门禁挡住返工，用证据判定「完成」（品质把控轴，智能体 Harness）—— 模型选择、推理深度、上下文用量都由系统从外部强制。11 个专业 AI 智能体与 31 个技能协作，对新项目自动应用 TDD（默认），对测试覆盖率低的既有项目自动应用 DDD。

用 Go 编写的单一二进制 —— 无依赖即可在所有平台上即时运行。


{{< callout type="info" >}}
**一句话概括:** MoAI-ADK 是一个把"将与 AI 的对话作为文档(SPEC)留存、安全地改进代码(DDD/TDD)、自动验证质量(TRUST 5)"的工作 —— **由成本·自我改进·品质把控三个轴从外部强制** —— 一并完成的智能体开发套件。
{{< /callout >}}
...
只优化成本,品质就会崩塌,随之而来的是返工与调试循环 —— 而返工恰恰是所有 token 支出里最贵的一项。只立品质门禁,每轮会话都会重复同样的失误。没有成本上限就让自主循环跑起来,一个任务失稳就会蚕食配额。三个轴互相支撑 —— **成本**靠**品质**挡住返工才经济地维持,**品质**靠**循环**把通过的样本沉淀为可强制的规则,**循环**靠**成本**门禁在超预算前停下、停在合适的价位。
...
声明完成条件后,会话会自行工作直到条件满足（`/moai goal`），路由决策与门禁证据作为观察沉淀下来,成为 harness 学习的原料。观察沿 4 层学习阶梯（观察 → 启发式 → 规则 → 自动更新）晋升为指令,自动更新始终只在用户批准门禁之下应用。
