...
| `--profile <high\|medium\|low>` | 覆盖模型+effort 配置文件(保存到 `llm.yaml` 的 `profile`)。规范封闭集合为 `high`/`medium`/`low`,legacy `max` 也接受作为输入并归一化为 `high`(`internal/config/profile.go` 的 `NormalizeProfile`) |
