...
同步结束后,终端会输出部署结果摘要。codex-cli 用的技能镜像(`.agents/skills`)默认以符号链接形式部署;在无法创建链接的环境里会退回为复制部署。这一退回发生时,`moai update` 会在摘要输出里明确说明 —— 副本不像链接那样跟着源头走,所以知道落地的是哪种形态是有意义的。

### 删除前备份(pre-clean backup)

在重新分发模板之前清理 MoAI 管理的根目录(`.claude/`·`.moai/` 下的模板管理路径)时,其中的**模板不会分发的文件只有先备份之后**才会被清理。备份进入按执行时间戳命名的目录 `.moai-backups/<timestamp>/pre-clean/<root>/...`,保持原有相对路径原样;**备份失败则清理本身中止** —— 不存在没有备份就删除的路径。

这道安全装置让你不丢失放在管理根目录里的本地专用文件(个人规则、实验技能等)。哪些属于管理根目录、本地专用文件应该放在哪里,请遵循项目的本地开发指南。备份创建成功后,摘要输出会一并告知其路径。
...
3. **[快速开始](/zh/getting-started/quickstart)** —— 将新功能应用到项目
