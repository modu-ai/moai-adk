...
{{< callout type="info" >}} `-k` 是看板链的标记，`-f` 是**工厂模式** (Factory Mode) 的专用进入标记。 `-k` 一个标记有三种解释这点没变 —— 不带参数 / 带 SPEC-ID 是看板主控，`--name <角色>` 是看板伴随会话，数字是泳道 run。 一次启动只能带一个进入标记，所以 `-k` 和 `-f` 同时给出会报错。 详细契约见[看板模式](/zh/advanced/kanban-mode)和 [manager-lead 领导协调者](/zh/advanced/manager-lead)。 {{< /callout >}}
