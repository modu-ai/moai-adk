...
两个命令都调用 [ast-grep](https://ast-grep.github.io/) CLI（`sg`），而未安装 `sg` 时二者的行为是刻意不同的。`ast-grep` 是常被接入 CI 作为门禁的检测命令，因此会把安装指引写到 stderr 并**以非零码退出**——从未运行的扫描不能视为“没有问题”。`ast-edit` 是替换命令，"没有可应用的改动"本身就是正常的空操作，所以只输出提示并以 0 退出。安装 `sg` 请参阅 [ast-grep 快速开始](https://ast-grep.github.io/guide/quick-start.html)；`moai doctor` 也会报告它是否存在。
