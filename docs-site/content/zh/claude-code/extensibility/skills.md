...
从 MoAI-ADK 的视角看，技能同时横跨两大核心领域。在**代币经济学**层面，渐进式披露就是令牌预算设计 —— 只常驻负担一行说明（约 100 令牌），正文（约 5K 令牌）仅在使用时支付，比把知识常驻在 CLAUDE.md 里经济得多。在**递归式自我学习**层面，技能是挽具进化的编辑对象 —— 挽具基于循环积累的观察升级技能指令，正是 MoAI-ADK 自我进化的核心路径。编写规则·命名空间·渐进式披露令牌预算等实战细节，请参考下方的 MoAI-ADK 深入文档。
