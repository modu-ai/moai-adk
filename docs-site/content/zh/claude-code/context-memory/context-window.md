...
| 大小 (一般情况) | 含义 |
|---------------|------|
| 约 200K token | Haiku，以及网关或旧模型的 Sonnet/Opus 的标准窗口。对于一般的代码工作已经足够 |
| 约 1M token | Anthropic API 的 Sonnet 5·Opus 的原生窗口（及部分其他模型）。对大型代码库 (large codebase) 有利 |
...
如果需要调整自动压缩的触发时机，可以通过环境变量 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` 来改变阈值 (默认值: 约占整个上下文的 75~80%)。例如若想宽裕地压缩，可指定更低的值。

```bash
export CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=70  # 在 70% 时触发压缩
```
