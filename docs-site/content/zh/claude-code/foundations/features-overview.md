...
MoAI-ADK 正是运行在 Claude Code 之上的工作流工具。因此，只要把握这里介绍的功能概念，就能更快地理解 MoAI-ADK 如何编排子代理、技能与 hook。
...
| 插件 | 将技能、hook、子代理、MCP 打包分发的封装单元。 | [扩展](/claude-code/extensibility) |
| 子代理 | 在隔离上下文中独立执行后仅返回摘要的工作者。 | [代理与自动化](/claude-code/agentic) |
| 代理团队 | 多个独立会话共享任务与消息进行协作。 | [代理与自动化](/claude-code/agentic) |
| 工作树 | 用分离的工作目录对同一仓库进行并行开发。 | [代理与自动化](/claude-code/agentic) |
| 检查点 | 在工作过程中保存状态，以便回退。 | [代理与自动化](/claude-code/agentic) |
...
- **hook**：在 `PostToolUse`、`SessionStart` 等生命周期事件上执行脚本、HTTP 请求、提示词、子代理。适合每次都应以相同方式发生的自动化 (例如编辑后 lint)。
- **插件** (plugin)：将技能、hook、子代理、MCP 服务器打包为单一安装单元。在多个仓库间复用同一配置，或向他人分发时使用。

### 代理与自动化系列

- **子代理**：在自己的上下文窗口中处理任务后，仅将摘要结果返回主对话。在像读取数十个文件的调查任务那样、中间产物不应扰乱主上下文时非常有用。
- **代理团队** (agent team)：彼此独立的多个 Claude Code 会话通过共享任务列表与消息进行协作。适合验证相互竞争假设的调查或并行代码评审，属于实验性功能，默认处于禁用状态。
- **工作树**：将同一仓库置于分离的工作目录，使多个分支的工作能够无冲突地并行推进。
...
技能既可以是调用型行为 (`/deploy`)，也可以是参考知识 (API 风格指南)。当上下文窗口逐渐填满，或无需展示中间工作时，子代理更为合适。两者也可结合，子代理可预先加载特定技能，技能也可在隔离上下文中执行。
...
| 4 | [代理与自动化](/claude-code/agentic) | 用子代理、代理团队并行化工作的方法 |
