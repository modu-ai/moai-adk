...
提示缓存是一项 API 功能：**当请求的前段（前缀）与上一次请求相同时，不再重新处理该部分，而是复用它**。从缓存读出的令牌按基础输入单价的 **0.1 倍**计费，因此重复出现的上下文（系统提示、项目指令、对话历史）越大，节省效果越明显。如果说 MoAI-ADK 代币经济学中的“上下文瘦身”是在减少常驻加载的上下文，那么提示缓存就是把剩下的上下文拿来廉价复用。

{{< callout type="info" title="平台基础" >}}
平台层的背景说明见 [提示缓存](/zh/claude-code/context-memory/prompt-caching)。本页是 MoAI-ADK 视角的说明。
{{< /callout >}}
...
- **各模型的最小缓存令牌数**：比这更短的前缀不会被缓存（不报错，按普通方式处理）。例如：Fable 5 = 512、Opus 5·Sonnet 5 = 1,024、Opus 4.7 = 2,048、Haiku 4.5 = 4,096 令牌。
