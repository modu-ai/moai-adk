...
5. **证据导出** — 将要引用的验证证据导出到受版本跟踪的路径 `.moai/reports/<card-id>/`
...
所以契约真正钉死的是**引用前先导出**这项义务: 某份产物在被当作判定依据引用之前，要先搬到受版本跟踪的路径 `.moai/reports/<card-id>/`，而且引用点名的是单个**文件**，绝不是目录。只搬决定判定的那几行——退出码、失败摘要、被引用的那个数字——因为把暂存目录整个导出，无非是把从上下文里削掉的体积原样挪进仓库。留在原地的那部分，作为已知损失写进残余风险一栏；有意不导出的材料，绝不拿来当判定依据。
