...
收敛（convergence，把多个后端的判定合成一份最终判定的步骤）遵循既定顺序。第一，必备关卡后端全员 PASS 则最终判定为 PASS。第二，必备关卡后端任一 FAIL 则最终判定为 FAIL。第三，必备关卡之间意见分歧——一个 PASS、一个 FAIL——则最终判定跌到 FAIL，同时点亮意见不一致标志（disagreement_flag，表示必备后端之间判定分歧的标记）。这个标志连同残余风险说明一起，以建议信号出现在编排器的完成报告（Verification Matrix，完成报告的验证表）里。第四，设为建议或 off 的后端绝不把最终判定翻为 FAIL。建议后端的判定出于透明度被记录，只有与必备后端冲突时才加意见不一致标志。结果还随附一个字段——参与者数（participant_count，实际给出可比较 PASS 或 FAIL 判定的后端个数）；拿不出判定的 inconclusive 后端不是参与者，不计入。与之相配，意见不一致标志取三个值。`true` 表示观测到了分歧——单个后端自身内部复核出现分歧也算，观测到的分歧绝不丢弃。`false` 表示两个及以上参与者经过比较且没有分歧。`null` 表示参与比较的后端不足两个，"一致了"与"分歧了"都不成立。
...
## inconclusive —— 不给没读过的代码下判定

codex 和 GLM 后端在收不到要评审的 diff 时**根本不做这次调用** —— 结果记为 `inconclusive`(无法判定)，后端不被调用。这是对往昔缺陷的矫正 —— 以前，没拿到代码的后端照样对空输入给出充满自信的判定，实际观测里甚至出现过引用本仓库并不存在的仓库白名单的 `fail`。对没读过的代码下的判定无论朝哪个方向都不构成信息，所以收集失败现在止步于 `inconclusive`，不发生后端调用。

后端自己声明 `inconclusive` 时，收敛也把它**原样读作无法判定** —— 绝不合成出 PASS。这和"没有必需后端的 PASS 就出不了 PASS"是同一条保守脉络: 把无判定数成通过，必需关卡的信任就会悄悄漏掉。

收敛判定的**记录位置**也跟随被审计的那棵树。MCP 服务器是长寿子进程，工作目录跟不上工作树切换；以前判定记录簿在进程启动时从主检出解析一次并固定下来，于是在工作树里跑的审计把判定留在了那棵工作树的关卡永远不会去看的目录里。现在判定记录在调用时指名的 `project_root` 树之下 —— 从工作树跑审计时，判定留在那棵工作树的关卡会读的位置。例外是把 `.moai` 放在 git 之外的仓库的链接工作树：其判定记录在 primary 检出的 `.moai/state` 中，并附上该工作树的树标识；关卡无论从工作树还是 primary 读取，都能找到同一会话的判定，只要其中任一为 `fail` 就会阻断(`project_root` 参数的用法见 [MCP 服务器](/zh/guides/mcp-server/) 文档)。
