...
收敛判定的**记录位置**也跟随被审计的那棵树。MCP 服务器是长寿子进程，工作目录跟不上工作树切换；以前判定记录簿在进程启动时从主检出解析一次并固定下来，于是在工作树里跑的审计把判定留在了那棵工作树的关卡永远不会去看的目录里。现在判定记录在调用时指名的 `project_root` 树之下 —— 从工作树跑审计时，判定留在那棵工作树的关卡会读的位置(`project_root` 参数的用法见 [MCP 服务器](/zh/guides/mcp-server/) 文档)。
