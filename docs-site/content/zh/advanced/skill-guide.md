...
本技能的源码 lint 会机械地强制两组规则。`SVG060`-`SVG064` 是无障碍契约：根 `<svg>` 需要 `role="img"`、通过 `aria-labelledby` 指定的无障碍名称、位于首个子元素位置的 `<title>`、一个 `<desc>`，以及带有分图表前缀的 id。缺少这些的图形会被朗读为无名图像，其中的 `<text>` 一个也不会被读出（只有纯装饰性图形可以用 `aria-hidden="true"` 作为例外）。

`SVG070`-`SVG074` 管的是连接线的几何：连接线标签的遮罩必须与自身线条保持 6～10 单位的间距（小于 6 报错，大于 10 警告，而遮罩只在 16 单位以内才归属于该连接线），落在同一个方框边上的两条连接线，其到达点必须彼此分开，标签遮罩也不得与在它之后绘制的图形部分重叠。
