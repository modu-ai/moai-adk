...
| 代理 | 角色 | 特征 |
|----------|------|------|
| `Explore` | 只读代码探索和分析 | 继承主会话模型（上限为 opus，CC 2.1.198 之前固定为 Haiku），Read-only 工具 |
...
| 特征 | 说明 |
|------|------|
| **独立上下文** | 每个 sub-agent 在自己的上下文窗口中运行；大小随会话模型而定（Anthropic API 的 Sonnet 5 / Opus 为 1M，网关或旧模型为 200K） |
| **自定义提示** | 自定义系统提示定义行为 |
| **特定工具访问** | 仅提供必要的工具 |
| **独立权限** | 个别权限设置 |
...
| 技能非继承 | 不继承父会话的技能 |
| 独立上下文 | 每个代理都有独立的上下文（Anthropic API 的 Sonnet 5 / Opus 为 1M，网关或旧模型为 200K） |
