...
并非每张卡片都需要把三个工作列全部走一遍。积压在 backlog 的大多是简单整理工作，让这类卡片走完 plan → run → sync 全流程，仪式成本会超过改动带来的价值。主导在卡片离开 `backlog` 时为其分类，并在派发时一并写明。

| 类别 | 形状 | 经过的路径 |
|--------|------|---------------|
| A — 直接收尾 | 单文件 · 单行改动，不含设计判断，CI 能抓住回归 | 一个会话把卡片一直做到 PR；跳过 `plan` |
| B — 缺陷，原因未确立 | 有东西坏了，但原因尚未查明 | `run → sync`；跳过 `plan`，因此没有 SPEC |
| C — 设计变更 | 包含决策，或横跨多个子系统 | 三个工作列全部经过 |

Class A 只在经过验证的证据之上成立——对照 base 测量 diff、显示只改了一个文件，并确认将要合并的 head 上 CI 为绿，类别才被承认。"因为快"不能作为理由。速度是跳过各列的结果而非原因，仅以速度论证的 A 类别，就是一张被催赶的 C 类别。

Class B 跳过的是 `plan`，不是 sync 关卡的评审。原因未确立的缺陷恰恰是评审要抓住的东西，因此关卡保留到最后。卡片离开 `run` 之前，要把确立原因的证据——复现了什么、打印了什么——留在卡片的进度记录里。

一列同时只驻留一张卡片，且只有当各卡片分别占据不同的 worktree 时，才会真正并行运行。
