...
- **`-k` / `--kanban` 启动器开关** —— 同时接在 `moai cc` 和 `moai glm` 上。 不带参数（或带上 SPEC 标识符）进入为主控；以 `-k --name <角色>` 形式给出则作为伴随会话加入已经打开的 run。 CG 已停用，请用 `moai migrate cg` 预览迁移选项。
- **`-f` / `--factory` 启动器开关** —— 工厂模式的专用入口。`moai cc -f N` 会连同主控一起告诉你 `worker-1`…`worker-N` 各名工作者的启动命令，`moai cc -f worker` 则一次让一名工作者加入下一个空号（要指定编号则用 `-f worker-<n>`）。旧式的 `-f lane-<n>` 也会继续以已弃用别名的身份生效。详见下面的“工厂模式”一节。
...
如果说看板的形态是“三个角色接力搬运一张卡片”，那么**工厂模式**就是“N 名编号工作者同时搬运多张卡片”。卡片不在列与列之间移动，而是**整张**进入一名空闲工作者，由那名工作者在会话内按顺序通过 `plan → run → sync` —— 各阶段作为 `Agent()` 子智能体执行。进入用的专用标记是 `-f`。用 `moai cc -f 4` 打开主控和 4 名工作者，再用 `moai cc -f worker`（或 `moai glm -f worker`）一名一名地加工作者。旧式的 `-f lane-<n>` 也会继续以已弃用别名的身份生效。

每名工作者的并发智能体上限（10 个）、错峰启动的理由、工作者编号的归属（`factory.db`）、写入 spawn 的 worktree 隔离、与看板的分岔点的正文，由专页 [工厂模式](/zh/advanced/factory-mode) 讲解。
...
积压队列里堆的东西大多是杂活 —— 一行的修正、过时的引用、改个标志名。让这些东西全程走完 `plan → run → sync`，仪式的成本会超过变更的价值。所以主控会在每张卡片离开 `backlog` 时把它归入三个类别(Class A · Class B · Class C)之一，并在派单文里写明类别。
...
**不用的时候** —— 想在每个阶段之间亲自判断、审查中间产物时（这种情况请以回合为单位走普通 `plan → run → sync`）。 一两回合就结束的短工作。 CG 已停用，请用 `moai migrate cg` 预览迁移选项。
