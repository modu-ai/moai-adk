...
在会话启动器上加上 `--kanban`（简写 `-k`）开关即可启动。它不是新的子命令，也不是新的运行时——只是一个让看板链（`kanban_chain`，即预设了完成条件的目标预设）搭上去的进入契约。链的三个阶段（plan → run → sync —— 评审判定由 sync 关卡吸收）和人工关卡，都直接继承现有的 `/moai goal` 引擎和 `full-pipeline` 链接规则。
