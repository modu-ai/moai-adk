...
工厂主控干的事与看板主控不同。看板主控协调的是一张卡片的各个阶段，而工厂主控**把已挑选的卡片分给空闲的泳道**。读队列、挑卡片不是工厂主控的活儿 —— 挑卡片的永远是操作者（`moai todo next <n>`），工厂只路由这些**已挑选（picked）**的卡片。看板工头循环（单独的 `/loop`）也不例外: 它只派送已被标记为 `picked` 的下一张卡片，绝不自己挑。分配的单位永远是整张卡片 —— 每张卡片都完整地进一条泳道，由那条泳道在会话内把串行三阶段路径（`plan → run → sync`，一个阶段结束才进下一个）走到底。每个阶段都由泳道以 `Agent()` 子智能体的形式生成并运行，卡片绝不会被拆到多条泳道上。
