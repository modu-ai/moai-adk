...
工厂主控干的事与看板主控不同。看板主控协调的是一张卡片的各个阶段，而工厂主控**轮询积压队列**，把操作者（或看板工头循环 —— 单独的 `/loop`）选中的卡片**分给空闲的泳道**。分配的单位永远是整张卡片 —— 每张卡片都完整地进一条泳道，由那条泳道在会话内把串行三阶段路径（`plan → run → sync`，一个阶段结束才进下一个）走到底。每个阶段都由泳道以 `Agent()` 子智能体的形式生成并运行，卡片绝不会被拆到多条泳道上。
