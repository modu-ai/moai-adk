...
一旦判定结果已经拿到，积压数字后面会再多出一个 `✓N`，渲染成 `🔄 TODO: 1/3 ✓2` 这样。
...
- **第四行（按条件）—— 以什么身份、积了多少**: 显示会话名 (🏷️)、智能体名 (👤)、积压现状 (🔄 `TODO: 进行中/待办`)。在看板伴随会话这类有名字的会话里自然出现；没有观测来源时对应段落会缩水，全部为空则整行省略。这里也是高亮显示会话名的位置 —— 开着多个终端、分不清哪个窗口是哪个角色时，它是第一个信号。判定结果一旦拿到，积压数字后面会再多出一个 `✓N`，表示被选中的卡片里有多少已经被集成分支的历史提到过——这是一条附注，不是从选中数里减掉的值，判定还没拿到时连零都不显示，直接安静地省略。
...
订阅用户可通过 5H/7D 条形图判断剩余使用额度。使用量与上下文占用都很高时，可考虑停止会话并通过交接继续。已停用的 CG 启动器不能作为调整费用路径的选项。
...
flowchart TD
    ...
    B -- "1M 上下文<br/>(Opus 5, Sonnet 5, Fable, GLM-5.3)" --> C{"使用率 ≥50%?"}
...
阈值按模型等级各不相同，因为窗口越大，越早换会话对预防 SSE 停滞越有利。1M 上下文模型在装满一半 (50%) 时点亮 soft 标记，200K 模型在 90% 时点亮。hard 标记是预先计入 auto-compact 动作时点的天花板。不过运行时的 auto-compact 常常先于这个天花板出手，所以 hard 阶段实际上是较少点亮的强信号。
...
**交接标记不显示时**多半正常。1M 模型在 50% 以下、200K 模型在 90% 以下，就是还没到阈值。如果超过了阈值还不出现，确认模型的窗口大小是否正确映射（尤其 GLM 校正）。
