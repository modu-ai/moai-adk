...
先写测试、再让测试通过，按 RED-GREEN-REFACTOR 循环实现。各阶段分别代表什么，请见 [SPEC 驱动开发](/zh/core-concepts/spec-based-dev)。

#### DDD 模式（现有项目 / 测试覆盖率低于 10%）

先用特性测试把现有行为固定下来，再按 ANALYZE-PRESERVE-IMPROVE 循环逐步改进。详细内容请见 [DDD](/zh/core-concepts/ddd)。
