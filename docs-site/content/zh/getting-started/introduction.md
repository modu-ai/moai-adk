...
MoAI-ADK 采用 SPEC 优先的开发方法论,并通过 **TRUST 5** 质量框架确保代码质量。
...
- **8 个专业代理**: 针对各开发阶段专业化的 AI 代理团队 (7 个 MoAI 自定义 + Anthropic 内置 Explore)
- **27 项技能**: 支持各种开发场景的可扩展技能库
- **多语言支持**: 支持韩语、英语、日语、中文 4 种语言
- **Adaptive Thinking**: 通过 `ultrathink` 关键字在 Opus 4.7+/4.8 和 Sonnet 4.6 的内置推理模式中分析复杂问题
- **Ralph-Style LSP Integration**: 基于 LSP 的自主工作流和实时质量反馈

## 主要功能

MoAI-ADK 提供 8 个专业 AI 代理和 27 项技能，自动化和优化整个开发工作流。

### 代理类别

| 类别 | 数量 | 关键代理 |
|----------|-------|------------|
| **管理器** | 4 | manager-spec、manager-develop、manager-docs、manager-git |
| **评估器** | 2 | plan-auditor、sync-auditor |
| **构建器** | 1 | builder-harness |
| **探索** | 1 | Anthropic 内置 (只读代码分析) |

### 模型策略 (令牌优化)

MoAI-ADK 根据 Claude Code 订阅计划为 8 个代理分配最优的 AI 模型。在计划的速率限制内最大化质量。

| 策略 | 计划 | 🟣 Opus | 🔵 Sonnet | 🟡 Haiku | 用途 |
|------|------|---------|-----------|----------|------|
| **High** | Max $200/月 | 16 | 5 | 3 | 最高质量、最大吞吐量 |
| **Medium** | Max $100/月 | 3 | 17 | 4 | 质量与成本平衡 |
| **Low** | Plus $20/月 | 0 | 13 | 11 | 经济型、无 Opus 访问 |
