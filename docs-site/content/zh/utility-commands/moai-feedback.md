...
| 收集项目 | 说明 | 示例 | 收集方式 |
|-----------|------|------|-----------|
| MoAI-ADK 版本 | 当前安装的版本 (`moai version`) | v3.1.3 | 保证(始终收集) |
...
`/moai feedback` 通过以下细节行为增强 Issue 创建过程。
...
起草的报告经由 `gh issue create` 发出去之前,会先运行**擦洗契约**(scrubbing contract)。遮蔽擦洗器盖住密钥、令牌、路径这类敏感值;漏洞分类器判定正文是否读作安全漏洞披露;被遮蔽值的清单(遮蔽日志)和提交失败的条目(重试队列)各自留作记录。这一流程可以在终端用 `moai feedback scrub`(遮蔽预览)和 `moai feedback queue`(重试队列查看)两个动词直接确认。

分类器读正文的时机在**遮蔽之前**。漏洞报告正是靠那些会成为破绽迹象的字符串来识别的,而遮蔽恰恰会把这些字符串抹掉 —— 事后分类就会在"一旦公开就出事"的那条路径上悄无声息地失效。所以顺序是定死的。

这份契约是技能正文遵循的规约,不是沙箱。手动在 GitHub 上开 Issue 会绕过全部三道(遮蔽、分类、确认询问) —— 直接往公开仓库写字时,同样的谨慎就落在你手上。
