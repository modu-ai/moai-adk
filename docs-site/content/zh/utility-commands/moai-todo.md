...
flowchart TD
    ...
    Queue --> Pick["/moai todo next<br/>人挑一个"]
...
| `/moai todo done <n>` | 移除第 `n` 号条目（在别处完成了，或不再需要）。会显示删掉的是什么。 |
| `/moai todo next` | 把队列作为选项呈现，让人来挑下一张卡片。 |
...
| `spec_id` | 在条目被挑中、SPEC 撰写完成之后才会填写。在那之前是 `null`，这个值的有无正是"还是待办条目"与"已是看板上的卡片"的分界。 |
| `state` | `queued` · `picked` · `dropped`。被挑中的条目也留在文件里，让进行中的工作可见；卡片到达 `done` 时被移除。 |

文件以原子方式写入（先写临时文件再改名），写入中途崩溃也不会截断队列。文件缺失不是错误，而是**空队列**；格式损坏的文件只报告、不触碰——这里存的人的意图，是唯一无法重新生成的值。

## 挑选下一张卡片

`/moai todo next` 把队列作为选项呈现。从最旧的开始、每次一个、最多到工具允许的四个，其余在正文中摘要列出，什么都不隐藏。主导会话在 `/clear` 之后的第一手呈现队列时，用的也是同样的方式。
