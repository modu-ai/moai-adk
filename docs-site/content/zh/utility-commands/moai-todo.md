...
{{< callout type="info" >}}
**一句话概括**：`/moai todo` 用来记录接下来要做的事。添加条目、查看列表、归档已完成的卡片，再选择下一张卡片。选择卡片不会自动创建 SPEC，具体流程由卡片分类决定。
{{< /callout >}}
...
待办队列的一个条目是**一行意图**，并不是 SPEC 或计划书。用户选择后，Class A 直接完成收尾，Class B 不需要 SPEC，按 run → sync 推进，Class C 则在 plan 阶段编写 SPEC，再进入 run → sync。
...
flowchart TD
    ...
    Class --> A["A: 直接收尾"]
    Class --> B["B: run → sync"]
    Class --> Plan["C: plan → run → sync<br/>在 plan 阶段编写 SPEC"]
...
| `state` | 生命周期判别字段。取值为 `queued` · `picked` · `dropped`——"还是待办条目"与"已上看板的卡片"由这个值区分。被挑中的条目也留在文件里，让进行中的工作可见。把条目**移出活跃队列**的途径只有一条：由人手动执行 `moai todo done`——工作完成时不会有任何自动清除。`done` 不会删除卡片和指向它的记录，而是把它们**归档**：不再出现在 `list` · `next` · `why` · `analyze` 中，`moai todo undone <n>` 会把两者一起、连同原来的位置一并恢复。丢弃不同于归档：`moai todo drop <n> "<理由>"` 把卡片移入 `dropped` 但仍留在活跃队列文件里，并把理由加在文本前面，`moai todo undrop <n>` 则精确地把它还原。`dropped` 的卡片不再是挑选候选。 |

变更在取得队列锁后，通过 WAL 模式的 SQLite 事务提交。队列不存在时按**空队列**处理；无法读取或格式损坏的数据会保留并报告错误。Web 控制台也会显示读取失败的提示，不会把这种情况显示为 0 张卡片。
...
2. 按卡片分类推进：Class A 直接收尾，Class B 不需要 SPEC，按 run → sync 推进；Class C 在 plan 阶段编写 SPEC，再进入 run → sync。
3. 对于需要 SPEC 的卡片，确定标识符后使用 `moai todo next <n> --spec <SPEC-ID>` 建立关联。关联需要显式操作，已选择的卡片没有 SPEC 也可能是正常情况。
...
| `moai todo export-json` | 把活着的队列以旧格式的 `backlog.json` 写到数据库旁边，供早于数据库的版本读取。它是运行那一刻的副本，不是实时镜像，因此请在替换二进制之前立刻导出。与活跃条目不同，归档的卡片**会被包含**在这次导出中——但只要导出的记录里确实带有归档行，命令就会在 stderr 上说明：不认识归档功能的旧版二进制会在第一次写入时丢弃该字段。之后的 `todo` 动词会把这个文件原样留在原处。读取动词 `list`、`why`、`pr`、`history` 还会在 stderr 上说明：数据库旁边的这个文件并不是队列，以免把过时的导出当成活着的状态。 |
