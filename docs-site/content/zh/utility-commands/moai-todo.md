...
| `project_uuid` / `card_uuid` | 项目与卡片的持久 identity。对尚未分配 identity 的旧数据执行 `list --json` 时，这些键不会省略，而是返回 `null`；这种纯读取既不分配 UUID，也不修改数据库。此后，获准的写入成功时会填入规范的小写 UUIDv7 字符串。Git 仓库与不使用 Git 的普通文件夹遵循同一契约。                                                                                                                                                                                                                                                                                                                                                              |
| `spec_id`                    | 指向 SPEC 标识符的可选关联。挑选时用 `--spec` 传入就会当场填写；还不知道时，即使处于 `picked` 状态也保持 `null`。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `runtime`                    | 最近一次运行开始与卡片分配报告。`runs` 和 `assignments` 始终是数组，其中的 UUID 是 Todo identity 的逻辑投影；runtime 表不会另存 UUID 列。`reported_state` 只是报告值，并不具有决定卡片完成状态的权威。                                                                                                                                                                                                                                                                                                                                                                                                                     |
