...
flowchart TD
    ...
    Single --> Quality["evaluator-active<br/>Sequential 4-perspective analysis"]
...
| **evaluator-active** | Code quality analysis (default mode) | Sequential 4-perspective analysis |
...
Default mode uses a single `evaluator-active` agent to analyze all 4 perspectives sequentially. `--team` mode uses 4 expert reviewers analyzing simultaneously for deeper analysis, but consumes approximately 4x more tokens.
