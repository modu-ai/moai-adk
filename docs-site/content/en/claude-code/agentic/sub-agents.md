...
That covers the Claude Code-level subagent concept. MoAI-ADK operates a **13-agent catalog** on top of this mechanism — the Manager family (manager-spec / manager-develop / manager-docs / manager-git / manager-design / manager-lead) handles the plan→run→sync lifecycle and Tier L coordination, the Evaluator family (plan-auditor / sync-auditor) handles independent audits, builder-harness handles harness scaffold generation, super-advisor handles high-reasoning consultation, e2e-tester handles E2E test execution across web/mobile/desktop, mission-governor decides on GTD auto missions, and the Anthropic built-in `Explore` handles read-only exploration. The separation of planning and auditing — the agent that built something never checks its own work — is the core design of this catalog. Declaratively assigning each agent the model and reasoning depth (effort) matched to its task is the tokenomics principle of "plan deeply, implement cheaply, verify independently." Details are covered in the advanced guides below.
