...
For `audit_multi` the root is forwarded to the codex backend of the fan-out only. The GLM backend is an HTTP call out to z.ai with no working directory to bind a root to, so it takes no root — a GLM verdict is not about any tree.
