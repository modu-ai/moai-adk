...
{{< callout type="info" >}}
The default tier is **medium**. Changing the tier does not change the model class — only each agent's Opus reasoning depth moves. `low` keeps most agentic rows on Opus `medium` and uses Sonnet only on single-shot rows and `e2e-tester`; `high` raises only `builder-harness` and `e2e-tester` one effort level above medium. No tier assigns `max` to any agent. Set it with the `--model-policy` flag or in the initialization wizard.
{{< /callout >}}
