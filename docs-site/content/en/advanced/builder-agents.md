...
{{< callout type="info" >}}
  **One-line summary**: Builder agents are the **extension toolkit factory** of MoAI-ADK. You can customize the system by creating skills, agents, and plugins.
{{< /callout >}}
...
| Agent  | `builder-agent`    | Define new expert roles              | MoAI delegates                |
...
allowed-tools: # Allowed tools (comma-separated or list)
  ...
  - Grep
  - Glob
model: inherit # Model to use (defaults to session model if omitted; 'inherit' recommended)
...
## Creating Agents (builder-harness)

The `builder-harness` agent is the primary agent creation mechanism. It generates **dynamic project-specific harness teams** based on a Socratic interview, creating specialized agents in `.claude/agents/harness/` and project configuration in `.moai/harness/`.

### Dynamic Agent Generation

Rather than manually defining new agents, `builder-harness` interviews the user to understand the project's unique domains and automatically generates agents tailored to those domains. This is invoked via `/moai project` during the Socratic interview phase.

### Agent Definition Structure (Reference)

Harness-generated agents are defined as markdown files with metadata in YAML frontmatter.

```markdown
---
name: my-project-specialist
description: >
  Project-specific specialist. Responsible for domain-specific tasks
  in the context of this project. Auto-generated by builder-harness.
tools: Read, Write, Edit, Grep, Glob, Bash
model: inherit # Inherits session model
skills: # Skills injected by builder-harness
  - moai-domain-backend
...
OUT OF SCOPE:
- ML model development (delegate to a general-purpose agent with data-science context)
- Infrastructure configuration (delegate to a general-purpose agent with devops/CI context)

## Delegation Protocol

- For ML model needs: spawn `Agent(general-purpose)` with data-science domain instructions
- For infrastructure setup: spawn `Agent(general-purpose)` with devops/CI domain instructions
...
Builder agents are automatically invoked when you request them in natural language to MoAI.
...
| ----------------------- | --------------------------------------------------------------------- |
| Cannot create subagents | Subagents cannot spawn other subagents                                |
| User interaction limited| Subagents cannot interact directly with users (only MoAI can)       |
