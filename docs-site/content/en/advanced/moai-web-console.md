...
![MoAI Web Console — project name and profile in header, profile card, six setting tabs](/img/moai-web-console.png)
...
Edit per-profile values like model·inference strength·language·display settings. When using multiple profiles in one project — for example `medium` normally, `max` for difficult work — switch profiles in the profile card and adjust each profile's detailed values in the tabs.
