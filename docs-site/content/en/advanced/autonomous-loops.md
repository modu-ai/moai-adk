...
Disagreement is surfaced as `disagreement_flag` + `residual_risk_note` in the `ConvergenceResult` — it NEVER hard-blocks the flow on its own. `disagreement_flag` is three-valued alongside `participant_count` (how many backends produced a comparable `pass`/`fail` verdict): `true` = a divergence was observed (a single participant's own synthesis divergence included), `false` = 2+ participants compared with none diverging, `null` = fewer than 2 participants, where neither agreement nor disagreement is a grounded claim. Independence is structurally preserved: the secondary fan-out goroutines receive only `(target, focus, model, effort)` and never the `claude_verdict`, so codex and GLM produce uncorrelated second opinions rather than contaminated re-samples. Mandatory fail-open holds in both directions — a missing or unauthenticated optional backend returns `VerdictInconclusive` and falls back to claude, never a hard error.
