...
> **GLM 팀원이 쓰는 모델**: Claude 티어 넷(Opus·Sonnet·Haiku·Fable)이 모두
> 1M 컨텍스트의 `glm-5.3-flash`(기본값) 하나로 묶입니다. Claude Code는 자동 압축 창을
> Opus 슬롯 기준으로 한 번만 잡고 다른 슬롯에서 뜬 에이전트도 그 값을
> 물려받기 때문에, 작은 모델을 섞으면 제 한계를 넘겨도 압축이 안 걸립니다.
> 티어 구분은 모델이 아니라 추론 깊이(effort) 축이 맡습니다. 자세한 매핑은
> [멀티 LLM 소개](/ko/multi-llm)를 참고하세요.
