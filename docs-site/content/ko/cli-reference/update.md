...
동기화가 끝나면 배포 결과 요약이 터미널에 나옵니다. codex-cli용 스킬 미러(`.agents/skills`)는 기본으로 심볼릭 링크로 배포되는데, 링크를 만들 수 없는 환경에서는 복사로 대체 배포됩니다. 이 대체가 일어나면 `moai update`가 요약 출력에 그 사실을 명시적으로 알립니다 — 복사본은 링크와 달리 원본이 바뀌어도 스스로 따라가지 않으므로, 어떤 형태로 설치됐는지 아는 것이 의미가 있습니다.
