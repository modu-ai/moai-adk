...
하나의 프로필이 곧 하나의 `CLAUDE_CONFIG_DIR` 이기 때문에, 여러 에이전트(스스로 일하는 AI) 가 서로 다른 모델·effort·출력 스타일을 써야 하는 하네스(harness) 시나리오에서 충돌 없는 격리 단위가 됩니다. 따라서 동일 머신에서 작업용·실험용을 오가는 관리자 에이전트에게 프로필은 사실상 "독립된 작업 환경" 과 같습니다.
...
{{< callout type="note" >}}
아래 설명하는 프로젝트별 기억은 다음 릴리스에 포함됩니다. 현재 배포판(v3.0.1)은 전역 기록 하나(`last_profile`)만 남기므로, 프로젝트 B 에서 `-p` 로 프로필을 지정하면 프로젝트 A 가 기억하던 값이 덮어씌워집니다.
{{< /callout >}}
...
{{< callout type="note" >}}
아래 안내 메시지는 다음 릴리스에 포함됩니다. 현재 배포판(v3.0.1)은 아무 예고 없이 로그인 화면으로 넘어갑니다.
{{< /callout >}}
