...
빈 폴더에서 시작하면 관리자 에이전트와 하네스(harness) 가 동작할 규칙·스킬·훅이 모두 없기 때문에, 이 커맨드는 템플릿 카탈로그를 한 번에 깔아 두는 역할을 합니다. SPEC 워크플로우와 품질 게이트가 모두 이 초기 구조를 전제로 동작하므로, 처음 한 번 정확히 세팅해 두면 이후 `moai update` 로 일관되게 유지됩니다. 따라서 새 프로젝트의 첫 단추이자 다른 모든 `moai` 커맨드가 의존하는 기반 셋업 커맨드입니다.
