...
하네스(harness) 가 정상 동작하려면 `.moai/` 와 `.claude/` 디렉터리가 모두 초기화돼 있어야 하는데, 이 조건이 깨져 있으면 다른 `moai` 커맨드가 애매한 오류를 내기 때문에, 이 커맨드는 가장 먼저 실행해 볼 한눈짚기 용도로 쓰입니다. 따라서 SPEC 을 새로 시작하거나 디버깅 세션에 들어가기 전 상태를 확인하는 기본 진입점입니다.
