...
SPEC 하나가 여러 관리자 에이전트와 하네스(harness) 규칙을 거치며 수정되는 구조에서, 핵심 불변조건이 실수로 건드려지면 전체 시스템의 일관성이 깨집니다. 그래서 이 커맨드는 규칙마다 zone 을 명시해 두고, FROZEN 영역이 소스 파일과 다르게 바뀌었는지를 기계적으로 검증합니다. CI 파이프라인에서 `moai constitution guard` 를 걸어 두면 위반을 머지 전에 잡아낼 수 있기 때문에, 대규모 룰 개편 시 안전망으로 쓰입니다.
