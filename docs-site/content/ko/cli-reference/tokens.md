...
- **컨텍스트 스냅샷** — 기록 시점에 해당 세션의 컨텍스트 사용 상태(`.moai/state/context-usage/<session-id>.json`)가 있으면 그 값이 함께 들어갑니다.

## 언제 기록하나

설계상 카드 또는 세션이 **닫히는 시점**의 기록입니다. 칸반 런에서는 카드 하나가 끝날 때마다, 단일 세션에서는 큰 작업이 끝날 때 남기면 카드별 비용 비교가 성립합니다. 이 명령 자체는 토큰을 소비하지 않습니다 — 이미 발생한 사용량을 트랜스크립트에서 다시 세는 회계입니다.
