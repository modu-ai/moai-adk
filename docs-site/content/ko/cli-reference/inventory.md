...
여러 SPEC 을 동시에 진행하는 워크플로우에서는 어느 세션이 어느 하네스(harness)와 워크트리를 쓰고 있는지 한눈에 보기 어렵기 때문에, 이 커맨드는 세 자원을 하나의 읽기 전용 뷰로 모아 줍니다. 따라서 관리자 에이전트가 새 작업을 spawn 하기 전에 경합을 점검하는 첫 단계로 적합합니다.
