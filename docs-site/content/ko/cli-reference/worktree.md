...
SPEC 단위 작업을 main 체크아웃에서 동시에 진행하면 서로의 커밋이 꼬이고, 한 세션이 다른 세션의 작업 트리를 실수로 바꾸는 일이 생길 수 있기 때문에, 워크트리는 관리자 에이전트가 안전하게 병렬로 작업하기 위한 물리적 격리 층입니다. 따라서 하네스(harness) 가 Tier L SPEC 을 worktree 분산으로 처리할 때 이 커맨드가 동기화·정리·복구를 책임집니다.
