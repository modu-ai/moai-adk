...
`moai ast-grep` 은 코드를 구문 트리 단위로 스캔하고, `moai ast-edit` 은 매칭된 코드를 실제로 치환합니다. 텍스트 기반 `grep` 과 달리 구문 구조로 매칭하므로 공백·줄바꿈·변수명 차이에 흔들리지 않습니다. 하네스(harness) 품질 게이트 가운데 정적 분석 축을 담당하기 때문에, 룰이 선언한 패턴을 정확히 잡아 내는 신뢰성이 관리자 에이전트가 세운 SPEC 수용 기준의 PASS/FAIL 판정으로 직결됩니다. 따라서 룰 파일을 고칠 때는 매칭이 구문 단위로 들어맞는지 반드시 `--dry` 로 먼저 확인해야 합니다.
