...
SPEC 단위로 작업하는 관리자 에이전트가 동시에 같은 파일을 고치면 서로의 변경을 덮어쓰는 치명적인 경합이 생길 수 있기 때문에, 이 레지스트리는 활성 세션의 존재를 알리는 단일 조율 지점을 제공합니다. 따라서 하네스(harness) 의 pre-spawn 검사가 이 데이터를 읽어 다음 에이전트를 spawn 해도 안전한지를 판정합니다.
