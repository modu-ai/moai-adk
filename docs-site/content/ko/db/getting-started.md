...
`/moai db init` 한 번이면 프로젝트의 데이터베이스 메타데이터가 자리를
잡습니다. 이후 마이그레이션 파일을 추가할 때마다 스키마 문서가 따라오므로,
에이전트도 팀원도 항상 최신 스키마를 같은 자리에서 봅니다.
...
먼저 `/moai project`가 만들어둔 필수 파일이 있는지 확인합니다:
