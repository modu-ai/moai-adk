...
MoAI-ADK의 데이터베이스 워크플로우는 프로젝트의 스키마 메타데이터를 중앙에서
관리합니다. `/moai db` 명령어로 마이그레이션 파일을 스캔하고, 스키마 문서를
자동 생성하며, 문서와 실제 마이그레이션 사이의 드리프트를 감지합니다.

이 스키마 문서는 사람만 보는 문서가 아닙니다. **에이전틱 하네스** 관점에서
`.moai/project/db/`는 에이전트의 영속 컨텍스트(파일 기반 메모리)입니다.
에이전트가 세션마다 마이그레이션 파일을 다시 읽어 스키마를 재구성하는 대신
정리된 스키마 문서 하나를 참조하므로, 같은 정보를 얻는 데 드는 토큰이 크게
줄어듭니다 — 토크노믹스가 문서 구조에까지 적용되는 사례입니다.

## 주요 기능

- **대화형 초기화** — `/moai db init`으로 데이터베이스 엔진, ORM, 마이그레이션 도구를 선택하고 메타데이터 템플릿 자동 생성
- **자동 동기화** — PostToolUse 훅이 마이그레이션 파일 변경을 감지해 자동 리프레시
- **드리프트 감지** — `/moai db verify`로 스키마 문서와 마이그레이션 파일 간 불일치 검사
- **16개 언어 지원** — Go, Python, TypeScript, JavaScript, Rust, Java, Kotlin, C#, Ruby, PHP, Elixir, C++, Scala, R, Flutter, Swift
...
더 자세한 내용은 [/moai db 명령어 가이드](/ko/db/getting-started)를 참조하세요.
