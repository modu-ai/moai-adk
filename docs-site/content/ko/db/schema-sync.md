...
MoAI의 데이터베이스 워크플로우는 마이그레이션 파일 변경을 자동으로 감지해
스키마 문서를 동기화합니다. 사람이 "문서 갱신"을 기억할 필요가 없도록,
Claude Code의 PostToolUse 훅에 이 관찰 루프를 붙여 두었습니다 — 에이전트가
일하면 문서가 따라오는 구조입니다.
...
파일을 저장할 때마다 스캔이 돌면 낭비이므로, 부분 편집으로 인한 오탐을
막기 위해 **10초 디바운스 윈도우**가 설정됩니다:
