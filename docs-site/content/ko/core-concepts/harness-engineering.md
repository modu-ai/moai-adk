...
특히 두 번째 핵심이 지금의 발전을 이끕니다. AI의 재귀적 자기 개선 (RSI) 의 현실적인 단기 경로는 모델 가중치를 직접 수정하는 것이 아니라 **모델을 둘러싼 하네스를 개선하는 것**입니다. MoAI-ADK는 정확히 이 길을 따릅니다 — 모델이 아니라 하네스 (스킬·에이전트 지침) 를 재귀적으로 개선합니다.
...
지침을 고치는 것도 하나의 실험입니다. 규칙·에이전트·훅 같은 하네스 구성요소를 수정할 때는 **결정 관측성**이 적용됩니다: 편집마다 반증 가능한 예측 (어떤 실패 클래스가 사라지는가) 을 기록하고, 채택 전에 이중 검사를 통과해야 합니다 — **held-in** (원래 잡으려던 실패를 실제로 잡는가) 과 **held-out** (기존 가드·테스트가 여전히 통과하는가). 기각된 편집도 기록으로 남아, 같은 실패한 시도를 반복하지 않습니다.
