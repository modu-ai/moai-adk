...
특히 두 번째 기둥이 핵심 혁신입니다. AI의 재귀적 자기 개선 (RSI) 의 현실적인 단기 경로는 모델 가중치를 직접 수정하는 것이 아니라 **모델을 둘러싼 하네스를 개선하는 것**입니다. MoAI-ADK는 정확히 이 길을 따릅니다 — 모델이 아니라 하네스 (스킬·에이전트 지침) 를 재귀적으로 개선합니다.
