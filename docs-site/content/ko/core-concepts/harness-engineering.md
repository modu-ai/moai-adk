...
이 루프는 `/moai loop`에서 기본 최대 10회까지 반복되며, 수렴 감지 (같은 에러 반복 시 대안 전략 적용) 를 포함합니다. 완료 조건을 직접 선언하고 싶다면 goal 엔진 (`/moai goal "<조건>"`) 을 사용합니다 — 조건이 충족되거나 턴 한도에 닿을 때까지 세션이 스스로 일합니다.
