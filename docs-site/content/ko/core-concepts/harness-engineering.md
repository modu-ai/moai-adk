...
기존 바이브코딩은 AI에게 자유롭게 코드를 생성하게 한 뒤 결과를 사람이 하나하나 검토합니다. 하네스 엔지니어링은 그 반대입니다 — **규격 (SPEC), 자동 검증, 지속적 피드백 루프**로 AI 에이전트를 이끌어 품질이 흔들리지 않는 코드를 만들어 냅니다.

그럼 하네스란 무엇일까요? 기본 모델을 감싸고 실행을 지휘하는 시스템 전체를 말합니다. 모델이 어떻게 생각하고 계획할지, 도구를 어떻게 호출할지, 컨텍스트를 어떻게 인지하고 관리할지, 산출물을 어디에 저장할지, 결과를 어떻게 평가할지를 결정하는 계층입니다. MoAI-ADK는 Claude Code 위에 얹히는 바로 이 하네스입니다.

## 세 가지 핵심과 하네스

하네스 엔지니어링은 v3.0의 세 가지 핵심이 만나는 지점입니다.

| 핵심 | 하네스에서의 역할 |
|------|------------------|
| **토크노믹스** | 하네스가 작업마다 모델·추론 깊이를 배정하고 토큰 예산을 지킵니다 |
| **에이전틱 루프 엔지니어링** | 루프 (`/moai loop`, goal 엔진) 가 돌며 관찰을 축적하고, 하네스가 그 관찰로 학습합니다 |
| **에이전틱 하네스** | 11개 에이전트 카탈로그, 3-phase 워크플로우, TRUST 5 게이트가 실행 환경을 구성합니다 |

특히 두 번째 핵심이 지금의 발전을 이끕니다. AI가 스스로를 재귀적으로 개선하는 (RSI) 현실적인 단기 경로는 모델 가중치를 직접 손보는 쪽이 아니라 **모델을 감싼 하네스를 개선하는 쪽**입니다. MoAI-ADK는 정확히 이 길을 따릅니다 — 모델이 아니라 하네스 (스킬·에이전트 지침) 를 재귀적으로 개선합니다.
...
이렇게 해 두면 에이전트가 파일을 아무 데나 만드는 일을 막고 프로젝트 구조를 일관되게 유지할 수 있습니다.
...
이 루프는 `/moai loop`에서 기본 최대 5회(`ralph.max_iterations`)까지 반복되며, 수렴 감지 (같은 에러 반복 시 대안 전략 적용) 를 포함합니다. 완료 조건을 직접 선언하고 싶다면 goal 엔진 (`/moai goal "<조건>"`) 을 사용합니다 — 조건이 충족되거나 턴 한도에 닿을 때까지 세션이 스스로 일합니다.

### 4. Context Map (컨텍스트 맵)

`/moai codemaps`가 만들어 두는 아키텍처 문서는 코드베이스의 전체 구조를 에이전트에게 그대로 넘겨줍니다. 그 덕분에 에이전트는 다음을 할 수 있습니다:
...
지침을 고치는 일도 하나의 실험입니다. 규칙·에이전트·훅 같은 하네스 구성요소를 수정할 때는 **결정 관측성**이 적용됩니다. 편집할 때마다 반증할 수 있는 예측 (어떤 실패 클래스가 사라지는가) 을 적어 두고, 채택하기 전에 두 가지 검사를 통과해야 합니다 — **held-in** (원래 잡으려던 실패를 실제로 잡는가) 과 **held-out** (기존 가드·테스트가 여전히 통과하는가). 기각된 편집도 기록으로 남기 때문에, 이미 실패한 시도를 다시 되풀이하지 않습니다.
...
`moai-*` prefix 스킬과 `moai-harness-*`는 **MoAI-ADK 패키지가 제공하는 범용 자산**입니다. 모든 사용자 프로젝트에 배포되며, `moai update`를 실행하면 최신 template으로 **덮어쓰기**됩니다. 그래서 이 자산을 직접 수정하면 다음 업데이트 때 고친 내용이 사라집니다.
...
직접 만든 도메인 특화 스킬이나 에이전트가 `moai update` 뒤에도 살아남게 하려면 **반드시 `hns-*` prefix를 사용**하세요 (에이전트는 `.claude/agents/harness/` 에 배치). `moai-*` 또는 `moai-harness-*` prefix를 쓰면 template-managed로 분류되어 다음 업데이트 때 덮어써집니다. `/moai harness "자연어 요청"` 으로 하네스를 생성하면 Builder가 이 규칙에 맞는 이름을 자동으로 배정합니다.
