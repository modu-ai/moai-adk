...
MoAI-ADK는 **비용 · 자기 개선 · 품질 통제** 세 축으로 Claude Code를 감싸는 **Agentic Development Kit**입니다. 같은 품질의 코드를 더 적은 토큰으로 (비용 축, 토크노믹스), 매 세션이 돌 때마다 관찰을 규칙으로 쌓아 하네스가 점점 나아지게 하고 (자기 개선 축, 에이전틱 루프 엔지니어링), SPEC 3단계와 TRUST 5 게이트로 재작업을 막아 '끝'을 증거로 판정합니다 (품질 통제 축, 에이전틱 하네스) — 모델 선택·추론 깊이·컨텍스트 사용량을 시스템이 바깥에서 강제합니다. 11개의 전문 AI 에이전트와 31개의 스킬이 협력하고, 새 프로젝트에는 TDD (기본값), 테스트 커버리지가 낮은 기존 프로젝트에는 DDD를 자동으로 적용합니다.

Go로 작성된 단일 바이너리 -- 의존성 없이 모든 플랫폼에서 즉시 실행됩니다.


{{< callout type="info" >}}
**한 줄 요약:** MoAI-ADK는 "AI와 나눈 대화를 문서 (SPEC) 로 남기고, 안전하게 코드를 개선 (DDD/TDD) 하며, 품질을 자동 검증 (TRUST 5) 하는" 일을 — **비용·자기 개선·품질 통제 세 축을 시스템이 바깥에서 강제하면서** 수행하는 에이전틱 개발 키트입니다.
{{< /callout >}}
...
비용 대비 품질을 극대화하는 지능적 자원 분배입니다. 작업 단계와 SPEC 크기에 따라 모델과 추론 깊이를 선언적으로 배정하는 **3-계층 모델 정책**, Claude 리더와 GLM 워커를 조합해 구현 비용을 60-70% 줄이는 **CG 모드**, 예산 초과 전에 안전하게 멈추는 **Token Circuit Breaker**, 그리고 항시 로드 컨텍스트를 줄이는 **컨텍스트 다이어트**가 이 기둥을 이룹니다.
...
비용만 최적화하면 품질이 무너지고, 이어서 재작업과 디버깅 루프가 따라옵니다 — 그런데 재작업이야말로 모든 토큰 지출 중 가장 비쌉니다. 품질 게이트만 세우면 매 세션마다 같은 실수가 반복됩니다. 비용 상한 없이 자율 루프를 돌리면 과제 하나가 튀어 쿼터를 갉아먹습니다. 세 축은 서로를 떠받칩니다 — **비용**은 **품질**이 재작업을 막아 경제적으로 유지되고, **품질**은 **루프**가 통한 패턴을 기억해 강제 가능하며, **루프**는 **비용** 게이트가 초과 전에 멈춰 알맞은 가격에 머뭅니다.

### 비용 축 — 토크노믹스

토큰 단가는 계속 내려가지만, 에이전틱 개발의 토큰 사용량은 그보다 빠르게 늘어납니다. 에이전트가 여러 개 돌고, 컨텍스트가 길어지고, 추론이 깊어질수록 비용을 결정하는 것은 모델 가격이 아니라 **토큰 운용 방식**입니다.

비용 축의 실천 방식은 세 가지입니다.
...
토크노믹스 기둥의 실전 도구입니다. Leader가 **Claude API**를, Workers가 **GLM API**를 사용하는 하이브리드 모드로, tmux 세션 수준 환경 변수 격리로 구현됩니다. 전략·계획·감사는 Claude가, 대량 구현은 GLM이 맡아 구현 중심 작업에서 60-70% 비용을 절감합니다.
...
Task 도구 완료 시 PostToolUse 훅이 메트릭을 로깅합니다. 이 데이터로 에이전트 효율성을 분석하고 토큰 소비를 최적화하세요. 토크노믹스는 측정에서 시작합니다.
