...
이미 코드가 쌓인 브라운필드 프로젝트에서는 TDD 앞에 **pre-RED 분석 단계**가 하나 붙습니다. 테스트를 쓰기 전에 기존 코드를 읽고 현재 동작부터 파악하는 단계입니다.
...
| `loop` | 진단 기반 반복 자동 수정 (goal 엔진 위의 프리셋, 기본 최대 5회) | `--max N`, `--auto-fix`, `--seq` |
...
/moai loop      # 반복 수정: 완료 조건 충족까지 반복 (기본 최대 5회)
```
