...
| 낱말 | 뜻 | 예시 |
|---|---|---|
| **레인** (lane) | 카드 한 장을 끝까지 나르는 병렬 작업 흐름 하나. 세션 하나와 워크트리 하나의 짝이다. 물리 칸반 보드의 스윔레인처럼, 서로 다른 흐름이 섞이지 않도록 각자의 차선을 달린다. "레인 로컬 검증" (lane-local verification) 은 그 레인의 변경이 건드릴 수 있는 테스트만 그 레인이 돌린다는 뜻. | `run` 세션이 워크트리 `.claude/worktrees/t0`에서 일하는 것이 레인 하나 |
...
| **런 아이디** (run-id) | 리드가 시작할 때 알려주는 짧은 식별자. 리더 소켓 경로와 `MOAI_KANBAN_ID` 에만 들어가고 세션 이름에는 리드까지 포함해 들어가지 않는다 — 모든 세션은 역할 이름으로 부른다. | 리드가 알려주는 `a1b2c3`; 세션 이름 자체는 `lead` |
| **워크트리** (worktree) | 카드의 작업이 일어나는 격리된 체크아웃. 런처로 진입한다 (`moai cc -w <name>` / `EnterWorktree`) — raw `git worktree add`로 만들지 않는다. 디렉터리 이름이 카드 ID를 들고, 브랜치 이름은 무슨 일을 했는지 알려주는 `WT-<slug>` 형태다 — 하이픈 3토막 이내, 24자 이내, 카드 ID는 넣지 않는다. 워크트리는 단계보다 오래 산다: 하나가 run부터 sync까지를 관통한다. | 브랜치 `WT-todo-queue`인 `.claude/worktrees/t0` |
...
```mermaid
flowchart TD
    subgraph LaneA["레인 — 카드 t0: run 세션 + worktrees/t0"]
        A0["카드 t0"] --> A1["구현과 검증은<br/>worktrees/t0 안에서만"]
    end
    subgraph LaneB["레인 — 카드 t1: 두 번째 run 세션 + worktrees/t1"]
        B0["카드 t1"] --> B1["구현과 검증은<br/>worktrees/t1 안에서만"]
    end
```
...
4. `run`이 **워크트리** `.claude/worktrees/t0`에서 구현한다. 브랜치는 카드 ID가 아니라 한 일을 담은 이름을 쓴다 (`WT-t0`이 아니라 `WT-todo-queue`) — 이 세션과 워크트리의 짝이 곧 **레인**이고, 검증도 레인 안에서 (변경이 건드릴 수 있는 테스트만) 돌아간다. 리드가 시작할 때 알려주는 `a1b2c3`이 **런 아이디**이고, 리드 세션 이름 자체는 `lead`다.
5. sync 칸을 같은 방식으로 지나 — sync 게이트가 리뷰 판정을 흡수해 내린다 — sync가 PR을 올리면 카드는 done에 닿는다.

이 내내 카드의 작업은 `.claude/worktrees/t0` 하나에서 일어납니다. 병렬로 돌던 다른 레인의 워크트리와는 끝까지 섞이지 않습니다.
