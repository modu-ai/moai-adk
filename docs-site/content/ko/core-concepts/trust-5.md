...
에이전트가 코드를 빠르게 만들어 내는 능력은 곧 품질이 아닙니다. MoAI-ADK의 모든 코드가 통과해야 하는 5가지 품질 원칙이 **TRUST 5** (Tested · Readable · Unified · Secured · Trackable) 입니다. TRUST 5는 에이전틱 하네스의 품질 게이트로, 에이전트가 아무리 빠르게 코드를 찍어내도 이 게이트를 넘지 못하면 완료로 치지 않습니다. **왜냐하면** "완료되었다"는 보고가 실제로 관측된 증거가 아니면 재작업과 디버깅 루프로 다시 돌아가고, 재작업이야말로 토큰 예산을 가장 많이 갉아먹는 지점이기 **때문입니다**. 그래서 **harness** 는 SPEC 3단계 워크플로우의 매 끝마다 TRUST 5 게이트를 걸어 "된 것 같다"가 아니라 "증거가 있다"로 완료를 판정합니다.
...
TRUST 5 품질 게이트는 SPEC 범위에 따라 3단계 깊이로 적용됩니다. 복잡도 추정기
(Complexity Estimator) 가 SPEC 범위를 보고 하네스 레벨을 자동으로 정합니다.

| 레벨        | 이름 | 검증 범위                                              |
| ----------- | ---- | ----------------------------------------------------- |
| `minimal`   | 최소 | 빠른 검증 — 핵심 게이트만 통과                         |
| `standard`  | 표준 | 기본 검증 — 기본 품질 게이트 실행 (기본값)            |
| `thorough`  | 심층 | 전체 검증 — sync-auditor 독립 평가 + TRUST 5 전체 적용 |
...
{{< callout type="info" >}}
  **가독성 팁:** 6개월 뒤의 내가 이 코드를 읽고 바로 알아볼 수 있을지 스스로
  물어보세요. 아니라면 이름을 바꾸거나 주석을 덧붙이세요.
{{< /callout >}}
...
MoAI-ADK는 **LSP** (Language Server Protocol) 로 코드 품질을 실시간으로
확인합니다. LSP는 IDE에서 빨간 밑줄로 오류를 짚어 주는 바로 그 시스템입니다.
