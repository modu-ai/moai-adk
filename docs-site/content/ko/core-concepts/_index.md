...
MoAI-ADK v3.0을 이해하는 데 필요한 핵심 개념을 소개합니다. v3.0의 가치는 세 가지 핵심으로 요약됩니다 — **토크노믹스** (Token Economics), **에이전틱 루프 엔지니어링** (Agentic Loop Engineering), **에이전틱 하네스** (Agentic Harness). 이 섹션에서는 그 세 가지가 실제 개발 흐름에서 어떻게 맞물려 돌아가는지 하나씩 풀어봅니다. 세 가지가 서로를 먹여 살리기 **때문에** — 품질 게이트가 재작업을 막아야 비용이 유지되고, 루프의 관찰이 쌓여야 품질 규칙이 강제되며, 비용 게이트가 예산을 끊어 줘야 루프가 적정 가격에 머뭅니다 — **SPEC** · **TRUST 5** · **harness** 같은 단어를 어느 한 페이지에서만 보더라도 그 뒤에 얽힌 나머지 둘의 맥락이 함께 따라옵니다. 그래서 이 섹션은 단어의 사전이 아니라, 사이트 전체가 공유하는 어휘를 처음부터 세팅하는 곳입니다.
