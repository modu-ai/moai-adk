...
MoAI-ADK의 SPEC 기반 개발 방법론을 자세히 안내합니다. SPEC은 에이전틱 하네스의 입력이자 토크노믹스의 숨은 도구입니다 — 요구사항이 파일로 남아 있으면 세션을 끊고 `/clear`로 컨텍스트를 비워도 SPEC 한 줄로 작업을 이어갈 수 있습니다. 같은 설명을 되풀이하며 토큰을 태울 일이 없습니다.
