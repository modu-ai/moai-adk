...
MoAI-ADK의 SPEC 기반 개발 방법론을 자세히 안내합니다. SPEC은 에이전틱 하네스의 입력이자 토크노믹스의 숨은 도구입니다 — 요구사항이 파일로 남아 있으면 세션을 끊고 `/clear`로 컨텍스트를 비워도 SPEC 한 줄로 작업을 이어갈 수 있습니다. 같은 설명을 되풀이하며 토큰을 태울 일이 없습니다. **왜냐하면** 맥락이 세션에 머무는 게 아니라 파일에 남기 **때문에**, 어제 한 결정이 오늘 커밋으로 직행하고, **harness** 안의 모든 **에이전트** 가 같은 **SPEC** 을 읽고 같은 방향으로 일합니다.
...
| "소금 약간" 같은 모호한 표현 없음 | GEARS 형식으로 명확하게 | 모호함 제거                      |

## 왜 SPEC이 필요한가?

### 바이브코딩의 맥락 유실 문제

AI와 대화하며 코드를 짤 때 가장 큰 걸림돌은 **맥락 유실**입니다.
...
SPEC은 대화 내용을 **파일로 저장**해 이 문제를 뿌리부터 해결합니다. 파일에 남은 결정은 컨텍스트 윈도우와 상관없이 그대로 살아남습니다 — 하네스 엔지니어링에서 말하는 "durable state in files" (파일에 담긴 지속 상태) 의 대표적인 예입니다.
...
어제 "사용자 인증 기능"을 두고 1시간 동안 AI와 이야기를 나눴다고 해봅시다. JWT를 쓸지
세션을 쓸지, 토큰 만료 시간은 얼마로 할지, 리프레시 토큰은 어디에 저장할지... 이걸
처음부터 다시 이야기해야 합니다.
...
SPEC 문서는 **manager-spec 에이전트**가 자동으로 만듭니다. 개발자가 GEARS 형식을
외울 필요는 없습니다. 자연어로 요청하면 에이전트가 알아서 옮겨 씁니다.
...
2. **명확화 질문:** 모호한 부분이 있으면 사용자에게 질문합니다 (예: "JWT와 세션
   중 어떤 방식을 선호하시나요?")
3. **GEARS 변환:** 자연어를 5가지 요구사항 유형으로 자동 분류합니다
...
일상적인 요청을 GEARS 형식으로 어떻게 바꾸는지 비교합니다.
...
{{< callout type="info" >}}
  GEARS 형식을 직접 작성하지 않아도 됩니다. `/moai plan`에 자연어로 요청하면
  **manager-spec 에이전트가 자동으로 GEARS 형식으로 변환**합니다. 위 표는 어떻게
  변환되는지 이해하기 위한 참고 자료입니다.
{{< /callout >}}

## SPEC 라이프사이클과 Era 분류

SPEC은 한 번 쓰고 끝나는 문서가 아니라 **계획(plan) → 구현(run) → 동기화(sync)** 라는 라이프사이클을 따릅니다. MoAI-ADK는 각 SPEC이 어느 시대(era)의 규약으로 작성됐는지 자동으로 분류하고, 현대 규약을 따르는 SPEC에만 드리프트(drift, 규약 이탈) 검사를 겁니다.
...
**V2.x · V3R2-R4 · V3R5** 로 분류된 SPEC은 **grandfather 절이 보호**합니다. 이 세 era는 작성 당시의 규약을 제대로 지킨 것이므로 현대 V3R6 규약을 거슬러 적용하지 않습니다.
...
이 분류 체계 덕분에 오래된 SPEC에서 거짓 양성(false positive)이 튀어나오지 않고, 지금 작성 중인 SPEC의 규약 준수만 정확하게 검증할 수 있습니다.
