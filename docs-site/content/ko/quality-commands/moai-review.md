...
리뷰의 기본 담당은 **sync-auditor** — 코드를 만든 에이전트가 아닌 독립 평가자입니다. 만든 사람이 검사하지 않는다는 하네스 원칙이 리뷰 명령어에도 그대로 적용됩니다. @MX 태그 준수 여부도 함께 검사하여, AI 에이전트가 코드를 더 잘 이해할 수 있도록 돕습니다.
...
보안, 성능, 품질, UX 전문가가 각각 독립적으로 리뷰하므로 더 깊이 있는 분석이 가능합니다. 그만큼 토큰 소비도 커지므로 (약 4배), 보안·결제처럼 중요도가 높은 변경에 선택적으로 사용하는 것이 경제적입니다.
...
전체 코드베이스가 아니라 변경 범위만 리뷰 대상으로 잡는 것도 토큰 효율을 위한 설계입니다.
...
- [/moai gate - 커밋 전 품질 게이트](/quality-commands/moai-gate)
