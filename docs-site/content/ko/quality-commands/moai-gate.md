...
"작업 크기에 맞는 검증 깊이"라는 v3 토크노믹스 원칙이 이 명령어의 설계 이유입니다 — 커밋마다 5-10분짜리 전체 품질 파이프라인을 돌리는 대신, 30초짜리 게이트로 대부분의 결함을 커밋 전에 걸러냅니다.
...
`/moai gate`는 프로젝트 루트의 indicator 파일을 우선순위 순서대로 확인해 첫 번째 매칭에 해당하는 toolchain을 사용합니다. 16개 언어는 모두 동등하게 지원됩니다.
...
검증 도구는 세 단계 깊이로 나뉘어 있고, 상황에 맞는 깊이를 고르는 것이 원칙입니다.
