...
| `--fresh` | 강제 fresh 모드 — 공유 진단 스냅샷 소비를 모두 비활성화하고 모든 검사를 새로 실행 | `/moai gate --fresh` |
...
검사를 실행하기 전, 현재 작업 트리에 대한 공유 진단 스냅샷을 조회합니다. 신선한 스냅샷 (키 일치 + TTL 이내, 기본 10분) 이 이 게이트가 돌릴 검사 카테고리를 커버하면, 재실행 대신 기록된 결과를 재사용하고 보고서에 `Test | PASS (snapshot)`처럼 표시합니다. 오래된 스냅샷은 절대 증거로 인용하지 않고 재실행합니다. `--fresh` 모드에서는 이 단계를 통째로 건너뜁니다.
...
개별 검사 타임아웃은 60초, 전체 게이트 타임아웃은 90초입니다. 타임아웃되면 WARNING으로 보고하되 차단하지 않습니다. 실행된 (재사용이 아닌) 전체 범위 검사 결과는 `.moai/state/verify/` 아래 공유 스냅샷 스토어에 기록되어, 다운스트림 소비자 (run-phase 사전 리뷰 게이트, sync 사전 게이트, stop-goal 평가기) 가 트리가 그대로인 동안 재사용할 수 있습니다.
