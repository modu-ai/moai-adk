...
| `moai todo done <n> [--expect <prefix>] [--require-landed]` | `n`번 항목을 잠금 아래 살아 있는 대기열에서 빼냅니다. `t<n>` 형태의 명시적 id를 권장합니다 — 동시 추가로 위치가 움직일 수 있기 때문입니다. 카드와 그 카드를 가리키던 모든 기록은 **삭제가 아니라 보관**되어, `undone`이 둘 다 되살릴 수 있습니다. 보관된 행은 `list` · `next` · `why` · `analyze`와 중복 판정 입력에서 보이지 않습니다. `--expect <prefix>`는 카드 텍스트가 그 접두사로 시작하지 않으면 거부합니다 — 엉뚱한 카드를 닫는 사고를 막는 안전장치입니다. `--require-landed`는 착지 판정을 물어 "착지하지 않았다"는 확답이 나올 때만 거부하고, 판정이 불확실하면 통과시킵니다 — 이 판정은 설정된 기준 ref 위의 커밋이 카드를 언급하는지만 물으며, 그 판정 자체가 항상 맞다는 보장은 아닙니다. 플래그가 없으면 착지 질의는 전혀 실행하지 않고 비용도 붙지 않으며, 어느 경로로 거부되든 대기열 기록은 바이트 그대로 남습니다. 성공한 `done`은 표준출력에 착지 판정 토큰을 정확히 하나 남깁니다 — `done <id> landing=landed|not-landed|unknown` — 플래그가 없을 때도 마찬가지이며, 그때는 질의를 아예 실행하지 않았으므로 `unknown`입니다. 판정이 불확실해 통과한 경우도 같은 `unknown`으로 남아, "통과했다"와 "질의가 한 번도 돌지 않았다"가 더는 표준출력에서 같은 글자로 보이지 않습니다. |
