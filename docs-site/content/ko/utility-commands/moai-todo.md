...
| `state` | 생명주기 판별자입니다. `queued` · `picked` · `dropped` 중 하나이며, "아직 백로그의 항목"인지 "이미 보드 위로 넘어간 카드"인지는 이 값이 가립니다. 고른 항목도 파일에 남아 무엇이 진행 중인지 보이며, 카드가 `done`에 닿을 때 제거됩니다. |
...
3. 고를 때 식별자를 몰랐다면, plan 세션이 알려 주는 대로 항목에 붙입니다.
