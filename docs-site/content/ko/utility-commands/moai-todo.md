...
{{< callout type="info" >}}
**한 줄 요약**: `/moai todo`는 다음에 할 일을 기록하는 대기열입니다. 항목을 넣고, 목록을 보고, 완료한 카드를 보관하고, 다음에 착수할 카드를 고릅니다. 선택만으로 SPEC이 생기지는 않으며, 카드 분류에 따라 작업 절차가 정해집니다.
{{< /callout >}}
...
백로그의 한 항목은 **의도 한 줄**이며 SPEC이나 계획서가 아닙니다. 사람이 고른 뒤 Class A는 바로 마무리하고, Class B는 SPEC 없이 run → sync를 진행하며, Class C는 plan에서 SPEC을 작성한 뒤 run → sync로 이어집니다.
...
flowchart TD
    ...
    Class --> A["A: 바로 마무리"]
    Class --> B["B: run → sync"]
    Class --> Plan["C: plan → run → sync<br/>plan에서 SPEC 작성"]
...
| `project_uuid` / `card_uuid` | 프로젝트와 카드의 영속 identity입니다. identity가 없는 이전 데이터를 `list --json`으로 읽을 때도 키는 생략되지 않고 `null`이며, 이 순수 조회는 UUID를 발급하거나 데이터베이스를 바꾸지 않습니다. 이후 승인된 쓰기가 성공하면 canonical lowercase UUIDv7 문자열로 채워집니다. Git 저장소와 Git 없는 일반 폴더에 같은 계약을 적용합니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `spec_id`                    | SPEC 식별자로 가는 선택적 연결고리입니다. 고르는 시점에 `--spec`으로 알려 주면 그때 채워지고, 모르면 `picked` 상태에서도 `null`로 남습니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `runtime`                    | 최신 실행 시작과 카드 할당 보고입니다. `runs`와 `assignments`는 언제나 배열이며, 각 항목의 UUID는 Todo identity를 논리적으로 투영합니다. runtime 테이블에 UUID 열을 따로 저장하지 않습니다. `reported_state`는 보고값일 뿐 카드 완료 상태의 권위가 아닙니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `state`                      | 생명주기 판별자입니다. `queued` · `picked` · `dropped` 중 하나이며, "아직 백로그의 항목"인지 "이미 보드 위로 넘어간 카드"인지는 이 값이 가립니다. 고른 항목도 파일에 남아 무엇이 진행 중인지 보입니다. 항목을 살아 있는 대기열에서 **빼내는** 길은 `moai todo done` 하나뿐이고 사람이 직접 실행합니다 — 일이 끝났다고 저절로 지워지는 경로는 없습니다. `done`은 카드와 그 카드를 가리키던 모든 기록을 **삭제하지 않고 보관소로 옮깁니다**: `list` · `next` · `why` · `analyze`에는 더는 보이지 않지만, `moai todo undone <n>`이 카드가 있던 위치까지 그대로 되살립니다. 폐기는 보관과 다릅니다: `moai todo drop <n> "<이유>"`는 카드를 대기열 파일에 그대로 둔 채 `dropped`로 옮기고 이유를 텍스트 앞에 붙이며, `moai todo undrop <n>`이 이를 정확히 되돌립니다. `dropped` 카드는 고를 후보에서 빠집니다. |

변경은 대기열 잠금을 잡은 뒤 WAL 모드의 SQLite 트랜잭션으로 반영합니다. 대기열이 없으면 **빈 대기열**로 처리하고, 읽을 수 없거나 형식이 깨진 데이터는 오류를 알리고 보존합니다. 웹 콘솔도 읽기 실패를 카드 수 0으로 표시하지 않고, 대기열을 읽지 못했다는 안내를 보여 줍니다.
...
2. 칸반 카드 분류를 따릅니다. Class A는 바로 마무리하고, Class B는 SPEC 없이 run → sync, Class C는 plan에서 SPEC을 작성한 뒤 run → sync를 진행합니다.
3. SPEC이 필요한 카드에서는 식별자가 정해진 뒤 `moai todo next <n> --spec <SPEC-ID>`로 연결합니다. 연결은 명시적으로 수행하며, 선택된 카드에 SPEC이 없는 상태도 정상일 수 있습니다.
...
# 항목을 보관소로 옮깁니다 — 번호(t4)나 명시적 id 둘 다 받습니다
$ moai todo done 4

# 되돌리기: 보관된 카드를 있던 자리 그대로 되살립니다
$ moai todo undone 4

# 텍스트가 접두사로 시작할 때만 처리 — 잘못된 카드를 닫는 사고 방지
$ moai todo done 4 --expect "인증 미들웨어"
...
| `moai todo done <n> [--expect <prefix>] [--require-landed]`                                          | `n`번 항목을 잠금 아래 살아 있는 대기열에서 빼냅니다. `t<n>` 형태의 명시적 id를 권장합니다 — 동시 추가로 위치가 움직일 수 있기 때문입니다. 카드와 그 카드를 가리키던 모든 기록은 **삭제가 아니라 보관**되어, `undone`이 둘 다 되살릴 수 있습니다. 보관된 행은 `list` · `next` · `why` · `analyze`와 중복 판정 입력에서 보이지 않습니다. `--expect <prefix>`는 카드 텍스트가 그 접두사로 시작하지 않으면 거부합니다 — 엉뚱한 카드를 닫는 사고를 막는 안전장치입니다. `--require-landed`는 착지 판정을 물어 "착지하지 않았다"는 확답이 나올 때만 거부하고, 판정이 불확실하면 통과시킵니다 — 이 판정은 답한 ref 위의 커밋 **제목**이 여섯 가지 귀속 위치 가운데 하나로 카드를 가리키는지만 봅니다(관례적 커밋 스코프, 끝 괄호 크레딧, 크레딧 뒤 PR 참조, 카드 주도 머지, 통합 머지 끝 괄호 안의 카드 토큰, `merge: <card>`). 본문 언급, 문장 가운데 언급, 브랜치 이름은 아무것도 세지 않습니다 — 귀속은 메시지 어디든 나타나는 토큰이 아니라 제목 안의 **위치**입니다. ref는 프로젝트 설정(`git_strategy.worktree_base_branch`) → `refs/remotes/origin/HEAD` → 내장 기본값(`origin/main`) 순서로 정해지며, 설정 없이 아래 단계에서 답했을 때는 그 출처를 표준오류로 알려 줍니다. 플래그가 없으면 착지 질의는 전혀 실행하지 않고 비용도 붙지 않으며, 어느 경로로 거부되든 대기열 기록은 바이트 그대로 남습니다. 성공한 `done`은 표준출력에 착지 판정 토큰을 정확히 하나 남깁니다 — 플래그가 있으면 `done <id> landing=landed | not-landed | unknown ref=<ref>`, 없으면 `done <id> landing=landed | not-landed | unknown`— 플래그가 없을 때는 질의를 아예 실행하지 않았으므로`unknown`입니다. 판정이 불확실해 통과한 경우도 같은 `unknown`으로 남아, "통과했다"와 "질의가 한 번도 돌지 않았다"가 더는 표준출력에서 같은 글자로 보이지 않습니다. |
| `moai todo undone <n>`                                                                               | 보관된 카드를 있던 위치 그대로, 그 카드를 가리키던 모든 기록과 함께 살아 있는 대기열로 되살리고 보관 항목은 비웁니다. `done` 다음 `undone`은 대기열 기록을 정확히 같은 바이트로 되돌립니다. id가 그사이 다른 살아 있는 카드에 재발급됐으면 그 충돌을 지목하며 거부하고, 살아 있는 카드는 건드리지 않습니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `moai todo export-json`                                                                              | 살아 있는 대기열을 옛 형식의 `backlog.json`으로 데이터베이스 옆에 써 냅니다. 데이터베이스를 모르는 이전 릴리스가 읽을 수 있게 하려는 것입니다. 실행한 순간의 사본일 뿐 실시간 거울이 아니므로, 바이너리를 바꾸기 직전에 내보내십시오. 보관된 카드는 살아 있는 항목과 달리 이 내보내기에 **포함**됩니다 — 다만 내보낸 레코드에 보관 카드가 하나라도 있으면, 보관 기능을 모르는 이전 바이너리가 첫 쓰기에서 그 필드를 버린다는 사실을 stderr로 알립니다. 이후의 `todo` 명령들은 이 파일을 놓아둔 자리에 그대로 둡니다. 읽기 명령인 `list`·`why`·`pr`·`history`는 여기에 더해, 데이터베이스 옆의 이 파일이 대기열이 아니라는 사실을 stderr로 알립니다. 오래된 내보내기를 살아 있는 상태로 착각하지 않게 하려는 것입니다.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
