...
| `state` | 생명주기 판별자입니다. `queued` · `picked` · `dropped` 중 하나이며, "아직 백로그의 항목"인지 "이미 보드 위로 넘어간 카드"인지는 이 값이 가립니다. 고른 항목도 파일에 남아 무엇이 진행 중인지 보입니다. 대기열에서 항목을 지우는 길은 `moai todo done` 하나뿐이고 사람이 직접 실행합니다 — 일이 끝났다고 저절로 지워지는 경로는 없습니다. (`dropped`은 레코드 스키마에 정의된 값일 뿐, 이를 설정하는 명령은 없습니다.) |
