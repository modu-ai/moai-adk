...
flowchart TD
    ...
    Queue --> Pick["/moai todo next<br/>사람이 하나 선택"]
...
| `/moai todo done <n>` | `n`번 항목을 제거합니다(다른 데서 끝났거나 더 이상 필요 없어진 경우). 무엇을 지웠는지 보여 줍니다. |
| `/moai todo next` | 대기열을 선택지로 띄워 사람이 다음 카드를 고르게 합니다. |
...
| `spec_id` | 항목을 골라 SPEC을 저작한 뒤에 채워집니다. 그 전까지는 `null`이고, 이 값의 유무가 "아직 백로그의 항목"과 "이미 보드 위의 카드"를 가릅니다. |
| `state` | `queued` · `picked` · `dropped`. 고른 항목도 파일에 남아 무엇이 진행 중인지 보이며, 카드가 `done`에 닿을 때 제거됩니다. |
...
`/moai todo next`는 대기열을 선택지로 띄웁니다. 오래된 것부터 한 항목에 하나씩, 도구가 허용하는 네 개까지 보여 주고 나머지는 본문에 요약해 아무것도 가려지지 않게 합니다. 리드 세션이 `/clear` 뒤 첫 수로 대기열을 제시할 때도 같은 방식입니다.
