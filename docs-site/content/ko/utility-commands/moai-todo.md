...
| `moai todo export-json` | 살아 있는 대기열을 옛 형식의 `backlog.json`으로 데이터베이스 옆에 써 냅니다. 데이터베이스를 모르는 이전 릴리스가 읽을 수 있게 하려는 것입니다. 실행한 순간의 사본일 뿐 실시간 거울이 아니므로, 바이너리를 바꾸기 직전에 내보내십시오. 이후의 `todo` 명령들은 이 파일을 놓아둔 자리에 그대로 둡니다. |
