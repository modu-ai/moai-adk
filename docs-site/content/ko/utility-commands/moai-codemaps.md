...
생성된 문서는 `.moai/project/codemaps/` 디렉토리에 저장되며, 사람과 AI 에이전트 모두가 코드베이스를 빠르게 이해할 수 있도록 돕습니다. 하네스 엔지니어링 용어로는 **컨텍스트 맵** — 에이전트가 매 세션 아키텍처를 재발견하는 대신 항상 참조할 수 있는 파일 기반 지도입니다. 반복 탐색 비용을 문서 생성 1회로 대체한다는 점에서 토큰 절약 효과도 큽니다.
...
생성된 지도가 실제 코드와 일치하는지 기계적으로 확인하는 단계입니다 — 문서도 "생성했다"가 아니라 검증을 거쳐야 완료로 판정됩니다.
