...
발견 단계에서는 확신이 낮거나 심각도가 낮은 것도 **모두** 보고합니다(각각 confidence·severity 부여). 필터링은 판정 단계(must-pass 임계값 + 조화 평균 점수)가 downstream에서 담당합니다 — 발견 단계의 목표는 커버리지입니다.

## --security 정본 절차

`--security` 플래그를 지정하면 보안 관점이 우선순위를 받아 더 깊이 분석합니다.
