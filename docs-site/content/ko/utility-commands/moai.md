...
모든 워크플로우 단계가 성공적으로 끝나면, MoAI는 완료 보고서(배너/서술형)에 작업 완료를 명시해 결과를 분명히 합니다.

## LLM 모드 라우팅

MoAI-ADK가 비용을 줄이는 장치 중 하나입니다. llm.yaml 설정에 따라 단계별로 Claude와 GLM을 자동 라우팅합니다. 전략·계획은 Claude가, 대량 구현은 저비용 GLM이 담당하는 하이브리드가 가능합니다.
