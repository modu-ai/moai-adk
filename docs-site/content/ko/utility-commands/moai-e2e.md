...
웹·모바일·데스크탑 애플리케이션의 E2E (End-to-End) 테스트를 생성하고 실행하는 명령어입니다. 프로젝트 유형을 **자동 감지**하고, 플랫폼에 맞는 **CLI 우선 툴체인**을 선택해 토큰을 최소화하며 실행합니다. 전체 실행 로그는 파일로 돌리고 컨텍스트에는 종료 코드와 요약만 남기기 때문에, 긴 테스트 출력이 컨텍스트를 잡아먹지 않습니다.
...
단위 테스트가 함수 하나를 검증한다면, E2E 테스트는 **사용자가 실제로 거치는 흐름 전체**를 검증합니다. `/moai e2e`는 이 과정을 자동으로 처리합니다. 프로젝트가 웹인지 모바일인지 데스크탑인지 스스로 가려낸 다음, 플랫폼별 기본 툴체인을 골라 테스트 스크립트를 쓰고 실행합니다.
...
데스크탑-네이티브 레인은 세 OS (macOS·Windows·Linux) 의 접근성 레시피를 모두 문서로 갖추고 있지만, **호스트 OS 규칙**에 따라 호스트와 다른 OS의 레시피는 설명으로만 남겨 두고, 실제 프로브와 실행은 호스트 OS에서만 합니다.
...
프로젝트 문서와 라우트 정의 (routes.ts, urls.py, router.go, 내비게이션 그래프 등) 를 읽어 테스트할 사용자 여정 후보를 추립니다. 로그인, 핵심 기능, 오류 처리처럼 중요한 경로가 앞순위입니다.
...
모든 여정 단계는 **검증할 수 있는 결과**와 짝을 이룹니다. 단언 (assertion) 없이 화면만 넘기는 스크립트는 쓰지 않습니다.
...
- **진입 승인 1회**: 첫 위임 전에 오케스트레이터가 한 번 승인을 받습니다. 이 승인 하나가 루프 전체를 덮으므로 다음 반복부터는 다시 묻지 않습니다. 거절하면 원래의 수동 진행 흐름으로 돌아갑니다.
- **저지 그룹화**: 서로 다른 파일을 건드리는 독립 저지는 병렬 fan-out, 같은 모듈을 건드리는 의존 저지는 순차 처리합니다 (동시 쓰기 충돌 방지).
- **루프 한계**: 최대 3회 반복 (`ci-autofix-protocol.md`와 동일). green이면 보고하고, 3회를 다 쓰면 남은 실패와 아티팩트 경로를 사용자에게 돌려줍니다.
...
보고서·트레이스·스크린샷·레코딩은 프로젝트 안의 `e2e/` 디렉토리에 저장하고 **경로만 인용**합니다. 내용을 컨텍스트에 그대로 펼쳐 넣지 않습니다.
...
Electron도 Tauri도 아닌 **네이티브 데스크탑 앱** (순수 macOS 앱, WinUI, Qt/GTK 등) 이 잡히면 이 분기로 빠지지 않습니다. 데스크탑-네이티브 자동화 레인 (macOS의 axcli, Windows의 FlaUI.WebDriver, Linux의 dogtail) 으로 넘어가 테스트가 정상적으로 진행됩니다. "E2E 대상 없음" 분기는 테스트할 표면이 하나도 없는 순수 라이브러리에만 해당합니다.
