...
에이전트에게 프로젝트 지식을 파일로 남겨 두는 것은 하네스 설계의 기본 패턴이고, @MX 태그는 그 패턴을 **코드 레벨**에 적용한 것입니다. 에이전트가 매번 전체 코드를 정독해 위험 지점을 재발견하는 대신, 표지판을 따라가면 됩니다 — 탐색 토큰을 아끼면서 (토크노믹스) 위험 지점을 놓치지 않게 (품질) 하는 이중 효과입니다.
...
핵심 원칙은 "모든 코드에 태그"가 아니라 **"AI가 가장 먼저 주목해야 할 코드에만 태그"**입니다. 대부분의 코드는 어떤 조건도 만족하지 않아 태그가 없고, 그것이 정상입니다.
...
P1 및 P2 파일을 심층 분석하여 정확한 태그 설명을 생성합니다. 프로젝트 컨텍스트 (tech.md, structure.md, product.md)를 활용합니다. 전 파일 정독이 아니라 우선순위 상위 파일만 깊게 읽는 것 — 스캔 자체도 토큰 효율적으로 설계되어 있습니다.
...
@MX 태그는 SPEC 3-Phase 전 단계에 통합되어 있습니다 — plan에서 대상 식별, run에서 생성/갱신, sync에서 검증 및 누락 보완:

| 워크플로우 | MX 통합 방식 |
|-----------|-------------|
| `/moai run` | DDD ANALYZE 단계에서 자동 트리거, 태그 생성/갱신 |
| `/moai sync` | 동기화 중 MX 검증 자동 실행 |
