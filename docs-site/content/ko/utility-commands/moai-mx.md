...
코드베이스를 스캔해 **@MX 코드 주석**을 붙이는 명령어입니다. @MX 태그는 AI 에이전트가 코드의 맥락과 의도, 위험을 빠르게 파악하도록 돕는 코드 레벨 어노테이션입니다.

{{< callout type="info" >}}
**한 줄 요약**: `/moai mx`는 "AI를 위한 코드 이정표 설치기" 입니다. fan-in이 높은 함수, 위험 구역, 아직 덜 된 지점을 알아서 찾아 `@MX:ANCHOR`·`@MX:WARN`·`@MX:NOTE`·`@MX:TODO` 태그를 코드에 심습니다.
{{< /callout >}}
...
에이전트가 코드를 이해하는 데 드는 비용은 곧 컨텍스트 (토큰) 입니다. @MX 태그는 "이 함수는 8곳에서 부르니 시그니처를 함부로 바꾸지 말 것" 같은 맥락을 코드 옆에 바로 박아 둡니다. 그래서 에이전트가 매번 코드베이스 전체를 다시 뜯어보지 않아도 됩니다. 하네스 엔지니어링 관점에서 이것은 **코드에 박아 두는 앵커**입니다. 되풀이되던 탐색 비용을 한 번의 어노테이션으로 갈음해 비용을 아낍니다.
...
프로젝트 언어를 알아내고 (16개 언어, 마커 파일 우선순위) 언어별 주석 접두사 (`//`, `#` 등) 를 정합니다. `.moai/project/tech.md`·`structure.md`·`product.md`·`README.md`를 읽어 태그 설명에 쓸 프로젝트 맥락을 챙기고, 스캔 범위와 토큰 예산을 셈합니다. `--no-discovery`를 주면 이 단계를 건너뜁니다.

### Pass 1: 전체 파일 스캔

모든 소스 파일을 언어별 패턴으로 Glob해 fan-in 분석 (함수·메서드 참조 횟수), 복잡도 감지 (줄 수·분기·중첩 깊이), 패턴 감지 (goroutine·async·threading·unsafe) 를 한 뒤, 점수순으로 정렬한 우선순위 큐 (P1-P4) 를 만듭니다.

### Pass 2: 선택적 심층 읽기

P1·P2 파일만 꼼꼼히 읽어 함수 시그니처와 호출 패턴을 살피고, 프로젝트 맥락 (tech.md·structure.md·product.md) 을 반영한 정확한 태그 설명을 언어별 주석 문법에 맞춰 씁니다.
...
- **`/moai sync`**: sync 단계에서 MX 검증이 알아서 돌아갑니다. 마지막 sync 이후 바뀐 파일을 훑어 빠진 @MX 태그를 확인하고, `--skip-mx` 플래그가 없으면 태그를 채워 넣은 뒤 sync 보고서에 태그 변경 내역을 함께 싣습니다.
- **`/moai run`**: DDD ANALYZE 단계에서 코드베이스에 @MX 태그가 하나도 없으면 3-Pass가 저절로 시작됩니다. 이미 있던 태그는 확인하고 손보며, 새로 쓴 코드에는 새 태그를 답니다.
