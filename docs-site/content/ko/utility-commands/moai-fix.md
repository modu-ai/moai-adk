...
`/moai loop`와 달리 **딱 1회만** 실행되므로, 빠르게 현재 상태를 깨끗하게 만들고 싶을 때 적합합니다. 루프 계열에서 보면 `/moai fix`는 **단발 (1회) 프리셋**입니다. 반복이 필요 없는 명확한 오류에 루프를 돌리는 건 토큰 낭비이고, 작업 크기에 맞는 가장 싼 도구를 고르는 것이 비용을 줄이는 올바른 선택입니다.
