...
`/moai loop`와 달리 **딱 1회만** 실행되므로, 빠르게 현재 상태를 깨끗하게 만들고 싶을 때 적합합니다. 루프 계열에서 보면 `/moai fix`는 **단발 (1회) 프리셋**입니다 — 반복이 필요 없는 명확한 오류에 루프를 돌리는 것은 토큰 낭비이므로, 작업 크기에 맞는 가장 싼 도구를 고르는 것이 토크노믹스 관점의 올바른 선택입니다.
...
수집된 이슈를 **위험도에 따라 4단계로 분류**합니다. 레벨에 따라 자동 수정 여부가 달라집니다. 안전한 것은 기계가 처리하고, 위험한 것은 사람의 승인을 받는 — 자율성과 안전 게이트를 함께 두는 하네스 설계 원칙이 여기에도 적용됩니다.
...
| 에이전트 | 역할 | 주요 작업 |
|----------|------|----------|
| **MoAI 오케스트레이터** | 병렬 스캔 조율 | 이슈 수집, 레벨 분류, 사용자 승인 |
| **manager-develop** | 수정 실행 | Level 1-2 자동 수정, Level 3-4 승인 후 수정 |
