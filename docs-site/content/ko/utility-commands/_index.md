...
자율 자동화, 반복 수정 루프, 코드 관리, 피드백을 담당하는 명령어 모음입니다.


{{< callout type="info" >}}
유틸리티 명령어는 워크플로우 명령어 (`/moai plan`, `/moai run`, `/moai sync`) 와 달리 **빠른 자동화와 문제 해결**에 초점을 맞춥니다.
{{< /callout >}}

## 에이전틱 루프 엔지니어링이 돌아가는 곳

v3의 세 가지 핵심 중 하나인 **에이전틱 루프 엔지니어링**이 실제로 도는 자리가 이 섹션입니다. `/moai loop`는 진단 도구가 찾은 이슈 큐를 비울 때까지 반복하는 진단형 루프이고, `/moai goal`은 완료 조건을 선언해 두면 그 조건이 충족될 때까지 세션이 스스로 일하는 조건 선언형 루프입니다. `/moai fix`는 같은 계열의 단발 (1회) 프리셋입니다. 루프가 돌면서 남긴 관찰은 하네스 학습의 원료가 됩니다. 루프가 관찰을 쌓고 하네스가 그것을 학습해 지침을 고쳐 가는 되먹임이 바로 여기서 시작됩니다.
