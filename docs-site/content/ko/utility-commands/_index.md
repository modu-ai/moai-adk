...
유틸리티 명령어는 SPEC 3-Phase 라이프사이클 (`/moai plan` → `/moai run` → `/moai sync`) 바깥에서 **빠른 자동화와 문제 해결**을 맡는 명령어 모음입니다. 진단 도구가 찾은 오류를 반복해 고치는 루프 (`loop`, `fix`), 완료 조건을 선언해 세션을 계속 돌리는 조건 선언형 루프 (`goal`), 코드 위생 (`clean`, `mx`, `gate`), 아키텍처 문서화 (`codemaps`), 다관점 코드 리뷰 (`review`), E2E 테스트 (`e2e`), 그리고 피드백 제출 (`feedback`) 까지 맡습니다. 워크플로우 명령어가 "기능 하나를 통째로 만드는" 긴 흐름이라면, 유틸리티 명령어는 "지금 당장 이 문제를 풀고 싶을 때" 쓰는 짧고 뾰족한 도구들입니다. 따라서 두 계열은 서로 겹치지 않고 보완합니다.
...
## 에이전틱 루프 엔지니어링이 돌아가는 곳

v3의 세 가지 핵심 중 하나인 **에이전틱 루프 엔지니어링**이 실제로 도는 자리가 이 섹션입니다. `/moai loop`는 진단 도구가 찾은 이슈 큐를 비울 때까지 반복하는 진단형 루프이고, `/moai goal`은 완료 조건을 선언해 두면 그 조건이 충족될 때까지 세션이 스스로 일하는 조건 선언형 루프입니다. `/moai fix`는 같은 계열의 단발 (1회) 프리셋입니다. 루프가 돌면서 남긴 관찰은 하네스 학습의 원료가 됩니다. 루프가 관찰을 쌓고 하네스가 그것을 학습해 지침을 고쳐 가는 되먹임이 바로 여기서 시작됩니다. 그래서 이 섹션의 명령어들은 단순한 편의 도구가 아니라, 세 번째 핵심 가치가 런타임에서 현장적으로 검증되는 현장입니다.
