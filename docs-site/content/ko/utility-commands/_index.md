...
v3의 세 가지 핵심 중 하나인 **에이전틱 루프 엔지니어링**이 실제로 도는 곳이 이 섹션입니다. `/moai loop`는 진단 도구가 찾은 이슈 큐를 비울 때까지 반복하는 진단형 루프이고, `/moai goal`은 완료 조건을 선언하면 조건이 충족될 때까지 세션이 스스로 일하는 조건 선언형 루프입니다. `/moai fix`는 같은 계열의 단발 (1회) 프리셋입니다. 루프가 돌며 남긴 관찰은 하네스 학습의 원료가 됩니다 — 루프가 관찰을 축적하고, 하네스가 학습하여 지침이 진화하는 되먹임 고리의 입구입니다.
