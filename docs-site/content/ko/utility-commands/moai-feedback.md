...
| 수집 항목 | 설명 | 예시 | 수집 방식 |
|-----------|------|------|-----------|
| MoAI-ADK 버전 | 현재 설치된 버전 (`moai version`) | v3.1.3 | 보장 (항상 수집) |
...
`/moai feedback`은 다음 세부 동작들로 이슈 생성 과정을 보강합니다.
...
작성된 보고서가 `gh issue create`로 나가기 전에 **스크러빙 계약**이 동작합니다. 마스킹 스크러버가 시크릿·토큰·경로 같은 민감값을 가리고, 취약점 분류기가 본문이 보안 취약점 제보인지 판정하고, 가리긴 값의 목록(마스크 로그)과 제출에 실패한 항목(재시도 큐)을 각각 기록으로 남깁니다. 이 절차는 터미널에서 `moai feedback scrub`(가리기 미리보기)과 `moai feedback queue`(재시도 큐 확인) 동사로 직접 확인할 수 있습니다.

분류기가 본문을 읽는 시점은 **마스킹 이전**입니다. 취약점 보고는 취약점의 징후가 되는 문자열로 인식되는데, 마스킹이 그 문자열을 지워 버리면 사후 분류는 정확히 "공개되면 안 되는 경로"에서만 조용히 실패합니다. 그래서 순서가 정해져 있습니다.

이 계약은 스킬 본문이 따르는 규약이지 샌드박스가 아닙니다. 손으로 GitHub에서 이슈를 열면 세 가지(마스킹·분류·확인 질문) 모두 우회됩니다 — 공개 저장소에 직접 쓸 때는 같은 주의가 사용자 몫입니다.
