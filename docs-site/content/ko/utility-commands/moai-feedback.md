...
`/moai feedback`은 별도 플래그를 받지 않습니다. 피드백 유형은 자유 입력한 내용에서 자동으로 판별되며, 제목과 설명은 오케스트레이터의 단일 `AskUserQuestion` 라운드로 수집됩니다. 자연어로 문제나 제안을 설명하기만 하면 됩니다.
...
| 수집 항목 | 설명 | 예시 | 수집 방식 |
|-----------|------|------|-----------|
| MoAI-ADK 버전 | 현재 설치된 버전 (`moai version`) | v3.0.2 | 보장 (항상 수집) |
...
작성된 피드백 내용은 `gh` 실패로 유실되지 않으며, 로컬 임시 파일이 복구 수단이 됩니다.
...
단순한 단일 절차 작업에 서브에이전트를 띄우지 않는 것도 비용을 줄이는 원칙입니다. 위임은 필요할 때만, 가장 싼 경로로 합니다.
