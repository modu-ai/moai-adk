...
## 또 다른 표면 — `moai clean --codex-skills` (유령 스킬 등록 제거)

{{< callout type="warning" >}}
이 스코프는 **`~/.codex/config.toml`을 수정합니다** — `moai clean`의 스코프 중 `~/.moai` 바깥에 쓰는 유일한 경우입니다. 기본은 dry-run이고, `--force`를 붙여도 쓰기 전에 파일을 먼저 백업합니다.
{{< /callout >}}

Codex는 등록된 스킬을 `~/.codex/config.toml`(또는 `$CODEX_HOME/config.toml`)의 `[[skills.config]]` 배열 테이블 항목으로 기록합니다. 그 항목이 가리키던 파일을 지워도 Codex는 항목을 지우지도, 불평하지도 않습니다 — 등록은 그대로 **유령**으로 남습니다. `moai doctor`는 그 개수를 세어 알려줄 뿐이고, 실제로 지우는 손이 이 스코프입니다.

```bash
# 기본은 dry-run — 무엇이 지워질지, 무엇이 왜 남는지 보여주기만 합니다
$ moai clean --codex-skills

# 실제 제거 — 먼저 백업하고 그 경로와 sha256을 보고합니다
$ moai clean --codex-skills --force
```

- **부재가 증명된 것만 지웁니다.** 비어 있지 않은 `path`를 선언했고, 그 경로가 절대 경로이거나 홈이 해석되는 home-relative이며, 해석된 경로에 대한 `stat`이 "파일이 없다"고 답할 때에만 제거 대상이 됩니다.
- **`enabled`는 판정 기준이 아닙니다.** `enabled = true`인데 경로가 사라진 항목도 유령입니다. 꺼 두었다고 보호되지 않고, 켜 두었다고 지워지지도 않습니다.
- **제거된 항목 바깥은 한 바이트도 바뀌지 않습니다.** 다른 테이블, 주석, 여러 줄 리터럴, 공백, 줄 끝 형식이 그대로 남습니다 — 파일 전체를 다시 직렬화하지 않습니다.
- `--home`과 `--codex-skills`는 대상 파일이 서로 달라 함께 쓸 수 없습니다.

### 절대 지우지 않는 일곱 부류

| 부류 | 남는 이유 |
|---|---|
| 상대 경로(`skills/x/SKILL.md`) | 무엇을 기준으로 풀지 정한 것이 없어 부재를 판정할 수 없습니다 |
| 이상한 모양의 경로(`~other/x`) | 해석되지 않습니다 |
| 홈이 해석되지 않는 home-relative 경로 | 존재 여부를 판정할 수 없습니다 |
| 끝까지 가지 못한 `stat` — 권한 거부, 심링크 루프, I/O 오류 | 부재가 아니라 판정 불가입니다 |
| `path` 키가 없거나 `path = ""` | 확인할 대상 자체가 없습니다 |
| 해석에 성공한 경로 — **디렉터리도 해석에 성공한 것입니다** | 부재가 아닙니다 |
| 줄 범위 안에 파서가 인식하지 못한 줄이 있는 항목 | 그 범위가 여러 줄 리터럴에 삼켜진 멀쩡한 등록을 품고 있을 수 있습니다 |

남긴 항목은 사유와 함께 전부 보고합니다. 지운 것만 알려주면 유령이 다 사라졌다고 믿게 되기 때문입니다.
