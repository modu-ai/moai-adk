...
원래의 구조적 제약은 **서브에이전트는 다른 서브에이전트를 spawn할 수 없다**(subagents cannot spawn other subagents)였습니다 — 위임은 메인 대화에서 한 단계만 내려갔습니다. 이후 기본값이 바뀌었으며(아래 참조), 평평한 계층은 이제 런타임 보장이 아니라 구성 선택입니다.

### v2.1.219 이후: 중첩 기본 활성화 (깊이 3)

중첩은 v2.1.172에 도입되었고, v2.1.217–2.1.218에서 잠시 기본 비활성이었다가, v2.1.219부터 **기본 활성화**되었습니다 — 체인지로그에 따르면 서브에이전트는 기본적으로 깊이 3까지 중첩 spawn할 수 있습니다. 중첩을 끄려면 `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=1`을 설정하세요.
...
flowchart TD
    ...
    M --> B[서브에이전트 B<br/>검증]
    M --> C[서브에이전트 C<br/>구현]
    A -.->|조건: Agent 도구 있으면<br/>기본 깊이3까지| X["중첩 서브에이전트<br/>(제한적)"]
