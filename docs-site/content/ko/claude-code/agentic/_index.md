...
서브에이전트·에이전트 팀·다이내믹 워크플로우라는 세 가지 오케스트레이션 원시를 중심으로, 워크트리 분리·목표 지향 실행·예약 작업·대규모 코드베이스 탐색·모범 사례까지 차례로 이어집니다. MoAI-ADK가 말하는 **에이전틱 루프 엔지니어링**(사람이 매 턴 개입하는 대신 루프 자체를 설계하고, 루프가 남긴 관찰로 하네스를 학습시키는 것)이 딛고 서는 것도 정확히 이 그룹의 메커니즘입니다. `/goal`의 조건 평가 루프, 서브에이전트 위임, 워크플로우 팬아웃이 바로 그것입니다.
