...
MoAI-ADK는 이 네이티브 팀 런타임 위에 **CG 모드** (`moai cg`, Claude + GLM 하이브리드)를 얹어 비용을 최적화합니다. 리더는 Claude로 전략·계획·감사를 조율하고, 팀원은 tmux 세션 단위 환경 격리를 통해 GLM 환경을 상속받아 대량 구현 작업을 수행하는 방식입니다. "누가 무엇을 하는가"라는 팀 구조 질문에 "어떤 모델이 얼마짜리 일을 하는가"라는 토크노믹스 답을 결합한 것으로, 구현 중심 SPEC·코드 생성·테스트 작성처럼 토큰 소비가 큰 작업에서 **60-70% 비용 절감**을 얻습니다.
