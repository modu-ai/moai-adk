...
**"에이전트 팀" 두 가지를 구분하세요.** 이 페이지가 다루는 것은 **Claude Code 네이티브 Agent Teams 런타임**(tmux/iTerm2 페인, 공유 TaskList, `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`)으로, 유효하며 `moai cg` 하이브리드 모드가 그대로 활용합니다. 반면 MoAI의 **정적 agent-team 오케스트레이션 계층**(구 `workflow.team.*` 설정, `--team` 강제 플래그)은 **은퇴**했습니다 — `--team` 강제 시 `MODE_TEAM_UNAVAILABLE` 과 함께 서브에이전트 모드로 폴백합니다. 즉 네이티브 런타임은 살아 있고, MoAI 자체 워크플로우 오케스트레이션의 정적 팀 계층만 사라진 것입니다.
{{< /callout >}}
...
한 가지 구분해 둘 것이 있습니다. MoAI-ADK는 자체 워크플로우 오케스트레이션에서는 정적 에이전트 팀 계층을 은퇴시키고 순차 서브에이전트와 동적 워크플로우를 기본으로 사용하지만, 이 페이지에서 다룬 Claude Code의 네이티브 팀 런타임 (tmux 페인, 공유 작업 목록)은 CG 모드가 그대로 활용합니다. 즉 "팀"이라는 실행 형태는 살아 있고, 그 용도가 협업 조율에서 비용 라우팅으로 옮겨간 셈입니다.
