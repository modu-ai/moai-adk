...
bypass 모드는 권한 확인을 건너뛰므로 믿을 수 있는 격리 환경에서만 쓰는 편이 안전합니다. MoAI-ADK도 워크플로 단계에 맞춰 이 모드들을 갈아 씁니다. 특히 plan 모드는 "계획은 깊게 검토하고 구현은 승인을 받은 뒤에 시작한다"는 MoAI-ADK의 구현 착수 승인 게이트(plan→run 휴먼 게이트)와 발상이 정확히 같습니다. 값비싼 구현 턴에 들어가기 전에 값싼 읽기 전용 턴으로 방향을 못 박아 두는 것이 토큰 관점에서도 가장 경제적입니다.
