...
셸 모드는 명령과 그 출력을 대화 맥락에 추가하므로, 빠른 셸 작업을 하면서도 Claude가 결과를 볼 수 있습니다. 긴 명령은 `Ctrl+B`로 백그라운드로 보낼 수 있고, 입력 칸이 비어 있을 때 `Escape`나 `Backspace`로 셸 모드를 빠져나옵니다.
...
스크린샷이나 디자인 시안을 `Ctrl+V`로 붙여넣으면 커서 자리에 `[Image #N]` 칩이 들어갑니다. 칩을 프롬프트 안에서 위치 기준으로 참조할 수 있어 텍스트와 이미지를 섞어 설명할 수 있습니다.
...
bypass 모드는 권한 확인을 건너뛰므로 신뢰할 수 있는 격리 환경에서만 쓰는 것이 안전합니다. MoAI-ADK도 워크플로 단계에 맞춰 이 모드들을 활용합니다. 특히 plan 모드는 MoAI-ADK의 "계획은 깊게 검토하고, 구현은 승인 후에 시작한다"는 구현 착수 승인 게이트(plan→run 휴먼 게이트)와 정확히 같은 철학 위에 있습니다. 값비싼 구현 턴에 들어가기 전에 저렴한 읽기 전용 턴으로 방향을 확정하는 것이 토큰 관점에서도 가장 경제적입니다.
