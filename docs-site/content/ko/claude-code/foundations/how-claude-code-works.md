...
{{< callout type="info" >}}
**한 줄 요약**: Claude Code는 추론하는 모델과 행동하는 도구를 묶어 "맥락 수집 → 행동 → 결과 검증"을 스스로 반복하는 터미널 네이티브 코딩 에이전트이며, MoAI-ADK의 세 기둥이 모두 이 루프 위에 서 있습니다.
{{< /callout >}}
...
에이전틱 루프는 **추론하는 모델** (model)과 **행동하는 도구** (tools)라는 두 축으로 돌아갑니다. 여기에 대화와 파일이 담기는 컨텍스트, 그리고 행동을 통제하는 권한이 더해집니다.
...
## MoAI-ADK와의 연결 — 루프 위에 세워지는 세 기둥

이 페이지에서 본 에이전틱 루프는 MoAI-ADK 세 기둥의 공통 토대입니다.
