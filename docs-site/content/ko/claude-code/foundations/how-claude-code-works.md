...
{{< callout type="info" >}}
**한 줄 요약**: Claude Code는 추론하는 모델과 행동하는 도구를 묶어 "맥락 수집 → 행동 → 결과 검증"을 스스로 반복하는 터미널 네이티브 코딩 에이전트이며, MoAI-ADK의 세 가지 핵심이 모두 이 루프 위에 서 있습니다.
{{< /callout >}}
...
핵심 개념은 **에이전틱 하네스** (agentic harness)입니다. 하네스란 기본 모델을 둘러싸고 실행을 조율하는 시스템으로, 모델이 어떻게 생각하고 계획할지, 어떤 도구를 부를지, 컨텍스트를 어떻게 관리할지, 산출물을 어디에 저장하고 결과를 어떻게 평가할지를 결정하는 계층입니다. Claude Code는 모델인 Claude를 감싸서 도구, 컨텍스트 관리, 실행 환경을 제공함으로써, 텍스트만 생성하던 언어 모델을 실제로 코드베이스를 다루는 유능한 코딩 에이전트로 바꿉니다. MoAI-ADK가 말하는 "코드를 직접 쓰는 대신 에이전트가 잘 일할 환경을 설계한다"는 하네스 엔지니어링이 바로 이 개념의 연장선입니다.
...
컨텍스트 윈도우가 가득 찬 경우, Claude는 자동으로 컨텍스트를 압축합니다. 압축 시에는 먼저 초반의 도구 호출 결과물들을 정리하고, 그다음 남은 정보를 요약합니다. 또한 MCP 도구 정의는 명시적으로 요청할 때까지 지연되어, 필요한 도구만 그 시점에 로드됩니다.
...
## MoAI-ADK와의 연결 — 루프 위에 세워지는 세 가지 핵심

이 페이지에서 본 에이전틱 루프는 MoAI-ADK 세 가지 핵심의 공통 토대입니다.
