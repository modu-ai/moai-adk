...
핵심 개념은 **에이전틱 하네스** (agentic harness)입니다. 하네스란 기본 모델을 둘러싸고 실행을 조율하는 시스템 — 모델이 어떻게 생각하고 계획할지, 어떤 도구를 부를지, 컨텍스트를 어떻게 관리할지, 산출물을 어디에 저장하고 결과를 어떻게 평가할지를 결정하는 계층입니다. Claude Code는 모델인 Claude를 감싸서 도구, 컨텍스트 관리, 실행 환경을 제공함으로써, 텍스트만 생성하던 언어 모델을 실제로 코드베이스를 다루는 유능한 코딩 에이전트로 바꿉니다. MoAI-ADK가 말하는 "코드를 직접 쓰는 대신 에이전트가 잘 일할 환경을 설계한다"는 하네스 엔지니어링이 바로 이 개념의 연장선입니다.
...
컨텍스트 윈도우가 가득 찬 경우, Claude는 자동으로 컨텍스트를 압축합니다. 압축 시에는 먼저 초반의 도구 호출 결과물들을 정리하고, 그 다음 남은 정보를 요약하는 순서로 진행됩니다. 또한 MCP 도구 정의는 명시적으로 요청할 때까지 지연되어, 필요한 도구만 필요한 시점에 로드됩니다.
