...
슬래시 명령은 세 가지 종류로 구성됩니다. 자주 쓰는 명령을 정리하면 다음과 같습니다. 전체 목록은 입력창에서 `/`를 입력하면 확인할 수 있고, 공식 명령 레퍼런스는 [code.claude.com/docs/en/commands](https://code.claude.com/docs/en/commands) 를 참고합니다.
...
마크다운 파일 상단의 YAML frontmatter로 동작을 조정합니다. 모든 필드는 선택이며, 모델이 자동으로 호출할 시점을 판단하는 데 쓰이므로 `description`은 특히 권장됩니다.
