...
핵심 규칙은 단 하나입니다. **명령은 메시지의 맨 앞에서만** 인식됩니다. 명령 이름 뒤에 따라오는 텍스트는 그 명령의 인자(argument)로 넘어갑니다.
...
슬래시 명령은 세 가지 종류로 구성됩니다. 자주 쓰는 명령을 정리하면 다음과 같습니다. 전체 목록은 입력창에서 `/`를 입력하면 확인할 수 있고, 공식 명령 레퍼런스는 [code.claude.com/docs/en/commands](https://code.claude.com/docs/en/commands)를 참고하세요.

### 내장 명령 (Built-in)

| 명령 | 용도 | 버전 |
| :--- | :--- | :--- |
| `/goal <condition>` | 완료 조건을 세우고 여러 턴에 걸쳐 자율 진행 (Haiku가 주기적으로 확인) | v2.1.139+ |
...
| 명령 | 용도 |
| :--- | :--- |
| `/loop` (별칭: `/proactive`) | 반복 루프 실행 (Ralph/interval 기반) |
| `/batch` | 배치 실행 |
