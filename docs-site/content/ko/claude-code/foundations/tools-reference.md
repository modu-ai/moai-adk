...
`TodoWrite`는 v2.1.142 이후 기본 비활성화되었고, 그 자리를 `TaskCreate`/`TaskUpdate`/`TaskList`/`TaskGet` 계열 도구가 대신합니다.
...
- `Read`는 절대 경로만 받도록 안내되어 있고, 토큰 한도를 넘는 큰 파일은 `offset`·`limit`으로 나눠 읽습니다.
...
Claude는 대체로 알아서 알맞은 도구를 고르지만, 같은 목적에 더 정확하고 효율적인 길이 따로 있습니다. 다음 흐름은 검색 작업에서 권장되는 우선순위입니다.
...
- **이름으로 파일 찾기**에는 `Glob`을, **내용으로 줄 찾기**에는 `Grep`을 씁니다. 두 도구는 전용 인덱싱과 안전한 출력 형식을 갖췄습니다.
- **`Bash`로 `grep`·`find`·`cat`을 대신 호출하지 마세요.** Bash는 권한 확인을 거치는 데다, 출력이 길어질수록 컨텍스트를 압박하고, 전용 도구가 붙여 주는 정렬·잘림·줄 번호 같은 구조도 사라집니다.
