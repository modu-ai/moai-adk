...
Claude Code의 기능은 크게 두 갈래입니다. 하나는 모델이 코드를 다룰 때 늘 쓰는 **내장 도구** (built-in tools)이고, 다른 하나는 필요에 따라 사용자가 덧붙이는 **확장 레이어** (extension layer)입니다. 이 페이지는 두 갈래를 모두 펼쳐 놓고, 기능마다 한 줄 설명과 함께 더 깊은 문서로 가는 길을 알려 줍니다.

MoAI-ADK는 바로 이 Claude Code 위에서 동작하는 에이전틱 개발 키트입니다. 여기 나열된 기능은 MoAI-ADK 세 가지 핵심의 원재료이기도 합니다. 컨텍스트 윈도우와 캐싱은 **토크노믹스**의, 서브에이전트·팀·워크트리는 **에이전틱 루프 엔지니어링**의, 스킬·훅·MCP·플러그인은 **에이전틱 하네스**의 재료입니다. 여기서 개념을 잡아 두면 MoAI-ADK가 이것들을 어떻게 오케스트레이션하는지 훨씬 빠르게 이해할 수 있습니다.
...
- **서브에이전트**: 자기만의 컨텍스트 윈도우에서 작업을 끝낸 뒤 요약만 메인 대화로 돌려줍니다. 파일 수십 개를 읽는 조사 작업처럼 중간 산출물로 메인 컨텍스트를 어지럽히면 안 될 때 요긴합니다.
