...
스킬은 호출형 동작 (`/deploy`)일 수도 있고 참조 지식 (API 스타일 가이드)일 수도 있습니다. 컨텍스트 윈도우가 차오르거나 중간 작업을 보일 필요가 없을 때는 서브에이전트가 적합합니다. 둘은 결합할 수도 있어서 서브에이전트가 특정 스킬을 미리 로드하거나 스킬이 격리 컨텍스트에서 실행될 수도 있습니다.
