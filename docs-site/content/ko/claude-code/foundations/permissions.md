...
자주 반복되는 안전한 읽기 전용 명령을 `allow`에 미리 등록해 두면 프롬프트가 눈에 띄게 줄어듭니다. {{< icon check ok >}} 반대로 민감한 파일이나 위험한 명령은 `deny`로 확실히 막아 둡니다.

## 권한 모드

세션 전체의 기본 태도는 권한 모드로 정합니다. 네 가지 모드가 있으며, 대화형 세션에서는 `Shift+Tab`으로 돌려 가며 바꿉니다.
