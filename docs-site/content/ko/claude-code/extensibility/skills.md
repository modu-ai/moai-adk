...
스킬은 MoAI-ADK 관점에서 두 가지 핵심에 동시에 걸쳐 있습니다. **토크노믹스** 측면에서 점진적 공개는 곧 토큰 예산 설계입니다. 설명 한 줄 (~100 토큰)만 상시 부담하고 본문 (~5K 토큰)은 사용 시점에만 지불하므로, 지식을 CLAUDE.md에 상주시키는 것보다 훨씬 경제적입니다. **에이전틱 루프 엔지니어링** 측면에서 스킬은 하네스 진화의 편집 대상입니다. 루프가 축적한 관찰을 바탕으로 하네스가 스킬 지침을 업그레이드하는 것이 MoAI-ADK 자가 진화의 핵심 경로입니다. 작성 규칙·네임스페이스·점진적 공개 토큰 예산 같은 실전 세부는 아래 MoAI-ADK 심화 문서를 참고하시기 바랍니다.
