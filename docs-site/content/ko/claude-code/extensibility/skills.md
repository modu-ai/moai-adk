...
CLAUDE.md 내용은 항상 컨텍스트에 상주하지만, 스킬 본문은 실제로 사용될 때만 로드됩니다. 따라서 길고 상세한 참조 자료를 두어도 필요하기 전까지는 토큰 비용이 거의 들지 않습니다. 참고로 사용자 정의 명령(`.claude/commands/`)은 스킬에 통합되었으며, 기존 명령 파일도 그대로 동작합니다.
...
프론트매터는 모두 선택 항목이지만, Claude가 언제 이 스킬을 써야 할지 판단하는 `description`은 사실상 필수입니다.
