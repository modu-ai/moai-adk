...
MoAI-ADK는 `mcp__context7` 같은 문서 조회 MCP를 워크플로우에 통합해 사용하며, `/moai project`의 하네스 자동 구성 단계는 프로젝트 도메인에 맞는 MCP 프로비저닝까지 포함합니다.

토큰 관점에서 눈여겨볼 것은 위의 도구 검색 (Tool Search) 지연 로드입니다. MCP 도구 정의는 컨텍스트에서 꽤 큰 덩어리인데, 필요할 때만 로드되므로 서버를 여러 개 연결해도 항시 부담이 거의 없습니다 — MoAI-ADK의 컨텍스트 다이어트가 기대는 플랫폼 메커니즘 중 하나입니다. 서버 등록 절차, 인증 패턴, 스코프 선택, 그리고 MoAI 에이전트가 MCP 도구를 어떻게 호출하는지 등 실전 내용은 별도 심화 가이드에 정리되어 있습니다. 이 페이지에서 개념을 잡았다면 다음 단계로 그 가이드를 참고하시기 바랍니다.
