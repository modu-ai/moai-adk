...
같은 `hooks` 블록이라도 위치에 따라 적용 범위가 달라집니다. `~/.claude/settings.json`에 두면 모든 프로젝트에, 프로젝트의 `.claude/settings.json`에 두면 해당 프로젝트에만 걸립니다. 플러그인과 스킬의 프런트매터에도 훅을 선언할 수 있어, 배포 단위로 묶어서 가져갈 수도 있습니다.
{{< /callout >}}
