...
Claude Code 플러그인은 흩어져 있던 확장 기능을 하나의 패키지로 묶어 팀과 커뮤니티에 배포하는 단위이며, 마켓플레이스는 그 패키지를 발견하고 설치하는 카탈로그입니다. 하네스 관점에서 보면 앞의 세 문서에서 익힌 스킬·훅·MCP라는 재료를 "하나의 설치 가능한 하네스 조각"으로 포장하는 배포 계층입니다.
...
플러그인(plugin)은 Claude Code의 여러 확장 요소를 한 디렉터리에 묶어 **공유·재사용·버전 관리**할 수 있게 만든 패키지입니다. `.claude/` 디렉터리에 직접 두는 단독 설정과 달리, 플러그인은 매니페스트 파일로 자기 정체를 밝히고, 마켓플레이스를 타고 다른 프로젝트와 팀으로 퍼집니다.
...
플러그인 루트(`.claude-plugin/plugin.json`이 아닌 플러그인 디렉터리 자체)에 요소별 디렉터리를 둡니다. **주의:** `.claude-plugin/` 안에는 `plugin.json` **만** 들어가고, 스킬·명령·에이전트·hook 등 모든 구성 요소는 플러그인 루트에 위치합니다.
...
이처럼 플러그인 하나가 스킬·hook·MCP를 동시에 담을 수 있어, "이 작업에 필요한 모든 확장"을 한 번의 설치로 전달합니다. 예를 들어 `commit-commands` 플러그인은 commit·push·PR 생성 스킬을 묶어 제공하고, `pr-review-toolkit`은 PR 리뷰 전용 에이전트를 함께 배포합니다.

## 마켓플레이스: 발견·설치·관리

마켓플레이스(marketplace)는 누군가 만든 플러그인 목록을 담은 카탈로그입니다. 쓰는 순서는 두 단계입니다. 먼저 카탈로그를 **추가**해 둘러볼 수 있게 한 뒤, 원하는 플러그인을 **하나씩 설치**합니다. 앱 스토어를 등록하는 것과 개별 앱을 내려받는 것을 분리해 생각하면 됩니다.
...
`/plugin`을 실행하면 **Discover / Installed / Marketplaces / Errors** 네 개 탭짜리 플러그인 관리자가 열립니다. Discover 탭의 상세 패널에서는 설치 전에 컨텍스트 비용(Context cost) 추정치, 최종 업데이트 날짜, 함께 깔릴 명령·에이전트·스킬·hook·MCP·LSP 목록을 미리 볼 수 있습니다.
...
플러그인과 마켓플레이스는 **높은 신뢰가 필요한 구성 요소**입니다. 사용자 권한으로 임의 코드를 실행할 수 있기 때문입니다. 신뢰하는 출처에서만 설치하세요.
...
MoAI-ADK 자체는 플러그인이 아닙니다. `moai init`이 `.claude/` 디렉터리에 하네스 자산(스킬·에이전트·hook·설정)을 직접 풀어 놓는 방식입니다. 그래도 이 페이지의 두 가지는 MoAI-ADK 사용자에게도 곧바로 와닿습니다. 하나는 Discover 탭의 **컨텍스트 비용** (Context cost) 추정치입니다. 토크노믹스 감각을 그대로 옮겨 놓은 지표이니, 확장 하나를 깔 때마다 항시 컨텍스트가 얼마나 늘어나는지 보고 판단하세요. 다른 하나는 코드 인텔리전스(LSP) 플러그인입니다. MoAI-ADK의 언어별 품질 게이트가 쓰는 진단 신호와 같은 계열이라, 쓰는 언어의 LSP 플러그인을 깔아 두면 편집 직후 타입 오류를 같은 턴에 잡아내는 루프가 훨씬 촘촘해집니다.
...
{{< callout type="tip" >}}
설치하려는 플러그인이 보이지 않으면 마켓플레이스가 오래된 것일 수 있습니다. `/plugin marketplace update <marketplace-name>`으로 목록을 새로 고친 뒤 다시 설치해 보세요.
{{< /callout >}}
