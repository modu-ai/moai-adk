...
플러그인의 핵심은 `.claude-plugin/plugin.json` **매니페스트** 입니다. 이 파일이 플러그인의 이름·설명·버전을 정의하며, `name` 필드는 곧 스킬의 네임스페이스 접두사가 됩니다.
...
플러그인 루트에는 요소별 디렉터리를 둡니다. 한 가지 흔한 실수는 이 디렉터리들을 `.claude-plugin/` 안에 넣는 것인데, `.claude-plugin/` 안에는 `plugin.json` 만 들어가고 나머지는 모두 **플러그인 루트** 에 위치해야 합니다.
