...
MoAI-ADK 자체는 플러그인이 아니라 `moai init`이 `.claude/` 디렉터리에 하네스 자산 (스킬·에이전트·hook·설정)을 직접 배포하는 방식을 사용합니다. 그렇지만 이 페이지의 두 가지는 MoAI-ADK 사용자에게도 직접적입니다. 첫째, Discover 탭의 **컨텍스트 비용** (Context cost) 추정치는 토크노믹스 감각을 그대로 반영한 지표입니다 — 확장 하나를 설치할 때마다 항시 컨텍스트가 얼마나 늘어나는지 보고 판단하세요. 둘째, 코드 인텔리전스 (LSP) 플러그인은 MoAI-ADK의 언어별 품질 게이트가 활용하는 진단 신호와 같은 계열이므로, 사용하는 언어의 LSP 플러그인을 설치해 두면 편집 직후 타입 오류를 같은 턴에 잡는 루프가 훨씬 촘촘해집니다.
