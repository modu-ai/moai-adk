...
플러그인 (plugin) 은 Claude Code의 여러 확장 요소를 하나의 디렉터리에 묶어 **공유·재사용·버전 관리** 할 수 있는 패키지입니다. `.claude/` 디렉터리에 직접 두는 단독 설정과 달리, 플러그인은 매니페스트 파일로 정체성을 가지며 마켓플레이스를 통해 다른 프로젝트와 팀에 배포됩니다.
...
`/plugin` 을 실행하면 **Discover / Installed / Marketplaces / Errors** 4개 탭이 있는 플러그인 관리자가 열립니다. Discover 탭의 상세 패널에서는 설치 전에 컨텍스트 비용 (Context cost) 추정치, 최종 업데이트 날짜, 설치될 명령·에이전트·스킬·hook·MCP·LSP 목록을 미리 확인할 수 있습니다.
...
MoAI-ADK 자체는 플러그인이 아니라 `moai init`이 `.claude/` 디렉터리에 하네스 자산 (스킬·에이전트·hook·설정)을 직접 배포하는 방식을 사용합니다. 그렇지만 이 페이지의 두 가지는 MoAI-ADK 사용자에게도 직접 닿습니다. 하나는 Discover 탭의 **컨텍스트 비용** (Context cost) 추정치로, 토크노믹스 감각을 그대로 반영한 지표입니다. 확장 하나를 설치할 때마다 항시 컨텍스트가 얼마나 늘어나는지 보고 판단하세요. 다른 하나는 코드 인텔리전스 (LSP) 플러그인인데, MoAI-ADK의 언어별 품질 게이트가 활용하는 진단 신호와 같은 계열이라 사용하는 언어의 LSP 플러그인을 설치해 두면 편집 직후 타입 오류를 같은 턴에 잡는 루프가 훨씬 촘촘해집니다.
