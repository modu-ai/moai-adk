...
MoAI-ADK는 이를 **세션 핸드오프** (session handoff)로 제공합니다. 컨텍스트 사용량이 모델별 임계값에 다가가면 오케스트레이터가 진행 상태를 디스크에 저장하고, 다음 세션에서 그대로 붙여넣어 이어갈 수 있는 붙여넣기용 재개 메시지 (paste-ready resume message)를 만들어 줍니다. `/clear` 이후 이 메시지 하나로 새 세션이 직전 작업을 자급자족하게 이어받습니다.
