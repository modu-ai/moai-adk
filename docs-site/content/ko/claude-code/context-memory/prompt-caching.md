...
프롬프트 캐싱은 책에 꽂아 둔 **책갈피**와 같습니다. 매 요청은 같은 앞부분(시스템 프롬프트·프로젝트 컨텍스트·이전 대화)을 다시 담아 보내는데, 앞부분이 직전과 동일하면 처음부터 다시 읽지 않고 책갈피가 꽂힌 지점까지 건너뜁니다. 그래서 앞부분이 오래 그대로일수록 캐시 효과가 크고, 앞부분이 바뀌면 그 지점부터는 다시 읽어야 합니다.
{{< /callout >}}

## 프롬프트 캐싱이 필요한 이유

모델은 요청과 요청 사이에 아무것도 기억하지 못합니다. 그래서 Claude Code는 메시지를 보낼 때마다 새 API 요청을 만들고, **전체 컨텍스트** (시스템 프롬프트, 프로젝트 컨텍스트, 이전 메시지와 도구 결과 전부, 새 메시지)를 다시 보냅니다.

핵심은 새 내용이 늘 **맨 끝에 덧붙는다**는 점입니다. 따라서 각 요청의 대부분은 직전 요청과 동일합니다. 프롬프트 캐싱은 바로 이 "변하지 않은 부분"을 다시 처리하지 않게 해주는 메커니즘입니다.

## 캐시는 어떻게 동작하나

API는 요청의 **시작 부분**을 최근에 처리한 내용과 맞춰 봅니다. 이 시작 부분을 **프리픽스** (prefix)라고 부릅니다. 일반적인 턴에서는 직전 요청 전체가 프리픽스가 되고, 가장 최근에 주고받은 한 번의 교환만 새 내용입니다.
...
대화 계층만 바뀌면 시스템 프롬프트와 프로젝트 컨텍스트는 캐시된 채로 남습니다. 반대로 시스템 프롬프트가 바뀌면 그 뒤 내용이 전부 다른 프리픽스 뒤에 놓이므로 **전체가 무효화**됩니다.

프롬프트 텍스트에 포함되지 않지만 캐시 키의 일부인 두 가지가 더 있습니다.

- **모델**: 모델마다 캐시가 분리됩니다. `/model`로 모델을 바꾸면 내용이 같아도 전체를 다시 계산합니다.
- **노력 수준** (effort level): 같은 모델이라도 노력 수준마다 캐시가 따로입니다. 세션 중간에 `/effort`로 바꾸면 전체를 다시 계산하며, Claude Code가 적용 전에 확인을 요청합니다.
...
- `/compact`는 작업과 작업 사이의 자연스러운 길목에서 실행합니다.
- 잘못된 길로 들어섰다면 `/compact` 대신 `/rewind`로 이미 캐시된 이전 턴까지 되돌립니다.

캐시 범위는 사실상 **한 머신·한 디렉터리 단위**로 잡힙니다. 시스템 프롬프트가 작업 디렉터리, 플랫폼, 셸, OS 버전, 자동 메모리 경로를 담기 때문입니다. 같은 저장소의 워크트리도 디렉터리가 다르므로 서로의 캐시를 공유하지 않습니다.
