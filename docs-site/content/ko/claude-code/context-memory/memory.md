...
파일 기반 영속 메모리는 MoAI-ADK **재귀적 자가 학습** 기둥의 원료이기도 합니다. 루프가 돌며 남긴 관찰 — 사용자 교정, 실패 패턴, 라우팅 결정 — 이 메모리 파일에 쌓이고, 하네스가 그 축적을 바탕으로 스킬과 에이전트 지침을 개선합니다. "루프가 관찰을 축적하고, 하네스가 학습하여 지침이 진화한다"는 문장의 첫 번째 고리가 바로 이 페이지의 메모리 메커니즘입니다. MoAI 고유의 메모리 운영 규칙과 인덱스 관리 방식은 별도 문서에서 자세히 다룹니다.
