...
weight: 20
draft: false
description: "Claude Code가 CLAUDE.md와 자동 메모리로 세션을 넘어 프로젝트 지식을 기억하는 방법을 입문자 눈높이로 정리합니다."
---

Claude Code는 세션을 닫으면 그동안 나눈 대화를 전부 잊어버립니다. 그런데도 다음에 다시 켤 때 매번 프로젝트를 처음부터 설명하지 않아도 되는 까닭은, **사람이 적어둔 지침**과 **Claude가 스스로 모은 노트**를 파일로 보관해두기 때문입니다. 이 페이지는 그 두 가지 메모리 메커니즘이 어떻게 세션을 넘어 지식을 이어주는지를 다룹니다.
...
{{< callout type="info" >}}
**한 줄 요약**: CLAUDE.md는 사람이 미리 적어두는 영구 지침이고, 자동 메모리(auto memory)는 Claude가 일하면서 스스로 채워가는 학습 노트입니다. 둘 다 매 세션 시작에 컨텍스트로 로드되어, 매번 같은 것을 다시 가르쳐야 하는 수고를 덜어줍니다.
{{< /callout >}}

{{< callout type="info" title="비유로 이해하기" >}}
CLAUDE.md는 새 팀원에게 건네주는 **프로젝트 수첩**입니다. "여기선 커밋 전에 꼭 테스트를 돌린다" 같은 규칙을 미리 적어두면, Claude는 첫날부터 그 규칙을 알고 시작합니다. 자동 메모리는 그 팀원이 일하면서 **스스로 끄적이는 작업 노트**입니다. "이 프로젝트에선 pnpm을 쓴다"는 사실을 한 번 배우면 다음날 다시 물어보지 않습니다. 수첩은 사람이 쓰고, 노트는 Claude가 씁니다. 둘 다 책상 위(컨텍스트)에 매일 아침 펼쳐둡니다.
{{< /callout >}}

## 두 가지 메모리 메커니즘

모든 세션은 빈 컨텍스트 윈도우(context window)로 시작합니다. 세션을 넘어 지식을 전달하는 방법은 크게 두 가지이고, 둘은 서로를 보완하며 매 대화 시작에 함께 로드됩니다.
...
두 메모리 모두 **강제 설정이 아니라 컨텍스트** (context, not enforced configuration)입니다. Claude는 이를 읽고 따르려 하지만, 무조건 준수를 보장하지는 않습니다. 특정 동작을 반드시 차단하려면 메모리가 아니라 `PreToolUse` hook을 사용해야 합니다.
...
매번 다시 설명하게 되는 사실을 적어두는 곳입니다. 다음 신호가 나타나면 추가할 타이밍입니다.

- {{< icon check-circle ok >}} Claude가 같은 실수를 두 번째로 반복할 때
- {{< icon check-circle ok >}} 코드 리뷰가 Claude가 알았어야 할 코드베이스 사항을 잡아낼 때
- {{< icon check-circle ok >}} 지난 세션에 입력했던 정정을 또 입력하고 있을 때
- {{< icon check-circle ok >}} 새 팀원에게 똑같이 설명해줘야 할 컨텍스트일 때
...
자동 메모리는 사람이 아무것도 적지 않아도 Claude가 세션을 넘어 지식을 쌓게 합니다. 작업하면서 빌드 명령, 디버깅 통찰, 아키텍처 노트, 코드 스타일 선호, 워크플로우 습관 등을 스스로 기록합니다. 매 세션 무조건 무언가를 저장하는 것은 아니고, 앞으로의 대화에 유용할지 판단해 기록할 가치가 있는 것만 남깁니다.
...
{{< callout type="warning" title="메모리는 세션 시작에 한 번 로드됩니다" >}}
CLAUDE.md와 `MEMORY.md`는 **세션이 시작될 때 한 번** 읽혀 컨텍스트에 들어갑니다. 그래서 세션 도중에 CLAUDE.md를 고쳐도 그 자리에서는 반영되지 않고, 다음 세션이나 다시 로드할 때 비로소 적용됩니다. 지금 세션에 무엇이 로드됐는지 확인하거나 자동 메모리를 켜고 끄려면 `/memory`를 실행하세요.
{{< /callout >}}
...
- {{< icon check ok >}} **간결하게**: `MEMORY.md`는 인덱스로 유지하고 상세는 주제 파일로 분리합니다. CLAUDE.md는 파일당 200줄 이하를 목표로 합니다.
- {{< icon check ok >}} **한 파일에 한 사실**: 한 주제는 한 파일에 모읍니다. `testing.md`, `api-design.md`처럼 서술적인 파일명을 씁니다.
- {{< icon check ok >}} **구체적으로**: 모호한 표현 대신 확인할 수 있는 문장을 씁니다 ("커밋 전 `npm test` 실행"처럼).
- {{< icon check ok >}} **모순 정리**: 서로 충돌하는 지침은 주기적으로 제거합니다. 충돌이 남으면 Claude가 어느 쪽을 따를지 임의로 결정합니다.
- {{< icon warning warn >}} **강제가 필요하면 hook으로**: 매 커밋 전처럼 특정 시점에 반드시 실행해야 하는 일은 메모리가 아니라 hook으로 작성합니다.
