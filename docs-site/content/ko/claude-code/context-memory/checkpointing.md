...
체크포인팅은 작업 중 Claude가 파일을 편집하기 직전의 상태를 자동으로 포착합니다. 그래서 규모가 큰 코드베이스를 건드리는 야심 찬 작업도, 언제든 직전 상태로 돌아갈 수 있다는 믿음 아래 과감하게 밀어붙일 수 있습니다.
...
체크포인트는 **세션 단위의 빠른 복구**를 위한 장치일 뿐, Git 같은 버전 관리 시스템을 대신하지 않습니다. 체크포인트는 "로컬 되돌리기", Git은 "영구 기록"으로 생각하면 역할 구분이 명확합니다.
...
복원(restore) 계열은 상태를 **되돌립니다**. 코드 변경이든 대화 기록이든, 아니면 둘 다를 취소합니다. 반면 요약(summarize) 계열은 디스크의 파일은 그대로 두고 대화의 일부만 AI가 만든 요약으로 **압축합니다**.
