...
| 크기 (일반론) | 의미 |
|---------------|------|
| 약 200K 토큰 | Haiku, 그리고 게이트웨이·구형 모델의 Sonnet/Opus 표준 윈도우. 일반적인 코드 작업에 충분 |
| 약 1M 토큰 | Anthropic API의 Sonnet 5·Opus 네이티브 윈도우(및 일부 다른 모델). 규모가 큰 코드베이스에 유리 |
...
긴 작업에 들어가기 전이나 도중에 `/context`를 한 번 실행해 어떤 항목이 컨텍스트를 차지하는지 확인하는 습관이 관리에 큰 도움이 됩니다.
