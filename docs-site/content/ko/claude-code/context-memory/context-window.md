...
| 약 1M 토큰 | Anthropic API의 Sonnet 5·Opus 네이티브 윈도우(및 일부 다른 모델). 대규모 코드베이스 (large codebase)에 유리 |
...
자동 압축의 시점을 조정해야 한다면, 환경 변수 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` 로 임계값 (기본값: 전체 컨텍스트 대비 약 75~80%)을 변경할 수 있습니다. 예를 들어 여유 있게 압축하고 싶다면 더 낮은 값을 지정하세요.
...
이 가운데 메모리와 체크포인트는 MoAI-ADK의 SPEC 워크플로 및 세션 핸드오프와 직접 맞물립니다. MoAI-ADK는 이 페이지의 원리를 **컨텍스트 다이어트** (context diet)라는 이름의 운영 규율로 확장합니다 — 항시 로드되는 지침을 최소화하고, 모델별 임계값 (1M 컨텍스트 모델은 사용량 50%, 200K 모델은 90%)에 도달하면 진행 상태를 디스크에 저장한 뒤 붙여넣기 한 번으로 다음 세션을 이어가는 세션 핸드오프를 제공하며, statusline에 컨텍스트 사용률 (CW%)을 상시 표시해 임계 접근을 미리 알립니다. 여기서는 "컨텍스트가 차기 전에 미리 비우고, 중요한 상태는 디스크에 남긴다"는 모범 사례 (best practices)만 기억하면 충분합니다.
