...
MoAI-ADK는 이 Claude Code 위에서 도는 오케스트레이션 계층입니다. 세 가지 핵심 가치(**토크노믹스** (Token Economics), **에이전틱 루프 엔지니어링** (Agentic Loop Engineering), **에이전틱 하네스** (Agentic Harness))는 모두 이 섹션에서 다루는 Claude Code의 기본 메커니즘에서 출발합니다. 컨텍스트 윈도우와 프롬프트 캐싱을 모르면 토크노믹스를 설계할 수 없고, 서브에이전트와 `/goal`을 모르면 에이전틱 루프를 이해할 수 없으며, 스킬·훅·MCP를 모르면 하네스를 만들 수 없습니다.

{{< callout type="info" >}}
**한 줄 요약**: 이 섹션은 도구(플랫폼)인 Claude Code 자체를 익히는 단계입니다. MoAI-ADK가 이 바탕 위에서 세 가지 핵심을 어떻게 쌓아 올리는지는 핵심 개념과 심화 학습 섹션에서 이어집니다.
{{< /callout >}}
