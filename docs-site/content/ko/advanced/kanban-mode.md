...
세션 런처에 `--kanban`(짧게 `-k`) 스위치를 붙여 시작합니다. 새 하위 명령도, 새 런타임도 아닙니다 — 세션 런처가 칸반 모드 환경을 무장하는 진입 계약일 뿐입니다. 체인의 세 단계(plan → run → sync — 검토 판정은 sync 게이트가 흡수)와 휴먼 게이트는 기존 `/moai goal` 엔진과 `full-pipeline` 체이닝 규칙을 그대로 상속합니다.
...
- **부트스트랩 안내** — 리드 세션이 열리면 SessionStart 훅이 런 식별자와 세 개의 동반 세션 실행 명령(`moai cc -k --name plan` 등)을 사용자 언어로 출력합니다. 동반 세션에 닿는 안내는 어느 런에 합류했는지와 세션 이름을 알립니다. 이름은 역할만으로 붙고(`plan`, `run`, `sync`), 같은 역할 이름이 이미 살아 있는 세션이 차지하면 다음 번호가 붙습니다(`plan-1`, `plan-2`, …).
...
배치의 근거는 레인마다 필요한 추론의 종류입니다. plan과 sync는 판단과 리뷰를 도는 칸이라 Claude에 두고, run은 구현 중심이라 GLM로 비용을 낮춥니다. 리더는 판정을 내리는 자리가 아니라 큐를 지키며 카드를 나르는 자리라, 상시 대기 비용이 크지 않은 GLM이 어울립니다. GLM 리더 아래에서 Claude 판정이 필요해지면 `judge`라는 이름의 세션으로 빠져나갑니다 — GLM 리더가 Claude를 쓰는 유일한 경로입니다. 한 계정이 429로 막히기 시작하면 레인을 계정에 분산해 배치하는 운영이 통합니다. 이 조합은 어디까지나 기본 추천일 뿐, 다른 조합이나 전 세션을 한 백엔드로 통일해도 무방합니다.

스크린샷의 상태 표시줄에 보이는 모델 라벨은 촬영 당시 한 운영자 세션의 구성을 반영한 것이지 배포 기본값은 아닙니다.

## 번호 붙은 워커 런 — 같은 -k, 워커 수를 붙이면

{{< callout type="info" >}}
{{< icon flash primary >}} **소속 가치**: 다중 세션 오케스트레이션 · 토크노믹스
{{< /callout >}}

칸반이 "역할 3개가 한 카드를 단계별로 나르는" 모양이라면, 같은 칸반 모드의 두 번째 형태인 **번호 붙은 워커 런**은 "번호 붙은 워커 N개가 카드 여러 장을 동시에 나르는" 모양입니다. 진입은 같은 `-k` 토큰에 **숫자를 붙이는 것**으로 끝납니다 — 새 플래그가 아니라 같은 스위치의 두 번째 해석입니다. (개명 전 `-f` 시절의 옛 이름은 "팩토리 모드"였습니다.)

```bash
# 리드 — 워커 4짜리 칸반 런 (worker-1..worker-4 실행 명령을 알려 줍니다)
$ moai cc -k 4

# 워커 — 각자 자기 터미널에서, 한 명령에 번호가 들어갑니다
...
리드가 하는 일이 역할 칸반의 리드와 다릅니다. 역할 칸반의 리드가 한 카드의 단계 사이를 조율한다면, 번호 붙은 워커 런의 리드는 **백로그 큐를 폴링해** 운영자가 고른 카드를 **빈 워커에 배분**합니다. 카드 클래스에 따라 배분 방식이 갈립니다 — A/B클래스(단순 변경·원인 미확립 결함)는 워커 하나에 통째로 맡기고, C클래스(설계 변경)는 `plan → run → sync` 직렬 3단계 경로로 보냅니다. 워커가 한창 일할 때는 새 카드를 굳이 깨우지 않고, 슬롯이 비는 순간에 맞춰 분배합니다(스태거 팬아웃 — 동시 기동 경쟁을 피하는 지연 분배).

워커 수를 생략하고 `-k --name worker-<i>` 형태만 쓰면 **기본 8 워커** 런으로 해석됩니다. 혼합 백엔드 런처(`moai cg`)는 칸반과 같은 이유로 거부됩니다(`FACTORY_MODE_UNSUPPORTED_BACKEND`).

{{< callout type="warning" >}}
{{< icon warning warn >}} **`-f`/`--factory`는 은퇴했습니다.** 과거 팩토리 전용 플래그였던 `-f`는 진입이 `-k <N>`으로 통일되며 은퇴했고, 지금은 명시적 에러를 냅니다. 조용히 무시되는 플래그가 아니라 "어디로 가야 하는지"를 알려 주는 에러입니다.
{{< /callout >}}
...
모든 카드가 세 작업 컬럼을 다 돌 필요는 없습니다. 백로그에 쌓이는 대부분은 단순 정리 작업이라, 그런 카드를 plan → run → sync 전 과정에 태우면 변경 가치보다 의례 비용이 더 큽니다. 리드는 카드가 `backlog`를 떠날 때 클래스를 분류해 디스패치에 함께 밝힙니다.

| 클래스 | 모양 | 지나가는 경로 |
|--------|------|---------------|
| A — 직접 마감 | 파일 1개 · 1줄짜리 변경으로 설계 판단이 없고 CI가 회귀를 잡음 | 한 세션이 PR까지 통째로; `plan` 건너뜀 |
| B — 결함, 원인 미확립 | 뭔가 잘못됐는데 원인이 아직 밝혀지지 않음 | `run → sync`; `plan`을 건너뛰어 SPEC 없음 |
| C — 설계 변경 | 결정이 포함되거나 서브시스템에 걸침 | 세 작업 컬럼 모두 |

Class A는 검증된 증거 위에서만 성립합니다 — diff를 베이스 대비 측정해 파일 1개임을 보이고, 머지될 head에서 CI가 초록인지 확인해야 클래스가 인정됩니다. "빠르니까"는 근거가 될 수 없습니다. 속도는 컬럼을 건너뛴 결과이지 이유가 아니어서, 속도만으로 정당화된 A 클래스는 서두른 C 클래스입니다.

Class B가 건너뛰는 것은 `plan`이지 sync 게이트의 리뷰가 아닙니다. 원인이 밝혀지지 않은 결함이야말로 리뷰가 잡아내는 것이므로 게이트는 끝까지 남습니다. 카드가 `run`을 떠나기 전에, 원인을 확립한 증거 — 무엇을 재현했고 무엇이 출력됐는지 — 를 카드의 진행 기록에 남깁니다.

한 컬럼에는 한 카드만 머물 수 있고, 카드들은 저마다 다른 worktree를 차지할 때만 실제로 병렬로 돕니다.
...
- [`/moai loop`](/ko/utility-commands/moai-loop) — bare `/loop`로 구동하는 무인 포어맨: 백로그 큐를 지켜보고 운영자가 고른 카드를 격리 워커에 배분·증거 수집을 한 세션에서 반복
