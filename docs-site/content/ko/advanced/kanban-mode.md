...
팩토리 리드가 하는 일이 칸반 리드와 다릅니다. 칸반 리드가 한 카드의 단계 사이를 조율한다면, 팩토리 리드는 **이미 선택된 카드를 빈 레인에 배분**합니다. 큐를 읽어 카드를 고르는 일은 팩토리 리드의 몫이 아닙니다 — 카드를 고르는 주체는 언제나 운영자(`moai todo next <n>`)이고, 팩토리는 그렇게 **선택된(picked) 카드만** 라우팅합니다. 칸반 포어맨 루프(단독 `/loop`)도 예외가 아니어서, 이미 `picked`로 표시된 다음 카드를 배차할 뿐 스스로 고르지는 않습니다. 배분의 단위는 언제나 카드 한 장 전부입니다 — 모든 카드는 한 레인에 통째로 가고, 그 레인이 세션 안에서 직렬 3단계 경로(`plan → run → sync`, 한 단계가 끝나야 다음 단계)를 끝까지 수행합니다. 각 단계는 레인이 `Agent()` 하위 에이전트로 스폰해 돌리며, 카드를 여러 레인에 쪼개 배분하는 일은 없습니다.
