...
- **`-f` / `--factory` 런처 스위치** — 팩토리 모드 전용 진입. `moai cc -f`는 팩토리 리드를 엽니다(워커 하나, `worker-1`). `moai cc -f worker`로 다음 빈 번호에 워커를 한 명씩 늘립니다(정확한 번호를 고르려면 `-f worker-<n>`). 레거시 `-f lane-<n>`도 지원 중단 예정 별칭으로 계속 동작합니다. 아래 "팩토리 모드" 절에서 다룹니다.
...
칸반이 "역할 3개가 한 카드를 단계별로 나르는" 모양이라면, **팩토리 모드**는 "번호 붙은 워커 N명이 카드 여러 장을 동시에 나르는" 모양입니다. 카드는 칸을 옮겨 다니지 않고 빈 워커 하나에 **통째로** 들어가, 그 워커가 세션 안에서 `plan → run → sync`를 순서대로 지나갑니다 — 각 단계는 `Agent()` 하위 에이전트로 실행됩니다. 진입은 전용 토큰 `-f`입니다. `moai cc -f`로 리드를 열고, `moai cc -f worker`(또는 `moai glm -f worker`)로 워커를 한 명씩 늘립니다. 레거시 `-f lane-<n>`도 지원 중단 예정 별칭으로 동작합니다.
