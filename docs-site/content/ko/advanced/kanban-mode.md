...
- **부트스트랩 안내** — 리드 세션이 열리면 SessionStart 훅이 런 식별자와 세 개의 동반 세션 실행 명령(`moai cc -k --name plan` 등)을 사용자 언어로 출력합니다. 동반 세션에 닿는 안내는 어느 런에 합류했는지만 알립니다. 합류한 역할은 안내에 없고 세션 레코드에 따로 기록됩니다.
