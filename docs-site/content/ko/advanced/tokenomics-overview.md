...
토크노믹스(Token Economics)는 MoAI-ADK v3.0의 세 가지 핵심 가운데 비용을 다루는 영역입니다. 토큰 단가가 낮아져도 에이전틱 개발은 토큰을 대량으로 소비하므로, 비용을 결정하는 것은 모델 가격이 아니라 토큰 운용 방식입니다. 이 페이지는 토크노믹스의 전체 구조를 개관하고, 각 하위 주제의 심화 페이지로 연결합니다.

## 왜 토크노믹스인가

에이전트가 여러 개 돌고, 컨텍스트가 길어지고, 추론이 깊어질수록 단일 세션의 토큰 소비는 급격히 증가합니다. 토큰 단가 하락이 토큰 사용량 증가를 따라가지 못하는 상황에서, 하네스가 토큰을 어떻게 측정하고 라우팅하고 다이어트하고 방어하는지가 비용 경쟁력의 핵심이 됩니다.
...
v3.0의 제품 차별화는 세 가지 핵심으로 구성됩니다. 토크노믹스는 그중 비용을 맡은 영역이며, 나머지 두 핵심과 긴밀하게 연결됩니다.
...
{{< icon package >}} 유지되는 각 에이전트에 모델과 추론 깊이(effort)를 선언적으로 배정합니다. 활성 프로필(`high`/`medium`/`low`)이 프로필 매트릭스의 한 열을 선택하고, 각 에이전트를 그 작업에 걸맞은 추론 깊이 단계에 배치하면서 Sonnet은 단발성 기계적 행에 남겨 비용 대비 품질을 극대화합니다. 상세한 프로필 매트릭스는 [프로필 매트릭스](/ko/advanced/profile-matrix/) 페이지를 참조하세요.
...
B층의 라우팅을 구체화하는 것이 모델 프로필 정책입니다. MoAI-ADK v3.0은 Haiku를 라우팅 모델 세트에서 배제하고, 작업 성격에 맞춘 3-티어 구조로 작업을 분산합니다 — 단발성 행에는 Sonnet, 에이전틱 사다리 전체에는 Opus, 호출 빈도가 가장 낮은 두 행에는 `max` effort입니다. 이 설계의 근거와 프로필 매트릭스 구현은 다음 두 페이지에서 다룹니다.
