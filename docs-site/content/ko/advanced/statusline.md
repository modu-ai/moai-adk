...
리포지토리와 브랜치는 하나의 통합 세그먼트로 렌더됩니다. `owner/name` 부분은 Claude Code v2.1.145부터 stdin에 추가된 `workspace.repo`에서 오고, 그 뒤에 원격 대비 앞/뒤 커밋 수가 `5/0`처럼 슬래시로 붙습니다. 브랜치는 로컬 git에서 읽습니다. 리포 표시에는 📡 글리프가 붙고 브랜치와는 ASCII 파이프(`|`)로 이어져, 두 값이 합쳐지면 "어느 리포의 어느 브랜치에서 일하고 있는가"가 한눈에 들어옵니다. worktree(연결된 별도 작업 디렉터리)에서 작업 중일 때는 브랜치 앞에 `[WT]` 표시가 붙어 일반 체크아웃과 구분됩니다.

### 열린 항목 수 — 🐛 이슈, 🔀 변경요청

리포 세그먼트 뒤에는 이 리포지토리에 열려 있는 **이슈 수(🐛)와 변경요청 수(🔀)**가 이어집니다. 예전에는 넷째 줄에 따로 있던 값인데, 지금은 대상 리포와 같은 자리에 붙어 "이 숫자가 어느 리포의 것인가"를 위치로 말합니다.

각 숫자는 **자기 글리프를 달고 슬래시로 묶이지 않습니다**. 앞의 앞/뒤 커밋 쌍(`5/0`)이 이미 슬래시로 묶인 쌍이라, 뒤에 또 슬래시 쌍을 놓으면 브랜치의 앞/뒤를 "이슈 대 PR"로 잘못 읽게 되기 때문입니다. 그래서 쌍은 슬래시로, 카운트는 글리프로 — 모양으로 갈라 둡니다. 0인 값은 생략하고, 둘 다 0이면 접미사 전체가 사라집니다.
...
| `none` (또는 `off`) | 세지 않습니다 — 접미사가 렌더되지 않습니다 |
| 그 밖의 값 | **탐지로 되돌아가지 않고** 아무것도 렌더하지 않습니다 |

마지막 줄이 중요합니다. 오타를 냈을 때 호스트 이름이 시사하는 쪽으로 조용히 세어 버리면 틀린 숫자가 맞는 것처럼 보입니다. 그래서 알 수 없는 값은 세그먼트가 **사라지는** 증상으로 나타나, 방금 고친 설정 값까지 곧장 되짚을 수 있게 했습니다.
...
해당 CLI(`gh` 또는 `glab`)가 PATH에 없으면 숫자가 없는 것이지 오류가 아닙니다 — 접미사가 조용히 비고 나머지 줄은 그대로 그려집니다. GitHub 쪽은 총계를 한 번에 묻고, GitLab 쪽은 한 페이지(최대 100건)만 열거하므로 열린 항목이 그보다 많으면 페이지 수에서 멈춥니다.

git 상태는 사서함 글리프로 작업 현황을 먼저 알려 줍니다 — 📬 스테이지됨, 📫 수정 있음, 📪 추적 안 된 파일 있음, 📭 깨끗함 — 그리고 `+스테이지 M수정 ?추적안됨` 개수가 뒤따릅니다. 색과 모양 둘 다 쓰므로 흑백 터미널에서도 읽힙니다.
