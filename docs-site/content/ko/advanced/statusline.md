...
해당 CLI(`gh` 또는 `glab`)가 PATH에 없으면 숫자가 없는 것이지 오류가 아닙니다 — 마지막으로 캐시된 값이 그대로 남고, 값을 받아 온 적이 없으면 쌍이 통째로 사라지며 나머지 줄은 그대로 그려집니다. 나중에 CLI를 설치하면 다음 갱신에서 쌍이 저절로 돌아옵니다. GitHub 쪽은 총계를 한 번에 묻고, GitLab 쪽은 한 페이지(최대 100건)만 열거하므로 열린 항목이 그보다 많으면 페이지 수에서 멈춥니다.
