...
- **넷째 줄(조건부) — 누구로서, 얼마나 쌓여 있는가**: 세션 이름(🏷️), 에이전트 이름(👤), 백로그 현황(🔄 `TODO: 진행 중 / 대기`), 열린 이슈·PR 수(🔀)를 보여 줍니다. 칸반 동반 세션처럼 이름 있는 세션에서 자연스럽게 붙고, 관측 원본이 없으면 그 세그먼트는 줄어들며 전부 비면 줄 자체가 생략됩니다. 세션 이름을 강조 표시하는 자리이기도 해서, 터미널 여러 개를 열어 두고 어떤 창이 어느 역할인지 헷갈릴 때 첫 신호가 됩니다.
