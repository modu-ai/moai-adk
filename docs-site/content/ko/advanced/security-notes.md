...
에이전틱 하네스는 에이전트에게 실행 권한을 넘기는 시스템입니다. 권한을 넘기는 만큼 자격증명과 업데이트 경로가 안전해야 하네스 전체를 신뢰할 수 있습니다. 이 페이지는 MoAI-ADK v3.0.0에서 도입된 **사용자 가시 보안 변경 사항**을 정리합니다. 이 보안 강화는 도입 이후 v3.0 계열 현행까지 계속 유효합니다. 항목마다 CWE 매핑, 바뀐 동작, 자체 점검 명령을 함께 실었습니다.

## Why — 왜 이 페이지가 존재하는가

`SPEC-V3R5-SECURITY-CRIT-001`(PR #1032, merge commit `03a2552a2`)은 v2.14.0 → v3.0.0 사이의 코드 리뷰에서 드러난 **P0 release blocker 보안 결함 3건**을 정정했습니다. 이 페이지는 그 정정 내용과, 사용자가 자기 환경에서 새 보호가 작동하는지 직접 확인하는 절차를 4-locale 공식 안내로 명문화합니다.
...
세 항목 모두 회귀 테스트로 잠겨 있어 같은 결함이 다시 들어올 수 없습니다.

## CWE-732 — settings.local.json 권한 강화 (Permission Hardening) {#cwe-732}

### 변경 사항

`.claude/settings.local.json` 파일을 만들거나 갱신할 때 파일 권한을 **`0o600`**(소유자만 read/write)으로 강제합니다. 이전에는 `0o644`(소유자 read/write + group/world read)로 생성돼, 다중 사용자 워크스테이션에서 같은 호스트의 다른 로컬 사용자가 `ANTHROPIC_AUTH_TOKEN` 같은 민감 자격증명을 읽을 수 있었습니다.
...
기존 `settings.local.json`의 권한을 확인합니다.
...
권한이 `644`이거나 그보다 느슨하게 표시되면 MoAI-ADK가 다음 세션 시작 때 자동으로 `0o600`으로 되돌립니다. 곧바로 고치려면 다음과 같이 실행하세요.
...
`group-readable`을 전제하는 워크플로우(같은 프로젝트 디렉터리를 별도 OS 사용자가 읽는 아주 드문 시나리오)는 깨질 수 있습니다. 이 트레이드오프는 의도한 것이며, 보안이 분명히 우선입니다.

## CWE-214 — tmux IPC token argv 노출 차단 {#cwe-214}

### 변경 사항

`moai cg`(CG 모드)가 GLM token(`ANTHROPIC_AUTH_TOKEN`)을 tmux 세션 환경 변수에 주입할 때 **argv 채널**(`tmux set-environment <KEY> <VALUE>`) 대신 **source-file 채널**(`tmux source-file <tmp>`)을 씁니다. 이제 token은 `ps auxe`, `/proc/<pid>/cmdline`, auditd 로그, sysmon 추적, 크래시 덤프 어디에도 평문으로 남지 않습니다.

CG 모드는 토크노믹스의 핵심 절감 수단(Claude 리더 + GLM 워커, 60-70% 절감)인 만큼 자격증명 경로의 보안이 특히 중요합니다.

### 구현 흐름

1. `~/.moai/run/` 아래에 임시 파일을 `mkstemp`로 생성 (mode `0o600` 자동 + 명시적 `chmod 0o600`)
...
argv에는 임시 파일 경로만 남고 token 자체는 드러나지 않습니다.

### 위협 모델

- **공격자**: 같은 호스트의 로컬 사용자 + 시스템 로그 수집 (`ps`, `/proc`, auditd, sysmon)
- **공격 표면**: tmux env injection의 argv 채널
- **누설 정보**: GLM API token이 잠시 드러나는 순간
...
`CLAUDE_CONFIG_DIR`(디렉터리 경로), `ANTHROPIC_BASE_URL`(URL), `ANTHROPIC_DEFAULT_*_MODEL`(모델 이름)처럼 token이 아닌 값은 그대로 argv 경로를 씁니다. 의도한 설계이며 토큰 누설 위험과는 무관합니다.

### 실패 시 동작

source-file 주입이 실패하면(디스크 가득 참, tmux source-file 실패 등) **argv fallback으로 흘리지 않고** `ErrTmuxSensitiveInjectFailed` sentinel error를 반환해 주입 자체를 중단합니다. 실패했다고 편한 길로 돌아가지 않는 것이 이 설계의 핵심입니다.

### 자체 점검

CG 모드 실행 중에 token이 argv로 드러나는지 확인합니다.
...
세션이 끝난 뒤 `~/.moai/run/`에 파일이 남아 있으면 직접 지워도 됩니다(보안 위협은 아닙니다 — 이미 unlink를 시도한 파일입니다).

### 사용자 책임

`~/.moai/.env.glm` source 파일은 사용자 환경에서 `0o600` 권한을 유지해야 합니다. 이 권한은 `moai glm` 명령이 자동으로 설정합니다.
...
`moai update`의 자동 업데이트 흐름은 **checksum 검증을 우회할 수 없습니다**. release의 `checksums.txt` 다운로드나 파싱이 실패하면 sentinel error `ErrChecksumUnavailable`을 반환하고 update 흐름을 **중단**합니다 — binary 다운로드는 아예 시도하지 않습니다.

### Retry 정책

`checksums.txt` 다운로드는 지수 백오프로 **3회까지 retry**합니다.
...
retry가 모두 실패하면 `ErrChecksumUnavailable` sentinel로 종료합니다. **`--skip-checksum` 같은 우회 옵션은 존재하지 않습니다**.

### Defense-in-depth

`version.Checksum` 필드가 empty string인 채로 `downloadAndVerify`에 도달하면 binary 다운로드를 진행하지 않고 `ErrChecksumUnavailable`을 반환합니다. checker 단계와 updater 단계 두 겹으로 막아 소리 없는 우회를 차단합니다.

### 위협 모델

- **공격자**: 네트워크 MITM (전체를 차단하지는 못해도 `checksums.txt` URL만 골라 차단하거나 throttle 가능)
- **공격 표면**: checksums.txt 없이도 binary가 설치되던 silent fallback
...
2. Proxy나 firewall이 GitHub release asset 도메인을 허용하는지 확인
3. 일시적인 GitHub CDN 장애일 수 있으니 잠시 후 재시도
4. **`--skip-checksum` 같은 우회 옵션은 제공하지 않습니다** — 의도한 정책입니다

차단이 계속되면 binary를 직접 설치하세요.
...
위 다섯 항목이 모두 기대값과 맞으면 보안 강화가 정상 작동하는 것입니다 (v3.0.0에서 도입되어 v3.0 계열 현행까지 유효).
