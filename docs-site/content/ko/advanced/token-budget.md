...
GLM-5.3(`moai glm` / `moai cg` GLM 패널)는 1M 컨텍스트 모델이므로 50% 임계로 운영합니다. Claude Code가 보고하는 `context_window_size`는 Claude 슬롯 기준(Opus=1M, Sonnet/Haiku=200K)이라, GLM 세션에서 원시 telemetry가 ~180K로 나와도 MoAI가 1M로 바로잡습니다. statusline의 CW% 게이지를 신뢰하세요.
...
5. **증거 반출** — 인용할 검증 증거를 추적 경로 `.moai/reports/<카드-id>/` 로 반출합니다.
...
그래서 계약이 못 박는 것은 **인용 전 반출**입니다. 어떤 산출물을 판정 근거로 인용하려면 그 전에 추적 경로 `.moai/reports/<카드-id>/` 로 옮겨야 하고, 인용은 디렉터리가 아니라 **파일 하나를 이름 붙입니다**. 옮기는 것은 판정을 결정한 줄(종료 코드, 실패 요약, 인용하는 수치)뿐입니다 — 스크래치를 통째로 옮기면 컨텍스트에서 덜어낸 부피가 저장소로 자리만 옮기게 되므로, 남겨 둔 원문은 스크래치에 두고 그 손실 위험을 판정문의 잔여 위험 항목에 적습니다. 반출하지 않기로 한 것을 인용해서는 안 됩니다.
