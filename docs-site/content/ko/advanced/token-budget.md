...
토크노믹스 4-층 구조의 D층인 예산 가드(Budget defense)를 심화해서 다룹니다. 에이전트가 컨텍스트 윈도우 한계에 다다랐을 때 세션이 멈추지 않고, 진행 상태를 보존하면서 다음 세션이 이어받을 수 있도록 하는 우아한 중단(graceful abort) 메커니즘을 설명합니다.
...
예산 가드는 이 문제를 사전에 해결합니다. 컨텍스트 사용량이 임계에 도달하기 전에 시스템이 우아한 중단을 수행하여, 세션이 손실 없이 다음 단계로 전환되도록 보장합니다.
