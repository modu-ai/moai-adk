...
GLM-5.3(`moai glm`)는 1M 컨텍스트 모델이므로 50% 임계로 운영합니다. Claude Code가 보고하는 `context_window_size`는 Claude 슬롯 기준(Opus=1M, Sonnet/Haiku=200K)이라, GLM 세션에서 원시 telemetry가 ~180K로 나와도 MoAI가 1M로 바로잡습니다. statusline의 CW% 게이지를 신뢰하세요.
...
5. **증거 기록** — 인용할 검증 증거 가운데 판정을 결정한 줄을 추적되는 판정서 `.moai/reports/<카드-id>/verdict.md` 안에 적습니다.
...
여기에 한 겹이 더 있습니다. 파일로 돌린 증거가 `/tmp`에 그대로 남으면 OS가 주기적으로 지워 버립니다(macOS 재부팅, Linux tmpfs 리마운트, systemd-tmpfiles). 그래서 검증 출력은 먼저 `.moai/state/verify/<session>/` 아래에 담습니다. 다만 이 자리는 **머신-로컬 스크래치**입니다 — `/tmp` 소거는 견디지만 gitignore 대상이라 clone 에도, CI 러너에도, 다른 머신에도 따라가지 않습니다. `/tmp`를 견디는 것과 감사 시점에 열리는 것은 같은 말이 아닙니다.

그래서 계약이 못 박는 것은 **판정을 결정한 줄을 판정서 안에 적는 것**입니다. 어떤 근거로 주장을 세우려면 그 전에 그 줄을 추적되는 판정서 `.moai/reports/<카드-id>/verdict.md` 안에 적어야 하고, 인용은 디렉터리가 아니라 **그 파일 하나를 이름 붙입니다**. 카드 디렉터리에서 추적되는 이름은 판정서 하나뿐이라, 그 옆에 따로 쓴 파일은 여전히 무시되고 그것을 인용하면 쓴 트리 밖에서는 열리지 않습니다. 적는 것은 판정을 결정한 줄(종료 코드, 실패 요약, 인용하는 수치)뿐입니다 — 스크래치를 통째로 옮기면 컨텍스트에서 덜어낸 부피가 저장소로 자리만 옮기게 되므로, 남겨 둔 원문은 스크래치에 두고 그 손실 위험을 판정문의 잔여 위험 항목에 적습니다. 스크래치에 남겨 둔 것을 인용해서는 안 됩니다.
