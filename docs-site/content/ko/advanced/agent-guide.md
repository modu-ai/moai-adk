...
flowchart TD
    ...
    subgraph TIER1["Tier 1: Manager 에이전트 (7개)"]
        MS["manager-spec<br>SPEC 생성"]
        MD["manager-develop<br>DDD/TDD 통합 구현"]
    ...
    subgraph TIER2["Tier 2: Expert 에이전트 (6개)"]
    ...
    subgraph TIER3["Tier 3: Builder 에이전트 (1개)"]
        BA["builder-harness<br>통합 아티팩트 생성"]
    end
...
| `manager-develop` | DDD/TDD 통합 구현 (cycle_type으로 전환) | `moai-workflow-ddd`, `moai-workflow-tdd`, `moai-foundation-core` | Read, Write, Edit, Bash |
...
# Run 단계: manager-develop가 DDD 사이클 실행
...
| `expert-debug` | 디버깅은 manager-quality(diagnostic-mode)에 통합됨 | — | — |
| `expert-testing` | 테스트 전략은 manager-develop RED phase에 통합됨, 부하 테스트는 expert-performance에 이관 | — | — |
...
| 에이전트 | 역할 | 생성물 |
|----------|------|--------|
| `builder-harness` | 통합 아티팩트 생성 (에이전트, 스킬, 플러그인, 커맨드, 훅, MCP 서버, LSP 서버) | `.claude/agents/`, `.claude/skills/`, `.claude-plugin/` |
...
SEMAP (Semantic Agent Protocol)은 에이전트의 **행동 범위를 명시적으로 제한**하는 계약 시스템입니다. manager-develop, manager-quality에 적용됩니다.
...
| 버그 수정 | manager-quality (diagnostic) | "이 에러 원인을 찾아줘" |
| 테스트 작성 | manager-develop (RED phase) | "이 함수의 테스트를 추가해줘" |
| 보안 검토 | expert-security | "보안 취약점 점검해줘" |
| SPEC 생성 | manager-spec | `/moai plan "기능 설명"` |
| DDD 구현 | manager-develop (DDD) | `/moai run SPEC-XXX` |
...
├── expert-debug.md          # retired — manager-quality로 통합
├── expert-testing.md        # retired — manager-develop/expert-performance로 분리
├── expert-refactoring.md
├── manager-spec.md
├── manager-ddd.md           # retired — manager-develop로 통합
├── manager-tdd.md           # retired — manager-develop로 통합
├── manager-docs.md
├── manager-quality.md
├── manager-strategy.md
├── manager-project.md
├── manager-git.md
├── builder-agent.md         # retired — builder-harness로 통합
├── builder-skill.md         # retired — builder-harness로 통합
├── builder-plugin.md        # retired — builder-harness로 통합
└── builder-harness.md       # 활성 — 통합 아티팩트 생성
```
...
# 2. SPEC을 바탕으로 manager-develop가 구현
...
# - expert-testing: retired — manager-develop RED phase / expert-performance로 분리
