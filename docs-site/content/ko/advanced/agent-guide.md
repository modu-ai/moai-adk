...
weight: 30
draft: false
description: "MoAI-ADK의 13개 핵심 에이전트 카탈로그 — 역할, 단계 범위, 계획-감사 분리 원칙, 계층 구조."
---

MoAI-ADK가 쓰는 13개 에이전트(스스로 일하는 AI 도우미) 카탈로그를 처음부터 차근차근 안내합니다. 이 문서는 "에이전트가 무엇이고, 왜 여럿 쓰는지, 어떻게 협업하는지"를 친구에게 설명할 수 있을 만큼 명확하게 풉니다.
...
flowchart TD
    ...
    MOAI --> ETC["나머지 5개<br>팀 생성 · 고추론 자문 · E2E 테스트 · 미션 판정 · 코드 탐색"]
```

에이전트는 모두 Claude Code의 하위 에이전트(Sub-agent) 시스템 위에서 돕니다. 하위 에이전트마다 독립된 컨텍스트 창, 맞춤 시스템 지시문, 선별된 도구, 별도 권한이 주어집니다. 이 기반 위에 MoAI-ADK가 13개 전문 역할을 올려 놓은 것입니다.
...
둘째, **비용**. 에이전트마다 컨텍스트를 새로 채우고 추론을 돌려야 하므로 위임 한 번마다 토큰 비용이 듭니다. 그래서 "많을수록 좋다"가 성립하지 않습니다. 카탈로그는 v3 과정에서 22 → 17 → 8 → 10 → 12 → 13으로 다듬어졌고, 지금은 각 역할이 겹치지 않는 최소한으로 유지됩니다. 에이전트 수를 줄이는 일 자체가 토크노믹스(토큰을 아껴 쓰는 일)의 일부입니다.
...
MoAI-ADK는 **13개 에이전트** (12개 MoAI 사용자 정의 + 1개 Anthropic 내장 `Explore`)를 씁니다. 다음은 역할별로 묶은 전체 목록입니다.
...
| `mission-governor` | 승인된 GTD 자동 미션의 봉인된 스냅샷을 읽고, 구조화된 판정 하나만 돌려줍니다 (읽기 전용) |

`mission-governor`는 판정을 내리기만 하고 **직접 실행하지는 않습니다**. 파일을 쓰거나 셸·Git 명령을 돌리거나 큐를 건드리거나 레인에 일을 내리는 일이 전혀 없고, 커밋·머지·승인도, 감사 판정도 하지 않습니다. 돌려준 판정을 검사하고 실제로 상태를 바꾸는 쪽은 결정론적 실행기(정해진 규칙대로만 움직이는 자동 처리기)입니다. 도구 목록도 `Read`, `Grep`, `Glob`, `Skill` 넷뿐이며, 요청이 봉인된 범위를 벗어나거나 판단에 필요한 증거가 없거나 낡았으면 blocker 판정을 돌려줍니다.

{{< callout type="info" >}}
`mission-governor`는 아래 [에이전트 선택 결정 트리](#에이전트-선택-결정-트리)에 **의도적으로 자리가 없습니다**. 오케스트레이터가 골라 부르는 에이전트가 아니라 GTD 자동 미션 워크플로가 직접 불러 쓰는 판정 역할이기 때문입니다. 결정 트리에 안 보인다고 해서 빠뜨린 것이 아닙니다.
{{< /callout >}}
...
이렇게 하면 카탈로그는 13개로 작게 유지하면서, 도메인 깊이는 스킬 주입으로 채웁니다. 에이전트 수를 늘려 토큰 비용을 키우는 대신, 스킬(재사용 가능한 작업 지시서 묶음)을 갈아 끼우는 구조입니다.
...
**주의**: 과거 세션에서 복사한 재개 메시지에 이런 이름이 들어 있으면 오케스트레이터가 생성을 거부합니다. 이때는 같은 일을 `Agent(general-purpose)`에 도메인 지시문을 실어 보내거나, 13개 현행 에이전트 가운데 하나로 대신 돌립니다. 대체 경로는 `.claude/rules/moai/workflow/archived-agent-rejection.md`에 정리돼 있습니다.
{{< /callout >}}
...
13개 에이전트 가운데 `manager-lead`만이 `tools:` 목록에 `Agent` 도구를 담고 있습니다. 나머지는 모두 `Agent`를 빼서 평면 구조를 유지합니다. 그래서 오케스트레이터 → `manager-lead`가 1단, `manager-lead` → 리프 워커가 2단이고, 3단은 생기지 않습니다.
...
마일스톤 하나가 끝나면 다음으로 넘어가기 전 세 단계를 밟습니다. 먼저 각 인수 기준(AC)의 검증 출력을 머신-로컬 스크래치에 담고, 그중 판정을 결정한 줄만 추적되는 판정서 `.moai/reports/<카드-id>/verdict.md` 안에 적어 감사 때 열리게 만듭니다. 카드 디렉터리에서 추적되는 이름은 그 판정서 하나뿐입니다. 이어서 진행 기록에 그 판정서를 이름 붙인 한 줄 요약 행을 추가하며, 마지막으로 `/compact`로 컨텍스트를 압축합니다. 압축 뒤 토큰이 줄고 핸드오프 임계값(1M 계열 50%, 200K/256K 계열 90%) 아래로 떨어져야 다음 마일스톤으로 넘어갑니다 — 줄지 않으면 실패한 폴드로 보고 다시 계획합니다.
...
**1단 — 스폰 시점 정적 필터.** 모든 에이전트 정의는 frontmatter에 `tools:` 허용 목록(CSV 문자열, 예: `tools: Read, Write, Edit`)을 가지며, 목록 밖의 도구는 호출할 수 없습니다. 읽기 전용 역할은 이 목록 자체를 줄여 권한을 만듭니다 — 감사자나 교차 검증 워커는 쓰기 도구(Write, Edit)를 목록에서 빼서 파일을 직접 고치는 경로를 없앱니다. 다만 이것은 좁히기이지 봉인이 아닙니다 — `Bash`가 목록에 남아 있으면 그쪽으로 쓸 수 있으므로, 읽기 전용인지는 도구 이름이 없는지가 아니라 쓰기 능력이 없는지로 판별해야 합니다.
...
| 규칙 | 내용 |
|------|------|
| 사용자 질문은 오케스트레이터 전용 | `AskUserQuestion`은 오케스트레이터만 쓰며 런타임이 이 경계를 강제합니다. 사용자 입력이 필요한 서브에이전트는 프롬프트 대신 구조화된 blocker 보고서를 반환하고, 오케스트레이터가 사용자에게 물어 답을 실어 다시 보냅니다 |
| sweep 서브에이전트도 질문 불가 | 동적 워크플로우(sweep)의 서브에이전트는 메인 세션 아래에서 돌며 사용자에게 프롬프트할 수 없습니다. 질문이 필요하면 오케스트레이터의 채널로 거쳐 갑니다 |
...
병렬 조사·리뷰는 병렬 하위 에이전트 팬아웃으로, 순차 코딩은 하위 에이전트 체인으로 처리합니다. CG는 폐기되었습니다. `moai migrate cg`로 이전 선택지를 먼저 확인하세요.
