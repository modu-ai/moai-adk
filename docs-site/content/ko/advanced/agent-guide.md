...
마일스톤 하나가 끝나면 다음으로 넘어가기 전 세 단계를 밟습니다. 먼저 각 인수 기준(AC)의 검증 출력을 머신-로컬 스크래치에 담고, 그중 판정을 결정한 줄만 추적되는 판정서 `.moai/reports/<카드-id>/verdict.md` 안에 적어 감사 때 열리게 만듭니다. 카드 디렉터리에서 추적되는 이름은 그 판정서 하나뿐입니다. 이어서 진행 기록에 그 판정서를 이름 붙인 한 줄 요약 행을 추가하며, 마지막으로 `/compact`로 컨텍스트를 압축합니다. 압축 뒤 토큰이 줄고 핸드오프 임계값(1M 계열 50%, 200K 계열 90%) 아래로 떨어져야 다음 마일스톤으로 넘어갑니다 — 줄지 않으면 실패한 폴드로 보고 다시 계획합니다.
