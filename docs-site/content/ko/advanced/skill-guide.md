...
MoAI-ADK에는 총 **31개 스킬**이 6개 카테고리로 분류되어 있습니다. `moai` umbrella 라우터 1개와 30개의 전문 스킬로 구성됩니다 (Foundation, Workflow, Domain, Reference, Meta/Harness, Design). 프로그래밍 언어 지원은 `rules/moai/languages/` 아래의 규칙으로 제공되며 별도 스킬이 아닙니다.
...
> `moai` umbrella 스킬 (통합 `/moai` 라우터)은 총 31개에 포함되지만 분류된 기능 스킬은 아니며, 이 가이드에 설명된 서브커맨드를 디스패치합니다.
...
- **기존 방식**: 31개 스킬 전체 로드 = 약 160,000 토큰 (불가능)
