...
12. **교차 세션(Cross-Session)** — 세션 간 메시징의 수신 자세(posture): 인바운드 처리 방식(`accept` · `hold` · `refuse`), 머신 간 전송 격리, 보류 대화 만료. `crosssession.yaml`을 편집하며, 런처가 다음 `moai cc`/`glm`/`cg` 실행부터 이 값을 세션에 주입합니다 — 이미 돌고 있는 세션은 띄울 때의 자세를 유지합니다
