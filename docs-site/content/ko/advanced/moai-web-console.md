...
![MoAI Web Console — 헤더의 프로젝트 이름과 프로필 선택기, 추가·이름변경·삭제 컨트롤이 함께 있는 프로필 바, 아홉 개 설정 탭](/img/moai-web-console.png)
...
프로필을 자주 전환하며 값을 비교하는 일이 대표적인 사례입니다. 평소엔 `medium` 프로필, 어려운 작업엔 `high` 프로필을 쓴다고 할 때, 프로필 바에서 전환하고 탭에서 각 프로필의 값을 바로 비교할 수 있습니다.
...
프로필 선호도는 모델, 추론 강도, 언어, 표시 설정 등 프로필마다 따로 값을 갖는 항목입니다. 프로필 바에서 전환하고 탭에서 값을 바꿉니다. 프로필 추가·이름변경·삭제 컨트롤이 선택기 바로 옆에 있어, 프로필 생애주기 전체가 한 줄에 들어갑니다 — 별도의 프로필 관리 카드는 없습니다.

## 콘솔 구조 — 9개 탭

콘솔은 아래 순서의 9개 탭으로 구성됩니다.

1. **사용자 정보(Identity)** — 표시 이름과 프로젝트 단위 identity 필드
2. **언어(Language)** — 대화·커밋 메시지·코드 주석·문서 언어
3. **LLM** — 권한 모드·모델·추론 강도 (세션이 시작될 때 적용되는 세 가지)
4. **서드파티 LLM(3rd Party LLM)** — 티어별 GLM 모델 선택, 티어별 추론 강도, GLM API 키
5. **워크플로우(Workflow)** — 실행 모드·기본 모드·agentic-loop·loop-prevention 설정
6. **Git·워크트리(Git & Worktree)** — `git_strategy.mode`, 프로필별 `merge_method` 3종, 워크트리·branch-guard 토글
7. **감사(Audit)** — 감사 모델과 백엔드별 게이트(claude / codex / glm)
8. **에이전트(Agents)** — 에이전트별 프로필·모델 할당
9. **리포트(Report)** — 리포트 형식과 출력 선호

탭 목록이 패널 순서의 단일 원천입니다 — 화면 위쪽에 보이는 순서가 아래에 렌더되는 순서와 같습니다. 워크플로우·Git & 워크트리·감사 탭 분리는 한 탭이 너무 빽빽해지는 일을 막아 줍니다. 과거에는 이 세 탭이 한 워크플로우 탭에 몰려 있었고, `git_strategy` 섹션은 필드는 있지만 렌더 표면이 없어 UI 에 보이지 않았습니다 — 이 렌더 표면 복구가 재설계의 한 축입니다.

### 위젯 정직성

각 필드는 값의 실제 도메인과 들어맞는 위젯으로 렌더됩니다.

- **bool 필드** 는 체크박스가 아니라 2옵션 라디오 그룹(사용 / 미사용)으로 렌더됩니다. 체크박스는 "끄고 켜는" 느낌을 암시하지만, 라디오 쌍은 현재 선택을 명시적으로 보여줍니다.
- **닫힌 집합 필드** (`execution_mode`, `audit.model`, `harness.*` 따위)는 select 또는 라디오 그룹으로 렌더되고, 저장 시 집합 밖의 값은 적용 계층에서 거부됩니다. 닫힌 집합을 자유 텍스트로 받으면 무효 값을 입력할 틈이 생기는데, 콘솔은 더 이상 그 틈을 주지 않습니다.
- **진정 열린 필드** (저장소 경로, 출력 디렉터리, GLM API 키)는 자유 텍스트로 남습니다 — 도메인이 정말로 열려 있기 때문입니다.

### GLM 정직 배지

서드파티 LLM 탭에서 추론 강도의 런타임 전달 채널은 세션 단위 환경변수 하나뿐입니다 — 티어별 전달 경로는 없습니다. 그래서 티어별 추론 강도 값은 **저장 전용** 입니다: 설정에 저장은 되지만, 런타임은 세션 단위 effort 를 z.ai 의 3-상태로 collapse 한 값만 읽어옵니다. 탭에는 **적용 원천** (세션 단위 effort)을 명시하는 배지가 있어, 티어별 값이 저장 전용이라는 점이 암시가 아니라 명시로 드러납니다.
