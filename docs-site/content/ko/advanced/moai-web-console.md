...
{{< callout type="info" >}}
루프백 전용이 인증 없음의 전제입니다. 역방향 프록시나 `0.0.0.0` 바인딩으로 외부에 노출하는 구성은 지원하지 않습니다. 원격에서 봐야 하면 SSH 터널로 로컬 포트를 전달하세요. SSH 터널을 쓰면 브라우저가 여전히 `localhost` 주소로 콘솔을 열므로 이 검사와 함께 계속 동작합니다.
{{< /callout >}}
