...
weight: 85
draft: false
description: "여섯 영역(개요·칸반·SPEC·모니터·설정·할 일)으로 구성된 로컬 운영 콘솔 — 실시간 갱신, 읽기 전용 관측, 설정 편집."
---
# MoAI Web Console
...
{{< callout type="info" >}}
**한 줄 요약:** 콘솔은 관측 화면 다섯과 설정 화면 하나를 왼쪽 레일로 묶은 운영 셸입니다. 관측 화면은 읽기만 하고, 설정 화면은 터미널 마법사와 같은 검증·저장 계층을 씁니다.
{{< /callout >}}

## 운영 셸 — 왼쪽 레일과 상단바

화면은 세 조각으로 나뉩니다. 왼쪽 **레일**에 여섯 영역이 세로로 놓이고, 위쪽 **상단바**에 현재 화면 제목과 상태 표시가 오고, 나머지가 **본문**입니다. 어느 영역에 있든 레일과 상단바는 같은 자리에 남습니다.
...
| 설정(Settings) | `/settings` | 프로필 선호도와 프로젝트 섹션 편집 |
| 할 일(Todo) | `/todo` | 백로그 큐 읽기 전용 — 세 상태의 카드를 모두 나열 |

상단바 오른쪽에 오는 것은 영역에 따라 다릅니다. 관측 영역 다섯에서는 **실시간 표시등**이, 설정 영역에서는 **저장 클러스터**(변경 개수와 저장 버튼)가 놓입니다. 문맥 칩(`lang` · `model` · `effort` · `dev`)은 설정 영역에서만 렌더됩니다 — 지금 편집 중인 프로필의 핵심 값을 저장 전에 눈으로 확인하라는 용도이기 때문입니다.
...
```mermaid
flowchart TD
    Rail["왼쪽 레일<br/>개요 · 칸반 · SPEC · 모니터 · 설정 · 할 일"]
    Top["상단바<br/>제목 · 실시간 표시등 / 저장 클러스터"]
    Body["본문<br/>영역별 화면"]
    Read["읽기 전용 영역<br/>개요 · 칸반 · SPEC · 모니터 · 할 일"]
...
**체인 세션 보드**는 다섯 역할을 카드로 늘어놓고 각 역할의 세션 id, 백엔드, 모델, 추론 강도, 컨텍스트 사용량, 마지막 하트비트를 적습니다. 단계 상태는 하트비트에서 **추정한** 값이라 추정 표식이 함께 붙고, 모델·추론 강도·컨텍스트는 그 세션 자신의 텔레메트리 기록에서 가져옵니다. 칸이 비어 있으면 그 값을 담은 기록이 아직 없다는 뜻입니다 — 채워 넣지 않는 것이 규율입니다.

체인 보드 옆에는 **팩토리 레인** 목록이 놓입니다. 등록된 레인마다 한 줄씩, 세션·카드·SPEC 식별자·백엔드를 적습니다. 레인의 프로세스 식별자가 어느 세션에도 닿지 않거나, 두 레지스트리 중 어느 쪽에서든 세션 하나를 가리키지 못하면 그 줄은 **확정 불가**로 표시되고 값을 하나도 싣지 않습니다. 엉뚱한 세션으로 조회가 성공해 다른 레인의 기록을 이 레인의 것처럼 그리는 편이 빈 줄보다 나쁜 결과이기 때문입니다. 등록된 레인이 없으면 없다고 적습니다.
...
`moai todo` 가 쓰는 백로그 큐에 주소를 하나 붙였습니다. 화면은 **세 상태의 카드를 모두** 나열합니다 — `queued` · `picked` · `dropped` — 각 카드의 id, 내용, 상태 뱃지, 그리고 딸린 SPEC id가 있으면 그것까지. 아무것도 걸러내지 않습니다. 이 화면이 답해야 할 질문이 "그 카드 어디로 갔나"이고, 대기 중인 것만 보여주는 작업 화면으로는 그 질문에 답할 수 없기 때문입니다.

읽기만 합니다. 추가 · 선택 · 폐기는 `moai todo` 몫이고, 콘솔은 큐에 쓰지도, 큐의 잠금을 잡지도 않습니다. 연결된 워크트리에서 열어도 빈 화면이 아니라 **주 체크아웃**의 큐가 보입니다 — 큐는 저장소마다 하나인 통로이고, 어느 파일을 보고 있는지 알 수 있도록 해석된 경로를 머리글에 적어 둡니다.

대기열이 없거나 정상적으로 읽은 대기열에 카드가 없으면 HTTP 200과 빈 대기열 안내를 표시합니다. 읽기에 실패한 경우에도 HTTP 200을 반환하지만, 카드 수 0 대신 읽기 실패 안내를 표시합니다. 저장 계층의 원시 오류는 화면에 노출하지 않습니다.

## 실시간 갱신 — 신호만 보내고 다시 가져온다

관측 영역은 감시 중인 파일이 바뀌면 갱신됩니다. 서버는 `GET /events`에 SSE 스트림을 열고, 아래 표의 프로젝트 경로와 해석된 홈 디렉터리를 감시합니다. 파일 변경 신호는 250밀리초 단위로 묶어 보냅니다.
...
| `kanban` | `.moai/state/todo`, 해석된 홈 Todo 디렉터리(`~/.moai/db/<project-key>/todo`), 해석된 Factory 디렉터리 |
| `config` | `.moai/config/sections` |

`config` 이벤트만 다르게 다룹니다. 설정을 편집하는 중에 화면이 밑에서 바뀌면 입력하던 값이 사라지므로, 갱신하지 않고 "설정 파일이 바뀌었다"는 배너만 띄웁니다.

시작할 때 없던 디렉터리는 1초 간격으로 다시 등록을 시도합니다. 새 디렉터리의 감시가 등록되면 SSE 연결이 정상이어도 화면 갱신 신호를 보냅니다. 연결이 끊기면 상단바 표시등이 끊김 상태로 바뀌고, 재연결을 세 번 실패하면 브라우저가 30초 간격으로 화면을 다시 읽습니다.
...
- **기록이 없는 값**(역할별 모델 · 추론 강도 · 컨텍스트 사용량)은 그럴듯한 값으로 채우지 않고 비워 둡니다. 세션 하나에 붙일 수 없는 팩토리 레인도 다른 레인의 기록을 빌려 그리지 않고 확정 불가로 표시합니다.
...
codex 설정은 원래 감사 탭과 MCP 탭에 나뉘어 있습니다. "이 프로젝트는 codex 를 어떻게 쓰고 있나"를 알려면 탭 두 곳을 오가며 한 번도 같이 보이지 않는 값들을 읽어야 했습니다. Codex 탭은 그 값들을 한 화면에 모읍니다.

**모아 보여주기만 하고 고치지는 않는 것은 미완성이 아니라 선택한 절충입니다.** 설정 화면의 모든 패널은 하나의 `<form>` 안에 들어 있고, 저장 처리는 같은 이름으로 들어온 값 중 **첫 번째 것만** 읽습니다. 그래서 같은 설정을 두 패널에서 컨트롤로 그리면, 뒤에 놓인 패널에서 고친 값이 **아무 경고 없이 버려집니다**. bool 설정에 딸린 숨은 동반 필드만 따로 놓이는 경우는 더 나쁩니다 — 그 값은 "명시적인 false" 로 읽히므로, 사용자가 페이지를 열어 저장만 해도 MCP 도구 토글 여섯 개가 꺼집니다.

그래서 Codex 탭은 값과 링크만 렌더하고, 이름을 가진 입력 요소는 하나도 만들지 않습니다. 모든 설정은 원래 자기 탭에 그대로 선언되어 있고 거기서 편집합니다. 이 화면을 나중에 "편집 가능하게 고치는" 것은 위의 손실을 되살리는 일입니다.

**새로 생긴 설정은 하나도 없습니다.** 새 설정 키도, 새 저장 경로도, `.moai/config` 아래 새 파일도 없습니다. 설정은 이미 있던 것이고, 새로 생긴 것은 그것을 보는 자리뿐입니다.

MCP 도구 토글은 디스크에 값이 비어 있는 경우가 많은데, 콘솔은 빈 값을 **켜진 것**으로 읽습니다. 이 탭은 빈 값을 임의로 해석해 두 번째 분류기가 되는 대신 `(설정 없음)` 이라고 적고, 그룹 머리글에 "값이 비어 있으면 켜진 것으로 읽습니다" 라는 사실을 함께 둡니다.
...
추론 강도의 런타임 전달 채널은 세션 단위 환경변수 하나뿐이라, 티어별로 적어 둔 추론 강도는 **저장 전용**입니다. 설정에는 남지만 런타임은 세션 단위 값만 읽습니다. GLM 설정 탭에는 적용 원천을 명시하는 배지가 있어, 이 사실이 암시가 아니라 명시로 드러납니다.
...
**루프백 전용.** 콘솔은 `127.0.0.1` 에만 바인딩되므로 원격 호스트에서는 닿지 않습니다. 다만 루프백 포트는 계정별로 나뉘지 않아서, 같은 머신에 로그인한 다른 계정은 이 포트에 접속할 수 있습니다.

**데이터베이스 없음.** 별도 DB 를 띄우지 않습니다. 읽고 쓰는 값은 현재 프로젝트의 `.moai/` 아래 파일이 전부입니다.

**인증 없음.** 로그인이나 토큰 계층이 없습니다. 루프백 바인딩은 원격 호스트를 막지만 같은 머신의 다른 계정은 막지 못합니다. 여러 사람이 함께 쓰는 머신이라면 콘솔이 켜져 있는 동안 그 계정들도 콘솔에 접속할 수 있습니다.

**명령 실행 없음.** 관측 영역은 GET 이외의 메서드를 거부하고, 어떤 화면도 서버에서 명령을 실행하지 않습니다. SPEC 의 status 전이도 콘솔이 하지 않습니다 — 전이의 소유자는 각 단계의 관리자 에이전트입니다.

**모든 경로에 Host 검사.** 정적 파일을 포함한 모든 경로에서, 메서드와 관계없이 `Host` 헤더가 루프백 이름(`localhost` · `127.0.0.1` · `[::1]`, 포트 유무 무관)이 아니거나 헤더 자체가 없으면 403 으로 거부됩니다. 루프백 바인딩만으로는 DNS 리바인딩을 막을 수 없기 때문입니다 — 다른 사이트의 페이지가 브라우저를 `127.0.0.1` 에 접속시키면서 `Host` 에는 공격자 도메인을 실어 보내면, 브라우저는 그 페이지가 응답을 읽도록 허용합니다. 같은 출처(`Sec-Fetch-Site`) 검사는 여전히 상태를 바꾸는 요청에만 적용됩니다.

{{< callout type="info" >}}
루프백 전용이 인증 없음의 전제입니다. 역방향 프록시나 `0.0.0.0` 바인딩으로 외부에 노출하는 구성은 지원하지 않습니다. 원격에서 봐야 하면 SSH 터널로 로컬 포트를 전달하세요. SSH 터널을 쓰면 브라우저가 여전히 `localhost` 주소로 콘솔을 열므로 이 검사와 함께 계속 동작합니다. 다만 이는 실제 SSH 서버가 아니라 포트 전달을 그대로 흉내 낸 시험 환경에서 확인한 것이라, 검증된 보장이 아니라 예상되는 동작으로 받아들여야 합니다.
{{< /callout >}}
