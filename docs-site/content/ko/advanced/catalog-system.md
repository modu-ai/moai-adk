...
토크노믹스는 토큰에만 걸리는 원칙이 아닙니다. 프로젝트에 배포하는 템플릿 파일 하나하나가 결국 세션이 읽어 들일 컨텍스트 후보이기 때문입니다. 카탈로그 시스템은 "필요한 것만 배포한다"는 원칙으로 이 비용을 초기화 단계에서부터 줄입니다.

## 개요

MoAI-ADK의 카탈로그 시스템은 모든 에이전트, 스킬, 규칙을 **3계층 매니페스트**(`catalog.yaml`)로 관리합니다. 기본 배포는 **slim 모드**로, 핵심 템플릿(core)만 내려보내므로 초기화가 빠르고 프로젝트에 남는 파일도 가볍습니다. 전부 받아야 한다면 `--all` 플래그를 씁니다.

## 3계층 매니페스트

배포 대상은 모두 세 계층 가운데 하나에 들어갑니다.
...
엔트리마다 `name`, `tier`, `path`, `hash`, `version` 필드를 둡니다. `hash` 필드에 콘텐츠 해시가 들어 있어, 배포된 파일이 깨졌거나 임의로 바뀌었는지 로더가 확인할 수 있습니다. 스킬 디렉터리의 진입점 파일 이름은 `SKILL.md`입니다(소문자 `skill.md`가 아닙니다).

## Slim 모드와 --all 플래그

기본 배포는 **slim 모드**로 `catalog.core`만 내려보냅니다. 전부 받아야 하면 `--all` 플래그나 `MOAI_DISTRIBUTE_ALL=1` 환경변수를 씁니다.
...
1. `catalog.core`(skills + agents)는 언제나 포함 — slim 모드의 기본
2. `--all` 플래그나 `MOAI_DISTRIBUTE_ALL=1` 환경변수가 걸려 있으면 `catalog.optional_packs`와 `catalog.harness_generated`를 추가로 배포

## Typed Loader

`LoadCatalog()` 함수가 매니페스트를 타입 안전하게 읽어 들입니다. 문자열 파싱에 기대지 않고 구조체 단위로 검증하므로, 매니페스트 오류는 배포 전에 걸러집니다.
...
`moai update`도 같은 카탈로그를 기준으로 움직입니다. slim으로 초기화한 프로젝트는 core만, `--all`로 초기화한 프로젝트는 전체를 업데이트합니다.
