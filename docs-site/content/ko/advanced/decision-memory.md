...
의사결정 메모리(Decision Memory)는 MoAI-ADK의 **장기 학습 계층**입니다. AskUserQuestion 라운드에서 사용자의 선택을 관찰해 두었다가, 나중에 같은 의사결정 지점을 만나면 통계적 다수 선택을 근거로 적응형 권장을 내놓습니다.
...
하나의 AskUserQuestion 호출에 여러 질문을 담을 때는 정보 이익이 가장 높은 질문을 앞에 둡니다. 그래야 사용자가 핵심 결정을 먼저 끝내고, 덜 중요한 질문은 나중에 만납니다.
...
권장 옵션의 설명에는 그 권장이 성립하는 전제 조건을 반드시 밝혀야 합니다. 전제가 깨졌을 때 사용자가 곧바로 거부할 수 있도록 `"Recommended when <전제조건>"` 형태로 제시합니다. 전제를 밝히지 않은 권장은 설계 결함입니다.
...
과거의 선택이 오늘의 선호를 그대로 대변하지는 않습니다. transient 엔트리에는 **power-law 감쇠 + 28일 TTL**(REQ-ADM-011, REQ-ADM-012)이 적용되어 가중치가 서서히 줄고, TTL이 지나면 밀려납니다. core-tier에서는 가중치가 가장 낮은 항목부터 강등합니다.
...
Model Selection과 Effort Level도 여기에 들어갑니다 — 의사결정 메모리가 학습한 선호가 결국 모델과 추론 깊이 배정으로 이어지므로, 이 시스템은 토크노믹스의 개인화 계층이기도 합니다.
...
관찰이 부족할 때는 그 사실을 숨기지 않고 그대로 밝힙니다.
...
아직 학습 중인 상태라는 점을 사용자가 분명히 알 수 있습니다.
