...
@MX TAG는 코드 레벨 주석으로, AI 에이전트가 개발 세션 사이에 **컨텍스트·불변량·위험 구역**을 전달하는 표준 수단입니다. 프롬프트는 무시될 수 있지만 코드에 새겨진 주석은 코드와 함께 살아 남아, 다음 에이전트가 코드를 처음 읽는 순간 의도와 제약을 즉시 파악할 수 있습니다.
