...
MoAI-ADK는 기본 제공되는 32개 스킬, 8개 에이전트 외에도 사용자가 직접 확장할 수
...
allowed-tools: # 허용 도구 (쉼표 구분 또는 리스트)
  ...
  - Grep
  - Glob
model: inherit # 사용할 모델 (생략 시 현재 모델, 'inherit' 권장)
...
- ML 모델 개발 (manager-develop에 위임)
- 인프라 구성 (manager-develop에 위임)

## Delegation Protocol

- ML 모델 필요시: manager-develop
- 인프라 설정시: manager-develop
