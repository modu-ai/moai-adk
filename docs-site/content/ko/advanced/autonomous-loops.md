...
{{< icon arrow-right >}} `/goal`은 Claude Code의 네이티브 TUI 명령입니다. 사용자가 입력하는 명령이며, 모델이 사용자를 대신하여 호출할 수 없습니다. 이것이 **HUMAN-ONLY** 제약입니다.

완료 조건을 선언하면, 각 턴 종료 후 작은 빠른 모델(Haiku 기본)이 조건이 충족되었는지 평가합니다. 충족되지 않았으면 다른 턴을 시작하고, 충족되면 루프가 종료됩니다.

```text
/goal go test ./... exits 0 && lint is clean, or stop after 20 turns
```

조건은 최대 4,000자까지 가능하며, 턴/시간 한도를 포함하여 루프를 바운드할 수 있습니다. `/goal` 단독으로 상태를 확인하고, `/goal clear`로 조기 종료할 수 있습니다.
...
{{< icon arrow-right >}} `/moai loop`는 진단 도구가 찾은 이슈 큐를 스캔하고, 각 이슈를 수정한 후, 큐가 비거나 진단이 깨끗해질 때까지 반복하는 결정론적 루프입니다. 이것은 goal 엔진 위의 프리셋입니다.
...
`/goal <condition>`은 완료 조건을 설정하고, 조건이 참이 될 때까지 Claude가 프롬프트 없이 작업을 계속합니다. 각 턴 후, 작은 빠른 모델이 조건을 평가합니다.
...
{{< icon warning danger >}} 모든 루프 원시에 대해 안전 가드레일은 변경되지 않습니다.

- **Implementation Kickoff Approval** (plan → run HUMAN GATE)은 어떤 루프로도 bypass 불가합니다. `/goal`이 활성화되어 있어도 run-phase 진입 전 사용자 승인은 필수입니다.
- **안전 경계 unchanged** — 루프가 활성화되어도 "되돌리기 어려운 / 공유 시스템 작업 전 확인" 경계는 완화되지 않습니다. goal 평가자는 계속 여부만 결정하며, 파괴적 작업을 사전 승인하지 않습니다.
