...
MoAI-ADK v3의 세 가지 핵심 중 하나는 **에이전틱 하네스** (Agentic Harness)입니다. 코드를 직접 쓰는 대신, 에이전트가 잘 일할 환경(SPEC 문서, 품질 게이트, 피드백 루프)을 설계한다는 뜻입니다. 워크플로우 명령어는 이 하네스의 중심인 **plan → run → sync** 파이프라인을 실행합니다.
...
단계별 토큰 예산이 다른 것도 v3의 비용 절감(토크노믹스) 설계의 일부입니다. 계획은 깊은 추론이 필요하지만 산출물이 작고 (30K), 구현은 코드량이 많아 넉넉한 예산이 필요하며 (180K), 문서 동기화는 그 중간 (40K)입니다. 단계 사이에 `/clear`로 컨텍스트를 비우는 관행도 같은 이유에서 나옵니다. 이전 단계의 대화를 다음 단계로 끌고 가지 않아야 각 단계가 예산을 온전히 씁니다.
