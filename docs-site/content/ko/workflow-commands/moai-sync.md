...
`/moai sync`가 PR을 연 뒤, 오케스트레이터가 깨진 required check를 확인해 자동 fix 루프로 넘겨줍니다. PR을 올린 뒤 사람이 CI 화면을 들여다보고 있을 필요 없이 루프가 결과를 받아 대응합니다. 에이전틱 루프 엔지니어링이 CI 영역까지 뻗은 구조입니다.
