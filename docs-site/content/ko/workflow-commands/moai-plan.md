...
`/moai plan`은 MoAI-ADK 워크플로우의 **Phase 1 (Plan)** 명령어입니다. 자연어로 된 기능 요청을 **EARS** (Easy Approach to Requirements Syntax) 형식의 구조화된 SPEC 문서로 변환합니다. 내부적으로 **manager-spec** 에이전트가 요구사항을 분석하고, 모호함 없는 명세서를 생성합니다.
...
요구사항을 **파일로 저장**하여 영구 보존합니다. EARS 형식으로 **모호함 없이** 구조화합니다. 세션이 끊겨도 SPEC만 읽으면 **이어서 작업**할 수 있습니다.
