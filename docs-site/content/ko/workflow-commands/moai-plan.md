...
계획 단계는 v3 비용 설계에서 가장 깊은 추론이 배정되는 단계입니다. 여기서 요구사항이 명확해질수록 뒤따르는 구현 단계의 재작업과 토큰 낭비가 줄어듭니다. 그래서 MoAI-ADK는 "계획은 깊게, 구현은 싸게"라는 배분 원칙을 따르고, 생성된 SPEC은 **plan-auditor**가 독립적으로 감사합니다. 만든 에이전트가 스스로 검사하지 않습니다.
...
Plan phase에서 SPEC 문서의 요약본 (`spec-compact.md`)을 자동 생성합니다. Run phase에서 전체 spec.md 대신 요약본을 로드하여 **~30% 토큰을 절약**합니다. SPEC 라이프사이클 안에 비용 절감 장치가 내장되어 있는 대표적 예입니다.
...
MoAI-ADK v3.0.0 부터는 **GEARS**(Generalized Expression for AI-Ready Specs)를 SPEC 작성의 권장 표기법으로 도입합니다. 기존 EARS 표기법은 **6 개월** 동안 하위 호환을 유지하며, 그동안 점진적으로 GEARS로 마이그레이션할 수 있습니다. 새로운 SPEC은 처음부터 GEARS 패턴을 따르도록 권장합니다.

GEARS는 EARS 의 5 가지 핵심 패턴을 유지하면서, AI 코딩 에이전트가 더 명확히 해석할 수 있도록 의미 경계를 가다듬은 표기법입니다. 핵심 변경은 **IF/THEN 패턴의 폐기**(WHEN 으로 정규화)와 **WHERE 의 의미 재정의**(정적 전제조건/구성/기능 플래그)입니다.
...
마이그레이션 윈도우는 v3.0.0 출시 시점부터 **6 개월** 또는 `SPEC-V3R6-GEARS-SWEEP-001`(provisional) 일괄 정정 SPEC 완료 시점 중 먼저 도래하는 시점까지 유효합니다. 윈도우 동안의 동작은 다음과 같습니다.

- **비-strict 모드(기본값)**: `LegacyEARSKeyword` 코드의 warning 만 발생, lint 실패 없음
- **`--strict` 모드(opt-in)**: warning 이 error 로 승격되어 CI 차단
- **기존 88 개 SPEC**: 본 SPEC 의 범위에서 직접 수정하지 않음(REQ-GM-007). 일괄 정정은 후속 SWEEP SPEC 의 책임

### LegacyEARSKeyword 진단

`internal/spec/lint.go` 의 `isLegacyEARSPattern()` 헬퍼가 EARS legacy IF/THEN 패턴을 탐지하면 다음과 같은 메시지를 출력합니다.
...
downstream 도구(검증기, 코드 생성기, IDE 플러그인 등)에서 SPEC 텍스트를 매칭할 때는 다음과 같이 마이그레이션하세요.

- `IF .* THEN` 매칭을 향후 `WHEN .* shall` 매칭으로 전환
- 6 개월의 deprecation 윈도우를 인지하고, 윈도우 종료 시점까지 양쪽 패턴을 모두 인식하도록 구현
- `LegacyEARSKeyword` finding 코드를 upgrade 신호로 활용
...
이 정규화는 트리거를 "조건"이 아닌 "사건"으로 명시함으로써 AI 에이전트의 의도 해석 모호성을 줄이고, 테스트 케이스 작성 시 입력/검증 시점을 더 명확히 만들어 줍니다.
...
MoAI가 `AskUserQuestion`으로 질문할 때, 추천 배치를 안내하는 5가지 원칙이 적용됩니다:
...
4. **전제조건 공개** — 각 추천 옵션은 성립 전제조건을 *"Recommended when <precondition>"* 형식으로 명시하여 즉시 트레이드오프를 평가할 수 있게 합니다.

5. **숙련도 기반 적응형 강도** — 추천 강도는 세션 카운트에 따라 조절됩니다:
   - **전문가**(20+ 세션): 약한 강도 — inferred preference를 `(권장)` override 없이 공개만(info-centric, 자율성 존중)
   - **일반 사용자**(5-19 세션): 강한 강도 — `(권장)` + 투명한 근거 제시
   - **Cold-start**(<5 세션): 중립 강도 — override 없음, 시스템 기본값 적용

### 프라이버시 및 안전

- **세션 범위 토글**: `moai preference toggle`로 프로젝트별 개인화 비활성화(세션 간 비영속)
- **민감 도메인 게이트**: 보안 관련 주제(취약점, 침투 test, 유출)는 중립 추천 + 공개 로그
- **자동 감쇠**: Transient 선호는 28일 후 soft-delete, stable 선호(명시적 표시)는 보존
- **Advisory 캡처**: PostToolUse 캡처 훅은 AskUserQuestion 실행을 절대 차단하지 않음(fail-open 설계)
- **Recovery-Signal Carve-Out**: recovery 턴(compact 복구, prompt_too_long 등)에서 advisory 훅은 복구에 양보(recovery-signal carve-out 준거, doctrine-honest)

### 기술 구현

{{< callout type="info" >}}
**내부 동작**: 5가지 원칙은 `.claude/rules/moai/core/askuser-protocol.md` § Recommendation Placement Principles에 명세화되어 있으며, `moai.md`에 렌더링됩니다. 캡처 훅은 `internal/hook/user_decision_capture.go`에 구현되어 있으며 schema 허용 파싱과 도메인 분류를 지원합니다. 감쇠 정책은 power-law 함수 `(age+1)^(-0.5)`를 따르며 α=0.5 고정(Standard tier). 전체 아키텍처와 수용 기준은 프로젝트의 SPEC 문서를 참조하세요.
{{< /callout >}}
