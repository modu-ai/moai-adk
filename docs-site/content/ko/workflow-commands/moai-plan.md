...
SPEC을 만들면서 **독립된 Git 워크트리**까지 함께 만들어 병렬 개발 환경을 갖춥니다:
...
{{< callout type="info" >}}
  `--worktree` 옵션은 **여러 기능을 동시에 개발**할 때 쓸모가 있습니다. SPEC마다
  워크트리가 따로 잡히므로 서로 부딪히지 않습니다.
{{< /callout >}}
...
**--worktree**는 작업 디렉토리를 따로 하나 만들어 완전히 격리된 환경을 내줍니다. **--branch**는 지금 리포지토리에 새 브랜치만 팝니다. 여러 기능을 동시에 개발한다면 --worktree 쪽을 권합니다.
