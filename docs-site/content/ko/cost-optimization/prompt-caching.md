...
프롬프트 캐싱은 동일한 프롬프트 접두사를 여러 요청에서 재사용하여 90% 할인된
비용으로 추론 비용을 절감합니다(기본 비용의 0.1배). MoAI-ADK 토크노믹스의
"컨텍스트 다이어트" 축이 항시 로드 컨텍스트를 줄이는 쪽이라면, 프롬프트
캐싱은 남은 컨텍스트를 싸게 재사용하는 쪽입니다. 이 가이드는 손익분기 규칙,
캐시 메커니즘, 그리고 MoAI 프로젝트에서 캐싱을 언제 활성화할지 설명합니다.

## 손익분기 규칙

**1시간 캐시는 세션에서 2개 이상의 연속 API 요청이 발생할 때만 활성화하세요.**

단일 요청 세션(원샷 쿼리, 단일 턴 명령)은 2배의 기록 프리미엄이 발생하며
캐시 재사용 이득이 없어서, 요청당 비용이 캐싱 없는 기본선보다 오히려
높아집니다. 반대로 1시간 이내에 여러 턴 상호작용이나 반복 SPEC 분석이 있다면
캐싱은 두 번째 요청에서 비용을 상쇄하고 후속 요청에서 67% 이상을 절감합니다.
