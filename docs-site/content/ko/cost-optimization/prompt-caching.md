...
입력 단가의 **0.1배**로 청구되므로, 매번 되풀이되는 컨텍스트(시스템 프롬프트,
프로젝트 지침, 대화 이력)가 클수록 절감 폭도 커집니다. MoAI-ADK
...
- **접두사 매칭**: 캐시가 적중하려면 중단점 앞의 내용(도구 정의·시스템
  프롬프트·메시지 이력 포함)이 **100% 같아야** 합니다. 공백 하나만 달라도
  그 지점 뒤는 전부 다시 계산합니다.
- **가격 배수** (기본 입력 단가 대비): 캐시 기록 5분 TTL **1.25배** ·
  1시간 TTL **2배** · 캐시 읽기 **0.1배**.
- **TTL(수명)**: 기본 5분, 옵션 1시간. TTL 안에서 재사용될 때마다 수명이
  무료로 연장됩니다.
- **모델별 최소 캐시 토큰**: 이보다 짧은 접두사는 캐시에 올라가지
  않습니다(오류 없이 그냥 처리). 예: Fable 5 = 512, Opus 5·Sonnet 5 = 1,024,
...
- 응답의 `cache_creation_input_tokens`(기록) / `cache_read_input_tokens`(읽기)
  비율이 지표입니다. 읽기 쪽이 높을수록 캐싱이 잘 먹고 있다는 뜻입니다.
- MoAI statusline의 cache_hit 세그먼트로 세션 도중에도 실시간으로 확인할 수 있습니다.
- 기록 쪽이 턴마다 계속 높다면, 위의 "무효화하는 행동" 가운데 무언가가
  접두사를 건드리고 있다는 신호입니다.

## API 직접 호출 시 (참고)

아래 예시는 **Anthropic API를 직접 호출하는 개발자**만 참고하면 됩니다.
Claude Code 사용자에게는 해당하지 않습니다.
