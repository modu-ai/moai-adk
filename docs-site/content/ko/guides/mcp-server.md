...
답을 쥐고 있는 쪽은 호출자뿐입니다. 서버가 추론하는 대신 입력으로 받는 이유가 이것입니다.
...
`audit_multi`에서는 루트가 fan-out의 codex 백엔드로만 전달됩니다. GLM 백엔드는 z.ai로 나가는 HTTP 호출이라 루트를 묶을 작업 디렉터리 자체가 없고, 따라서 루트를 받지 않습니다 — GLM의 판정은 어느 트리에 대한 것도 아닙니다.
