...
{{< icon target primary >}} **MoAI-ADK** — 비용을 가르는 것은 단가가 아니라 **배정**임을 전제합니다. DeepSWE 벤치마크에서 Opus 5 의 가장 낮은 추론이 가장 높은 추론의 Sonnet 5 보다 점수가 높으면서 과제당 비용은 16분의 1 이었습니다. 재시도 루프가 청구서를 쓰지, 토큰 단가가 아닙니다. 그래서 작업마다 맞는 모델과 추론 깊이를 배정하고, 컨텍스트를 다이어트하고, 예산 초과 전에 멈춥니다. CG는 폐기되었습니다. `moai migrate cg`로 이전 선택지를 먼저 확인하세요.
