...
MoAI-ADK를 처음 만나는 분을 위한 온보딩 경로입니다. **소개 → 설치 → 빠른 시작** 순서로 읽으면 30분 안에 첫 MoAI-ADK 프로젝트를 실행할 수 있습니다. 설치는 단일 바이너리 하나를 받는 것이 전부이고, 첫 SPEC 실행까지 별도 런타임이나 의존성은 필요 없습니다.

{{< callout type="info" >}}
이미 설치를 마쳤다면 [빠른 시작](./quickstart)으로 바로 이동하세요. CLI 플래그가 궁금하면 [CLI 레퍼런스](./cli)를, 문제가 있다면 [자주 묻는 질문](./faq)을 확인하세요.
{{< /callout >}}
...
{{< callout type="info" >}}
**다음 단계**: 설치를 마쳤다면 [핵심 개념](/ko/core-concepts/)에서 v3.0의 세 기둥 — 토크노믹스 · 에이전틱 루프 엔지니어링 · 에이전틱 하네스 — 와 SPEC·DDD·TRUST 5 등 MoAI-ADK의 설계 철학을 학습할 수 있습니다.
{{< /callout >}}
