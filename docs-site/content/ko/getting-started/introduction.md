...
MoAI-ADK는 Claude Code 구독 요금제에 맞춰 에이전트에 최적의 AI 모델을 할당합니다. 요금제의 사용량 제한 내에서 품질을 극대화합니다. 계획·감사처럼 추론이 무거운 단계에는 상위 모델을, 반복 작업에는 경량 모델을 배정합니다.
...
{{< callout type="info" >}}
기본 티어는 **medium** 입니다. `low` 티어는 상위 모델 (Opus) 없이도 전체 워크플로우가 동작하도록 설계되었습니다. `max` 티어에서는 핵심 단계 (계획, 감사) 에 Opus를, 일반 작업에 경량 모델을 배정합니다. `--model-policy` 플래그 또는 초기화 마법사에서 설정합니다.
{{< /callout >}}
