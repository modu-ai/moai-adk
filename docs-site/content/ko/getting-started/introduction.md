...
- **8개 전문 에이전트**: 각 개발 단계에 특화된 AI 에이전트 팀 (7개 MoAI 커스텀 + Anthropic 내장 Explore)
- **31개 스킬**: 다양한 개발 시나리오를 지원하는 확장 가능한 스킬 라이브러리
- **다국어 지원**: 한국어, 영어, 일본어, 중국어 4개 언어 지원
- **Adaptive Thinking**: `ultrathink` 키워드를 통해 Opus 4.7+/4.8과 Sonnet 4.6의 내장 추론 모드로 복잡한 문제 분석
- **Ralph-Style LSP Integration**: LSP 기반 자율 워크플로우와 실시간 품질 피드백

## 주요 기능

MoAI-ADK는 8개의 전문화된 AI 에이전트와 31개의 스킬을 제공하여 개발 워크플로우 전반을 자동화하고 최적화합니다.

### 에이전트 카테고리

| 카테고리 | 수량 | 주요 에이전트 |
|----------|------|--------------|
| **Manager** | 4개 | manager-spec, manager-develop, manager-docs, manager-git |
| **Evaluator** | 2개 | plan-auditor, evaluator-active |
| **Builder** | 1개 | builder-harness |
| **Explore** | 1개 | Anthropic 내장 (읽기 전용 코드 분석) |
