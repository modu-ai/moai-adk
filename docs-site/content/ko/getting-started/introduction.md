...
| **에이전틱 하네스** | 코드를 직접 쓰는 대신 에이전트가 일할 환경을 설계 | 10개 에이전트 · SPEC 3-phase · TRUST 5 |
...
MoAI 오케스트레이터는 직접 구현하지 않고 10개의 전문 에이전트에게 작업을 위임합니다. 계획과 감사는 분리되어 있습니다 — 만든 사람이 검사하지 않습니다.
...
{{< callout type="info" >}}
Plus $20 요금제는 Opus를 포함하지 않습니다. **Low** 정책을 설정하면 상위 모델 없이도 사용량 제한 오류 없이 전체 워크플로우가 동작합니다. 상위 요금제에서는 핵심 단계 (계획, 감사) 에 Opus를, 일반 작업에 경량 모델을 배분합니다.
{{< /callout >}}
