...
MoAI-ADK의 인터랙티브 설정 마법사를 통해 첫 설정을 완료하세요. 언어, Git 자동화 범위, 모델 정책, 하네스 프로필을 개발 환경에 맞게 구성합니다. 여기서 정한 값은 전부 `.moai/config/sections/` 아래 YAML 파일로 저장되므로, 나중에 언제든 파일을 직접 고치거나 마법사를 다시 실행해 바꿀 수 있습니다.
