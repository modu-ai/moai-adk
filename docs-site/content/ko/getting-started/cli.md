...
MoAI-ADK 명령줄 인터페이스의 모든 명령어와 옵션을 참조하세요. 터미널의 `moai` (Go 바이너리) 와 Claude Code 대화창의 `/moai` (슬래시 서브커맨드) 는 서로 다른 도구입니다 — 이 문서는 터미널 CLI를 다룹니다.
...
MoAI-ADK는 Claude Code 구독 요금제에 맞춰 에이전트에 최적의 AI 모델을 할당합니다 — 토크노믹스의 출발점입니다. 계획·감사처럼 추론이 무거운 단계에는 상위 모델을, 반복 작업에는 경량 모델을 배정합니다.
...
Profile 설정 중 **기본 모델** 선택 시 1M 컨텍스트 변형을 선택할 수 있습니다. `[1m]` 접미사는 별도 모델이 아니라 Claude Code의 네이티브 컨텍스트 윈도우 수정자입니다:

- `opus` / `opus[1m]`
- `sonnet` / `sonnet[1m]`
- `fable` / `fable[1m]`

이 변형들은 대용량 코드베이스 분석이나 긴 문서 작업에 적합합니다.
