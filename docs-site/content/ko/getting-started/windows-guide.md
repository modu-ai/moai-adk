...
WSL은 Windows 안에서 Linux 환경을 그대로 쓰게 해 주며, MoAI-ADK의 모든 기능을 완벽하게 지원합니다.
...
가장 권장하는 방법은 WSL(아래 [WSL 설정 가이드](#wsl-설정-가이드) 참조)에서 작업하는 것입니다. WSL 네이티브 파일시스템은 비-ASCII 홈 경로 문제를 아예 겪지 않습니다.
...
> **성능 팁**: WSL 네이티브 파일시스템(`~/` 하위)에서 작업하면 파일시스템을 넘나드는 오버헤드가 없어 가장 빠릅니다.
