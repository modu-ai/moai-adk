...
신규 프로젝트와 기능 개발에 권장하는 기본 방법론입니다. 테스트를 먼저 작성합니다.
...
브라운필드 프로젝트 (기존 코드베이스) 에서는 **RED 앞에 분석 단계**가 하나 붙습니다. 테스트를 쓰기 전에 기존 코드를 읽고 현재 동작부터 파악합니다.

### DDD 방법론 (테스트 커버리지 < 10% 기존 프로젝트)

테스트 커버리지가 거의 없는 기존 프로젝트를 안전하게 리팩토링하기 위한 방법론입니다.
