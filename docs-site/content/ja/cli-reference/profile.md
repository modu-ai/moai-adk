...
{{< callout type="note" >}}
以下で説明するプロジェクト別の記憶は次のリリースに含まれます。現在の配布版 (v3.0.1) はグローバル記録 1 つ (`last_profile`) だけを残すため、プロジェクト B で `-p` によりプロファイルを指定すると、プロジェクト A が記憶していた値が上書きされます。
{{< /callout >}}
...
{{< callout type="note" >}}
以下の案内メッセージは次のリリースに含まれます。現在の配布版 (v3.0.1) は何の予告もなくログイン画面に切り替わります。
{{< /callout >}}
