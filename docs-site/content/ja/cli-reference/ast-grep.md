...
どちらも [ast-grep](https://ast-grep.github.io/) CLI（`sg`）を利用しますが、`sg` が未インストールの場合の挙動は意図的に異なります。`ast-grep` は CI のゲートにも使う検出コマンドなので、インストール案内を stderr に出力し **ゼロ以外のコードで終了** します。実行されなかったスキャンを「問題なし」と読み違えてはならないためです。`ast-edit` は置換コマンドで、「適用対象なし」は正常な無操作なので、案内を出して 0 で終了します。`sg` のインストールは [ast-grep クイックスタート](https://ast-grep.github.io/guide/quick-start.html) を参照してください。インストール状況は `moai doctor` でも確認できます。
