...
MoAI-ADK が扱うワークツリーは大きく 2 層です。どちらも最終的には git worktree (1 つの git リポジトリが複数の作業ディレクトリを束ねる根本の仕組み) の上に乗っていますが、**誰が作り、どこに置かれ、どれくらい長く生きるか**が異なります。この違いを知っておかないと「いま自分はどの worktree で作業しているのか?」を勘違いしやすくなります。

1. **L1 — `.claude/worktrees/`**: プロジェクトディレクトリの内側に作られます。Claude Code ランタイムが自ら生成し、セッションが終わるときれいな状態のツリーは自動的に整理されます。セッション単位の一時空間と考えてください。
2. **L2 — `~/.moai/worktrees/`**: プロジェクトの外、ユーザーのホームディレクトリの下に作られます。MoAI が SPEC 単位の並列開発のためにユーザーがオプトインして作り、run と sync の各段階を通じて同じツリーを再利用します。破棄はユーザーが `moai worktree done` で明示的に行う必要があります。

下の短い図で、2 層の場所と主体をひと目で把握します。より深い生成・破棄のライフサイクルと `isolation: worktree` の挙動は [プラットフォーム基礎のワークツリードキュメント](/ja/claude-code/agentic/worktrees) が扱います。

```mermaid
flowchart TD
    Repo[1 つの git リポジトリ<br/>.git オブジェクトデータベースを共有]
    Repo --> L1[L1 — .claude/worktrees/<br/>プロジェクト内、セッション単位の一時]
    Repo --> L2[L2 — ~/.moai/worktrees/<br/>プロジェクト外、SPEC 単位の持続]
    L1 --> Who1[Claude Code ランタイムが自律生成]
