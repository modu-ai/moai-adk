...
5つのツールがオプションの文字列 `project_root` を受け取ります：`spec_progress`、`spec_audit`、`spec_drift`、`codex_audit`、`audit_multi`。この呼び出しが対象とするツリーを指す値で、渡す値は呼び出し側自身の `git rev-parse --show-toplevel` の結果です。
...
答えを握っているのは呼び出し側だけです。サーバーが推論するのではなく入力として受け取る理由がこれです。
...
`audit_multi` では、ルートは fan-out の codex バックエンドにのみ転送されます。GLM バックエンドは z.ai へ出ていく HTTP 呼び出しで、ルートを結びつける作業ディレクトリ自体がないため、ルートを受け取りません — GLM の判定はどのツリーについてのものでもありません。
