...
`/moai review` は **read-only、レポート専用のツール**です — 欠陥を見つけて報告するだけで、いかなるファイルも修正しません。見つかった問題を実際に直すには `/moai fix` (または `/moai loop`) に渡します。つまり `/moai review` で問題を**報告**し、`/moai loop` で有限のイシュー集合を**修正**する階層関係です。
