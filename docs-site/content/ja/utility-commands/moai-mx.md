...
エージェントがコードを理解するためにかかるコストは、すなわちコンテキスト (トークン) です。@MX タグは「この関数は 8 箇所から呼ばれるのでシグネチャをむやみに変えないこと」といった文脈をコードのそばに直接埋め込んでおき、エージェントが毎回コードベース全体を再分析しなくてもよいようにします。ハーネスエンジニアリングの観点では、これは **コードに埋め込むアンカー** です。反復探索コストを 1 回のアノテーションで置き換えるトークノミクス装置となります。
