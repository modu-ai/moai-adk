...
| 収集項目 | 説明 | 例 | 収集方式 |
|-----------|------|------|-----------|
| MoAI-ADK バージョン | 現在インストールされているバージョン (`moai version`) | v3.1.3 | 保証 (常に収集) |
...
`/moai feedback` は次の詳細動作で Issue 作成プロセスを補強します。
...
起草されたレポートが `gh issue create` で出ていく前に、**スクラビング契約**が動作します。マスキングスクラバーがシークレット・トークン・パスのような機微値を覆い、脆弱性分類器が本文をセキュリティ脆弱性の報告と読むかを判定し、覆った値の一覧 (マスクログ) と送信に失敗した項目 (リトライキュー) をそれぞれ記録として残します。この手順はターミナルから `moai feedback scrub` (マスクのプレビュー) と `moai feedback queue` (リトライキュー確認) の動詞で直接確認できます。

分類器が本文を読むのは**マスキングより前**です。脆弱性報告はその徴候となる文字列で認識されるのに、マスキングがその文字列を消してしまうと、事後分類はまさに「公開してはならない経路」でのみ静かに失敗します。だから順序は固定されています。

この契約はスキル本文が従う規約であって、サンドボックスではありません。GitHub で手で Issue を開けば 3 つ (マスキング・分類・確認質問) すべてが回避されます — 公開リポジトリに直接書くときは同じ注意があなたのものです。
