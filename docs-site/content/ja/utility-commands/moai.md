...
4. **オーケストレーションモードの選択** (Phase 4): 6 モードカタログ (trivial / background / agent-team(引退) / parallel / sub-agent / workflow) から自律選択

つまり `/moai "ログインバグを直して"` のようにサブコマンドなしで自然言語だけ入力しても、意図分析を経て適切なワークフロー (修正なら fix 系列、新機能なら plan→run→sync パイプライン) につながります。

### パイプラインゲート

基本パイプラインは 4 つの名前付きゲートを順に通過します:

1. **Plan-audit ゲート** (plan-auditor): SPEC 計画産出物を独立して監査し、FAIL または INCONCLUSIVE なら中断
2. **実装着手承認** (plan→run ヒューマンゲート): パイプライン進入時に正確に 1 回、点数に関わらず常にユーザー承認を受ける
3. **Phase 4 モード選択** (6 モードカタログ): 実装着手承認後に自律選択、progress.md に記録
4. **Sync-audit ゲート** (sync-auditor): 同期結果を 4 次元で評価し、FAIL または INCONCLUSIVE ならチェーンを中断
...
| `--max N`           | ループ反復上限の指定 (デフォルト値 100) | `/moai "機能" --loop --max 20` |
...
flowchart TD
    ...
    subgraph D["Phase 0: 並列探索 (15-30 秒)"]
        ...
        D2["WebSearch / WebFetch / Context7 MCP<br/>外部ドキュメント調査"]
        D3["LSP / リント / カバレッジ測定<br/>品質基準線の確認"]
    end
...
- **Phase 0 (並列探索)**: Explore サブエージェントと外部検索・品質基準線測定が同時に実行され 2-3 倍の速度向上
...
Explore サブエージェント (Anthropic ビルトイン、読み取り専用) がコードベースを分析している間、オーケストレータが外部ドキュメント調査 (WebSearch/WebFetch/Context7 MCP) と品質基準線測定 (LSP/リント/カバレッジ) を同時に回してプロジェクトの文脈を素早く把握します:

| 主体                      | 役割            | 作業                                     |
| ------------------------- | --------------- | ---------------------------------------- |
| **Explore** (サブエージェント) | コードベース分析 | 関連ファイル、アーキテクチャパターン、既存実装の発見 |
| **WebSearch / WebFetch / Context7 MCP** | 外部ドキュメント調査  | 公式ドキュメント、API ドキュメント、類似実装の例      |
| **品質基準線測定** (オーケストレータ直接) | 品質基準線     | テストカバレッジ、リント状態、技術的負債    |
...
**[HARD] Task\* ツールが必須:** すべての作業追跡は `TaskCreate` / `TaskUpdate` / `TaskList` / `TaskGet` で行います。この 4 つのツールはセッション開始時にスキーマがロードされない **遅延 (deferred) ツール** なので、初めて使う前に `ToolSearch(query: "select:TaskCreate,TaskUpdate,TaskList,TaskGet", max_results: 5)` を一度実行してスキーマを引き込む必要があります (`AskUserQuestion` も同じ遅延ツールです)。

- 課題発見時: `TaskCreate` (pending 状態)
- 作業開始前: `TaskUpdate` (in_progress 状態)
- 作業完了後: `TaskUpdate` (completed 状態)
...
[完了]
  ...
  → 完了報告書 (Completion Report) バナーで作業完了を明示
```
