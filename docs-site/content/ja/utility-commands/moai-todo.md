...
flowchart TD
    ...
    Queue --> Pick["/moai todo next<br/>人がひとつ選択"]
...
| `/moai todo done <n>` | `n` 番の項目を削除します(別の場所で終わった、不要になった場合)。何を消したかを表示します。 |
| `/moai todo next` | キューを選択肢として表示し、人が次のカードを選べるようにします。 |
...
| `spec_id` | 項目を選んで SPEC を作成した後に埋まります。それまでは `null` で、この値の有無が「まだバックログの項目」と「すでにボード上のカード」を分けます。 |
| `state` | `queued` · `picked` · `dropped`。選んだ項目もファイルに残り、何が進行中かが見えます。カードが `done` に届くときに削除されます。 |
...
`/moai todo next` はキューを選択肢として表示します。古いものからひと項目ずつ、ツールが許す4つまで表示し、残りは本文に要約して何も隠されないようにします。リードセッションが `/clear` 後の初手としてキューを提示するときも同じ方法です。
