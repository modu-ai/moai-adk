...
| `moai todo done <n> [--expect <prefix>] [--require-landed]` | `n` 番の項目をロックの下で生きているキューから外します。明示的な `t<n>` id 推奨 — 同時追加で位置が動きうるためです。カードとそれを指すすべての記録は**削除ではなくアーカイブ**され、`undone` が両方を復元します。アーカイブされた行は `list` · `next` · `why` · `analyze` および重複判定の入力には現れません。`--expect <prefix>` はカードのテキストがその接頭辞で始まらない限り拒否します — 誤ったカードを閉じる事故への安全策です。`--require-landed` は着地判定に問い合わせ、「着地していない」という肯定的な答えが返ったときだけ拒否し、判定が不確実なときは通過させます — この判定は設定された基準 ref 上のコミットがカードを言及しているかだけを尋ねるもので、尋ねられること自体がその答えの正しさを保証するわけではありません。フラグがなければ `done` は着地クエリを一切実行せずコストも増えず、どの拒否経路でもキューの記録はバイト単位で変わりません。成功したすべての `done` は標準出力に着地判定トークンをちょうど一つ残します — `done <id> landing=landed|not-landed|unknown` — フラグがなくても同様で、その場合はクエリが実行されていないため `unknown` になります。判定が不確実で通過した場合も同じ `unknown` になるため、「通過した」と「クエリが一度も走らなかった」がもはや標準出力上で同じバイト列にはなりません。 |
