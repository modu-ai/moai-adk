...
1. **--branch**: 従来の feature ブランチの生成
2. **フラグなし** (デフォルト): SPEC のみ生成、BODP ゲートでユーザーがブランチ戦略を選択

`--no-issue` はブランチ戦略と独立したオプションで、GitHub Issue 生成ステップ (Phase 12) を省略します。

### ワークツリーで計画するには

plan は作業スペースを生成しません。隔離された環境で計画するには、**先にワークツリーへ入ってから** plan を実行します:

```bash
moai cc -w payment          # その場でワークツリーに入る
> /moai plan "決済システムの実装"
```

現在のセッションを保ったまま新しい tmux ウィンドウで開くには `--spawn` を付けます:
...
{{< callout type="info" >}}
  **複数の機能を同時に開発** するときは、機能ごとにワークツリーを分ければ互いに衝突
  しません。入るのはランチャーの役目で、plan はその中でそのまま動きます。
{{< /callout >}}
...
### Q: ワークツリーで作業するにはどうしますか?

plan にワークツリーを生成するフラグはありません。ランチャーで**先に入ってから**(`moai cc -w <名前>`)plan を実行します。**--branch** は現在のリポジトリに新しいブランチを作るだけの別オプションです。複数の機能を同時に開発するなら、ワークツリーに入るほうが互いに衝突しません。
...
1. **Fisher 情報タイミング** — 不確実性が最も高いとき (p≈0.5、Fisher 情報 I=p(1−p) が最大の決定境界) に質問を提示します。p≈0 または p≈1 (ほぼ確定) のときはシステムが自動処理し質問を省略します。
