...
`/moai run` は MoAI-ADK ワークフローの **フェーズ 2 (Run)** コマンドです。フェーズ 1 で作成された SPEC 文書を読み、**ANALYZE-PRESERVE-IMPROVE** サイクルを通じて既存の機能を壊さずに安全にコードを実装します。内部的には **manager-develop** エージェントが全プロセスを管理します。

{{< callout type="info" >}}
**家のリフォームで理解する DDD**

DDD の ANALYZE-PRESERVE-IMPROVE サイクルは **家のリフォーム** のようなものです:
...
{{< callout type="warning" >}}
  `/moai run` を実行する前に必ず `/clear` を実行してください。Plan フェーズで使用したトークンをクリアして、Run フェーズで **コンテキストウィンドウを十分に活用**（Anthropic API の Sonnet 5 / Opus は最大 1M、ゲートウェイ・旧モデルは 200K）できるようにする必要があります。
{{< /callout >}}
...
**manager-spec** サブエージェントが以下のタスクを実行します:
...
**sync-auditor** サブエージェントが TRUST 5 検証を実行します:
...
**ステップ 3: manager-develop が自動的に実行するタスク**

manager-develop エージェントが SPEC を実装するために実行する 4 つのフェーズです。
...
manager-develop エージェントが **自動的に進行状況を保存** します。`/clear` 後にもう一度 `/moai run SPEC-XXX` を実行すると、SPEC 文書を基に作業を継続できます。

### Q: テストカバレッジ 85% を達成するのが難しい場合は?

`quality.yaml` でカバレッジ目標を調整できますが、**推奨しません**。
85% はコアロジックがテストされたことを保証する最低基準です。カバレッジが不足している場合、manager-develop が不足しているテストを自動的に追加します。
...
Run フェーズ開始時に SPEC 複雑度に応じて品質パイプラインの深さを自動決定します。

| レベル | 対象 | evaluator | スキップフェーズ |
|------|------|-----------|---------------|
| **minimal** | シンプルなバグ修正、設定変更 | 非活性 | 0、0.5、2.0、2.5、2.75、2.8a |
| **standard** | 一般的な機能開発 (デフォルト) | final-pass (フェーズ 2.8a のみ) | なし |
| **thorough** | セキュリティ/決済など重要な機能 | per-sprint (フェーズ 2.0 + 2.8a) | なし |

失敗時自動エスカレーション: minimal → standard → thorough (最大 2 回)

### フェーズ 0.9: JIT 言語検出 (言語自動検出)

プロジェクトの主要言語を自動検出してエージェントスポーン時に適切な言語スキルを挿入します。
...
### フェーズ 0.95: スケール基準モード選択 (規模ベースモード選択)

SPEC 規模に応じて最適な実行モードを自動選択します。
...
| マルチドメイン | ファイル ≥ 10 または ドメイン ≥ 3 | **Full Pipeline** |
| 大規模変更 | complexity ≥ 7 + --team | **Team Mode** |

### フェーズ 2.0: Sprint Contract (thorough 専用)

thorough レベルでのみ実行されます。sync-auditor と実装前に Done 基準を事前に合意します。

**契約内容:**
- 通過すべき具体的なテストケース
- 識別されたエッジケース
- ハード閾値 (カバレッジ %, パフォーマンス目標、セキュリティ要件)

最大 2 ラウンドの交渉後、evaluator 推奨案で確定されます。

### フェーズ 2.8a/2.8b 分離

既存フェーズ 2.8 が 2 つのステップに分離されました:

- **フェーズ 2.8a**: sync-auditor 能動評価 (Functionality/Security/Craft/Consistency)
- **フェーズ 2.8b**: sync-auditor TRUST 5 静的検証 (既存動作)

{{< callout type="warning" >}}
Security FAIL = 全体 FAIL。最大 3 回の修正・評価サイクル後、ユーザーに報告されます。
{{< /callout >}}

### Drift Guard (スコープ逸脱検出)

DDD/TDD サイクル完了時に計画比実際変更を比較します。

- drift ≤ 20%: 情報記録のみ
- 20% < drift ≤ 30%: 警告
- drift > 30%: フェーズ 2.7 再計画ゲート トリガー

### tasks.md 永続化アーティファクト

タスク分解を `.moai/specs/SPEC-{ID}/tasks.md` に記録します。Git 追跡可能で Drift Guard で参照されます。

### spec-compact.md

Run フェーズ進入時に SPEC 要約版を自動ロードして、約 30% トークンを節約します。`.moai/specs/SPEC-{ID}/spec-compact.md` が存在すれば全 spec.md の代わりに使用されます。

## 関連ドキュメント

- [ドメイン駆動開発](/core-concepts/ddd) - ANALYZE-PRESERVE-IMPROVE サイクル詳細説明
- [TRUST 5 品質システム](/core-concepts/trust-5) - 品質ゲート詳細説明
- [/moai plan](./moai-plan) - 前のフェーズ: SPEC 文書作成
- [/moai sync](./moai-sync) - 次のフェーズ: 文書同期と PR
