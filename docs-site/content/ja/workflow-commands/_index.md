...
`/moai` オーケストレーターは 16 個のサブコマンドをルーティングします。このセクション (ワークフロー) は SPEC 3-Phase ライフサイクルのコマンドを、[ユーティリティコマンド](/ja/utility-commands/) セクションは自動化・修正ループ・コード管理・フィードバックのコマンドを扱います。
