...
## `/cd` キャッシュ保持再開 (CC 2.1.169+)

ディレクトリ境界をまたいで多段階ワークフローを再開する際 (例: run と sync の間に L2 worktree に入る)、Claude Code 2.1.169+ は `/cd <path>` を提供します — セッションの作業ディレクトリを **プロンプトキャッシュを保持したまま** 切り替えるコマンドで、蓄積された推論コンテキストが cwd 変更時に再構築される代わりに保持されます。これは新しいターミナルを開くことに対するキャッシュ保持の代替です: `/cd` はコンテキストを保持し、新しいターミナルはコールドスタートします。run フェーズのコンテキストを保持したまま L2 worktree に sync フェーズで入る場合、`/cd <worktree-path>` が低摩擦なパスです。切り替えが `cwd` フィールドに反映される仕組みは [Statusline ガイド](/ja/advanced/statusline) を参照してください。
