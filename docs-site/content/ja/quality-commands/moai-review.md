...
flowchart TD
    ...
    Single --> Quality["sync-auditor<br/>4観点の順次分析"]
...
| **sync-auditor** | コード品質分析 (デフォルトモード) | 4観点の順次分析 |
...
デフォルトモードは `sync-auditor` エージェントが4つの観点を順次分析します。`--team` モードは4人の専門レビュアーが同時に分析するため、より深い分析が可能ですが、トークン消費は約4倍になります。
