...
Claude Code のネイティブなチームメイトランタイムと、MoAI で廃止された静的 Agent Teams オーケストレーションは別の機能です。このページのネイティブ API の説明は、廃止された CG ランチャーが利用できることを意味しません。 CG は廃止されました。`moai migrate cg` で移行先を確認してください。
