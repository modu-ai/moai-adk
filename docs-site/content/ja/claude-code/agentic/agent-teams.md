...
エージェントチームは **実験的な機能であり、デフォルトで無効化** されています。Claude Code v2.1.32 以上が必要で、環境変数 `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` を `1` に設定して有効化します。シェル環境に直接指定するか、`settings.json` に登録します。
...
エージェントチームは 2 つの表示モードをサポートします。**インプロセス (in-process)** はすべてのチームメイトがメインターミナル内で動作し、どのターミナルでも追加設定なしに使えます。**スプリットペイン (split panes)** はチームメイトごとに別の窓を開き、tmux または iTerm2 が必要です。デフォルトは `"auto"` で、tmux セッション内で実行中ならスプリットペインを、そうでなければインプロセスを使います。

モードは `~/.claude/settings.json` の `teammateMode` キーで指定するか、単一セッション限定で `--teammate-mode in-process` フラグによって強制できます。
