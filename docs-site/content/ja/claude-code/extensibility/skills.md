...
CLAUDE.md の内容は常にコンテキストに常駐しますが、スキル本文は実際に使用されるときだけロードされます。そのため、長く詳細な参照資料を置いておいても、必要になるまではトークンコストがほとんどかかりません。

### スキルとカスタムコマンド

スキルより前は、`.claude/commands/` ディレクトリでユーザー定義コマンドを置いていました。現在は、**スキルがコマンド機能を含む** ため、`.claude/commands/deploy.md` と `.claude/skills/deploy/SKILL.md` の両方がある場合、スキルが優先します。既存のコマンドファイルも動作しますが、新しい拡張はスキルで作成することが推奨されます。
...
description: このコードベースの API 設計パターン。エンドポイントを作成またはレビューするときに使用します。
...
```yaml
---
description: コミットされていない変更を要約し、リスク要素を表示します。何が変わったかを尋ねるときに使用します。
---
...
上記の変更を 2～3 個の箇条書きで要約した後、漏れたエラー処理やハードコーディングなどのリスクを列挙します。
```
