...
| `Ctrl+X Ctrl+K` | すべてのバックグラウンドサブエージェントを停止 |
| `Opt+P` | モデルを切り替え |
| `Opt+O` | 高速モードをトグル |
...
- **`/recap`**: セッションの要約（session recap）を生成します。自動的には、3分以上または3ターン以上進行したセッションで有効化されます。
- **タスクリスト**: 多段階タスクで Claude が作成したタスクリストを `Ctrl+T` で展開・折りたたみします。タスクリストはコンテキスト圧縮中にも保持されます。
