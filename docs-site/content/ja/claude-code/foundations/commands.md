...
スラッシュコマンドは 3 つの種類で構成されます。よく使うコマンドを整理すると以下のとおりです。全リストは入力欄で `/` を入力するか、公式コマンドリファレンスの [code.claude.com/docs/en/commands](https://code.claude.com/docs/en/commands) を参照してください。
...
| `/workflows` | ダイナミックワークフロー実行リスト管理 UI | v2.1.139+ |
| `/rewind` (別名: `/checkpoint`, `/undo`) | コードと会話を以前のチェックポイントに戻す | v2.1.191+ |
| `/context [all]` | 現在のコンテキストウィンドウ使用量を分析 | デフォルト |
...
| `/model` | AI モデル選択 | デフォルト |
| `/background` (別名: `/bg`) | バックグラウンド実行 | v2.1.139+ |
| `/fork <directive>` | 会話を継承するフォークサブエージェント | v2.1.161+ |
| `/recap` | セッション要約 | デフォルト |
| `/btw` | サイド質問 | v2.1.187+ |
| `/cd` | セッション作業ディレクトリ変更、プロンプトキャッシュを保存 | v2.1.169+ |
| `/schedule` (別名: `/routines`) | 予約タスク | v2.1.72+ |
| `/branch`, `/tasks`, `/plan`, `/doctor`, `/skills`, `/reload-skills`, `/reload-plugins` | その他の管理コマンド | デフォルト |

### スキルコマンド `[Skill]`

| コマンド | 用途 |
| :--- | :--- |
| `/loop` (別名: `/proactive`) | 反復ループを実行（Ralph/インターバルベース） |
| `/batch` | バッチ実行 |
...
| コマンド | 用途 |
| :--- | :--- |
| `/deep-research` | 複数の Web 検索を並列実行し、結果を交差検証するリサーチ（WebSearch 必須） |

### コマンド可用性の注記

- 同じ機能を複数の名前で呼べる場合が多くあります（別名）。
- 一部のコマンドはプラットフォーム、プラン、環境によって表示の有無が変わります。
- `ultracode` は現在のワークフロートリガーキーワード（pre-v2.1.160 では `workflow` でした）であると同時に `/effort` レベルです。
