...
MoAI-ADK をシステムにインストールする方法を案内します。インストールされるのは Go でビルドされた単一バイナリ 1 つだけです。Python も仮想環境もパッケージマネージャーも必要ありません。
...
{{< callout type="info" >}}
Go エディション (v2.0+) は単一バイナリで、Python ランタイムや仮想環境が必要ありません。起動時間は約 800ms から 5ms へと大きく短縮されました。
{{< /callout >}}
...
詳しい内容は [アップデートガイド](https://adk.mo.ai.kr/cli-reference/update) を参照してください。
