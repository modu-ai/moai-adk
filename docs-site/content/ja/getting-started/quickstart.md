...
テストを先に書き、そのテストを通していく RED-GREEN-REFACTOR サイクルで実装します。各ステップが何を意味するかは [SPEC ベース開発](/ja/core-concepts/spec-based-dev) で扱います。

#### DDD モード (既存プロジェクト / テストカバレッジ 10% 未満)

既存の動作を特性テストで押さえたうえで少しずつ改善していく ANALYZE-PRESERVE-IMPROVE サイクルで進めます。詳しくは [DDD](/ja/core-concepts/ddd) で扱います。
...
| 複雑な機能 | `/moai` | 自動最適化 |
| 並列開発 | `moai cc -w <名前>` でワークツリーに入る | 独立環境の保証 |
