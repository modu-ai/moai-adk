...
MoAI-ADK は **コスト** (トークノミクス) · **自己改善** (エージェンティック・ループ・エンジニアリング) · **品質統制** (エージェンティック・ハーネス) の 3 つの軸で Claude Code を包む Agentic Development Kit です。同じ品質のコードをより少ないトークンで、完了条件を宣言するとループが自ら働き積み重なった観察をハーネス学習の原料に上げ、SPEC 3-phase と TRUST 5 ゲートで「終わり」を証拠で判定します — モデル選択・推論深度・コンテキスト使用量はシステムが管理します。Go で書かれた単一バイナリなので依存性なしで実行できます。
...
{{< callout type="info" >}}
**コスト軸 (トークノミクス) の実践ツール:** z.ai GLM は Claude Code と完全互換の AI バックエンドです。**CG モード** (`moai cg`、tmux 必須) では Claude リーダーがオーケストレーション・アーキテクチャ決定・コードレビューを担い、GLM チームメイトが実装・テスト・ドキュメント化を並列に処理することで、実装中心の作業で **50~70% のトークンを節約** します。アーキテクチャ設計やセキュリティレビューのように深い推論が必要なときは Claude 専用 (`moai cc`) を使います。
...
## 自己改善軸 — ループが自ら働き、ハーネスが学習する

{{< callout type="info" >}}
**自己改善軸 (エージェンティック・ループ・エンジニアリング) の実践ツール:** 完了条件を宣言すると条件が満たされるまでセッションが自ら働きます。`/moai goal "<条件>"` は条件宣言型の自律ループ、`/moai loop` は LSP 診断・AST-grep・リンターが見つけた課題キューを全部空にするまで反復修正 (デフォルト最大 10 回)、`/moai fix` は単一パスの自動修正です。ループが残した観察 (ユーザー修正、失敗パターン、ルーティング決定) は 4 階層の学習のはしご (観察 → ヒューリスティック → ルール → 自動アップデート、ユーザー承認ゲートの下) に沿ってハーネス指針へ積み重なります — 次のセッションは前のセッションの失敗を反復しません。
{{< /callout >}}
