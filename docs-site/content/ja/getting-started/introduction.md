...
{{< callout type="info" >}}
**自己改善 (エージェンティック・ループ・エンジニアリング) の実践ツール:** 完了条件を宣言すると条件が満たされるまでセッションが自ら働きます。`/moai goal "<条件>"` は条件宣言型の自律ループ、`/moai loop` は LSP 診断・AST-grep・リンターが見つけた課題キューを全部空にするまで反復修正 (デフォルト最大 10 回)、`/moai fix` は単一パスの自動修正です。ループが残した観察 (ユーザー修正、失敗パターン、ルーティング決定) は 4 階層の学習のはしご (観察 → ヒューリスティック → ルール → 自動アップデート、ユーザー承認ゲートの下) に沿ってハーネス指針へ積み重なります — 次のセッションは前のセッションの失敗を反復しません。
{{< /callout >}}
