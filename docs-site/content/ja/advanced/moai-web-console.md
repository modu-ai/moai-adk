...
{{< callout type="info" >}}
ループバック専用が認証なしの前提です。リバースプロキシや `0.0.0.0` バインドで外部に公開する構成はサポートしません。リモートから見る必要があれば SSH トンネルでローカルポートを転送してください。SSH トンネルを使う場合、ブラウザは引き続き `localhost` アドレスでコンソールを開くため、この検査下でも動作します。
{{< /callout >}}
