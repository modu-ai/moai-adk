...
8. **Codex** — 散らばっている codex 設定 12 個を 1 画面にまとめて見せる**読み取り専用のミラー**。監査バックエンドと codex の固定値、codex を使うかどうか、codex 関連の MCP ツールトグル、検出された実行ファイルが一箇所に並びます。ここで値を直すことはなく、各行はその値を実際に持つタブへつながります
...
12. **交差セッション（Cross-Session）** — セッション間メッセージングの受信姿勢(posture): インバウンド処理方式(`accept` · `hold` · `refuse`)、マシン間送信の隔離、保留対話の期限。`crosssession.yaml` を編集し、ランチャーが次回の `moai cc`/`glm` 実行からこの値をセッションに注入します — すでに動いているセッションは起動時の姿勢を保ちます
