...
| 設定（Settings） | `/settings` | プロファイル設定とプロジェクトセクションの編集 |
...
キューが存在しない場合や、正常に読み取ったキューにカードがない場合は、HTTP 200 と空のキューの案内を返します。読み取りに失敗した場合も HTTP 200 を返しますが、0 件と表示せず、読み取れなかったことを案内します。保存層の生のエラーは表示しません。

## リアルタイム更新 — 信号だけ送り、取り直す

観測領域は監視中のファイルが変わると更新されます。サーバーは `GET /events` で SSE ストリームを開き、下表のプロジェクト内のパスと解決されたホームディレクトリを監視します。ファイル変更の通知は 250 ミリ秒単位でまとめて送ります。
...
| `kanban` | `.moai/state/todo`、解決されたホーム内の Todo ディレクトリ（`~/.moai/db/<project-key>/todo`）、解決された Factory ディレクトリ |
| `config` | `.moai/config/sections` |

`config` イベントだけは扱いが異なります。設定を編集している最中に画面が下から変わると入力中の値が消えてしまうため、更新はせず「設定ファイルが変わった」というバナーだけを出します。

起動時に存在しなかったディレクトリは、1 秒ごとに監視の登録を再試行します。新しいディレクトリを登録すると、SSE 接続が正常な場合も更新信号を送ります。接続が切れると表示灯が切断状態になり、再接続に 3 回失敗すると、ブラウザは 30 秒間隔のポーリングに切り替わります。
...
8. **Codex 設定（Codex）** — 散らばっている codex 設定 12 個を 1 画面にまとめて見せる**読み取り専用のミラー**。監査バックエンドと codex の固定値、codex を使うかどうか、codex 関連の MCP ツールトグル、検出された実行ファイルが一箇所に並びます。ここで値を直すことはなく、各行はその値を実際に持つタブへつながります
9. **エージェント（Agents）** — エージェントごとのプロファイル・モデル割り当て
10. **レポート（Report）** — レポート形式と出力の設定
11. **MCP** — `moai mcp-server` のツールごとの有効トグル。書き込み可能なツールには区分マークが付きます
12. **セッション間メッセージ（Cross-Session）** — セッション間メッセージングの受信姿勢(posture): インバウンド処理方式(`accept` · `hold` · `refuse`)、マシン間送信の隔離、保留対話の期限。`crosssession.yaml` を編集し、ランチャーが次回の `moai cc`/`glm` 実行からこの値をセッションに注入します — すでに動いているセッションは起動時の姿勢を保ちます
13. **フィードバック** — フィードバックワークフローが issue を出す対象リポジトリと、送信前の確認トグル
14. **品質ゲート** — コミット時の heavy ゲートを動かすかどうか。ランナーは `MOAI_PRECOMMIT=1` のときだけこの値を見ます

各タブの横の数字は、そのタブがレンダーするフィールド数です。エラーのあるタブには数字の代わりに警告マークが付くので、どのタブを開くべきかが一覧からすぐ分かります。

### Codex タブが読み取り専用である理由

codex の設定はもともと監査タブと MCP タブに分かれていました。「このプロジェクトは codex をどう使っているのか」を知るには 2 つのタブを行き来し、一度も一緒には表示されない値を読む必要がありました。Codex タブはそれらを 1 画面にまとめます。

**まとめて見せるだけで編集させないのは、未完成ではなく選んだ折衷です。** 設定領域のパネルはすべて 1 つの `<form>` の中にあり、保存処理は同じ名前で送られてきた値のうち**最初の 1 つだけ**を読みます。そのため同じ設定を 2 つのパネルでコントロールとして描くと、後ろのパネルで直した値が**警告もなく捨てられます**。bool 設定はさらに厄介で、付随する隠しフィールドだけが単独で置かれると *明示的な false* として読まれるため、ページを開いて保存しただけで MCP ツールのトグルが 6 つ無効になります。

そこで Codex タブは値とリンクだけをレンダーし、名前を持つ入力要素を 1 つも作りません。すべての設定は元のタブに宣言されたまま残り、編集もそこで行います。この画面を後から「編集できるように直す」ことは、上の損失をそのまま呼び戻すことです。

**新しく増えた設定は 1 つもありません。** 新しい設定キーも、新しい保存経路も、`.moai/config` 以下の新しいファイルもありません。設定はすでにあったもので、新しいのはそれをまとめて見る場所だけです。

MCP ツールのトグルはディスク上で空であることが多く、コンソールは空の値を**有効**として読みます。このタブはその空白を自分で解釈して二つ目の分類器になる代わりに `(未設定)` と書き、「値が空なら有効として扱い、明示的な `false` だけがツールを無効にする」という事実をグループ見出しに併記します。
...
推論強度のランタイム伝達チャネルはセッション単位の環境変数 1 つだけなので、ティアごとに書いた推論強度は**保存専用**です。設定には残りますが、ランタイムはセッション単位の値だけを読みます。GLM設定タブには適用元を明示するバッジがあり、この事実が示唆ではなく明示として現れます。
...
**ループバック専用。** コンソールは `127.0.0.1` にのみバインドされるため、リモートホストからは届きません。ただしループバックのポートはアカウントごとに分かれていないので、同じマシンにログインしている別アカウントからは接続できます。

**データベースなし。** 別途 DB を立てません。読み書きする値は現在のプロジェクトの `.moai/` 配下のファイルがすべてです。

**認証なし。** ログインやトークンのレイヤーはありません。ループバックへのバインドはリモートホストを締め出しますが、同じマシンの別アカウントは締め出せません。複数人で使うマシンでは、コンソールが起動している間はそれらのアカウントからも接続できます。

**コマンド実行なし。** 観測領域は GET 以外のメソッドを拒否し、どの画面もサーバー上でコマンドを実行しません。SPEC の status 遷移もコンソールは行いません — 遷移の所有者は各フェーズのマネージャーエージェントです。

**すべてのリクエストへの Host 検査。** 静的ファイルを含むすべてのルートで、メソッドを問わず `Host` ヘッダーがループバック名(`localhost`・`127.0.0.1`・`[::1]`、ポートの有無は問わない)でない場合、またはヘッダー自体がない場合は 403 で拒否されます。ループバックへのバインドだけでは DNS リバインディングを防げないためです — 別サイトのページがブラウザを `127.0.0.1` に接続させつつ `Host` に攻撃者のドメインを乗せると、ブラウザはそのページに応答を読ませてしまいます。同一オリジン(`Sec-Fetch-Site`)検査は引き続き状態を変更するリクエストにのみ適用されます。

{{< callout type="info" >}}
ループバック専用が認証なしの前提です。リバースプロキシや `0.0.0.0` バインドで外部に公開する構成はサポートしません。リモートから見る必要があれば SSH トンネルでローカルポートを転送してください。SSH トンネルを使う場合、ブラウザは引き続き `localhost` アドレスでコンソールを開くため、この検査下でも動作します。ただしこれは実際の SSH サーバーではなく、ポート転送をそのまま模した試験環境で確認した結果であり、検証済みの保証ではなく想定される挙動として理解してください。
{{< /callout >}}
