...
12. **交差セッション（Cross-Session）** — セッション間メッセージングの受信姿勢(posture): インバウンド処理方式(`accept` · `hold` · `refuse`)、マシン間送信の隔離、保留対話の期限。`crosssession.yaml` を編集し、ランチャーが次回の `moai cc`/`glm`/`cg` 実行からこの値をセッションに注入します — すでに動いているセッションは起動時の姿勢を保ちます
