...
weight: 85
draft: false
description: "5 つの領域（概要・カンバン・SPEC・モニター・設定）で構成されたローカル運用コンソール — リアルタイム更新、読み取り専用の観測、設定編集。"
---
# MoAI Web Console
...
{{< callout type="info" >}}
**一言でいうと:** コンソールは 4 つの観測領域と 1 つの設定領域を左レールにまとめた運用シェルです。観測領域は読み取りのみを行い、設定領域はターミナルウィザードと同じ検証・永続化レイヤーを使います。
{{< /callout >}}

## 運用シェル — 左レールとアプリバー

画面は 3 つに分かれます。左の**レール**に 5 つの領域が縦に並び、上の**アプリバー**に現在の画面タイトルと状態が乗り、残りが**本文**です。どの領域にいてもレールとアプリバーは同じ位置に残ります。
...
| 設定（Settings） | `/settings` | プロファイル設定とプロジェクトセクションの編集（9 タブ） |

アプリバー右側に何が出るかは領域によって変わります。観測領域 4 つでは**リアルタイム表示灯**が、設定領域では**保存クラスター**（変更件数と保存ボタン）が置かれます。コンテキストチップ（`lang` · `model` · `effort` · `dev`）は設定領域だけにレンダーされます — 編集中のプロファイルの主要な値を保存前に目で確認するためのものだからです。
...
```mermaid
flowchart TD
    Rail["左レール<br/>概要 · カンバン · SPEC · モニター · 設定"]
    Top["アプリバー<br/>タイトル · リアルタイム表示灯 / 保存クラスター"]
    Body["本文<br/>領域ごとの画面"]
    Read["読み取り専用領域<br/>概要 · カンバン · SPEC · モニター"]
...
その下の**カンバンチェーンバー**は、現在のカードが `lead → plan → run → review → sync` の 5 役割をどこまで通過したかを 1 行で示します。セッションのない役割があれば、その地点をチェーンが止まった場所として表示します。続いて**進行中 SPEC** の一覧、**要注意**パネル（MUST-FIX drift・失敗した検証・停滞したゴール・空の役割だけを集めます）、右側に**セッション**パネルが置かれます。

## カンバン — 2 つのボード

カンバン領域には性格の異なるボードが 2 つ、上下に置かれます。

**チェーンセッションボード**は 5 つの役割をカードとして並べ、各役割のセッション id、バックエンド、モデル、推論強度、コンテキスト使用量、最後のハートビートを記録します。ステージ状態はハートビートから**推定した**値なので推定マークが付き、モデル・推論強度・コンテキストはまだ記録されない値なので空のままにします — 埋めないことが規律です。
...
- **記録されていない値**（役割ごとのモデル・推論強度・コンテキスト使用量）はもっともらしい値で埋めず、空のままにします。
...
レールで設定を選ぶと、その下に 9 つのタブが縦のリストとして開きます。
