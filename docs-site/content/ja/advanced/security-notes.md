---
title: セキュリティノート
description: "MoAI-ADK セキュリティ強化の変更事項 (v2.20.0-rc1 導入、v3.0 系列で現行有効) — CWE-732/214/345 マッピング、ユーザー自己点検手順"
weight: 72
draft: false
tags: ["security", "cwe", "audit"]
---

エージェンティックハーネスはエージェントに実行権限を渡すシステムです。権限を渡すシステムであるほど、資格情報とアップデート経路のセキュリティがハーネスの信頼の基盤を成します。本ページは MoAI-ADK v2.20.0-rc1 時点で導入された **ユーザー可視のセキュリティ変更事項** を整理します。このセキュリティ強化は導入以降、v3.0 系列の現行まで有効です。各項目は CWE マッピング、変更された動作、自己点検コマンドを含みます。
...
上記 5 項目がすべて期待値を満たせばセキュリティ強化が正常に動作しています (v2.20.0-rc1 で導入され、v3.0 系列の現行まで有効)。
