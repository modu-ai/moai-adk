...
MoAI-ADKには計**32スキル**が6カテゴリに分類されています。`moai` umbrellaルーター1個と31の専門スキルで構成されます (Foundation、Workflow、Domain、Reference、Meta/Harness、Design)。プログラミング言語サポートは`rules/moai/languages/`配下のルールで提供され、個別スキルではありません。
...
| スキル名 | 説明 |
|----------|------|
| `moai-design-system` | Intent-firstデザイン、アクセシビリティ、デザイントークン |

> `moai` umbrellaスキル (統合`/moai`ルーター) は合計32に含まれますが、分類された機能スキルではなく、このガイドで説明されるサブコマンドをディスパッチします。
...
- **従来方式**: 32スキル全ロード = 約160,000トークン (不可能)
