...
weight: 20
draft: false
description: "MoAI-ADKスキルシステム — プログレッシブ・ディスクロージャー(progressive disclosure)で必要な知識だけを必要な瞬間にロードする知識レイヤー。"
---
...
このスキルのソース lint は、2 系統のルールを機械的に強制します。`SVG060`-`SVG064` はアクセシビリティ契約で、ルートの `<svg>` に `role="img"`、`aria-labelledby` で指し示すアクセシブルな名前、先頭の子要素としての `<title>`、`<desc>`、そしてダイアグラムごとの接頭辞を付けた id を要求します。これらを欠いたグラフィックは名前のない画像として案内され、中の `<text>` は一つも読まれません (純粋な装飾用のグラフィックだけが `aria-hidden="true"` による例外です)。

`SVG070`-`SVG074` は結線の幾何です。結線ラベルのマスクは自身の線から 6〜10 単位離れている必要があり (6 未満はエラー、10 超は警告で、マスクは 16 単位以内でのみその結線に帰属します)、1 つの箱の辺に届く 2 本の結線は到達点を互いに離さなければならず、ラベルのマスクがそれより後に描かれる図形と部分的に重なることも許されません。
...
| `moai-ref-ui-polish`       | UI デザイン完成度、インターフェースポリッシュのリファレンス |
| `moai-ref-cross-model-audit` | クロスモデル監査の収束 (codex・GLM の並列レビューと判定収束) |

### Meta/Harness (システム拡張) - 3 個

| スキル名              | 説明                                        |
| ---------------------- | ------------------------------------------- |
| `moai-meta-harness`    | **DEPRECATED** — レガシーの 7-Phase メタハーネス。v4 Builder(`/moai:harness <自然言語の要求>`)へリダイレクトします |
| `moai-harness-learner` | Harness 学習サブシステム、自動アップデート提案 |
| `moai-kanban-foreman`  | 無人カンバンフォアマンの1周 — バックログを監視し、選ばれたカードを隔離ワーカーへ配分し、証拠を収集 |
