...
5. **証拠のエクスポート** — 引用する検証証拠を追跡対象のパス`.moai/reports/<card-id>/`へエクスポート
...
そこで契約が定めるのは**引用前のエクスポート**の義務です。ある成果物を判定の根拠として引用するなら、その前に追跡対象のパス`.moai/reports/<card-id>/`へ移し、引用はディレクトリではなく**ファイル1つを名指し**します。移すのは判定を決めた行(終了コード、失敗の要約、引用する数値)だけです — スクラッチを丸ごと移せば、コンテキストから削った分量がリポジトリへ場所を変えるだけだからです。残したままの原文はスクラッチに置き、その損失の恐れを判定文の残余リスクの項に書き留めます。エクスポートしないと決めたものを根拠として差し出してはいけません。
