---
title: トークン予算管理と安全な中止
weight: 2
draft: false
---

トークノミクス4層構造のD層である予算防御(Budget defense)を深掘りします。エージェントがコンテキストウィンドウの限界に達したとき、セッションが停止せず、進行状況を保全しながら次のセッションが引き継げるようにする安全な中止(graceful abort)メカニズムを説明します。

{{< callout type="info" title="プラットフォームの基礎" >}}
プラットフォーム層の背景については [コンテキストウィンドウ](/ja/claude-code/context-memory/context-window) を参照してください。MoAI-ADK としての説明はこのページです。
{{< /callout >}}
...
予算防御はこの問題を事前に解決します。コンテキスト使用量がしきい値に達する前にシステムが安全な中止を実行し、セッションが損失なく次のステップへ移行するように保証します。
...
|-------------|-----------|-------------------|---------|
| Opus 5.5 (1M) | 1,000,000 トークン | 50% | ~500,000 トークン |
| GLM-5.3 (1M) | 1,000,000 トークン | 50% | ~500,000 トークン |
| Fable / Sonnet 5 (1M) | 1,000,000 トークン | 50% | ~500,000 トークン |
| Sonnet 4.x 以前の標準 (200K) | 200,000 トークン | 90% | ~180,000 トークン |
...
SPEC-TOKEN-BUDGET-STOP-001で実装された安全な中止メカニズムは次の手順で動作します。
...
5. **証拠の書き込み** — 引用する検証証拠のうち判定を決めた行を、追跡対象の判定書`.moai/reports/<card-id>/verdict.md`に書き込む
...
そこで検証出力は、まず`.moai/state/verify/<session>/`配下に置きます。ただしこのディレクトリは**マシンローカルのスクラッチ**です — `/tmp`の消去は生き延びますが、`context-usage/`や`active-sessions.json`と同じgitignoredのランタイム状態領域であり、cloneにもCIランナーにも他のマシンにも渡りません。`/tmp`を生き延びることと監査時点で到達できることは同じ意味ではありません。

そこで契約が定めるのは**判定を決めた行を判定書に書き込む**義務です。ある主張を立てるなら、その前にその行を追跡対象の判定書`.moai/reports/<card-id>/verdict.md`に書き込み、引用はディレクトリではなく**そのファイル1つを名指し**します。カード ディレクトリで追跡される名前は判定書ひとつだけで、その隣に置いたファイルは無視されたままなので、それを引用すれば書いたツリーの外では開けません。書き込むのは判定を決めた行(終了コード、失敗の要約、引用する数値)だけです — スクラッチを丸ごと移せば、コンテキストから削った分量がリポジトリへ場所を変えるだけだからです。残したままの原文はスクラッチに置き、その損失の恐れを判定文の残余リスクの項に書き留めます。スクラッチに残したものを根拠として差し出してはいけません。
