...
1. **証拠の保存とエクスポート** — そのマイルストーンで回したAC検証コマンドの出力を、まず`.moai/state/verify/<セッション>/M<マイルストーン>.<AC-id>.{log,out}`に残します。ただしこの置き場は**マシンローカルのスクラッチ**にすぎません — `/tmp`の消去は生き延びますが、gitignoreの対象なのでcloneにもCIランナーにも他のマシンにも渡りません。そのためAC行が証拠を引用する**前に**、判定を決めた行(終了コード、失敗の要約、その行が引用する数値)を追跡対象のパス`.moai/reports/<カード-id>/M<マイルストーン>.<AC-id>.log`へエクスポートし、引用ではそのファイル1つを名指しします。動かすのは名指ししたファイルだけで、スクラッチディレクトリを丸ごとエクスポートすることはありません。残したままの原文が失われる恐れは、判定文の残余リスクの項に書き留めます。
2. **折り畳み行を書く** — `progress.md`の§E.2欄に1行の要約を追加します。この行は「M2: AC-004=PASS, AC-005=PASS | evidence: .moai/reports/t123/M2-report.md | fold-at: 2026-08-12T...」の形式に従います。のちの監査段階で、この行の証拠パスは実在するファイルを指していなければならず、そこで開けるのは追跡対象のパスだけです。
