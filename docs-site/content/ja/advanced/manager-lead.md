...
|---|---|---|
| 作業単位 | 1つのSPECの中のマイルストーン | カンバンボードのカード(-k)、またはファクトリーワーカーに割り当てられたカード(-f) |
| 実際に働く主体 | 自ら生成するリーフ`Agent()`スポーン | 運用者が手動で立ち上げた同伴セッション(-k: plan · run · sync)とワーカー(-f: worker-1…worker-N) |
...
役割Bは、カンバンモード(`moai cc -k`)とファクトリーモード(`moai cc -f N`)で**リードセッションがディスパッチ周期を担う**作業です。カンバンリードは`lead > plan > run > sync`のチェーン上でカードをボードに流し — `plan`セッションはカードごとのSPEC執筆を並列`Agent()`ワーカーへfan-outします — ファクトリーリードは運用者が選んだカードを空きワーカーへ丸ごと割り当てます。どちらもセッションを新たに作ることはありません。同伴セッションとワーカーは運用者がターミナルごとに手動で立ち上げ、リードは名前で宛先を指定してメッセージを送ります。
...
役割Bにおけるリードセッションの姿勢は、双方向にノンブロッキングです。並列作業が背後で回っている間もユーザーとの会話は流れ続け、ワーカーや同伴セッションの調整が次のユーザー回答を待って止まることもありません。リードセッションはオーケストレータのチャネルを通じてユーザーと対話し(エージェント自身はブロッカー報告を返すだけです)、並列に回せる作業 — 読み取り専用の検証バッチ、報告の突き合わせ、リードが自ら抱えるカードごとのSPEC執筆 — はバックグラウンドの`Agent()`スポーンへ流します。

ここに**サブエージェント優先のトークン規律**が重なります。リードとワーカーのコンテキストには調整だけを残し、本体の作業はすべて下位エージェントへ降ろします。カードごとの執筆・検証・報告作成がリードの窓の中で起きれば4つの窓が同時に重くなりますが、下位エージェントへ降ろせばそれぞれの窓で完結し、リードには要約だけが戻ります。同時スポーンの上限はセッションあたり10個で、GLMバックエンドではスポーンを**名前なし**で立ち上げます — 名前付きのスポーンは`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`の下で結果を返さないインプロセスのチームメイトに変わることがあるためです。
...
この図で注意して見るべきは矢印の向きです。オーケストレータは`manager-lead`までしか呼ばず、その下のリーフエージェントを呼ぶのは`manager-lead`だけです。リーフエージェントは別のエージェントを呼べません(点線が指す封印)。これが「深さ2の封印」であり、階層が際限なく深くなるのを防ぐ構造的な安全網です。役割Bでも同じです — リードのバックグラウンド`Agent()`スポーンは1段だけで、同伴セッションとワーカーはそもそもリードが作ったものではなく、運用者が立ち上げた独立したセッションです。
...
役割Bの進入はもっと単純です。セッションのSessionStart文脈がカンバンモード(`moai cc -k`)またはファクトリーモード(`moai cc -f N`)の`lead`役割を宣言していればそれで足り、しきい値は適用されません — ボード(またはワーカーの束)そのものが作業だからです。サブエージェントのスポーンにはSessionStart文脈がないため、役割Bにスポーン経由で進入することはできません。
...
この分担で最も目を引くのは、`Agent`ツールを持つのが`manager-lead`だけだという点です。13のエージェントのうち`Agent`を持つのは`manager-lead`のみで、ほかの管理者エージェントはすべて`Agent`を外したツール一覧でフラットな階層を保ちます。フラットな階層をただ1か所で開く代わりに、その下のリーフエージェントは再び`Agent`を持てないよう塞ぎ、階層が2段を超えないように封印します。これが「深さ2の封印」であり、CIガード(`manager_lead_depth_test.go`)が守ります。
...
ここでの`manager-lead`の`Write`と`Edit`は調整の用途にのみ使われます。すなわち`progress.md`の§E.2欄に折り畳み行を追加したり、検証出力をいったんスクラッチに置いたうえで判定を決めた部分だけを`.moai/reports/<カード-id>/`へエクスポートしたりするためのもので、ソースコードに触れることはありません。コードは常にリーフエージェントの仕事です。
...
1. **証拠の保存と判定書への書き込み** — そのマイルストーンで回したAC検証コマンドの出力を、まず`.moai/state/verify/<セッション>/M<マイルストーン>.<AC-id>.{log,out}`に残します。ただしこの置き場は**マシンローカルのスクラッチ**にすぎません — `/tmp`の消去は生き延びますが、gitignoreの対象なのでcloneにもCIランナーにも他のマシンにも渡りません。そのためAC行が証拠を引用する**前に**、判定を決めた行(終了コード、失敗の要約、その行が引用する数値)を追跡対象の判定書`.moai/reports/<カード-id>/verdict.md`に書き込み、引用ではそのファイル1つを名指しします。カード ディレクトリで追跡される名前は判定書ひとつだけで、その隣に置いたファイルは無視されたままなので、それを引用すればこのマシンの外では開けないパスになります。移すのは判定を決めた行だけで、スクラッチディレクトリを丸ごと移すことはありません。残したままの原文が失われる恐れは、判定文の残余リスクの項に書き留めます。
2. **折り畳み行を書く** — `progress.md`の§E.2欄に1行の要約を追加します。この行は「M2: AC-004=PASS, AC-005=PASS | evidence: .moai/reports/t123/verdict.md | fold-at: 2026-08-12T...」の形式に従います。のちの監査段階で、この行の証拠パスは実在するファイルを指していなければならず、そこで開けるのは判定書だけです。
...
```bash
# 引用した証拠が監査時点で開けるかの確認 — この条件を満たすのは追跡対象のパスだけです
ls .moai/reports/t123/M2-report.md

# スクラッチに置いた原文はこのマシンにだけとどまります
