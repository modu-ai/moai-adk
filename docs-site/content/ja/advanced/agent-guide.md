...
weight: 30
draft: false
description: "MoAI-ADK v3.0の13個のコアエージェントカタログ — 役割、フェーズ範囲、計画-監査の分離原則。"
---

MoAI-ADK v3.0 の 13 個のコアエージェントカタログを詳しく解説します。
...
エージェント数は v3 期間中に 22 → 17 → 8 → 10 → 12 → **13** へと精錬されました。エージェントが多ければ良いわけではありません — 委任のたびにコンテキストコストがかかるため、カタログを絞ること自体がトークノミクスの一部です。
...
MoAI-ADK は **13 個のコアエージェント** (12 個の MoAI カスタム + 1 個の Anthropic ビルトイン) を使用します。
...
| `mission-governor` | 承認済み GTD 自動ミッションの封印されたスナップショットを読み、構造化された判定を 1 つだけ返します (読み取り専用) | inherit / high {{< icon flash warn >}} | 判定するだけで実行はしません — 判定を検査し状態を変えるのは決定論的な実行器です |

`mission-governor` は **判定するだけで、自ら実行はしません**。ファイルを書くことも、シェルや Git のコマンドを走らせることも、キューに触れることも、レーンに作業を配ることもなく、コミット・マージ・承認も、監査判定も行いません。返された判定を検査して実際に状態を変えるのは決定論的な実行器 (決められた規則どおりにだけ動く自動処理器) です。ツール一覧も `Read`、`Grep`、`Glob`、`Skill` の 4 つだけで、リクエストが封印された範囲を外れた場合や、判断に必要な証拠が欠けている・古い場合は blocker 判定を返します。

{{< callout type="info" >}}
`mission-governor` は下の [エージェント選択デシジョンツリー](#エージェント選択デシジョンツリー) に **意図的に居場所がありません**。オーケストレーターが選んで呼ぶエージェントではなく、GTD 自動ミッションのワークフローが直接呼び出す判定役だからです。ツリーに見当たらないのは抜け落ちではなく設計です。
{{< /callout >}}
...
1. **証拠の保存と判定書への書き込み** — 各 AC の検証コマンド出力を `.moai/state/verify/<session>/M<n>.<AC-id>.{log,out}` にリダイレクトします。`/tmp` は OS が消去するため使いませんが、`/tmp` を生き延びることと監査時点で開けることは別の話です — このディレクトリは gitignore された**マシンローカルのスクラッチ**です。AC 行が証拠を引用する前に、判定を決めた行を追跡対象の判定書 `.moai/reports/<card-id>/verdict.md` に書き込み、引用ではそのファイル 1 つを名指しします — カード ディレクトリで追跡される名前はそれひとつだけです。証拠を取得できなかった AC は `PASS` ではなく `GAP` と表記します。
...
12 個の MoAI カスタムエージェントは `.claude/agents/moai/` ディレクトリにマークダウンファイルとして定義されます。`Explore` は Anthropic ビルトインのため、ディスク上にファイルがありません。
...
| 規則 | 内容 |
|------|------|
| ユーザーへの質問はオーケストレーター専用 | `AskUserQuestion` を使うのはオーケストレーターだけで、ランタイムがこの境界を強制します。ユーザー入力が必要なサブエージェントはプロンプトの代わりに構造化された blocker レポートを返し、オーケストレーターがユーザーに尋ねて答えを添えて再送します |
| sweep サブエージェントも質問不可 | 動的ワークフロー (sweep) のサブエージェントはメインセッションの下で動き、ユーザーにプロンプトできません。質問が必要ならオーケストレーターのチャネルを経由します |
