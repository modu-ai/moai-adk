...
## `disallowedTools` MCP サーバーレベル強制

エージェントの `disallowedTools` frontmatter フィールドが MCP ツールを参照する場合 (例: `mcp__context7__*`, `mcp__web_search_prime__webSearchPrime`)、制限は MCP サーバーレベルで強制されます (CC 2.1.178+): ツールの specs がエージェントに全く公開されないため、エージェントは間接的にも呼び出せません。これは組み込みツールに対する通常のツール拒否動作よりも厳しいです — MCP ツールに対する `disallowedTools` エントリは hard gate であり、soft prompt ではありません。MCP ツールアクセスのスコープを指定する際、この点を念頭に置いてエージェントを作成してください。

## ネストされた `.claude/` の優先順位

ネストチェーンに沿って複数の `.claude/agents/` ディレクトリに同じエージェント名が現れる場合 (プロジェクトルート vs ネストされたサブディレクトリ独自の `.claude/agents/`)、**closest-directory-wins** (最も近いディレクトリ優先) ルールが衝突を解決します: 現在の作業ディレクトリに最も近い `.claude/agents/` が、ツリーの上位にあるものをシャドウします。これは、ネストされた `.claude/` ディレクトリ配下でスキル、ワークフロー、output-styles に既に適用されている優先順位と同じです — 最も内側の `.claude/` が勝ちます。管理 (enterprise) 設定は、ネストの深さに関わらず優先度 1 のままです。
