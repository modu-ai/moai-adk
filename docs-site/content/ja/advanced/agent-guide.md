...
flowchart TD
    ...
    subgraph EVALS["Evaluatorエージェント"]
        ...
        EA["evaluator-active<br/>品質の4次元採点"]
    end
...
| `evaluator-active` | 独立した懐疑的品質評価、4次元採点（機能/セキュリティ/工芸/一貫性） | `moai-foundation-quality` | Read, Grep, Bash |
...
| セキュリティレビュー | evaluator-active | "コードをセキュリティ観点でレビュー" |
...
flowchart TD
    ...
    C --> D["4. evaluator-active<br/>品質評価"]
...
{{< callout type="info" >}}
**ヒント**: エージェントを直接指定する必要はありません。MoAIに自然言語でリクエストすれば最適のエージェントが自動的に選択されます。「APIを作成して」と言えば`manager-develop`が、「このコードをレビューして」と言えば`evaluator-active`が自動的に呼び出されます。
{{< /callout >}}
