...
MoAI-ADK v3.0 の 10 個のコアエージェントカタログを詳しく解説します。
...
エージェント数は v3 期間中に 22 → 17 → 8 → **10** へと精錬されました。エージェントが多ければ良いわけではありません — 委任のたびにコンテキストコストがかかるため、カタログを絞ること自体がトークノミクスの一部です。
...
MoAI-ADK は **10 個のコアエージェント** (9 個の MoAI カスタム + 1 個の Anthropic ビルトイン) を使用します。
...
9 個の MoAI カスタムエージェントは `.claude/agents/moai/` ディレクトリにマークダウンファイルとして定義されます。
