...
Layer Bのルーティングを具体化するのがモデルプロファイルポリシーです。MoAI-ADK v3.0はHaikuをルーティングモデルセットから除外し、作業の性格に合わせた3層構造で作業を分散します — 単発の行にはSonnet、エージェンティックな階梯全体にはOpus、より高い effort は判断を担う行（監査・助言・調整）に集めます。この設計の根拠とプロファイルマトリクス実装は次の2ページで扱います。
