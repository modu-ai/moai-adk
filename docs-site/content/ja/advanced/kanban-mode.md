...
セッションランチャーに `--kanban`(短縮形 `-k`)スイッチを付けて開始します。新しいサブコマンドでも新しいランタイムでもありません — ランチャーがカンバンモード環境を武装するための進入契約にすぎません。チェーンの3段階(plan → run → sync — レビュー判定は sync ゲートが吸収)とヒューマンゲートは、既存の `/moai goal` エンジンと `full-pipeline` チェーニング規則をそのまま継承します。
...
- **ブートストラップ案内** — リードセッションが開くと、SessionStart フックがラン識別子と3つの同伴セッション実行コマンド(`moai cc -k --name plan` など)をユーザーの言語で出力します。同伴セッションに届く案内は、どのランに合流したかとセッション名を伝えます。名前はロールだけで付けられ(`plan`, `run`, `sync`)、同じロール名がすでに生存セッションに取られていれば次の番号が付きます(`plan-1`, `plan-2`, …)。
...
理由はレーンごとに必要な推論の種類です。plan と sync は判断とレビューを回す列なので Claude に置き、run は実装中心なので GLM でコストを下げます。リードは判定を下す席ではなく、キューを見張ってカードを運ぶ席なので、待ち続けても安い GLM が合います。GLM リードの下で Claude の判定が必要になったら、`judge` という名前のセッションから抜け道を作ります — GLM リードが Claude を使う唯一の経路です。あるアカウントが 429 で詰まり始めたら、レーンをアカウントに分散して置く運用が効きます。この組み合わせはあくまで既定の推奨 — 別の組み合わせも、全セッションをひとつのバックエンドに統一しても構いません。

スクリーンショットのステータスラインに見えるモデルラベルは、撮影時のひとりの運営者のセッション構成を反映したもので、配布既定値ではありません。

## 番号付きワーカーラン — 同じ -k、ワーカー数を付ける

{{< callout type="info" >}}
{{< icon flash primary >}} **属する価値**: マルチセッションオーケストレーション · トークノミクス
{{< /callout >}}

カンバンが「3つのロールが1枚のカードを段階ごとに運ぶ」形なら、同じカンバンモードの2つ目の形である**番号付きワーカーラン**は「番号付きワーカー N が複数のカードを同時に運ぶ」形です。進入は同じ `-k` トークンに**数値を付ける**だけで終わりです — 新しいフラグではなく、同じスイッチの2つ目の解釈です。(改称前の `-f` 時代の旧名は「ファクトリーモード」でした。)

```bash
# リード — ワーカー4つのカンバンラン (worker-1..worker-4 の実行コマンドを表示します)
$ moai cc -k 4

# ワーカー — 各自自分のターミナルで、1コマンドに番号が入ります
...
リードの仕事は役割カンバンのリードとは違います。役割カンバンのリードが1枚のカードの段階の間を調整するのに対し、番号付きワーカーランのリードは**バックログキューをポーリングして**、運営者が選んだカードを**空きワーカーに配分**します。配分の仕方はカードクラスで分かれます — A/Bクラス(単純な変更・原因未確立の欠陥)はワーカー1つに丸ごと任せ、Cクラス(設計変更)は `plan → run → sync` の直列3段階経路へ送ります。ワーカーが忙しい最中は新しいカードを無理に起こさず、スロットが空く瞬間に合わせて配分します(ステガード・ファンアウト — 同時起動の競合を避ける遅延配分)。

ワーカー数を省略して `-k --name worker-<i>` 形式だけを使うと、**既定8ワーカー**のランとして解釈されます。混合バックエンドランチャー(`moai cg`)はカンバンと同じ理由で拒否されます(`FACTORY_MODE_UNSUPPORTED_BACKEND`)。

{{< callout type="warning" >}}
{{< icon warning warn >}} **`-f`/`--factory` は引退しました。** かつてファクトリー専用フラグだった `-f` は、進入が `-k <N>` に統一されるのに伴って引退し、現在は明示的なエラーを出します。黙って無視されるフラグではなく、「どこへ行くべきか」を教えるエラーです。
{{< /callout >}}
...
すべてのカードが 3 つの作業列をすべて巡る必要はありません。バックログに積まれる大半は単純な整理作業で、そうしたカードを plan → run → sync の全過程に通すと、変更の価値を上回る手続きコストがかかります。リードはカードが `backlog` を離れるときにクラスを分類し、ディスパッチとともに明示します。

| クラス | 形 | 通る経路 |
|--------|------|---------------|
| A — 直接クローズ | ファイル 1 つ・1 行の変更で設計判断を含まず、CI がリグレッションを検出 | ひとつのセッションが PR まで通して担当; `plan` をスキップ |
| B — 欠陥、原因未確立 | 何かが壊れているが、原因がまだ確立されていない | `run → sync`; `plan` をスキップするため SPEC なし |
| C — 設計変更 | 決定を含む、またはサブシステムにまたがる | 3 つの作業列すべて |

Class A は検証済みの証拠の上でのみ成立します — diff をベースに対して測定して対象がファイル 1 つであることを示し、マージされる head で CI が緑であることを確認して初めて、クラスが認められます。「速いから」は根拠になりません。速度は列をスキップした結果であって理由ではないので、速度だけで正当化された A クラスは、急かされた C クラスです。

Class B がスキップするのは `plan` であって、sync ゲートのレビューではありません。原因が確立されていない欠陥こそレビューが捉えるものなので、ゲートは最後まで残ります。カードが `run` を離れる前に、原因を確立した証拠 — 何を再現し、何が出力されたか — をカードの進行記録に残します。

ひとつの列にとどまるカードは一度に 1 枚だけで、各カードが異なる worktree を占めるときにのみ実際に並列で動きます。
...
- [`/moai loop`](/ja/utility-commands/moai-loop) — bare `/loop` で駆動する無人フォアマン: バックログキューを見張り、運営者が選んだカードを隔離ワーカーに配分・証拠収集をひとつのセッションで繰り返す
