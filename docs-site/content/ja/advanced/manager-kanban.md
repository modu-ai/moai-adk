...
Tier L規模のSPECを順次モード(serial)で実装すると、オーケストレータは`manager-develop`エージェントをマイルストーンごとに順に呼びます。この流れはほとんどの場合にうまく合いますが、SPECが大きくなると2つの現象が現れます。
...
`manager-kanban`はすべての実行に既定で敷かれる経路ではありません。オーケストレータはSPECが次の3条件を**すべて**満たすときだけ`manager-kanban`に作業を預けます。3条件のうち1つでも欠ければ、標準の順次モード(serial)のまま進みます。
...
1つ強調しておきたいのは、`manager-kanban`が新しい実行モードではないということです。Phase 4の4つのモード(direct, serial, fanout, sweep)はそのままで、`manager-kanban`はserial(順次呼び)形状の委任対象です。新しいモードができたわけではなく、実験的なagent-team面(明示的な`--team`要求でのみ選択)がカタログのモードになったわけでもありません。
...
同時に呼ぶエージェント数は、MoAI fanoutの3〜5同時上限を超えません。5を超える偵察が必要なら、`manager-kanban`はそれらを順に束ねて呼びます。
...
- **実行モード一覧** — direct, serial, fanout, sweep。そのままです(agent-teamは明示要求専用の実験的な脚注のままで、自動選択されません)。
- **新しいモード** — 新しいモードはありません。`manager-kanban`はserial形状の順次委任対象です。
- **`--mode` の値** — `autopilot`, `loop`, `team`, `pipeline`の値はそのままです。新しい値は追加されず、agent-teamは明示要求専用の実験面のままです(`MODE_TEAM_UNAVAILABLE`センチネルは文書化された履歴として維持されます)。

この約束は「エージェントを1つ追加しただけでオーケストレーション層が複雑になるのか?」という自然な懸念への答えです。`manager-kanban`はすでにあるserialの器の中に入るエージェント1つにすぎず、新しい器を作りません。
