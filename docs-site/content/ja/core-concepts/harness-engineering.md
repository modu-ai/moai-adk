...
このループは `/moai loop` でデフォルト最大 5 回 (`ralph.max_iterations`) まで反復され、収束検知 (同じエラーの繰り返し時に代替戦略を適用) を含みます。完了条件を自ら宣言したい場合は goal エンジン (`/moai goal "<条件>"`) を使います — 条件が満たされるかターン上限に達するまで、セッションが自ら働き続けます。
