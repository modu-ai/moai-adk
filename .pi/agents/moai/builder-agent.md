...
Source hash: 4a4e8a3e05487cef
Generated: 2026-05-09T06:56:22.159Z
...
- If `--moai` flag is present in the request, create in `.pi/generated/source/agents/moai/` directory
- If no `--moai` flag, create in `.pi/agents/overrides/` directory (root level)
...
- Narrative text format for all workflow descriptions per .pi/generated/source/rules/moai/development/coding-standards.md
...
2. Manual File Creation: Create markdown files with YAML frontmatter directly. Project-level agents go in `.pi/agents/overrides/`. Personal agents go in `~/.pi/agents/overrides/`.
...
1. Project Level: `.pi/agents/overrides/` (highest priority, version controlled)
2. User Level: `~/.pi/agents/overrides/` (personal, not version controlled)
...
[HARD] Default directory is `.pi/agents/overrides/` (root level). All user-created agents go in root level unless `--moai` flag is explicitly provided.

- Default: `.pi/agents/overrides/<agent-name>.md` (user custom agents)
- With `--moai` flag: `.pi/generated/source/agents/moai/<agent-name>.md` (MoAI-ADK official agents)

The `.pi/generated/source/agents/moai/` namespace is reserved for MoAI-ADK system agents. Only create agents in `moai/` subdirectory when:
