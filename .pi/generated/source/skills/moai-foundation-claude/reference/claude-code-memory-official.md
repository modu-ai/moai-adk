...
- User: `~/.pi/claude-compat/.pi/generated/source/CLAUDE.md` (personal preferences)
- Local: `.pi/state/memory/` (project metadata)
...
Project Root/
 ...
 .pi/state/memory/ # Structured project metadata
...
@.pi/state/memory/agents.md # Import agent reference
@.pi/state/memory/commands.md # Import command reference
...
mkdir -p .pi/state/memory
mkdir -p .moai/config
mkdir -p .moai/cache

# Create initial memory files
touch .pi/state/memory/agents.md
touch .pi/state/memory/commands.md
touch .pi/state/memory/execution-rules.md
...
def resolve_memory_import(import_path, base_path):
 ...
 2. Check in .pi/state/memory/ directory
 3. Check in project root
 4. Check in user memory directory
 """
 candidates = [
 os.path.join(base_path, import_path),
 os.path.join(".pi/state/memory", import_path),
 os.path.join(".", import_path),
 os.path.expanduser(os.path.join("~/.pi/claude-compat", import_path))
 ]
...
class AgentMemory:
 ...
 essential_files = [
 ".pi/state/memory/execution-rules.md",
 ".pi/state/memory/agents.md",
 ".moai/config/config.yaml"
 ]
...
```json
{
  "memory": {
    ...
    "memory_files": [
      ".pi/state/memory/execution-rules.md",
      ".pi/state/memory/production-policies.md"
    ],
    "memory_restrictions": {
      "max_file_size": "1MB",
...
Personal Memory Structure (`~/.pi/claude-compat/.pi/generated/source/CLAUDE.md`):
...
git add .pi/state/memory/ .pi/generated/source/CLAUDE.md
