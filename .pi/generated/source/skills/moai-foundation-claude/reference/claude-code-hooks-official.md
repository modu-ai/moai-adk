...
- Location: `~/.pi/generated/source/settings/claude-settings.json` (user) or `.pi/generated/source/settings/claude-settings.json` (project)
...
```json
{
 ...
 {
 "type": "command",
 "command": "echo 'Executing bash command:' >> ~/.pi/claude-compat/hooks.log"
 }
 ]
 }
 ]
...
- Global: `~/.pi/generated/source/settings/claude-settings.json` (user-wide hooks)
- Project: `.pi/generated/source/settings/claude-settings.json` (project-specific hooks)
- Local: `.pi/overrides/settings.local.json` (local overrides)
...
```json
{
 ...
 {
 "type": "command",
 "command": "echo \"$(date '+%Y-%m-%d %H:%M:%S') - Tool: $TOOL_NAME, Duration: $DURATION_MS ms, Success: $SUCCESS\" >> ~/.pi/claude-compat/session-logs/$SESSION_ID.log"
 }
 ]
 },
 {
 ...
 {
 "type": "command",
 "command": "echo \"$(date '+%Y-%m-%d %H:%M:%S') - Session: $SESSION_ID, Event: $EVENT_TYPE\" >> ~/.pi/claude-compat/activity.log"
 }
 ]
 }
 ]
