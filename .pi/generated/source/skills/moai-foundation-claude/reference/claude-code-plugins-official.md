...
Plugins are reusable extensions that bundle Claude Code configurations for distribution across projects. Unlike standalone configurations in `.pi/generated/source/` directories, plugins can be installed via marketplaces, shared across teams, and version-controlled independently.

## Plugin vs Standalone Configuration

Standalone Configuration (`.pi/generated/source/` directory):
...
- Location: ~/.pi/claude-compat/plugins/
- Availability: All projects for current user
- Use case: Personal productivity tools

### Project Scope

- Location: .pi/generated/source/settings/claude-settings.json (reference only, not copied)
