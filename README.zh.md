...
这三种成本同出一源：模型是一个按 Token 计费的随机工作者，它既不记得你的预算、你的质量标准，也不记得上一个会话在哪里结束。像成本上限、通过的测试套件，或跨 `/clear` 的连续性这类属性，无法靠一回合接一回合地提示凭空产生——一个遗忘它们的回合，随时可能只因一次糟糕的采样便前功尽弃，且每回合皆然。这些属性必须由模型*周围*的系统来强制保障，无论任何单个回合结果如何，该系统都能稳稳守住。这个系统就是 Harness——而构建它，正是 Harness 工程与 Anthropic 自身的代理指南所共同汇聚之处。MoAI-ADK 把这三者都视为可用机制解决的工程问题，而非无法改变的现实。

---

## MoAI-ADK 如何解决

如果问题是结构性的，那么解法也必须是结构性的。下表的每一行都是一个运行在模型逐回合裁量*之外*的机制——声明式配置、钩子与门禁——因此即使模型出错，它依然稳固成立。每个痛点都对应一个具体机制，并配有可衡量的证据。
...
分界线在于裁量与保障之别：左栏里每一种行为都取决于模型在这一具体回合上是否选得好；右栏里同样的行为则是流水线的属性，无论任何单个回合是否做对，都会被强制保障。
