...
### 💻 `/moai:2-run` - 实现 (TDD或DDR)

```bash
> /moai:2-run SPEC-001
```

根据`.moai/config/sections/quality.yaml`中的`development_mode`设置路由到TDD或DDR:
...
| **manager-ddr**      | DDR重构执行      | `/moai:2-run` (development_mode: ddr 时)    |
