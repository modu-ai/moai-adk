...
旧版 v2.x `/agency` 命令已完全融入 `/moai design`。旧版 v2.x 项目可通过以下命令自动迁移：
...
# 旧版 v2.x 命令（已弃用，重定向至 /moai design — 参考 SPEC-AGENCY-ABSORB-001）
...
使用旧版 v2.x `/agency` 的现有项目可以通过以下命令迁移到 `/moai design` (详细信息参见 [SPEC-AGENCY-ABSORB-001](.moai/specs/SPEC-AGENCY-ABSORB-001/spec.md))：

```bash
moai migrate agency
```

此命令将旧版 v2.x 数据安全地移动到 `.moai/project/brand/` 和 `.moai/config/sections/design.yaml`。原始数据保留为 `.agency.archived/` 以供恢复。
