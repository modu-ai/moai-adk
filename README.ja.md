...
カンバンモードは作業ひとつを**ターミナル 1 台ではなく 5 台に**分割する。リード・セッションがチェーンを運転し、4 つのコンパニオン・セッションが `plan`・`run`・`review`・`sync` の 1 列ずつを担い、**自分の列の文脈だけ**を背負う。無制限になるわけではない — セッションごとの上限はそのままある。変わるのは、どのセッションも 4 フェーズ分の履歴を背負わなくなる点であり、だから同じ予算ではるかに遠くまで届き、終わったフェーズはカードを失わずに空にできる。

<p align="center">
  <img src="./assets/images/kanban-five-sessions.png" alt="カンバンモードの 1 ラン — リード・セッションと 4 つのコンパニオン・セッションが、それぞれのターミナルで、それぞれのモデルと推論強度で動いている" width="100%">
</p>
...
moai cc -k --name plan-<run-id>     # コンパニオン、各自別ターミナルで
moai cc -k --name run-<run-id>
moai cc -k --name review-<run-id>
moai cc -k --name sync-<run-id>
```

コンパニオン・セッションは**ターミナルを 1 つずつ新しく開いて手動で**立ち上げる。セッションが別のセッションの代わりに立つことはない。どの列でも `moai cc` の代わりに `moai glm` を使えば、その列だけ GLM バックエンドで動く。

ボードは `backlog → plan → run → review → sync → done` の 6 列である。`backlog` には意図的に担当セッションを置かない。だから仕事は人が入れたときだけボードに入る。
...
```text
オペレーター ── /moai todo ──▶ backlog ─▶ plan ─▶ run ─▶ review ─▶ sync ─▶ done
...
| ラン ID (run-id) | リードが開始時に告知する短い識別子。コンパニオンのセッション名が共有する |
...
`moai web` はローカル・コンソールを立ち上げる。カンバン画面で 5 セッション・チェーンと SPEC パイプラインを一緒に眺め、Overview・Specs・Monitor・Settings 画面も併せ持つ。
...
`--kanban`（短縮 `-k`）はセッション・ランチャーのスイッチで、`kanban_chain` ゴール・プリセットを武装し、単一の SPEC を `plan → run → verify → sync` で押し進め、マルチセッション・ボードで調整する。ボードの背骨が** Origin-Trail Chain**だ — append-only の JSONL 系譜ツリーでワークツリーの祖先を追跡し、深さの健忘（`/clear` 後のルートからリーフへのチェーン復元）を解決し、ハートビートの鈍りから死んだリーダー・セッションを検出する。
...
> **今すぐ使える**: `moai cc -k`（または `moai glm -k`）でリードを立ち上げ、`-k --name <role>-<run-id>` でコンパニオンを 1 つずつ接続する — ターミナル 1 台に 1 つ、手で立ち上げる。`moai chain <status|lineage|back|list|prune>` で系譜を読み、`moai todo <add|list|next|done>` で `backlog` 列を運用する。起動手順は上の「v3.1 の新機能 — カンバンモード」節にある。
