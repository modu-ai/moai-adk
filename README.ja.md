...
**plan_type プロファイル**。API 従量課金とサブスクリプションでは最適な配分が異なる。API ではドルが唯一の制約であり、サブスクリプションでは週間トークンクォータが制約である。60 セルプロファイル行列（10 エージェント × 3 ティア × 2 plan_type）が、課金種別に異なるモデル/effort を適用する。
