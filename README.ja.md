...
レガシー v2.x の `/agency` コマンドは `/moai design` に完全に統合されました。レガシー v2.x プロジェクトは以下で自動マイグレーションできます:
...
メリット:
- デュアル `/moai` + レガシーコマンドではなく、単一の統合デザインワークフロー
...
| **Evaluator** | 2 | sync-auditor, plan-auditor | 独立的品質評価、計画段階文書監査 |
| **Legacy v2.x (retired)** | 6 | planner, copywriter, designer, builder, evaluator, learner (absorbed — [SPEC-AGENCY-ABSORB-001](.moai/specs/SPEC-AGENCY-ABSORB-001/spec.md)) | クリエイティブプロダクションパイプライン (歴史的参照) |
...
|------------|---------|------|----------|
| `plan` | `spec` | SPECドキュメント作成（EARS形式） | `--worktree`, `--branch`, `--resume SPEC-XXX`, `--team`, `--tmux` |
...
| `moai worktree new <name>` | 新しいGit worktreeを作成（並列ブランチ開発）。`--tmux` 追加でworktree内にtmuxセッションを自動作成 |
...
| **実装** | 20エージェント (manager/expert/builder) | 4スキル (copywriting、brand-design、design-import、gan-loop) + sync-auditor |
...
5. **品質保証** — sync-auditorがPlaywrightテスト、Lighthouse監査、4次元スコアリングを実行
6. **GAN Loop** — 品質未達ならexpert-frontendとsync-auditorがmoai-workflow-gan-loop経由で反復（最大5ラウンド）
...
| **sync-auditor** | Playwrightビジュアルテスト＋Lighthouse監査。4次元スコアリング：デザイン品質(30%)、オリジナリティ(25%)、完成度(25%)、機能性(20%) |
| **moai-workflow-gan-loop** | GAN Loop反復管理：Builder-Evaluator間でSprint Contract協商、実装、スコアリング、停滞検知 |
### GAN Loop：敵対的品質保証

sync-auditorは**デフォルトで懐疑的** — 欠陥を見つけるよう調整されています。
...
**反復フロー**：sync-auditorがfile:line参照付き具体的フィードバック提供 → Builder修正 → 再評価。3回失敗後ユーザーにエスカレーション。
...
ブランドコンテキストは**全スキル**に不変制約として伝達されます。sync-auditorはブランド一貫性を必須合格基準として評価。5プロジェクト以上完了後、インタビューは主要3質問に短縮。
...
# レガシー v2.x コマンド（deprecated、/moai design にリダイレクト — SPEC-AGENCY-ABSORB-001 参照）
...
### レガシー v2.x デザインドメインからの移行

レガシー v2.x の `/agency` を使用中の既存プロジェクトは、以下のコマンドで `/moai design` に移行できます（詳細は [SPEC-AGENCY-ABSORB-001](.moai/specs/SPEC-AGENCY-ABSORB-001/spec.md)）：

```bash
moai migrate agency
```

このコマンドはレガシー v2.x データを `.moai/project/brand/` と `.moai/config/sections/design.yaml` に安全に移動します。元のデータは復旧用に `.agency.archived/` に保持されます。
