...
BANNED_PATTERNS = (
    (
        re.compile(r"(?i)ignore (the )?(claude|constitution|steering|instructions)"),
        "헌법/지침 무시는 허용되지 않습니다.",
    ),
    (
        re.compile(r"(?i)disable (all )?(hooks?|guards?|polic(y|ies))"),
        "Hook/Guard 해제 요청은 차단되었습니다.",
    ),
    (re.compile(r"(?i)rm -rf"), "위험한 셸 명령을 프롬프트로 제출할 수 없습니다."),
    (re.compile(r"(?i)drop (all )?safeguards"), "안전장치 제거 요청은 거부됩니다."),
    (
        re.compile(r"(?i)clear (all )?(memory|steering)"),
...
def _check_hybrid_system_status() -> dict[str, str]:
    """하이브리드 시스템 상태 확인"""
    current_dir = Path.cwd()

    # TypeScript 프로젝트 존재 확인
    ts_project = current_dir / "moai-adk-ts"
    has_typescript = ts_project.exists() and (ts_project / "package.json").exists()

    # Python 브릿지 존재 확인
    python_bridge = current_dir / "src" / "moai_adk" / "core" / "bridge"
    has_python_bridge = python_bridge.exists() and (python_bridge / "typescript_bridge.py").exists()

    if has_typescript and has_python_bridge:
        return {
            "status": "full_hybrid",
            "description": "Python + TypeScript 완전 통합 🔗"
        }
    elif has_python_bridge:
        return {
            "status": "python_only",
...
def _show_session_notice() -> None:
    """세션 시작 알림 표시 (최초 1회) - 하이브리드 시스템 포함"""
    ...
    # 하이브리드 시스템 상태 확인
    hybrid_status = _check_hybrid_system_status()

    # 알림 표시
    print("🚀 MoAI-ADK 하이브리드 프로젝트가 감지되었습니다!", file=sys.stderr)
    print(
        "📖 개발 가이드: CLAUDE.md | TRUST 원칙: .moai/memory/development-guide.md",
        file=sys.stderr,
    )
    print("⚡ 하이브리드 워크플로우: /moai:1-spec → /moai:2-build → /moai:3-sync", file=sys.stderr)
    print(f"🔗 시스템 상태: {hybrid_status['description']}", file=sys.stderr)
...
def main() -> None:
    ...
    for pattern, message in BANNED_PATTERNS:
        if pattern.search(prompt):
            ...
            print(
                "HINT: CLAUDE.md와 @.moai/project/* 문서를 기반으로 목표/제약을 명시해 주세요.",
                file=sys.stderr,
            )
            sys.exit(2)
