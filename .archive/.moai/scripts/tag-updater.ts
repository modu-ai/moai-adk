#!/usr/bin/env tsx
// CODE-TAG-UPDATER-001: TAG 인덱스 업데이트 및 관리 스크립트
// 연결: SPEC-TAG-UPDATE-001 → SPEC-TAG-UPDATER-001 → CODE-TAG-UPDATE-001
...
async function validateTagDatabase(database: TagDatabase): Promise<string[]> {
  ...
  for (const tag of primaryTags) {
    if (tag.type === 'REQ' && tag.children.length === 0) {
      issues.push(`@SPEC TAG에 연결된 @SPEC이 없습니다: ${tag.id}`);
    }
    if (tag.type === 'DESIGN' && !tag.parents.some(p => database.tags[p]?.type === 'REQ')) {
      issues.push(`@SPEC TAG에 연결된 @SPEC가 없습니다: ${tag.id}`);
    }
  }
