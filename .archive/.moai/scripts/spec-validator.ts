#!/usr/bin/env tsx
// CODE-SPEC-VALIDATOR-001: SPEC 문서 검증 스크립트
// 연결: SPEC-VALIDATION-001 → SPEC-SPEC-VALIDATOR-001 → CODE-VALIDATOR-001
...
const VALIDATION_RULES: ValidationRule[] = [
  ...
  },
  {
    ...
    description: '@SPEC 섹션이 있는지 확인',
    severity: 'error',
    check: (content: string) => /@SPEC:[A-Z]+-\d+/.test(content),
    message: '@SPEC 섹션이 없습니다. 요구사항 섹션은 필수입니다.',
    fix: (content: string) => {
      if (!/@SPEC:[A-Z]+-\d+/.test(content)) {
        const insertPoint = content.indexOf('\n\n') + 2 || content.length;
        const reqSection = `\n## @SPEC:FEATURE-001 요구사항\n\n### 배경\n여기에 배경을 작성하세요.\n\n### 문제\n여기에 문제를 작성하세요.\n\n`;
  ...
  },
  {
    ...
    severity: 'error',
    check: (content: string) => /@SPEC:[A-Z]+-\d+/.test(content),
    message: '@SPEC 섹션이 없습니다. 설계 섹션은 필수입니다.'
  },
  {
    name: 'has-task-section',
    description: '@CODE 섹션이 있는지 확인',
    severity: 'error',
    check: (content: string) => /@CODE:[A-Z]+-\d+/.test(content),
    message: '@CODE 섹션이 없습니다. 구현 섹션은 필수입니다.'
  },
  {
