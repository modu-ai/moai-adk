...
- **SPEC**: `@SPEC`, `@SPEC`, `@CODE` 태그가 포함된 상세 SPEC 우선 작성
...
- **3단계 워크플로우 추적**:
  - `/moai:1-spec`: @SPEC, @SPEC, @CODE 태그로 SPEC 작성
  - `/moai:2-build`: @TEST, @CODE 태그로 TDD 구현
  - `/moai:3-sync`: @DOC, @TAG 태그로 문서 동기화
...
- 새 TAG 생성 전 중복 확인: `rg "@SPEC:<키워드>" -n`
- TAG ID 규칙: `<도메인>-<3자리>` (예: AUTH-003)

### TAG BLOCK 템플릿

```text
# @CODE:<DOMAIN-ID> | Chain: @SPEC → @SPEC → @CODE → @TEST
# Related: @CODE:<ID>, @DOC:<ID>
```
