...
│   ├── @SPEC:MKDOCS-MATERIAL-001 (MkDocs Material 설계)
│   ├── @CODE:SITE-STRUCTURE-001 (사이트 구조 구현)
│   └── @TEST:SITE-FUNCTIONALITY-001 (사이트 기능 테스트)
├── @SPEC:CONTENT-AUTOMATION-001 (콘텐츠 자동화 요구사항)
│   ├── @SPEC:AUTO-GENERATION-001 (자동 생성 설계)
│   ├── @CODE:DOCS-AUTOMATION-001 (문서 자동화 구현)
│   └── @TEST:AUTO-SYNC-001 (자동 동기화 테스트)
└── @SPEC:UX-DESIGN-001 (UX 설계 요구사항)
    ├── @SPEC:RESPONSIVE-DESIGN-001 (반응형 설계)
    ├── @CODE:THEME-CUSTOMIZATION-001 (테마 커스터마이징)
...
- `@CODE:DOCS-001`: MkDocs 문서 빌더
- `@CODE:DOCS-002`: 메인 문서 빌더 클래스
- `@CODE:API-GEN-001`: API 문서 생성기
- `@CODE:API-GEN-002`: 자동 API 문서 생성기
- `@CODE:RELEASE-NOTES-001`: 릴리스 노트 변환기
- `@CODE:RELEASE-NOTES-002`: 릴리스 노트 변환 클래스

#### 2. 구현 작업 TAG (Task)
- `@CODE:DOC-BUILDER-001` ~ `@CODE:DOC-BUILDER-006`: 문서 빌더 작업들
- `@CODE:API-GEN-001` ~ `@CODE:API-GEN-007`: API 생성기 작업들
- `@CODE:RELEASE-NOTES-001` ~ `@CODE:RELEASE-NOTES-007`: 릴리스 노트 작업들
...
**@DOC:LIVING-DOCUMENT-001** - Living Document 원칙 완전 구현