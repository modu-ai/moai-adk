...
var fs, path, import_events, FileMonitor;
...
var fs2, path2, DEFAULT_MAPPINGS, LanguageDetector;
  ...
  "src/claude/hooks/workflow/language-detector.ts"() {
    ...
    LanguageDetector = class {
      ...
      detectProjectLanguages() {
        const rootPath = path2.resolve(this.projectRoot);
        const languages = [];
       ...
       */
      hasFile(filename) {
        return fs2.existsSync(path2.join(this.projectRoot, filename));
      }
      /**
       ...
       */
      loadMappings() {
        const mappingPath = path2.join(this.projectRoot, ".moai", "config", "language_mappings.json");
        try {
          if (fs2.existsSync(mappingPath)) {
            const data = fs2.readFileSync(mappingPath, "utf-8");
...
var import_child_process, fs3, path3, TIMEOUT_SECONDS, TestRunner;
var init_test_runner = __esm({
  "src/claude/hooks/workflow/test-runner.ts"() {
    ...
    TestRunner = class {
       ...
       */
      detectPytest() {
        const testsDir = path3.join(this.projectRoot, "tests");
        if (fs3.existsSync(testsDir) && this.commandExists("pytest")) {
          return {
            name: "pytest",
       ...
       */
      detectNpm() {
        const packageJson = path3.join(this.projectRoot, "package.json");
        if (fs3.existsSync(packageJson) && this.commandExists("npm")) {
          return {
            name: "npm test",
       ...
       */
      detectGo() {
        const goMod = path3.join(this.projectRoot, "go.mod");
        if (fs3.existsSync(goMod) && this.commandExists("go")) {
          return {
            name: "go test",
            args: ["go", "test", "./..."]
          };
       ...
       */
      detectCargo() {
        const cargoToml = path3.join(this.projectRoot, "Cargo.toml");
        if (fs3.existsSync(cargoToml) && this.commandExists("cargo")) {
          return {
            name: "cargo test",
...
var import_child_process2, fs4, path4, SessionNotifier;
var init_session_notice = __esm({
  "src/claude/hooks/session/session-notice.ts"() {
    ...
    SessionNotifier = class {
      ...
      constructor(projectRoot) {
        this.projectRoot = projectRoot || process.cwd();
        this.moaiConfigPath = path4.join(this.projectRoot, ".moai", "config.json");
      }
      async execute(input) {
        try {
          if (this.isMoAIProject()) {
      ...
      async getProjectStatus() {
        return {
          projectName: path4.basename(this.projectRoot),
      ...
      isMoAIProject() {
        const requiredPaths = [
          path4.join(this.projectRoot, ".moai"),
          path4.join(this.projectRoot, ".claude", "commands", "moai")
        ];
        return requiredPaths.every((p) => fs4.existsSync(p));
      }
      /**
      ...
      checkConstitutionStatus() {
        ...
        for (const filePath of criticalFiles) {
          if (!fs4.existsSync(path4.join(this.projectRoot, filePath))) {
      ...
      getMoAIVersion() {
        try {
          if (fs4.existsSync(this.moaiConfigPath)) {
            const configData = fs4.readFileSync(this.moaiConfigPath, "utf-8");
      ...
      getCurrentPipelineStage() {
        ...
        const specsDir = path4.join(this.projectRoot, ".moai", "specs");
        if (fs4.existsSync(specsDir)) {
          const hasSpecs = fs4.readdirSync(specsDir).some((dir) => fs4.existsSync(path4.join(specsDir, dir, "spec.md")));
          if (hasSpecs) {
      ...
      getSpecProgress() {
        ...
        try {
          const specDirs = fs4.readdirSync(specsDir).filter((name) => fs4.statSync(path4.join(specsDir, name)).isDirectory()).filter((name) => name.startsWith("SPEC-"));
          const totalSpecs = specDirs.length;
          let completed = 0;
          for (const specDir of specDirs) {
            const specPath = path4.join(specsDir, specDir, "spec.md");
            const planPath = path4.join(specsDir, specDir, "plan.md");
            if (fs4.existsSync(specPath) && fs4.existsSync(planPath)) {
              completed++;
            }
          }
  ...
  "src/claude/hooks/index.ts"() {
    ...
    HookSystem = class {
      ...
      static async loadConfig(projectRoot) {
        const configPath = path5.join(projectRoot, ".moai", "config", "hooks.json");
        try {
          if (fs5.existsSync(configPath)) {
            const configData = fs5.readFileSync(configPath, "utf-8");
            const config = JSON.parse(configData);
            return config;
          }
        } catch (error) {
...
var steering_guard_exports = {};
__export(steering_guard_exports, {
  SteeringGuard: () => SteeringGuard,
  main: () => main7
});
module.exports = __toCommonJS(steering_guard_exports);
...
var fs6, path6, os, BANNED_PATTERNS, SESSION_NOTIFIED_FILE, SteeringGuard;
var init_steering_guard = __esm({
  "src/claude/hooks/security/steering-guard.ts"() {
    ...
    SESSION_NOTIFIED_FILE = path6.join(os.tmpdir(), "moai_session_notified");
    SteeringGuard = class {
      name = "steering-guard";
      async execute(input) {
      ...
      checkMoAIProject() {
        ...
        const moaiPath = path6.join(currentDir, ".moai");
        const claudePath = path6.join(currentDir, "CLAUDE.md");
        return fs6.existsSync(moaiPath) && fs6.existsSync(claudePath);
      }
      /**
      ...
      checkHybridSystemStatus() {
        const currentDir = process.cwd();
        const tsProject = path6.join(currentDir, "moai-adk-ts");
        const hasTypeScript = fs6.existsSync(tsProject) && fs6.existsSync(path6.join(tsProject, "package.json"));
        const pythonBridge = path6.join(currentDir, "src", "moai_adk", "core", "bridge");
        const hasPythonBridge = fs6.existsSync(pythonBridge) && fs6.existsSync(path6.join(pythonBridge, "typescript_bridge.py"));
       ...
       */
      showSessionNotice() {
        if (fs6.existsSync(SESSION_NOTIFIED_FILE)) {
          return;
        }
        ...
        console.error("");
        try {
          fs6.writeFileSync(SESSION_NOTIFIED_FILE, "notified");
        } catch {
        }
      }
    };
