...
| `/alfred:9-feedback`        | สร้าง GitHub Issue แบบโต้ตอบ (ประเภท → ชื่อเรื่อง → คำอธิบาย → ลำดับความสำคัญ) | GitHub Issue + ป้ายกำกับอัตโนมัติ + ลำดับความสำคัญ + URL |
...
>
> 💡 **ใหม่ใน v0.7.0+**: ใช้ `/alfred:9-feedback` เพื่อสร้าง GitHub Issue ได้ทันทีระหว่างการพัฒนา ทีมทั้งหมดสามารถติดตามและอภิปรายปัญหาได้โดยไม่ยุ่งเกะกะการทำงาน
...
#### 2. PreToolUse (ก่อนดำเนินการเครื่องมือ)
...
| ----------- | ------------------------------------------------------------------------------------------------- | ---------- |
| **v0.8.2**  | 📖 การอัปเดตคำศัพท์ EARS: "Constraints" → "Unwanted Behaviors" (เพื่อความชัดเจน)                  | 2025-10-29 |
| **v0.8.1**  | 🔄 การเปลี่ยนชื่อคำสั่ง: `/alfred:9-help` → `/alfred:9-feedback` + การปรับปรุงเวิร์กโฟลว์ข้อเสนอแนะของผู้ใช้ | 2025-10-28 |
| **v0.8.0**  | 🏷️ ระบบสร้าง @DOC TAG อัตโนมัติ + การปรับปรุงการตรวจสอบเวอร์ชัน SessionStart                      | 2025-10-27 |
| **v0.7.0**  | 🌍 ระบบแปลเป็นภาษาท้องถิ่นอย่างสมบูรณ์ (อังกฤษ, เกาหลี, ญี่ปุ่น, จีน, สเปน)                         | 2025-10-26 |
| **v0.6.3**  | ⚡ เวิร์กโฟลว์อัปเดต 3 ขั้นตอน: การปรับปรุงประสิทธิภาพ 70-80% ผ่านการทำงานแบบขนาน                 | 2025-10-25 |
| **v0.6.0**  | 🏗️ การปรับโครงสร้างสถาปัตยกรรมหลัก + โครงสร้างเมตาดาต้า SPEC ที่ปรับปรุงแล้ว (7 จำเป็น + 9 เสริม)   | 2025-10-24 |
| **v0.5.7**  | 🎯 การทำงานอัตโนมัติของ SPEC → GitHub Issue + การรวม CodeRabbit + ความคิดเห็น PR อัตโนมัติ          | 2025-10-27 |
| **v0.4.11** | ✨ ระบบ TAG Guard + การปรับปรุงการจัดรูปแบบ CLAUDE.md + การทำความสะอาดโค้ด                        | 2025-10-23 |

> 📦 **ติดตั้งตอนนี้**: `uv tool install moai-adk` หรือ `pip install moai-adk`
...
| ไฟล์                           | ขนาด  | วัตถุประสงค์                         | ผู้ใช้                      |
| ------------------------------ | ----- | ------------------------------------ | --------------------------- |
| `CONFIG-SCHEMA.md`             | ~12KB | คำจำกัดความสคีมา `.moai/config.json` | project-manager             |
| `DEVELOPMENT-GUIDE.md`         | ~14KB | คู่มือเวิร์กโฟลว์ SPEC-First TDD     | Sub-agent ทั้งหมด, นักพัฒนา |
| `GITFLOW-PROTECTION-POLICY.md` | ~6KB  | นโยบายการป้องกันสาขา Git             | git-manager                 |
| `SPEC-METADATA.md`             | ~9KB  | มาตรฐาน SPEC YAML frontmatter (SSOT) | spec-builder, doc-syncer    |

**การวิเคราะห์การนำไปใช้ (7 ไฟล์)**: รายงานภายในและเอกสารนโยบายสำหรับการจัดการ Skills การปรับปรุงเวิร์กโฟลว์ และการวิเคราะห์การรวมทีม

### ไฟล์หน่วยความจำจะถูกโหลดเมื่อใด?
...
**Just-In-Time (การดำเนินการคำสั่ง)**:

- `/alfred:1-plan` → `SPEC-METADATA.md`, `DEVELOPMENT-GUIDE.md`
- `/alfred:2-run` → `DEVELOPMENT-GUIDE.md`
- `/alfred:3-sync` → `DEVELOPMENT-GUIDE.md`

**แบบมีเงื่อนไข (ตามความจำเป็น)**:

- การเปลี่ยนแปลง Config → `CONFIG-SCHEMA.md`
- การดำเนินการ Git → `GITFLOW-PROTECTION-POLICY.md`
- การสร้าง Skill → `SKILLS-DESCRIPTION-POLICY.md`

### เหตุใดไฟล์หน่วยความจำจึงสำคัญ

1. **แหล่งความจริงเดียว (SSOT)**: มาตรฐานแต่ละอันถูกกำหนดเพียงที่เดียว ขจัดความขัดแย้ง
2. **ประสิทธิภาพบริบท**: การโหลด JIT ลดภาระเซสชันเริ่มต้น (เริ่มต้นเพียง 3 ไฟล์)
3. **การตัดสินใจที่สอดคล้อง**: Sub-agent ทั้งหมดปฏิบัติตามกฎเดียวกันจาก `CLAUDE-RULES.md`
4. **การตรวจสอบย้อนกลับได้**: เมตาดาต้า SPEC กฎ @TAG มาตรฐาน Git ทั้งหมดมีเอกสาร

### ความถี่ในการใช้งาน

| ลำดับความสำคัญ | ไฟล์                                               | รูปแบบการใช้งาน    |
| -------------- | -------------------------------------------------- | ------------------ |
| สูงมาก         | `CLAUDE-RULES.md`                                  | การตัดสินใจทั้งหมด |
| สูง            | `DEVELOPMENT-GUIDE.md`, `SPEC-METADATA.md`         | คำสั่งทั้งหมด      |
| ปานกลาง        | `CLAUDE-AGENTS-GUIDE.md`, `CLAUDE-PRACTICES.md`    | การประสานงาน Agent |
| ต่ำ            | `CONFIG-SCHEMA.md`, `GITFLOW-PROTECTION-POLICY.md` | การดำเนินการเฉพาะ  |

> 📚 **การวิเคราะห์ฉบับสมบูรณ์**: ดู `.moai/memory/MEMORY-FILES-USAGE.md` สำหรับเอกสารที่ครอบคลุมเกี่ยวกับว่าใครใช้แต่ละไฟล์ โหลดเมื่อใด อ้างอิงที่ไหน และทำไมจึงจำเป็น
...
MoAI-ADK ไม่ใช่แค่เครื่องมือสร้างโค้ด Alfred SuperAgent กับทีม 19 คน และร่วมกันรับประกัน:
...
- ✅ Tests: 467/476 ผ่าน (85.60% coverage)
- 🏷️ TAG Guard: การตรวจสอบ @TAG อัตโนมัติใน PreToolUse Hook
