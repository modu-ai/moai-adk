...
| **cc-manager**   | **중앙 관제탑** | **모든 Claude Code 표준화 통제**    |
| **debug-helper** | 오류 진단       | 일반 오류 분석 + 개발 가이드 검사   |

## Claude Code 중앙 관제탑 (cc-manager)

### 🎯 표준화 통제 시스템

**cc-manager**는 MoAI-ADK Claude Code 환경의 중앙 관제탑으로서, 모든 커맨드/에이전트 파일의 생성, 표준화, 검증을 통제합니다.

#### 핵심 기능

- **표준 템플릿 강제**: 모든 Claude Code 파일이 표준 구조를 준수하도록 자동 검증
- **중구난방 방지**: 상충하는 가이드라인을 사전 차단하고 일관성 유지
- **실시간 검증**: 파일 생성/수정 시 표준 위반 사항을 즉시 감지
- **자동 수정 제안**: 비표준 파일 발견 시 구체적인 개선 방안 제시

#### 자동 실행 조건

- MoAI-ADK 프로젝트에서 Claude Code 파일 작업 시 자동 활성화
- 커맨드/에이전트 생성/수정 요청 시 표준 검증
- 설정 최적화가 필요한 경우 자동 개입

#### 직접 호출

```bash
@agent-cc-manager "새 에이전트 생성: [에이전트명]"
@agent-cc-manager "표준 검증"
