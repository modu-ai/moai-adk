...
| 에이전트              | 전문 영역               | 호출 방법              | 사용 시점          |
| --------------------- | ----------------------- | ---------------------- | ------------------ |
| **Explore** 🔍         | 코드베이스 탐색, 파일 검색 | `@agent-Explore`       | 코드베이스 탐색 시 |
...
❌ SPEC 문서 작성 실패: .moai/specs/ 디렉토리 권한 거부
  → Unix/Linux/macOS: chmod 755 .moai/specs 실행 후 재시도
  → Windows: 관리자 권한으로 실행하거나 디렉토리 속성에서 권한 확인
...
**플랫폼별 오류 메시지 제공**:
모든 에이전트는 플랫폼을 자동 감지하여 적절한 해결 방법을 제공합니다.
- **Windows**: PowerShell 명령어, 관리자 권한, 경로 구분자 `\` 사용
- **Unix/Linux/macOS**: bash 명령어, chmod/chown, 경로 구분자 `/` 사용
