#  | SPEC: SPEC-LOGGING-001/spec.md | TEST: tests/unit/test_logger.py
"""
...
class SensitiveDataFilter(logging.Filter):
    ...
    # :DOMAIN - Define sensitive data patterns
    PATTERNS = [
...
def setup_logger(
    ...
    """
    # :DOMAIN - Determine logging level
    if level is None:
    ...
    # :INFRA - Create and configure logger
    logger = logging.getLogger(name)
    logger.setLevel(level)
    logger.handlers.clear()  # Remove existing handlers to avoid duplicates

    # :INFRA - Ensure log directory exists
    if log_dir is None:
        log_dir = ".moai/logs"
    log_path = Path(log_dir)
    log_path.mkdir(parents=True, exist_ok=True)

    # :INFRA - Define log format
    formatter = logging.Formatter(
        fmt="[%(asctime)s] [%(levelname)s] [%(name)s] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S",
    )

    # :INFRA - Console handler
