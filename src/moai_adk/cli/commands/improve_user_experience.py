...
def format_metrics_table(metrics: dict) -> Table:
    ...
    table.add_row("Performance", f"{perf.success_rate if perf else 0:.2f}",
                  "✅ Good" if (perf and perf.is_good) else "❌ Needs Improvement")

    # Navigation metrics
    nav = metrics.get('navigation')
    table.add_row("Navigation", f"{nav.structure_score if nav else 0:.2f}",
                  "✅ Good" if (nav and nav.is_good) else "❌ Needs Improvement")

    # Content metrics
    content = metrics.get('content')
    table.add_row("Content", f"{content.accuracy_score if content else 0:.2f}",
                  "✅ Good" if (content and content.is_good) else "❌ Needs Improvement")

    # Accessibility metrics
    acc = metrics.get('accessibility')
    table.add_row("Accessibility", f"{1.0 if (acc and acc.is_good) else 0.0:.2f}",
...
def format_recommendations(recommendations: list) -> Table:
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        if "error" in rec.lower() or "keyboard" in rec.lower() or "accuracy" in rec.lower():
            priority = "High"
...
def generate_markdown_report(analysis_report: dict, improvement_plan: dict) -> str:
    ...
    report.append(f"- Success Rate: {performance.success_rate if performance else 0:.2f}")
    report.append(f"- Average Load Time: {performance.load_time if performance else 0:.2f}s")
    report.append(f"- Response Time: {performance.response_time if performance else 0:.2f}s")
    ...
    report.append(f"- Structure Score: {navigation.structure_score if navigation else 0:.2f}")
    ...
    report.append(f"- Keyboard Navigation: {'✅' if (accessibility and accessibility.keyboard_navigation) else '❌'}")
    report.append(f"- Screen Reader Support: {'✅' if (accessibility and accessibility.screen_reader_support) else '❌'}")
    report.append(f"- Color Contrast: {'✅' if (accessibility and accessibility.color_contrast) else '❌'}")
    report.append(f"- Responsive Design: {'✅' if (accessibility and accessibility.responsive_design) else '❌'}")
    report.append(f"- ARIA Labels: {'✅' if (accessibility and accessibility.aria_labels) else '❌'}")
    report.append("")
    ...
    report.append("")
    report.append(f"**Estimated Duration**: {improvement_plan.get('estimated_duration', 'N/A')}")
    report.append("")
