...
/moai:1-plan 명령어 실행 시 스펙 생성이 안 됩니다.

## 재현 단계

1. `/moai:1-plan "새로운 기능 추가"` 명령 실행
2. 단계 1~4를 모두 진행
3. 최종 확인 후 생성 버튼 클릭
...
/moai:9-feedback 명령어에 자동 정보 수집 기능이 필요합니다.
버그 리포트 시 환경 정보(버전, OS, Git 상태)를 자동으로 수집하면 좋을 것 같습니다.

## 사용 시나리오

버그를 발견한 사용자가 /moai:9-feedback을 실행하면,
...
/moai:9-feedback 실행 시 4단계 질문 (이슈 타입 → 제목 → 설명 → 우선순위)
...
/moai:9-feedback의 AskUserQuestion 디자인 개선을 검토중입니다.
