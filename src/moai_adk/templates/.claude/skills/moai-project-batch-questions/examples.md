...
**User**: Runs `/moai:0-project` on a new project
...
if (detectedMode === "team") {
  const teamBatch = await AskUserQuestion({
    questions: [
      {
        question:
          "GitHub 저장소의 'Automatically delete head branches' 설정 상태는?",
        header: "GitHub 설정",
        multiSelect: false,
        options: [
...
const config = {
  language: {
    conversation_language:
      languageBatch["언어 선택"] === "한국어" ? "ko" : "en",
    agent_prompt_language:
      languageBatch["에이전트 언어"] === "English (Global)"
        ? "english"
        : "localized",
    conversation_language_name:
      languageBatch["언어 선택"] === "한국어" ? "한국어" : "English",
  },
  user: {
...
if (teamBatch) {
  config.github = {
    ...
    spec_git_workflow:
      teamBatch["Git 워크플로우"] === "Feature Branch + PR"
        ? "feature_branch"
        : "develop_direct",
...
**User**: Runs `/moai:0-project setting` to modify existing configuration
  ...
  questions: [
    {
      ...
      options: [
        ...
        },
        {
          label: "팀 모드 설정",
          description:
            currentConfig.mode === "team" ? "팀 모드 활성화됨" : "개인 모드",
        },
        {
          label: "보고서 생성",
  ...
  questions: [
    ...
    },
    {
      ...
      options: [
        ...
        {
          label: "Java",
          description: "스프링 부트, 엔터프라이즈 애플리케이션",
        },
      ],
    },
  ],
...
class BatchPerformanceTracker {
  ...
  generateReport(): PerformanceReport {
    ...
    const avgTimeSpent =
      this.metrics.reduce((sum, m) => sum + m.timeSpent, 0) / totalBatches;
