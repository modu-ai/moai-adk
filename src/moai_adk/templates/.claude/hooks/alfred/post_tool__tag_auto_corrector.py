...
Real-time monitoring detects missing TAGs and generates correction suggestions.

Features:
- TAG state validation after file modification
- Auto-generation suggestions for missing TAGs
- TAG chain integrity verification
- Auto-correction feature (based on configuration)
...
def should_monitor_tool(tool_name: str, tool_args: Dict[str, Any], result: Dict[str, Any]) -> bool:
    ...
    Returns:
        True if tool should be monitored
    """
...
def create_checkpoint_if_needed(rollback_manager: RollbackManager, file_paths: List[str]) -> Optional[str]:
    ...
    """
    try:
        # Create checkpoint only when important files are changed
...
def main() -> None:
    ...
    try:
        ...
        if applied_corrections:
            ...
            response["message"] = f"✅ {len(applied_corrections)} auto-corrections applied successfully"
            ...
            response["message"] = "✅ TAG policy compliance confirmed"

        print(json.dumps(response, ensure_ascii=False, indent=2))

    except Exception as e:
        # Log only and continue on exception
        error_response = {
            "monitoring_completed": False,
            "error": f"Hook execution error: {str(e)}",
            "message": "Error occurred during hook execution but handled normally"
        }
