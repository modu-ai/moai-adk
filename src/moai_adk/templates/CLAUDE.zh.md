...
**问题**：当多个代理并行操作时，可能会同时修改同一文件，导致冲突和数据丢失。

**解决方案**：并行执行前进行依赖分析

**执行前检查清单**：

1. **文件访问分析**：收集每个代理将访问的文件，识别重叠的文件访问模式，检测读写冲突
2. **依赖图构建**：映射代理间文件依赖关系，识别独立任务集，标记依赖任务集
3. **执行模式选择**：并行（无文件重叠）、顺序（检测到文件重叠）、混合（部分重叠）

### 代理工具要求

所有执行代码修改的代理必须包含 Read、Write、Edit、Grep、Glob、Bash、TodoWrite 工具。

### 循环预防守卫

**重试策略**：每个操作最多重试 3 次，检测失败模式，使用回退链，3 次失败后请求用户指导

### 平台兼容性

**最佳实践**：文件修改优先使用 Edit 工具而非 sed/awk。Edit 工具是跨平台的，可避免特定平台的语法问题。
...
MoAI-ADK 使用 Memory MCP 服务器提供跨会话的持久存储。这使得用户偏好保留、项目上下文保存和学习模式存储成为可能。

### 内存类别

**用户偏好** (前缀: `user_`):
- `user_language`: 对话语言
- `user_coding_style`: 首选编码约定
- `user_naming_convention`: 变量命名风格

**项目上下文** (前缀: `project_`):
- `project_tech_stack`: 使用的技术
- `project_architecture`: 架构决策
- `project_conventions`: 项目特定规则

**学习模式** (前缀: `pattern_`):
- `pattern_preferred_libraries`: 常用库
- `pattern_error_resolutions`: 常见错误解决方案

**会话状态** (前缀: `session_`):
- `session_last_spec`: 上次工作的 SPEC ID
- `session_pending_tasks`: 未完成任务

### 使用协议

**会话开始时：**
1. 检索 `user_language` 并应用于响应
...
使用 `mcp__memory__*` 工具：
- `mcp__memory__store`: 存储键值对
- `mcp__memory__retrieve`: 获取存储的值
- `mcp__memory__list`: 列出所有键
- `mcp__memory__delete`: 删除键

### 代理间上下文共享

Memory MCP 使代理能够在工作流执行期间共享上下文。
...
**工作流示例：**
1. manager-spec 存储: `context_SPEC-001_requirements`
2. manager-ddd 检索: `context_SPEC-001_requirements`
3. expert-backend 存储: `context_SPEC-001_api`
4. expert-frontend 检索: `context_SPEC-001_api`
5. manager-docs 检索所有: `context_SPEC-001_*`

**已启用的代理：**
- manager-spec, manager-ddd, manager-docs, manager-strategy
- expert-backend, expert-frontend

有关详细模式，请参阅 Skill("moai-foundation-memory")。
