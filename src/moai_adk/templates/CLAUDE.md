...
프로젝트의 locale 설정은 `.moai/config.json` 파일에 저장됩니다:
...
프로젝트 locale 설정은 다음 항목에 영향을 줍니다:

#### 1. SessionStart Hook 메시지
Claude Code 세션 시작 시 프로젝트 locale에 맞는 언어로 메시지가 표시됩니다:
...
#### 2. Git 커밋 메시지
Git 커밋 메시지는 프로젝트 locale에 따라 자동으로 생성됩니다:
...
**태국어 (th)**:
```bash
🔴 RED: [คำอธิบายการทดสอบ]
🟢 GREEN: [คำอธิบายการนำไปใช้]
♻️ REFACTOR: [คำอธิบายการปรับปรุง]
📝 DOCS: [คำอธิบายเอกสาร]
```

#### 3. CLI 명령어 출력
`moai-adk` CLI 도구의 모든 출력 메시지가 locale에 맞게 표시됩니다:

```bash
# 한국어 (ko)
moai-adk init
→ "MoAI-ADK 프로젝트 초기화 중..."
→ "✅ 프로젝트 'my-project' 초기화 완료"
...
#### 4. README 문서
프로젝트 초기화 시 해당 locale의 README 파일이 생성됩니다:
- `README.md` - 영어 (기본)
- `README.ko.md` - 한국어
- `README.ja.md` - 일본어
- `README.zh.md` - 중국어
...
#### 기존 프로젝트 변경
`.moai/config.json` 파일을 직접 수정:
```json
{
  "project": {
    "locale": "en"  // ko → en으로 변경
  }
}
```

변경 후 Claude Code를 재시작하면 새로운 locale이 적용됩니다.

### i18n 메시지 파일 위치

모든 번역 메시지는 다음 위치에 저장됩니다:
- **Hook 메시지**: `.claude/hooks/alfred/core/i18n.py` + `docs/i18n/*.json`
- **CLI 메시지**: `src/moai_adk/i18n.py` + `docs/i18n/*.json`
- **메시지 파일**: `docs/i18n/{locale}.json` (ko, en, ja, zh, th)

### 커스텀 메시지 추가

프로젝트별 커스텀 메시지를 추가하려면 `docs/i18n/{locale}.json` 파일을 수정하세요:

```json
{
  "custom_message_key": "커스텀 메시지",
  "another_key": "{name}님 환영합니다"
}
```
...
# 간단한 메시지
message = t("custom_message_key", locale="ko")

# 변수 포함 메시지
message = t("another_key", locale="ko", name="홍길동")
```