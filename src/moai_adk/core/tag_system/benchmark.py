...
class MemoryProfiler:
    ...
    def create_comparison_report(self, json_usage: int, sqlite_usage: int, dataset_size: int) -> MemoryProfileResult:
        """메모리 사용량 비교 리포트 생성"""
        json_usage_mb = json_usage / 1024 / 1024
        sqlite_usage_mb = sqlite_usage / 1024 / 1024

        efficiency_improvement = ((json_usage_mb - sqlite_usage_mb) / json_usage_mb) * 100
...
class TagPerformanceBenchmark:
    ...
    def create_performance_report(self, json_results: Dict[str, float],
        ...
        overall_improvement = sum(improvement_ratios) / len(improvement_ratios) if improvement_ratios else 0
