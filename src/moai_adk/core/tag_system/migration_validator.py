...
from .migration_models import (
    ValidationError, MigrationResult, BackupInfo
)
...
class MigrationValidator:
    ...
    def validate_json_data(self, json_data: Dict[str, Any], strict_mode: bool = False) -> List[ValidationError]:
        """JSON 데이터 검증"""
        errors = []

        # 버전 확인
        if "version" not in json_data:
            errors.append(ValidationError("missing_field", "버전 정보가 없습니다", "root.version"))

        # 인덱스 데이터 확인
        if "index" not in json_data:
            errors.append(ValidationError("missing_field", "인덱스 데이터가 없습니다", "root.index"))
        else:
            index_data = json_data["index"]
            if not isinstance(index_data, dict):
                errors.append(ValidationError("invalid_type", "인덱스는 딕셔너리여야 합니다", "root.index"))
            else:
                # 각 TAG 키 검증
        ...
        if "references" in json_data:
            ...
            if not isinstance(references_data, dict):
                errors.append(ValidationError("invalid_type", "참조는 딕셔너리여야 합니다", "root.references"))
            else:
                # 각 참조 관계 검증
    ...
    def generate_detailed_statistics(self, result: MigrationResult, db_manager: TagDatabaseManager):
        """상세 통계 생성"""
        try:
            # 카테고리별 통계
