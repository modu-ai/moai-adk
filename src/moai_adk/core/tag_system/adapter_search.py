        ...
        try:
            ...
            for tag in tags:
                ...
                if include_tag:
                    ...
                    enriched_tag['category_group'] = self._get_category_group(tag.get('category', ''))
                    enriched_tag['tag_key'] = f"{tag.get('category', '')}:{tag.get('identifier', '')}"
            ...
            if sort_by == 'identifier':
                filtered_tags.sort(key=lambda x: x.get('identifier', ''), reverse=reverse)
            elif sort_by == 'file_path':
                filtered_tags.sort(key=lambda x: x.get('file_path', ''), reverse=reverse)
            elif sort_by == 'line_number':
                filtered_tags.sort(key=lambda x: x.get('line_number', 0), reverse=reverse)
        ...
        try:
            ...
            if direction in ['forward', 'both']:
                ...
                forward_chain = self._build_reference_chain(
                    start_tag['id'], 'forward', max_depth, forward_visited, include_details, category_filter
                )
                result['forward_chain'] = forward_chain
                result['statistics']['total_forward'] = self._count_chain_nodes(forward_chain)

            # Backward chain (이 TAG를 참조하는 것들)
            if direction in ['backward', 'both']:
                backward_visited = {start_tag['id']}
                backward_chain = self._build_reference_chain(
                    start_tag['id'], 'backward', max_depth, backward_visited, include_details, category_filter
                )
                result['backward_chain'] = backward_chain
                result['statistics']['total_backward'] = self._count_chain_nodes(backward_chain)
            ...
            result['statistics']['max_depth_reached'] = min(max_depth,
                max(self._get_chain_depth(result['forward_chain']),
        ...
        for ref in references:
            ...
            if include_details:
                node.update({
                    ...
                    'category_group': self._get_category_group(target_tag['category'])
                })

            # 재귀적으로 다음 레벨 구축
            sub_chain = self._build_reference_chain(
                target_tag_id, direction, max_depth, visited.copy(),
                include_details, category_filter, current_depth + 1
            )
            if sub_chain:
