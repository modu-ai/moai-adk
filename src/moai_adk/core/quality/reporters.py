...
class ViolationReporter:
    ...
    def generate_violation_report(self, violations: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        report = {
            'summary': {
                ...
                'compliance_rate': self._calculate_compliance_rate(total_files_scanned, violations)
            },
            'violations_by_type': {
    ...
    def _count_files_with_violations(self, violations: Dict[str, List[Dict[str, Any]]]) -> int:
        ...
        for violation_list in violations.values():
            for violation in violation_list:
                if violation.get('violation', True):  # Default True for actual violations
                    if 'file_path' in violation:
    ...
    def _get_worst_violations(self, violations: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if file_violations:
            ...
            worst['largest_file'] = {
                ...
                'times_over_limit': round(worst_file.get('line_count', 0) / self.limits.MAX_FILE_LINES, 2)
            }
        ...
        if func_violations:
            ...
            worst['longest_function'] = {
                ...
                'times_over_limit': round(worst_func.get('line_count', 0) / self.limits.MAX_FUNCTION_LINES, 2)
            }

        # Worst complexity violation
        complexity_violations = violations.get('complexity', [])
        if complexity_violations:
            worst_complex = max(complexity_violations, key=lambda x: x.get('complexity', 0))
            worst['most_complex_function'] = {
                ...
                'times_over_limit': round(worst_complex.get('complexity', 0) / self.limits.MAX_COMPLEXITY, 2)
            }
    ...
    def _generate_recommendations(self, violations: Dict[str, List[Dict[str, Any]]]) -> List[str]:
        ...
        if not recommendations:
            recommendations.append("All code complies with TRUST 5 principles guidelines!")
