...
class SystemManager:
    ...
    def check_nodejs_and_npm(self) -> bool:
        ...
        if not self._check_command_exists("node"):
            print("⚠️  Node.js가 설치되어 있지 않습니다.")
            print("   ccusage statusLine 기능을 사용하려면 Node.js가 필요합니다.")
            print("   Node.js 설치: https://nodejs.org")
            return False

        # Check npm
        if not self._check_command_exists("npm"):
            print("⚠️  npm이 설치되어 있지 않습니다.")
            print("   ccusage statusLine 기능을 사용하려면 npm이 필요합니다.")
            return False
    ...
    def _validate_nodejs_environment(self) -> bool:
        ...
        try:
            ...
            print(f"✅ Node.js {node_version} 감지됨")
            print(f"✅ npm {npm_version} 감지됨")
    ...
    def _test_ccusage_availability(self) -> bool:
        ...
        try:
            ...
            if ccusage_result.returncode == 0 or "ccusage" in ccusage_result.stdout.lower():
                print("✅ ccusage 패키지 접근 가능 확인됨")
                print("💡 statusLine에서 실시간 Claude Code 사용량 추적이 활성화됩니다.")
                return True
            else:
                print("⚠️  ccusage 패키지 접근 실패")
                print("   statusLine 기능이 제한될 수 있습니다.")
                return False

        except subprocess.TimeoutExpired:
            print("⚠️  ccusage 접근 테스트 시간 초과")
            print("   네트워크 상태를 확인해주세요.")
            return False
        except Exception as e:
            logger.error("ccusage test error: %s", e)
            print(f"⚠️  ccusage 테스트 중 오류: {e}")
            return False
