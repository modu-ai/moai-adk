...
class TraceabilityChecker:
    ...
    def check_traceability_chains(self, tags: dict[str, list[str]]) -> dict[str, any]:
        ...
        for tag in tags:
            ...
            related_count = sum(
                1 for other_tag in tags if other_tag != tag and tag_id in other_tag
            )
...
def print_report(report: dict[str, any], verbose: bool = False):
    ...
    # 전체 결과 판정
    overall_score = (
        (summary["valid_tags"] / max(summary["total_tags"], 1)) * 0.4
...
def main():
    ...
    parser.add_argument("--output", "-o", type=Path, help="JSON 보고서 파일 출력")
    parser.add_argument(
        "--threshold", type=float, default=0.8, help="통과 기준 점수 (0.0-1.0)"
    )
