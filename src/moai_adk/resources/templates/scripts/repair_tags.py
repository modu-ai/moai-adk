...
class TagRepairer:
                ...
                for code_file in src_path.rglob("*"):
                    if code_file.suffix in [".py", ".js", ".ts", ".tsx", ".jsx", ".md"]:
                        try:
                            ...
                            for tag in tags:
                                all_tags[tag].append(
                                    str(code_file.relative_to(self.project_root))
                                )
                        except (UnicodeDecodeError, PermissionError):
                            continue
        ...
        for tag, locations in all_tags.items():
            try:
                ...
                if tag_type == "REQ":
                    ...
                    matching_tasks = [
                        t for t in all_tags.keys() if t.startswith(task_pattern)
                    ]
                    ...
                    matching_tests = [
                        t for t in all_tags.keys() if t.startswith(test_pattern)
                    ]
    ...
    def has_references(self, tag: str, all_tags: dict[str, list[str]]) -> bool:
        ...
        try:
            ...
            return any(
                tag_id in other_tag for other_tag in all_tags if other_tag != tag
            )
        except IndexError:
            return False

    def generate_repair_preview(
        self, missing_links: list[tuple[str, str]]
    ) -> list[dict[str, any]]:
        """수리 미리보기 생성"""
        preview = []
    ...
    def update_traceability_index(self):
        ...
        for chain_name, chain_types in self.traceability_chains.items():
            for i in range(len(chain_types) - 1):
                ...
                from_tags = [
                    tag for tag in all_tags.keys() if tag.startswith(f"@{from_type}:")
                ]
                to_tags = [
                    tag for tag in all_tags.keys() if tag.startswith(f"@{to_type}:")
                ]
...
def main():
    ...
    parser.add_argument(
        "--project-root", type=Path, default=Path.cwd(), help="프로젝트 루트 디렉토리"
    )
    parser.add_argument(
        "--dry-run", action="store_true", default=True, help="미리보기 모드 (기본값)"
    )
    parser.add_argument("--execute", action="store_true", help="실제 수리 실행")
    parser.add_argument("--auto", action="store_true", help="CI/CD용 자동 실행")
