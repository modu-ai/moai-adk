    ...
    def validate_specify_stage(self) -> dict[str, any]:
        ...
        for spec_file in spec_files:
            if spec_file.exists():
                ...
                # EARS 키워드 확인
                ears_keywords = re.findall(
                    r"\b(WHEN|IF|WHILE|WHERE|UBIQUITOUS)\b", content
                )
                if not ears_keywords:
                    ...
                    results["checks"].append(
                        f"{spec_file}: EARS 키워드 {len(ears_keywords)}개 확인"
                    )
                ...
                if clarifications:
                    results["warnings"].append(
                        f"{spec_file}: 미해결 명확화 항목 {len(clarifications)}개"
                    )
                ...
                if not user_stories:
                    ...
                    results["checks"].append(
                        f"{spec_file}: User Story {len(user_stories)}개 확인"
                    )
        ...
        for plan_file in plan_files:
            if plan_file.exists():
                ...
                if not research_path.exists():
                    results["warnings"].append(
                        f"{research_path}: research.md 파일 누락"
                    )
    ...
    def validate_tasks_stage(self) -> dict[str, any]:
        ...
        for tasks_file in tasks_files:
            if tasks_file.exists():
                ...
                # TDD 마커 확인
                if (
                    "Tests First (TDD)" not in content
                    and "RED-GREEN-REFACTOR" not in content
                ):
                ...
                if not parallel_markers:
                    ...
                    results["checks"].append(
                        f"{tasks_file}: 병렬 작업 {len(parallel_markers)}개 확인"
                    )
                ...
                if not task_numbers:
                    ...
                    results["checks"].append(
                        f"{tasks_file}: 태스크 {len(task_numbers)}개 확인"
                    )
...
def main():
    ...
    parser.add_argument(
        "--stage",
        choices=["SPECIFY", "PLAN", "TASKS", "IMPLEMENT", "ALL"],
        default="ALL",
        help="검증할 단계",
    )
    parser.add_argument(
