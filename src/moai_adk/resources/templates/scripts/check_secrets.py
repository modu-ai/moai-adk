...
class SecretsScanner:
                    ...
                    for pattern in patterns:
                        ...
                        for match in matches:
                            ...
                            if matched_value.lower() in {
                                v.lower() for v in self.allowed_values
                            }:
                                continue

                            # 주석이나 문서인 경우 severity 낮춤
                            severity = self.determine_severity(
                                line, secret_type, matched_value
                            )

                            secrets.append(
                                {
                                    "type": secret_type,
                                    "value": matched_value[:20] + "..."
    ...
    def determine_severity(self, line: str, secret_type: str, value: str) -> str:
        ...
        if any(
            ...
            for keyword in ["todo", "fixme", "example", "예시", "샘플"]
        ):
            return "LOW"
    ...
    if verbose or summary["high_severity"] > 0 or summary["medium_severity"] > 0:
        ...
        for file_path, file_secrets in report["secrets"].items():
            ...
            filtered_secrets = (
                file_secrets
                if verbose
                else [s for s in file_secrets if s["severity"] in ["HIGH", "MEDIUM"]]
            )
...
def main():
    ...
    parser.add_argument("--output", "-o", type=Path, help="JSON 보고서 출력 파일")
    parser.add_argument(
        "--fail-on",
