...
class TrustPrinciplesChecker:
    ...
    def check_simplicity_principle(self) -> Dict[str, Any]:
                ...
                for spec_dir in specs_dir.glob("SPEC-*"):
                    if spec_dir.is_dir():
                        ...
                        if spec_file.exists():
                            ...
                            if not re.search(
                                r"\[COMPLETED\]|\[CANCELLED\]", content, re.IGNORECASE
                            ):
        ...
        if project_count > self.thresholds["max_projects"]:
            issues.append(
                f"활성 프로젝트 {project_count}개 > 최대 허용 {self.thresholds['max_projects']}개"
            )
        ...
        return {
            "passed": project_count <= self.thresholds["max_projects"]
            and project_count > 0,
    ...
    def check_architecture_principle(self) -> Dict[str, Any]:
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            try:
                ...
                # 라이브러리 패턴 감지
                if re.search(
                    r"class\s+\w+\(.*\):|def\s+\w+\(.*\).*->.*:|from\s+\.\w+\s+import",
                    content,
                ):
        ...
        if not has_dependency_management:
            issues.append(
                "의존성 관리 파일 없음 (package.json, pyproject.toml, requirements.txt 중 하나 필요)"
            )
        ...
        if not has_claude_code_manager:
            issues.append(
                "claude-code-manager 에이전트가 없음 (Claude Code 통합 관리 필수)"
            )
    ...
    def check_testing_principle(self) -> Dict[str, Any]:
        ...
        if test_coverage < self.thresholds["min_test_coverage"]:
            issues.append(
                f"테스트 커버리지 {test_coverage}% < 최소 요구 {self.thresholds['min_test_coverage']}%"
            )
        ...
        for py_file in self.project_root.rglob("*.py"):
            ...
            try:
                ...
                if re.search(r"import logging|from logging", content):
                    ...
                    # 구조화된 로깅 확인
                    if re.search(
                        r"logging\.basicConfig|logging\.getLogger.*\.info|log\.info.*{",
                        content,
                    ):
    ...
    def check_versioning_principle(self) -> Dict[str, Any]:
                ...
                try:
                    ...
                    for version in matches:
                        version_files.append(
                            str(version_file.relative_to(self.project_root))
                        )
                        ...
                        if re.match(r"^\d+\.\d+\.\d+(-\w+(\.\d+)?)?$", version):
                            ...
                            issues.append(
                                f"잘못된 버전 형식 {version} in {version_file.name}"
                            )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                valid_git_tags = [
                    tag for tag in git_tags if re.match(r"^v?\d+\.\d+\.\d+", tag)
                ]
    ...
    def generate_report(self, output_path: Optional[Path] = None) -> str:
        ...
        for key, name in principles.items():
            ...
            report_lines.extend(
                [f"## {name}", f"상태: {status} ({result['score']}/100)", ""]
            )
...
def main():
    ...
    # 개발 가이드 검증 실행
    checker = TrustPrinciplesChecker(project_root, verbose=args.verbose)
    results = checker.run_full_check()

    # 결과 출력
    report = checker.generate_report(Path(args.report) if args.report else None)
