...
| 에이전트 | 역할 | 모델 | 출처 |
|----------|------|------|------|
| planner | 요청을 BRIEF로 확장 | opus | fork: manager-spec |
| copywriter | 마케팅 카피 (JSON) | sonnet | new |
| designer | 디자인 시스템 & UI 스펙 | sonnet | new |
| builder | 코드 구현 (TDD) | sonnet | fork: expert-frontend |
| evaluator | Playwright 테스트 & 스코어링 | sonnet | fork: evaluator-active |
...
피드백은 `learnings.md`에 축적되며, 신뢰도 >= 0.80 및 관측 5회 이상일 때 스킬 규칙으로 승격됩니다.

### 빠른 시작

```bash
/agency brief "내 AI 스타트업을 위한 SaaS 랜딩 페이지"
