...
### 논거 1 — 품질은 프롬프트가 아니라 구조가 만든다

프롬프트로 심은 규율은 컨텍스트가 압축되는 순간 사라진다. "테스트 먼저", "커버리지 85%", "작성자와 검토자 분리" 같은 기준은 매 턴 다시 말해줄 수 없고, 말해줘도 모델이 지켰는지 스스로 증명하지 못한다. MoAI-ADK는 이 기준을 파이프라인으로 강제한다 — 모든 변경은 SPEC 3-페이즈(plan → run → sync)를 통과하고, TRUST 5 게이트(85%+ 커버리지 포함)가 통과 증거를 요구하며, 계획을 쓴 에이전트와 감사하는 에이전트를 분리해 자기 채점을 막는다. "된 것 같다"가 아니라 테스트 출력과 인수 기준이 완료를 판정한다.

### 논거 2 — 비용은 모델 단가가 아니라 배정이 결정한다

아래 [2.0에서 3.0으로](#20에서-30으로) 절의 실측이 보여주듯, 같은 Claude 계열 안에서도 해결 과제당 비용은 두 배 넘게 갈린다 — 약한 모델을 최고 effort로 굴리는 쪽이 오히려 비싸고 점수도 낮다. 이 배정을 사람이 매 작업마다 수동으로 맞출 수는 없다. MoAI-ADK는 Tier×Phase 매트릭스로 모델과 추론 깊이를 선언적으로 배정하고, 검증 로그를 디스크로 리다이렉트해 컨텍스트를 다이어트하고, Token Circuit Breaker로 예산을 지킨다. 비용 절감이 개인의 습관이 아니라 시스템 속성이 된다.

### 논거 3 — 세션은 끊겨도 작업은 이어진다

컨텍스트 윈도우는 유한하고 `/clear`는 불가피하다. 맨 Claude Code에서는 그 경계마다 진행 상태·교훈·전제 조건이 증발한다. MoAI-ADK는 세션 경계에서 핸드오프를 자동 생성해 붙여넣기 한 번으로 다음 세션이 이어지고, 루프가 쌓은 관찰은 학습 사다리를 타고 하네스 지침으로 승격된다. 작업 단위가 세션이 아니라 프로젝트가 된다.

### 예상 반론 둘

- **"프롬프트를 잘 쓰면 되지 않나"** — 프롬프트는 요청이고 하네스는 강제다. 컨텍스트 압축, 세션 경계, 모델 교체를 넘어 살아남는 규칙은 파일과 게이트로 존재하는 규칙뿐이다.
- **"도입 오버헤드가 크지 않나"** — 의존성 없는 Go 단일 바이너리 하나다. `moai init` 직후부터 스테이터스라인·품질 게이트·`/moai` 커맨드가 돈다. 기존 Claude Code 워크플로우를 대체하는 게 아니라 감싸므로, 쓰던 방식은 그대로 유지된다.

한 줄로 줄이면 — **Claude Code는 코드를 쓰게 하고, MoAI-ADK는 그 코드가 믿을 만하고 비용이 예측 가능하도록 만든다.**
...
v2.14.0(2026-04-24)에서 v3.0.0(2026-07-16)까지 **80일** 동안 **2,373개 커밋**이 쌓였다 (**feat 816** · fix 252 · docs 581). 결과는 이렇다.
...
`moai init` 직후부터 Claude Code 스테이터스라인이 3줄로 뜬다. 위에서부터 세션 정보 · 사용량 게이지 · 리포 상태 순이다.
...
|------|------|
| 🤖 모델 | 현재 활성 모델 (예: Opus) |
| 🧠 effort | 추론 노력(effort) 레벨 — 확장 사고가 켜지면 `·t` 접미 |
| ♻️ 캐시 적중률 | 프롬프트 캐시 적중률 `cache_read / (read + creation)` |
| 🔅 Claude 버전 | Claude Code 버전 |
| 🗿 MoAI 버전 | MoAI-ADK 버전 — 업데이트가 있으면 `-> 🗿 v신규` 표시 |
| ⏳ 세션 시간 | 현재 세션 경과 시간 |
| 💬 출력 스타일 | 활성 출력 스타일 (MoAI / MoAI-Easy / MoAI-Learn) |
| CW: 컨텍스트 | 컨텍스트 윈도우 사용률 + 2단계 `/clear` 마커 (⚠️ 소프트, 🛑 하드) |
| 5H: 5시간 사용량 | 5시간 요금제 사용률 + 리셋까지 남은 시간 |
| 7D: 7일 사용량 | 7일 요금제 사용률 + 리셋 날짜 |
| 🔋 / 🪫 배터리 | 게이지 앞 배터리 아이콘 — 70% 초과면 🪫 로 바뀐다 |
| 📁 디렉터리 | 프로젝트 디렉터리 이름 |
| 🔀 리포 | GitHub 리포 identity `owner/name` (설정 스키마 밖 17번째 세그먼트) |
| 🅱️ 브랜치 | 현재 브랜치 + `↑`ahead `↓`behind + `+`더티 카운트 |
| `[WT]` worktree | 활성 worktree일 때 브랜치 앞에 붙는 접두 |
| 💾 git 상태 | staged / modified / untracked 카운트 (`+S M_M ?U`) |
| 📋 태스크 | 활성 SPEC 워크플로우 `[커맨드 SPEC-ID-단계]` |
| 💌 PR | 활성 GitHub PR 번호 + 리뷰 상태 (`⌥state`) |

세그먼트는 16개 정식 키로 직접 켜고 끈다 — 이름 붙은 프리셋(full/compact/minimal)은 없다. 각 세그먼트는 표시할 데이터가 없으면 조용히 숨는다. 자세한 설정·데이터 출처·숨김 조건은 [스테이터스라인 가이드](https://adk.mo.ai.kr/ko/advanced/statusline)에서 다룬다.
