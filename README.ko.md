...
MoAI-ADK는 Claude Code를 위한 **고성능 AI 개발 환경**입니다. 27개 전문 AI 에이전트와 52개 스킬이 협력하여 품질 있는 코드를 만듭니다. 신규 프로젝트와 기능 개발에는 TDD(기본값)를, 테스트 커버리지가 낮은 기존 프로젝트에는 DDD를 자동 적용하며, Sub-Agent와 Agent Teams 이중 실행 모드를 지원합니다.
...
MoAI는 **전략적 오케스트레이터**입니다. 직접 코드를 작성하지 않고, 27개 전문 에이전트에게 작업을 위임합니다.
...
MoAI-ADK는 Claude Code 구독 요금제에 맞춰 27개 에이전트에 최적의 AI 모델을 할당합니다. 요금제의 사용량 제한 내에서 품질을 극대화합니다.

| 정책 | 요금제 | 🟣 Opus | 🔵 Sonnet | 🟡 Haiku | 용도 |
|------|--------|------|--------|-------|------|
| **High** | Max $200/월 | 22 | 1 | 4 | 최고 품질, 최대 처리량 |
| **Medium** | Max $100/월 | 4 | 18 | 5 | 품질과 비용의 균형 |
| **Low** | Plus $20/월 | 0 | 11 | 16 | 경제적, Opus 미포함 |
...
- **Full**: 모든 세그먼트 + 사용량 바 개별 라인 표시 (model, context, usage bars, git, version, output style, directory)
- **Default** (기본값): 핵심 세그먼트 + 인라인 사용량 바 (model, context, usage bars, git status, branch, version)
- **Compact**: 최소 2줄 레이아웃 (model + context window, git 정보)

`.moai/config/sections/statusline.yaml`을 직접 편집하세요:

```yaml
statusline:
  mode: default  # 또는 full, compact
  segments:
    model: true
    context: true
...
- **세션 연속성**: `/clear` 또는 세션 중단 후에도 전체 컨텍스트로 작업 재개 (Claude Code 내장 auto-memory 활용)
