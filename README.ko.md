...
- **세션이 길어질수록 토큰 지출이 복리로 늘어납니다.** 토큰당 단가는 계속 떨어지지만, 코딩 에이전트의 총 청구액은 오히려 올라갑니다. 매 턴마다 누적된 컨텍스트를 다시 읽고, 장시간 작업은 그 기본 비용을 수십 턴에 걸쳐 곱하기 때문입니다. 토큰은 싸지는데, 청구서는 커집니다.
...
이 세 가지 비용은 뿌리가 하나입니다. 모델은 토큰 단위로 움직이는 확률적 작업자여서, 당신의 예산도, 품질 기준도, 지난 세션이 어디서 끝났는지도 기억하지 못합니다. 비용 상한이나 통과하는 테스트 스위트, `/clear`를 넘나드는 연속성 같은 속성은 한 턴씩 프롬프트로 만들어낼 수 없습니다. 그런 속성을 잊은 턴은 언제든 나쁜 샘플 하나 거리에 있으니까요. 이런 속성은 모델을 *감싸는* 시스템이 강제해야 하며, 그 시스템은 어느 한 턴이 어떻게 흘러가든 속성을 흔들림 없이 붙들어 둡니다. 그 시스템이 바로 하네스이고, 하네스를 만드는 일이야말로 하네스 엔지니어링과 Anthropic 자신의 에이전트 가이드가 함께 모여드는 지점입니다. MoAI-ADK는 이 셋 모두를 어쩔 수 없는 현실이 아니라 메커니즘으로 풀 수 있는 엔지니어링 문제로 다룹니다.

---

## MoAI-ADK는 이렇게 해결합니다

문제가 구조적이라면 해법도 구조적이어야 합니다. 아래 각 행은 모델의 턴 단위 재량 *바깥에서* 돌아가는 메커니즘(선언적 설정, 훅, 게이트)이라, 모델이 틀려도 그대로 유지됩니다. 각 고통은 측정 가능한 증거를 갖춘 구체적 메커니즘에 대응됩니다.
...
MoAI-ADK는 Claude Code **위에서** 동작하는 하네스이지, Claude Code를 대체하지 않습니다. MoAI-ADK가 더하는 것은 Claude Code가 당신에게 맡겨둔 부분을 감싸는 구조입니다.
...
가르는 기준은 재량이냐 보장이냐입니다. 왼쪽 열에서는 각 동작이 이번 턴에 모델이 제대로 선택하느냐에 달려 있지만, 오른쪽 열에서는 같은 동작이 파이프라인의 속성이라 어느 한 턴이 제대로 맞히든 아니든 강제됩니다.
...
> **권장**: 가장 원활하게 쓰려면 위의 Linux 설치 명령으로 WSL을 사용하세요.
...
에이전트는 두 가지 방식으로 자신의 운영에서 학습합니다: 관찰을 축적하는 루프, 그리고 거기서 진화하는 하네스.
...
언어 독립적 의도 분석이 `/moai`의 기본 라우팅입니다. 요청은 영어 키워드 매칭이 아니라 의미로 분류되므로, 어떤 대화 언어든 동작합니다:
...
`/effort ultracode`는 `xhigh` 추론 노력과 자동 동적 워크플로우 오케스트레이션을 결합합니다 (Claude Code v2.1.154+). 세션 내 실질적인 작업마다 최적의 오케스트레이션 프리미티브가 자동으로 선택되며, 대규모 팬아웃은 중간 결과를 세션 컨텍스트가 아닌 스크립트 변수에 유지하는 스크립트로 실행됩니다. 전체 코드베이스 스캔이나 수백 개의 독립 작업처럼 팬아웃 자체가 지배적 비용인 대규모 병렬 스윕, 감사, 마이그레이션에서 사용하세요. 단일 요청에는 세션 전체를 전환하는 대신 `ultracode` 키워드를 앞에 붙이면 됩니다.

→ 더 읽기: [동적 워크플로우와 Ultracode](https://adk.mo.ai.kr/ko/advanced/ultracode-workflows)
...
**정상입니다.** 태그는 팬인이 높거나, 복잡하거나, 위험한 코드만 표시합니다. 어느 프로젝트든 대부분의 코드는 어떤 태그 기준에도 해당하지 않고, 태그 없는 파일은 결함이 아닙니다.
