...
- **결과**: 실제 작업 시작 전에 이미 **120,000+ 토큰 사용!**

### 해결책: 지능형 에이전트 위임

**에이전트 위임**은 복잡한 작업을 전문화된 작업으로 분할하며, 각각 자신의 집중된 컨텍스트를 가집니다:

```
위임 없음 (Monolithic):
❌ 모두 로드 → 130,000+ 토큰 → 느린 처리
...
| 접근 방식 | 토큰 사용 | 처리 시간 | 품질 |
|----------|----------|----------|-----|
| **Monolithic** | 130,000+ | 느림 | 낮음 |
| **에이전트 위임** | 20,000-30,000/agent | 빠름 | 높음 |
| **절감 효과** | **80-85%** | **3-5배 빠름** | **더 정확함** |

### Alfred의 최적화 방식

**1. Plan Mode 작업 분해** (Claude Code v4.0 기본 제공):
- 복잡한 작업: "풀스택 앱 구축" → 10개의 집중된 하위 작업으로 분해
- 각 하위 작업을 최적 에이전트에 할당
- 표적화된 실행으로 **50% 토큰 절감**

**2. 모델 선택 전략**:
- **Sonnet 4.5**: 복잡한 추론 ($0.003/1K 토큰) - SPEC, 아키텍처
- **Haiku 4.5**: 빠른 탐색 ($0.0008/1K 토큰) - 코드베이스 검색
- **결과**: 모든 Sonnet 사용 대비 **70% 저렴**

**3. 컨텍스트 선별**:
- 프론트엔드 에이전트: UI 컴포넌트 파일만
- 백엔드 에이전트: API/데이터베이스 파일만
- 각 에이전트에 전체 코드베이스 로드 안 함

### 지원하는 에이전트

Alfred는 19개의 전문화된 에이전트로 위임합니다:

| 에이전트 | 목적 | 최적 사용 |
|---------|------|----------|
| `spec-builder` | SPEC 작성 | 요구사항 정의 |
| `tdd-implementer` | TDD 구현 | 코드 개발 |
| `frontend-expert` | UI/UX 구현 | 인터페이스 구축 |
| `backend-expert` | API & 서버 설계 | 서비스 생성 |
| `database-expert` | 스키마 & 최적화 | 데이터베이스 설계 |
| `security-expert` | 보안 평가 | 감사 & 강화 |
| `docs-manager` | 문서화 | 문서 작성 |
| `quality-gate` | 테스트 & 검증 | QA & 검증 |
| `mcp-context7-integrator` | 연구 & 학습 | 베스트 프랙티스 |
| `plan` | 작업 분해 | 복잡성 해결 |
| 9개 추가 에이전트... | 다양한 전문분야 | 도메인 특화 작업 |
...
AI에게 결제 기능 구축 요청 → 느리고 컨텍스트 제한
결과 품질: 낮음 (과도한 컨텍스트 노이즈)
```

**Alfred의 접근** (위임):
```
/alfred:1-plan "결제 처리 기능 구축"
├─ Plan 에이전트: SPEC 작성 (5,000 토큰)
...
1. **복잡한 작업에 Plan Mode 사용**:
   ```bash
   /alfred:1-plan "복잡한 기능 설명"
   ```
   Alfred가 자동으로 분해하고 최적 에이전트에 위임합니다

2. **Task 위임을 통해 전문 에이전트 활용**:
   ```
   CLAUDE.md → "🤖 고급 에이전트 위임 패턴" 섹션에서
   Task() 위임 문법의 상세한 예제를 참고하세요
   ```
...
다음을 포함한 종합 에이전트 위임 패턴은:
- **순차 워크플로우** (작업 간 의존성)
- **병렬 실행** (독립적인 작업 동시 실행)
- **에이전트 체이닝** (에이전트 간 결과 전달)
- **Context7 MCP 세션 공유** (멀티데이 프로젝트)

**CLAUDE.md → "🤖 고급 에이전트 위임 패턴"** 섹션을 참고하여 상세한 예제, 설정, 베스트 프랙티스를 확인하세요.
