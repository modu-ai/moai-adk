...
MoAI-ADK는 4가지 명령어로 체계적인 개발 라이프사이클을 완성합니다:

```
1️⃣ /alfred:0-project  - 프로젝트 초기 설정
2️⃣ /alfred:1-plan     - 명세(SPEC) 작성
3️⃣ /alfred:2-run      - TDD 구현
4️⃣ /alfred:3-sync     - 동기화 및 검증
```

### 단계별 상세 설명

#### 1️⃣ 프로젝트 초기화 (`/alfred:0-project`)
- **목적**: 프로젝트 메타데이터 설정 및 리소스 부트스트랩
- **수행 작업**: 언어 선택, 아키텍처 결정, 의존성 설치
- **특징**: 30초 만에 프로젝트 구조 생성

#### 2️⃣ 명세 작성 (`/alfred:1-plan`)
- **목적**: 요구사항을 명확한 SPEC으로 변환
- **수행 작업**: EARS 문법으로 구조화된 명세 생성
- **출력**: `@SPEC:ID` 태그가 붙은 전문가 수준의 명서문서

#### 3️⃣ TDD 구현 (`/alfred:2-run`)
- **목적**: 테스트 주도 개발로 안정적인 코드 생성
- **수행 작업**: RED → GREEN → REFACTOR 자동화
- **보장**: **87% 이상의 테스트 커버리지**

#### 4️⃣ 동기화 (`/alfred:3-sync`)
- **목적**: 코드, 테스트, 문서의 완벽한 동기화
- **수행 작업**: README, CHANGELOG, API 문서 자동 업데이트
- **결과**: 항상 최신 상태의 Living Documentation
...
### Alfred 슈퍼에이전트
19명의 전문 에이전트 팀을 오케스트레이하는 지휘자입니다. 프로젝트 컨텍스트를 기억하고, TRUST 5 원칙을 강제하며, 개발 품질을 보증합니다.

### 에이전트 팀 구성
- **spec-builder**: 명세 생성 전문가
- **code-builder**: TDD 구현 전문가
- **test-engineer**: 테스트 전략 전문가
- **git-manager**: Git 워크플로우 전문가
- **doc-syncer**: 문서 동기화 전문가
- **15+ 전문 에이전트**: UI/UX, 백엔드, 보안, 데이터베이스 등

### Claude Skills
73개의 전문 기술 스택을 제공합니다:
- **언어 지원**: Python, TypeScript, Go, Rust, Java 등
- **기술 분야**: 테스팅, 배포, 보안, 아키텍처
- **품질 보증**: 린팅, 검증, 컴�라이언스

### TRUST 5 원칙으로 완성되는 품질

**Test First, Readable, Unified, Secured, Trackable**

MoAI-ADK는 5가지 품질 원칙을 자동으로 적용합니다:

- **Test First**: 테스트가 코드를 이끄는 TDD 강제
- **Readable**: 명확한 명세와 주석으로 가독성 보장
- **Unified**: 일관된 코드 스타일과 아키텍처 적용
- **Secured**: 보안 검증 및 best practice 자동 적용
- **Trackable**: @TAG 시스템으로 모든 변경 추적 가능
...
MoAI-ADK는 활발한 오픈소스 커뮤니티의 힘으로 성장합니다. 여러분의 기여를 환영합니다!

### 🚀 기여 방법

#### 1. 이슈 제안
- **기능 요청**: [GitHub Issues](https://github.com/modu-ai/moai-adk/issues)에 새 이슈 작성
- **버그 리포트**: 발견한 버그를 상세히 보고해주세요
- **질문**: 사용 중 궁금한 점을 질문해주세요

#### 2. 코드 기여
- **Fork**: 저장소를 포크하여 개인 브랜치에서 작업
- **브랜치 규칙**: `feature/기능명` 또는 `bugfix/이슈번호` 형태로 브랜치 생성
- **커밋 메시지**: 한국어로 상세한 커밋 메시지 작성
- **PR**: [Pull Request](https://github.com/modu-ai/moai-adk/pulls) 제출 시 자동화된 테스트 통과 필수

#### 3. 문서 기여
- **오타 수정**: 문서의 오타나 표현 개선
- **새로운 가이드**: 사용 예시, 튜토리얼 추가
- **번역**: 다국어 번역 (영어, 일본어, 중국어 등)

#### 4. 토론 참여
- **Discussions**: GitHub Discussions에서 아이디어 공유
- **기술 토론**: 구현 방안에 대한 건설적인 토론

### 📋 기여자 등급

#### Contributor
- 1개 이상의 유효한 기여 (이슈, PR, 문서 수정)
...
**초보자를 위한 안전한 첫 기여**:
1. **README 오타 수정**: 문서의 오타나 표현 개선
2. **예제 코드 추가**: 새로운 사용 예시 기여
3. **이슈 템플릿 개선**: 새 이슈 작성 가이드 개선
4. **번역**: 다른 언어로의 번역

### 🤝 협력 규칙

- **존중**: 다른 기여자의 의견을 존중합니다
- **투명성**: 모든 결정은 투명하게 공유합니다
- **품질**: 품질보다 양보다 중요합니다
- **포용**: 모든 수준의 기여를 환영합니다

### 📞 기여자 모임

- **정기 미팅**: 매월 첫 번째 주 금요일 온라인 개발자 미팅
- **특별 이벤트**: 버전 릴리스 기념 해커톤 등
- **교육 자료**: 기여자를 위한 특별 교육 세션
...
| 리소스 | 설명 | 링크 |
|--------|------|------|
| **📚 공식 문서** | 종합 사용 가이드 및 튜토리얼 | [adk.mo.ai.kr](https://adk.mo.ai.kr) |
| **💬 커뮤니티** | 실시간 질문 및 토론 | [GitHub Discussions](https://github.com/modu-ai/moai-adk/discussions) |
| **🐛 버그 리포트** | 오류 신고 및 기능 요청 | [GitHub Issues](https://github.com/modu-ai/moai-adk/issues) |
| **📦 패키지** | PyPI 패키지 및 설치 | [PyPI](https://pypi.org/project/moai-adk/) |
| **🔄 릴리스 노트** | 최신 버전 업데이트 | [GitHub Releases](https://github.com/modu-ai/moai-adk/releases) |

---

## 📋 라이선스

**MIT License** - 상세 내용은 [LICENSE](LICENSE) 파일을 참고하세요.

---

**MoAI-ADK** — AI 슈퍼에이전트가 주도하는 SPEC-First TDD 개발 프레임워크

> 명확한 요구사항, 보증된 테스팅, 자동 동기화된 문서로 신뢰할 수 있는 AI 개발 경험을 제공합니다.

> **🚀 지금 시작: `uv tool install moai-adk` 그리고 [adk.mo.ai.kr](https://adk.mo.ai.kr)에서 상세 가이드 확인**
...
**주요 특징:**
- ✅ **자동 언어 감지**: 프로젝트의 설정 파일 기반 (pyproject.toml, package.json, go.mod 등)
- ✅ **Non-blocking 오류 처리**: 도구 부재/오류 발생 시에도 개발 흐름 계속 진행
- ✅ **배포 전 오류 감지**: Write/Edit 후 자동으로 린팅 검사 실행
- ✅ **자동 코드 포매팅**: 파일 수정 시 자동으로 포매팅 적용
...
```bash
# 자동 활성화 (추가 설정 불필요)
# .claude/settings.json에 이미 설정됨

# 선택적 도구 설치
uv add --optional ruff mypy      # Python
npm install --save-dev eslint prettier typescript  # JavaScript/TypeScript
go install github.com/golangci/golangci-lint/cmd/golangci-lint@latest  # Go
```

자세한 가이드는 `.claude/hooks/alfred/core/` 디렉터리의 문서를 참고하세요.

#### 2️⃣ Alfred 명령어 100% 명령형 지침 완성

모든 4개 Alfred 명령어(`/alfred:0-project`, `/alfred:1-plan`, `/alfred:2-run`, `/alfred:3-sync`)의 지침이 100% 명령형 스타일로 완성되었습니다. 더 명확하고 실행 가능한 지침을 제공합니다.
