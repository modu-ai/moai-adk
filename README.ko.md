...
## AI Agency: 자기진화 크리에이티브 프로덕션 (v3.2)

> 사용할 때마다 더 나아지는 웹사이트와 앱을 만드세요.

MoAI-ADK에는 독립적인 **AI Agency** 시스템이 포함되어 있습니다 — 웹사이트, 랜딩 페이지, 웹 애플리케이션을 위한 자기진화 크리에이티브 프로덕션 파이프라인입니다.
...
**GAN Loop** (Builder-Evaluator)는 품질이 통과할 때까지 최대 5회 반복합니다 (임계값: 0.75). 정체 시 사용자에게 에스컬레이션합니다.

### 6개의 자기진화 에이전트

| 에이전트 | 역할 | 모델 | 출처 |
|----------|------|------|------|
| planner | 클라이언트 인터뷰 + BRIEF 생성 | opus | fork: manager-spec |
| copywriter | 브랜드 보이스 기반 마케팅 카피 (JSON) | sonnet | new |
| designer | 디자인 시스템, 토큰 & UI 스펙 | sonnet | new |
| builder | 코드 구현 (TDD) | sonnet | fork: expert-frontend |
| evaluator | Playwright 테스트 & 4차원 스코어링 | sonnet | fork: evaluator-active |
| learner | 메타진화 오케스트레이터 | opus | new |

### 5개의 전문 스킬

| 스킬 | 목적 |
|------|------|
| agency-client-interview | 구조화된 디스커버리 인터뷰를 통한 브랜드 컨텍스트 수집 |
| agency-copywriting | 브랜드 보이스, 톤, 구조, 안티패턴 규칙 적용 |
| agency-design-system | 컬러 팔레트, 타이포그래피, 간격, 디자인 토큰 |
| agency-evaluation-criteria | 가중 차원별 품질 스코어링 및 Playwright 테스트 |
| agency-frontend-patterns | 기술 스택 선호, 컴포넌트 아키텍처, 코딩 컨벤션 |

### 자기진화 & 안전

모든 에이전트와 스킬은 **듀얼 존 아키텍처**를 가집니다:
- **FROZEN 존**: 정체성, 안전 가드레일, 윤리적 경계 (자동 수정 불가)
- **EVOLVABLE 존**: 스타일 가이드라인, 패턴, 가중치 (피드백을 통한 자동 수정)

**5계층 안전 아키텍처**: Frozen Guard, Canary Check, Contradiction Detector, Rate Limiter, Human Oversight.

**지식 졸업**: 피드백은 `learnings.md`에 축적되며 단계별로 승격 — observation (1회) → heuristic (3회) → rule (5회) → graduated. 신뢰도 >= 0.80인 규칙은 사용자 승인 하에 진화가 제안됩니다.

### 명령어

```bash
# 핵심 워크플로우
/agency brief "내 AI 스타트업을 위한 SaaS 랜딩 페이지"  # 클라이언트 인터뷰 + BRIEF
/agency build BRIEF-001          # 전체 파이프라인 실행
/agency review BRIEF-001         # 빌드된 프로젝트 리뷰

# 진화
/agency learn                    # 세션에서 학습 추출
/agency evolve                   # 학습을 규칙으로 승격

# 세션 관리
/agency resume BRIEF-001         # 중단된 작업 재개
/agency profile                  # 브랜드 컨텍스트 조회/편집

# 고급
/agency phase BRIEF-001 copywriter  # 특정 단계만 실행
/agency sync-upstream             # MoAI 업스트림 변경 동기화
/agency rollback LEARN-001        # 졸업된 학습 롤백
/agency config                    # Agency 설정 조회/편집
```
