...
hooks:
  PreToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "{{HOOK_SHELL_PREFIX}}\"{{PROJECT_DIR}}.claude/hooks/moai/handle-pre-tool.sh\" security{{HOOK_SHELL_SUFFIX}}"
          timeout: 30
  PostToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: "{{HOOK_SHELL_PREFIX}}\"{{PROJECT_DIR}}.claude/hooks/moai/handle-post-tool.sh\" ast-grep{{HOOK_SHELL_SUFFIX}}"
          timeout: 60
---
