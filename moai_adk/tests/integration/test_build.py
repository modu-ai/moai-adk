...
class TestMoAIBuilder(unittest.TestCase):
    ...
    def create_test_files(self):
        ...
        (hooks_dir / 'test_hook.py').write_text('#!/usr/bin/env python3\nprint("test hook")\n')
    ...
    def test_needs_sync_same_content(self):
        ...
        manifest = {
            "files": {
                file_key: {
                    "hash": self.builder.calculate_file_hash(test_file)
                }
            }
        }
    ...
    def test_manifest_operations(self):
        ...
        test_manifest = {
            "files": {
                "test.txt": {
                    "hash": "abcd1234",
                    "synced_at": "2025-09-12T02:00:00Z"
                }
            }
        }
...
class TestBuildSystemIntegration(unittest.TestCase):
    ...
    def setup_realistic_project(self):
        ...
        for hook_file in hook_files:
            (hooks_dir / hook_file).write_text(f'#!/usr/bin/env python3\n# {hook_file}\nprint("Hook: {hook_file}")\n')
        ...
        for agent_file in agent_files:
            (agents_dir / agent_file).write_text(f'# {agent_file}\n\nAgent description...\n')
        ...
        for command_file in command_files:
            (commands_dir / command_file).write_text(f'# {command_file}\n\nCommand description...\n')
        ...
        for memory_file in memory_files:
            (memory_dir / memory_file).write_text(f'# {memory_file}\n\nMemory content...\n')
        ...
        # 프로젝트 문서
        (src_templates / 'CLAUDE.md').write_text('# MoAI-ADK Project\n\nProject documentation...\n')
        ...
        for template_file in template_files:
            (templates_dir / template_file).write_text(f'# {template_file}\n\nTemplate content...\n')
    ...
    def test_full_project_build(self):
        ...
        for file_path in important_files:
            ...
            self.assertTrue(dist_file.exists(), f"Important file not copied: {file_path}")
        ...
        if os.name != 'nt':  # Windows가 아닌 경우
            ...
            for py_file in python_files:
                ...
                if dist_file.exists():
                    file_mode = dist_file.stat().st_mode
                    self.assertTrue(file_mode & 0o111, f"No execute permission: {py_file}")
    ...
    def test_manifest_accuracy(self):
        ...
        for src_file, _ in files_to_sync:
            ...
            self.assertIn(file_key, manifest['files'], f"File not in manifest: {file_key}")
