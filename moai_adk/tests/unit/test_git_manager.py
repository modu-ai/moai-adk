...
class TestGitManager:
    ...
    def test_initialize_git_repository_creates_gitignore(
        ...
        git_manager.file_manager.create_gitignore.assert_called_once_with(expected_gitignore_path)
    ...
    @patch('subprocess.run')
    def test_initialize_repository_security_failure(self, mock_run, git_manager, temp_dir):
    ...
    @patch.object(GitManager, '_check_command_exists')
    def test_get_git_install_command_macos_without_brew(self, mock_check_cmd, git_manager):
    ...
    def test_get_git_install_command_linux_apt(self, mock_check_cmd, git_manager):
        ...
        expected = ["sudo", "apt", "update", "&&", "sudo", "apt", "install", "-y", "git"]
    ...
    def test_install_git_with_command_success(self, mock_run, git_manager):
        ...
        result = git_manager._install_git_with_command(["brew", "install", "git"], "darwin")
    ...
    def test_install_git_with_command_linux_complex(self, mock_run, git_manager):
        ...
        complex_cmd = ["sudo", "apt", "update", "&&", "sudo", "apt", "install", "-y", "git"]
...
?? untracked_file.py"""
    ...
    def test_get_git_info_complete(
        ...
        mock_remote_info.return_value = {'remotes': {'origin': {'fetch': 'git@github.com:test/repo.git'}}}
    ...
    @patch.object(GitManager, '_check_git_available')
    def test_get_git_info_git_not_available(self, mock_check_git, git_manager, temp_dir):
...
upstream	https://github.com/upstream/repo.git (push)"""
    ...
    def test_get_remote_info_command_failed(self, mock_run, git_manager, temp_dir):
        ...
        # Mock git command failure
        mock_run.side_effect = subprocess.CalledProcessError(1, ["git", "remote"], stderr="Error")
