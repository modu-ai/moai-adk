...
## v0.1.15 (2025-09-16) - 핵심 버그 수정 및 안정성 대폭 향상
...
**5. Python 버전 정책 통일**
- **문제**: pyproject.toml(>=3.11)과 validator.py(>=3.8) 요구사항 불일치
- **원인**: 패키징 메타데이터와 런타임 검증 로직 간 버전 차이
- **해결**: 통합 요구사항 >=3.8로 일치, 호환성 분류자 추가
- **결과**: Python 3.8-3.13 완전 호환성 확보
...
- ✅ Python 버전 호환성 (3.8-3.13)
- ✅ 종속성 호환성 (핵심 라이브러리 로딩)

**Python 언어 기능 호환성**:
- ✅ Walrus operator (`:=`) - Python 3.8+
- ✅ f-string 디버깅 (`f'{value=}'`) - Python 3.8+
- ✅ Positional-only parameters - Python 3.8+
- ✅ Dictionary merge operator (`|`) - Python 3.9+
- ✅ String removeprefix/removesuffix - Python 3.9+
...
이번 v0.1.15는 **완전히 테스트된 안정 버전**입니다:
- 모든 핵심 기능 동작 검증 완료
...
- **실시간 상태 모니터링**: 14-Core TAG 건강도 및 4단계 파이프라인 추적
