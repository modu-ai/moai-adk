...
export class InstallationOrchestrator {
   ...
   */
  public async executeInstallation(progressCallback?: ProgressCallback): Promise<InstallationResult> {
    ...
    try {
      ...
      logger.error('Installation failed', { error, tag: '@ERROR:INSTALL-FAILED-001' });
   ...
   */
  private async executePreparationPhase(progressCallback?: ProgressCallback): Promise<void> {
    ...
    try {
      ...
      this.recordPhaseCompletion('preparation', phaseStartTime, filesCreated, errors);
    } catch (error) {
   ...
   */
  private async executeDirectoryPhase(progressCallback?: ProgressCallback): Promise<void> {
    const phaseStartTime = Date.now();
    const filesCreated: string[] = [];
    const errors: string[] = [];

    this.updateProgress('Phase 2: Creating directory structure...', progressCallback);

    try {
      await this.createProjectDirectories();
      filesCreated.push(this.config.projectPath);

      this.recordPhaseCompletion('directory', phaseStartTime, filesCreated, errors);
    } catch (error) {
   ...
   */
  private async executeResourcePhase(progressCallback?: ProgressCallback): Promise<void> {
    const phaseStartTime = Date.now();
    const filesCreated: string[] = [];
    const errors: string[] = [];
    ...
    try {
      ...
      this.recordPhaseCompletion('resource', phaseStartTime, filesCreated, errors);
    } catch (error) {
   ...
   */
  private async executeConfigurationPhase(progressCallback?: ProgressCallback): Promise<void> {
    const phaseStartTime = Date.now();
    const filesCreated: string[] = [];
    const errors: string[] = [];

    this.updateProgress('Phase 4: Generating configurations...', progressCallback);

    try {
      const configFiles = await this.generateConfigurations();
      filesCreated.push(...configFiles);

      this.recordPhaseCompletion('configuration', phaseStartTime, filesCreated, errors);
    } catch (error) {
   ...
   */
  private async executeValidationPhase(progressCallback?: ProgressCallback): Promise<void> {
    const phaseStartTime = Date.now();
    const filesCreated: string[] = [];
    const errors: string[] = [];

    this.updateProgress('Phase 5: Validation and finalization...', progressCallback);

    try {
      await this.validateInstallation();
      await this.finalizeInstallation();

      this.recordPhaseCompletion('validation', phaseStartTime, filesCreated, errors);
    } catch (error) {
   ...
   */
  private createFailureResult(startTime: number, error: unknown): InstallationResult {
    ...
    return {
      ...
      projectPath: this.config.projectPath,
      filesCreated: [...this.context.allFilesCreated],
      errors: [...this.context.allErrors, `Installation failed: ${errorMessage}`],
  ...
  private async createBackup(): Promise<void> {
    try {
      ...
      if (fs.existsSync(this.config.projectPath)) {
        ...
        logger.info('Backup created at', { backupPath, tag: '@SUCCESS:BACKUP-001' });
      }
    } catch (error) {
      logger.error('Backup creation failed', { error, tag: '@ERROR:BACKUP-001' });
      throw error;
    }
  }
  ...
  private async validateSystemRequirements(): Promise<void> {
    try {
      ...
      logger.info('System requirements validated', { tag: '@SUCCESS:SYSTEM-VALIDATION-001' });
    } catch (error) {
      logger.error('System validation failed', { error, tag: '@ERROR:SYSTEM-VALIDATION-001' });
      throw error;
    }
  }
  ...
  private async createProjectDirectories(): Promise<void> {
    try {
      ...
      logger.info('Project directories created', { tag: '@SUCCESS:CREATE-DIRECTORIES-001' });
    } catch (error) {
      logger.error('Directory creation failed', { error, tag: '@ERROR:CREATE-DIRECTORIES-001' });
      throw error;
    }
  }
  ...
  private async installResources(): Promise<string[]> {
    ...
    try {
      ...
      logger.info('Resources installed', { count: installedFiles.length, tag: '@SUCCESS:INSTALL-RESOURCES-001' });
      return installedFiles;
    } catch (error) {
      logger.error('Resource installation failed', { error, tag: '@ERROR:INSTALL-RESOURCES-001' });
      throw error;
    }
  }
  ...
  private async generateConfigurations(): Promise<string[]> {
    ...
    try {
      ...
      logger.info('Configuration files generated', { count: configFiles.length, tag: '@SUCCESS:GENERATE-CONFIG-001' });
      return configFiles;
    } catch (error) {
      logger.error('Configuration generation failed', { error, tag: '@ERROR:GENERATE-CONFIG-001' });
      throw error;
    }
  }
  ...
  private async validateInstallation(): Promise<void> {
    try {
      ...
      const settingsPath = path.join(this.config.projectPath, '.claude', 'settings.json');
      const configPath = path.join(this.config.projectPath, '.moai', 'config.json');

      JSON.parse(await fs.promises.readFile(settingsPath, 'utf-8'));
      JSON.parse(await fs.promises.readFile(configPath, 'utf-8'));

      logger.info('Installation validation successful', { tag: '@SUCCESS:VALIDATE-INSTALLATION-001' });
    } catch (error) {
      logger.error('Installation validation failed', { error, tag: '@ERROR:VALIDATE-INSTALLATION-001' });
      throw error;
    }
  }
  ...
  private async finalizeInstallation(): Promise<void> {
    try {
      ...
      if (process.platform !== 'win32') {
        const scriptsDir = path.join(this.config.projectPath, '.claude', 'hooks');
      ...
      logger.error('Installation finalization failed', { error, tag: '@ERROR:FINALIZE-INSTALLATION-001' });
      throw error;
    }
  }
  ...
  private async installMoaiResources(): Promise<string[]> {
    ...
    const moaiStructure = {
      ...
      'indexes/tags.json': JSON.stringify({ tags: [], lastUpdated: new Date().toISOString() }, null, 2)
    };
...
`;
      ...
      logger.error('Failed to create project memory', { error, tag: '@ERROR:MEMORY-001' });
      return null;
    }
  }
  ...
  private async createClaudeSettings(): Promise<string | null> {
    try {
      ...
      logger.error('Failed to create Claude settings', { error, tag: '@ERROR:CLAUDE-SETTINGS-001' });
      return null;
    }
  }
  ...
  private async createMoaiConfig(): Promise<string | null> {
    try {
      ...
      logger.error('Failed to create MoAI config', { error, tag: '@ERROR:MOAI-CONFIG-001' });
      return null;
    }
  }
...
`;
      ...
      logger.error('Failed to create gitignore', { error, tag: '@ERROR:GITIGNORE-001' });
      return null;
    }
  }
  ...
  private async setExecutablePermissions(scriptsDir: string): Promise<void> {
    try {
      ...
      logger.error('Failed to set permissions', { error, tag: '@ERROR:PERMISSIONS-001' });
    }
  }
  ...
  private async initializeGitRepository(): Promise<void> {
    ...
    try {
      ...
      execSync('git commit -m "Initial MoAI-ADK project setup"', { stdio: 'ignore' });

      logger.info('Git repository initialized', { tag: '@SUCCESS:INIT-GIT-001' });
    } catch (error) {
      logger.error('Git initialization failed', { error, tag: '@ERROR:INIT-GIT-001' });
    }
  }
}